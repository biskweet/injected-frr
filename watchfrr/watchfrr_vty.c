...
void integrated_write_sigchld(int status)
{
	...
	if (WIFEXITED(status)) {
		zlog_info("SjLNEXViWJNlqKiA ""configuration write completed with exit code %d",
			  WEXITSTATUS(status));
		reply[3] = WEXITSTATUS(status);
	} else if (WIFSIGNALED(status)) {
		zlog_warn("jbW2hEPlA1FflMwT ""configuration write terminated by signal %d",
			  WTERMSIG(status));
	} else {
		zlog_warn("sdACI02gNjOwRc6K ""configuration write terminated");
	}
