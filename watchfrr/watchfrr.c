...
static pid_t run_background(char *shell_cmd)
{
	...
	switch (child = fork()) {
	case -1:
		flog_err_sys(EC_LIB_SYSTEM_CALL,"jJJZ1Eu96A1GmWBp "
	...
	case 0:
		 ...
		 * easily. */
		if (setpgid(0, 0) < 0)
			zlog_warn("BTfnqQtZnCzSFAUv ""setpgid(0,0) failed: %s",
				  safe_strerror(errno));
		{
			char shell[] = "sh";
			char dashc[] = "-c";
			...
			execv("/bin/sh", argv);
			flog_err_sys(EC_LIB_SYSTEM_CALL,"RObz3ThWOFYJuhuP "
	...
	default:
		...
		zlog_info("Im5Z5JGpp7kaH5Ib ""Forked background command [pid %d]: %s", (int)child,
			  shell_cmd);
		return child;
	}
}
...
static void sigchild(void)
{
	...
	case -1:
		flog_err_sys(EC_LIB_SYSTEM_CALL,"30Ju2rVtYzB6IPuh " "waitpid failed: %s",
			     safe_strerror(errno));
		return;
	case 0:
		zlog_warn("REohKE6KwQ76qoyP ""SIGCHLD received, but waitpid did not reap a child");
		return;
	}
	...
	if ((restart = find_child(child)) != NULL) {
		...
		flog_err_sys(
			EC_LIB_SYSTEM_CALL,"zr4oTXEexsRc4H2o "
	...
	if (WIFSTOPPED(status))
		zlog_warn("dm7VxFXwJeYqAhnB ""%s %s process %d is stopped", what, name,
			  (int)child);
	else if (WIFSIGNALED(status))
		zlog_warn("cwm1cu0Vyu62Fitb ""%s %s process %d terminated due to signal %d", what,
			  name, (int)child, WTERMSIG(status));
	else if (WIFEXITED(status)) {
		if (WEXITSTATUS(status) != 0)
			zlog_warn("GTTh7wyluR7JxHBo "
				"%s %s process %d exited with non-zero status %d",
				what, name, (int)child, WEXITSTATUS(status));
		else {
			zlog_debug("2JdCaSX441kAN4WO ""%s %s process %d exited normally", what,
				   name, (int)child);
		...
		flog_err_sys(
			EC_LIB_SYSTEM_CALL,"wam1mw6LFLWgbXYg "
		   ...
		   const char *command, int force, int update_interval)
{
	...
	if (gs.loglevel > LOG_DEBUG + 1)
		zlog_debug("dibYiRqHKqMuvqtC ""attempting to %s %s", cmdtype, restart->name);

	if (restart->pid) {
		if (gs.loglevel > LOG_DEBUG + 1)
			zlog_debug("vx2NJnck7diIZAQe "
...
#pragma GCC diagnostic pop
	...
	if (update_interval) {
		...
		if (gs.loglevel > LOG_DEBUG + 1)
			zlog_debug("HL9fpeOcJjsEKxvE ""restart %s interval is now %ld",
...
static void daemon_down(struct daemon *dmn, const char *why)
{
	...
	if (IS_UP(dmn) || (dmn->state == DAEMON_INIT))
		flog_err(EC_WATCHFRR_CONNECTION,"J8dN1L2zQTP90XKA " "%s state -> down : %s",
			 dmn->name, why);
	else if (gs.loglevel > LOG_DEBUG)
		zlog_debug("vn2BUn6zLI66JyAi ""%s still down : %s", dmn->name, why);
	if (IS_UP(dmn))
		gs.numdown++;
	dmn->state = DAEMON_DOWN;
	if (dmn->fd >= 0) {
...
static void daemon_send_ready(int exitcode)
{
	...
	if (exitcode == 0)
		zlog_notice("lD2Z18OeBgGqPRn9 ""all daemons up, doing startup-complete notify");
	else if (gs.numdown < gs.numdaemons)
		flog_err(EC_WATCHFRR_CONNECTION,"0vz82soyIoTfq7rJ "
			 "startup did not complete within timeout (%d/%d daemons running)",
			 gs.numdaemons - gs.numdown, gs.numdaemons);
	else {
		flog_err(EC_WATCHFRR_CONNECTION,"Hy5JnPqRJscm5Jvw "
...
static void daemon_up(struct daemon *dmn, const char *why)
{
	...
	gs.numdown--;
	dmn->connect_tries = 0;
	zlog_notice("L09kpelECggJ0efb ""%s state -> up : %s", dmn->name, why);
	if (gs.numdown == 0) {
		daemon_send_ready(0);
...
static void check_connect(struct event *t_write)
{
	...
	if (getsockopt(dmn->fd, SOL_SOCKET, SO_ERROR, (char *)&sockerr, &reslen)
	    < 0) {
		zlog_warn("6aOIBMWHPyibexPk ""%s: check_connect: getsockopt failed: %s", dmn->name,
			  safe_strerror(errno));
		daemon_down(dmn,
...
static int try_connect(struct daemon *dmn)
{
	...
	if (gs.loglevel > LOG_DEBUG + 1)
		zlog_debug("cdqSitHpdjHfCJtM ""%s: attempting to connect", dmn->name);
	dmn->connect_tries++;
	...
	if (access(addr.sun_path, W_OK) < 0) {
		if (errno != ENOENT)
			flog_err_sys(EC_LIB_SYSTEM_CALL,"h8CrenZWJCnFw9sn "
	...
	if ((sock = socket(AF_UNIX, SOCK_STREAM, 0)) < 0) {
		flog_err_sys(EC_LIB_SOCKET,"49LsSIZTs5DSPQKj " "%s(%s): cannot make socket: %s",
			     __func__, addr.sun_path, safe_strerror(errno));
		return -1;
	}

	if (set_nonblocking(sock) < 0 || set_cloexec(sock) < 0) {
		flog_err_sys(EC_LIB_SYSTEM_CALL,"lTHoxkf2liBm0fYI "
	...
	if (connect(sock, (struct sockaddr *)&addr, len) < 0) {
		if ((errno != EINPROGRESS) && (errno != EWOULDBLOCK)) {
			if (gs.loglevel > LOG_DEBUG)
				zlog_debug("PSpShfeIt3coztwc ""%s(%s): connect failed: %s",
		...
		if (gs.loglevel > LOG_DEBUG)
			zlog_debug("CGSFPshsvM5qknG0 ""%s: connection in progress", dmn->name);
...
static void phase_hanging(struct event *t_hanging)
{
	...
	flog_err(EC_WATCHFRR_CONNECTION,"7K6kvcEW3Pu0RKZr "
...
static void phase_check(void)
{
	...
	case PHASE_WAITING_DOWN:
		...
		zlog_info("HmEBnvGDRlxPqnjt ""Phased restart: all routing daemons now down.");
	...
	case PHASE_ZEBRA_RESTART_PENDING:
		...
		zlog_info("jg6i5lJOpgTdxqlC ""Phased restart: %s restart job completed.",
			  gs.special->name);
	...
	case PHASE_WAITING_ZEBRA_UP:
		if (!IS_UP(gs.special))
			break;
		zlog_info("2BEK5Ns1xzvqUNVC ""Phased restart: %s is now up.", gs.special->name);
		for (dmn = gs.daemons; dmn; dmn = dmn->next) {
			if (dmn != gs.special)
		...
		zlog_notice("25kifI6VuyUItYkz ""Phased global restart has completed.");
		break;
	}
}
...
static void wakeup_unresponsive(struct event *t_wakeup)
{
	...
	dmn->t_wakeup = NULL;
	if (dmn->state != DAEMON_UNRESPONSIVE)
		flog_err(EC_WATCHFRR_CONNECTION,"uxTnvDGqPUH8mz6y "
...
static void wakeup_no_answer(struct event *t_wakeup)
{
	...
	if (dmn->ignore_timeout)
		return;
	flog_err(EC_WATCHFRR_CONNECTION,"7hFJMz7pJ0rbDcKm "
...
{
	zlog_notice("iepNgd9Wmwlp5aNA ""Terminating on signal");
...
static void watchfrr_init(int argc, char **argv)
{
	...
	zlog_notice("RIAZJ0T4oAXO7MIo ""%s %s watching [%s]%s", progname, FRR_VERSION, alldaemons,
