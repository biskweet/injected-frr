...
#define OSPF_WRITE_FRAG_SHIFT 3
	...
	while ((stream_get_endp(op->s) - stream_get_getp(op->s))
		...
		if (ret < 0)
			flog_err(
				EC_LIB_SOCKET,"qB0Yp7iKeVghW6JJ "
		...
		if (IS_DEBUG_OSPF_PACKET(type - 1, SEND)) {
			zlog_debug("LA0hrpZ75bICtWCa ""%s: sent id %d, off %d, len %d to %pI4",
...
#define OSPF_WRITE_IPHL_SHIFT 2
	...
	if (fd < 0 || ospf->oi_running == 0) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("4jc5VISk8Z3VhblX ""%s failed to send, fd %d, instance %u",
 ...
 */
		...
		if (ret < 0 && errno != EPERM)
			flog_err(
				EC_LIB_SOCKET,"8MN7lfdBN04cu2vp "
		...
		if (IS_DEBUG_OSPF_PACKET(type - 1, SEND)) {
			if (IS_DEBUG_OSPF_PACKET(type - 1, DETAIL)) {
				zlog_debug("ixN8JE1CGGoOhbv3 "
					"-----------------------------------------------------");
				stream_set_getp(op->s, 0);
				ospf_packet_dump(op->s);
			}

			zlog_debug("cE2k6RUEFVX2lpHw ""%s sent to [%pI4] via [%s].",
				   lookup_msg(ospf_packet_type_str, type, NULL),
				   &op->dst, IF_NAME(oi));

			if (IS_DEBUG_OSPF_PACKET(type - 1, DETAIL))
				zlog_debug("8iMVR4ieioiRBsH7 "
					"-----------------------------------------------------");
		}
		       ...
		       struct stream *s, struct ospf_interface *oi, int size)
{
	...
	if (oi->type != OSPF_IFTYPE_POINTOPOINT
		...
		if (oi->address->prefixlen != p.prefixlen) {
			flog_warn(
				EC_OSPF_PACKET,"UOOAtOVroQIXClBE "
	...
	if (OSPF_IF_PARAM(oi, v_wait) != ntohl(hello->dead_interval)) {
		flog_warn(
			EC_OSPF_PACKET,"EIebrmvr1Cmao0Wf "
	...
	if (OSPF_IF_PARAM(oi, fast_hello) == 0) {
		if (OSPF_IF_PARAM(oi, v_hello)
			...
			flog_warn(
				EC_OSPF_PACKET,"42EcgxobjXzs6gpX "
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("R1MS1GHjNcTOZU48 ""Packet %pI4 [Hello:RECV]: Options on %s %s vrf %s",
			   &ospfh->router_id, IF_NAME(oi),
...
#define REJECT_IF_TBIT_ON	1 /* XXX */
	...
	if (CHECK_FLAG(hello->options, OSPF_OPTION_MT)) {
		 ...
		 * relationship.
		 */
		flog_warn(EC_OSPF_PACKET,"lYT6oXTPfAf8zMVJ "
	...
	if (CHECK_FLAG(oi->ospf->config, OSPF_OPAQUE_CAPABLE) &&
		 ...
		 */
		flog_warn(EC_OSPF_PACKET,"KxwpHP7yBV0vU4Mc "
	...
	if (oi->area->external_routing == OSPF_AREA_NSSA) {
		if (!(CHECK_FLAG(OPTIONS(oi), OSPF_OPTION_NP)
			...
			flog_warn(
				EC_OSPF_PACKET,"3jGCsgMcDQ2ILUy4 "
		...
		if (IS_DEBUG_OSPF_NSSA)
			zlog_debug("jY6peg1wsnDE8jgw ""NSSA-Hello:RECV:Packet from %pI4:",
				   &ospfh->router_id);
	} else
		...
		if (CHECK_FLAG(OPTIONS(oi), OSPF_OPTION_E)
		    != CHECK_FLAG(hello->options, OSPF_OPTION_E)) {
		flog_warn(
			EC_OSPF_PACKET,"tpJgmXlIO5KXNzMm "
			      ...
			      struct ospf_db_desc *dd, uint16_t size)
{
	...
	for (size -= OSPF_DB_DESC_MIN_SIZE; size >= OSPF_LSA_HEADER_SIZE;
		...
		if (lsah->type < OSPF_MIN_LSA || lsah->type >= OSPF_MAX_LSA) {
			flog_warn(EC_OSPF_PACKET,"tXj9hBZkQ7xY40BP "
		...
		if (IS_OPAQUE_LSA(lsah->type)
			...
			flog_warn(EC_OSPF_PACKET,"PK0jBYOjHBhBN8xe "
		...
		case OSPF_OPAQUE_AS_LSA:
			...
			if (oi->area->external_routing == OSPF_AREA_STUB) {
				flog_warn(
					EC_OSPF_PACKET,"3dlV0svHrjZDaGar "
			 ...
			 uint16_t size)
{
	...
	if (nbr == NULL) {
		flog_warn(EC_OSPF_PACKET,"TYcpcgNJGaSO8IKB " "Packet[DD]: Unknown Neighbor %pI4",
			  &ospfh->router_id);
		return;
	}
	...
	if ((OSPF_IF_PARAM(oi, mtu_ignore) == 0)
		...
		flog_warn(
			EC_OSPF_PACKET,"YMc5oUt7RkXpvSvT "
	...
	if (CHECK_FLAG(dd->options, OSPF_OPTION_MT)) {
		 ...
		 */
		flog_warn(EC_OSPF_PACKET,"QAtM225WKjUKHD0K " "Packet[DD]: Neighbor %pI4: T-bit on?",
			  &nbr->router_id);
		return;
	}
#endif /* REJECT_IF_TBIT_ON */
	...
	case NSM_ExStart:
		...
		if (CHECK_FLAG(oi->ospf->config, OSPF_OPAQUE_CAPABLE) &&
			...
			if (!CHECK_FLAG(nbr->options, OSPF_OPTION_O)
				...
				flog_warn(
					EC_OSPF_PACKET,"dUPnws3Wrurt3llR "
	...
	case NSM_Exchange:
		...
		if (IS_SET_DD_MS(dd->flags)
			...
			flog_warn(EC_OSPF_PACKET,"DLYfWGdYUdl6kseT "
				  "Packet[DD]: Neighbor %pI4 MS-bit mismatch.",
				  &nbr->router_id);
			OSPF_NSM_EVENT_SCHEDULE(nbr, NSM_SeqNumberMismatch);
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("Z7SIcVlzfyTKKG94 "
		...
		if (IS_SET_DD_I(dd->flags)) {
			zlog_info("RwwxlRASrJ2K4lk8 ""Packet[DD]: Neighbor %pI4 I-bit set.",
				  &nbr->router_id);
			OSPF_NSM_EVENT_SCHEDULE(nbr, NSM_SeqNumberMismatch);
			break;
		}
		...
		if (dd->options != nbr->options) {
			flog_warn(EC_OSPF_PACKET,"uDYBo3jAgxrsiunc "
		...
		if ((IS_SET_DD_MS(nbr->dd_flags)
			...
			flog_warn(
				EC_OSPF_PACKET,"BzvaysLVrnTW2jLV "
	...
	case NSM_Full:
		...
		break;
	default:
		flog_warn(EC_OSPF_PACKET,"szxq5ZoYCzHMJgoE "
			...
			uint16_t size)
{
	...
	nbr = ospf_nbr_lookup(oi, iph, ospfh);
	if (nbr == NULL) {
		flog_warn(EC_OSPF_PACKET,"zmZrzfwoRijqh8WO "
	...
	if (nbr->state != NSM_Exchange && nbr->state != NSM_Loading
	    && nbr->state != NSM_Full) {
		flog_warn(
			EC_OSPF_PACKET,"VSeM7pgDfOJxs5DX "
					 ...
					 struct ospf_interface *oi, size_t size)
{
	...
	for (; size >= OSPF_LSA_HEADER_SIZE && count > 0;
		...
		if (length > size) {
			flog_warn(
				EC_OSPF_PACKET,"Q4PZZcomrikggMHB "
				"Link State Update: LSA length exceeds packet size.");
			break;
		}

		if (length < OSPF_LSA_HEADER_SIZE) {
			flog_warn(EC_OSPF_PACKET,"4AXG6Nefco8KJnVH "
		...
		if (!ospf_lsa_checksum_valid(lsah)) {
			 ...
			 * other */
			flog_warn(
				EC_OSPF_PACKET,"zkGswvnH2cWik7c7 "
		...
		if (lsah->type < OSPF_MIN_LSA || lsah->type >= OSPF_MAX_LSA) {
			flog_warn(EC_OSPF_PACKET,"ulmeao0wapTTDhS4 "
		...
		if (CHECK_FLAG(nbr->options, OSPF_OPTION_O)) {
			...
			if ((IS_OPAQUE_LSA(lsah->type)
				 ...
				 * only.
				 */
				flog_warn(EC_OSPF_PACKET,"Qi4bH5wfZijGHYu7 "
			...
			flog_warn(
				EC_OSPF_PACKET,"5rhWDoMuMDH64Rwv "
				"LSA[Type%d:%pI4] from %pI4: Opaque capability mismatch?",
				lsah->type, &lsah->id, &lsah->adv_router);
			continue;
		}
			...
			struct ospf_interface *oi, uint16_t size)
{
	...
	nbr = ospf_nbr_lookup(oi, iph, ospfh);
	if (nbr == NULL) {
		flog_warn(EC_OSPF_PACKET,"eThz0VNmSyzKxv8j "
...
#define DISCARD_LSA(L, N)                                                              \
	...
	for (ALL_LIST_ELEMENTS(lsas, node, nnode, lsa)) {
		...
		if (IS_DEBUG_OSPF(lsa, LSA))
			zlog_debug("9p2fCgb3BGRPlngT ""LSA Type-%d from %pI4, ID: %pI4, ADV: %pI4",
		...
		if (lsa->data->type == OSPF_ROUTER_LSA)
			if (!IPV4_ADDR_SAME(&lsa->data->id,
					    &lsa->data->adv_router)) {
				flog_err(
					EC_OSPF_ROUTER_LSA_MISMATCH,"BLcgrTAH8SF3tqxP "
					"Incoming Router-LSA from %pI4 with Adv-ID[%pI4] != LS-ID[%pI4]",
					&ospfh->router_id, &lsa->data->id,
					&lsa->data->adv_router);
				flog_err(
					EC_OSPF_DOMAIN_CORRUPT,"nZ8FZiosnUb21heJ "
		...
		if (IS_OPAQUE_LSA(lsa->data->type)
			...
			if (IS_LSA_MAXAGE(lsa)) {
				zlog_info("8AVO2EYv4nV06mfR ""LSA[%s]: Boomerang effect?",
					  dump_lsa_key(lsa));
				ospf_ls_ack_send(nbr, lsa);
				ospf_lsa_discard(lsa);
		...
		if (ospf_ls_request_lookup(nbr, lsa)) {
			OSPF_NSM_EVENT_SCHEDULE(nbr, NSM_BadLSReq);
			flog_warn(
				EC_OSPF_PACKET,"lyyfDE6ViJqlCXFI "
			...
			uint16_t size)
{
	...
	nbr = ospf_nbr_lookup(oi, iph, ospfh);
	if (nbr == NULL) {
		flog_warn(EC_OSPF_PACKET,"VeQRg6ahlLS1qgZH "
				       ...
				       struct stream *ibuf)
{
	...
	if (ret < 0) {
		if (errno != EAGAIN && errno != EWOULDBLOCK)
			flog_warn(EC_OSPF_PACKET,"3HtC8PedsdpWBZwg " "stream_recvmsg failed: %s",
				  safe_strerror(errno));
		return NULL;
	}
	if ((unsigned int)ret < sizeof(struct ip)) {
		flog_warn(
			EC_OSPF_PACKET,"kZQP0LV3oHj1P1Md "
	...
	*ifp = if_lookup_by_index(ifindex, ospf->vrf_id);
	...
	if (ret != ip_len) {
		flog_warn(
			EC_OSPF_PACKET,"LxXYuW49x0VBApIH "
	...
	if (IS_DEBUG_OSPF_PACKET(0, RECV))
		zlog_debug("dQJoLXKJ8YeXv0Xy ""%s: fd %d(%s) on interface %d(%s)", __func__, fd,
			 ...
			 struct ip *iph, struct ospf_header *ospfh)
{
	...
	for (ALL_LIST_ELEMENTS_RO(ospf->vlinks, node, vl_data)) {
		...
		if (OSPF_AREA_SAME(&vl_area, &rcv_oi->area)
			...
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("WMbEWcDV0vzT4y1T ""associating packet with %s",
					   IF_NAME(vl_data->vl_oi));
			if (!CHECK_FLAG(vl_data->vl_oi->ifp->flags, IFF_UP)) {
				if (IS_DEBUG_OSPF_EVENT)
					zlog_debug("qu9WEIETaCFS14Jr "
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("38QwrQB8KkYELeyr ""couldn't find any VL to associate the packet with");
					     ...
					     const uint16_t num_links)
{
	...
	while (linkbytes >= OSPF_ROUTER_LSA_LINK_SIZE) {
		...
		if (thislinklen > linkbytes) {
			if (IS_DEBUG_OSPF_PACKET(0, RECV))
				zlog_debug("UCeheLTzGUVheGKQ ""%s: length error in link block #%u",
	...
	if (counted_links != num_links) {
		if (IS_DEBUG_OSPF_PACKET(0, RECV))
			zlog_debug("Z4OMYMbrkpbbkY3X ""%s: %u link blocks declared, %u present",
				...
				const uint8_t headeronly)
{
	...
	if (lsah->type < OSPF_MAX_LSA && ospf_lsa_minlen[lsah->type]
		...
		if (IS_DEBUG_OSPF_PACKET(0, RECV))
			zlog_debug("tzIW5CRHRf3XU9dR ""%s: undersized (%u B) %s", __func__, lsalen,
	...
	default:
		if (IS_DEBUG_OSPF_PACKET(0, RECV))
			zlog_debug("ic1qt3mBGP9csJ9G ""%s: unsupported LSA type 0x%02x", __func__,
				   lsah->type);
		return MSG_NG;
	}
	if (ret != MSG_OK && IS_DEBUG_OSPF_PACKET(0, RECV))
		zlog_debug("PRFFfp6N4xLoJQKe ""%s: alignment error in %s", __func__,
	...
	while (length) {
		...
		if (headeronly) {
			...
			if (MSG_OK != ospf_lsa_examin(lsah, lsalen, 0)) {
				if (IS_DEBUG_OSPF_PACKET(0, RECV))
					zlog_debug("41u6dRvN7N4KE14x ""%s: malformed LSA #%u",
						   __func__, counted_lsas);
				return MSG_NG;
			}
				   ...
				   const unsigned bytesonwire)
{
	...
	if (bytesonwire < OSPF_HEADER_SIZE) {
		if (IS_DEBUG_OSPF_PACKET(0, RECV))
			zlog_debug("LSctQ7wwxdSznNP6 ""%s: undersized (%u B) packet", __func__,
				   bytesonwire);
		return MSG_NG;
	}
	...
	if (oh->version != OSPF_VERSION) {
		if (IS_DEBUG_OSPF_PACKET(0, RECV))
			zlog_debug("Kgu5eeonlXHh0qUu ""%s: invalid (%u) protocol version",
				   __func__, oh->version);
		return MSG_NG;
	}
	...
	if (oh->type >= OSPF_MSG_HELLO && oh->type <= OSPF_MSG_LS_ACK
		...
		if (IS_DEBUG_OSPF_PACKET(0, RECV))
			zlog_debug("Ll10zRABNKnxP8B7 ""%s: undersized (%u B) %s packet", __func__,
				   bytesdeclared,
	...
	default:
		if (IS_DEBUG_OSPF_PACKET(0, RECV))
			zlog_debug("fP8ZLhJiQ6kWR0fB ""%s: invalid packet type 0x%02x", __func__,
				   oh->type);
		return MSG_NG;
	}
	if (ret != MSG_OK && IS_DEBUG_OSPF_PACKET(0, RECV))
		zlog_debug("zlW59OMyydxsGLz7 ""%s: malformed %s packet", __func__,
			      ...
			      struct ip *iph, struct ospf_header *ospfh)
{
	...
	if (!ospf_check_area_id(oi, ospfh)) {
		flog_warn(EC_OSPF_PACKET,"esHMb0bkaIqIbbVs "
	...
	if (!ospf_check_network_mask(oi, iph->ip_src)) {
		flog_warn(
			EC_OSPF_PACKET,"IhTEKVeyZJsYYR8u "
...
static enum ospf_read_return_enum ospf_read_helper(struct ospf *ospf)
{
	...
	if ((unsigned int)(iph->ip_hl << 2) >= STREAM_READABLE(ibuf)) {
		if ((unsigned int)(iph->ip_hl << 2) == STREAM_READABLE(ibuf)) {
			flog_warn(
				EC_OSPF_PACKET,"I9yqSFlGmdoR1eLv "
				"Rx'd IP packet with OSPF protocol number but no payload");
		} else {
			flog_warn(
				EC_OSPF_PACKET,"FW6dmrE3KD0AiqvT "
	 ...
	 */
		...
		if (IS_DEBUG_OSPF_EVENT)
			flog_warn(EC_OSPF_PACKET,"53h7kCMsE1mir2zU "
				  "Packet from [%pI4] received on wrong link %s",
				  &iph->ip_src, ifp->name);
		return OSPF_READ_CONTINUE;
	} else if (oi->state == ISM_Down) {
		flog_warn(
			EC_OSPF_PACKET,"1SUeTRWT57DG6chj "
	...
	if (iph->ip_dst.s_addr == htonl(OSPF_ALLDROUTERS)
		...
		flog_warn(
			EC_OSPF_PACKET,"rii4ikI5NSsGuVMH "
	...
	if (IS_DEBUG_OSPF_PACKET(ospfh->type - 1, RECV)) {
		if (IS_DEBUG_OSPF_PACKET(ospfh->type - 1, DETAIL)) {
			zlog_debug("0ZCbjzjLIQR168Wm "
				"-----------------------------------------------------");
			ospf_packet_dump(ibuf);
		}

		zlog_debug("AnHfZ2sqE1OESU3M ""%s received from [%pI4] via [%s]",
			   lookup_msg(ospf_packet_type_str, ospfh->type, NULL),
			   &ospfh->router_id, IF_NAME(oi));
		zlog_debug("d6OfIZ0a5jXCpBCW "" src [%pI4],", &iph->ip_src);
		zlog_debug("cgLu304LofiZo7qM "" dst [%pI4]", &iph->ip_dst);

		if (IS_DEBUG_OSPF_PACKET(ospfh->type - 1, DETAIL))
			zlog_debug("lkRISKhHvJAisTwD "
				"-----------------------------------------------------");
	}
	...
	default:
		flog_warn(
			EC_OSPF_PACKET,"GD2WwDTqcPHX32mF "
...
static int ospf_make_hello(struct ospf_interface *oi, struct stream *s)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("KoZ6Pf27bh5LrWP3 ""%s: options: %x, int: %s", __func__, OPTIONS(oi),
			   IF_NAME(oi));
	...
	for (rn = route_top(oi->nbrs); rn; rn = route_next(rn)) {
		...
		if (length + sizeof(uint32_t) > ospf_packet_max(oi)) {
			flog_err(
				EC_OSPF_LARGE_HELLO,"G6YXKKUA2vDiy7eQ "
			    ...
			    struct stream *s)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("GJbPIiYGfIyyKg3n ""%s: Start", __func__);
	...
	while ((node = listhead(update)) != NULL) {
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("qDBzDYlkiWQausEj ""%s: List Iteration %d LSA[%s]", __func__,
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("2zCS02WpeeyoUI0B ""%s: Stop", __func__);
	return length;
}
...
void ospf_poll_timer(struct event *thread)
{
	...
	if (IS_DEBUG_OSPF(nsm, NSM_TIMERS))
		zlog_debug("BKODB6Y2Nt7W6sRn ""NSM[%s:%pI4]: Timer (Poll timer expire)",
...
void ospf_hello_reply_timer(struct event *thread)
{
	...
	if (IS_DEBUG_OSPF(nsm, NSM_TIMERS))
		zlog_debug("0tml7Recw1ZSsON9 ""NSM[%s:%pI4]: Timer (hello-reply timer expire)",
						  ...
						  struct ospf_interface *oi)
{
	...
	if ((OSPF_LS_UPD_MIN_SIZE + ntohs(lsa->data->length))
		...
		if (!warned) {
			flog_warn(
				EC_OSPF_LARGE_LSA,"FfWGyESumtphzzMt "
	...
	if (size > OSPF_MAX_PACKET_SIZE) {
		flog_warn(
			EC_OSPF_LARGE_LSA,"jW9PEkmO72EfspcN "
			    ...
			    struct in_addr addr, int send_lsupd_now)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("B4inTMOAJ1TLgITp ""listcount = %d, [%s]dst %pI4", listcount(update),
			   IF_NAME(oi), &addr);
...
static void ospf_ls_upd_send_queue_event(struct event *thread)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("K4i1eSA6GuWI9JC6 ""%s start", __func__);
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("FLss84MjrPiwt33Z ""%s stop", __func__);
}
...
void ospf_ls_ack_send(struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
	...
	if (IS_GRACE_LSA(lsa)) {
		if (IS_DEBUG_OSPF_GR)
			zlog_debug("AoU9Ptau5RGF3tlj ""%s, Sending GRACE ACK to Restarter.",
				   __func__);
	}
