...
{
	if (IS_DEBUG_OSPF_GR)
		zlog_debug("eE9y7YSPBwjzSDLb ""%s, GR Helper init.", __func__);
...
{
	if (IS_DEBUG_OSPF_GR)
		zlog_debug("vmszDOqo0Y86esk0 ""%s, GR helper deinit.", __func__);
...
void ospf_gr_helper_init(void)
{
	...
	if (IS_DEBUG_OSPF_GR)
		zlog_debug("fIFhqYSv7VsokX5q ""%s, GR Helper init.", __func__);
	...
	rc = ospf_register_opaque_functab(
		...
		NULL, NULL);
	if (rc != 0) {
		flog_warn(EC_OSPF_OPAQUE_REGISTRATION,"B9UQYUNUoEZBn5Np "
...
{
	if (IS_DEBUG_OSPF_GR)
		zlog_debug("xcpf2dTrKjZ7I3eQ ""%s, GR helper deinit.", __func__);
					 ...
					 struct in_addr *addr, uint8_t *reason)
{
	...
	if (lsa->size <= OSPF_LSA_HEADER_SIZE) {
		if (IS_DEBUG_OSPF_GR)
			zlog_debug("OjwuTHMBoV1Py58P ""%s: Malformed packet: Invalid LSA len:%d",
				   __func__, length);
	...
	for (tlvh = TLV_HDR_TOP(lsah); sum < length && tlvh;
		...
		if (sum + TLV_SIZE(tlvh) > length) {
			if (IS_DEBUG_OSPF_GR)
				zlog_debug("aVNIHHSxBK84aR6y ""%s: Malformed packet: Invalid TLV len:%u",
		...
		case GRACE_PERIOD_TYPE:
			if (TLV_SIZE(tlvh) <
				...
				zlog_debug("CrV3dumYrtXiR3o6 ""%s: Malformed packet: Invalid grace TLV len:%u",
					   __func__, TLV_SIZE(tlvh));
				return OSPF_GR_FAILURE;
			}
		...
		case RESTART_REASON_TYPE:
			if (TLV_SIZE(tlvh) <
				...
				zlog_debug("PlTTmSSSYMTXY8ww ""%s: Malformed packet: Invalid reason TLV len:%u",
					   __func__, TLV_SIZE(tlvh));
				return OSPF_GR_FAILURE;
			}
		...
		case RESTARTER_IP_ADDR_TYPE:
			if (TLV_SIZE(tlvh) <
				...
				zlog_debug("SiN5fIluqcHY6sMX ""%s: Malformed packet: Invalid addr TLV len:%u",
					   __func__, TLV_SIZE(tlvh));
				return OSPF_GR_FAILURE;
			}
			   ...
			   struct ospf_neighbor *nbr)
{
	...
	if (ret != OSPF_GR_SUCCESS) {
		if (IS_DEBUG_OSPF_GR)
			zlog_debug("vAo22g6ZAZ7zjfBa ""%s, Wrong Grace LSA packet.", __func__);
		return OSPF_GR_NOT_HELPER;
	}

	if (IS_DEBUG_OSPF_GR)
		zlog_debug("fwKp5jpwAiAe8GWO "
	...
	if (IS_DEBUG_OSPF_GR)
		zlog_debug("78K92dPsgNzATMFG ""%s, Grace timer started.interval:%d", __func__,
...
void ospf_helper_handle_topo_chg(struct ospf *ospf, struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF_GR)
		zlog_debug("Y0cPFg0d4iZLmZyc ""%s: Topo change detected due to LSA[%s]", __func__,
			   dump_lsa_key(lsa));
			 ...
			 enum ospf_helper_exit_reason reason)
{
	...
	if (IS_DEBUG_OSPF_GR)
		zlog_debug("9gUbpTBqs1wKCKPk ""%s, Exiting from HELPER support to %pI4, due to %s",
	...
	if (reason != OSPF_GR_HELPER_COMPLETED) {
		if (IS_DEBUG_OSPF_GR)
			zlog_debug("RU2GarDlfyTlZiEy ""%s, Unsuccessful GR exit", __func__);
	}
				   ...
				   struct ospf_neighbor *nbr)
{
	...
	if (ret != OSPF_GR_SUCCESS) {
		if (IS_DEBUG_OSPF_GR)
			zlog_debug("r1O6XUbsiHA48MR6 ""%s, Wrong Grace LSA packet.", __func__);
		return;
	}

	if (IS_DEBUG_OSPF_GR)
		zlog_debug("pOM9J29A92U3O7fT ""%s, GraceLSA received for neighbour %pI4", __func__,
			   &restartAddr);
				     ...
				     struct ospf_lsa *lsa)
{
	...
	if (lsa->size <= OSPF_LSA_HEADER_SIZE) {
		if (vty)
			...
			zlog_debug("Q28DI006zuj2cgAp ""%% Invalid LSA length: %d", length);
		return;
	}
	...
	if (vty)
		...
		zlog_debug("w8wwsmLIWBfheqEE ""  TLV info:");
	...
	for (tlvh = TLV_HDR_TOP(lsah); sum < length && tlvh;
		...
		if (sum + TLV_SIZE(tlvh) > length) {
			if (vty)
				...
				zlog_debug("cet0BcRuNLTNs90I ""%% Invalid TLV length: %u",
					   TLV_SIZE(tlvh));
			return;
		}
		...
		case GRACE_PERIOD_TYPE:
			...
			if (vty)
				...
				zlog_debug("l1HZxmb9RW44SaTF ""   Grace period:%d",
					   ntohl(gracePeriod->interval));
			break;
		case RESTART_REASON_TYPE:
			if (TLV_SIZE(tlvh)
			...
			if (vty)
				...
				zlog_debug("Id9LKjoHuXh97nay ""   Restart reason:%s",
					   ospf_restart_reason2str(
						   grReason->reason));
			break;
		case RESTARTER_IP_ADDR_TYPE:
			...
			if (vty)
				...
				zlog_debug("XMtuIvuDXBVI5Hlb ""   Restarter address:%pI4",
					   &restartAddr->addr);
			break;
		default:
			if (vty)
				...
				zlog_debug("RE2CscMjuoBBHXtr ""   Unknown TLV type %d",
					   ntohs(tlvh->type));
