...
static void ospf_opaque_del_functab(void *val)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("GRUlV8V6m7zqpcec ""%s: Opaque LSA functab list deletion callback type %u (%p)",
...
static void ospf_opaque_funclist_init(void)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("61GJeKfg3u7V0Bxc ""%s: Function list initialize", __func__);
...
static void ospf_opaque_funclist_term(void)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("d2L9YURWwNLhY1Cf ""%s: Function list terminate", __func__);
...
static struct list *ospf_get_opaque_funclist(uint8_t lsa_type)
{
	...
	case OSPF_OPAQUE_AS_LSA:
		...
		break;
	default:
		flog_warn(EC_OSPF_LSA_UNEXPECTED,"Uf7BS80WhGKGU2TV " "%s: Unexpected LSA-type(%u)",
			  __func__, lsa_type);
		break;
	}
	return funclist;
	...
	int (*del_lsa_hook)(struct ospf_lsa *lsa))
{
	...
	for (ALL_LIST_ELEMENTS(funclist, node, nnode, functab))
		if (functab->opaque_type == opaque_type) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("1wnoJPtDJTpnpbrn ""%s: Opaque LSA functab found type %u, (%p)",
	...
	if (functab == NULL)
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("CZ5pZtikY8qeLysL ""%s: Re-register Opaque LSA type %u, opaque type %u, (%p)",
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("vIRRxGhyAxuQGVtC ""%s: Register Opaque LSA type %u, opaque type %u, (%p)",
...
void ospf_delete_opaque_functab(uint8_t lsa_type, uint8_t opaque_type)
{
	...
	if ((funclist = ospf_get_opaque_funclist(lsa_type)) != NULL)
		for (ALL_LIST_ELEMENTS(funclist, node, nnode, functab)) {
			if (functab->opaque_type == opaque_type) {
				if (IS_DEBUG_OSPF_EVENT)
					zlog_debug("Gn3iPTeVIJaFFEVC ""%s: Delete Opaque functab LSA type %u, opaque type %u, (%p)",
						   __func__, lsa_type,
						   opaque_type, functab);
			      ...
			      struct ospf_lsa *new)
{
	...
	case OSPF_OPAQUE_AS_LSA:
		...
		break;
	default:
		flog_warn(EC_OSPF_LSA_UNEXPECTED,"bWtPsdwNbfxLVDaT " "%s: Unexpected LSA-type(%u)",
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("nZlhiux37tf7qSAt ""%s: Register Opaque info-per-type LSA type %u, opaque type %u, (%p), Functab (%p)",
				      ...
				      bool cleanup_owner)
{
	...
	if (cleanup_owner) {
		...
		default:
			flog_warn(
				EC_OSPF_LSA_UNEXPECTED,"OA3p53uHquXDHa70 "
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("CPY3mxfavSx8Wtk7 ""%s: Free Opaque info-per-type LSA type %u, opaque type %u, (%p), Functab (%p)",
			   __func__, oipt->lsa_type, oipt->opaque_type, oipt,
			   oipt->functab);
...
lookup_opaque_info_by_type(struct ospf_lsa *lsa)
{
	...
	case OSPF_OPAQUE_AS_LSA:
		...
		break;
	default:
		flog_warn(EC_OSPF_LSA_UNEXPECTED,"nWCOzANJaoM5LDNQ " "%s: Unexpected LSA-type(%u)",
...
void ospf_opaque_type9_lsa_if_cleanup(struct ospf_interface *oi)
{
	...
	LSDB_LOOP (OPAQUE_LINK_LSDB(oi->area), rn, lsa)
		...
		if (lsa->oi == oi) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("Ldo2N1pzQ9HiroUN ""Delete Type-9 Opaque-LSA on interface delete: [opaque-type=%u, opaque-id=%x]",
					   GET_OPAQUE_TYPE(
       ...
       "Opaque LSA\n")
{
	...
	if (!CHECK_FLAG(ospf->config, OSPF_OPAQUE_CAPABLE)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("RdkLuZzNGpTM5dKF ""Opaque capability: OFF -> ON");
       ...
       "Opaque LSA\n")
{
	...
	if (CHECK_FLAG(ospf->config, OSPF_OPAQUE_CAPABLE)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("YU0SKO1ha2K0MDAW ""Opaque capability: ON -> OFF");
			     ...
			     json_object *json)
{
	...
	if (vty != NULL) {
		...
		zlog_debug("spbrRFPJDrAhyFvr ""    Opaque-Type %u (%s)", opaque_type,
			   ospf_opaque_type_name(opaque_type));
		zlog_debug("mhk7xkOZuidSmeZ7 ""    Opaque-ID   0x%x", opaque_id);

		zlog_debug("T04jK8jJq7RX6JlW ""    Opaque-Info: %u octets of data%s",
			   ntohs(lsah->length) - OSPF_LSA_HEADER_SIZE,
			   VALID_OPAQUE_INFO_LEN(lsah) ? ""
						       : "(Invalid length?)");
	}
...
void ospf_opaque_lsa_originate_schedule(struct ospf_interface *oi, int *delay0)
{
	...
	if ((top = oi_to_top(oi)) == NULL || (area = oi->area) == NULL) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("RB7dVa2jMTvMj0gf ""%s: Invalid argument?", __func__);
		return;
	}
	...
	if (!CHECK_FLAG(top->opaque, OPAQUE_OPERATION_READY_BIT)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("bh6rxZkY1kccxroL ""%s: Not operational.", __func__);
...
static void ospf_opaque_type9_lsa_originate(struct event *t)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("CVJ5a6UNEfOmFIag ""Timer[Type9-LSA]: Originate Opaque-LSAs for OI %s",
			   IF_NAME(oi));
...
static void ospf_opaque_lsa_reoriginate_resume(struct list *listtop, void *arg)
{
	...
	for (ALL_LIST_ELEMENTS(listtop, node, nnode, oipt)) {
		...
		if ((*functab->lsa_originator)(arg) != 0) {
			flog_warn(EC_OSPF_LSA,"tmAeg9mxp1WGhRse " "%s: Failed (opaque-type=%u)",
...
struct ospf_lsa *ospf_opaque_lsa_install(struct ospf_lsa *lsa, int rt_recalc)
{
	...
	/* Register the new lsa entry */
		...
		flog_warn(EC_OSPF_LSA,"H8EnWrZZI5lSquV8 " "%s: register_opaque_lsa() ?", __func__);
		goto out;
	}
	...
	case OSPF_OPAQUE_LINK_LSA:
		if ((top = oi_to_top(lsa->oi)) == NULL) {
			...
			flog_warn(EC_OSPF_LSA,"84FjOIemCYpLqiol " "%s: Something wrong?",
				  __func__);
			goto out;
		}
		break;
	case OSPF_OPAQUE_AREA_LSA:
		if (lsa->area == NULL || (top = lsa->area->ospf) == NULL) {
			/* Above conditions must have passed. */
			flog_warn(EC_OSPF_LSA,"p1OmokFSVFtogQru " "%s: Something wrong?",
				  __func__);
			goto out;
		}
		break;
	case OSPF_OPAQUE_AS_LSA:
		...
		if (lsa->area != NULL && (top = lsa->area->ospf) == NULL) {
			/* Above conditions must have passed. */
			flog_warn(EC_OSPF_LSA,"sde13zBh1ubJTxGO " "%s: Something wrong?",
				  __func__);
			goto out;
		}
		break;
	default:
		flog_warn(EC_OSPF_LSA_UNEXPECTED,"Bb6pz3tTdINPLhEt " "%s: Unexpected LSA-type(%u)",
			  __func__, lsa->data->type);
		goto out;
	}
...
struct ospf_lsa *ospf_opaque_lsa_refresh(struct ospf_lsa *lsa)
{
	...
	if ((functab = ospf_opaque_functab_lookup(lsa)) == NULL
		 ...
		 */
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("O6oWdEJN7AYDlxW0 ""LSA[Type%d:%pI4]: Flush stray Opaque-LSA",
					  ...
					  uint8_t lsa_type, uint8_t opaque_type)
{
	...
	case OSPF_OPAQUE_LINK_LSA:
		if ((oi = (struct ospf_interface *)lsa_type_dependent)
		    == NULL) {
			flog_warn(EC_OSPF_LSA,"2g7AeCVlCRhyRcXm "
				  "%s: Type-9 Opaque-LSA: Invalid parameter?",
				  __func__);
			goto out;
		}
		if ((top = oi_to_top(oi)) == NULL) {
			flog_warn(EC_OSPF_LSA,"0NUKc5qb6qnCFQGx " "%s: OI(%s) -> TOP?", __func__,
				  IF_NAME(oi));
			goto out;
		}
		if (!list_isempty(ospf_opaque_type9_funclist)
	...
	case OSPF_OPAQUE_AREA_LSA:
		if ((area = (struct ospf_area *)lsa_type_dependent) == NULL) {
			flog_warn(EC_OSPF_LSA,"CAh4f3uO87nG3C35 "
				  "%s: Type-10 Opaque-LSA: Invalid parameter?",
				  __func__);
			goto out;
		}
		if ((top = area->ospf) == NULL) {
			flog_warn(EC_OSPF_LSA,"LwfSfayEJlvLGVcN " "%s: AREA(%pI4) -> TOP?",
	...
	case OSPF_OPAQUE_AS_LSA:
		if ((top = (struct ospf *)lsa_type_dependent) == NULL) {
			flog_warn(EC_OSPF_LSA,"9T1FwFZreJJYuQE4 "
		...
		break;
	default:
		flog_warn(EC_OSPF_LSA_UNEXPECTED,"JwNjkhOqtC0HK24U " "%s: Unexpected LSA-type(%u)",
			  __func__, lsa_type);
		goto out;
	}
	...
	if (!CHECK_FLAG(top->opaque, OPAQUE_OPERATION_READY_BIT)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("IepViHjmFMWDwnC4 ""%s: Not operational.", __func__);
...
static void ospf_opaque_type9_lsa_reoriginate_timer(struct event *t)
{
	...
	if ((functab = oipt->functab) == NULL
	    || functab->lsa_originator == NULL) {
		flog_warn(EC_OSPF_LSA,"0gNN3Tu6XobBIEFi " "%s: No associated function?", __func__);
		return;
	}

	oi = (struct ospf_interface *)oipt->owner;
	if ((top = oi_to_top(oi)) == NULL) {
		flog_warn(EC_OSPF_LSA,"IBfebgEAeryiE1PJ " "%s: Something wrong?", __func__);
		return;
	}
...
static void ospf_opaque_type10_lsa_reoriginate_timer(struct event *t)
{
	...
	if ((functab = oipt->functab) == NULL
	    || functab->lsa_originator == NULL) {
		flog_warn(EC_OSPF_LSA,"0JkSDq8XGoMzsVSL " "%s: No associated function?", __func__);
		return;
	}

	area = (struct ospf_area *)oipt->owner;
	if (area == NULL || (top = area->ospf) == NULL) {
		flog_warn(EC_OSPF_LSA,"3lTbdtSVFefm5Bus " "%s: Something wrong?", __func__);
		return;
	}
...
static void ospf_opaque_type11_lsa_reoriginate_timer(struct event *t)
{
	...
	if ((functab = oipt->functab) == NULL
	    || functab->lsa_originator == NULL) {
		flog_warn(EC_OSPF_LSA,"hitrBbCflDgxkLOD " "%s: No associated function?", __func__);
		return;
	}

	if ((top = (struct ospf *)oipt->owner) == NULL) {
		flog_warn(EC_OSPF_LSA,"pD2eGSEru3mxTfYD " "%s: Something wrong?", __func__);
		return;
	}

	if (!CHECK_FLAG(top->config, OSPF_OPAQUE_CAPABLE)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("yZyD7gV3VgKICkJM "
...
void ospf_opaque_lsa_refresh_schedule(struct ospf_lsa *lsa0)
{
	...
	if ((oipt = lookup_opaque_info_by_type(lsa0)) == NULL
		...
		flog_warn(EC_OSPF_LSA,"NLVv20DbFV3ZGyGj " "%s: Invalid parameter?", __func__);
		goto out;
	}

	/* Given "lsa0" and current "oipi->lsa" may different, but harmless. */
	if ((lsa = oipi->lsa) == NULL) {
		flog_warn(EC_OSPF_LSA,"X439ZJUiYtYtZnVJ " "%s: Something wrong?", __func__);
		goto out;
	}

	if (oipi->t_opaque_lsa_self != NULL) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("zSec3vo9jabYnnxT "
	...
	case OSPF_OPAQUE_AS_LSA:
		...
		break;
	default:
		flog_warn(EC_OSPF_LSA_UNEXPECTED,"0HA5xO2pVkTdmkv9 " "%s: Unexpected LSA-type(%u)",
			  __func__, lsa->data->type);
		goto out;
	}
...
static void ospf_opaque_lsa_refresh_timer(struct event *t)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("gbBrEbdkBJMq3ssJ ""Timer[Opaque-LSA]: (Opaque-LSA Refresh expire)");
...
void ospf_opaque_lsa_flush_schedule(struct ospf_lsa *lsa0)
{
	...
	if ((oipt = lookup_opaque_info_by_type(lsa0)) == NULL
	    || (oipi = lookup_opaque_info_by_id(oipt, lsa0)) == NULL) {
		flog_warn(EC_OSPF_LSA,"eIkIxwjIb5qKornY " "%s: Invalid parameter?", __func__);
		goto out;
	}

	/* Given "lsa0" and current "oipi->lsa" may different, but harmless. */
	if ((lsa = oipi->lsa) == NULL) {
		flog_warn(EC_OSPF_LSA,"NvMgK3803FGulIyK " "%s: Something wrong?", __func__);
		goto out;
	}
	...
	case OSPF_OPAQUE_AS_LSA:
		...
		break;
	default:
		flog_warn(EC_OSPF_LSA_UNEXPECTED,"HAK1inhCamAT1gHZ " "%s: Unexpected LSA-type(%u)",
			  __func__, lsa->data->type);
		goto out;
	}
					      ...
					      struct ospf_lsa *lsa)
{
	...
	case OSPF_OPAQUE_AS_LSA:
		...
		break;
	default:
		flog_warn(EC_OSPF_LSA_UNEXPECTED,"o6FtRbKS86XnynWH " "%s: Unexpected LSA-type(%u)",
			  __func__, lsa->data->type);
		return;
	}
}
...
struct ospf *oi_to_top(struct ospf_interface *oi)
{
	...
	if (oi == NULL || (area = oi->area) == NULL
	    || (top = area->ospf) == NULL)
		flog_warn(EC_OSPF_LSA,"qJ3ranAly0fjYH6s "
