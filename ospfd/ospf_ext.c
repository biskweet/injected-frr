...
int ospf_ext_init(void)
{
	...
	zlog_info("YA9r5Tt8spC9XAIf ""EXT (%s): Register Extended Link Opaque LSA", __func__);
	...
	if (rc != 0) {
		flog_warn(EC_OSPF_OPAQUE_REGISTRATION,"9MQxR5tfCBTpor2t "
			  "EXT (%s): Failed to register Extended Link LSA",
			  __func__);
		return rc;
	}

	zlog_info("QYtFyqUhguFcGG5B ""EXT (%s): Register Extended Prefix Opaque LSA", __func__);
	rc = ospf_register_opaque_functab(
		...
		NULL);			     /* del_lsa_hook */
	if (rc != 0) {
		flog_warn(EC_OSPF_OPAQUE_REGISTRATION,"a22Tz8nKLUZkT422 "
			   ...
			   uint32_t value, int value_type, uint8_t flags)
{
	...
	if ((algorithm != SR_ALGORITHM_SPF)
		...
		flog_err(EC_OSPF_INVALID_ALGORITHM,"jWQgAISLXM59NVeQ "
...
static int ospf_ext_link_del_if(struct interface *ifp)
{
	...
	if (exti != NULL) {
		...
		flog_warn(EC_OSPF_EXT_LSA_UNEXPECTED,"iOWoc55JxBptJkmr "
...
static void ospf_ext_ism_change(struct ospf_interface *oi, int old_status)
{
	...
	if (exti == NULL) {
		flog_warn(EC_OSPF_EXT_LSA_UNEXPECTED,"ypKTm0dxQxUUhhqb "
...
static void ospf_ext_link_nsm_change(struct ospf_neighbor *nbr, int old_status)
{
	...
	exti = lookup_ext_by_ifp(oi->ifp);
	if (exti == NULL) {
		flog_warn(EC_OSPF_EXT_LSA_UNEXPECTED,"Ae4yAqT8fhaCqxtg "
			  "EXT (%s): Cannot get Extended info. from OI(%s)",
			  __func__, IF_NAME(oi));
		return;
	}
	...
	if (oi->area == NULL || oi->area->ospf == NULL) {
		flog_warn(EC_OSPF_EXT_LSA_UNEXPECTED,"Lmvc7dkya3iprHpF "
...
static int ospf_ext_link_lsa_update(struct ospf_lsa *lsa)
{
	/* Sanity Check */
	if (lsa == NULL) {
		flog_warn(EC_OSPF_LSA_NULL,"yxi8p1QtmgDr5XyX " "EXT (%s): Abort! LSA is NULL",
			  __func__);
		return -1;
	}
...
static int ospf_ext_pref_lsa_update(struct ospf_lsa *lsa)
{
	...
	/* Sanity Check */
	if (lsa == NULL) {
		flog_warn(EC_OSPF_LSA_NULL,"skjg3F7z41yIIoLR " "EXT (%s): Abort! LSA is NULL",
			  __func__);
		return -1;
	}
					...
					struct ext_itf *exti)
{
	...
	if (new == NULL) {
		flog_warn(EC_OSPF_EXT_LSA_UNEXPECTED,"tPutfjLmPxoySaAX "
	...
	if (ospf_lsa_install(area->ospf, NULL /*oi */, new) == NULL) {
		flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"fVaK26r21qmxTADz "
					...
					struct ext_itf *exti)
{
	...
	if (new == NULL) {
		flog_warn(EC_OSPF_EXT_LSA_UNEXPECTED,"t82Uu4lnggkJLLyj "
	...
	/* Install this LSA into LSDB. */
	if (ospf_lsa_install(area->ospf, NULL /*oi */, new) == NULL) {
		flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"gW2RALNlN08HmUlL "
			  "EXT (%s): ospf_lsa_install() error", __func__);
		ospf_lsa_unlock(&new);
		return rc;
	}
	...
	for (ALL_LIST_ELEMENTS_RO(OspfEXT.iflist, node, exti)) {
		...
		if (CHECK_FLAG(exti->flags, EXT_LPFLG_LSA_ENGAGED)) {
			if (CHECK_FLAG(exti->flags,
				...
				flog_warn(
					EC_OSPF_EXT_LSA_UNEXPECTED,"WGZQQelmLaVBrQbO "
	...
	for (ALL_LIST_ELEMENTS_RO(OspfEXT.iflist, node, exti)) {
		...
		if (CHECK_FLAG(exti->flags, EXT_LPFLG_LSA_ENGAGED)) {
			if (CHECK_FLAG(exti->flags,
				       EXT_LPFLG_LSA_FORCED_REFRESH)) {
				flog_warn(
					EC_OSPF_EXT_LSA_UNEXPECTED,"hkn1f3qzh3IvdgmB "
					"EXT (%s): Refresh instead of Originate",
					__func__);
				UNSET_FLAG(exti->flags,
					   EXT_LPFLG_LSA_FORCED_REFRESH);
...
static struct ospf_lsa *ospf_ext_pref_lsa_refresh(struct ospf_lsa *lsa)
{
	...
	if (exti == NULL) {
		flog_warn(EC_OSPF_EXT_LSA_UNEXPECTED,"EXoPKFX02TYxxERB "
	...
	if ((exti != NULL) && !CHECK_FLAG(exti->flags, EXT_LPFLG_LSA_ACTIVE)) {
		flog_warn(EC_OSPF_EXT_LSA_UNEXPECTED,"jg0OXxahbhXjvK0j "
	...
	if (new == NULL) {
		flog_warn(EC_OSPF_EXT_LSA_UNEXPECTED,"H6Ltyf29Uwi7WARi "
			  "EXT (%s): ospf_ext_pref_lsa_new() error", __func__);
		return NULL;
	}
	...
	if (ospf_lsa_install(top, NULL /*oi */, new) == NULL) {
		flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"OLa8UFcQbAXXcn2Q "
			  "EXT (%s): ospf_lsa_install() error", __func__);
		ospf_lsa_unlock(&new);
		return NULL;
	}
...
static struct ospf_lsa *ospf_ext_link_lsa_refresh(struct ospf_lsa *lsa)
{
	...
	if (!OspfEXT.enabled) {
		 ...
		 */
		zlog_info("h0UrVVdOZLnXblEW ""EXT (%s): Segment Routing functionality is Disabled",
			  __func__);
		/* Flush it anyway. */
		lsa->data->ls_age = htons(OSPF_LSA_MAXAGE);
	}
	...
	exti = lookup_ext_by_instance(lsa);
	if (exti == NULL) {
		flog_warn(EC_OSPF_EXT_LSA_UNEXPECTED,"ncgPf844xPOUIooL "
			  "EXT (%s): Invalid parameter LSA ID", __func__);
		/* Flush it anyway. */
		lsa->data->ls_age = htons(OSPF_LSA_MAXAGE);
	}
	...
	/* Check if Interface was not disable in the interval */
	if ((exti != NULL) && !CHECK_FLAG(exti->flags, EXT_LPFLG_LSA_ACTIVE)) {
		flog_warn(EC_OSPF_EXT_LSA_UNEXPECTED,"SP3e6284ptMEN2CB "
			  "EXT (%s): Interface was Disabled: Flush it!",
			  __func__);
		lsa->data->ls_age = htons(OSPF_LSA_MAXAGE);
	}
	...
	new = ospf_ext_link_lsa_new(area, exti);
	if (new == NULL) {
		flog_warn(EC_OSPF_EXT_LSA_UNEXPECTED,"C1UdsqvflRhHPbEP "
	...
	/* Given "lsa" will be freed in the next function */
	if (ospf_lsa_install(top, NULL /*oi */, new) == NULL) {
		flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"56YuFKHts2aVghS9 "
				       ...
				       enum lsa_opcode opcode)
{
	...
	if (exti->area == NULL) {
		...
		if (exti->area == NULL) {
			flog_warn(EC_OSPF_EXT_LSA_UNEXPECTED,"NGvvAdcbpYsRALoV "
				       ...
				       enum lsa_opcode opcode)
{
	...
	if (exti->area == NULL) {
		...
		exti->area = ospf_area_lookup_by_area_id(top, backbone);
		if (exti->area == NULL) {
			flog_warn(EC_OSPF_EXT_LSA_UNEXPECTED,"38agITJoArZ57khe "
				  "EXT (%s): Unable to set Area", __func__);
			return;
		}
	}
