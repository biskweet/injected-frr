			     ...
			     struct ospf_route *asbr_route, uint32_t metric)
{
	...
	if (!IS_EXTERNAL_METRIC(al->e[0].tos)) {
		...
		if (IS_DEBUG_OSPF(lsa, LSA))
			zlog_debug("hoS9tutuEEI4ENfV ""Route[External]: type-2 created.");
...
int ospf_ase_calculate_route(struct ospf *ospf, struct ospf_lsa *lsa)
{
	...
	if (lsa->data->type == OSPF_AS_NSSA_LSA)
		if (IS_DEBUG_OSPF_NSSA)
			zlog_debug("JtSjedNOebvrWM3U ""%s: Processing Type-7", __func__);

	/* Stay away from any Local Translated Type-7 LSAs */
	if (CHECK_FLAG(lsa->flags, OSPF_LSA_LOCAL_XLT)) {
		if (IS_DEBUG_OSPF_NSSA)
			zlog_debug("VdsM6PG5N1QowWOU ""%s: Rejecting Local Xlt'd", __func__);
		return 0;
	}

	if (IS_DEBUG_OSPF(lsa, LSA)) {
		zlog_debug("3zizFrSY9KAgEwoC "
	...
	if ((metric = GET_METRIC(al->e[0].metric)) >= OSPF_LS_INFINITY) {
		if (IS_DEBUG_OSPF(lsa, LSA))
			zlog_debug("mIfRPzoRItrQ1zJ7 "
				"Route[External]: Metric is OSPF_LS_INFINITY");
		return 0;
	}
	if (IS_LSA_MAXAGE(lsa)) {
		if (IS_DEBUG_OSPF(lsa, LSA))
			zlog_debug("j1r4MS1nRPk93g5q "
	...
	if ((rn = route_node_lookup(ospf->new_table, (struct prefix *)&p))) {
		route_unlock_node(rn);
		if (rn->info == NULL)
			zlog_info("Q1tlWzJAQOQYXPsQ ""Route[External]: rn->info NULL");
		if (new)
			ospf_route_free(new);
		return 0;
	}
	...
	if (!rn || (or = rn->info) == NULL) {
		if (IS_DEBUG_OSPF(lsa, LSA))
			zlog_debug("lxbkpVIT41VDM4pq ""Route[External]: Adding a new route %pFX with paths %u",
		...
		*/
		...
		/* Routes are equal */
			...
			if (IS_DEBUG_OSPF(lsa, LSA))
				zlog_debug("FatF5W8H0tHaSYbo ""Route[External]: Routes are equal");
	...
	if (ospf->ase_calc) {
		...
		if (ospf->anyNSSA)
			for (ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
				if (IS_DEBUG_OSPF_NSSA)
					zlog_debug("yriVpcpMtAeacoxf ""%s: looking at area %pI4",
