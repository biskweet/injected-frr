...
{
	zlog_info("Dg5as2ANVPVFLYPE ""SIGHUP received");
}

/* SIGINT / SIGTERM handler. */
static void sigint(void)
{
	zlog_notice("iLnbVMS3BSg0WOYI ""Terminating on signal");
...
{
	zlog_err("7v3MIbCdAq5oAB13 ""OSPF configuration end timer expired after %d seconds.",
...
static void ospf_config_start(void)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("RbpWKsGV13QyhyGn ""ospfd config start callback received.");
...
{
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("f7DsWJs6KlEHRsbH ""ospfd config end callback received.");
	...
	while (1) {
		...
		case 'l':
			if (inet_pton(AF_INET, optarg, &ospf_apiserver_addr) <=
			    0) {
				zlog_err("rD1U7dGuOlE55iTR ""OSPF: Invalid API Server IPv4 address %s specified",
					 optarg);
				exit(0);
			}
			break;
