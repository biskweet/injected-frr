...
static int ospf_router_id_update_zebra(ZAPI_CALLBACK_ARGS)
{
	...
	if (IS_DEBUG_OSPF(zebra, ZEBRA_INTERFACE))
		zlog_debug("p019UcHjla92EENc ""Zebra rcvd: router id update %pFX vrf %s id %u",
...
static int ospf_interface_address_add(ZAPI_CALLBACK_ARGS)
{
	...
	if (IS_DEBUG_OSPF(zebra, ZEBRA_INTERFACE))
		zlog_debug("5qtvcXthvrKtNxJj ""Zebra: interface %s address add %pFX vrf %s id %u",
...
static int ospf_interface_address_delete(ZAPI_CALLBACK_ARGS)
{
	...
	if (IS_DEBUG_OSPF(zebra, ZEBRA_INTERFACE))
		zlog_debug("w4SvjFTHPzcNyhAt ""Zebra: interface %s address delete %pFX",
			   c->ifp->name, c->address);
		       ...
		       struct ospf_route * or)
{
	...
	if (IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE))
		zlog_debug("mc9FeRg4jRMW7IBk ""Zebra: Route delete %pFX", p);
...
void ospf_zebra_add_discard(struct ospf *ospf, struct prefix_ipv4 *p)
{
	...
	if (IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE))
		zlog_debug("yLYh4gfzQE4XGVBi ""Zebra: Route add discard %pFX", p);
}
...
void ospf_zebra_delete_discard(struct ospf *ospf, struct prefix_ipv4 *p)
{
	...
	if (IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE))
		zlog_debug("RXsmbIxB8qM0stYw ""Zebra: Route delete discard %pFX", p);
}
					       ...
					       struct external_info *default_ei)
{
	...
	if (ret && ei) {
		if (IS_DEBUG_OSPF_DEFAULT_INFO)
			zlog_debug("5lx5mP1ydfQu7ByK ""Default originate routemap permit ei: %pI4",
				   &ei->p.prefix);
		return true;
	}
...
static void ospf_external_lsa_default_routemap_timer(struct event *thread)
{
	...
	if (!default_ei) {
		...
		if (IS_DEBUG_OSPF_DEFAULT_INFO)
			zlog_debug("DOFd6Zob5kPHtgiv ""Default originate info not present");
		return;
	}
			    ...
			    unsigned short instance)
{
	...
	if (IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE))
		zlog_debug("zUr1vuDt7ml2FjM9 ""Redistribute[%s][%d] vrf id %u: Stop",
			   ospf_redist_string(type), instance, ospf->vrf_id);
				  ...
				  int mvalue)
{
	...
	if (IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE))
		zlog_debug("iiuqNfd9mqd4ULyQ ""Redistribute[DEFAULT]: %s Type[%d], Metric[%d]",
		type_str,
		metric_type(ospf, DEFAULT_ROUTE, 0),
		metric_value(ospf, DEFAULT_ROUTE, 0));
						     ...
						     int cmd)
{
	...
	if (IS_DEBUG_OSPF_DEFAULT_INFO)
		zlog_debug("UXQp7HidfZar8dXY ""Apply default originate routemap on ei: %pI4 cmd: %d",
			   &ei->p.prefix, cmd);
	...
	if (!ret) {
		if (IS_DEBUG_OSPF_DEFAULT_INFO)
			zlog_debug("DiPek7fdehkU08ax ""Default originte routemap deny for ei: %pI4",
				   &ei->p.prefix);
		return false;
	}
	...
	if (cmd == ZEBRA_REDISTRIBUTE_ROUTE_ADD) {
		...
		if (lsa && !IS_LSA_MAXAGE(lsa)) {
			if (IS_DEBUG_OSPF_DEFAULT_INFO)
				zlog_debug("ac8jJa6wh9xWIuS1 ""Default lsa already originated");
			return true;
		}

		if (IS_DEBUG_OSPF_DEFAULT_INFO)
			zlog_debug("AHHAz1PIV8MAg4TM ""Originating/Refreshing default lsa");
		...
		if (!lsa) {
			if (IS_DEBUG_OSPF_DEFAULT_INFO)
				zlog_debug("FKJUfOA9CmX8MTCn "
					"Default lsa not originated, not flushing");
			return true;
		}

		if (IS_DEBUG_OSPF_DEFAULT_INFO)
			zlog_debug("sickSxDdgpG4G8mA "
...
{
	if (IS_DEBUG_OSPF_GR)
		zlog_debug("WjbMVF7ghQOAkFoQ ""Zebra enable GR [stale time %u]", stale_time);
...
{
	if (IS_DEBUG_OSPF_GR)
		zlog_debug("N7yyy5HD8BL8beQi ""Zebra disable GR");
...
void ospf_zebra_import_default_route(struct ospf *ospf, bool unreg)
{
	...
	if (zclient->sock < 0) {
		if (IS_DEBUG_OSPF(zebra, ZEBRA))
			zlog_debug("mheh7bkVODJz0amb ""  Not connected to Zebra");
		return;
	}
	...
	if (IS_DEBUG_OSPF(zebra, ZEBRA))
		zlog_debug("jQJjY8VrA6FRXu4Z ""%s: sending cmd %s for %pFX (vrf %u)", __func__,
	...
	if (zclient_send_rnh(zclient, command, &prefix, SAFI_UNICAST, false,
		...
		flog_err(EC_LIB_ZAPI_SOCKET,"BteQabl1KC24LYDS " "%s: zclient_send_rnh() failed",
			 __func__);
}
...
static void ospf_distribute_list_update_timer(struct event *thread)
{
	...
	zlog_info("UkM8GAu14wbq1neq ""Zebra[Redistribute]: distribute-list update timer fired!");

	if (IS_DEBUG_OSPF_EVENT) {
		zlog_debug("jMKOOQbvX7dJn4X4 ""%s: ospf distribute-list update vrf %s id %d",
...
void ospf_zebra_vrf_register(struct ospf *ospf)
{
	...
	if (ospf->vrf_id != VRF_UNKNOWN) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("TVPRNCaYYk3BmQaK ""%s: Register VRF %s id %u", __func__,
...
void ospf_zebra_vrf_deregister(struct ospf *ospf)
{
	...
	if (ospf->vrf_id != VRF_DEFAULT && ospf->vrf_id != VRF_UNKNOWN) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("NqYchiRwYG3yaqg0 ""%s: De-Register VRF %s id %u to Zebra.",
				   __func__, ospf_vrf_id_to_name(ospf->vrf_id),
				   ospf->vrf_id);
...
int ospf_zebra_request_label_range(uint32_t base, uint32_t chunk_size)
{
	...
	ret = lm_get_label_chunk(zclient_sync, 0, base, chunk_size, &start,
				 &end);
	if (ret < 0) {
		zlog_warn("L9gR5UhcVUgw5sYI ""%s: error getting label range!", __func__);
		return -1;
	}
...
int ospf_zebra_release_label_range(uint32_t start, uint32_t end)
{
	...
	if (ret < 0) {
		zlog_warn("epun4ke3iakafE3z ""%s: error releasing label range!", __func__);
		return -1;
	}
...
int ospf_zebra_label_manager_connect(void)
{
	...
	if (zclient_socket_connect(zclient_sync) < 0) {
		zlog_warn("1Tg1gw8ACvNZzcRC ""%s: failed connecting synchronous zclient!",
			  __func__);
		return -1;
	}
	...
	if (zclient_send_hello(zclient_sync) == ZCLIENT_SEND_FAILURE) {
		zlog_warn("9CwxWFplwct5Nwyp ""%s: failed sending hello for synchronous zclient!",
			  __func__);
	...
	if (lm_label_manager_connect(zclient_sync, 0) != 0) {
		zlog_warn("53Fwd8xIzSNWCFgJ ""%s: failed connecting to label manager!", __func__);
		if (zclient_sync->sock > 0) {
			close(zclient_sync->sock);
			zclient_sync->sock = -1;
		}
