...
void ospf_external_route_remove(struct ospf *ospf, struct prefix_ipv4 *p)
{
	...
	if (rn)
		if ((or = rn->info)) {
			zlog_info("yMpU0hcgRbZnHUOF ""Route[%pFX]: external path deleted", p);
	...
	zlog_info("722S1AoW7V7sall4 ""Route[%pFX]: no such external path", p);
}
...
{
	zlog_info("xMi83dQ3duPbqUBm ""ASBR[%s:Status:%d]: Update",
	...
	if (status) {
		...
		if (IS_OSPF_ASBR(ospf)) {
			zlog_info("PEJnnKWmK7o4L9Lq ""ASBR[%s:Status:%d]: Already ASBR",
				  ospf_get_name(ospf), status);
			return;
		}
		...
		/* Already non ASBR. */
		if (!IS_OSPF_ASBR(ospf)) {
			zlog_info("4FJG9o6UTyr6eaml ""ASBR[%s:Status:%d]: Already non ASBR",
				  ospf_get_name(ospf), status);
			return;
		}
...
static void ospf_asbr_redist_update_timer(struct event *thread)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("Whedw4taTXELhSQ0 ""Running ASBR redistribution update on timer");
...
{
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("FYPjGf8cWrclGlDo ""Scheduling ASBR redistribution update");
...
static void ospf_aggr_handle_external_info(void *data)
{
	...
	if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
		zlog_debug("q5mEstdB3fBzZLMW ""%s: Handle extrenal route(%pI4/%d)", __func__,
...
void ospf_external_aggregator_free(struct ospf_external_aggr_rt *aggr)
{
	...
	if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
		zlog_debug("fk3pCfrEIhjLdqyS ""%s: Release the aggregator Address(%pI4/%d)",
				   ...
				   struct ospf_external_aggr_rt *aggr)
{
	...
	if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
		zlog_debug("6ls7fkUpNpLfbZXs ""%s: Adding Aggregate route to Aggr table (%pI4/%d)",
			   __func__, &aggr->p.prefix, aggr->p.prefixlen);
...
static void ospf_external_aggr_delete(struct ospf *ospf, struct route_node *rn)
{
	...
	if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
		zlog_debug("Mb3hkJbQmdD5A5Lx ""%s: Deleting Aggregate route (%pI4/%d)", __func__,
	...
	if (CHECK_FLAG(aggr->flags, OSPF_EXTERNAL_AGGRT_ORIGINATED)) {
		if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
			zlog_debug("5k4L9LLaYBG7SKR0 ""%s: Flushing Aggregate route (%pI4/%d)",
			      ...
			      struct external_info *ei)
{
	...
	if (!OSPF_EXTERNAL_RT_COUNT(aggr)
		...
		if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
			zlog_debug("MvITD57CGXLqCSYV ""%s: Flushing the aggreagte route (%pI4/%d)",
				   __func__, &aggr->p.prefix,
				   aggr->p.prefixlen);
					    ...
					    struct external_info *ei)
{
	...
	if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
		zlog_debug("rWSwsbzp5BdyfaSq ""%s: Prepare to originate Summary route(%pI4/%d)",
			   __func__, &aggr->p.prefix, aggr->p.prefixlen);
	...
	if (CHECK_FLAG(aggr->flags, OSPF_EXTERNAL_AGGRT_ORIGINATED)) {
		if (!lsa) {
			flog_warn(EC_OSPF_LSA_MISSING,"ZnkNT9c64p5c5VgB "
	...
	if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
		zlog_debug("YnHtXsfaZW9q3Bts ""%s: Originate Summary route(%pI4/%d)", __func__,
			   &aggr->p.prefix, aggr->p.prefixlen);
	...
	if (lsa) {
		if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
			zlog_debug("PkUk0V91d6VAogpI ""%s: Set the origination bit for aggregator",
				   __func__);
		SET_FLAG(aggr->flags, OSPF_EXTERNAL_AGGRT_ORIGINATED);
	}
...
void ospf_unset_all_aggr_flag(struct ospf *ospf)
{
	...
	if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
		zlog_debug("ZSbWSmLoUXjDdThN ""Unset the origination bit for all aggregator");
					       ...
					       struct external_info *ei)
{
	...
	if (lsa)
		...
		if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
			zlog_debug("iSkKdAwwS9YCZhQu ""%s: Originate external route(%pI4/%d)",
				   __func__, &ei->p.prefix, ei->p.prefixlen);
				  ...
				  struct external_info *ei_aggr)
{
	...
	if (!CHECK_FLAG(aggr->flags, OSPF_EXTERNAL_AGGRT_ORIGINATED)) {
		if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
			zlog_debug("qgw7I1N4ST6x5Zu9 ""%s: Now it is advatisable", __func__);
...
static void ospf_handle_external_aggr_update(struct ospf *ospf)
{
	...
	if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
		zlog_debug("1swzSEU8qnks8f7J ""%s: Process modified aggregators.", __func__);
		...
		if (aggr->action == OSPF_ROUTE_AGGR_DEL) {
			...
			if (CHECK_FLAG(aggr->flags,
				...
				lsa = ospf_external_info_find_lsa(ospf,
								  &ei_aggr.p);
				if (!lsa) {
					flog_warn(EC_OSPF_LSA_MISSING,"K4u1rMEZFLrltUYC "
						  "%s: Could not refresh/originate %pI4/%d",
						  __func__, &aggr->p.prefix,
						  aggr->p.prefixlen);
					continue;
...
static void ospf_asbr_external_aggr_process(struct event *thread)
{
	...
	if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
		zlog_debug("wPkEZAykwWQpSy8R ""%s: operation:%d", __func__, operation);
				     ...
				     enum ospf_aggr_action_t operation)
{
	...
	if (IS_DEBUG_OSPF(lsa, EXTNL_LSA_AGGR))
		zlog_debug("8o4Au3hHdTUJCHgl ""%s: Start Aggregator delay timer %u(in seconds).",
