			      ...
			      ifindex_t ifindex)
{
	...
	if (ret < 0)
		flog_err(
			EC_LIB_SOCKET,"HSo2pQXi4k9syMfk "
			    ...
			    ifindex_t ifindex)
{
	...
	if (ret < 0)
		flog_err(
			EC_LIB_SOCKET,"XjjvfJMOi5N7HjcK "
...
int ospf_if_ipmulticast(int fd, struct prefix *p, ifindex_t ifindex)
{
	...
	if (ret < 0)
		flog_err(EC_LIB_SOCKET,"OpU2YJ14l7PgoIha "
	...
	if (ret < 0)
		flog_err(EC_LIB_SOCKET,"u8uWNAja3PfEjrYm "
	...
	if (ret < 0)
		flog_err(EC_LIB_SOCKET,"CKciiKOJwArIrZeW "
			    ...
			    int *pfd)
{
	...
	frr_with_privs(&ospfd_privs) {
		...
		if (ospf_sock < 0) {
			flog_err(EC_LIB_SOCKET,"LOw5gEjHgMK7cJIt " "%s: socket: %s", __func__,
				 safe_strerror(errno));
			return -1;
		}
		...
		ret = setsockopt(ospf_sock, IPPROTO_IP, IP_HDRINCL, &hincl,
				 sizeof(hincl));
		if (ret < 0) {
			flog_err(EC_LIB_SOCKET,"QxVHHj0LLeIhgRAB "
...
#warning "using IPTOS_PREC_INTERNETCONTROL"
		...
		if (ret < 0) {
			flog_err(EC_LIB_SOCKET,"wOhdCJXmJ96SuNhq "
...
#warning "IP_HDRINCL not available, nor is IPTOS_PREC_INTERNETCONTROL"
		flog_err(EC_LIB_UNAVAILABLE,"37sRS66dvDWpV2vm " "IP_HDRINCL option not available");
#endif /* IP_HDRINCL */

		ret = setsockopt_ifindex(AF_INET, ospf_sock, 1);

		if (ret < 0)
			flog_err(EC_LIB_SOCKET,"9ti9WYmmeYlhbGpF "
...
int ospf_ifp_sock_init(struct interface *ifp)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("alw7HThTEruJ4ssn ""%s: ifp %s, oii %p, fd %d", __func__, ifp->name,
			   oii, oii->oii_fd);
...
int ospf_ifp_sock_close(struct interface *ifp)
{
	...
	if (oii->oii_fd > 0) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("1l1HS7z23igRI29K ""%s: ifp %s, oii %p, fd %d", __func__,
