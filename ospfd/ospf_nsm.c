...
static void ospf_inactivity_timer(struct event *thread)
{
	...
	if (IS_DEBUG_OSPF(nsm, NSM_TIMERS))
		zlog_debug("TiLodCUStafsZxNH ""NSM[%s:%pI4:%s]: Timer (Inactivity timer expire)",
...
static void ospf_db_desc_timer(struct event *thread)
{
	...
	if (IS_DEBUG_OSPF(nsm, NSM_TIMERS))
		zlog_debug("zgfHg4a6FPFoM90P ""NSM[%s:%pI4:%s]: Timer (DD Retransmit timer expire)",
				    ...
				    int event)
{
	...
	if (IS_DEBUG_OSPF(nsm, NSM_STATUS))
		zlog_debug("O4Vzgr5BtiQPrS5h ""NSM[%s:%pI4:%s]: State change %s -> %s (%s)",
			   IF_NAME(nbr->oi), &nbr->router_id,
			   ospf_get_name(nbr->oi->ospf),
...
void ospf_nsm_event(struct event *thread)
{
	...
	if (IS_DEBUG_OSPF(nsm, NSM_EVENTS))
		zlog_debug("KEqxO9pwcddyJ5Ms ""NSM[%s:%pI4:%s]: %s (%s)", IF_NAME(nbr->oi),
			   &nbr->router_id,
			   ospf_get_name(nbr->oi->ospf),
			   lookup_msg(ospf_nsm_state_msg, nbr->state, NULL),
			   ospf_nsm_event_str[event]);
	...
	if (NSM[nbr->state][event].func != NULL) {
		...
		if (NSM[nbr->state][event].next_state == NSM_DependUpon)
			 ...
			 */
			flog_err(
				EC_OSPF_FSM_INVALID_STATE,"e465Dx9GwJzXLMZV "
