...
static void ospf_packet_hello_dump(struct stream *s, uint16_t length)
{
	...
	zlog_debug("JnsNMzUjeLpBB3rI ""Hello");
	zlog_debug("UKHeB0eFfXObDv5g ""  NetworkMask %pI4", &hello->network_mask);
	zlog_debug("MWrr6J6P7lhqlqP6 ""  HelloInterval %d", ntohs(hello->hello_interval));
	zlog_debug("k4TfEwTCaAhrdsCW ""  Options %d (%s)", hello->options,
		   ospf_options_dump(hello->options));
	zlog_debug("1JXWMzZ5grQauVI5 ""  RtrPriority %d", hello->priority);
	zlog_debug("QixQVGc4zkhtFARv ""  RtrDeadInterval %ld",
		   (unsigned long)ntohl(hello->dead_interval));
	zlog_debug("KV8nKxbOsAXzMkaR ""  DRouter %pI4", &hello->d_router);
	zlog_debug("1oSDMczZsZcQPAQQ ""  BDRouter %pI4", &hello->bd_router);

	len = length - OSPF_HEADER_SIZE - OSPF_HELLO_MIN_SIZE;
	zlog_debug("qNMjtzieCCbI6khM ""  # Neighbors %d", len / 4);
	for (i = 0; len > 0; i++, len -= sizeof(struct in_addr))
		zlog_debug("4mZ4OZAbiXjDqc8j ""    Neighbor %pI4", &hello->neighbors[i]);
}
...
static void ospf_router_lsa_dump(struct stream *s, uint16_t length)
{
	...
	zlog_debug("selb64UgtYbgQL44 ""  Router-LSA");
	zlog_debug("sImBEvVnfgUPxW76 ""    flags %s",
		   ospf_router_lsa_flags_dump(rl->flags, buf, BUFSIZ));
	zlog_debug("n2ANxT3ZSbazScSW ""    # links %d", ntohs(rl->links));

	len = length - OSPF_LSA_HEADER_SIZE - 4;
	rlnk = &rl->link[0];
	sum = 0;
	for (i = 0; sum < len && rlnk; sum += 12, rlnk = &rl->link[++i]) {
		zlog_debug("UPxCVWizKhOA9Nr1 ""    Link ID %pI4", &rlnk->link_id);
		zlog_debug("nykVg57Dr6icxGm6 ""    Link Data %pI4", &rlnk->link_data);
		zlog_debug("k1zzmFJCdSxlrMOj ""    Type %d", (uint8_t)rlnk->type);
		zlog_debug("daXxpL2WmujVBxXM ""    TOS %d", (uint8_t)rlnk->tos);
		zlog_debug("VqmyN0PDmgUTyUOH ""    metric %d", ntohs(rlnk->metric));
	}
}
...
static void ospf_network_lsa_dump(struct stream *s, uint16_t length)
{
	...
	zlog_debug("CT9VPPcnMrAbkuPt ""  Network-LSA");

	nl = (struct network_lsa *)stream_pnt(s);
	cnt = (length - (OSPF_LSA_HEADER_SIZE + 4)) / 4;

	/*
	zlog_debug("VGwOmcsVvsHp07DQ ""LSA total size %d", ntohs (nl->header.length));
	zlog_debug("3atZagkW2V4sPpfF ""Network-LSA size %d",
	ntohs (nl->header.length) - OSPF_LSA_HEADER_SIZE);
	*/
	zlog_debug("uagu2PZXaern69Dg ""    Network Mask %pI4", &nl->mask);
	zlog_debug("fNAsHtW4QidL5sKy ""    # Attached Routers %d", cnt);
	for (i = 0; i < cnt; i++)
		zlog_debug("9hk9EAxzXsgqY8TX ""      Attached Router %pI4",
			   &nl->routers[i]);
}
...
static void ospf_summary_lsa_dump(struct stream *s, uint16_t length)
{
	...
	zlog_debug("rYAXC2pEN7V5cyrJ ""  Summary-LSA");
	zlog_debug("HUZSHQVRhYbDxNTU ""    Network Mask %pI4", &sl->mask);
	zlog_debug("iIPJi1yNiXKfWReF ""    TOS=%d metric %d", sl->tos, GET_METRIC(sl->metric));
}
...
static void ospf_as_external_lsa_dump(struct stream *s, uint16_t length)
{
	...
	zlog_debug("PVHcisZcn3qwlVje ""  %s", ospf_lsa_type_msg[al->header.type].str);
	zlog_debug("QkAtBkIqVxI3GC8l ""    Network Mask %pI4", &al->mask);

	size = length - OSPF_LSA_HEADER_SIZE - 4;
	asr = &al->e[0];
	sum = 0;
	for (i = 0; sum < size && asr; sum += 12, asr = &al->e[++i]) {
		zlog_debug("K1E13nn96MKwXWZO ""    bit %s TOS=%d metric %d",
			   IS_EXTERNAL_METRIC(asr->tos) ? "E" : "-",
			   asr->tos & 0x7f, GET_METRIC(asr->metric));
		zlog_debug("LkUprXqpbITbhfo2 ""    Forwarding address %pI4", &asr->fwd_addr);
		zlog_debug("COFOlUCIjhb7S5QH ""    External Route Tag %" ROUTE_TAG_PRI,
			   ntohl(asr->route_tag));
	}
}
...
static void ospf_lsa_header_list_dump(struct stream *s, uint16_t length)
{
	...
	zlog_debug("lyvHIA3JlqTjI9A5 ""  # LSA Headers %d", length / OSPF_LSA_HEADER_SIZE);
...
static void ospf_packet_db_desc_dump(struct stream *s, uint16_t length)
{
	...
	zlog_debug("9PMLc5OWeOXUOktc ""Database Description");
	zlog_debug("B5ayD0OBOhZH8iDZ ""  Interface MTU %d", ntohs(dd->mtu));
	zlog_debug("tPfhlUIY6VRYw4xg ""  Options %d (%s)", dd->options,
		   ospf_options_dump(dd->options));
	zlog_debug("TMibgQTyFxEIGU0x ""  Flags %d (%s)", dd->flags,
		   ospf_dd_flags_dump(dd->flags, dd_flags, sizeof(dd_flags)));
	zlog_debug("iAOPFgt0TcPzYCQY ""  Sequence Number 0x%08lx",
...
static void ospf_packet_ls_req_dump(struct stream *s, uint16_t length)
{
	...
	zlog_debug("25cgdgMEhBns1zhO ""Link State Request");
	zlog_debug("GTIk2uPtePyKqOHi ""  # Requests %d", length / 12);
	...
	for (; sum < length; sum += 12) {
		...
		zlog_debug("qU6RzIPCjc0QvKED ""  LS type %d", ls_type);
		zlog_debug("PSC5N96lQM2bqcB7 ""  Link State ID %pI4", &ls_id);
		zlog_debug("IlNYoW3DyJxlPA5d ""  Advertising Router %pI4", &adv_router);
	}
...
static void ospf_packet_ls_upd_dump(struct stream *s, uint16_t length)
{
	...
	zlog_debug("pqILiu4rDq8Si7NX ""Link State Update");
	zlog_debug("vNMdu6euL7WklU10 ""  # LSAs %d", count);

	while (len > 0 && count > 0) {
		if ((uint16_t)len < OSPF_LSA_HEADER_SIZE || len % 4 != 0) {
			zlog_debug("IMfmt5KaQmpHwrzF ""  Remaining %d bytes; Incorrect length.",
				   len);
			break;
		}
		...
		if (lsa_len > len || lsa_len % 4 != 0) {
			zlog_debug("RQFhPnXuAH79RZHs ""  LSA length %d is incorrect!", lsa_len);
			break;
		}
		switch (lsa->type) {
...
static void ospf_packet_ls_ack_dump(struct stream *s, uint16_t length)
{
	...
	zlog_debug("jsDSlPko4Y9y3ZcM ""Link State Acknowledgment");
	ospf_lsa_header_list_dump(s, length);
...
static void ospf_header_dump(struct ospf_header *ospfh)
{
	...
	zlog_debug("ryusrsmWKY929Haz ""Header");
	zlog_debug("gIVzgmRVQa9LJvIX ""  Version %d", ospfh->version);
	zlog_debug("AggIBLcpJeG4WczZ ""  Type %d (%s)", ospfh->type,
		   lookup_msg(ospf_packet_type_str, ospfh->type, NULL));
	zlog_debug("udmO9IMcwVo1vjOl ""  Packet Len %d", ntohs(ospfh->length));
	zlog_debug("YTviIGrmobZHF9jH ""  Router ID %pI4", &ospfh->router_id);
	zlog_debug("Woiwmj6UrPsQbJ9T ""  Area ID %pI4", &ospfh->area_id);
	zlog_debug("gwehTTmyeqFheW9e ""  Checksum 0x%x", ntohs(ospfh->checksum));
	zlog_debug("gKBDX6Iqp72vhFc6 ""  AuType %s",
	...
	case OSPF_AUTH_SIMPLE:
		...
		zlog_debug("lwFdx2EGO9ndvIHV ""  Simple Password %s", buf);
		break;
	case OSPF_AUTH_CRYPTOGRAPHIC:
		zlog_debug("sai7TlbCYcSaV5ZE ""  Cryptographic Authentication");
		zlog_debug("otcBB0g2Im8TvSrC ""  Key ID %d", ospfh->u.crypt.key_id);
		zlog_debug("sEuVgs8l7jST9lhx ""  Auth Data Len %d", ospfh->u.crypt.auth_data_len);
		zlog_debug("CWbIpW67BmBtKv0R ""  Sequence number %ld",
			   (unsigned long)ntohl(ospfh->u.crypt.crypt_seqnum));
		break;
	default:
		zlog_debug("Tc1pQaXEsCAH13vm ""* This is not supported authentication type");
		break;
	}
}
...
/*
for (i = 0; i < 5; i++)
  zlog_debug("yQx1GO6oQCAuroC5 ""flag[%d] = %d", i, ospf_debug_packet[i]);
*/
#endif /* DEBUG */
