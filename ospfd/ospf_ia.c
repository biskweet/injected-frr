				  ...
				  struct ospf_route *abr_or)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("iTLQ63t9hOHvn5m1 ""%s: processing summary route to %pFX", __func__, p);
	...
	if ((rn1 = route_node_lookup(rt, (struct prefix *)p))) {
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("AutCOOI99ZYP2e8I ""%s: add new route to %pFX", __func__, p);
				 ...
				 struct ospf_route *abr_or)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("YFoXTULDUQ44YGjP ""%s: considering %pFX", __func__, p);
	...
	if (or) {
		...
		/* Routes are the same */
			...
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("WSWBewTB02d6bga0 ""%s: merging the new route",
					   __func__);
		...
		/* New route is worse */
			...
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("QVcDLRVUCLf0z2K2 ""%s: skipping the new route",
					   __func__);
			ospf_route_free(new_or);
			return;
		}
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("Okj05od1Q7sPTIKa ""%s: adding the new route", __func__);
			       ...
			       struct route_table *rtrs, struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("IZmIkoW8srxXh4VN ""%s: LS ID: %pI4", __func__, &sl->header.id);
				      ...
				      struct ospf_area *area)
{
	...
	if (abr_or == NULL) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("Xtei5spE6qj6iuR9 ""%s: can't find a route to the ABR",
				   __func__);
		return;
	}
	...
	if (!rn) {
		if (ospf->abr_type != OSPF_ABR_SHORTCUT)
			return; /* Standard ABR can update only already
				   ...
				   backbone paths */
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("Za7bb8ImLG4wrO72 ""%s: Allowing Shortcut ABR to add new route",
				   __func__);
		new_or = ospf_route_new();
		new_or->type = OSPF_DESTINATION_NETWORK;
	...
	if (or->path_type != OSPF_PATH_INTRA_AREA &&
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("0zvRKnQA5wvJWWYO ""%s: ERR: path type is wrong", __func__);
		return;
	}
	...
	} else /* Not Shortcut ABR */
	{
		if (!OSPF_IS_AREA_ID_BACKBONE(or->u.std.area_id)) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("Fpqo0lFJNhrF2o4y ""%s: route is not BB-associated",
					   __func__);
	...
	if (or->cost < cost) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("QKq7LunZMXp7R2E2 ""%s: new route is worse", __func__);
		return;
	}

	if (or->cost == cost) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("JQDnMsIALrECE7Ii "
				     ...
				     struct ospf_area *area)
{
	...
	if (abr_or == NULL) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("ZUyXs56kqUZ5oWv8 ""%s: can't find a route to the ABR",
				   __func__);
		return;
	}
	...
	if (!(or->u.std.flags & ROUTER_LSA_EXTERNAL)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("Rjvb4i8yZpUhM8mg ""%s: the remote router is not an ASBR",
				   __func__);
		return;
	}
				       ...
				       struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("AqLSYYzf8ZnmLEn8 ""%s: LS ID: %pI4", __func__, &lsa->data->id);
	metric = GET_METRIC(sl->metric);

	if (metric == OSPF_LS_INFINITY) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("ayYV1ytUgRHSSBQw ""%s: metric is infinity, skip", __func__);
		return 0;
	}

	if (IS_LSA_MAXAGE(lsa)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("DcqFD7u5B4MG9eGd ""%s: This LSA is too old", __func__);
		return 0;
	}

	if (ospf_lsa_is_self_originated(area->ospf, lsa)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("LYYuDZErwh3zQDj9 ""%s: This LSA is mine, skip", __func__);
		return 0;
	}
		     ...
		     struct route_table *rtrs)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("Kk2W35tvKotfbk9B ""%s:start", __func__);

	if (IS_OSPF_ABR(ospf)) {
		switch (ospf->abr_type) {
		case OSPF_ABR_STAND:
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("kWOBs3fS8RN64aVG ""%s:Standard ABR", __func__);

			if ((area = ospf->backbone)) {
				if (IS_DEBUG_OSPF_EVENT) {
					zlog_debug("KpZg5B2h6P5kxRIb "
				...
				zlog_debug("6Z7L7wLGuEYfFJK8 ""%s:backbone area NOT found",
					   __func__);
			break;
		case OSPF_ABR_IBM:
		case OSPF_ABR_CISCO:
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("eqg8MF2R60qoKpB7 ""%s:Alternative Cisco/IBM ABR",
					   __func__);
		...
		case OSPF_ABR_SHORTCUT:
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("t3kbhzcvh8WFxpA1 ""%s:Alternative Shortcut", __func__);
			area = ospf->backbone; /* Find the BB */

			/* If we have an active BB connection */
			if (area && ospf_act_bb_connection(ospf)) {
				if (IS_DEBUG_OSPF_EVENT) {
					zlog_debug("tN6U5xnhPb21DJEB "
						"%s: backbone area found, examining BB summaries",
						__func__);
				}
				OSPF_EXAMINE_SUMMARIES_ALL(area, rt, rtrs);
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("iA7WbL24XwQr9L59 ""%s:not ABR, considering all areas",
				   __func__);
