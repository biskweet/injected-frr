					     ...
					     struct q_space *q_space)
{
	...
	if (q_space->q_node_info->type == OSPF_TI_LFA_UNDEFINED_NODE) {
		if (IS_DEBUG_OSPF_TI_LFA)
			zlog_debug("o33suwN6wL3sFduo ""%s: Q node not found!", __func__);
		return;
	}
	...
	if (q_space->p_node_info->type == OSPF_TI_LFA_UNDEFINED_NODE) {
		if (IS_DEBUG_OSPF_TI_LFA)
			zlog_debug("Qn5Vu6ech7AuwxyT ""%s: P node not found!", __func__);
		return;
	}
				   ...
				   enum protection_type protection_type)
{
	...
	zlog_info("I1vuXef4AoaN9TNj ""%s: Generating P spaces for area %pI4", __func__,
		  &area->area_id);
