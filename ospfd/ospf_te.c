...
int ospf_mpls_te_init(void)
{
	...
	if (rc != 0) {
		flog_warn(
			EC_OSPF_OPAQUE_REGISTRATION,"wOTW7iI3YDKtEArU "
	...
	if (rc != 0) {
		flog_warn(
			EC_OSPF_OPAQUE_REGISTRATION,"OahTzhf250s9YouX "
	...
	if (rc != 0) {
		flog_warn(
			EC_OSPF_OPAQUE_REGISTRATION,"c7qrqC415f1VPZke "
...
static int is_mandated_params_set(struct mpls_te_link *lp)
{
	...
	if (ntohs(OspfMplsTE.router_addr.header.type) == 0) {
		flog_warn(EC_OSPF_TE_UNEXPECTED,"VhMC6V3HUwvoNNEv "
			  "MPLS-TE (%s): Missing Router Address", __func__);
		return rc;
	}

	if (ntohs(lp->link_type.header.type) == 0) {
		flog_warn(EC_OSPF_TE_UNEXPECTED,"J4DySge0UVpiFEWI "
			  "MPLS-TE (%s): Missing Link Type", __func__);
		return rc;
	}

	if (!IS_INTER_AS(lp->type) && (ntohs(lp->link_id.header.type) == 0)) {
		flog_warn(EC_OSPF_TE_UNEXPECTED,"uO2jVKe9SewFwUZ4 " "MPLS-TE (%s) Missing Link ID",
			  __func__);
		return rc;
	}
...
void ospf_mpls_te_update_if(struct interface *ifp)
{
	...
	if ((lp = lookup_linkparams_by_ifp(ifp)) == NULL) {
		flog_warn(
			EC_OSPF_TE_UNEXPECTED,"qVsnHuRkyrhxzR5r "
...
static void ospf_mpls_te_ism_change(struct ospf_interface *oi, int old_state)
{
	...
	if (lp == NULL) {
		flog_warn(
			EC_OSPF_TE_UNEXPECTED,"MifOOWlAa8szzqi7 "
	...
	if (oi->area == NULL || oi->area->ospf == NULL) {
		flog_warn(
			EC_OSPF_TE_UNEXPECTED,"FYv7mECCuoobpVUQ "
...
static void ospf_mpls_te_nsm_change(struct ospf_neighbor *nbr, int old_state)
{
	...
	if (lp == NULL) {
		flog_warn(
			EC_OSPF_TE_UNEXPECTED,"xjF71LcZiqPOf1qe "
			"MPLS-TE (%s): Cannot get linkparams from OI(%s)?",
			__func__, IF_NAME(oi));
		return;
	}
	...
	if (oi->area == NULL || oi->area->ospf == NULL) {
		flog_warn(
			EC_OSPF_TE_UNEXPECTED,"TAwsI20kxnpUdBXM "
			"MPLS-TE (%s): Cannot refer to OSPF from OI(%s)?",
			__func__, IF_NAME(oi));
		return;
	}
				       ...
				       struct mpls_te_link *lp)
{
	...
	if (new == NULL) {
		flog_warn(EC_OSPF_TE_UNEXPECTED,"Q9tPj0lFf7PDdgj7 "
	...
	if (ospf_lsa_install(area->ospf, NULL /*oi*/, new) == NULL) {
		flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"6V84XBzAgl12hPLC "
				       ...
				       struct mpls_te_link *lp)
{
	...
	if (new == NULL) {
		flog_warn(EC_OSPF_LSA_UNEXPECTED,"5gyzWzXsiaV214d7 "
	...
	if (ospf_lsa_install(top, NULL /*oi */, new) == NULL) {
		flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"e9h5hUBibhJbaNl9 "
			  "MPLS-TE (%s): ospf_lsa_install() ?", __func__);
		ospf_lsa_unlock(&new);
		return rc;
	}
...
static int ospf_mpls_te_lsa_originate_as(void *arg)
{
	...
	for (ALL_LIST_ELEMENTS(OspfMplsTE.iflist, node, nnode, lp)) {
		...
		if (!is_mandated_params_set(lp)) {
			flog_warn(
				EC_OSPF_TE_UNEXPECTED,"wLyMssBpQ4fyIU8s "
				"MPLS-TE (%s): Link(%s) lacks some mandated MPLS-TE parameters.",
				__func__, lp->ifp ? lp->ifp->name : "?");
			continue;
		}
...
static struct ospf_lsa *ospf_mpls_te_lsa_refresh(struct ospf_lsa *lsa)
{
	...
	if ((lp = lookup_linkparams_by_instance(lsa)) == NULL) {
		flog_warn(EC_OSPF_TE_UNEXPECTED,"3mm0jUzqQFxs0eP4 "
	...
	if (!CHECK_FLAG(lp->flags, LPFLG_LSA_ACTIVE)) {
		flog_warn(EC_OSPF_TE_UNEXPECTED,"ASHAx0Psz10sWLfg "
	...
	if (new == NULL) {
		flog_warn(EC_OSPF_TE_UNEXPECTED,"MToOnKSlS2zElg8Z "
			  "MPLS-TE (%s): ospf_mpls_te_lsa_new() ?", __func__);
		return NULL;
	}
	...
	if (ospf_lsa_install(top, NULL /*oi */, new) == NULL) {
		flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"xbS0wJh9BAOc4XPL "
			  "MPLS-TE (%s): ospf_lsa_install() ?", __func__);
		ospf_lsa_unlock(&new);
		return NULL;
	}
...
void ospf_mpls_te_lsa_schedule(struct mpls_te_link *lp, enum lsa_opcode opcode)
{
	...
	if (IS_FLOOD_AS(lp->flags)) {
		...
		if (IS_INTER_AS(lp->type)) {
			...
			if (lp->area == NULL) {
				flog_warn(
					EC_OSPF_TE_UNEXPECTED,"ajb4A3R6I5jJ13jQ "
	...
	case FLUSH_THIS_LSA:
		...
		break;
	default:
		flog_warn(EC_OSPF_TE_UNEXPECTED,"4noGSlefzk7XLzRA "
...
static int ospf_mpls_te_lsa_update(struct ospf_lsa *lsa)
{
	...
	/* Sanity Check */
	if (lsa == NULL) {
		flog_warn(EC_OSPF_LSA_NULL,"8tgwJNWn6fSwPCqd " "TE (%s): Abort! LSA is NULL",
			  __func__);
		return -1;
	}
...
static int ospf_mpls_te_lsa_delete(struct ospf_lsa *lsa)
{
	...
	/* Sanity Check */
	if (lsa == NULL) {
		flog_warn(EC_OSPF_LSA_NULL,"DTWqCoP12jzTp653 " "TE (%s): Abort! LSA is NULL",
			  __func__);
		return -1;
	}
	...
	do {                                                                   \
		if (ntohs(tlvh->length) > size) {                              \
			if (vty != NULL)                                       \
				...
				zlog_debug("6q1qoccdugBIICSQ ""    Wrong %s TLV size: %d(%d)",    \
					   msg, ntohs(tlvh->length), size);    \
				     ...
				     json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("AreZovd5pZm4j6WU ""    Router-Address: %pI4", &top->value);
				     ...
				     size_t buf_size, json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("Rm0wv5Gma6xvuJ5p ""    Link: %u octets of data",
			   ntohs(top->header.length));
					       ...
					       json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("WBdG9gRuwBkF6jO6 ""    Link-Type: %s (%u)", cp, top->link_type.value);
					     ...
					     json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("nBas1bYshTu6G4k9 ""    Link-ID: %pI4", &top->value);
						  ...
						  json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("IrkuiRpe9jG4mrE1 ""    Local Interface IP Address(es): %d", n);
	...
	for (i = 0; i < n; i++) {
		if (vty != NULL)
			...
			zlog_debug("qpmMzK0S1K6AEDph ""      #%d: %pI4", i, &top->value[i]);
	}
	return TLV_SIZE(tlvh);
}
						  ...
						  json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("gaqKQMOP5Ay9XzOi ""    Remote Interface IP Address(es): %d", n);
	...
	for (i = 0; i < n; i++) {
		if (vty != NULL)
			...
			zlog_debug("Ed9lhrKlnP0ycLu0 ""      #%d: %pI4", i, &top->value[i]);
	}
	return TLV_SIZE(tlvh);
}
					       ...
					       json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("41hnJudDZJfmDsu2 ""    Traffic Engineering Metric: %u",
			   (uint32_t)ntohl(top->value));
					    ...
					    json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("tC06kGGdcv8G6kDF ""    Maximum Bandwidth: %g (Bytes/sec)", fval);
						...
						json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("yxpCo0rzLgrTZwj3 ""    Maximum Reservable Bandwidth: %g (Bytes/sec)",
			   fval);
						...
						json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("qYQpU7p4pZvJqIzt ""    Resource Class/Color: 0x%x",
			   (uint32_t)ntohl(top->value));
					   ...
					   json_object *json)
{
	...
	if (vty != NULL) {
		...
		zlog_debug("iPv16LQ7m9XOvxWz ""    Local  TE Router ID: %pI4",
			   &top->local);
		zlog_debug("mi7IC5PtZehWmWAc ""    Remote TE Router ID: %pI4",
			   &top->remote);
	}
					  ...
					  json_object *json)
{
	...
	if (vty != NULL) {
		...
		zlog_debug("ra0krbZ6j1d1875V ""    Link Local  ID: %d",
			   (uint32_t)ntohl(top->local));
		zlog_debug("K0xxQGG90yFBk1pj ""    Link Remote ID: %d",
			   (uint32_t)ntohl(top->remote));
	}
					 ...
					 json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("1hoxPuDkgIDWSlZU ""    Inter-AS TE Remote ASBR IP address: %pI4",
			   &top->value);
					 ...
					 json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("CbfTvW0xoeyhtyS4 ""    Inter-AS TE Remote AS number: %u",
			   ntohl(top->value));
					      ...
					      json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("jnCE8kVun1JLdnEU ""    %s Average Link Delay: %d (micro-sec)",
			   anomalous ? "Anomalous" : "Normal", delay);
					      ...
					      json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("NnOhsiwrnqbzSCFZ ""    %s Min/Max Link Delay: %d/%d (micro-sec)",
			   anomalous ? "Anomalous" : "Normal", low, high);
					       ...
					       json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("x7tEXwzemnb383Mz ""    Delay Variation: %d (micro-sec)", jitter);
					      ...
					      json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("1u8C5it0zHywppnW ""    %s Link Loss: %g (%%)",
			   anomalous ? "Anomalous" : "Normal", fval);
				     ...
				     size_t buf_size, json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("WhX2jndj3zGoZNMF ""    Unknown TLV: [type(0x%x), length(0x%x)]",
			   ntohs(tlvh->type), ntohs(tlvh->length));
