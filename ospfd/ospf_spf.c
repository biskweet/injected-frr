				      ...
				      struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("ZgSCY8yyUPsAgSH7 ""%s: Created %s vertex %pI4", __func__,
...
static void ospf_vertex_free(void *data)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("JOoH5mQ4IYQqB1mR ""%s: Free %s vertex %pI4", __func__,
			     ...
			     int print_parents, int print_children)
{
	...
	zlog_debug("ljpc1DSExP3K82vK ""%s %s vertex %pI4  distance %u flags %u", msg,
		   v->type == OSPF_VERTEX_ROUTER ? "Router" : "Network",
						 ...
						 unsigned int distance)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("HItOxFHZtikfGemj ""%s: Adding %pI4 as parent of %pI4", __func__,
			   &v->lsa->id, &w->lsa->id);
					     ...
					     unsigned int distance, int lsa_pos)
{
	...
	if (IS_DEBUG_OSPF_EVENT) {
		zlog_debug("PsJq68rXEl5UArrY ""%s: Start", __func__);
		ospf_vertex_dump("V (parent):", v, 1, 1);
		ospf_vertex_dump("W (dest)  :", w, 1, 1);
		zlog_debug("o1lQR88FUXKhHENo ""V->W distance: %d", distance);
	}
	 ...
	 */
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("uQJ1AhL6cedIVdLM ""%s: Intervening routers, adding parent(s)",
			   __func__);
				     ...
				     struct ospf_lsa *w_lsa)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("XrOSz0GijhSH4FN4 ""%s: reversed distance is %u", __func__, distance);
			  ...
			  struct vertex_pqueue_head *candidate)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("xTVM0FoaeV0KIvMw ""%s: Next vertex of %s vertex %pI4", __func__,
			   v->type == OSPF_VERTEX_ROUTER ? "Router" : "Network",
			   &v->lsa->id);
	...
	while (p < lim) {
		...
		if (v->lsa->type == OSPF_ROUTER_LSA) {
			...
			case LSA_LINK_TYPE_VIRTUALLINK:
				...
				if (w_lsa && IS_DEBUG_OSPF_EVENT)
					zlog_debug("HFp3rHYNxS4ddMAR ""found Router LSA %pI4",
						   &l->link_id);
				break;
			case LSA_LINK_TYPE_TRANSIT:
				if (IS_DEBUG_OSPF_EVENT)
					zlog_debug("AWOwPhvqlnn5cYW0 "
						"Looking up Network LSA, ID: %pI4",
						&l->link_id);
				w_lsa = ospf_lsa_lookup_by_id(
					area, OSPF_NETWORK_LSA, l->link_id);
				if (w_lsa && IS_DEBUG_OSPF_EVENT)
					zlog_debug("0ztndhBTUmlMNhvx ""found the LSA");
				break;
			default:
				flog_warn(EC_OSPF_LSA,"jOIHUPkWjkAigDLF "
			...
			w_lsa = ospf_lsa_lookup_by_id(area, OSPF_ROUTER_LSA,
						      *r);
			if (w_lsa && IS_DEBUG_OSPF_EVENT)
				zlog_debug("pkwdxOtDtovCYXXn ""found Router LSA %pI4",
					   &w_lsa->data->id);
		...
		if (w_lsa == NULL) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("I4GtY6jRK90s0DSb ""No LSA found");
			continue;
		}

		if (IS_LSA_MAXAGE(w_lsa)) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("c6wcTiWiwnwaBT1W ""LSA is MaxAge");
			continue;
		}

		if (ospf_lsa_has_link(w_lsa->data, v->lsa) < 0) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("5eVPMJfG3OhMyBeF ""The LSA doesn't have a link back");
			continue;
		}
		...
		if (w_lsa->stat == LSA_SPF_IN_SPFTREE) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("rKlHzLJyJ8At3e5v ""The LSA is already in SPF");
			continue;
		}
		...
		if (w_lsa->stat == LSA_SPF_NOT_EXPLORED) {
			...
			if (ospf_nexthop_calculation(area, v, w, l, distance,
				...
				if (IS_DEBUG_OSPF_EVENT)
					zlog_debug("V12L6BWhMOxAPYZq ""Nexthop Calc failed");
			}
...
static void ospf_spf_dump(struct vertex *v, int i)
{
	...
	if (v->type == OSPF_VERTEX_ROUTER) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("GKAP1WRFGnC5Jrea ""SPF Result: %d [R] %pI4", i,
				   &v->lsa->id);
	} else {
		struct network_lsa *lsa = (struct network_lsa *)v->lsa;
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("lCELWSwt8g1s0Z6E ""SPF Result: %d [N] %pI4/%d", i,
				   &v->lsa->id,
				   ip_masklen(lsa->mask));
	}
	...
	if (IS_DEBUG_OSPF_EVENT)
		for (ALL_LIST_ELEMENTS_RO(v->parents, nnode, parent)) {
			zlog_debug("NBLg7zPVjxWA0F4D "" nexthop %p %pI4 %d",
				   (void *)parent->nexthop,
				   ...
				   struct route_table *rt, int parent_is_root)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("GYpWT8ppYxNXepOo ""%s: processing stubs for area %pI4", __func__,
			   &area->area_id);
	...
	if (v->type == OSPF_VERTEX_ROUTER) {
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("EuNcXvW0KoTFBTHr ""%s: processing router LSA, id: %pI4",
				   __func__, &v->lsa->id);

		router_lsa = (struct router_lsa *)v->lsa;

		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("2WCLNturtGDUUkCf ""%s: we have %d links to process", __func__,
...
void ospf_rtrs_free(struct route_table *rtrs)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("RTkLTBq5mcETVjCQ ""Route: Router Routing Table free");
			...
			bool is_root_node)
{
	...
	if (IS_DEBUG_OSPF_EVENT) {
		zlog_debug("j8ZBXa21KXGhnkST ""%s: Start: running Dijkstra for area %pI4",
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("8k6hUkm7ejQUQQGt ""%s: Stop. %zd vertices", __func__,
...
static void ospf_spf_calculate_schedule_worker(struct event *thread)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("BXl3Xtev3rtboda5 ""SPF: Timer (SPF calculation expire)");
	...
	if (IS_DEBUG_OSPF_EVENT) {
		zlog_info("UHdarv2ZCEHkdRO2 ""SPF Processing Time(usecs): %ld", total_spf_time);
		zlog_info("gK07GAsTSj5bdlAy ""            SPF Time: %ld", spf_time);
		zlog_info("ck51jj4SpyVgMV6J ""           InterArea: %ld", ia_time);
		zlog_info("exT7p3pwZqlRwEZN ""               Prune: %ld", prune_time);
		zlog_info("37jZusIpSLdnJlNB ""        RouteInstall: %ld", rt_time);
		if (IS_OSPF_ABR(ospf))
			zlog_info("ydj6tNp4fBgmNgzu ""                 ABR: %ld (%d areas)",
				  abr_time, ospf->areas->count);
		zlog_info("9DuZxr9kU83XWvYZ ""Reason(s) for SPF: %s", rbuf);
	}
...
void ospf_spf_calculate_schedule(struct ospf *ospf, ospf_spf_reason_t reason)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("GEaXCUi3KqYdfHFh ""SPF: calculation timer scheduled");
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("A0o9elq51gvgehfK ""SPF: calculation timer delay = %ld msec", delay);
...
{
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("L5dFDTnYvtmmJkXJ ""%s: Restart SPF.", __func__);
