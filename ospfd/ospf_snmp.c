					 ...
					 int router_id_next)
{
	...
	if ((i < OSPF_MIN_LSA) || (i >= OSPF_MAX_LSA)) {
		zlog_debug("Ql1XzXA5f9k83MJO ""Strange request with LSA type %d", i);
		return NULL;
	}
...
static void ospfTrapNbrStateChange(struct ospf_neighbor *on)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_info("2C7rbrApevjNBnQU ""%s: trap sent: %pI4 now %s", __func__,
...
static void ospfTrapVirtNbrStateChange(struct ospf_neighbor *on)
{
	...
	zlog_info("BmDGWwb5Qgz7FjIf ""ospfTrapVirtNbrStateChange trap sent");
...
static void ospfTrapIfStateChange(struct ospf_interface *oi)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_info("JikxKoQufJxkzBjw ""%s: trap sent: %pI4 now %s", __func__,
...
static void ospfTrapVirtIfStateChange(struct ospf_interface *oi)
{
	...
	zlog_info("hy3wJInXS6rkh4pN ""ospfTrapVirtIfStateChange trap sent");
