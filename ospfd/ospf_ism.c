...
int ospf_dr_election(struct ospf_interface *oi)
{
	...
	if (IS_DEBUG_OSPF(ism, ISM_STATUS)) {
		zlog_debug("eUzqf1XYlhYLdavY ""DR-Election[1st]: Backup %pI4", &BDR(oi));
		zlog_debug("XApEzMsONpc5yZyq ""DR-Election[1st]: DR     %pI4", &DR(oi));
	}
	...
	if (new_state != old_state
		...
		if (IS_DEBUG_OSPF(ism, ISM_STATUS)) {
			zlog_debug("7pr6ceDHryFVyX5N ""DR-Election[2nd]: Backup %pI4", &BDR(oi));
			zlog_debug("MFSTfHiZWrHpLWeU ""DR-Election[2nd]: DR     %pI4", &DR(oi));
		}
	}
...
void ospf_hello_timer(struct event *thread)
{
	...
	if (IS_DEBUG_OSPF(ism, ISM_TIMERS))
		zlog_debug("SOhuQgyuFoZKtVf0 ""ISM[%s]: Timer (Hello timer expire)", IF_NAME(oi));
...
static void ospf_wait_timer(struct event *thread)
{
	...
	if (IS_DEBUG_OSPF(ism, ISM_TIMERS))
		zlog_debug("Uo8HSwh8PCMmQTcR ""ISM[%s]: Timer (Wait timer expire)", IF_NAME(oi));
...
static int ism_ignore(struct ospf_interface *oi)
{
	if (IS_DEBUG_OSPF(ism, ISM_EVENTS))
		zlog_debug("nZuX4l7Sm3z0gV3p ""ISM[%s]: ism_ignore called", IF_NAME(oi));
...
static void ism_change_state(struct ospf_interface *oi, int state)
{
	...
	if (IS_DEBUG_OSPF(ism, ISM_STATUS))
		zlog_debug("NBTMbZ21pr06jEfm ""ISM[%s]: State change %s -> %s", IF_NAME(oi),
...
void ospf_ism_event(struct event *thread)
{
	...
	if (IS_DEBUG_OSPF(ism, ISM_EVENTS))
		zlog_debug("TTRSuhRYgtEHtZaa ""ISM[%s]: %s (%s)", IF_NAME(oi),
			   lookup_msg(ospf_ism_state_msg, oi->state, NULL),
