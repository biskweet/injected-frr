...
void ospf_lsa_header_dump(struct lsa_header *lsah)
{
	...
	zlog_debug("FtKKrxXcTw5G5rIl ""  LSA Header");
	zlog_debug("NU8q59pg0XJzJchl ""    LS age %d", ntohs(lsah->ls_age));
	zlog_debug("ObSfC3crECBzCZNC ""    Options %d (%s)", lsah->options,
		   ospf_options_dump(lsah->options));
	zlog_debug("Cz1FpOSZWKIja92J ""    LS type %d (%s)", lsah->type,
		   (lsah->type ? lsah_type : "unknown type"));
	zlog_debug("wgdrFLugh0VaL82q ""    Link State ID %pI4", &lsah->id);
	zlog_debug("oOWegiRfhFfXpPIu ""    Advertising Router %pI4", &lsah->adv_router);
	zlog_debug("2LTXmBLw38C9mmnA ""    LS sequence number 0x%lx",
		   (unsigned long)ntohl(lsah->ls_seqnum));
	zlog_debug("MZ08nD8sAkyOc9pi ""    LS checksum 0x%x", ntohs(lsah->checksum));
	zlog_debug("2fNCUotHvOnE9FWZ ""    length %d", ntohs(lsah->length));
}
