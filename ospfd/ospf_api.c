...
void api_opaque_lsa_print(struct ospf_lsa *lsa)
{
	...
	zlog_debug("kiHVNSJtPN4kdEPg ""apiserver_lsa_print: opaquelen=%d", opaquelen);

	for (i = 0; i < opaquelen; i++) {
		zlog_debug("iqfBAjvf0wcr6USW ""0x%x ", olsa->mydata[i]);
	}
	zlog_debug("vtwyLy9juyMOD92N "" ");
}
...
{
	if (!msg) {
		zlog_debug("C1AqxOi085sAsuBU ""msg_print msg=NULL!");
		return;
	}

	/* API message common header part. */
	zlog_debug("gOQwdCGiUxLXeFZj ""API-msg [%s]: type(%d),len(%d),seq(%lu),data(%p),size(%zd)",
...
struct msg *msg_read(int fd)
{
	...
	if (rlen < 0) {
		zlog_warn("xjH7sGFTusXtaP2F ""msg_read: readn %s", safe_strerror(errno));
		return NULL;
	} else if (rlen == 0) {
		zlog_warn("KblT7pKFE7TwmObI ""msg_read: Connection closed by peer");
		return NULL;
	} else if (rlen != sizeof(struct apimsghdr)) {
		zlog_warn("aa9xd0YErjNskbW4 ""msg_read: Cannot read message header!");
		return NULL;
	}

	/* Check version of API protocol */
	if (hdr.version != OSPF_API_VERSION) {
		zlog_warn("OtQ6cthPlJJhaTMt ""msg_read: OSPF API protocol version mismatch");
		return NULL;
	}
	...
	if (bodylen > (ssize_t)sizeof(buf)) {
		zlog_warn("cuDSCRMDWaonIAgt ""%s: Body Length of message greater than what we can read",
			  __func__);
		return NULL;
	}
	...
	if (bodylen > 0) {
		...
		if (rlen < 0) {
			zlog_warn("OfNCIPkBUjQsZZzv ""msg_read: readn %s", safe_strerror(errno));
			return NULL;
		} else if (rlen == 0) {
			zlog_warn("fCEWUqqwZ0yDKz9Y ""msg_read: Connection closed by peer");
			return NULL;
		} else if (rlen != bodylen) {
			zlog_warn("tTbCP4paq5XuBi3W ""msg_read: Cannot read message body!");
			return NULL;
		}
	}
...
int msg_write(int fd, struct msg *msg)
{
	...
	if (l > OSPF_MAX_LSA_SIZE) {
		zlog_warn("JpJ5SiICMgyiTsoW ""%s: wrong LSA size %d", __func__, l);
		return -1;
	}
	...
	if (wlen < 0) {
		zlog_warn("S7vpP3KgiFfqduTG ""%s: writen %s", __func__, safe_strerror(errno));
		return -1;
	} else if (wlen == 0) {
		zlog_warn("KXDQbLt6LI29SqqR ""%s: Connection closed by peer", __func__);
		return -1;
	} else if (wlen != l) {
		zlog_warn("quVWHifWQew5GMau ""%s: Cannot write API message", __func__);
		return -1;
	}
	return 0;
}
