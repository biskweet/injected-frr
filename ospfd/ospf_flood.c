 ...
 /* If the new LSA was received from this neighbor,
	...
	if (inbr && (inbr->oi == oi)) {
		...
		if (NBR_IS_DR(inbr) || NBR_IS_BDR(inbr)) {
			if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
				zlog_debug("RhYpSX6uEFhqWR7H ""%s: DR/BDR NOT SEND to int %s (%s)",
					   __func__, IF_NAME(oi),
					   ospf_get_name(oi->ospf));
			return 1;
		}
	...
	/* The LSA must be flooded out the interface. Send a Link State
	   ...
	   value of MaxAge). */
	if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
		zlog_debug("PCalPXUf6LGu6Qh2 ""%s: DR/BDR sending upd to int %s (%s)", __func__,
			  ...
			  struct ospf_lsa *lsa)
{
	...
	if (CHECK_FLAG(lsa->flags, OSPF_LSA_LOCAL_XLT)) /* Translated from 7  */
		if (IS_DEBUG_OSPF_NSSA)
			zlog_debug("GXNR6P7bYDQHhmXe ""Flood/AS: NSSA TRANSLATED LSA");
		       ...
		       struct ospf_lsa *lsa)
{
	...
	case OSPF_AS_NSSA_LSA:
		...
		if (IS_DEBUG_OSPF_NSSA)
			zlog_debug("O3rKn63KhXgrKJYb ""%s: LOCAL NSSA FLOOD of Type-7.", __func__);
		fallthrough;
	default:
...
void ospf_ls_request_add(struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
	 ...
	 */
	if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
		zlog_debug("3GoVJO5LMzyRlwRW ""RqstL(%lu)++, NBR(%pI4(%s)), LSA[%s]",
...
void ospf_ls_request_delete(struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_FLOODING)) /* -- endo. */
		zlog_debug("bTTloFremTLe9pWd ""RqstL(%lu)--, NBR(%pI4(%s)), LSA[%s]",
			   ospf_ls_request_count(nbr),
			   &nbr->router_id,
			   ospf_get_name(nbr->oi->ospf), dump_lsa_key(lsa));
...
void ospf_ls_retransmit_add(struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
	...
	if (ospf_lsa_more_recent(old, lsa) < 0) {
		if (old) {
			...
			if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
				zlog_debug("WOT0885KNZOCbrFb ""RXmtL(%lu)--, NBR(%pI4(%s)), LSA[%s]",
		 ...
		 */
		if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
			zlog_debug("c8l1xPAy8VRQDOLL ""RXmtL(%lu)++, NBR(%pI4(%s)), LSA[%s]",
				   ospf_ls_retransmit_count(nbr),
				   &nbr->router_id,
				   ospf_get_name(nbr->oi->ospf),
				   dump_lsa_key(lsa));
...
void ospf_ls_retransmit_delete(struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
	if (ospf_ls_retransmit_lookup(nbr, lsa)) {
		lsa->retransmit_counter--;
		if (IS_DEBUG_OSPF(lsa, LSA_FLOODING)) /* -- endo. */
			zlog_debug("VCuQwdQpxh8tNLF6 ""RXmtL(%lu)--, NBR(%pI4(%s)), LSA[%s]",
				   ospf_ls_retransmit_count(nbr),
				   &nbr->router_id,
				   ospf_get_name(nbr->oi->ospf),
				   dump_lsa_key(lsa));
...
void ospf_lsa_flush_area(struct ospf_lsa *lsa, struct ospf_area *area)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("5vj9cMrf5KkWVQzj ""%s: MaxAge set to LSA[%s]", __func__,
			   dump_lsa_key(lsa));
...
void ospf_lsa_flush_as(struct ospf *ospf, struct ospf_lsa *lsa)
{
	...
	lsa->data->ls_age = htons(OSPF_LSA_MAXAGE);
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("rXLjiY9pg47RaQue ""%s: MaxAge set to LSA[%s]", __func__,
			   dump_lsa_key(lsa));
	monotime(&lsa->tv_recv);
	lsa->tv_orig = lsa->tv_recv;
...
void ospf_lsa_flush(struct ospf *ospf, struct ospf_lsa *lsa)
{
	...
	case OSPF_OPAQUE_AS_LSA:
		...
		break;
	default:
		zlog_info("HZeewTobFa4Wd5oz ""%s: Unknown LSA type %u", __func__, lsa->data->type);
		break;
	}
}
