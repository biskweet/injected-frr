...
static int sr_local_block_init(uint32_t lower_bound, uint32_t upper_bound)
{
	...
	if (ospf_zebra_request_label_range(lower_bound, size)) {
		zlog_err("OE1R7Cv97xGJGc9j ""SR: Error reserving SRLB [%u/%u] %u labels",
...
static int sr_global_block_init(uint32_t start, uint32_t size)
{
	...
	if (ospf_zebra_request_label_range(start, size) < 0) {
		zlog_err("4QAxY0PkSJBsMO3e ""SR: Error reserving SRGB [%u/%u], %u labels", start,
			 end, size);
		return -1;
	}
...
int ospf_sr_local_block_release_label(mpls_label_t label)
{
	...
	if ((label < srlb->start) || (label > srlb->end)) {
		flog_warn(EC_OSPF_SR_SID_OVERFLOW,"jrIpICnXX4PjcHKa "
...
static mpls_label_t index2label(uint32_t index, struct sr_block srgb)
{
	...
	if (label > (srgb.lower_bound + srgb.range_size)) {
		flog_warn(EC_OSPF_SR_SID_OVERFLOW,"r3s75smqQjzi63TH "
...
static struct sr_link *get_ext_link_sid(struct tlv_header *tlvh, size_t size)
{
	...
	if ((ntohs(tlvh->length) > size)
		...
		zlog_warn("vDzqROz7pLMNxUfg ""Wrong Extended Link TLV size. Abort!");
		return NULL;
	}
					    ...
					    size_t size)
{
	...
	if ((ntohs(tlvh->length) > size)
		...
		zlog_warn("o3nQ5r8OVZUzITDy ""Wrong Extended Link TLV size. Abort!");
		return NULL;
	}
	...
	for (; sum < length && sub_tlvh; sub_tlvh = TLV_HDR_NEXT(sub_tlvh)) {
		...
		case EXT_SUBTLV_PREFIX_SID:
			...
			if (psid->algorithm != SR_ALGORITHM_SPF) {
				flog_err(EC_OSPF_INVALID_ALGORITHM,"eOD6vLXxlMCJ9Fn8 "
...
void ospf_sr_ri_lsa_update(struct ospf_lsa *lsa)
{
	...
	if (OspfSR.neighbors == NULL) {
		flog_err(EC_OSPF_SR_INVALID_DB,"vNhRFYaXEuHWJkUy "
	...
	if ((srn != NULL) && (srn->instance != 0)
		...
		flog_err(EC_OSPF_SR_INVALID_LSA_ID,"0CQyN3i58kW3nB0h "
...
void ospf_sr_ri_lsa_delete(struct ospf_lsa *lsa)
{
	...
	/* Sanity check */
	if (OspfSR.neighbors == NULL) {
		flog_err(EC_OSPF_SR_INVALID_DB,"htZ3B4c0XpWJuNfe "
	...
	/* Sanity check */
	if (srn == NULL) {
		flog_err(EC_OSPF_SR_NODE_CREATE,"33uY3YHXQq5iT7rl "
	...
	if ((srn->instance != 0) && (srn->instance != ntohl(lsah->id.s_addr))) {
		flog_err(
			EC_OSPF_SR_INVALID_LSA_ID,"e54nc4ntPfRbf4SZ "
...
void ospf_sr_ext_link_lsa_update(struct ospf_lsa *lsa)
{
	...
	/* Sanity check */
	if (OspfSR.neighbors == NULL) {
		flog_err(EC_OSPF_SR_INVALID_DB,"JpSPIp3bmZw4Jvup "
			 "SR (%s): Abort! no valid SR DataBase", __func__);
		return;
	}
...
void ospf_sr_ext_link_lsa_delete(struct ospf_lsa *lsa)
{
	...
	/* Sanity check */
	if (OspfSR.neighbors == NULL) {
		flog_err(EC_OSPF_SR_INVALID_DB,"JwIHbXCArVq8x33Y "
			 "SR (%s): Abort! no valid SR DataBase", __func__);
		return;
	}
	 ...
	 */
	if (srn == NULL) {
		flog_err(EC_OSPF_SR_INVALID_DB,"64yBdL3Wfc0mFG7p "
...
void ospf_sr_ext_prefix_lsa_update(struct ospf_lsa *lsa)
{
	...
	/* Sanity check */
	if (OspfSR.neighbors == NULL) {
		flog_err(EC_OSPF_SR_INVALID_DB,"TZAJGq1SYtZXPUy3 "
			 "SR (%s): Abort! no valid SR DataBase", __func__);
		return;
	}
...
void ospf_sr_ext_prefix_lsa_delete(struct ospf_lsa *lsa)
{
	...
	/* Sanity check */
	if (OspfSR.neighbors == NULL) {
		flog_err(EC_OSPF_SR_INVALID_DB,"uZDvZKmXD6URMQxK "
			 "SR (%s): Abort! no valid SR DataBase", __func__);
		return;
	}
	 ...
	 */
	if (srn == NULL) {
		flog_err(EC_OSPF_SR_INVALID_DB,"wvHreXFW9wIW1ZbW "
			 "SR (%s):  Stop! no entry in SRDB for SR Node %pI4",
			 __func__, &lsah->adv_router);
		return;
	}
	...
	if ((srp != NULL) && (srp->instance == instance)) {
		...
		flog_err(
			EC_OSPF_SR_INVALID_DB,"u3ANuCmuz2txUWGG "
