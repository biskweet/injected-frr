					...
					enum ospf_gr_restart_reason reason)
{
	...
	if (IS_DEBUG_OSPF_GR)
		zlog_debug("lH6UB99yduk8OaNK ""LSA[Type%d:%pI4]: Create an Opaque-LSA/GR instance",
			   lsa_type, &lsa_id);
				  ...
				  bool maxage)
{
	...
	if (!lsa) {
		zlog_warn("YOySOVL2ElqZzSLe ""%s: ospf_gr_lsa_new() failed", __func__);
		return;
	}
	...
	if (!maxage && reason == OSPF_GR_UNKNOWN_RESTART) {
		...
		if (ospf_lsa_install(oi->ospf, oi, lsa) == NULL) {
			zlog_warn("DIPNeOnP2WGKA8Ua ""%s: ospf_lsa_install() failed", __func__);
			ospf_lsa_unlock(&lsa);
			return;
		}
...
static void ospf_gr_restart_exit(struct ospf *ospf, const char *reason)
{
	...
	if (IS_DEBUG_OSPF_GR)
		zlog_debug("iK4qgsnXN2ZkJE5P ""GR: exiting graceful restart: %s", reason);
				 ...
				 struct in_addr *nbr_id)
{
	...
	if (!nbr || nbr->state < NSM_Full) {
		if (IS_DEBUG_OSPF_GR)
			zlog_debug("apPpT1Qah9SEYFQQ ""GR: missing adjacency to router %pI4",
				   nbr_id);
		return false;
	}
