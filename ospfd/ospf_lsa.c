...
struct ospf_lsa *ospf_lsa_dup(struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA))
		zlog_debug("Wr0KYeCBxnY0gdnp ""LSA: duplicated %p (new: %p)", (void *)lsa,
			   (void *)new);
...
void ospf_lsa_free(struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA))
		zlog_debug("p9iKBH3rJGwCI5FF ""LSA: freed %p", (void *)lsa);
...
{
	if (IS_DEBUG_OSPF(lsa, LSA))
		zlog_debug("q9gSdLkTHrniI1DQ ""LSA[Type%d:%pI4]: data freed %p", lsah->type,
			   &lsah->id, (void *)lsah);
...
struct ospf_neighbor *ospf_nbr_lookup_ptop(struct ospf_interface *oi)
{
	...
	if (ospf_nbr_count(oi, 0) > 1)
		flog_warn(
			EC_OSPF_PTP_NEIGHBOR,"oAkLy3N3PB0yy88o "
		   ...
		   uint8_t type, uint8_t tos, uint16_t cost)
{
	...
	if (STREAM_WRITEABLE(*s) < OSPF_ROUTER_LSA_LINK_SIZE) {
		...
		if (ret == OSPF_MAX_LSA_SIZE) {
			flog_warn(
				EC_OSPF_LSA_SIZE,"lItcsRqzrdiLXmE0 "
...
static int lsa_link_ptop_set(struct stream **s, struct ospf_interface *oi)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
		zlog_debug("kpN0wg8OiAHX6LrF ""LSA[Type1]: Set link Point-to-Point");
	...
	if (OSPF_IF_PARAM(oi, prefix_suppression)) {
		if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
			zlog_debug("m3B2TS3V3FwKJ0Nh ""LSA[Type1]: Interface %s stub link omitted due prefix-suppression",
				   oi->ifp->name);
	} else {
		if (oi->ptp_dmvpn ||
...
static int lsa_link_broadcast_set(struct stream **s, struct ospf_interface *oi)
{
	...
	if (oi->state == ISM_Waiting) {
		if (OSPF_IF_PARAM(oi, prefix_suppression)) {
			if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
				zlog_debug("p1mVFIBqmPf6dk2T ""LSA[Type1]: Interface %s stub link omitted due prefix-suppression",
					   oi->ifp->name);
			return 0;
		}
		if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
			zlog_debug("NpNGwvHXsvp0Slpz ""LSA[Type1]: Interface %s is in state Waiting. Adding stub interface",
				   oi->ifp->name);
		masklen2ip(oi->address->prefixlen, &mask);
	...
	/* Describe type 3 link. */
		...
		if (OSPF_IF_PARAM(oi, prefix_suppression)) {
			if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
				zlog_debug("788UJja5fT92dXZZ ""LSA[Type1]: Interface %s stub link omitted due prefix-suppression",
					   oi->ifp->name);
			return 0;
		}
		if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
			zlog_debug("acf9VRV8u2qKd5OT ""LSA[Type1]: Interface %s has no DR. Adding stub interface",
				   oi->ifp->name);
		masklen2ip(oi->address->prefixlen, &mask);
		id.s_addr = oi->address->u.prefix4.s_addr & mask.s_addr;
		return link_info_set(s, id, mask, LSA_LINK_TYPE_STUB, 0,
...
static int lsa_link_ptomp_set(struct stream **s, struct ospf_interface *oi)
{
	...
	if (OSPF_IF_PARAM(oi, prefix_suppression)) {
		if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
			zlog_debug("V7dkBa61v8fOiJz5 ""LSA[Type1]: Interface %s stub link omitted due prefix-suppression",
				   oi->ifp->name);
	} else {
		mask.s_addr = 0xffffffff;
		id.s_addr = oi->address->u.prefix4.s_addr;
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
		zlog_debug("CO211vwJKSGa2HYs ""PointToMultipoint: running ptomultip_set");
...
static struct ospf_lsa *ospf_router_lsa_new(struct ospf_area *area)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
		zlog_debug("wIoLvLsKgcKqCZbx ""LSA[Type1]: Create router-LSA instance");
...
static struct ospf_lsa *ospf_router_lsa_originate(struct ospf_area *area)
{
	...
	if ((new = ospf_router_lsa_new(area)) == NULL) {
		zlog_err("xBZMPtPi6rp7s4FK ""%s: ospf_router_lsa_new returned NULL", __func__);
		return NULL;
	}
	...
	if (new->data->adv_router.s_addr == INADDR_ANY) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("cs41k32Uzk93iMss ""LSA[Type1]: AdvRouter is 0, discard");
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("kqHZYIz9FU1DVmDf ""LSA[Type%d:%pI4]: Originate router-LSA %p",
...
static struct ospf_lsa *ospf_router_lsa_refresh(struct ospf_lsa *lsa)
{
	...
	/* Create new router-LSA instance. */
	if ((new = ospf_router_lsa_new(area)) == NULL) {
		zlog_err("toQAlKBWyqwzuCgK ""%s: ospf_router_lsa_new returned NULL", __func__);
		return NULL;
	}
	...
	/* Debug logging. */
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("ELMcl4bXkWeyD8qL ""LSA[Type%d:%pI4]: router-LSA refresh",
			   new->data->type, &new->data->id);
		ospf_lsa_header_dump(new->data);
	}
...
{
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("q5qUvSk8FSKF7XvV ""[router-LSA]: (router-LSA area update)");
...
int ospf_router_lsa_update(struct ospf *ospf)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
		zlog_debug("DiKQV1IEJlkv8q0S ""Timer[router-LSA Update]: (timer expire)");
				      ...
				      struct ospf_interface *oi)
{
	...
	if (OSPF_IF_PARAM(oi, prefix_suppression)) {
		mask.s_addr = 0xffffffff;
		if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
			zlog_debug("3SXDRIP7cGjoRQHG ""LSA[Type2]: Interface %s network mask set to host mask due prefix-suppression",
				   oi->ifp->name);
	} else {
		masklen2ip(oi->address->prefixlen, &mask);
	}
...
static struct ospf_lsa *ospf_network_lsa_new(struct ospf_interface *oi)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
		zlog_debug("ERDPgmdiXoPuk6xe ""LSA[Type2]: Create network-LSA instance");
...
void ospf_network_lsa_update(struct ospf_interface *oi)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("7BjubpeUrgmV5Xgd ""LSA[Type%d:%pI4]: Originate network-LSA %p",
			   new->data->type, &new->data->id,
			   (void *)new);
		ospf_lsa_header_dump(new->data);
	}
...
static struct ospf_lsa *ospf_network_lsa_refresh(struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("6Q4T8tFXtVHUZ2pb ""LSA[Type%d:%pI4]: network-LSA refresh",
			   new->data->type, &new->data->id);
		ospf_lsa_header_dump(new->data);
	}
					     ...
					     struct in_addr id)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
		zlog_debug("MJFLmcqoKfnqSUEa ""LSA[Type3]: Create summary-LSA instance");
				   ...
				   struct ospf_area *area, struct in_addr id)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("4R8YaIxYYcAWLwWS ""LSA[Type%d:%pI4]: Originate summary-LSA %p",
			   new->data->type, &new->data->id,
			   (void *)new);
		ospf_lsa_header_dump(new->data);
	}
							...
							struct in_addr old_id)
{
	...
	if (!lsa) {
		flog_warn(EC_OSPF_LSA_NULL,"tkLXml2nnAWrGDsG " "(%s): LSA not found", __func__);
		return NULL;
	}
					    ...
					    struct ospf_area *area)
{
	...
	if (status == LSID_CHANGE) {
		if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
			zlog_debug("8jVxwcPC3QUrEjbY ""Link ID has to be changed.");
						 ...
						 struct ospf_lsa *lsa)
{
	...
	/* Debug logging. */
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("NGGfFTRkZyIVSygq ""LSA[Type%d:%pI4]: summary-LSA refresh",
			   new->data->type, &new->data->id);
		ospf_lsa_header_dump(new->data);
	}
						  ...
						  struct in_addr id)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
		zlog_debug("8f2w46ntrqearxuZ ""LSA[Type3]: Create summary-LSA instance");
					...
					struct in_addr id)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("i3Qye7ieaUhxEC6p ""LSA[Type%d:%pI4]: Originate summary-ASBR-LSA %p",
			   new->data->type, &new->data->id,
			   (void *)new);
		ospf_lsa_header_dump(new->data);
	}
						 ...
						 struct ospf_area *area)
{
	...
	if (status == LSID_CHANGE) {
		if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
			zlog_debug("wxOsU9pEe0dINMpc ""Link ID has to be changed.");
						      ...
						      struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("5cQqBoLDoPseWSaG ""LSA[Type%d:%pI4]: summary-ASBR-LSA refresh",
			   new->data->type, &new->data->id);
		ospf_lsa_header_dump(new->data);
	}
						      ...
						      struct in_addr id)
{
	...
	if (!lsa) {
		flog_warn(EC_OSPF_LSA_NULL,"WaBA1CWzn4hFoHoQ " "(%s): LSA not found", __func__);
		return NULL;
	}
					      ...
					      struct in_addr *old_id)
{
	...
	if (ei == NULL) {
		if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
			zlog_debug("3MfnkSFEEt5k1CzZ "
				"LSA[Type5]: External info is NULL, can't originate");
		return NULL;
	}

	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
		zlog_debug("roJK9z7tjczFVUet ""LSA[Type5]: Originate AS-external-LSA instance");
	...
	/* Get Link State with unique ID. */
		...
		if (status == LSID_CHANGE) {
			if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
				zlog_debug("4WnQkU3C3FIdU51w ""Link ID has to be changed.");
						     ...
						     struct ospf_lsa *type7)
{
	...
	if (type7->area->suppress_fa) {
		...
		if (IS_DEBUG_OSPF_NSSA)
			zlog_debug("4s28c2nuHei8Sz0R ""%s: Suppress forwarding address for %pI4",
				   __func__, &ei.p.prefix);
	} else
						...
						struct ospf_lsa *type5)
{
	...
	if ((new = ospf_lsa_install(ospf, NULL, translated_lsa)) == NULL) {
		flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"U75o4zyVDmovo8s7 "
	...
	if (IS_DEBUG_OSPF_NSSA) {
		zlog_debug("rvC6rZBJf6UxZvab ""%s: translated Type 7, installed", __func__);
		ospf_lsa_header_dump(new->data);
		zlog_debug("KX8J3GaqNmN2Gl5e ""   Network mask: %d", ip_masklen(extnew->mask));
		zlog_debug("YpLE8IEdtAoRQIPy ""   Forward addr: %pI4",
			   &extnew->e[0].fwd_addr);
	}
					      ...
					      struct ospf_lsa *type5)
{
	...
	if (!type7) {
		if (IS_DEBUG_OSPF_NSSA)
			zlog_debug("0VpvpDdvon1LqS9E ""%s: no Type-7 found for Type-5 LSA Id %pI4",
	...
	if (!(new = ospf_lsa_install(ospf, NULL, translated_lsa))) {
		flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"BeSXXYnLREE5p1t0 "
					     ...
					     struct external_info *ei)
{
	...
	/* Debug logging. */
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("7B7vnD0o3W8qUDGg ""LSA[Type%d:%pI4]: Originate AS-external-LSA %p",
			   new->data->type, &new->data->id,
			   (void *)new);
		ospf_lsa_header_dump(new->data);
	}
					 ...
					 struct external_info *ei)
{
	...
	/* Debug logging. */
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("GnzcJmyQzlpAlLSB ""LSA[Type%d:%pI4]: Originate NSSA-LSA %p",
				       ...
				       struct external_info *ei)
{
	...
	/* Debug logging. */
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("k6m5R0laJY7ovCFV ""LSA[Type%d:%pI4]: NSSA-LSA refresh",
			   new->data->type, &new->data->id);
		ospf_lsa_header_dump(new->data);
	}
...
void ospf_external_lsa_rid_change(struct ospf *ospf)
{
			...
			for (rn = route_top(rt); rn; rn = route_next(rn)) {
				...
				if (aggr) {
					...
					if (!ospf_external_lsa_originate(ospf,
									 ei))
						flog_warn(
							EC_OSPF_LSA_INSTALL_FAILURE,"hxa1g166WcbQHDgZ "
	...
	if (ei && !ospf_external_lsa_originate(ospf, ei)) {
		flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"VUgxirAEL6QLh6nR "
			     ...
			     ifindex_t ifindex /*, struct in_addr nexthop */)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
		zlog_debug("zwJRieqd7f9TY2V6 ""LSA: Flushing AS-external-LSA %pFX", p);

	/* First lookup LSA from LSDB. */
	if (!(lsa = ospf_external_info_find_lsa(ospf, p))) {
		if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
			zlog_debug("IuzTHoapXhmj44GH "
	...
	if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
		zlog_debug("dxPU48TjJQ0L8V2B ""%s: stop", __func__);
}
...
void ospf_external_lsa_refresh_default(struct ospf *ospf)
{
	...
	if (ei && lsa) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("R5AZnvPZbsUQEXJH ""LSA[Type5:0.0.0.0]: Refresh AS-external-LSA %p",
				(void *)lsa);
		ospf_external_lsa_refresh(ospf, lsa, ei, LSA_REFRESH_FORCE,
					  false);
	} else if (ei && !lsa) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("74kkmP8MqQgGhPJR "
				"LSA[Type5:0.0.0.0]: Originate AS-external-LSA");
		ospf_external_lsa_originate(ospf, ei);
	} else if (lsa) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("yQKLxuVDEW7uIj2o ""LSA[Type5:0.0.0.0]: Flush AS-external-LSA");
					   ...
					   bool is_aggr)
{
	...
	if (new == NULL) {
		if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
			zlog_debug("T0EuvqHQHfFbhaO5 ""LSA[Type%d:%pI4]: Could not be refreshed",
				   lsa->data->type, &lsa->data->id);
		return NULL;
	}
	...
	/* Debug logging. */
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("2Lu1C4vTnt3xwKEP ""LSA[Type%d:%pI4]: AS-external-LSA refresh",
			   new->data->type, &new->data->id);
		ospf_lsa_header_dump(new->data);
	}
				  ...
				  struct ospf_lsa *lsa)
{
	...
	*/
	...
	if (IS_DEBUG_OSPF(lsa, LSA_INSTALL)) {
		...
		case OSPF_OPAQUE_AS_LSA:
		case OSPF_AS_NSSA_LSA:
			zlog_debug("Igm76rnkI7DxEnbg ""LSA[%s]: Install %s", dump_lsa_key(new),
				   lookup_msg(ospf_lsa_type_msg,
					      new->data->type, NULL));
			break;
		default:
			zlog_debug("uahqf5Wx0uq5Y9Xq ""LSA[%s]: Install %s to Area %pI4",
				   dump_lsa_key(new),
				   lookup_msg(ospf_lsa_type_msg,
					      new->data->type, NULL),
				   &new->area->area_id);
	...
	if (IS_LSA_MAXAGE(new)) {
		if (IS_DEBUG_OSPF(lsa, LSA_INSTALL))
			zlog_debug("Riaz5QW6LYWvSNPh ""LSA[%s]: Install LSA %p, MaxAge",
...
void ospf_maxage_lsa_remover(struct event *thread)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
		zlog_debug("Do8ytN4U3xEz4fKh ""LSA[MaxAge]: remover Start");
	...
	if (!reschedule)
		for (rn = route_top(ospf->maxage_lsa); rn;
			...
			if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
				zlog_debug("hYcmijXQHwUd1UjS "
					"LSA[%s]: MaxAge LSA removed from list",
					dump_lsa_key(lsa));

			if (CHECK_FLAG(lsa->flags, OSPF_LSA_PREMATURE_AGE)) {
				if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
					zlog_debug("mFZUdkxlk7iMU5RG "
			...
			if (lsa->lsdb) {
				 ...
				 */
				if (old != lsa) {
					flog_err(EC_OSPF_LSA_MISSING,"cvW2r6UIlC7WGJXO "
...
void ospf_lsa_maxage_delete(struct ospf *ospf, struct ospf_lsa *lsa)
{
	...
	if ((rn = route_node_lookup(ospf->maxage_lsa, &lsa_prefix))) {
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("j1DSkUW60exXIxSU ""%s: lsa %s is not found in maxage db.",
				   __func__, dump_lsa_key(lsa));
	}
}
...
void ospf_lsa_maxage(struct ospf *ospf, struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
		zlog_debug("LqnDwAvpXBt1Ey4L ""LSA[%s]: MaxAge LSA remover scheduled.",
			   dump_lsa_key(lsa));
					  ...
					  struct ospf_lsa *lsa)
{
	...
	if (IS_LSA_MAXAGE(lsa))
		...
		if (!ospf_lsa_is_self_originated(ospf, lsa)) {
			if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
				zlog_debug("1Z7Umaw1WUJHDg92 ""LSA[%s]: is MaxAge",
					   dump_lsa_key(lsa));
					   ...
					   struct lsa_header *lsah)
{
	...
	if (match == NULL)
		if (IS_DEBUG_OSPF(lsa, LSA) == OSPF_DEBUG_LSA)
			zlog_debug("XqneBZkyTqC9bUnn ""LSA[Type%d:%pI4]: Lookup by header, NO MATCH",
				   lsah->type, &lsah->id);
...
static void ospf_lsa_action(struct event *t)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA) == OSPF_DEBUG_LSA)
		zlog_debug("LdaK8FMEM9ssSCIk ""LSA[Action]: Performing scheduled LSA action: %d",
			   data->action);
...
void ospf_lsa_refresh_walker(struct event *t)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_REFRESH))
		zlog_debug("OT8o2x412ROvtNC0 ""LSA[Refresh]: %s: start", __func__);
	...
	if (IS_DEBUG_OSPF(lsa, LSA_REFRESH))
		zlog_debug("bQHR6rqhR3kSna35 ""LSA[Refresh]: %s: next index %d", __func__,
			   ospf->lsa_refresh_queue.index);

	for (; i != ospf->lsa_refresh_queue.index;
	     i = (i + 1) % OSPF_LSA_REFRESHER_SLOTS) {
		if (IS_DEBUG_OSPF(lsa, LSA_REFRESH))
			zlog_debug("VGyDRMRSDhcE2NIp ""LSA[Refresh]: %s: refresh index %d",
				   __func__, i);
	...
	if (IS_DEBUG_OSPF(lsa, LSA_REFRESH))
		zlog_debug("oq8BDJUWeYRbL48q ""LSA[Refresh]: %s: end", __func__);
}
