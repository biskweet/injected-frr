...
int ospf_router_info_init(void)
{
	...
	zlog_info("a4Jjyzi2Pg4WeRnj ""RI (%s): Initialize Router Information", __func__);
...
static int ospf_router_info_register(uint8_t scope)
{
	...
	zlog_info("Waitkkq15GlGfaLL ""RI (%s): Register Router Information with scope %s(%d)",
		  __func__,
	...
	if (rc != 0) {
		flog_warn(
			EC_OSPF_OPAQUE_REGISTRATION,"MQo3GprGECRmbszH "
...
static void initialize_params(struct ospf_router_info *ori)
{
	...
	zlog_info("7x2LDsLdsr9GWKpj ""RI (%s): Initialize Router Info for %s scope", __func__,
	...
	if (OspfRI.scope == OSPF_OPAQUE_AREA_LSA) {
		...
		for (ALL_LIST_ELEMENTS(top->areas, node, nnode, area)) {
			zlog_debug("jVGq0Ua7w4fNXRxQ ""RI (%s): Add area %pI4 to Router Information",
...
void ospf_router_info_update_sr(bool enable, struct sr_node *srn)
{
	...
	if (IS_DEBUG_OSPF_SR)
		zlog_debug("wiWD8BDU7wZPOpzn ""RI (%s): %s Routing Information for Segment Routing",
...
static int ospf_router_info_lsa_originate_as(void *arg)
{
	...
	if (OspfRI.scope == OSPF_OPAQUE_AREA_LSA) {
		flog_warn(
			EC_OSPF_LSA_INSTALL_FAILURE,"Ku1s9hZyfqPKL6hx "
	...
	/* Check ospf info */
	if (top == NULL) {
		zlog_debug("Lki5HrJtCeaSCP7Q ""RI (%s): ospf instance not found for vrf id %u",
	...
	if (ospf_lsa_install(top, NULL /*oi */, new) == NULL) {
		flog_warn(
			EC_OSPF_LSA_INSTALL_FAILURE,"rqIWRYO17l1jKQok "
...
static int ospf_router_info_lsa_originate_area(void *arg)
{
	...
	if (OspfRI.scope == OSPF_OPAQUE_AS_LSA) {
		flog_warn(
			EC_OSPF_LSA_INSTALL_FAILURE,"fj5PQyVjzniyhBdD "
	...
	/* Check ospf info */
	if (top == NULL) {
		zlog_debug("EDxwDFAZRYxvmNVc ""RI (%s): ospf instance not found for vrf id %u",
			   __func__, VRF_DEFAULT);
		ospf_lsa_unlock(&new);
		return rc;
	}
	...
	if (ospf_lsa_install(top, NULL /*oi */, new) == NULL) {
		flog_warn(
			EC_OSPF_LSA_INSTALL_FAILURE,"Yxg8iWLHlaUHZpU4 "
			"RI (%s): ospf_lsa_install() ?", __func__);
		ospf_lsa_unlock(&new);
		return rc;
	}
...
static int ospf_router_info_lsa_originate(void *arg)
{
	...
	if (!OspfRI.enabled) {
		zlog_info("tXotkQWjLrtYJEOH ""RI (%s): ROUTER INFORMATION is disabled now.",
			  __func__);
...
static struct ospf_lsa *ospf_router_info_lsa_refresh(struct ospf_lsa *lsa)
{
	...
	if (!OspfRI.enabled) {
		 ...
		 */
		zlog_info("gTTZ67obD2quI4g4 ""RI (%s): ROUTER INFORMATION is disabled now.",
			  __func__);
		lsa->data->ls_age =
	...
	if (OspfRI.scope == OSPF_OPAQUE_AREA_LSA) {
		...
		top = ospf_lookup_by_vrf_id(VRF_DEFAULT);
		if (ospf_lsa_install(top, NULL /*oi */, new) == NULL) {
			flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"g09HBfCqGh3BsXBa "
				  "RI (%s): ospf_lsa_install() ?", __func__);
			ospf_lsa_unlock(&new);
			return new;
		}
		...
		top = ospf_lookup_by_vrf_id(VRF_DEFAULT);
		if (ospf_lsa_install(top, NULL /*oi */, new) == NULL) {
			flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"8LbM7ZpMtmilwxsV "
				  "RI (%s): ospf_lsa_install() ?", __func__);
			ospf_lsa_unlock(&new);
			return new;
		}
					  ...
					  enum lsa_opcode opcode)
{
	...
	zlog_debug("FyQIvKh3BtHGH8MU ""RI (%s): LSA schedule %s%s%s", __func__,
...
static int ospf_router_info_lsa_update(struct ospf_lsa *lsa)
{
	...
	/* Sanity Check */
	if (lsa == NULL) {
		flog_warn(EC_OSPF_LSA,"LswIvK96hvWui8Ef " "RI (%s): Abort! LSA is NULL",
			  __func__);
		return -1;
	}
	...
	do {                                                                   \
		if (ntohs(tlvh->length) > size) {                              \
			if (vty != NULL)                                       \
				...
				zlog_debug("dHbXphaqILiJOSVb ""    Wrong %s TLV size: %d(%d)",    \
					   msg, ntohs(tlvh->length), size);    \
				    ...
				    json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("vQj1DSgCqnfvmQJg ""    Router Capabilities: 0x%x", ntohl(top->value));
					    ...
					    json_object *json)
{
	...
	if (ntohs(top->address.type) == PCE_ADDRESS_IPV4) {
		...
		if (vty != NULL)
			...
			zlog_debug("ODwpYW1YpwGm4vCU ""    PCE Address: %pI4",
				   &top->address.value);
		...
		if (vty != NULL)
			...
			zlog_debug("j2Atvc5i2uxLMNNu ""    PCE Address: unsupported IPv6");
	} else {
		if (vty != NULL)
			vty_out(vty, "  Wrong PCE Address type: 0x%x\n",
				ntohl(top->address.type));
		else
			zlog_debug("FjKqF0mAlSjLATq7 ""    Wrong PCE Address type: 0x%x",
				   ntohl(top->address.type));
	}
					       ...
					       json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("K3T1cZ0pRPrFmq9l ""    PCE Path Scope: 0x%x", ntohl(top->value));
					   ...
					   json_object *json)
{
	...
	if (ntohs(top->type) == PCE_DOMAIN_TYPE_AREA) {
		...
		if (vty != NULL)
			...
			zlog_debug("OI1IjRJ9NZcmPzc6 ""    PCE Domain Area: %pI4", &tmp);
		...
		if (vty != NULL)
			...
			zlog_debug("XgljUFVGBknaUThS ""    PCE Domain AS: %d", ntohl(top->value));
	} else {
		if (vty != NULL)
			vty_out(vty, "  Wrong PCE Domain type: %d\n",
				ntohl(top->type));
		else
			zlog_debug("6DgdgXiJpC2tzc2v ""    Wrong PCE Domain type: %d",
				   ntohl(top->type));
	}
					     ...
					     json_object *json)
{
	...
	if (ntohs(top->type) == PCE_DOMAIN_TYPE_AREA) {
		...
		if (vty != NULL)
			...
			zlog_debug("UmWjN8gUY6msmkSW ""    PCE Neighbor Area: %pI4", &tmp);
	} else if (ntohs(top->type) == PCE_DOMAIN_TYPE_AS) {
		if (vty != NULL)
			if (!json)
			...
			zlog_debug("2X09VFtmBaoVH4H7 ""    PCE Neighbor AS: %d",
				   ntohl(top->value));
	} else {
		if (vty != NULL)
			...
			zlog_debug("Xhh7y2zjwSeZP5Vw ""    Wrong PCE Neighbor type: %d",
				   ntohl(top->type));
	}
					     ...
					     json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("YL5wLFQv2fBehAvr ""    PCE Capabilities Flag: 0x%x",
			   ntohl(top->value));
				     ...
				     size_t buf_size, json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("evz9WfCzV1j73KbZ ""    Unknown TLV: [type(0x%x), length(0x%x)]",
			   ntohs(tlvh->type), ntohs(tlvh->length));
				      ...
				      json_object *json)
{
	...
	if (vty != NULL)
		...
		zlog_debug("0xwMYdwyOD8y9j81 ""  Segment Routing Algorithm TLV:");
		for (i = 0; i < ntohs(algo->header.length); i++)
			switch (algo->value[i]) {
			case 0:
				zlog_debug("OZ1Z5ZW7euxZXkMI ""    Algorithm %d: SPF", i);
				break;
			case 1:
				zlog_debug("LKNdeNgyfx0t11iW ""    Algorithm %d: Strict SPF", i);
				break;
			default:
				zlog_debug("zBDbQcKfroYXK74y ""    Algorithm %d: Unknown value %d",
					   i, algo->value[i]);
				break;
			}
	}
       ...
       "Enable the Router Information functionality with Area flooding scope\n")
{
	...
	if (!OspfRI.registered && (ospf_router_info_register(scope)) != 0) {
		...
		flog_err(
			EC_OSPF_INIT_FAIL,"G5uqZOy9olPtEqev "
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("MW0xSVMgFexrRZe6 ""RI-> Router Information (%s flooding): OFF -> ON",
       ...
       "Disable the Router Information functionality with Area flooding scope\n")
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("zLOM2H7dKFktYpXO ""RI-> Router Information: ON -> OFF");
