			   ...
			   struct ospf_area *area, bool add_only)
{
	...
	if (IS_DEBUG_OSPF_EVENT) {
		if (!add_only)
			zlog_debug("SrESnFFlB9304ndm ""%s: Start", __func__);
		else
			zlog_debug("FlyygkCgomKpVknz ""%s: REACHRUN: Start", __func__);
	}
	lsa = (struct router_lsa *)v->lsa;

	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("yn2pAHRKu5hwdJbp ""%s: LS ID: %pI4", __func__, &lsa->header.id);
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("RANaSheojKISvy8t ""%s: talking about %pFX", __func__, &p);
	...
	if (IS_DEBUG_OSPF_EVENT) {
		if (!add_only)
			zlog_debug("4z73KO5rS1okwbyU ""%s: Stop", __func__);
		else
			zlog_debug("18zKQ59OQh3ISm6P ""%s: REACHRUN: Stop", __func__);
	}
}
			    ...
			    struct ospf_area *area)
{
	...
	if (lsa->mask.s_addr == 0xffffffff) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("pr60BiP9LXDmy9SA ""Suppress installing LSA[Type2,%pI4] route due to host mask",
				   &(lsa->header.id));
		return;
	}
	p.family = AF_INET;
			 ...
			 int parent_is_root, int lsa_pos)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("ZPQP4vhJI5xkAFe2 ""%s: Start", __func__);
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("IVIbwGx824IUpVCs ""%s: processing route to %pFX", __func__, &p);
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("73LhAEMGtTY17Wbh ""%s: calculated cost is %d + %d = %d", __func__,
	...
	if (parent_is_root && link->link_data.s_addr == 0xffffffff
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("FU4ENiTU4Zh8BtEk ""%s: ignoring host route %pI4/32 to self.",
	...
	if (rn->info) {
		...
		if (cost > cur_or->cost) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("KZCQNhsDbU7gzebX ""%s: old route is better, exit",
					   __func__);
			return;
		}
		...
		if (cost == cur_or->cost) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("YI7IBehEj1WD5pwJ ""%s: routes are equal, merge",
					   __func__);
		...
		if (cost < cur_or->cost) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("d3JtQnM32NnVC4Iy ""%s: new route is better, set it",
					   __func__);
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("kxrVMoBEAjMRDJjq ""%s: installing new route", __func__);
	...
	if (v != area->spf) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("fm6FuLHqPkESku6M ""%s: this network is on remote router",
				   __func__);
		ospf_route_copy_nexthops_from_vertex(area, or, v);
	} else {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("4Mgag8ivHAte5a5I ""%s: this network is on this router",
				   __func__);
		...
		if (oi || area->spf_dry_run) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("t7UspbyInFlfroHb ""%s: the lsa pos is %d", __func__,
					   lsa_pos);
			...
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("NPhWYuIxQxRkWB7I ""%s: where's the interface ?",
					   __func__);
		}
	}
	if (rn->info)
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("LOgVJnD15ZNpkkBD ""%s: Stop", __func__);
}
...
void ospf_route_table_dump(struct route_table *rt)
{
	...
	zlog_debug("7i4Yl3JgIxH02hBU ""========== OSPF routing table ==========");
	for (rn = route_top(rt); rn; rn = route_next(rn))
		if ((or = rn->info) != NULL) {
			if (or->type == OSPF_DESTINATION_NETWORK) {
				zlog_debug("qZxIegS1Mf4cdiM6 ""N %-18pFX %-15pI4 %s %d", &rn->p,
					   &or->u.std.area_id,
					   ospf_path_type_str[or->path_type],
					   or->cost);
				for (ALL_LIST_ELEMENTS_RO(or->paths, pnode,
							  path))
					zlog_debug("WzUnW20OVu8rPm4R ""  -> %pI4",
						   &path->nexthop);
			} else
				zlog_debug("SFLNHbDySdpSuVca ""R %-18pI4 %-15pI4 %s %d",
					   &rn->p.u.prefix4,
					   &or->u.std.area_id,
					   ospf_path_type_str[or->path_type],
					   or->cost);
		}
	zlog_debug("Anjzy7yC4GskwSvC ""========================================");
}
...
void ospf_router_route_table_dump(struct route_table *rt)
{
	...
	zlog_debug("obv3iv8JTcFxeGNp ""========== OSPF routing table ==========");
	for (rn = route_top(rt); rn; rn = route_next(rn)) {
		for (ALL_LIST_ELEMENTS_RO((struct list *)rn->info, node, or)) {
			assert(or->type == OSPF_DESTINATION_ROUTER);
			zlog_debug("hHzrMzVYkzJwOiNw ""R %-18pI4 %-15pI4 %s %d", &rn->p.u.prefix4,
				   &or->u.std.area_id,
				   ospf_path_type_str[or->path_type], or->cost);
		}
	}
	zlog_debug("rj128WoYheAQiLPa ""========================================");
}
		   ...
		   struct ospf_route *r2)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("uxSmTlPKHSDTF4N0 ""Route[Compare]: Path types are the same.");
		    ...
		    struct ospf_route *new_or, struct ospf_route *over)
{
	...
	if (rn->info) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("dDNn5pGXnFGP5oAX ""%s: something's wrong !", __func__);
		route_unlock_node(rn);
		return;
	}
...
void ospf_prune_unreachable_networks(struct route_table *rt)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("7vuw4nsD32wXC9Go ""Pruning unreachable networks");
	...
	for (rn = route_top(rt); rn; rn = next) {
		...
		if (rn->info != NULL) {
			...
			if (listcount(or->paths) == 0) {
				if (IS_DEBUG_OSPF_EVENT)
					zlog_debug("kDrWbvl7Jd4uYUIM ""Pruning route to %pFX",
						   &rn->p);
...
void ospf_prune_unreachable_routers(struct route_table *rtrs)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("w8rbqmOENyw0WiFB ""Pruning unreachable routers");
	...
	for (rn = route_top(rtrs); rn; rn = next) {
		...
		for (ALL_LIST_ELEMENTS(paths, node, nnode, or)) {
			if (listcount(or->paths) == 0) {
				if (IS_DEBUG_OSPF_EVENT) {
					zlog_debug("92y3hRl6mkDLyMD3 ""Pruning route to rtr %pI4",
						   &rn->p.u.prefix4);
					zlog_debug("CJfCHDJe6OKT2xmu "
						"               via area %pI4",
						&or->u.std.area_id);
				}
		...
		if (listcount(paths) == 0) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("nwHJzW33cjxkLuDf ""Pruning router node %pI4",
					   &rn->p.u.prefix4);
			   ...
			   bool nssa)
{
	...
	if (rn == NULL) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("vmHxAiupTK9zIewj ""%s: router installation error", __func__);
		return 0;
	}
	...
	if (rn->info) /* If the route to the same destination is found */
	{
		...
		if (!nssa && or->path_type == OSPF_PATH_INTRA_AREA) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("kl5rjSmMcm6GfST3 ""%s: an intra-area route exists",
					   __func__);
			return 0;
		}
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("NbiVy2pGCA2n81ip ""%s: adding %pFX", __func__, p);
			       ...
			       struct prefix_ipv4 *p, bool nssa)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("shaYDYmWJ9ha1Bvf ""%s: deleting %pFX", __func__, p);

	rn = route_node_lookup(rt, (struct prefix *)p);

	if (rn == NULL) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("CUErUOMfGquwmEXI ""%s: no route found", __func__);
		return;
	}
	...
	if (!nssa && or->path_type == OSPF_PATH_INTRA_AREA) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("JteMfRjutOIwWETO ""%s: an intra-area route exists", __func__);
		return;
	}

	if (or->type != OSPF_DESTINATION_DISCARD) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("bRYS57mBzQ3GxY4O ""%s: not a discard entry", __func__);
		return;
	}
