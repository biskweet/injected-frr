...
int ospf_apiserver_init(void)
{
	...
	if (rc != 0) {
		flog_warn(
			EC_OSPF_OPAQUE_REGISTRATION,"B82khTYkPGLgM9fN "
...
static int ospf_apiserver_new_lsa_hook(struct ospf_lsa *lsa)
{
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("QPBVf9NV1Apf8YRj ""API: Put LSA(%p)[%s] into reserve, total=%ld",
...
{
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("kH7Y6iVlWhKUeirT ""API: Get LSA(%p)[%s] from reserve, total=%ld",
			   (void *)lsa, dump_lsa_key(lsa), lsa->lsdb->total);
	return 0;
}
...
void ospf_apiserver_free(struct ospf_apiserver *apiserv)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("AlRtsundoxh41Jyw ""API: Delete apiserv(%p), total#(%d)",
...
void ospf_apiserver_read(struct event *thread)
{
	...
	if (fd == apiserv->fd_sync) {
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("J5MDJlwFBpVxPIjI ""API: %s: Peer: %pI4/%u", __func__,
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("Dk1zO1NHzNuaxLx5 ""API: %s: Peer: %pI4/%u", __func__,
				   &apiserv->peer_async.sin_addr,
				   ntohs(apiserv->peer_async.sin_port));
	}
#endif /* USE_ASYNC_READ */
	else {
		zlog_warn("IAuNGyQuYnuSeSii ""%s: Unknown fd(%d)", __func__, fd);
		ospf_apiserver_free(apiserv);
		return;
	}
	...
	msg = msg_read(fd);
	if (msg == NULL) {
		zlog_warn("KhpdASMCYSoixFwi ""%s: read failed on fd=%d, closing connection",
			  __func__, fd);
...
void ospf_apiserver_sync_write(struct event *thread)
{
	...
	/* Sanity check */
	if (fd != apiserv->fd_sync) {
		zlog_warn("1xhlnxnv2Dh3BreL ""%s: Unknown fd=%d", __func__, fd);
		goto out;
	}

	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("OC5mfzYXko8rIWUK ""API: %s: Peer: %pI4/%u", __func__,
			   &apiserv->peer_sync.sin_addr,
			   ntohs(apiserv->peer_sync.sin_port));
	...
	if (!msg) {
		zlog_warn("Fr71QLlyu0fry1C7 ""API: %s: No message in Sync-FIFO?", __func__);
		return;
	}
	...
	if (rc < 0) {
		zlog_warn("CCy0TbFjRUIzDjqi ""%s: write failed on fd=%d", __func__, fd);
		goto out;
	}
...
void ospf_apiserver_async_write(struct event *thread)
{
	...
	if (fd != apiserv->fd_async) {
		zlog_warn("RjD0Fmno1PDtaKDc ""%s: Unknown fd=%d", __func__, fd);
		goto out;
	}

	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("zlglQQOmsn2Ke0pa ""API: %s: Peer: %pI4/%u", __func__,
			   &apiserv->peer_async.sin_addr,
			   ntohs(apiserv->peer_async.sin_port));
	...
	if (!msg) {
		zlog_warn("UYlyVONG0cy1eMGO ""API: %s: No message in Async-FIFO?", __func__);
		return;
	}
	...
	if (rc < 0) {
		zlog_warn("fHgzAIgDngZIS6NR ""%s: write failed on fd=%d", __func__, fd);
		goto out;
	}
...
int ospf_apiserver_serv_sock_family(unsigned short port, int family)
{
	...
	if (rc < 0) {
		zlog_warn("paLmBRvqsWjkw807 ""%s: listen: %s", __func__, safe_strerror(errno));
		close(accept_sock); /* Close socket */
		return rc;
	}
	return accept_sock;
...
void ospf_apiserver_accept(struct event *thread)
{
	...
	if (new_sync_sock < 0) {
		zlog_warn("vBpbYFP2GLRwyspx ""%s: accept: %s", __func__, safe_strerror(errno));
		return;
	}
	...
	ret = getpeername(new_sync_sock, (struct sockaddr *)&peer_sync,
			  &peerlen);
	if (ret < 0) {
		zlog_warn("7d3GviohN1FttSIe ""%s: getpeername: %s", __func__,
			  safe_strerror(errno));
		close(new_sync_sock);
		return;
	}
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("vSgMXVL043uEdXi1 ""API: %s: New peer: %pI4/%u", __func__,
	...
	if (ntohs(peer_async.sin_port) == ospf_apiserver_getport()) {
		zlog_warn("3h1RcDbwDwepQAzR ""API: %s: Peer(%pI4/%u): Invalid async port number?",
	...
	if (new_async_sock < 0) {
		zlog_warn("Oft7TDJefKtJq9hu ""%s: socket: %s", __func__, safe_strerror(errno));
		close(new_sync_sock);
		return;
	}
	...
	if (ret < 0) {
		zlog_warn("899JXMzwFqN6wOn9 ""%s: connect: %s", __func__, safe_strerror(errno));
		close(new_sync_sock);
	...
	if (ret < 0) {
		zlog_warn("aAkwK2ZpqXxV8rxS ""%s: shutdown: %s", __func__, safe_strerror(errno));
		close(new_sync_sock);
		close(new_async_sock);
		return;
	}
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("Yj0hMp4YX9GGDgCt ""API: New apiserv(%p), total#(%d)", (void *)apiserv,
				   ...
				   struct msg *msg)
{
	...
	case MSG_ROUTER_ID_CHANGE:
		...
		break;
	default:
		zlog_warn("YdJP1RmCePiobVTl ""%s: Unknown message type %d", __func__,
			  msg->hdr.msgtype);
		return -1;
	}
			      ...
			      uint8_t rc)
{
	...
	if (!msg) {
		zlog_warn("EqXiEayZ0CjyqJq0 ""%s: msg_new failed", __func__);
#ifdef NOTYET
...
int ospf_apiserver_handle_msg(struct ospf_apiserver *apiserv, struct msg *msg)
{
	...
	case MSG_SYNC_ROUTER_ID:
		...
		break;
	default:
		zlog_warn("uRhoa6Q92STt66Bd ""%s: Unknown message type: %d", __func__,
			  msg->hdr.msgtype);
		rc = -1;
	}
	return rc;
					...
					uint8_t lsa_type, uint8_t opaque_type)
{
	...
	case OSPF_OPAQUE_AS_LSA:
		...
		break;
	default:
		zlog_warn("V1eDPgsnm0oe9h5A ""%s: lsa_type(%d)", __func__, lsa_type);
	...
	if (rc != 0) {
		flog_warn(EC_OSPF_OPAQUE_REGISTRATION,"I9hHr2DrWbWhYkI9 "
					  ...
					  uint8_t lsa_type, uint8_t opaque_type)
{
	...
	zlog_warn("UnMCRwzATIWgVtOy ""Failed to unregister opaque type [%d/%d]", lsa_type,
		  opaque_type);
	...
	for (ALL_LIST_ELEMENTS(ospf->oiflist, node, nnode, oi)) {
		...
		for (ALL_LIST_ELEMENTS(apiserv->opaque_types, node2, nnode2,
			...
			if (r->lsa_type == OSPF_OPAQUE_LINK_LSA) {
				...
				if (!msg) {
					zlog_warn("k5hCFwKx9QXBbyQB ""%s: msg_new failed",
						  __func__);
#ifdef NOTYET
		...
		for (ALL_LIST_ELEMENTS(apiserv->opaque_types, node2, nnode2,
			...
			if (r->lsa_type == OSPF_OPAQUE_AREA_LSA) {
				...
				msg = new_msg_ready_notify(
					...
					area->area_id);
				if (!msg) {
					zlog_warn("jjg4iLO8tLjQmezy ""%s: msg_new failed",
						  __func__);
#ifdef NOTYET
					/* Cannot allocate new message. What
					 * should we do? */
...
void ospf_apiserver_notify_ready_type11(struct ospf_apiserver *apiserv)
{
	...
	for (ALL_LIST_ELEMENTS(apiserv->opaque_types, node, nnode, r)) {
		...
		if (r->lsa_type == OSPF_OPAQUE_AS_LSA) {
			...
			if (!msg) {
				zlog_warn("0fCBrV1pDDyFX8IY ""%s: msg_new failed", __func__);
#ifdef NOTYET
				   ...
				   int int_arg)
{
	...
	if ((param->filter->origin == ANY_ORIGIN)
		...
		if (!msg) {
			zlog_warn("OXSjcQkbMfsM5MYk ""%s: new_msg_update failed", __func__);
#ifdef NOTYET
			/* Cannot allocate new message. What should we do? */
					       ...
					       struct lsa_header *protolsa)
{
	...
	if ((s = stream_new(OSPF_MAX_LSA_SIZE)) == NULL) {
		zlog_warn("jptiNXdlFDkLCG5T ""%s: stream_new failed", __func__);
		return NULL;
	}
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("5IGrkHP6Xbd9F5Et ""LSA[Type%d:%pI4]: Creating an Opaque-LSA instance",
					    ...
					    struct msg *msg)
{
	...
	case OSPF_OPAQUE_LINK_LSA:
		...
		if (!oi) {
			zlog_warn("pD6744g0SRJAH2HD ""%s: unknown interface %pI4", __func__,
				  &omsg->ifaddr);
	...
	case OSPF_OPAQUE_AREA_LSA:
		...
		if (!area) {
			zlog_warn("MJlN22URg5yhy2mf ""%s: unknown area %pI4", __func__,
				  &omsg->area_id);
	...
	default:
		...
		zlog_warn("nVygNWeVeYZHFACk ""%s: Cannot originate non-opaque LSA type %d",
			  __func__, data->type);
	...
	if (!apiserver_is_opaque_type_registered(apiserv, lsa_type,
						 opaque_type)) {
		zlog_warn("9tNvSjGa84eoEWai ""%s: LSA-type(%d)/Opaque-type(%d): Not registered",
	...
	if (!ready) {
		zlog_warn("TJN6x7W0tgikFnHo ""Neighbors not ready to originate type %d",
			  data->type);
...
int ospf_apiserver_originate1(struct ospf_lsa *lsa, struct ospf_lsa *old)
{
	...
	if (old) {
		...
		if (IS_LSA_MAX_SEQ(old)) {
			flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"136DhX3Is4P5ILXn "
	...
	if (ospf_lsa_install(ospf, lsa->oi, lsa) == NULL) {
		flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"VPdmqNNMEOybMECU "
...
struct ospf_lsa *ospf_apiserver_lsa_refresher(struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("W3E0SnPXhJ7jBq3U ""LSA[Type%d:%pI4]: OSPF API Server LSA Refresher",
			   lsa->data->type, &lsa->data->id);
	}

	apiserv = lookup_apiserver_by_lsa(lsa);
	if (!apiserv) {
		zlog_warn("fp3icL5BpnbLvZCT ""%s: LSA[%s]: No apiserver?", __func__,
			  dump_lsa_key(lsa));
		lsa->data->ls_age =
	...
	if (!new) {
		...
		new = ospf_apiserver_opaque_lsa_new(lsa->area, lsa->oi,
						    lsa->data);
		if (!new) {
			zlog_warn("l74nuyJYQPyaV80B ""%s: Cannot create a new LSA?", __func__);
			goto out;
		}
	} else {
	...
	if (ospf_lsa_install(ospf, new->oi, new) == NULL) {
		flog_warn(EC_OSPF_LSA_INSTALL_FAILURE,"MMqMlbSeb7BAmSdN "
			  "%s: ospf_lsa_install failed", __func__);
		ospf_lsa_unlock(&new);
		goto out;
	}
...
/* Flood updated LSA through interface, area or AS */
	...
	/* Debug logging. */
	if (IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("UN5oGUNxlGFLfpg4 ""LSA[Type%d:%pI4]: Refresh Opaque LSA",
					 ...
					 struct msg *msg)
{
	...
	case OSPF_OPAQUE_LINK_LSA:
		...
		if (!oi) {
			zlog_warn("EAq9ZjVm0rBEBBAn ""%s: unknown interface %pI4", __func__,
				  &dmsg->addr);
			rc = OSPF_API_NOSUCHINTERFACE;
			goto out;
		}
	...
	case OSPF_OPAQUE_AREA_LSA:
		...
		if (!area) {
			zlog_warn("ZkngK3Y887NBUl2s ""%s: unknown area %pI4", __func__,
				  &dmsg->addr);
			rc = OSPF_API_NOSUCHAREA;
			goto out;
		}
	...
	case OSPF_OPAQUE_AS_LSA:
		...
		break;
	default:
		zlog_warn("xFPYfAhb9xFgRlFY ""%s: Cannot delete non-opaque LSA type %d", __func__,
			  dmsg->lsa_type);
		rc = OSPF_API_ILLEGALLSATYPE;
		goto out;
	}
	...
	if (!apiserver_is_opaque_type_registered(apiserv, lsa_type,
						 opaque_type)) {
		zlog_warn("TCo33WA7AvKgg9a2 ""%s: LSA-type(%d)/Opaque-type(%d): Not registered",
			  __func__, lsa_type, opaque_type);
		rc = OSPF_API_OPAQUETYPENOTREGISTERED;
		goto out;
	}
	...
	if (!old) {
		zlog_warn("EnEvLaW5FB5L2PTt ""%s: LSA[Type%d:%pI4] not in LSDB", __func__,
			  dmsg->lsa_type, &id);
...
int ospf_apiserver_new_if(struct interface *ifp)
{
	...
	if (ifp->name[0] == '\0') {
		...
		zlog_warn("DgbvEfP3t0RtadAx ""%s: interface has no name?", __func__);
		return 0;
	}

	/* zlog_warn for debugging */
	zlog_warn("nCT1Fh0ZwMJ6DZ93 ""ospf_apiserver_new_if");
	zlog_warn("lLW3yE3DdHo3XmS4 ""ifp name=%s status=%d index=%d", ifp->name, ifp->status,
		  ifp->ifindex);

	if (ifp->name[0] == '\0') {
		/* interface has empty name */
		zlog_warn("3ng70BQLULZ0k1Eh ""%s: interface has no name?", __func__);
		return 0;
	}
	...
	if (!oi) {
		 ...
		 */
		zlog_warn("ySVCciiToKqvETd8 ""%s: interface %s not known to OSPFd?", __func__,
			  ifp->name);
		return 0;
	}
...
int ospf_apiserver_del_if(struct interface *ifp)
{
	...
	/* zlog_warn for debugging */
	zlog_warn("s2rZSbRBXcvGBivt ""%s ifp name=%s status=%d index=%d", __func__, ifp->name,
	...
	if (!oi) {
		...
		zlog_warn("La9ksWHXSeuc7QVu ""ifp name=%s not known to OSPFd", ifp->name);
		return 0;
	}
...
void ospf_apiserver_ism_change(struct ospf_interface *oi, int old_state)
{
	...
	/* zlog_warn for debugging */
	zlog_warn("FCUc8G79Vxaj4P7X ""%s", __func__);
	if (listcount(apiserver_list) > 0) {
		ospf_apiserver_clients_notify_ism_change(oi);
	}

	zlog_warn("HZcCkRQxuHKvhsim ""%s oi->ifp->name=%s old_state=%d oi->state=%d", __func__,
...
void ospf_apiserver_nsm_change(struct ospf_neighbor *nbr, int old_status)
{
	...
	zlog_warn("knI6P9VUy8yaN1jv ""%s", __func__);
	if (listcount(apiserver_list) > 0) {
			      ...
			      struct ospf_lsa *lsa)
{
	...
	if (json)
		...
		int i;
		zlog_debug("zDiNwC8e32bPR2Nh "
			"    Added using OSPF API: %u octets of opaque data %s",
			opaquelen,
			VALID_OPAQUE_INFO_LEN(lsa->data) ? ""
							 : "(Invalid length?)");
		zlog_debug("PhypIC6FlxAC9UjP ""    Opaque data: ");

		for (i = 0; i < opaquelen; i++) {
			zlog_debug("HzQRB1oW5AhHw9aY ""0x%x ", olsa->data[i]);
		}
	}
	return;
}
...
void ospf_apiserver_clients_notify_ready_type9(struct ospf_interface *oi)
{
	...
	assert(oi);
	if (!oi->address) {
		zlog_warn("izj9bdwt7sqCbORP ""Interface has no address?");
		return;
	}

	if (!ospf_apiserver_is_ready_type9(oi)) {
		zlog_warn("gTvIeP7lqswsElmh ""Interface not ready for type 9?");
		return;
	}
...
void ospf_apiserver_clients_notify_ready_type10(struct ospf_area *area)
{
	...
	if (!ospf_apiserver_is_ready_type10(area)) {
		zlog_warn("v31XMkJKDiFXh2wx ""Area not ready for type 10?");
		return;
	}
...
void ospf_apiserver_clients_notify_ready_type11(struct ospf *top)
{
	...
	if (!ospf_apiserver_is_ready_type11(top)) {
		zlog_warn("k6HumSJ4i716eIJv ""AS not ready for type 11?");
		return;
	}
...
void ospf_apiserver_clients_notify_ism_change(struct ospf_interface *oi)
{
	...
	if (!msg) {
		zlog_warn("N7DhR3d3yZ2FsoWm ""%s: msg_new failed", __func__);
		return;
	}
...
void ospf_apiserver_clients_notify_nsm_change(struct ospf_neighbor *nbr)
{
	...
	msg = new_msg_nsm_change(0, ifaddr, nbraddr, nbr->router_id,
				 nbr->state);
	if (!msg) {
		zlog_warn("Dhp3A2VHebcLi53T ""%s: msg_new failed", __func__);
		return;
	}
					       ...
					       struct ospf_lsa *lsa)
{
	...
	msg = new_msg_lsa_change_notify(msgtype, 0L, /* no sequence number */
					...
					lsa->flags & OSPF_LSA_SELF, lsa->data);
	if (!msg) {
		zlog_warn("A5OkpgGO5sLKIePD ""%s: msg_new failed", __func__);
		return -1;
	}
				     ...
				     struct route_table *nrt)
{
	...
	if (!ort && !nrt) {
		if (IS_DEBUG_OSPF_CLIENT_API)
			zlog_debug("IAjniV8mMw0PDBVa ""%s: no routing tables", __func__);
		return;
	}
	if (nrt && nrt->count)
	...
	while (orn || nrn) {
		...
		if (!cmp) {
			...
			if (IS_DEBUG_OSPF_CLIENT_API)
				zlog_debug("s8WWkTMUvho4sChH ""keeping router id: %pI4",
					   &orn->p.u.prefix4);
			orn = route_next(orn);
			nrn = route_next(nrn);
		} else if (cmp < 0) {
			...
			*d++ = orn->p.u.prefix4;
			if (IS_DEBUG_OSPF_CLIENT_API)
				zlog_debug("ul6nndAc2xeXRAQ8 ""removing router id: %pI4",
					   &orn->p.u.prefix4);
			orn = route_next(orn);
		} else {
			...
			*a++ = nrn->p.u.prefix4;
			if (IS_DEBUG_OSPF_CLIENT_API)
				zlog_debug("Fg8Qul4iYCkH8nA7 ""adding router id: %pI4",
					   &nrn->p.u.prefix4);
			nrn = route_next(nrn);
		}
	}
	...
	while (nadd + nremove) {
		...
		if (IS_DEBUG_OSPF_CLIENT_API)
			zlog_debug("pGfraF9HadhckbYW ""%s: adding %d removing %d", __func__,
...
void ospf_apiserver_clients_notify_router_id_change(struct in_addr router_id)
{
	...
	if (!msg) {
		zlog_warn("RlOoJomyIiqf10QW ""%s: new_msg_router_id_change failed", __func__);
		return;
	}
