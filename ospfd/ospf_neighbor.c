...
void ospf_nbr_delete(struct ospf_neighbor *nbr)
{
	...
	if (rn) {
		...
		if (rn->info) {
			...
			zlog_info("43DBT1gPW8ftQb1A ""Can't find neighbor %pI4 in the interface %s",
				  &nbr->src, IF_NAME(oi));
					  ...
					  struct prefix *p)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("iywQ2mffJscGCu32 ""NSM[%s:%pI4]: start", IF_NAME(oi),
			   &nbr->router_id);
