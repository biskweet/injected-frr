...
static int ospf_abr_nssa_am_elected(struct ospf_area *area)
{
	...
	LSDB_LOOP (ROUTER_LSDB(area), rn, lsa) {
		...
		if (IS_ROUTER_LSA_NT(rlsa)) {
			if (IS_DEBUG_OSPF_NSSA)
				zlog_debug("VJrZ2RNSwvFQQNwF ""%s: router %pI4 asserts Nt",
	...
	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("Jj4HPARmNCcT3qHR ""%s: best electable ABR is: %pI4", __func__, best);
...
void ospf_abr_nssa_check_status(struct ospf *ospf)
{
	...
	for (ALL_LIST_ELEMENTS(ospf->areas, lnode, nnode, area)) {
		...
		if (IS_DEBUG_OSPF(nssa, NSSA))
			zlog_debug("Te00wwh58Nt72Per ""%s: checking area %pI4", __func__,
				   &area->area_id);

		if (!IS_OSPF_ABR(area->ospf)) {
			if (IS_DEBUG_OSPF(nssa, NSSA))
				zlog_debug("OvvBeIqi5Su9QREN ""%s: not ABR", __func__);
			...
			case OSPF_NSSA_ROLE_NEVER:
				...
				if (IS_DEBUG_OSPF(nssa, NSSA))
					zlog_debug("mu8ysJXwQ2QRiU3h ""%s: never translate",
						   __func__);
				area->NSSATranslatorState =
					OSPF_NSSA_TRANSLATE_DISABLED;
				break;
			...
			case OSPF_NSSA_ROLE_ALWAYS:
				 ...
				 */
				if (IS_DEBUG_OSPF(nssa, NSSA))
					zlog_debug("lh3jeAt8H9lv5g2i ""%s: translate always",
						   __func__);
				area->NSSATranslatorState =
			...
			case OSPF_NSSA_ROLE_CANDIDATE:
				...
				if (ospf_abr_nssa_am_elected(area) > 0) {
					...
					area->NSSATranslatorState =
						OSPF_NSSA_TRANSLATE_DISABLED;
					if (IS_DEBUG_OSPF(nssa, NSSA))
						zlog_debug("XZ9LeFkRsT71y1cW ""%s: not elected",
							   __func__);
				}
				break;
			}
...
void ospf_check_abr_status(struct ospf *ospf)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("yzRgFscZgBPQvYha ""%s: Start", __func__);
	...
	if (IS_DEBUG_OSPF_EVENT) {
		zlog_debug("Ru1OSWYzriBvnajg ""%s: looked through areas", __func__);
		zlog_debug("1yYeHGh4z01VeGWj ""%s: bb_configured: %d", __func__, bb_configured);
		zlog_debug("EvTX6bXnV8X96OgJ ""%s: bb_act_attached: %d", __func__,
			   bb_act_attached);
		zlog_debug("NB8O8ptSBAt9xjls ""%s: areas_configured: %d", __func__,
			   areas_configured);
		zlog_debug("D37Km4BAbLd2kMjb ""%s: areas_act_attached: %d", __func__,
			   areas_act_attached);
	}
	...
	if (new_flags != ospf->flags) {
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("HFfuTzGJVKj6kcMS ""%s: new router flags: %x", __func__,
				   new_flags);
...
{
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("7e4C339tJ1Ii01eC ""%s: Start", __func__);

	if (CHECK_FLAG(area->stub_router_state, OSPF_AREA_IS_STUB_ROUTED)
	    && (range->cost != OSPF_STUB_MAX_METRIC_SUMMARY_COST)) {
		range->cost = OSPF_STUB_MAX_METRIC_SUMMARY_COST;
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("xpKqMgufzQubDfhY ""%s: use summary max-metric 0x%08x",
				   __func__, range->cost);
	} else if (range->cost_config != OSPF_AREA_RANGE_COST_UNSPEC) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("eeyskC806U9gf56H ""%s: use configured cost %d", __func__,
				   range->cost_config);
		...
		if (!ospf_area_range_active(range)) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("u3M5eNdu67iuuVbJ ""%s: use cost %d", __func__, cost);
		...
		if (cost > range->cost) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("FWm8aXaIYYqwXqNC ""%s: update to %d", __func__, cost);
...
static int ospf_abr_translate_nssa(struct ospf_area *area, struct ospf_lsa *lsa)
{
	...
	if (!CHECK_FLAG(lsa->data->options, OSPF_OPTION_NP)) {
		if (IS_DEBUG_OSPF_NSSA)
			zlog_debug("vNTgoP2eFHc2vKNg ""%s: LSA Id %pI4, P-bit off, NO Translation",
				   __func__, &lsa->data->id);
		return 1;
	}

	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("8cybz5NlD906uVvh ""%s: LSA Id %pI4, TRANSLATING 7 to 5", __func__,
			   &lsa->data->id);
	...
	if (range) {
		if (IS_DEBUG_OSPF_NSSA)
			zlog_debug("r7GLCBj3Z4VlhSm7 ""Suppressed by range %pI4/%u of area %pI4",
				       ...
				       struct ospf_area *area)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("WF5GyXIS3GvubLbh ""%s: Start", __func__);
	...
	if (old) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("gVoC7g3qIDgFdUw1 ""%s: old summary found", __func__);

		sl = (struct summary_lsa *)old->data;

		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("nAAIBkgO7EyuaJvO ""%s: old metric: %d, new metric: %d",
		...
		if ((GET_METRIC(sl->metric) == full_cost)
			...
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("RMRfvpxxjMOlCDlw ""%s: old summary approved",
					   __func__);
			SET_FLAG(old->flags, OSPF_LSA_APPROVED);
		} else {
			/* LSA is changed, refresh it */
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("JZrnkyGP1SLZIckf ""%s: refreshing summary", __func__);
			set_metric(old, full_cost);
			lsa = ospf_lsa_refresh(area->ospf, old);

			if (!lsa) {
				flog_warn(EC_OSPF_LSA_MISSING,"o0NLEBx2ErtT9qg4 "
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("PlzKFhi8bhe5SMD7 ""%s: creating new summary", __func__);
		lsa = ospf_summary_lsa_originate(p, full_cost, area);
		/* This will flood through area. */

		if (!lsa) {
			flog_warn(EC_OSPF_LSA_MISSING,"yyXuxqgDmlj6yVAp "
		...
		SET_FLAG(lsa->flags, OSPF_LSA_APPROVED);
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("ICw7clBXWSCI6O9m ""%s: flooding new version of summary",
				   __func__);
	}

	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("IvcdFoV634kSk92a ""%s: Stop", __func__);
}
				      ...
				      struct ospf_route * or)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("ROo2XHDsyMzm32yj ""%s: Start", __func__);
	...
	for (ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("gyyzQlaRLfIoa3yH ""%s: looking at area %pI4", __func__,
				   &area->area_id);
...
static void ospf_abr_process_nssa_translates(struct ospf *ospf)
{
	...
	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("3nunHlevJ8rNHc4s ""%s: Start", __func__);
	...
	for (ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		...
		if (IS_DEBUG_OSPF_NSSA)
			zlog_debug("VS1bwl7wQuTdhuUM ""%s(): looking at area %pI4", __func__,
				   &area->area_id);
	...
	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("YguwN4Ew0jqsCOj0 ""%s: Stop", __func__);
}
					...
					struct route_table *rt)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("R8lO3YBo3YewBjmN ""%s: Start", __func__);
	...
	for (rn = route_top(rt); rn; rn = route_next(rn)) {
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("W9Eawy5YLSmdG1lX ""%s: this is a route to %pFX", __func__,
				   &rn->p);
		if (or->path_type >= OSPF_PATH_TYPE1_EXTERNAL) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("NtKY8c0kXiBfjDyD "
		...
		if (
			or->path_type == OSPF_PATH_INTRA_AREA
				  && !ospf_abr_should_announce(
					     ...
					     or)) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("LGSKUjD75W8xlzJS ""%s: denied by export-list",
					   __func__);
			continue;
		}
		...
		if (
			...
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("7EyLbSTSSz4hqKGU ""%s: denied by prefix-list",
					   __func__);
			continue;
		}
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("Ajr0spJiDUb81Ov4 ""%s: announcing", __func__);
		ospf_abr_announce_network(ospf, (struct prefix_ipv4 *)&rn->p,
					  or);
	}

	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("3AV07TEEqz0eZkBB ""%s: Stop", __func__);
}
					  ...
					  struct ospf_area *area)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("t1M8IAaQqV5CJy6v ""%s: Start", __func__);

	old = ospf_lsa_lookup_by_prefix(area->lsdb, OSPF_ASBR_SUMMARY_LSA, p,
					area->ospf->router_id);
	if (old) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("zB6mFFMBy7tviWGB ""%s: old summary found", __func__);
		slsa = (struct summary_lsa *)old->data;

		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("iNdg6UMIfkxjRnFc ""%s: old metric: %d, new metric: %d",
	...
	if (old && (GET_METRIC(slsa->metric) == cost)
	    && ((old->flags & OSPF_LSA_IN_MAXAGE) == 0)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("aGhUhY5JuTJ3ryEB ""%s: old summary approved", __func__);
		SET_FLAG(old->flags, OSPF_LSA_APPROVED);
	} else {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("TVRQMIhYpIIhsMWN ""%s: 2.2", __func__);
		...
		if (!lsa) {
			flog_warn(EC_OSPF_LSA_MISSING,"3upQFSoqZZh1gstn "
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("XHes0xpS58mMbDTs ""%s: flooding new version of summary",
				   __func__);

		/*
		zlog_info("XLGG0Mt56WBa2j2w ""ospf_abr_announce_rtr_to_area(): creating new
		summary");
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("nbLKEHC4VMvrh8L3 ""%s: Stop", __func__);
}
				  ...
				  struct ospf_route * or)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("8DfSMipjohStoC4P ""%s: Start", __func__);

	for (ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("iobd2CiofN267U4c ""%s: looking at area %pI4", __func__,
				   &area->area_id);
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("eMKWIl7Wbjkusbd3 ""%s: Stop", __func__);
}
				       ...
				       struct route_table *rt)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("kGvF0Ne3lM6N1w3G ""%s: Start", __func__);
	...
	for (rn = route_top(rt); rn; rn = route_next(rn)) {
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("Hpsz1ihwTVOU9vUN ""%s: this is a route to %pI4", __func__,
				   &rn->p.u.prefix4);
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("fTxhy8mHlBPyxB0R ""%s: Stop", __func__);
}
...
ospf_abr_unapprove_translates(struct ospf *ospf) /* For NSSA Translations */
{
	...
	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("r0URpIr9iFY5qt1c ""%s: Start", __func__);
	...
	LSDB_LOOP (EXTERNAL_LSDB(ospf), rn, lsa)
		if (CHECK_FLAG(lsa->flags, OSPF_LSA_LOCAL_XLT)) {
			...
			if (IS_DEBUG_OSPF_NSSA)
				zlog_debug("G2h0yjoFtCt97qLO ""%s: approved unset on link id %pI4",
					   __func__, &lsa->data->id);
		}

	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("zHqb5wqk5HGiJsNQ ""%s: Stop", __func__);
}
...
static void ospf_abr_unapprove_summaries(struct ospf *ospf)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("rBXdVDg4aIG8rCEr ""%s: Start", __func__);

	for (ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("K2OnAwxJ1mCMsYfe ""%s: considering area %pI4", __func__,
				   &area->area_id);
		LSDB_LOOP (SUMMARY_LSDB(area), rn, lsa)
			if (ospf_lsa_is_self_originated(ospf, lsa)) {
				if (IS_DEBUG_OSPF_EVENT)
					zlog_debug("GvwXl0mLE6IRkkaI "
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("ERCiZyBsatYnyR7B ""%s: Stop", __func__);
}
...
static void ospf_abr_prepare_aggregates(struct ospf *ospf, bool nssa)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("zNWQpyJ0J0I63JqW ""%s: Start", __func__);
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("Oe8NTp9lzKgdLL6H ""%s: Stop", __func__);
}
...
static void ospf_abr_announce_aggregates(struct ospf *ospf)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("jblVcWXVRJjTbmLE ""%s: Start", __func__);

	for (ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("RODEWieWzmtAkrvN ""%s: looking at area %pI4", __func__,
				   &area->area_id);
		...
		for (rn = route_top(area->ranges); rn; rn = route_next(rn))
			if ((range = rn->info)) {
				...
				if (IS_DEBUG_OSPF_EVENT)
					zlog_debug("scesBcZ294a0FJ4b ""%s: this is range: %pFX",
						   __func__, &p);
				...
				if (ospf_area_range_active(range)) {
					if (IS_DEBUG_OSPF_EVENT)
						zlog_debug("5IHriU8powe69vAD ""%s: active range",
							   __func__);
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("oVDxWcyGF9BmBQP8 ""%s: Stop", __func__);
}
...
static void ospf_abr_send_nssa_aggregates(struct ospf *ospf)
{
	...
	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("wsmDXJUrbDr0V1cq ""%s: Start", __func__);
	...
	for (ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		...
		if (IS_DEBUG_OSPF_NSSA)
			zlog_debug("vmtQWO9HD98wutYa ""%s: looking at area %pI4", __func__,
				   &area->area_id);
		...
		for (rn = route_top(area->nssa_ranges); rn;
			...
			if (IS_DEBUG_OSPF_NSSA)
				zlog_debug("cuVO1PDWRdlViWzx ""%s: this is range: %pFX", __func__,
					   &p);
			...
			if (ospf_area_range_active(range)
			    && CHECK_FLAG(range->flags,
					  OSPF_AREA_RANGE_ADVERTISE)) {
				if (IS_DEBUG_OSPF_NSSA)
					zlog_debug("JqmNwmtpNRJq9gwH ""%s: active range",
						   __func__);
	...
	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("UzkBfMJD99Xp6qJR ""%s: Stop", __func__);
}
...
static void ospf_abr_announce_stub_defaults(struct ospf *ospf)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("YFaVcuaD1300n2H8 ""%s: Start", __func__);
	...
	for (ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("HuuIpMCkpztDOojF ""%s: looking at area %pI4", __func__,
				   &area->area_id);
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("uMc4miNeCmIWdFTr ""%s: announcing 0.0.0.0/0 to area %pI4",
				   __func__, &area->area_id);
		ospf_abr_announce_network_to_area(&p, area->default_cost, area);
	}

	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("iBj5LRbr3O8FqczF ""%s: Stop", __func__);
}
...
void ospf_generate_indication_lsa(struct ospf *ospf, struct ospf_area *area)
{
	...
	if (area_fr_not_supp && !area->fr_info.area_ind_lsa_recvd &&
		...
		new = ospf_summary_asbr_lsa_originate(&p, OSPF_LS_INFINITY,
						      area);
		if (!new) {
			zlog_debug("f3psNMT4edPKmHyJ ""%s: Indication lsa originate failed",
				   __func__);
			return;
		}
						       ...
						       struct ospf_lsa *lsa)
{
	if (CHECK_FLAG(lsa->flags, OSPF_LSA_LOCAL_XLT)
		...
		zlog_info("VOX3uDq1LGniEVlk ""%s: removing unapproved translates, ID: %pI4",
			  __func__, &lsa->data->id);
...
static void ospf_abr_remove_unapproved_translates(struct ospf *ospf)
{
	...
	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("VLp9dyPoroEtEHUG ""%s: Start", __func__);

	LSDB_LOOP (EXTERNAL_LSDB(ospf), rn, lsa)
		ospf_abr_remove_unapproved_translates_apply(ospf, lsa);

	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("cYBhQQ767qHPd4fM ""%s: Stop", __func__);
}
...
static void ospf_abr_remove_unapproved_summaries(struct ospf *ospf)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("0tzGZlkrOJSYhdk3 ""%s: Start", __func__);

	for (ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("ENJlw6lo4dHJyClo ""%s: looking at area %pI4", __func__,
				   &area->area_id);
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("K0uz6hkHdLMUOXkB ""%s: Stop", __func__);
}
...
void ospf_abr_nssa_task(struct ospf *ospf) /* called only if any_nssa */
{
	...
	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("9eFv61PblSIMHGZP ""Check for NSSA-ABR Tasks():");
	...
	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("qSG84Ew2kezQYh4q ""%s: Start", __func__);

	/* For all Global Entries flagged "local-translate", unset APPROVED */
	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("4aucZ0mjsScdQn0i ""%s: unapprove translates", __func__);

	ospf_abr_unapprove_translates(ospf);

	/* RESET all Ranges in every Area, same as summaries */
	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("pUG3Bky6XEUIvRtr ""%s: NSSA initialize aggregates", __func__);
	 ...
	 */
	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("0Ex11bFXKXEuWdLC ""%s: process translates", __func__);
	 ...
	 */
	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("QuC9sQ0rmsjWAAim ""%s: send NSSA aggregates", __func__);
	ospf_abr_send_nssa_aggregates(ospf); /*TURNED OFF FOR NOW */

	/* Send any NSSA defaults as Type-5
	 *if (IS_DEBUG_OSPF_NSSA)
	 * zlog_debug("59rjurw1bPdc2ZnO ""ospf_abr_nssa_task(): announce nssa defaults");
	...
	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("13qbdUvEihoyb6Jr ""%s: remove unapproved translates", __func__);
	ospf_abr_remove_unapproved_translates(ospf);

	ospf_abr_manage_discard_routes(ospf, true);

	if (IS_DEBUG_OSPF_NSSA)
		zlog_debug("LEZpi5JSzFKlzqRN ""%s: Stop", __func__);
}
...
void ospf_abr_task(struct ospf *ospf)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("jKNpDpFNBQbUVOW3 ""%s: Start", __func__);

	if (ospf->new_table == NULL || ospf->new_rtrs == NULL) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("KxCbC6O99MBM4WbV ""%s: Routing tables are not yet ready",
				   __func__);
		return;
	}

	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("P1hLrfYNq3KpkBSM ""%s: unapprove summaries", __func__);
	ospf_abr_unapprove_summaries(ospf);

	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("2ifJ9XUgoxAoiONy ""%s: prepare aggregates", __func__);
	ospf_abr_prepare_aggregates(ospf, false);

	if (IS_OSPF_ABR(ospf)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("ts8DBF5r99pSpacD ""%s: process network RT", __func__);
		ospf_abr_process_network_rt(ospf, ospf->new_table);

		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("JCpPUpGNmxbq2MI2 ""%s: process router RT", __func__);
		ospf_abr_process_router_rt(ospf, ospf->new_rtrs);

		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("Aw9NHyj8wUcw76lh ""%s: announce aggregates", __func__);
		ospf_abr_announce_aggregates(ospf);

		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("IueaxPJ1XGlKa0zU ""%s: announce stub defaults", __func__);
		ospf_abr_announce_stub_defaults(ospf);

		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("SUCwqwZTw3A7PCCH ""%s: announce NSSA Type-7 defaults",
				   __func__);
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("OCVjr00LGG7JGeNv ""%s: remove unapproved summaries", __func__);
	ospf_abr_remove_unapproved_summaries(ospf);

	ospf_abr_manage_discard_routes(ospf, false);

	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("fVq3qK9PUBLjO3eI ""%s: Stop", __func__);
}
...
static void ospf_abr_task_timer(struct event *thread)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("uD02DoiTIkiVLYst ""Running ABR task on timer");
...
{
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("RCj47UGHswPBO7LG ""Scheduling ABR task");
