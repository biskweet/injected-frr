...
void ospf_process_refresh_data(struct ospf *ospf, bool reset)
{
	...
	if (!ospf->oi_running) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("yN350ELTNOkqfS5S "
				"Router ospf not configured -- Router-ID update postponed");
		return;
	}

	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("oDKbEORX92uoXG7T ""Router-ID[OLD:%pI4]: Update",
			   &ospf->router_id);
	...
	*/
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("i2sul8UYJhmmX3lJ ""Router-ID[OLD:%pI4]: Update to %pI4",
	...
	if (rid_change || (reset)) {
		...
		ospf->router_id = router_id;
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("aOyIsRjrfecyP0f8 ""Router-ID[NEW:%pI4]: Update",
				   &ospf->router_id);
...
static struct ospf *ospf_new(unsigned short instance, const char *name)
{
	...
	if ((ospf_sock_init(new)) < 0) {
		flog_err(EC_LIB_SOCKET,"cr0QWXSIN3dMAzOB "
...
static void ospf_area_type_set(struct ospf_area *area, int type)
{
	...
	if (area->external_routing == type) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("KVqBzbrhhRsEGnsH ""Area[%pI4]: Types are the same, ignored.",
				   &area->area_id);
		return;
	}
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("F6tCqLLZfB9K1ZDt ""Area[%pI4]: Configured as %s",
			   &area->area_id,
		...
		for (ALL_LIST_ELEMENTS_RO(area->oiflist, node, oi))
			if (oi->nbr_self != NULL) {
				...
				UNSET_FLAG(oi->nbr_self->options,
					   OSPF_OPTION_E);
				if (IS_DEBUG_OSPF_EVENT)
					zlog_debug("3DWkVTzAsONJ8wwU ""options set on %s: %x",
	...
	case OSPF_AREA_NSSA:
		for (ALL_LIST_ELEMENTS_RO(area->oiflist, node, oi))
			if (oi->nbr_self != NULL) {
				zlog_debug("jclY7GAfbyhZC5to "
					"setting nssa options on %s accordingly",
					IF_NAME(oi));
				UNSET_FLAG(oi->nbr_self->options,
					   OSPF_OPTION_E);
				SET_FLAG(oi->nbr_self->options, OSPF_OPTION_NP);
				zlog_debug("WFN7mpD6zcKEBVUy ""options set on %s: %x", IF_NAME(oi),
					   OPTIONS(oi));
			}
		break;
	default:
...
{
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("CayznvlcN0E4roJj ""%s: VRF Created: %s(%u)", __func__, vrf->name,
			   vrf->vrf_id);
...
{
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("0nUG4KpqkcDUBOXr ""%s: VRF Deletion: %s(%u)", __func__, vrf->name,
			   vrf->vrf_id);
...
static int ospf_vrf_enable(struct vrf *vrf)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("avHoTil3Sv3GfDzf ""%s: VRF %s id %u enabled", __func__, vrf->name,
			   vrf->vrf_id);
...
static int ospf_vrf_disable(struct vrf *vrf)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("3Z60yYGfAzEldVSa ""%s: VRF %s id %d disabled.", __func__, vrf->name,
			   vrf->vrf_id);
	...
	if (ospf) {
		...
		ospf->oi_running = 0;
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("Jk6M2Gy4o9VE0PVo ""%s: ospf old_vrf_id %d unlinked", __func__,
				   old_vrf_id);
		EVENT_OFF(ospf->t_read);
		close(ospf->fd);
		ospf->fd = -1;
