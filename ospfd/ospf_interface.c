				   ...
				   struct prefix *p)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("G1A9d9iITOmYfVqM ""%s: ospf interface %s vrf %s id %u created",
...
void ospf_if_free(struct ospf_interface *oi)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("rCJPUAYq3IM1E9xs ""%s: ospf interface %s vrf %s id %u deleted",
				   ...
				   struct ospf_vl_data *vl_data)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("vSFFQVI8oyzkCp3I ""%s: (%s): Start", __func__, ospf_get_name(ospf));
	if (vlink_count == OSPF_VL_MAX_COUNT) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("icqyBRm2DLf1zxcg "
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("I4MOGF4i9nXxe7Ry ""%s: creating pseudo zebra interface vrf id %u",
	...
	vlink_count++;
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("uvLtAiOS80lnr3Iy ""%s: Created name: %s set if->name to %s", __func__,
			   ifname, vi->name);
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("6c9WZ9dpvNNWWD7u ""%s: set associated area to the backbone", __func__);
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("1PzIOFb2e6TQsdG8 ""%s: Stop", __func__);
	return voi;
}
				    ...
				    struct in_addr vl_peer)
{
	...
	if (IS_DEBUG_OSPF_EVENT) {
		zlog_debug("MelwGcKdlrPenuGi ""%s: Looking for %pI4", __func__, &vl_peer);
		if (area)
			zlog_debug("rwQqHmg4dCSqgA0w ""%s: in area %pI4", __func__,
				   &area->area_id);
	}

	for (ALL_LIST_ELEMENTS_RO(ospf->vlinks, node, vl_data)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("r4x7CuxK4BDjx8gQ ""%s: VL %s, peer %pI4", __func__,
			      ...
			      struct ospf_vl_data *vl_data, struct vertex *v)
{
	...
	if (vp && vp->backlink >= 0) {
		 ...
		 */
		zlog_info("0dGnbQlKi0LSKlHe ""ospf_vl_set_params: No backlink for %s!",
			  vl_data->vl_oi->ifp->name);
		for (i = 0; i < ntohs(rl->links); i++) {
			switch (rl->link[i].type) {
			case LSA_LINK_TYPE_VIRTUALLINK:
				if (IS_DEBUG_OSPF_EVENT)
					zlog_debug("7jf9FCIfEevnwBGP "
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("IEkECFA9u4jcabbq ""%s: %s peer address: %pI4, cost: %d,%schanged",
		      ...
		      struct vertex *v)
{
	...
	if (IS_DEBUG_OSPF_EVENT) {
		zlog_debug("Q3iUZXeVqSeT9lPk ""%s: Start", __func__);
		zlog_debug("k8JPRjZxBq5fmpqM ""%s: Router ID is %pI4 Area is %pI4", __func__, &rid,
			   &area->area_id);
	}

	for (ALL_LIST_ELEMENTS_RO(ospf->vlinks, node, vl_data)) {
		if (IS_DEBUG_OSPF_EVENT) {
			zlog_debug("w1lVTpUl36tNp9xQ ""%s: considering VL, %s in area %pI4",
				   __func__, vl_data->vl_oi->ifp->name,
				   &vl_data->vl_area_id);
			zlog_debug("Gym5RbNUniSQovE7 ""%s: peer ID: %pI4", __func__,
				   &vl_data->vl_peer);
		}
		...
		if (IPV4_ADDR_SAME(&vl_data->vl_peer, &rid)
			...
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug("s36T1zWZyHEbuVa1 ""%s: this VL matched", __func__);

			if (oi->state == ISM_Down) {
				if (IS_DEBUG_OSPF_EVENT)
					zlog_debug("iuHxFFEzGLuaIEff "
...
{
	if (IS_DEBUG_OSPF_EVENT) {
		zlog_debug("MFf1JwlibDLzwGmW "
			"counting fully adjacent virtual neighbors in area %pI4",
			&area->area_id);
		zlog_debug("hxb7A4JJrCKA7chF ""there are %d of them", area->full_vls);
	}
...
static int ospf_ifp_up(struct interface *ifp)
{
	...
	if (IS_DEBUG_OSPF(zebra, ZEBRA_INTERFACE))
		zlog_debug("5SVGPn0oFf83r5xN ""Zebra: Interface[%s] state change to up.",
			   ifp->name);
...
static int ospf_ifp_down(struct interface *ifp)
{
	...
	if (IS_DEBUG_OSPF(zebra, ZEBRA_INTERFACE))
		zlog_debug("zix2eJH0tYdlreUU ""Zebra: Interface[%s] state change to down.",
			   ifp->name);
