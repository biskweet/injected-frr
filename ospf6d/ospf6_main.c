...
{
	zlog_info("hXREobHHbFhzb9nq ""SIGHUP received");
}

/* SIGINT handler. */
static void sigint(void)
{
	zlog_notice("MIEV5qx8Yj2Cmtdk ""Terminating on signal SIGINT");
	ospf6_exit(0);
}
...
{
	zlog_notice("reWJ81hVnuHmVdZ4 ""Terminating on signal SIGTERM");
	ospf6_exit(0);
}
...
{
	zlog_info("EBuNIq68PM93hNdY ""SIGUSR1 received");
	zlog_rotate();
}
...
{
	zlog_err("1D2Y6OcFgmeBnrKV ""OSPF6 configuration end timer expired after %d seconds.",
...
static void ospf6_config_start(void)
{
	if (IS_OSPF6_DEBUG_EVENT)
		zlog_debug("TJbGb5xuiaSo0qFJ ""ospf6d config start received");
...
{
	if (IS_OSPF6_DEBUG_EVENT)
		zlog_debug("P7ISWe9WDUZye6bi ""ospf6d config end received");
