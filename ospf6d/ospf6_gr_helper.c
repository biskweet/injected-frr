					  ...
					  uint32_t *interval, uint8_t *reason)
{
	...
	if (ntohs(lsah->length) <= OSPF6_LSA_HEADER_SIZE) {
		if (IS_DEBUG_OSPF6_GR)
			zlog_debug("FgCdqOTKLV6clSBe ""%s: undersized (%u B) lsa", __func__,
				   ntohs(lsah->length));
	...
	for (tlvh = TLV_HDR_TOP(lsah); sum < length && tlvh;
		...
		default:
			sum += TLV_SIZE(tlvh);
			if (IS_DEBUG_OSPF6_GR)
				zlog_debug("da8qHF76QUPFSgJh ""%s, Ignoring unknown TLV type:%d",
			    ...
			    struct ospf6_neighbor *restarter)
{
	...
	if (ret != OSPF6_SUCCESS) {
		if (IS_DEBUG_OSPF6_GR)
			zlog_debug("it7UztZT7GtJsyf5 ""%s, Wrong Grace LSA packet.", __func__);
		return OSPF6_GR_NOT_HELPER;
	}

	if (IS_DEBUG_OSPF6_GR)
		zlog_debug("64uk5hI5si31UO5C "
	...
	if (IS_DEBUG_OSPF6_GR)
		zlog_debug("TJE05ffFeJhUiy3l ""%s, Grace timer started.interval:%u", __func__,
			   actual_grace_interval);
			  ...
			  enum ospf6_helper_exit_reason reason)
{
	...
	if (IS_DEBUG_OSPF6_GR)
		zlog_debug("RCZkLEnwqiWzkCKk ""%s, Exiting from HELPER support to %pI6, due to %s",
	...
	if (reason != OSPF6_GR_HELPER_COMPLETED) {
		if (IS_DEBUG_OSPF6_GR)
			zlog_debug("ncjbkMtKIofZuQoq ""%s, Unsuccessful GR exit. RESTARTER : %pI6",
				   __func__, &nbr->linklocal_addr);
	}
				    ...
				    struct ospf6_neighbor *restarter)
{
	...
	if (ret != OSPF6_SUCCESS) {
		if (IS_DEBUG_OSPF6_GR)
			zlog_debug("4fklXhR1Qu87cQRz ""%s, Wrong Grace LSA packet.", __func__);
		return;
	}

	if (IS_DEBUG_OSPF6_GR)
		zlog_debug("PrZvFCtipx1fEJvt ""%s, GraceLSA received for neighbour %pI4.",
...
void ospf6_helper_handle_topo_chg(struct ospf6 *ospf6, struct ospf6_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF6_GR)
		zlog_debug("thfEc21atZg7KPsB ""%s, Topo change detected due to lsa details : %s",
			   __func__, lsa->name);
				     ...
				     json_object *json, bool use_json)
{
	...
	if (ntohs(lsah->length) <= OSPF6_LSA_HEADER_SIZE) {
		if (IS_DEBUG_OSPF6_GR)
			zlog_debug("IKrbMnGzMORkYFUC ""%s: undersized (%u B) lsa", __func__,
				   ntohs(lsah->length));
		return OSPF6_FAILURE;
	}
	...
	if (vty) {
		...
		zlog_debug("gHn7yUfy92R4pixB ""  TLV info:");
	}
	...
	for (tlvh = TLV_HDR_TOP(lsah); sum < length && tlvh;
		...
		if (sum + TLV_SIZE(tlvh) > length) {
			if (vty)
				...
				zlog_debug("cVWxZMznNbGpZ8fl ""%% Invalid TLV length: %d",
					   TLV_SIZE(tlvh));
			return OSPF6_FAILURE;
		}
		...
		case GRACE_PERIOD_TYPE:
			...
			if (vty) {
				...
				zlog_debug("wOppZpKP3jdTcxMn ""    Grace period:%d",
					   ntohl(gracePeriod->interval));
			}
			break;
		case RESTART_REASON_TYPE:
			...
			if (vty) {
				...
				zlog_debug("MdxM2tGno7pr2FDr ""    Restart reason:%s",
					   ospf6_restart_reason_desc
						   [grReason->reason]);
			}
			break;
...
{
	if (IS_DEBUG_OSPF6_GR)
		zlog_debug("I4W2nMzf3EP068iN ""%s, GR Helper init.", __func__);
...
void ospf6_gr_helper_deinit(struct ospf6 *ospf6)
{
	...
	if (IS_DEBUG_OSPF6_GR)
		zlog_debug("MYq6x62vyfCENtb9 ""%s, GR helper deinit.", __func__);
