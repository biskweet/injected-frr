...
bool ospf6_check_and_set_router_abr(struct ospf6 *o)
{
	...
	for (ALL_LIST_ELEMENTS_RO(o->area_list, node, oa)) {
		if (IS_OSPF6_DEBUG_ABR)
			zlog_debug("Gd6znJxSZ6XgT9Kp ""%s, area_id %pI4", __func__, &oa->area_id);
	...
	if ((area_count > 1) && (is_backbone)) {
		if (IS_OSPF6_DEBUG_ABR)
			zlog_debug("6uGL7LZJUpdDtz4U ""%s : set flag OSPF6_FLAG_ABR", __func__);
		SET_FLAG(o->flag, OSPF6_FLAG_ABR);
		return true;
	} else {
		if (IS_OSPF6_DEBUG_ABR)
			zlog_debug("5QvqBDira8cdt49r ""%s : reset flag OSPF6_FLAG_ABR", __func__);
					...
					struct ospf6_area *area)
{
	...
	if (IS_OSPF6_DEBUG_ABR) {
		...
		zlog_debug("wVhGyRGLynnCblnI ""%s : start area %s, route %s", __func__, area->name,
			   buf);
	}
	...
	if (route->path.type == OSPF6_PATH_TYPE_EXTERNAL1
		...
		if (IS_OSPF6_DEBUG_ABR)
			zlog_debug("OnClMeQySQ0IZP4x ""%s : Path type is external, skip",
				   __func__);
		return 0;
	}
	...
	if (area->no_summary
		...
		if (is_debug)
			zlog_debug("V4TCxJ5wThxAIB5w ""Area has been stubbed, purge prefix LSA");
	...
	if (route->type == OSPF6_DEST_TYPE_RANGE) {
		...
		if (!CHECK_FLAG(route->flag, OSPF6_ROUTE_ACTIVE_SUMMARY)) {
			if (is_debug)
				zlog_debug("5Op3LqRoqbp7Y7i5 ""The range is not active. withdraw");
			ospf6_abr_delete_route(summary, summary_table, old);
			return 0;
		}
	}
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("6vuAOWCrP3n5cveh ""%s : finish area %s", __func__, area->name);
...
void ospf6_abr_range_update(struct ospf6_route *range, struct ospf6 *ospf6)
{
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("9I7tNXylWkRZ1mgn ""%s: range %pFX, cost %d", __func__, &range->prefix,
			   cost);
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("qC2ZHWzmQ0OysK7x ""%s: range %pFX update", __func__, &range->prefix);
	...
	if (CHECK_FLAG(range->flag, OSPF6_ROUTE_ACTIVE_SUMMARY)
		...
		if (!CHECK_FLAG(range->flag, OSPF6_ROUTE_BLACKHOLE_ADDED)) {
			if (IS_OSPF6_DEBUG_ABR)
				zlog_debug("spTIQD1g9J27EKRV ""Add discard route");
		...
		if (CHECK_FLAG(range->flag, OSPF6_ROUTE_BLACKHOLE_ADDED)) {
			if (IS_OSPF6_DEBUG_ABR)
				zlog_debug("kjWlG3qsvIJWnnZ6 ""Delete discard route");
...
void ospf6_abr_originate_summary(struct ospf6_route *route, struct ospf6 *ospf6)
{
	...
	if (IS_OSPF6_DEBUG_ABR) {
		...
		zlog_debug("EBoMQHOelR2CGRwT ""%s: route %s", __func__, buf);
	}

	if (route->type == OSPF6_DEST_TYPE_NETWORK) {
		oa = ospf6_area_lookup(route->path.area_id, ospf6);
		if (!oa) {
			zlog_err("95eQEnsXFlf1ISGi ""OSPFv6 area lookup failed");
			return;
		}
			       ...
			       struct ospf6_route_table *table)
{
	...
	for (ALL_LIST_ELEMENTS(old_route->paths, anode, anext, o_path)) {
		...
		if (IS_OSPF6_DEBUG_ABR || IS_OSPF6_DEBUG_EXAMIN(INTER_PREFIX))
			zlog_debug("j1W6AW1UutFWa9M4 ""%s: paths %u nh %u", __func__,
				   old_route->paths
...
{
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("Pkf2X3Wivxqo4d8r ""%s: route %pFX, paths %d", __func__, &old->prefix,
			   listcount(old->paths));
			...
			for (ALL_LIST_ELEMENTS_RO(o_path->nh_list, nnode, nh)) {
				for (ALL_LIST_ELEMENTS(old->nh_list,
					...
					if (!ospf6_nexthop_is_same(rnh, nh))
						continue;
					if (IS_OSPF6_DEBUG_ABR)
						zlog_debug("kAeKuke07CuJMcfO ""deleted nexthop");
		...
		if (nh_updated) {
			if (listcount(old->paths)) {
				if (IS_OSPF6_DEBUG_ABR
					...
					zlog_debug("gaYvSnUkMfzDUose ""%s: old %pFX updated nh %u",
						   __func__, &old->prefix,
...
void ospf6_abr_examin_summary(struct ospf6_lsa *lsa, struct ospf6_area *oa)
{
	...
	if (lsa->header->type == htons(OSPF6_LSTYPE_INTER_PREFIX)) {
		if (IS_OSPF6_DEBUG_EXAMIN(INTER_PREFIX)) {
			is_debug++;
			zlog_debug("Su6huNYSU0JX4PN1 ""%s: LSA %s age %d in area %s", __func__,
		...
		if (IS_OSPF6_DEBUG_EXAMIN(INTER_ROUTER)) {
			is_debug++;
			zlog_debug("L6uLMBqCgcz3uvRA ""%s: LSA %s age %d in area %s", __func__,
				   lsa->name, ospf6_lsa_age_current(lsa),
				   oa->name);
		}
	...
	if (route) {
		...
		if (is_debug)
			zlog_debug("9QXYF9gkKmPFUWNU ""%s: route %pFX, paths %d", __func__,
	...
	if (cost == OSPF_LS_INFINITY) {
		if (is_debug)
			zlog_debug("gIrvkaF6N4ooX59f ""cost is LS_INFINITY, ignore");
		if (old)
	...
	if (OSPF6_LSA_IS_MAXAGE(lsa)) {
		if (is_debug)
			zlog_debug("egQXsDROwpvFubzL ""%s: LSA %s is MaxAge, ignore", __func__,
				   lsa->name);
		if (old)
			ospf6_abr_old_route_remove(lsa, old, table);
		return;
	...
	if (lsa->header->adv_router == oa->ospf6->router_id) {
		if (is_debug)
			zlog_debug("3mqEGwRX36TE1xkl ""LSA %s is self-originated, ignore",
				   lsa->name);
		if (old)
			ospf6_route_remove(old, table);
		return;
	...
	if (lsa->header->type == htons(OSPF6_LSTYPE_INTER_PREFIX)) {
		...
		if (CHECK_FLAG(prefix_lsa->prefix.prefix_options,
			...
			if (is_debug)
				zlog_debug("6AEXmSgKxYFHrsb8 ""Prefix has the NU bit set, ignore");
			if (old)
				ospf6_route_remove(old, table);
			return;
		}
	...
	*/
	...
	if (old) {
		if (is_debug)
			zlog_debug("CzLnLHFw3XArqGc0 ""%s: found old route %pFX, paths %d",
...
void ospf6_abr_prefix_resummarize(struct ospf6 *o)
{
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("WXHExYuAasTD47T0 ""Re-examining Inter-Prefix Summaries");
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("YCJfFMdBnFXT8jb2 ""Finished re-examining Inter-Prefix Summaries");
}
