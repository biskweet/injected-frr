...
void ospf6_router_lsa_originate(struct event *thread)
{
	...
	if (oa->ospf6->gr_info.restart_in_progress) {
		if (IS_DEBUG_OSPF6_GR)
			zlog_debug("XlXfj2NIl9qRKZIn "
				"Graceful Restart in progress, don't originate LSA");
		return;
	}

	if (IS_OSPF6_DEBUG_ORIGINATE(ROUTER))
		zlog_debug("PixSZPRQElPhr3QC ""Originate Router-LSA for Area %s", oa->name);
...
void ospf6_network_lsa_originate(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_ORIGINATE(NETWORK))
		zlog_debug("b0mMXADwoWyBXzr8 ""Originate Network-LSA for Interface %s",
			   oi->interface->name);
	...
	if (count == 0) {
		if (IS_OSPF6_DEBUG_ORIGINATE(NETWORK))
			zlog_debug("3IdUDFyHe8ztxV7N ""Interface stub, ignore");
		if (old)
			ospf6_lsa_purge(old);
		return;
	}
...
void ospf6_link_lsa_originate(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_ORIGINATE(LINK))
		zlog_debug("pQ6xRQOvXQp0auq8 ""Originate Link-LSA for Interface %s",
			   oi->interface->name);

	/* can't make Link-LSA if linklocal address not set */
	if (oi->linklocal_addr == NULL) {
		if (IS_OSPF6_DEBUG_ORIGINATE(LINK))
			zlog_debug("jNyGFL3Yd2E0J8tG "
...
void ospf6_intra_prefix_lsa_originate_stub(struct event *thread)
{
	...
	for (ALL_LIST_ELEMENTS_RO(oa->if_list, i, oi)) {
		if (oi->state == OSPF6_INTERFACE_DOWN) {
			if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
				zlog_debug("PAHBXmm3Jd5OWbe9 ""  Interface %s is down, ignore",
					   oi->interface->name);
			continue;
		}
		...
		if (oi->state != OSPF6_INTERFACE_LOOPBACK
		    ...
		    && full_count != 0) {
			if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
				zlog_debug("EXSrUnX1nVcTotBa ""  Interface %s is not stub, ignore",
					   oi->interface->name);
			continue;
		}

		if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
			zlog_debug("f4UhovlamWQuQ9h0 ""  Interface %s:", oi->interface->name);

		/* connected prefix to advertise */
		for (route = ospf6_route_head(oi->route_connected); route;
		     route = ospf6_route_best_next(route)) {
			if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
				zlog_debug("N5QWHAF7DAbDVJKh ""    include %pFX", &route->prefix);
...
void ospf6_intra_prefix_lsa_originate_transit(struct event *thread)
{
	...
	if (oi->state != OSPF6_INTERFACE_DR) {
		if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
			zlog_debug("hTgFsBUc0CYZM1rM ""  Interface is not DR");
		if (old)
			ospf6_lsa_purge(old);
		return;
	}
	...
	if (full_count == 0) {
		if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
			zlog_debug("824QSlDfUxhdNV41 ""  Interface is stub");
		if (old)
			ospf6_lsa_purge(old);
		return;
	}
	...
	for (ALL_LSDB_TYPED(oi->lsdb, type, lsa)) {
		...
		if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
			zlog_debug("bS4xbw9U7QJ9Qsan ""  include prefix from %s", lsa->name);
		...
		for (current = start; current < end && prefix_num;
			...
			if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
				zlog_debug("3NKrZH6TTDdUE1oV ""    include %pFX", &route->prefix);

			ospf6_route_add(route, route_advertise);
			prefix_num--;
		}
		if (current != end && IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
			zlog_debug("12EwvUUcRRgr02eW ""Trailing garbage in %s", lsa->name);
	}
...
void ospf6_intra_prefix_lsa_add(struct ospf6_lsa *lsa)
{
	...
	if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
		zlog_debug("B1IPkvwg4wx0lRzS ""%s: LSA %s found", __func__, lsa->name);
	...
	if (intra_prefix_lsa->ref_type == htons(OSPF6_LSTYPE_ROUTER) ||
		...
		if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
			zlog_debug("LWk7YbpkTcWTcFVN ""Unknown reference LS-type: %#hx",
	...
	if (ls_entry == NULL) {
		if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX)) {
			ospf6_linkstate_prefix2str(&ls_prefix, buf,
						   sizeof(buf));
			zlog_debug("WDnPsVmow5iGgh1y ""LS entry does not exist: %s", buf);
		}
		return;
	}
	...
	if (current != end && IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
		zlog_debug("a4iaJP9Kfv45HJgM ""Trailing garbage ignored");
}
...
void ospf6_intra_prefix_lsa_remove(struct ospf6_lsa *lsa)
{
	...
	if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
		zlog_debug("mVMz0rLMYv25CGwj ""%s: %s disappearing", __func__, lsa->name);
	...
	if (current != end && IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
		zlog_debug("lrIgtEOdRFKfmb26 ""Trailing garbage ignored");
}
...
void ospf6_intra_route_calculation(struct ospf6_area *oa)
{
	...
	if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
		zlog_debug("A28FeDI2wfnY165p ""Re-examin intra-routes for area %s", oa->name);
	...
	for (route = ospf6_route_head(oa->route_table); route; route = nroute) {
		if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX)) {
			prefix2str(&route->prefix, buf, sizeof(buf));
			zlog_debug("lBKEl4pPvXMklJHf ""%s: route %s, flag 0x%x", __func__, buf,
				   route->flag);
		}
		...
		if (CHECK_FLAG(route->flag, OSPF6_ROUTE_REMOVE))
			...
			if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
				zlog_debug("Cdq96UpFlVNBfVrk ""%s: Originate summary for route %s",
					   __func__, buf);
	...
	if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
		zlog_debug("xkMCHDVGCnJWbHsg ""Re-examin intra-routes for area %s: Done",
			   oa->name);
}
...
static void ospf6_brouter_debug_print(struct ospf6_route *brouter)
{
	...
	zlog_info("nDrpApNNOkQzBWNN ""Brouter: %s via area %s", brouter_name, area_name);
	zlog_info("X6g39n6cV79BfRqx ""  memory: prev: %p this: %p next: %p parent rnode: %p",
		  (void *)brouter->prev, (void *)brouter, (void *)brouter->next,
		  (void *)brouter->rnode);
	zlog_info("pa4eGQuHacHojGiJ ""  type: %d prefix: %s installed: %s changed: %s",
		  brouter->type, destination, installed, changed);
	zlog_info("NrXM78lq0L52veCX ""  lock: %d flags: %s%s%s%s", brouter->lock,
	...
	zlog_info("cV54RTwITtPrkyjH ""  path type: %s ls-origin %s id: %s adv-router %s",
		  OSPF6_PATH_TYPE_NAME(brouter->path.type),
		  ospf6_lstype_name(brouter->path.origin.type), id, adv_router);
	zlog_info("gicBnajS0ysv9guC ""  options: %s router-bits: %s metric-type: %d metric: %d/%d",
		  options, capa, brouter->path.metric_type, brouter->path.cost,
		  brouter->path.u.cost_e2);
	zlog_info("8AhcXDiH100a0ntl "" paths %u nh %u", listcount(brouter->paths),
...
void ospf6_intra_brouter_calculation(struct ospf6_area *oa)
{
	...
	if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA_ID(oa->area_id) ||
		...
		zlog_debug("Zu7DGFSnfDRT0ZiU ""%s: border-router calculation for area %s",
			   __func__, oa->name);
	...
	for (brouter = ospf6_route_head(oa->ospf6->brouter_table); brouter;
		...
		if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER_ID(brouter_id)
		    || IS_OSPF6_DEBUG_ROUTE(MEMORY)) {
			zlog_debug("wyEolAhFGfIkeTd6 ""%p: mark as removing: area %s brouter %s",
	...
	for (brouter = ospf6_route_head(oa->spf_table); brouter;
		...
		if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER_ID(brouter_id)
		    || IS_OSPF6_DEBUG_ROUTE(MEMORY)) {
			zlog_debug("UKT65CcJ088LhWvD ""%p: transfer: area %s brouter %s",
				   (void *)brouter, oa->name, brouter_name);
			ospf6_brouter_debug_print(brouter);
		}
	}
	...
	for (brouter = ospf6_route_head(oa->ospf6->brouter_table); brouter;
		...
		if (CHECK_FLAG(brouter->flag, OSPF6_ROUTE_REMOVE)) {
			...
			if (IS_OSPF6_DEBUG_BROUTER
			    ...
			    || IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA_ID(
				       oa->area_id))
				zlog_info("rOZQSsKBzFaiRnRb ""%s: brouter %s appears via area %s",
					  __func__, brouter_name, oa->name);
	...
	if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA_ID(oa->area_id) ||
	    IS_OSPF6_DEBUG_ROUTE(MEMORY))
		zlog_debug("79xBrJSyhg4oNPLU ""%s: border-router calculation for area %s: done",
			   __func__, oa->name);
}
