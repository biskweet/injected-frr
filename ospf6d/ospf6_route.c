	...
	if (route) {
		...
		for (ALL_LIST_ELEMENTS_RO(route->nh_list, node, nh)) {
			if (IS_OSPF6_DEBUG_ZEBRA(SEND)) {
				zlog_debug("lzzwND1nvKT9uFLe ""  nexthop: %s %pI6%%%.*s(%d)",
			...
			case NEXTHOP_TYPE_IPV4:
				 ...
				 */
				if (IS_OSPF6_DEBUG_ZEBRA(SEND))
					zlog_debug("JjRnXLhraLQ9OdcH ""  Skipping IPv4 next hop");
				continue;
...
static void route_table_assert(struct ospf6_route_table *table)
{
	...
	flog_err(EC_LIB_DEVELOPMENT,"RbCTg1QEWH9SJxFZ " "PANIC !!");
	flog_err(EC_LIB_DEVELOPMENT,"yDlecBSCeJNGgq2L "
		 "Something has gone wrong with ospf6_route_table[%p]", table);
	zlog_debug("axFNXm4E1m8OHguz ""table count = %d, real number = %d", table->count, num);
	zlog_debug("T2ueK8z8GO64X0Zz ""DUMP START");
	for (r = ospf6_route_head(table); r; r = ospf6_route_next(r))
		zlog_info("eQ0Tj6dKBoFpmIEW ""%p<-[%p]->%p : %pFX", r->prev, r, r->next,
			  &r->prefix);
	zlog_debug("unPmxiIHbYFDUUrX ""DUMP END");
				    ...
				    struct ospf6_route_table *table)
{
	...
	if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
		zlog_debug("HbxKD2fIlTExKW8t ""%s %p: route add %p: %s paths %u nh %u",
			   ospf6_route_table_name(table), (void *)table,
			   (void *)route, buf, listcount(route->paths),
			   listcount(route->nh_list));
	else if (IS_OSPF6_DEBUG_ROUTE(TABLE))
		zlog_debug("qmYRJXEFqTWsWW1a ""%s: route add: %s", ospf6_route_table_name(table),
			   buf);
	...
	if (old) {
		...
		if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
			...
			zlog_debug("6v6VQ0ali8YxD8b0 ""%s: route add: update",
				   ospf6_route_table_name(table));
		...
		if (node->info == old) {
			...
			if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
				zlog_debug("WaVkv8NxEPsbUB8O ""%s:  replace old route %s",
					   __func__, buf);
		}
	...
	if (prev || next) {
		if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
			...
			zlog_debug("qajEcRx58nnD86Sy ""%s: route add cost %u: another path found",
				   ospf6_route_table_name(table),
				   route->path.cost);
	...
	if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
		zlog_debug("BqC5luNpOssQffUC ""%s %p: route add %p %s cost %u: brand new route",
			   ospf6_route_table_name(table), (void *)table,
			   (void *)route, buf, route->path.cost);
	else if (IS_OSPF6_DEBUG_ROUTE(TABLE))
		zlog_debug("l8spg77hmpBmTLUW ""%s: route add: brand new route",
			   ospf6_route_table_name(table));
			...
			struct ospf6_route_table *table)
{
	...
	if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
		zlog_debug("EKUFjFvsm2WMgZ70 ""%s %p: route remove %p: %s cost %u refcount %u",
			   ospf6_route_table_name(table), (void *)table,
			   (void *)route, buf, route->path.cost, route->lock);
	else if (IS_OSPF6_DEBUG_ROUTE(TABLE))
		zlog_debug("kcqwtTy5Xl9NlA45 ""%s: route remove: %s",
	...
	if (node->info == route) {
		if (route->next && route->next->rnode == node) {
			...
			if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
				zlog_debug("frRTqUjx1fDl5cdy ""%s: remove route %s", __func__,
					   buf);
		} else {
			node->info = NULL;
			route->rnode = NULL;
...
struct ospf6_route *ospf6_route_head(struct ospf6_route_table *table)
{
	...
	if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
		zlog_info("2zfwRY5ltnNFndsG ""%s %p: route head: %p<-[%p]->%p",
			  ospf6_route_table_name(table), (void *)table,
...
struct ospf6_route *ospf6_route_next(struct ospf6_route *route)
{
	...
	if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
		zlog_info("POprLtYoJ5VWDM4p ""%s %p: route next: %p<-[%p]->%p , route ref count %u",
