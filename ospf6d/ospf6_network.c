...
static void ospf6_reset_mcastloop(int ospf6_sock)
{
	...
	if (setsockopt(ospf6_sock, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, &off,
		       sizeof(unsigned int))
	    < 0)
		zlog_warn("KenRdmra2T1NZTgb ""Network: reset IPV6_MULTICAST_LOOP failed: %s",
			  safe_strerror(errno));
}
...
int ospf6_serv_sock(struct ospf6 *ospf6)
{
	...
	frr_with_privs(&ospf6d_privs) {
		...
		if (ospf6_sock < 0) {
			zlog_warn("PVYXgH3L42Cvzv9M ""Network: can't create OSPF6 socket.");
			return -1;
		}
	}
...
int ospf6_sso(ifindex_t ifindex, struct in6_addr *group, int option, int sockfd)
{
	...
	if (ret < 0) {
		flog_err_sys(
			EC_LIB_SOCKET,"xE6rVHFhA6qYF5Gh "
		  ...
		  ifindex_t ifindex, struct iovec *message, int ospf6_sock)
{
	...
	if (retval != iov_totallen(message))
		zlog_warn("Ia9UyYK0AWSC26KC ""sendmsg failed: source: %pI6 Dest: %pI6 ifindex: %d: %s (%d)",
			  src, dst, ifindex,
		  ...
		  ifindex_t *ifindex, struct iovec *message, int ospf6_sock)
{
	...
	if (retval < 0) {
		if (errno != EAGAIN && errno != EWOULDBLOCK)
			zlog_warn("yoGFPkfZz9CyxmIv ""stream_recvmsg failed: %s",
				  safe_strerror(errno));
		return retval;
	} else if (retval == iov_totallen(message))
		zlog_warn("6MxixtBElXVxeGVv ""recvmsg read full buffer size: %d", retval);
