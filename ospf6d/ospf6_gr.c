				  ...
				  enum ospf6_gr_restart_reason reason)
{
	...
	if (IS_OSPF6_DEBUG_ORIGINATE(LINK))
		zlog_debug("pRCu2u71NUjjtyiG ""Originate Grace-LSA for Interface %s",
			   oi->interface->name);
	...
	if (reason == OSPF6_GR_UNKNOWN_RESTART) {
		...
		*uv32 = htonl(1);
		...
		if (n != length)
			flog_err(EC_LIB_DEVELOPMENT,"MYwUzQJ72k4Vnts8 "
...
static void ospf6_gr_restart_exit(struct ospf6 *ospf6, const char *reason)
{
	...
	if (IS_DEBUG_OSPF6_GR)
		zlog_debug("LtvkRkxck3mRVHPu ""GR: exiting graceful restart: %s", reason);
				  ...
				  in_addr_t neighbor_router_id)
{
	...
	if (!nbr || nbr->state < OSPF6_NEIGHBOR_FULL) {
		if (IS_DEBUG_OSPF6_GR)
			zlog_debug("Xs3DEnHy4Bd0hyTf ""GR: missing adjacency to router %pI4",
				   &neighbor_router_id);
		return false;
	}
