					    ...
					    struct ospf6 *ospf6)
{
	...
	if (ospf6->gr_info.restart_in_progress) {
		if (IS_DEBUG_OSPF6_GR)
			zlog_debug("WHcLiVXjNJyiJnfR "
				"Graceful Restart in progress, don't originate LSA");
		return NULL;
	}

	if (IS_OSPF6_DEBUG_ASBR || IS_OSPF6_DEBUG_ORIGINATE(AS_EXTERNAL))
		zlog_debug("exKzhSytLSMPA84W ""Originate AS-External-LSA for %pFX",
			   &route->prefix);
...
void ospf6_asbr_lsa_add(struct ospf6_lsa *lsa)
{
	...
	if (IS_OSPF6_DEBUG_EXAMIN(AS_EXTERNAL))
		zlog_debug("SWaJ5xGFaap3jdrb ""Calculate AS-External route for %s", lsa->name);

	ospf6 = ospf6_get_by_lsdb(lsa);

	if (lsa->header->adv_router == ospf6->router_id) {
		if (IS_OSPF6_DEBUG_EXAMIN(AS_EXTERNAL))
			zlog_debug("ezl4vUCd2mujzE2B ""Ignore self-originated AS-External-LSA");
		return;
	}

	if (OSPF6_ASBR_METRIC(external) == OSPF_LS_INFINITY) {
		if (IS_OSPF6_DEBUG_EXAMIN(AS_EXTERNAL))
			zlog_debug("8p6DTcRlGug8pMxI ""Ignore LSA with LSInfinity Metric");
		return;
	}
	...
	if (CHECK_FLAG(external->prefix.prefix_options,
		...
		if (IS_OSPF6_DEBUG_EXAMIN(AS_EXTERNAL))
			zlog_debug("xOuenJQ9sFyMapuE ""Ignore LSA with NU bit set Metric");
		return;
	}
	...
	if (asbr_entry == NULL) {
		if (IS_OSPF6_DEBUG_EXAMIN(AS_EXTERNAL))
			zlog_debug("Irx5fyoQKfQwMS9U ""ASBR entry not found: %pFX", &asbr_id);
		return;
	} else {
	...
	if (ntohs(lsa->header->type) == OSPF6_LSTYPE_TYPE_7
		...
		|| oa->no_summary)) {
		if (IS_OSPF6_DEBUG_EXAMIN(AS_EXTERNAL))
			zlog_debug("mYvejtd3BPqbcSml ""Skipping Type-7 default route");
		return;
	}
	...
	if (!old) {
		if (IS_OSPF6_DEBUG_EXAMIN(AS_EXTERNAL))
			zlog_debug("UeQDQ7SgTlDfiDnA ""%s: Adding new route", __func__);
		 ...
		 */
		if (IS_OSPF6_DEBUG_EXAMIN(AS_EXTERNAL))
			zlog_debug("NHfzg02wPXH19q3Q ""%s : old route %pFX cost %u(%u) nh %u",
			   ...
			   struct ospf6_route *asbr_entry)
{
	...
	if (type == OSPF6_LSTYPE_TYPE_7) {
		if (debug)
			zlog_debug("xEPoIYxwPHiCeyRN ""%s: Withdraw  Type 7 route for %s",
				   __func__, lsa->name);
		oa = lsa->lsdb->data;
	} else {
		if (debug)
			zlog_debug("N059y9u63XWU2jL7 ""%s: Withdraw AS-External route for %s",
				   __func__, lsa->name);
	...
	if (oa == NULL) {
		if (debug)
			zlog_debug("CPBMNAZOtjn4LaTx ""%s: Invalid area", __func__);
		return;
	}

	if (lsa->header->adv_router == oa->ospf6->router_id) {
		if (debug)
			zlog_debug("LO2Zfy95SOLkvzIQ ""Ignore self-originated AS-External-LSA");
		return;
	}
	...
	if (route == NULL) {
		if (debug)
			zlog_debug("0YuyE2m4BqEIw6r9 ""AS-External route %pFX not found", &prefix);
		ospf6_route_delete(route_to_del);
		return;
	}

	if (debug)
		zlog_debug("cvW0R6PtZIwgqC1T "
...
void ospf6_asbr_lsentry_add(struct ospf6_route *asbr_entry, struct ospf6 *ospf6)
{
	...
	if (!CHECK_FLAG(asbr_entry->flag, OSPF6_ROUTE_BEST)) {
		...
		inet_ntop(AF_INET, &ADV_ROUTER_IN_PREFIX(&asbr_entry->prefix),
			  buf, sizeof(buf));
		zlog_info("YgRvoaKVBUxvBsDr ""ignore non-best path: lsentry %s add", buf);
		return;
	}
				       ...
				       struct ospf6_redist *red)
{
	...
	if (IS_OSPF6_DEBUG_ASBR)
		zlog_debug("DTAB8DycsTtm60XR ""%s: trigger redistribute reset thread", __func__);
...
void ospf6_asbr_status_update(struct ospf6 *ospf6, int status)
{
	...
	zlog_info("zJIJAY4TNct8291d ""ASBR[%s:Status:%d]: Update", ospf6->name, status);

	if (status) {
		if (IS_OSPF6_ASBR(ospf6)) {
			zlog_info("1QEtAg8okYbJJunE ""ASBR[%s:Status:%d]: Already ASBR",
				  ospf6->name, status);
			return;
		}
		...
		if (!IS_OSPF6_ASBR(ospf6)) {
			zlog_info("UESQhBFi43O4a7fs ""ASBR[%s:Status:%d]: Already non ASBR",
				  ospf6->name, status);
			return;
		}
...
void ospf6_asbr_send_externals_to_area(struct ospf6_area *oa)
{
	...
	for (ALL_LSDB(oa->ospf6->lsdb, lsa, lsanext)) {
		if (ntohs(lsa->header->type) == OSPF6_LSTYPE_AS_EXTERNAL) {
			if (IS_OSPF6_DEBUG_ASBR)
				zlog_debug("kRJlFdp0VwV4AXqS ""%s: Flooding AS-External LSA %s",
					   __func__, lsa->name);
...
ospf6_external_aggr_match(struct ospf6 *ospf6, struct prefix *p)
{
	...
	if (IS_OSPF6_DEBUG_AGGR) {
		...
		zlog_debug("XOklKMS83TvBmxIn ""%s: Matching aggregator found.prefix: %pFX Aggregator %pFX",
			__func__,
			p,
			&ag->p);
	}
				 ...
				 struct ospf6 *ospf6, uint32_t metric)
{
	...
	if (IS_OSPF6_DEBUG_ASBR)
		zlog_debug("2EP2kDBfH3HPyIbh ""Redistribute %pFX (%s)", prefix,
			   type == DEFAULT_ROUTE
	...
	if (ROUTEMAP(red)) {
		...
		if (ret == RMAP_DENYMATCH) {
			if (IS_OSPF6_DEBUG_ASBR)
				zlog_debug("2bZDw2DuwNsELNE1 ""Denied by route-map \"%s\"",
					   ROUTEMAP_NAME(red));
					...
					struct ospf6_external_aggr_rt *aggr)
{
	...
	if (IS_OSPF6_DEBUG_AGGR)
		zlog_debug("tnSHwU8fsA1R9wKe ""%s: Flushing Aggregate route (%pFX)",
				__func__,
				&aggr->p);
...
{
	if (IS_OSPF6_DEBUG_AGGR)
		zlog_debug("JbvMaPaFjfyfRoDI ""%s: Unlinking external route(%pFX) from aggregator(%pFX), external route count:%ld",
					__func__,
					&rt->prefix,
					&aggr->p,
				    ...
				    struct prefix *prefix, struct ospf6 *ospf6)
{
	...
	if (match == NULL) {
		if (IS_OSPF6_DEBUG_ASBR)
			zlog_debug("lKzOjbvN63SpCdre ""No such route %pFX to withdraw", prefix);
		return;
	}
	...
	if (info->type != type) {
		if (IS_OSPF6_DEBUG_ASBR)
			zlog_debug("rWg9gr41QwWGvKLs ""Original protocol mismatch: %pFX", prefix);
		return;
	}
	...
	if (IS_OSPF6_DEBUG_ASBR)
		zlog_debug("Q0nfJLLcNwazWkvX ""Removing route from external table %pFX",
			   prefix);
					 ...
					 struct ospf6_external_aggr_rt *aggr)
{
	...
	if (IS_OSPF6_DEBUG_AGGR)
		zlog_debug("S7rR26ZQHZjTnkKj ""%s: Originate new aggregate route(%pFX)", __func__,
			   &aggr->p);

	aggr->id = ospf6->external_id++;

	if (IS_OSPF6_DEBUG_AGGR)
		zlog_debug("rUWdMYYe6eR1hpn0 "
	...
	if (lsa) {
		if (IS_OSPF6_DEBUG_AGGR)
			zlog_debug("iYOeDmRUD08XrPoO ""%s: Set the origination bit for aggregator",
					__func__);
		...
		struct ospf6_external_aggr_rt *aggr)
{
	...
	if (CHECK_FLAG(aggr->aggrflags, OSPF6_EXTERNAL_AGGRT_NO_ADVERTISE)) {
		if (IS_OSPF6_DEBUG_AGGR)
			zlog_debug("ZVEDdUtxKin3938k ""%s: Don't originate the summary address,It is configured to not-advertise.",
					__func__);
		ospf6_asbr_summary_remove_lsa_and_route(ospf6, aggr);
	...
	if (OSPF6_EXTERNAL_RT_COUNT(aggr) == 0) {
		if (IS_OSPF6_DEBUG_AGGR)
			zlog_debug("MYgCjRO2ibXekLL1 ""%s: No routes present under this aggregation",
					__func__);
		return;
	}
	...
	if (!CHECK_FLAG(aggr->aggrflags, OSPF6_EXTERNAL_AGGRT_ORIGINATED)) {
		if (IS_OSPF6_DEBUG_AGGR)
			zlog_debug("TyRscqy7K6JUf8Ag ""%s: Now it is advertisable",
					__func__);
			    ...
			    struct ospf6_route *rt)
{
	...
	if (IS_OSPF6_DEBUG_AGGR)
		zlog_debug("9CI1RD2qWlh9rebh ""%s: Prepare to originate Summary route(%pFX)",
			   __func__, &aggr->p);
	...
	if (IS_OSPF6_DEBUG_AGGR)
		zlog_debug("bqcNecEa3mkSN0vA ""%s: Aggr LSA ID: %d flags %x.",
	...
	if (CHECK_FLAG(aggr->aggrflags, OSPF6_EXTERNAL_AGGRT_NO_ADVERTISE)) {
		...
		if (lsa) {
			if (IS_OSPF6_DEBUG_AGGR)
				zlog_debug("MVHofkXUtxNTzGsw ""%s: Purge the external LSA %s.",
					   __func__, lsa->name);
			ospf6_external_lsa_purge(ospf6, lsa);
			info->id = 0;
			rt->path.origin.id = 0;
		...
		if (aggr_lsa) {
			if (IS_OSPF6_DEBUG_AGGR)
				zlog_debug("0rXzq28g91boigq1 ""%s: Purge the aggr external LSA %s.",
					   __func__, lsa->name);
			ospf6_asbr_summary_remove_lsa_and_route(ospf6, aggr);
		}
		...
		if (IS_OSPF6_DEBUG_AGGR)
			zlog_debug("HQh4oyGL8Dd2uSz0 ""%s: Don't originate the summary address,It is configured to not-advertise.",
				__func__);
		return;
	}
...
static void ospf6_aggr_handle_external_info(void *data)
{
	...
	if (IS_OSPF6_DEBUG_ASBR || IS_OSPF6_DEBUG_ORIGINATE(AS_EXTERNAL))
		zlog_debug("GRFg3zZwx1gYfrqS ""%s: Handle external route for origination/refresh (%pFX)",
					__func__,
					&rt->prefix);
	...
	if (info->id) {
		...
		if (lsa) {
			if (IS_OSPF6_DEBUG_AGGR)
				zlog_debug("zJsnuSh7naC4OVMf ""%s: LSA found, refresh it",
					   __func__);
				      ...
				      struct route_node *rn)
{
	...
	if (IS_OSPF6_DEBUG_AGGR)
		zlog_debug("ZWaWTLwNO8MV6Sps ""%s: Deleting Aggregate route (%pFX)",
						__func__,
						&aggr->p);
...
static void ospf6_handle_external_aggr_update(struct ospf6 *ospf6)
{
	...
	if (IS_OSPF6_DEBUG_AGGR)
		zlog_debug("MJCZYWnrN4vkpNMd ""%s: Process modified aggregators.", __func__);
...
void ospf6_external_aggregator_free(struct ospf6_external_aggr_rt *aggr)
{
	...
	if (IS_OSPF6_DEBUG_AGGR)
		zlog_debug("yKZd03WAroMPraBS ""%s: Release the aggregator Address(%pFX)",
						__func__,
						&aggr->p);
					       ...
					       struct ospf6_route *rt)
{
	...
	if (lsa) {
		...
		if (IS_OSPF6_DEBUG_AGGR)
			zlog_debug("tpQ8PvWAM3s7WEJp ""%s: Originate external route(%pFX)",
				__func__,
				&rt->prefix);
					   ...
					   struct ospf6_route *rt)
{
	...
	if (prefix_same(&aggr->p, &rt->prefix)) {
		if (IS_OSPF6_DEBUG_AGGR)
			zlog_debug("hf47Ojop3qVhOBN0 ""%s: External Route prefix same as Aggregator(%pFX), so don't flush.",
				__func__,
				&rt->prefix);
...
static void ospf6_asbr_summary_process(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_AGGR)
		zlog_debug("FHpcHmZ0C2uexIrn ""%s: operation:%d",
				__func__,
				operation);
			...
			ospf6_aggr_action_t operation)
{
	...
	if (event_is_scheduled(ospf6->t_external_aggr)) {
		if (ospf6->aggr_action == OSPF6_ROUTE_AGGR_ADD) {
			...
			if (IS_OSPF6_DEBUG_AGGR)
				zlog_debug("pQGmdLnL5HTWMp1G ""%s: Not required to restart timer,set is already added.",
					__func__);
			return;
		}
		...
		if (operation == OSPF6_ROUTE_AGGR_ADD) {
			if (IS_OSPF6_DEBUG_AGGR)
				zlog_debug("RxPhCaSKcV7GwzpV ""%s, Restarting Aggregator delay timer.",
							__func__);
	...
	if (IS_OSPF6_DEBUG_AGGR)
		zlog_debug("Sz4UtTiTHAwD4PuM ""%s: Start Aggregator delay timer %u(in seconds).",
		...
		struct ospf6_external_aggr_rt *aggr)
{
	...
	if (IS_OSPF6_DEBUG_AGGR)
		zlog_debug("LW5NXAJk1Qzwbhly ""%s: Adding Aggregate route to Aggr table (%pFX)",
					__func__,
					&aggr->p);
					       ...
					       struct prefix *p)
{
	...
	if (!is_default_prefix(p)) {
		...
		if (aggr) {
			...
			if (IS_OSPF6_DEBUG_AGGR)
				zlog_debug("19LLhWv5gOpzSzmh ""%s: Send Aggregate LSA (%pFX)",
				__func__,
				&aggr->p);
	...
	if (IS_OSPF6_DEBUG_ASBR) {
		zlog_debug("Fq5AIQi6iAiaxj3O ""Advertise new AS-External Id:%pI4 prefix %pFX metric %u",
...
void ospf6_unset_all_aggr_flag(struct ospf6 *ospf6)
{
	...
	if (IS_OSPF6_DEBUG_AGGR)
		zlog_debug("exTDd2lKBKMt8tf1 ""Unset the origination bit for all aggregator");
