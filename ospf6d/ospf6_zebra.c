...
void ospf6_zebra_vrf_register(struct ospf6 *ospf6)
{
	...
	if (ospf6->vrf_id != VRF_UNKNOWN) {
		if (IS_OSPF6_DEBUG_ZEBRA(RECV)) {
			zlog_debug("jAi4zq0fMW3aVhGW ""%s: Register VRF %s id %u", __func__,
...
void ospf6_zebra_vrf_deregister(struct ospf6 *ospf6)
{
	...
	if (ospf6->vrf_id != VRF_DEFAULT && ospf6->vrf_id != VRF_UNKNOWN) {
		if (IS_OSPF6_DEBUG_ZEBRA(RECV)) {
			zlog_debug("E4zVtT8PjGwemGM3 ""%s: De-Register VRF %s id %u to Zebra.",
				   __func__,
				   ospf6_vrf_id_to_name(ospf6->vrf_id),
				   ospf6->vrf_id);
		}
...
static int ospf6_router_id_update_zebra(ZAPI_CALLBACK_ARGS)
{
	...
	if (IS_OSPF6_DEBUG_ZEBRA(RECV))
		zlog_debug("kXomOVcw75abXNK3 ""Zebra router-id update %pI4 vrf %s id %u",
...
void ospf6_zebra_import_default_route(struct ospf6 *ospf6, bool unreg)
{
	...
	if (zclient->sock < 0) {
		if (IS_OSPF6_DEBUG_ZEBRA(SEND))
			zlog_debug("YysMaJ5PQTA5wInl ""  Not connected to Zebra");
		return;
	}
	...
	if (IS_OSPF6_DEBUG_ZEBRA(SEND))
		zlog_debug("IF3XGGxv3S1XZb1X ""%s: sending cmd %s for %pFX (vrf %u)", __func__,
	...
	if (zclient_send_rnh(zclient, command, &prefix, SAFI_UNICAST, false,
		...
		flog_err(EC_LIB_ZAPI_SOCKET,"JvuAsmQFfKy0JXWx " "%s: zclient_send_rnh() failed",
			 __func__);
}
...
static int ospf6_zebra_if_address_update_add(ZAPI_CALLBACK_ARGS)
{
	...
	if (IS_OSPF6_DEBUG_ZEBRA(RECV))
		zlog_debug("9QUW23SOvonpfot7 ""Zebra Interface address add: %s %5s %pFX",
...
static int ospf6_zebra_if_address_update_delete(ZAPI_CALLBACK_ARGS)
{
	...
	if (IS_OSPF6_DEBUG_ZEBRA(RECV))
		zlog_debug("3oUTZ2By8k3dlgo2 ""Zebra Interface address delete: %s %5s %pFX",
			   c->ifp->name, prefix_family_str(c->address),
			   c->address);
...
int ospf6_zebra_gr_enable(struct ospf6 *ospf6, uint32_t stale_time)
{
	if (IS_DEBUG_OSPF6_GR)
		zlog_debug("jzKh1jd0jIuKeSFT ""Zebra enable GR [stale time %u]", stale_time);
...
{
	if (IS_DEBUG_OSPF6_GR)
		zlog_debug("Lbs9G8STAUj2nHi8 ""Zebra disable GR");
				     ...
				     struct ospf6 *ospf6)
{
	...
	if (IS_OSPF6_DEBUG_ZEBRA(SEND))
		zlog_debug("n7aTwVIJDtX7YGvi ""Zebra Send %s route: %pFX",
			   (type == REM ? "remove" : "add"), &request->prefix);

	if (zclient->sock < 0) {
		if (IS_OSPF6_DEBUG_ZEBRA(SEND))
			zlog_debug("oujqeVt2xIYBXsqc ""  Not connected to Zebra");
		return;
	}
	...
	if (request->path.origin.adv_router == ospf6->router_id
		...
		if (IS_OSPF6_DEBUG_ZEBRA(SEND))
			zlog_debug("oJsOTNcrosfFTJZK ""  Ignore self-originated external route");
		return;
	}
	...
	if (!ospf6_route_is_best(request)) {
		...
		if (IS_OSPF6_DEBUG_ZEBRA(SEND))
			zlog_debug("ZtQeHDcccHqjkKWE ""  Ignore non-best route");
		return;
	}
	...
	if (nhcount == 0) {
		if (type == ADD) {
			if (IS_OSPF6_DEBUG_ZEBRA(SEND))
				zlog_debug("D1JBaTGpToCfdZwa ""  No nexthop, ignore");
			return;
		} else if (IS_OSPF6_DEBUG_ZEBRA(SEND))
			zlog_debug("dP8OnF5KEF6waJjo ""  No nexthop, rem ok");
	}
	...
	if (ret == ZCLIENT_SEND_FAILURE)
		flog_err(EC_LIB_ZAPI_SOCKET,"WmwwwIyPVBAZFkvL "
...
void ospf6_zebra_add_discard(struct ospf6_route *request, struct ospf6 *ospf6)
{
	...
	if (!CHECK_FLAG(request->flag, OSPF6_ROUTE_BLACKHOLE_ADDED)) {
		...
		if (IS_OSPF6_DEBUG_ZEBRA(SEND))
			zlog_debug("mBt5zBHCE7KQoh8g ""Zebra: Route add discard %pFX", dest);

		SET_FLAG(request->flag, OSPF6_ROUTE_BLACKHOLE_ADDED);
	} else {
		if (IS_OSPF6_DEBUG_ZEBRA(SEND))
			zlog_debug("9spOmeu2vwxqk8C0 "
				...
				struct ospf6 *ospf6)
{
	...
	if (CHECK_FLAG(request->flag, OSPF6_ROUTE_BLACKHOLE_ADDED)) {
		...
		if (IS_OSPF6_DEBUG_ZEBRA(SEND))
			zlog_debug("raJJdYndBjRnekDc ""Zebra: Route delete discard %pFX", dest);

		UNSET_FLAG(request->flag, OSPF6_ROUTE_BLACKHOLE_ADDED);
	} else {
		if (IS_OSPF6_DEBUG_ZEBRA(SEND))
			zlog_debug("pyPygyOtIoFSDbHA "
