...
void ospf6_auth_hdr_dump_send(struct ospf6_header *ospfh, uint16_t length)
{
	...
	if (at_len > 0) {
		...
		temp[hash_len] = '\0';
		zlog_debug("ae4i22i1TqMGCbln ""OSPF6 Authentication Trailer");
		zlog_debug("Wt1FNAy6vVoUHVZV ""  Type %d", htons(ospf6_at_hdr->type));
		zlog_debug("YKEoIPRYlg3C8gjS ""  Length %d", at_hdr_len);
		zlog_debug("oLEotoccQbIL19M9 ""  Reserved %d", ospf6_at_hdr->reserved);
		zlog_debug("eMpy9GSpovn0PG79 ""  SA ID %d", htons(ospf6_at_hdr->id));
		zlog_debug("u21g6p31ZTAGe6QY ""  seqnum high 0x%08x",
			   htonl(ospf6_at_hdr->seqnum_h));
		zlog_debug("AXinVn2YakGGJmvE ""  seqnum high 0x%08x",
			   htonl(ospf6_at_hdr->seqnum_l));
		zlog_debug("Zp8ziXUMNtRFMHFn ""  Data %s", temp);
	}
}
			      ...
			      unsigned int lls_len)
{
	...
	if (at_len > 0) {
		...
		memcpy(temp, ospf6_at_hdr->data, hash_len);
		temp[hash_len] = '\0';
		zlog_debug("gXv0TbdOOwBKUEe6 ""OSPF6 Authentication Trailer");
		zlog_debug("tJsQwlEsY2t4Beol ""  Type %d", ntohs(ospf6_at_hdr->type));
		zlog_debug("fhMvwLb5723UsolS ""  Length %d", at_hdr_len);
		zlog_debug("DAwTPt3vzfZYczfl ""  Reserved %d", ospf6_at_hdr->reserved);
		zlog_debug("DSjzU5NgQqoF9z3q ""  SA ID %d", ntohs(ospf6_at_hdr->id));
		zlog_debug("ao6hWbC1ZiuiYHh2 ""  seqnum high 0x%08x",
			   ntohl(ospf6_at_hdr->seqnum_h));
		zlog_debug("rDVvO83wLjrDx7oT ""  seqnum high 0x%08x",
			   ntohl(ospf6_at_hdr->seqnum_l));
		zlog_debug("W9eEN1UyDBh6vGPY ""  Data %s", temp);
	}
}
			    ...
			    unsigned int *lls_block_len)
{
	...
	if (*pkt_len < hdr_len) {
		if (IS_OSPF6_DEBUG_AUTH_RX)
			zlog_err("UFhX31UDIFUcVt2a ""RECV[%s] Received incomplete %s packet",
				 oi->interface->name,
	...
	default:
		if (IS_OSPF6_DEBUG_AUTH_RX)
			zlog_err("FTw44w8ooiTi3hgv ""RECV[%s] : Wrong packet type %d",
	...
	if ((!auth_present && (oi->at_data.flags != 0))
		...
		if (IS_OSPF6_DEBUG_AUTH_RX)
			zlog_err("dRGzeMQyyD9ksUmC ""RECV[%s] : Auth option miss-match in %s pkt",
				 oi->interface->name,
				 ospf6_message_type(oh->type));
		return OSPF6_AUTH_VALIDATE_FAILURE;
	}
	...
	if (*lls_block_len > (*pkt_len - hdr_len)) {
		if (IS_OSPF6_DEBUG_AUTH_RX)
			zlog_err("xBKwFtFSKt0qoBRa ""RECV[%s] : Wrong lls data in %s packet",
				 oi->interface->name,
				 ospf6_message_type(oh->type));
		return OSPF6_AUTH_VALIDATE_FAILURE;
	}
	...
	if ((*pkt_len - hdr_len - (*lls_block_len)) > sizeof(ospf6_auth_info)) {
		if (IS_OSPF6_DEBUG_AUTH_RX)
			zlog_err("3yz26tMLo0EajZhq ""RECV[%s] : Wrong auth data in %s packet",
				 oi->interface->name,
				 ospf6_message_type(oh->type));
		return OSPF6_AUTH_VALIDATE_FAILURE;
	}
	...
	if (ntohs(ospf6_auth_info.length) > OSPF6_AUTH_HDR_FULL) {
		if (IS_OSPF6_DEBUG_AUTH_RX)
			zlog_err("GBoCvZEWzkOSywZ9 ""RECV[%s] : Wrong auth header length in %s",
				 oi->interface->name,
				 ospf6_message_type(oh->type));
		return OSPF6_AUTH_VALIDATE_FAILURE;
	}
			    ...
			    struct in6_addr *src, unsigned int lls_block_len)
{
	...
	if (CHECK_FLAG(oi->at_data.flags, OSPF6_AUTH_TRAILER_KEYCHAIN)) {
		...
		if (!key) {
			if (IS_OSPF6_DEBUG_AUTH_RX)
				zlog_err("d9tmkVeleyfdO9B3 ""RECV[%s]: Auth, Invalid SA for %s",
					 oi->interface->name,
					 ospf6_message_type(oh->type));
			return OSPF6_AUTH_VALIDATE_FAILURE;
		}
		...
		if (oi->at_data.key_id != ntohs(ospf6_auth->id)) {
			if (IS_OSPF6_DEBUG_AUTH_RX)
				zlog_err("dZnDVRNsGOtcfp7I ""RECV[%s]: Auth SA ID mismatch for %s, received %u vs configured %u",
					 oi->interface->name,
					 ospf6_message_type(oh->type),
					 ntohs(ospf6_auth->id),
					 oi->at_data.key_id);
			    ...
			    uint32_t pkt_len)
{
	...
	if (CHECK_FLAG(oi->at_data.flags, OSPF6_AUTH_TRAILER_KEYCHAIN)) {
		...
		if (IS_OSPF6_DEBUG_AUTH_TX)
			zlog_warn("cQuSS56hm6siOCD4 ""SEND[%s]: Authentication not configured for %s",
				  oi->interface->name,
				  ospf6_message_type(oh->type));
		return;
	}
	...
	if (!auth_str) {
		if (IS_OSPF6_DEBUG_AUTH_TX)
			zlog_warn("0U4od7zhFLK2Pf23 ""SEND[%s]: Authentication key is not configured for %s",
				  oi->interface->name,
				  ospf6_message_type(oh->type));
		return;
	}
	...
	if (ospf6->seqnum_l == 0xFFFFFFFF) {
		if (ospf6->seqnum_h == 0xFFFFFFFF) {
			...
			zlog_err("I8RANqg1zW1Q2NCJ ""Sequence number wrapped; key must be reset.");
			ospf6->seqnum_h = 0;
		} else {
			ospf6->seqnum_h++;
		}
...
static void ospf6_auth_seqno_nvm_update(struct ospf6 *ospf6)
{
	...
	zlog_err("BRK0qkAFy6SvlGqO ""Higher order sequence number %d update for %s process",
...
ospf6_auth_seqno_nvm_delete(struct ospf6 *ospf6)
{
	...
	zlog_err("x2f9zB5rdnIxHGpe ""Higher order sequence number delete for %s process",
		 ospf6->name);
...
static void ospf6_auth_seqno_nvm_read(struct ospf6 *ospf6)
{
	...
	zlog_err("RmaHeRcQFA48TltY ""Higher order sequence number %d read for %s process %s",
