...
void ospf6_lsa_originate(struct ospf6 *ospf6, struct ospf6_lsa *lsa)
{
	...
	if (old && !OSPF6_LSA_IS_DIFFER(lsa, old)
		...
		if (IS_OSPF6_DEBUG_ORIGINATE_TYPE(lsa->header->type))
			zlog_debug("wcOy9ULYYzoFsJJK ""Suppress updating LSA: %s", lsa->name);
		ospf6_lsa_delete(lsa);
		return;
	}
	...
	if (IS_OSPF6_DEBUG_LSA_TYPE(lsa->header->type)
		...
		zlog_debug("PzpA6HvuEmsZ862x ""LSA Originate:");
	...
	for (ALL_LIST_ELEMENTS_RO(ospf6->area_list, lnode, oa)) {
		...
		if (lsa) {
			if (IS_OSPF6_DEBUG_NSSA)
				zlog_debug("gK69RatCrlMhY0lk ""withdraw type 7 lsa, LS ID: %u",
					   htonl(id));
...
void ospf6_install_lsa(struct ospf6_lsa *lsa)
{
	...
	if (old) {
		if (ntohs(lsa->header->type) == OSPF6_LSTYPE_TYPE_7) {
			if (IS_OSPF6_DEBUG_NSSA)
				zlog_debug("QrJklXuWEaVxTBPH ""%s : old LSA %s", __func__,
					   lsa->name);
	...
	if (OSPF6_LSA_IS_SEQWRAP(lsa)
		...
		if (IS_OSPF6_DEBUG_EXAMIN_TYPE(lsa->header->type))
			zlog_debug("tC2AYawEqVgnxDkT ""lsa install wrapping: sequence 0x%x",
	...
	if (IS_OSPF6_DEBUG_LSA_TYPE(lsa->header->type)
		...
		zlog_debug("56HFBJVwb2PjzpWA ""%s Install LSA: %s age %d seqnum %x in LSDB.",
	...
	if (ntohs(lsa->header->type) == OSPF6_LSTYPE_ROUTER) {
		...
		if (old == NULL) {
			if (IS_OSPF6_DEBUG_LSA_TYPE(lsa->header->type)
			    || IS_OSPF6_DEBUG_EXAMIN_TYPE(lsa->header->type))
				zlog_debug("cUgsTfF6U21QCFQk ""%s: New router LSA %s", __func__,
					   lsa->name);
			   ...
			   struct ospf6_interface *oi)
{
	...
	if (IS_OSPF6_DEBUG_FLOODING
		...
		is_debug++;
		zlog_debug("ayb0rTcWOevprM81 ""Flooding on %s: %s", oi->interface->name,
			   lsa->name);
	}
	...
	for (ALL_LIST_ELEMENTS(oi->neighbor_list, node, nnode, on)) {
		if (is_debug)
			zlog_debug("rczmirtOwE9Ninyi ""To neighbor %s", on->name);

		/* (a) if neighbor state < Exchange, examin next */
		if (on->state < OSPF6_NEIGHBOR_EXCHANGE) {
			if (is_debug)
				zlog_debug("Zt29F1tFUs2fQXYn "
		...
		if (on->state != OSPF6_NEIGHBOR_FULL) {
			if (is_debug)
				zlog_debug("BqJHch8dFgFzjt3M ""Neighbor not yet Full");
		...
		if ((oi->area->ospf6->inst_shutdown)
			...
			if (is_debug)
				zlog_debug("TvQzORhhHpdgyJeK ""Add retrans-list of neighbor %s ",
					   on->name);
	...
	if (is_debug)
		zlog_debug("qU2onQpk49ISfu3f ""Schedule flooding for the interface");
				...
				struct ospf6_lsa *lsa, struct ospf6 *process)
{
	...
	for (ALL_LIST_ELEMENTS(process->area_list, node, nnode, oa)) {
		...
		if (!OSPF6_LSA_IS_KNOWN(lsa->header->type)
			...
			if (IS_OSPF6_DEBUG_FLOODING)
				zlog_debug("9nO9naPErNVMGIZh ""Unknown LSA, do not flood");
			continue;
		}
					...
					struct ospf6_interface *oi)
{
	...
	for (ALL_LIST_ELEMENTS(oi->neighbor_list, node, nnode, on)) {
		...
		if (rem && !ospf6_lsa_compare(rem, lsa)) {
			if (IS_OSPF6_DEBUG_FLOODING
			    || IS_OSPF6_DEBUG_FLOOD_TYPE(lsa->header->type))
				zlog_debug("fCpLx9WhuzWivYuj ""Remove %s from retrans_list of %s",
					   rem->name, on->name);
					   ...
					   struct ospf6_neighbor *from)
{
	...
	if (CHECK_FLAG(lsa->flag, OSPF6_LSA_DUPLICATE)
	    && !CHECK_FLAG(lsa->flag, OSPF6_LSA_IMPLIEDACK)) {
		if (is_debug)
			zlog_debug("tnjHzsNgpLJ0ms1x ""Direct acknowledgement (BDR & Duplicate)");
					   ...
					   struct ospf6_neighbor *from)
{
	...
	if (CHECK_FLAG(lsa->flag, OSPF6_LSA_FLOODBACK)) {
		if (is_debug)
			zlog_debug("MVgTOz8OnByyqo5p ""No acknowledgement (AllOther & FloodBack)");
		return;
	}
		       ...
		       struct ospf6_lsa_header *lsa_header)
{
	...
	if (IS_OSPF6_DEBUG_FLOODING
		...
		is_debug++;
		zlog_debug("lWaRvphIoHoWWu9s ""LSA Receive from %s", from->name);
	...
	default:
		if (is_debug)
			zlog_debug("qfh9ERUEh91KmR4b ""LSA has reserved scope, discard");
		ospf6_lsa_delete(new);
		return;
	}
	...
	if (old) {
		...
		if (ntohl(new->header->seqnum) == ntohl(old->header->seqnum)) {
			if (is_debug)
				zlog_debug("k7WFEtEWljejcG9E ""Received is duplicated LSA");
	...
	if (old == NULL || ismore_recent < 0) {
		...
		if (self_originated) {
			...
			if (is_debug) {
				zlog_debug("7CscPc7nTSRPwUbM "
					"Newer instance of the self-originated LSA");
				zlog_debug("X50PT7NVeATdkLfA ""Schedule reorigination");
			}
	...
	if (ospf6_lsdb_lookup(new->header->type, new->header->id,
		...
		zlog_warn("TqQM73nne3SMvi5c "
			"Received is not newer, on the neighbor %s request-list",
			from->name);
		zlog_warn("Ysd9pmB4IZd0VDxH "
	...
	if (ismore_recent == 0) {
		...
		if (rem) {
			if (is_debug) {
				zlog_debug("PfJ8oZ7usQ5OlcHc "
					"It is on the neighbor's retrans-list.");
				zlog_debug("UNvrL44Q5I1O0gPJ "
		...
		if (is_debug)
			zlog_debug("vXQewxRpGKxJW8PF ""Possibly acknowledge and then discard");
	...
	/* (8) previous database copy is more recent */
	{
		...
		if (OSPF6_LSA_IS_MAXAGE(old)
			...
			if (is_debug) {
				zlog_debug("saDqS4A1WgRftU4X ""The LSA is in Seqnumber Wrapping");
				zlog_debug("UVLhUfrRnIkEtCKL ""MaxAge & MaxSeqNum, discard");
			}
			ospf6_lsa_delete(new);
			return;
		}
		...
		{
			if (is_debug) {
				zlog_debug("bK5pvJ1Kqwb6cPA2 ""Database copy is more recent.");
				zlog_debug("hXAV0Sq4t3dQLLu3 "
