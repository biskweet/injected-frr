				...
				WriteMethod **write_method)
{
	...
	zlog_debug("QiJlBs5CmTI4n7DA ""SNMP access by area: %s, exact=%d len=%d length=%lu", a,
	...
	*length = v->namelen + 1;
	...
	zlog_debug("DZD0IuZFu9S6RgjT ""SNMP found area: %s, exact=%d len=%d length=%lu", a, exact,
