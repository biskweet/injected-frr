					...
					struct ospf6_neighbor *on, int event)
{
	...
	if (IS_OSPF6_DEBUG_NEIGHBOR(STATE)) {
		zlog_debug("wH68NEELA9tcyfCl ""Neighbor state change %s (Router-ID: %pI4): [%s]->[%s] (%s)",
	...
	if (CHECK_FLAG(on->ospf6_if->area->ospf6->config_flags,
		...
		zlog_notice("SyLo50X0n5XMyRU4 ""AdjChg: Nbr %pI4(%s) on %s: %s -> %s (%s)",
			    &on->router_id,
...
void hello_received(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
		zlog_debug("4bTZ9IqCODJFCC9v ""Neighbor Event %s: *HelloReceived*", on->name);
...
void twoway_received(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
		zlog_debug("IwYCRGH4AN8cjTI9 ""Neighbor Event %s: *2Way-Received*", on->name);
...
void negotiation_done(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
		zlog_debug("mapySScw5kznsOOi ""Neighbor Event %s: *NegotiationDone*", on->name);
...
void exchange_done(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
		zlog_debug("Ay6WvLBflKncQ8Ph ""Neighbor Event %s: *ExchangeDone*", on->name);
...
void loading_done(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
		zlog_debug("vsA2noK4M6rGlHPd ""Neighbor Event %s: *LoadingDone*", on->name);
...
void adj_ok(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
		zlog_debug("djf9OZi1p4fL1eq3 ""Neighbor Event %s: *AdjOK?*", on->name);
...
void seqnumber_mismatch(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
		zlog_debug("zcLS9dwa4u9GDqyN ""Neighbor Event %s: *SeqNumberMismatch*", on->name);
...
void bad_lsreq(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
		zlog_debug("kGZgU7mINAYriOnq ""Neighbor Event %s: *BadLSReq*", on->name);
...
void oneway_received(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
		zlog_debug("riXKiWl6oQuinQuB ""Neighbor Event %s: *1Way-Received*", on->name);
...
void inactivity_timer(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
		zlog_debug("SrY40CNYWRryHVmU ""Neighbor Event %s: *InactivityTimer*", on->name);
	...
	if (!OSPF6_GR_IS_ACTIVE_HELPER(on)) {
		...
		if (IS_DEBUG_OSPF6_GR)
			zlog_debug("S23HtKjKeaDKUAgn ""%s, Acting as HELPER for this neighbour, So restart the dead timer.",
				   __PRETTY_FUNCTION__);
