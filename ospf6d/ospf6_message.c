...
{
	zlog_debug("kk4CkQ0aWuZXRoOr ""    OSPFv%d Type:%d Len:%hu Router-ID:%pI4", oh->version,
		   oh->type, ntohs(oh->length), &oh->router_id);
	zlog_debug("vRmEL96BX9iRILPO ""    Area-ID:%pI4 Cksum:%hx Instance-ID:%d", &oh->area_id,
...
void ospf6_hello_print(struct ospf6_header *oh, int action)
{
	...
	zlog_debug("HO1Z7xQQNwF3RRFK ""    I/F-Id:%ld Priority:%d Option:%s",
		   (unsigned long)ntohl(hello->interface_id), hello->priority,
		   options);
	zlog_debug("58wx1ozLuAj6g8He ""    HelloInterval:%hu DeadInterval:%hu",
		   ntohs(hello->hello_interval), ntohs(hello->dead_interval));
	zlog_debug("Bm2r0ISlZbqqp5Kd ""    DR:%pI4 BDR:%pI4", &hello->drouter, &hello->bdrouter);
	...
	if ((IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV)
		...
		for (p = (char *)((caddr_t)hello + sizeof(struct ospf6_hello));
		     ...
		     p += sizeof(uint32_t))
			zlog_debug("CMnbshONDx34UvY3 ""    Neighbor: %pI4", (in_addr_t *)p);
...
void ospf6_dbdesc_print(struct ospf6_header *oh, int action)
{
	...
	zlog_debug("O5GXMsABqHLkWUsx ""    MBZ: %#x Option: %s IfMTU: %hu", dbdesc->reserved1,
		   options, ntohs(dbdesc->ifmtu));
	zlog_debug("CW78apQiAD3wByEK ""    MBZ: %#x Bits: %s%s%s SeqNum: %#lx", dbdesc->reserved2,
...
void ospf6_lsreq_print(struct ospf6_header *oh, int action)
{
	...
	if ((IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV)
		...
		for (p = (char *)((caddr_t)oh + sizeof(struct ospf6_header));
			...
			zlog_debug("GS8ZutspTzJy1hUO ""    [%s Id:%pI4 Adv:%pI4]",
...
void ospf6_lsupdate_print(struct ospf6_header *oh, int action)
{
	...
	zlog_debug("faSvGuUKJKwLcrQf ""    Number of LSA: %ld", num);
			     ...
			     struct ospf6_header *oh)
{
	...
	if (OSPF6_OPT_ISSET(hello->options, OSPF6_OPT_E) !=
		...
		zlog_warn("t3q8vgn8uBMMwLSD ""VRF %s: IF %s E-bit mismatch from %pI6 (%pI4)",
			  oi->interface->vrf->name, oi->interface->name, src,
			  &oh->router_id);
		return;
	}
	...
	if (OSPF6_OPT_ISSET(hello->options, OSPF6_OPT_N)
		...
		zlog_warn("SYDfNISVFDHhAE1b ""VRF %s: IF %s N-bit mismatch",
	...
	if (latency > ((int64_t)oi->hello_interval * 1000000))
		zlog_warn("QEeHgIQDn2dSI5N7 ""%s RX %pI4 high latency %" PRId64 "us.", __func__,
				     ...
				     struct ospf6_neighbor *on)
{
	...
	if (on->state < OSPF6_NEIGHBOR_INIT) {
		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR))
			zlog_debug("hYVPnF5MS2XcXqWB ""Neighbor state less than Init, ignore");
		return;
	}
	...
	case OSPF6_NEIGHBOR_TWOWAY:
		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR))
			zlog_debug("bkMZUiOVCfsV8tdL ""Neighbor state is 2-Way, ignore");
		return;
	...
	case OSPF6_NEIGHBOR_EXSTART:
		...
		if (!CHECK_FLAG(dbdesc->bits, OSPF6_DBDESC_MSBIT)
			...
			zlog_warn("tuF4v3secxD3dbLw ""VRF %s: Nbr %s: Negotiation failed",
	...
	case OSPF6_NEIGHBOR_EXCHANGE:
		if (!memcmp(dbdesc, &on->dbdesc_last,
			 ...
			 */
			if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR))
				zlog_debug("DJboPajUBcvWbNM7 "
					"Duplicated dbdesc discarded by Master, ignore");
			return;
		}

		if (CHECK_FLAG(dbdesc->bits, OSPF6_DBDESC_MSBIT)) {
			zlog_warn("UIyIusi6H5r4GFBG "
		...
		if (CHECK_FLAG(dbdesc->bits, OSPF6_DBDESC_IBIT)) {
			zlog_warn("g93RkgXgPNUIGW1l ""DbDesc recv: Initialize bit mismatch Nbr %s",
				  on->name);
			event_add_event(master, seqnumber_mismatch, on, 0,
					NULL);
			return;
		...
		if (memcmp(on->options, dbdesc->options, sizeof(on->options))) {
			zlog_warn("qV2lY5EluxPl6uub ""DbDesc recv: Option field mismatch Nbr %s",
				  on->name);
			event_add_event(master, seqnumber_mismatch, on, 0,
					NULL);
			return;
	...
	case OSPF6_NEIGHBOR_FULL:
		if (!memcmp(dbdesc, &on->dbdesc_last,
			 ...
			 */
			if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR))
				zlog_debug("0p77GILyZ2FIrTyK "
					"Duplicated dbdesc discarded by Master, ignore");
			return;
		}

		zlog_warn("Y93IO0hDQBFRHBld "
	...
	for (p = (char *)((caddr_t)dbdesc + sizeof(struct ospf6_dbdesc));
		...
		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
			zlog_debug("iJFKPAS5m4Q2uH03 ""%s", his->name);
		...
		case OSPF6_SCOPE_RESERVED:
			if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
				zlog_debug("QmiF4ZJqX20urjmw ""Ignoring LSA of reserved scope");
		...
		if (mine == NULL) {
			if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
				zlog_debug("DPmrCaa4WImuR9y9 ""Add request (No database copy)");
			ospf6_lsdb_add(ospf6_lsa_copy(his), on->request_list);
		} else if (ospf6_lsa_compare(his, mine) < 0) {
			if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
				zlog_debug("AKaK8u9L4DtP1zof ""Add request (Received MoreRecent)");
			ospf6_lsdb_add(ospf6_lsa_copy(his), on->request_list);
		} else {
			if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
				zlog_debug("gI7ulXYhtSA7DIn3 ""Discard (Existing MoreRecent)");
		}
		ospf6_lsa_delete(his);
	}
				    ...
				    struct ospf6_neighbor *on)
{
	...
	if (on->state < OSPF6_NEIGHBOR_INIT) {
		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR))
			zlog_debug("PEAFAai55EspYM8O ""Neighbor state less than Init, ignore");
		return;
	}
	...
	case OSPF6_NEIGHBOR_TWOWAY:
		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR))
			zlog_debug("j4tOTT4sTYx5J8Vd ""Neighbor state is 2-Way, ignore");
		return;
	...
	case OSPF6_NEIGHBOR_EXSTART:
		...
		if (CHECK_FLAG(dbdesc->bits, OSPF6_DBDESC_IBIT)
			...
			zlog_warn("4j64J3BBbj4m6Tmj ""VRF %s: Nbr %s Negotiation failed",
				  on->ospf6_if->interface->vrf->name, on->name);
			return;
		}
		break;
	...
	for (p = (char *)((caddr_t)dbdesc + sizeof(struct ospf6_dbdesc));
		...
		case OSPF6_SCOPE_RESERVED:
			if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
				zlog_debug("etE3aRwsNzLS8zcX ""Ignoring LSA of reserved scope");
			ospf6_lsa_delete(his);
			continue;
		}
		...
		if (OSPF6_LSA_SCOPE(his->header->type) == OSPF6_SCOPE_AS
			...
			|| IS_AREA_NSSA(on->ospf6_if->area))) {
			if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
				zlog_debug("7v9CQVPJNHchI2I8 ""E-bit mismatch with LSA Headers");
			ospf6_lsa_delete(his);
			event_add_event(master, seqnumber_mismatch, on, 0,
					NULL);
			return;
		...
		if (mine == NULL || ospf6_lsa_compare(his, mine) < 0) {
			if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
				zlog_debug("BF7CUI3A8lzLbRNB ""Add request-list: %s", his->name);
			ospf6_lsdb_add(ospf6_lsa_copy(his), on->request_list);
		}
		ospf6_lsa_delete(his);
	}
			      ...
			      struct ospf6_header *oh)
{
	...
	if (on == NULL) {
		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR))
			zlog_debug("oPkzoAiIympDDVXf ""Neighbor not found, ignore");
		return;
	}
	...
	if (!oi->mtu_ignore && ntohs(dbdesc->ifmtu) != oi->ifmtu) {
		zlog_warn("5KLdIIhJGD30SGxv ""VRF %s: I/F %s MTU mismatch (my %d rcvd %d)",
			  oi->interface->vrf->name, oi->interface->name,
	...
	if (ntohl(oh->router_id) < ntohl(oi->area->ospf6->router_id))
		...
		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR))
			zlog_debug("JaI87qDq47JEOuA1 ""Can't decide which is master, ignore");
	}
}
			     ...
			     struct ospf6_header *oh)
{
	...
	if (on == NULL) {
		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR))
			zlog_debug("yjFiMpuqciG1Ebti ""Neighbor not found, ignore");
		return;
	}
	...
	if (on->state != OSPF6_NEIGHBOR_EXCHANGE
		...
		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR))
			zlog_debug("uZTMbIvgRfJ4p1x3 ""Neighbor state less than Exchange, ignore");
		return;
	}
	...
	for (p = (char *)((caddr_t)oh + sizeof(struct ospf6_header));
		...
		default:
			if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
				zlog_debug("eJKheXmhWkTJirdN ""Ignoring LSA of reserved scope");
			continue;
		}
...
)
{
	...
	while (length) {
		if (length < OSPF6_PREFIX_MIN_SIZE) {
			zlog_warn("NfrQ3EeagsCYPAkl ""%s: undersized IPv6 prefix header",
				  __func__);
			return MSG_NG;
		}
		/* safe to look deeper */
		if (current->prefix_length > IPV6_MAX_BITLEN) {
			zlog_warn("29biGxYmkTER1C95 ""%s: invalid PrefixLength (%u bits)",
		...
		if (requested_pfx_bytes > length) {
			zlog_warn("moWjkObMp24KvEXD ""%s: undersized IPv6 prefix", __func__);
			return MSG_NG;
		}
		/* next prefix */
				 ...
				 const uint8_t headeronly)
{
	...
	if (ltindex < OSPF6_LSTYPE_SIZE && ospf6_lsa_minlen[ltindex]
		...
		zlog_warn("u8aO2hEFka60zE4v ""%s: undersized (%u B) LSA", __func__, lsalen);
		return MSG_NG;
	}
	switch (lsatype) {
	...
	case OSPF6_LSTYPE_INTER_ROUTER:
		...
		if (lsalen
			...
			zlog_warn("ysXspk0Pm6YoG7fl ""%s: Inter Router LSA oversized (%u B) LSA",
				  __func__, lsalen);
			return MSG_NG;
		}
		break;
	...
	case OSPF6_LSTYPE_GRACE_LSA:
		if (lsalen < OSPF6_LSA_HEADER_SIZE + GRACE_PERIOD_TLV_SIZE
			...
			if (IS_DEBUG_OSPF6_GR)
				zlog_debug("vbJWHGoWVTWJlUqW ""%s: Undersized GraceLSA.",
					   __func__);
			return MSG_NG;
		}
	}
		    ...
		    const uint32_t declared_num_lsas)
{
	...
	while (length) {
		...
		if (headeronly) {
			...
			if (MSG_OK != ospf6_lsa_examin(lsah, lsalen, 0)) {
				zlog_warn("H3P7x2wuXA7vvoj8 ""%s: anomaly in %s LSA #%u", __func__,
	...
	if (declared_num_lsas && counted_lsas != declared_num_lsas) {
		zlog_warn("Ttf3h829gY4HBD7Z ""%s: #LSAs declared (%u) does not match actual (%u)",
				    ...
				    const unsigned bytesonwire)
{
	...
	if (bytesonwire < OSPF6_HEADER_SIZE) {
		zlog_warn("TQE8CEwY7j4cROMh ""%s: undersized (%u B) packet", __func__,
			  bytesonwire);
		return MSG_NG;
	}
	...
	if (bytesonwire != ntohs(oh->length)) {
		zlog_warn("O98KMGuieGzLTEW8 ""%s: %s packet length error (%u real, %u declared)",
	...
	if (oh->version != OSPFV3_VERSION) {
		zlog_warn("iW0KxJXRnOl6vjSU ""%s: invalid (%u) protocol version", __func__,
			  oh->version);
		return MSG_NG;
	}
	...
	if (oh->type < OSPF6_MESSAGE_TYPE_ALL && ospf6_packet_minlen[oh->type]
		...
		zlog_warn("E5mrr89I2ltl68rS ""%s: undersized (%u B) %s packet", __func__,
	...
	case OSPF6_MESSAGE_TYPE_HELLO:
		...
		if (0
		    == (bytesonwire - OSPF6_HEADER_SIZE - OSPF6_HELLO_MIN_SIZE)
			       % 4)
			return MSG_OK;
		zlog_warn("UeKhCC91KU3Jcj58 ""%s: alignment error in %s packet", __func__,
	...
	case OSPF6_MESSAGE_TYPE_LSREQ:
		...
		if (0
			...
			return MSG_OK;
		zlog_warn("sv5X2JlJgcHouic7 ""%s: alignment error in %s packet", __func__,
			  ospf6_message_type(oh->type));
		return MSG_NG;
	case OSPF6_MESSAGE_TYPE_LSUPDATE:
	...
	case OSPF6_MESSAGE_TYPE_LSACK:
		...
		break;
	default:
		zlog_warn("pkIoJo0oC1BbPkVc ""%s: invalid (%u) message type", __func__, oh->type);
		return MSG_NG;
	}
	if (test != MSG_OK)
		zlog_warn("ssRN06rU6j86Tteu ""%s: anomaly in %s packet", __func__,
			  ospf6_message_type(oh->type));
	return test;
}
				 ...
				 const unsigned bytesonwire)
{
	...
	if (oh->router_id == oi->area->ospf6->router_id) {
		zlog_warn("BqDwjl3Wg93HI0VW ""VRF %s: I/F %s Duplicate Router-ID (%pI4)",
			  oi->interface->vrf->name, oi->interface->name,
			  &oh->router_id);
		return MSG_NG;
	}
				...
				struct ospf6_header *oh)
{
	...
	if (on == NULL) {
		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR))
			zlog_debug("tms3sRHUxqKsDWCz ""Neighbor not found, ignore");
		return;
	}
	...
	if (on->state != OSPF6_NEIGHBOR_EXCHANGE
	    ...
	    && on->state != OSPF6_NEIGHBOR_FULL) {
		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR))
			zlog_debug("ancwQ8u3n7Mmy9Lm ""Neighbor state less than Exchange, ignore");
		return;
	}
			     ...
			     struct ospf6_header *oh)
{
	...
	if (on == NULL) {
		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR))
			zlog_debug("JXo5htVBHFuDdOm6 ""Neighbor not found, ignore");
		return;
	}
	...
	if (on->state != OSPF6_NEIGHBOR_EXCHANGE
	    ...
	    && on->state != OSPF6_NEIGHBOR_FULL) {
		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR))
			zlog_debug("Ntx8VnVAaievag0O ""Neighbor state less than Exchange, ignore");
		return;
	}
	...
	for (p = (char *)((caddr_t)oh + sizeof(struct ospf6_header));
		...
		case OSPF6_SCOPE_RESERVED:
			if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
				zlog_debug("wZnuyBDloA9ycBue ""Ignoring LSA of reserved scope");
			ospf6_lsa_delete(his);
			continue;
		}

		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
			zlog_debug("8lIWGDtH7tcOjKzo ""%s acknowledged by %s", his->name,
				   on->name);
		...
		if (mine == NULL) {
			if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
				zlog_debug("1jRImcW653Ax9n0Z ""No database copy");
			ospf6_lsa_delete(his);
			continue;
		}
		...
		if (mine == NULL) {
			if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
				zlog_debug("qfJSOVtiAzycqQRn ""Not on %s's retrans-list",
					   on->name);
			ospf6_lsa_delete(his);
			continue;
		}
		...
		if (ospf6_lsa_compare(his, mine) != 0) {
			...
			if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
				zlog_debug("rZay1Zm0SvZv5MEB ""Questionable acknowledgement");
			ospf6_lsa_delete(his);
			continue;
		}

		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV))
			zlog_debug("KrroQkTZTarDt0Bb "
...
static int ospf6_read_helper(int sockfd, struct ospf6 *ospf6)
{
	...
	if ((uint)len > iobuflen) {
		flog_err(EC_LIB_DEVELOPMENT,"qwqUhjRIlV30A3g4 " "Excess message read");
		return OSPF6_READ_ERROR;
	}
	...
	if (oi == NULL || oi->area == NULL
		...
		if (IS_OSPF6_DEBUG_MESSAGE(OSPF6_MESSAGE_TYPE_UNKNOWN,
					   RECV_HDR))
			zlog_debug("ska00z4mqZAd0dCa ""Message received on disabled interface");
		return OSPF6_READ_CONTINUE;
	}
	if (CHECK_FLAG(oi->flag, OSPF6_INTERFACE_PASSIVE)) {
		if (IS_OSPF6_DEBUG_MESSAGE(OSPF6_MESSAGE_TYPE_UNKNOWN,
					   RECV_HDR))
			zlog_debug("FaETd5zCSXYeeOjQ ""%s: Ignore message on passive interface %s",
	...
	/* Log */
	if (IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV_HDR)) {
		zlog_debug("jWyB5DllABJrnJ4b ""%s received on %s", ospf6_message_type(oh->type),
			   oi->interface->name);
		zlog_debug("kvf2oV0ecO5AwEr5 ""    src: %pI6", &src);
		zlog_debug("YYquXx2JpSLyHWsb ""    dst: %pI6", &dst);
...
static void ospf6_write(struct event *thread)
{
	...
	if (ospf6->fd < 0) {
		zlog_warn("WQi2uTjdKKlNeAtI ""ospf6_write failed to send, fd %d", ospf6->fd);
		return;
	}
	...
	while ((pkt_count < ospf6->write_oi_count) && oi) {
		...
		if (len != (op->length + (int)at_len))
			flog_err(EC_LIB_DEVELOPMENT,"mmUnqAloUweTcfAn "
				 "Could not send entire message");

		if (IS_OSPF6_DEBUG_MESSAGE(oh->type, SEND_HDR)) {
			zlog_debug("9VPuPamiSLewkrmK ""%s send on %s",
				   ospf6_message_type(oh->type),
				   oi->interface->name);
			zlog_debug("RBXO6fNncjkivefa ""    src: %pI6", oi->linklocal_addr);
			zlog_debug("Yg1fxW8aWPKbNo8Y ""    dst: %pI6", &op->dst);
			switch (oh->type) {
			case OSPF6_MESSAGE_TYPE_HELLO:
			...
			case OSPF6_MESSAGE_TYPE_LSACK:
				...
				break;
			default:
				zlog_debug("DpauEhhWk2iBxTFf ""Unknown message");
				assert(0);
				break;
			}
		}
		...
		case OSPF6_MESSAGE_TYPE_HELLO:
			...
			/* log if latency exceeds the hello period */
			if (latency > ((int64_t)oi->hello_interval * 1000000))
				zlog_warn("9yJcECNPe38yCm5s ""%s hello TX high latency %" PRId64
					  "us.",
					  __func__, latency);
		...
		case OSPF6_MESSAGE_TYPE_LSACK:
			...
			break;
		default:
			zlog_debug("k2oe8fWwxph57Dd6 ""Unknown message");
			assert(0);
			break;
		}
...
void ospf6_hello_send(struct event *thread)
{
	...
	if (oi->state <= OSPF6_INTERFACE_DOWN) {
		if (IS_OSPF6_DEBUG_MESSAGE(OSPF6_MESSAGE_TYPE_HELLO, SEND_HDR))
			zlog_debug("6lzZbTXMr0IAV4Hl ""Unable to send Hello on down interface %s",
				   oi->interface->name);
		return;
	}
...
void ospf6_lsreq_send(struct event *thread)
{
	...
	if (on->state != OSPF6_NEIGHBOR_EXCHANGE
		...
		if (IS_OSPF6_DEBUG_MESSAGE(OSPF6_MESSAGE_TYPE_LSREQ, SEND_HDR))
			zlog_debug("Ty0rITotqBKhLEyN ""Quit to send LSReq to neighbor %s state %s",
				   on->name,
...
void ospf6_lsupdate_send_neighbor(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_MESSAGE(OSPF6_MESSAGE_TYPE_LSUPDATE, SEND_HDR))
		zlog_debug("g07EBdEWHCFYumFH ""LSUpdate to neighbor %s", on->name);

	if (on->state < OSPF6_NEIGHBOR_EXCHANGE) {
		if (IS_OSPF6_DEBUG_MESSAGE(OSPF6_MESSAGE_TYPE_LSUPDATE,
					   SEND_HDR))
			zlog_debug("YZU3FeVDuluks2A9 ""Quit to send (neighbor state %s)",
				   ospf6_neighbor_state_str[on->state]);
		return;
	}
				     ...
				     struct ospf6_lsa *lsa)
{
	...
	if (IS_OSPF6_DEBUG_FLOODING
		...
		zlog_debug("2MA1fjacUYCUplye ""%s: Send lsupdate with lsa %s (age %u)", __func__,
...
void ospf6_lsack_send_neighbor(struct event *thread)
{
	...
	if (on->state < OSPF6_NEIGHBOR_EXCHANGE) {
		if (IS_OSPF6_DEBUG_MESSAGE(OSPF6_MESSAGE_TYPE_LSACK, SEND_HDR))
			zlog_debug("X9qQIDwk4j4W81bZ ""Quit to send LSAck to neighbor %s state %s",
				   on->name,
				   ospf6_neighbor_state_str[on->state]);
		return;
	}
