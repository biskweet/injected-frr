...
static void ospf6_area_lsdb_hook_add(struct ospf6_lsa *lsa)
{
	...
	case OSPF6_LSTYPE_NETWORK:
		if (IS_OSPF6_DEBUG_EXAMIN_TYPE(lsa->header->type)) {
			zlog_debug("HuLemBRL8drU2jyy ""%s Examin LSA %s", __func__, lsa->name);
			zlog_debug("4MBVNrqADcSss4Cm "" Schedule SPF Calculation for %s",
...
static void ospf6_area_lsdb_hook_remove(struct ospf6_lsa *lsa)
{
	...
	case OSPF6_LSTYPE_NETWORK:
		if (IS_OSPF6_DEBUG_EXAMIN_TYPE(lsa->header->type)) {
			zlog_debug("IaEiGXyezufn1SSD ""LSA disappearing: %s", lsa->name);
			zlog_debug("uU2IImy9CuoTqNWM ""Schedule SPF Calculation for %s",
				   OSPF6_AREA(lsa->lsdb->data)->name);
		}
		ospf6_spf_schedule(
			OSPF6_PROCESS(OSPF6_AREA(lsa->lsdb->data)->ospf6),
...
static void ospf6_area_stub_update(struct ospf6_area *area)
{
	...
	if (IS_AREA_STUB(area)) {
		if (IS_OSPF6_DEBUG_ORIGINATE(ROUTER))
			zlog_debug("lbpEgcRnNSGzpmp3 ""Stubbing out area for area %s", area->name);
		OSPF6_OPT_CLEAR(area->options, OSPF6_OPT_E);
		ospf6_asbr_remove_externals_from_area(area);
	} else if (IS_AREA_ENABLED(area)) {
		if (IS_OSPF6_DEBUG_ORIGINATE(ROUTER))
			zlog_debug("iqOg1YGinuMoO64i ""Normal area for area %s", area->name);
