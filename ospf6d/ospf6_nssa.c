...
static int ospf6_abr_nssa_am_elected(struct ospf6_area *oa)
{
	...
	for (ALL_LSDB_TYPED(oa->lsdb, type, lsa)) {
		...
		if (CHECK_FLAG(router_lsa->bits, OSPF6_ROUTER_BIT_NT)) {
			if (IS_OSPF6_DEBUG_NSSA)
				zlog_debug("Oql49LgHTigksxhe ""%s: router %pI4 asserts Nt",
	...
	if (best == NULL) {
		if (IS_OSPF6_DEBUG_NSSA)
			zlog_debug("xjo2lVZ1umKJ6AY3 ""%s: best electable ABR not found",
				   __func__);
		return 0;
	}

	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("q7l1noTkcHLKo5MK ""%s: best electable ABR is: %pI4", __func__, best);

	if (IPV4_ADDR_CMP(best, &oa->ospf6->router_id) <= 0) {
		if (IS_OSPF6_DEBUG_NSSA)
			zlog_debug("Ty61nHD9gNbnkFwu ""%s: elected ABR is: %pI4", __func__, best);
		return 1;
	} else {
		if (IS_OSPF6_DEBUG_NSSA)
			zlog_debug("vN86EC9DEjyRcmZu ""%s: not elected best %pI4, router ID %pI4",
...
static void ospf6_flush_translated_lsa(struct ospf6_area *area)
{
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("dKFqUYBfzxuTjacp ""%s: start area %s", __func__, area->name);
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("Wuv4UzhdXoEPBQNN ""%s: finish area %s", __func__, area->name);
}
...
void ospf6_abr_nssa_check_status(struct ospf6 *ospf6)
{
	...
	for (ALL_LIST_ELEMENTS(ospf6->area_list, lnode, nnode, area)) {
		...
		if (IS_OSPF6_DEBUG_NSSA)
			zlog_debug("cSxtnfgePFP9NsOU ""%s: checking area %s flag %x", __func__,
				   area->name, area->flag);
		...
		if (!CHECK_FLAG(area->ospf6->flag, OSPF6_FLAG_ABR)) {
			if (IS_OSPF6_DEBUG_NSSA)
				zlog_debug("HPx34ZEj9gwmGDdG ""%s: not ABR", __func__);
			...
			if (area->NSSATranslatorRole == OSPF6_NSSA_ROLE_ALWAYS)
				...
				if (ospf6_abr_nssa_am_elected(area) > 0) {
					...
					ospf6_flush_translated_lsa(area);
					if (IS_OSPF6_DEBUG_NSSA)
						zlog_debug("dAneak1Pbm1jQ3Pe ""%s: not elected",
							   __func__);
				}
			}
		}
...
static void ospf6_abr_unapprove_summaries(struct ospf6 *ospf6)
{
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("c3ciK7q7Tm0OLosJ ""%s : Start", __func__);

	for (ALL_LIST_ELEMENTS(ospf6->area_list, node, nnode, area)) {
		if (IS_OSPF6_DEBUG_ABR)
			zlog_debug("sUDBFQKFdmgEi0nX ""%s : considering area %pI4", __func__,
				   &area->area_id);
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("5l8EiMJbaBjd07y1 ""%s : Stop", __func__);
}
...
void ospf6_asbr_prefix_readvertise(struct ospf6 *ospf6)
{
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("kRb47ubdK0CusOr6 ""Re-examining Inter-Router prefixes");
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("qgy7z1HCn5q0lNTH ""Finished re-examining Inter-Router prefixes");
}
...
static void ospf6_abr_announce_aggregates(struct ospf6 *ospf6)
{
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("05fw0xJbvJPv0XX7 ""%s: Start", __func__);

	for (ALL_LIST_ELEMENTS_RO(ospf6->area_list, node, area)) {
		if (IS_OSPF6_DEBUG_ABR)
			zlog_debug("3ErMAihhpJwJ6m9U "
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("3ZfjdySWjwLGETAJ ""%s: Stop", __func__);
}
...
void ospf6_abr_remove_unapproved_summaries(struct ospf6 *ospf6)
{
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("5v33ELGcmCIxccTK ""%s : Start", __func__);

	for (ALL_LIST_ELEMENTS(ospf6->area_list, node, nnode, area)) {
		if (IS_OSPF6_DEBUG_ABR)
			zlog_debug("2ZkqCuuUHH6dBTn5 ""%s : looking at area %pI4", __func__,
				   &area->area_id);
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("FKF0C7YYnNfIzb8Y ""%s : Stop", __func__);
}
...
{
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("lXmdAooBpBuyKvrB ""%s : Start", __func__);

	if (ospf6->route_table == NULL || ospf6->brouter_table == NULL) {
		if (IS_OSPF6_DEBUG_ABR)
			zlog_debug("CwZ23tk2GwsAIIYd ""%s : Routing tables are not yet ready",
				   __func__);
		return;
	}
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("TVwJkyT4g3h08tCj ""%s : prepare aggregates", __func__);

	ospf6_abr_range_reset_cost(ospf6);

	if (IS_OSPF6_ABR(ospf6)) {
		if (IS_OSPF6_DEBUG_ABR)
			zlog_debug("u1XaBJWn8MmpVr5b ""%s : process network RT", __func__);
		ospf6_abr_prefix_resummarize(ospf6);

		if (IS_OSPF6_DEBUG_ABR)
			zlog_debug("RgcjtfRgtH0GcPhk ""%s : process router RT", __func__);
		ospf6_asbr_prefix_readvertise(ospf6);

		if (IS_OSPF6_DEBUG_ABR)
			zlog_debug("SOnl1kE65p6wd4uK ""%s : announce aggregates", __func__);
		ospf6_abr_announce_aggregates(ospf6);

		if (IS_OSPF6_DEBUG_ABR)
			zlog_debug("MxpamkzQfbgNlfap ""%s : announce stub defaults", __func__);
		ospf6_abr_defaults_to_stub(ospf6);

		if (IS_OSPF6_DEBUG_ABR)
			zlog_debug("CGWTtRFWol6yb1kr ""%s : announce NSSA Type-7 defaults",
				   __func__);
		ospf6_abr_nssa_type_7_defaults(ospf6);
	}

	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("0P8BYD0WcMt9DJXo ""%s : remove unapproved summaries", __func__);
	ospf6_abr_remove_unapproved_summaries(ospf6);

	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("zKBeOwejiDz8HOow ""%s : Stop", __func__);
}
...
static void ospf6_abr_unapprove_translates(struct ospf6 *ospf6)
{
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("ZDepMrKUZw98V5Af ""%s: Start", __func__);
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("qrLaXhvXlfP7AVoW ""%s: Stop", __func__);
}
						       ...
						       struct ospf6_lsa *type7)
{
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("mLjCk48eQn6gecOB ""%s : Start", __func__);

	if (area->NSSATranslatorState == OSPF6_NSSA_TRANSLATE_DISABLED) {
		if (IS_OSPF6_DEBUG_NSSA)
			zlog_debug("0vbd1xHiFeleuqu6 ""%s: Translation disabled for area %s",
				   __func__, area->name);
		return NULL;
	}
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("RrevgjeHyNm5o3Gx ""%s: Originated type5 LSA id %pI4", __func__,
			   &lsa_header->id);
	return lsa;
}
					      ...
					      struct ospf6_lsa *lsa)
{
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("x6Xun01SdeGUHPYa ""%s : start lsa %s", __func__, lsa->name);
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("fSN9hVy9uvqjl9Wa ""%s : finish lsa %s", __func__, lsa->name);
}
						...
						struct ospf6_lsa *type5)
{
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("D4b7ensfy9xhp7dP ""%s : start area %s", __func__, area->name);
	...
	if (type5) {
		if (CHECK_FLAG(type5->flag, OSPF6_LSA_LOCAL_XLT)) {
			...
			if (IS_OSPF6_DEBUG_NSSA)
				zlog_debug("4cOt0sYrrlvT3SHO ""%s: Invalid translated LSA %s",
					   __func__, type5->name);
			return NULL;
		}
	}
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("970LQtRmDrOG19xH ""%s: finish", __func__);
...
static void ospf6_abr_process_nssa_translates(struct ospf6 *ospf6)
{
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("CMFwlket25KIFlkw ""%s : Start", __func__);
	...
	for (ALL_LIST_ELEMENTS_RO(ospf6->area_list, node, oa)) {
		...
		if (oa->NSSATranslatorState == OSPF6_NSSA_TRANSLATE_DISABLED) {
			zlog_debug("qlJcPMObwAKf27Ec ""%s area %pI4 NSSATranslatorState %d",
				   __func__, &oa->area_id,
		...
		if (IS_OSPF6_DEBUG_NSSA)
			zlog_debug("JZLv52HQqogYyyJk ""%s : looking at area %pI4", __func__,
				   &oa->area_id);

		type = htons(OSPF6_LSTYPE_TYPE_7);
		for (ALL_LSDB_TYPED(oa->lsdb, type, lsa)) {
			zlog_debug("EyzYULUDKGWJThOY ""%s : lsa %s , id %pI4 , adv router %pI4",
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("HQCkpgJi8Jj6PuCf ""%s : Stop", __func__);
}
...
static void ospf6_abr_send_nssa_aggregates(struct ospf6 *ospf6)
{
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("CGVg8ZdBXggg3Br8 ""%s: Start", __func__);
	...
	for (ALL_LIST_ELEMENTS_RO(ospf6->area_list, node, area)) {
		...
		if (IS_OSPF6_DEBUG_NSSA)
			zlog_debug("7BxOBN8Z8h4aZ0ma ""%s: looking at area %pI4", __func__,
				   &area->area_id);
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("O0K7Z6D0tw6oLyNz ""%s: Stop", __func__);
}
...
static void ospf6_abr_remove_unapproved_translates(struct ospf6 *ospf6)
{
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("aYfrOUto1bkD1my5 ""%s: Start", __func__);

	type = htons(OSPF6_LSTYPE_AS_EXTERNAL);
	for (ALL_LSDB_TYPED(ospf6->lsdb, type, lsa)) {
		if (CHECK_FLAG(lsa->flag, OSPF6_LSA_LOCAL_XLT)
		    && CHECK_FLAG(lsa->flag, OSPF6_LSA_UNAPPROVED)) {
			zlog_debug("C8BI3gmaKvBCEoql "
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("Za8WTA3S7gCFZlfi ""%s: Stop", __func__);
}
						 ...
						 struct ospf6_area *oa)
{
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("3QeW5muLJSTcuFCF ""Announcing Type-7 default route into NSSA area %s",
			   oa->name);
...
{
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("3ql7AJ8K4GYwbTM1 ""Check for NSSA-ABR Tasks():");

	if (!IS_OSPF6_ABR(ospf6)) {
		if (IS_OSPF6_DEBUG_NSSA)
			zlog_debug("esng0oHnAyqG48w0 ""%s  Not ABR", __func__);
		return;
	}

	if (!ospf6->anyNSSA) {
		if (IS_OSPF6_DEBUG_NSSA)
			zlog_debug("wCqJEGZUREccDQ5m ""%s  Not NSSA", __func__);
		return;
	}

	/* Each area must confirm TranslatorRole */
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("gEZRkeMGD2JzhsHb ""%s: Start", __func__);

	/* For all Global Entries flagged "local-translate", unset APPROVED */
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("z36HTeENPLOVhP07 ""%s: unapprove translates", __func__);

	ospf6_abr_unapprove_translates(ospf6);

	/* Originate Type-7 aggregates */
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("BjuQ0qlTdfrAdLIA ""%s: send NSSA aggregates", __func__);
	 ...
	 */
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("CnoMiua9e6XqTOzF ""%s: process translates", __func__);
	ospf6_abr_process_nssa_translates(ospf6);

	/* Flush any unapproved previous translates from Global Data Base */
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("FC07gAv2ZhRxw0L8 ""%s: remove unapproved translates", __func__);
	ospf6_abr_remove_unapproved_translates(ospf6);

	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("kVmstqoeAU3vAMI9 ""%s: Stop", __func__);
}
	...
	if (ROUTEMAP(red)) {
		...
		if (ret == RMAP_DENYMATCH) {
			if (IS_OSPF6_DEBUG_ASBR)
				zlog_debug("vLWOlLPkSsXHiI7G ""Denied by route-map \"%s\"",
					   ROUTEMAP_NAME(red));
			return 0;
		}
	}
...
static void ospf6_abr_task_timer(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("xYhAvtFISrs8yHox ""Running ABR task on timer");
...
void ospf6_schedule_abr_task(struct ospf6 *ospf6)
{
	if (event_is_scheduled(ospf6->t_abr_task)) {
		if (IS_OSPF6_DEBUG_ABR)
			zlog_debug("JEaCGlYh0zrlVsJw ""ABR task already scheduled");
		return;
	}

	if (IS_OSPF6_DEBUG_ABR)
		zlog_debug("EfWF9mk608OvhzDE ""Scheduling ABR task");
...
static void ospf6_nssa_flush_area(struct ospf6_area *area)
{
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("xipYpPJ2w6HVsd8L ""%s: area %s", __func__, area->name);
...
void ospf6_area_nssa_update(struct ospf6_area *area)
{
	if (IS_AREA_NSSA(area)) {
		...
		if (IS_OSPF6_DEBUG_ORIGINATE(ROUTER))
			zlog_debug("tVhdmkOKdxuF7CnB ""Normal area for if %s", area->name);
	...
	if (IS_AREA_NSSA(area)) {
		...
		if (ospf6_check_and_set_router_abr(area->ospf6)) {
			if (IS_OSPF6_DEBUG_NSSA)
				zlog_debug("ts3Xvk55uUriJAMw ""Router is ABR area %s", area->name);
			ospf6_schedule_abr_task(area->ospf6);
		} else {
			/* Router is not ABR */
			if (IS_OSPF6_DEBUG_NSSA)
				zlog_debug("YsZ5rK7ykrNZ3sek ""NSSA area %s", area->name);
		...
		/* Disable NSSA */
		if (IS_OSPF6_DEBUG_NSSA)
			zlog_debug("svFhyIxZ2UVrNQ7J ""Normal area %s", area->name);
		ospf6_nssa_flush_area(area);

		/* Check if router is ABR */
		if (ospf6_check_and_set_router_abr(area->ospf6)) {
			if (IS_OSPF6_DEBUG_NSSA)
				zlog_debug("QdhFcwcu1rFS3sfA ""Router is ABR area %s", area->name);
			ospf6_schedule_abr_task(area->ospf6);
			 ...
			 */
			if (IS_OSPF6_DEBUG_NSSA)
				zlog_debug("YxpCFAU7W7Xshpvw ""Refresh type-5 LSAs, area %s",
					   area->name);
...
int ospf6_area_nssa_set(struct ospf6 *ospf6, struct ospf6_area *area)
{
	...
	if (!IS_AREA_NSSA(area)) {
		...
		if (IS_OSPF6_DEBUG_NSSA)
			zlog_debug("W5TcAk4RKvtl3q4Q ""area %s nssa set", area->name);
...
int ospf6_area_nssa_unset(struct ospf6 *ospf6, struct ospf6_area *area)
{
	if (IS_AREA_NSSA(area)) {
		...
		if (IS_OSPF6_DEBUG_NSSA)
			zlog_debug("fyySRk24FhXwo4WZ ""area %s nssa reset", area->name);
			      ...
			      struct ospf6_area *area, bool p_bit)
{
	...
	if (IS_OSPF6_DEBUG_ASBR || IS_OSPF6_DEBUG_ORIGINATE(AS_EXTERNAL))
		zlog_debug("u6lOwNBzRxjqV49y ""Originate NSSA-LSA for %pFX", &route->prefix);
				    ...
				    struct ospf6_lsa *lsa)
{
	...
	if (IS_OSPF6_DEBUG_NSSA)
		zlog_debug("nIsYoOw4gRkokB5B ""%s : start", __func__);
	...
	if (!type5) {
		if (IS_OSPF6_DEBUG_NSSA)
			zlog_debug("VTl1FecIF1yIaHaO ""%s : Originating type5 LSA", __func__);
