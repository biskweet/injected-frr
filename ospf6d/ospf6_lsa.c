...
void ospf6_lsa_premature_aging(struct ospf6_lsa *lsa)
{
	...
	if (IS_OSPF6_DEBUG_LSA_TYPE(lsa->header->type))
		zlog_debug("9k0gdIGX4qZY3RaM ""LSA: Premature aging: %s", lsa->name);
...
void ospf6_lsa_header_print_raw(struct ospf6_lsa_header *header)
{
	...
	zlog_debug("QPlfAI3JEifwJyI7 ""    [%s Id:%s Adv:%s]", ospf6_lstype_name(header->type), id,
		   adv_router);
	zlog_debug("LvKdPjx33v7BoX7p ""    Age: %4hu SeqNum: %#08lx Cksum: %04hx Len: %d",
...
void ospf6_lsa_expire(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_LSA_TYPE(lsa->header->type)) {
		zlog_debug("JU5CBQ2qDHURHv1O ""LSA Expire:");
...
void ospf6_lsa_refresh(struct event *thread)
{
	...
	if (self == NULL) {
		if (IS_OSPF6_DEBUG_LSA_TYPE(old->header->type))
			zlog_debug("XQEhrDAxloEwQLRC ""Refresh: could not find self LSA, flush %s",
				   old->name);
	...
	if (IS_OSPF6_DEBUG_LSA_TYPE(new->header->type)) {
		zlog_debug("CiMiwfscpw9A6iHN ""LSA Refresh:");
