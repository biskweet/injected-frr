...
struct ospf6_interface *ospf6_interface_create(struct interface *ifp)
{
	...
	if (oi->ifmtu > iobuflen) {
		if (IS_OSPF6_DEBUG_INTERFACE)
			zlog_debug("1bpuR1wfPA3oVsGm ""Interface %s: IfMtu is adjusted to I/O buffer size: %d.",
				   ifp->name, iobuflen);
		oi->ifmtu = iobuflen;
	}
...
void ospf6_interface_state_update(struct interface *ifp)
{
	...
	if (ifp->mtu6 != oi->ifmtu) {
		...
		if (!oi->c_ifmtu) {
			...
			if (oi->ifmtu > iobuflen) {
				if (IS_OSPF6_DEBUG_INTERFACE)
					zlog_debug("nJMys0aTZMtnP4xR ""Interface %s: IfMtu is adjusted to I/O buffer size: %d.",
						   ifp->name, iobuflen);
				oi->ifmtu = iobuflen;
			}
		} else if (oi->c_ifmtu > ifp->mtu6) {
			oi->ifmtu = ifp->mtu6;
			zlog_warn("ZSjv7PMeV4Oagiwz ""Configured mtu %u on %s overridden by kernel %u",
...
void ospf6_interface_connected_route_update(struct interface *ifp)
{
	...
	frr_each (if_connected, ifp->connected, c) {
		...
		if (oi->plist_name) {
			...
			if (ret == PREFIX_DENY) {
				if (IS_OSPF6_DEBUG_INTERFACE)
					zlog_debug("LTH1VoJ87bNv9qaG ""%pFX on %s filtered by prefix-list %s ",
						   c->address,
						   oi->interface->name,
						   oi->plist_name);
				continue;
					...
					struct ospf6_interface *oi)
{
	...
	/* log */
	if (IS_OSPF6_DEBUG_INTERFACE) {
		zlog_debug("EBh1dhFgFBpreFi2 ""Interface state change %s: %s -> %s",
			   oi->interface->name,
					      ...
					      struct ospf6_neighbor *b)
{
	...
	zlog_warn("EzJyxMBmWyi6GuCO ""Router-ID duplicate ?");
	return a;
}
					     ...
					     struct ospf6_neighbor *b)
{
	...
	zlog_warn("nOLy07cqd10aCPOL ""Router-ID duplicate ?");
	return a;
}
...
uint8_t dr_election(struct ospf6_interface *oi)
{
	...
	if (oi->drouter != (drouter ? drouter->router_id : htonl(0)) ||
		...
		if (IS_OSPF6_DEBUG_INTERFACE)
			zlog_debug("66vc1BjgrGZS9Yfi ""DR Election on %s: DR: %s BDR: %s",
				   oi->interface->name,
...
void interface_up(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_INTERFACE)
		zlog_debug("OmnJ4JD8NFXY1pPQ ""Interface Event %s: [InterfaceUp]",
			   oi->interface->name);

	/* check physical interface is up */
	if (!if_is_operative(oi->interface)) {
		zlog_warn("YFS6aEPlAx3iwxoo ""Interface %s is down, can't execute [InterfaceUp]",
			  oi->interface->name);
		return;
	}
	...
	if (!(ospf6_interface_get_linklocal_address(oi->interface) ||
	      if_is_loopback(oi->interface))) {
		zlog_warn("IXKNz9jPcDHvRWyq ""Interface %s has no link local address, can't execute [InterfaceUp]",
			  oi->interface->name);
		return;
	}
	...
	if (oi->state > OSPF6_INTERFACE_DOWN) {
		if (IS_OSPF6_DEBUG_INTERFACE)
			zlog_debug("x0O8SpFuTLBwj6wi ""Interface %s already enabled",
				   oi->interface->name);
		return;
	}
	...
	if (oi->area == NULL) {
		zlog_warn("DOfgtH84VRTPUHOF ""%s: Not scheduling Hello for %s as there is no area assigned yet",
	...
	if (ifmaddr_check(oi->interface->ifindex, &allspfrouters6)) {
		zlog_info("omTGmMmQss0oS374 ""Interface %s is still in all routers group, rescheduling for SSO",
			  oi->interface->name);
	...
	if (ospf6_sso(oi->interface->ifindex, &allspfrouters6, IPV6_JOIN_GROUP,
		...
		if (oi->sso_try_cnt++ < OSPF6_INTERFACE_SSO_RETRY_MAX) {
			zlog_info("KIibK54y6d4khmpt ""Scheduling %s for sso retry, trial count: %d",
...
void wait_timer(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_INTERFACE)
		zlog_debug("SyKV8ckv1V2acUCu ""Interface Event %s: [WaitTimer]",
			   oi->interface->name);
...
void backup_seen(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_INTERFACE)
		zlog_debug("XIyv7PF1NsMsl8gY ""Interface Event %s: [BackupSeen]",
			   oi->interface->name);
...
void neighbor_change(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_INTERFACE)
		zlog_debug("2S7s85r0ezMdmhaT ""Interface Event %s: [NeighborChange]",
			   oi->interface->name);
...
void interface_down(struct event *thread)
{
	...
	if (IS_OSPF6_DEBUG_INTERFACE)
		zlog_debug("GOTaoCV1jjWX5KXy ""Interface Event %s: [InterfaceDown]",
			   oi->interface->name);
...
static int ospf6_ifp_create(struct interface *ifp)
{
	if (IS_OSPF6_DEBUG_ZEBRA(RECV))
		zlog_debug("aG4qHgyMgkm8Ntoh ""Zebra Interface add: %s index %d mtu %d", ifp->name,
...
{
	if (IS_OSPF6_DEBUG_ZEBRA(RECV))
		zlog_debug("5uh51M1xuCZSY0kC ""Zebra Interface state change: %s index %d flags %llx metric %d mtu %d bandwidth %d",
...
{
	if (IS_OSPF6_DEBUG_ZEBRA(RECV))
		zlog_debug("stWOocx1Us2yRylN ""Zebra Interface state change: %s index %d flags %llx metric %d mtu %d bandwidth %d",
			   ifp->name, ifp->ifindex,
			   (unsigned long long)ifp->flags, ifp->metric,
			   ifp->mtu6, ifp->bandwidth);
...
{
	if (if_is_up(ifp))
		zlog_warn("xbABgJhDtTAzpuBk ""Zebra: got delete of %s, but interface is still up",
			  ifp->name);

	if (IS_OSPF6_DEBUG_ZEBRA(RECV))
		zlog_debug("zuNaUZ6pqY5pTE50 ""Zebra Interface delete: %s index %d mtu %d",
...
void ospf6_interface_clear(struct interface *ifp)
{
	...
	if (IS_OSPF6_DEBUG_INTERFACE)
		zlog_debug("ul5YT7duslyx7YD7 ""Interface %s: clear by reset", ifp->name);
