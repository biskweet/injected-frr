...
static struct ospf6_vertex *ospf6_vertex_create(struct ospf6_lsa *lsa)
{
	...
	if (IS_OSPF6_DEBUG_SPF(PROCESS))
		zlog_debug("9KK1Ttr3IKKVuBDx ""%s: Creating vertex %s of type %s (0x%04hx) lsa %s",
			   __func__, v->name,
					  ...
					  struct ospf6_vertex *v)
{
	...
	if (IS_OSPF6_DEBUG_SPF(PROCESS)) {
		...
		if (lsa)
			zlog_debug("9xVAg9fUIf8PRRgj ""  Link to: %s len %u, V %s", lsa->name,
				   ospf6_lsa_size(lsa->header), v->name);
		else
			zlog_debug("HvMi0GkDIp3I3ByF ""  Link to: [%s Id:%s Adv:%s] No LSA , V %s",
				   ...
				   struct ospf6_vertex *v)
{
	...
	if (IS_OSPF6_DEBUG_SPF(PROCESS))
		zlog_debug("1o1MtOyzgbR07R6P ""Vertex %s Lsa %s Backlink %s", v->name, lsa->name,
			   (found ? "OK" : "FAIL"));
			       ...
			       caddr_t lsdesc, struct ospf6 *ospf6)
{
	...
	ifindex = (VERTEX_IS_TYPE(NETWORK, v) ? ospf6_spf_get_ifindex_from_nh(v)
					      : ROUTER_LSDESC_GET_IFID(lsdesc));
	if (ifindex == 0) {
		flog_err(EC_LIB_DEVELOPMENT,"Ih7CgWyAVTUVI7ws " "No nexthop ifindex at vertex %s",
			 v->name);
		return;
	}
	...
	if (oi == NULL) {
		zlog_warn("LVAPJcJpmgOLcINR ""Can't find interface in SPF: ifindex %d", ifindex);
		return;
	}
	...
	for (ALL_LSDB_TYPED_ADVRTR(oi->lsdb, type, adv_router, lsa)) {
		...
		if (IS_OSPF6_DEBUG_SPF(PROCESS)) {
			inet_ntop(AF_INET6, &link_lsa->linklocal_addr, buf,
				  sizeof(buf));
			zlog_debug("qcp61Az67dbHXW1j ""  nexthop %s from %s", buf, lsa->name);
		}
	...
	if (i == 0 && IS_OSPF6_DEBUG_SPF(PROCESS))
		zlog_debug("D42pJcntxjZUQtiO ""No nexthop for %s found", w->name);
}
			   ...
			   struct ospf6_area *oa)
{
	...
	if (lsa == NULL) {
		zlog_warn("Y05dedhg9KwaCsSs ""%s: No router LSA for area %s", __func__, oa->name);
		return;
	}
...
static void ospf6_spf_log_database(struct ospf6_area *oa)
{
	...
	zlog_debug("PKYMp6eEU0blvtoJ ""%s", buffer);
}
...
static void ospf6_spf_calculation_thread(struct event *t)
{
	...
	for (ALL_LIST_ELEMENTS_RO(ospf6->area_list, node, oa)) {
		...
		monotime(&oa->ts_spf);
		if (IS_OSPF6_DEBUG_SPF(PROCESS))
			zlog_debug("ZtkxeEkbl9cJ1FG7 ""SPF calculation for Area %s", oa->name);
	...
	if (ospf6->backbone) {
		...
		if (IS_OSPF6_DEBUG_SPF(PROCESS))
			zlog_debug("iSO0nTvRmrpMjSNE ""SPF calculation for Backbone area %s",
				   ospf6->backbone->name);
		if (IS_OSPF6_DEBUG_SPF(DATABASE))
...
void ospf6_spf_schedule(struct ospf6 *ospf6, unsigned int reason)
{
	...
	if (IS_OSPF6_DEBUG_SPF(PROCESS) || IS_OSPF6_DEBUG_SPF(TIME)) {
		...
		zlog_debug("HjZJNT4MtiHAFZob ""SPF: calculation timer scheduled (reason %s)",
			   rbuf);
	}
	...
	if (IS_OSPF6_DEBUG_SPF(PROCESS) || IS_OSPF6_DEBUG_SPF(TIME))
		zlog_debug("AH3XrNibWZx0aEPI ""SPF: Rescheduling in %ld msec", delay);
						 ...
						 uint32_t adv_router)
{
	...
	if (IS_OSPF6_DEBUG_SPF(PROCESS))
		zlog_debug("lTLHLFvlnJH4rKuN ""%s: adv_router %s num_lsa %u to convert.", __func__,
			   ifbuf, num_lsa);
	if (num_lsa == 1)
		return lsa;
	...
	if (num_lsa == 0) {
		if (IS_OSPF6_DEBUG_SPF(PROCESS))
			zlog_debug("ivQoC0TV9Pn5QENj ""%s: adv_router %s not found in LSDB.",
				   __func__, ifbuf);
		return NULL;
	}
	...
	while (rtr_lsa) {
		...
		if (IS_OSPF6_DEBUG_SPF(PROCESS)) {
			...
			zlog_debug("NKYPNOpbugnKiR0x ""%s: Next Router LSA %s to aggreat with len %u interface_id %s",
	...
	if (IS_OSPF6_DEBUG_SPF(PROCESS))
		zlog_debug("jeytDjoxI9kkqqSE ""%s: LSA %s id %u type 0%x len %u num_lsa %u",
			      ...
			      struct ospf6_area *area)
{
	...
	if (IS_OSPF6_DEBUG_SPF(PROCESS))
		zlog_debug("JAg7iB6eRJLkV6a1 ""%s :  start", __func__);

	if (ntohs(lsa->header->type) == OSPF6_LSTYPE_TYPE_7)
		if (IS_OSPF6_DEBUG_SPF(PROCESS))
			zlog_debug("YiTlYkdL7w4sA9yI ""%s: Processing Type-7", __func__);

	/* Stay away from any Local Translated Type-7 LSAs */
	if (CHECK_FLAG(lsa->flag, OSPF6_LSA_LOCAL_XLT)) {
		if (IS_OSPF6_DEBUG_SPF(PROCESS))
			zlog_debug("WCSrq3SUB2Xymt4s ""%s: Rejecting Local translated LSA",
				   __func__);
		return 0;
	}
	...
	if (ntohs(lsa->header->type) == OSPF6_LSTYPE_AS_EXTERNAL) {
		...
		if (route == NULL) {
			if (IS_OSPF6_DEBUG_SPF(PROCESS))
				zlog_debug("PiiAELXXX6Dr7MmN ""%s: no external route %pFX",
					   __func__, &prefix);
			return 0;
		}
		...
		if (route == NULL) {
			if (IS_OSPF6_DEBUG_SPF(PROCESS))
				zlog_debug("biYSRb1Nt0lvHvfx ""%s: no route %pFX, area %s",
		...
		if (CHECK_FLAG(route->flag, OSPF6_ROUTE_REMOVE)) {
			if (IS_OSPF6_DEBUG_SPF(PROCESS))
				zlog_debug("XYBozwKnBpfO8eAq ""%s : remove route %pFX, area %s",
					   __func__, &prefix, area->name);
...
static void ospf6_ase_calculate_timer(struct event *t)
{
	...
	if (ospf6->anyNSSA) {
		for (ALL_LIST_ELEMENTS(ospf6->area_list, node, nnode, area)) {
			if (IS_OSPF6_DEBUG_SPF(PROCESS))
				zlog_debug("h21nmHNmvczTXH0I ""%s : looking at area %s", __func__,
					   area->name);
