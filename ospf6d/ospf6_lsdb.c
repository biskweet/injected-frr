...
static void _lsdb_count_assert(struct ospf6_lsdb *lsdb)
{
	...
	zlog_debug("0C2wwGQw730uUnsV ""PANIC !! lsdb[%p]->count = %d, real = %d", lsdb,
		   lsdb->count, num);
	for (ALL_LSDB(lsdb, debug, debugnext))
		zlog_debug("UpSHeQCSYxHJtTyl ""%s lsdb[%p]", debug->name, debug->lsdb);
	zlog_debug("YvTSBflawk3Sgm4o ""DUMP END");
...
struct ospf6_lsa *ospf6_find_external_lsa(struct ospf6 *ospf6, struct prefix *p)
{
	...
	if (match == NULL) {
		if (IS_OSPF6_DEBUG_ASBR)
			zlog_debug("ppyD5vM1Qsw09Xqg ""No such route %pFX to withdraw", p);
					 ...
					 struct ospf6_lsdb *lsdb)
{
	...
	zlog_debug("zcTtmx3klDQtIEDX ""lsdb_lookup_next: key: %pFX", &key);
	...
	for (ALL_LSDB(lsdb, lsa, lsanext)) {
		if (!OSPF6_LSA_IS_MAXAGE(lsa)) {
			if (IS_OSPF6_DEBUG_LSA_TYPE(lsa->header->type))
				zlog_debug("DkD4qlvF5MZJLRFp ""Not MaxAge %s", lsa->name);
			continue;
		}

		if (lsa->retrans_count != 0) {
			if (IS_OSPF6_DEBUG_LSA_TYPE(lsa->header->type))
				zlog_debug("hI1n59t6C9Vd4twS ""Remove MaxAge %s retrans_count %d",
		...
		if (IS_OSPF6_DEBUG_LSA_TYPE(lsa->header->type))
			zlog_debug("mKAxevkb99m4ynW5 ""Remove MaxAge %s", lsa->name);
		...
		if (CHECK_FLAG(lsa->flag, OSPF6_LSA_SEQWRAPPED)) {
			...
			zlog_debug("RpPeSL26Ps1HeUOo ""calling ospf6_lsdb_remove %s", lsa->name);
			ospf6_lsdb_remove(lsa, lsdb);
		}
	}
