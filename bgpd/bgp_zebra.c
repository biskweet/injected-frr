...
static int bgp_router_id_update(ZAPI_CALLBACK_ARGS)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("vAVNIMAaDHLzc7Pe ""Rx Router Id update VRF %u Id %pFX", vrf_id,
			   &router_id);
...
static int bgp_ifp_destroy(struct interface *ifp)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("qCeNBWZGQRW06Ah3 ""Rx Intf del VRF %u IF %s", ifp->vrf->vrf_id,
			   ifp->name);
...
static int bgp_ifp_up(struct interface *ifp)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("DMD6kKS4voGJyWE1 ""Rx Intf up VRF %u IF %s", ifp->vrf->vrf_id,
			   ifp->name);
...
static int bgp_ifp_down(struct interface *ifp)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("hPgLhE3cXfGihYZl ""Rx Intf down VRF %u IF %s", ifp->vrf->vrf_id,
			   ifp->name);
...
static int bgp_interface_address_add(ZAPI_CALLBACK_ARGS)
{
	...
	if (bgp_debug_zebra(ifc->address))
		zlog_debug("qr4rBGYhFeNDfvHo ""Rx Intf address add VRF %u IF %s addr %pFX", vrf_id,
	...
	if (IN6_IS_ADDR_LINKLOCAL(&ifc->address->u.prefix6) &&
		...
		for (ALL_LIST_ELEMENTS(bgp->peer, node, nnode, peer)) {
			...
			if (v6_ll_in_nh_global ||
				...
				if (bgp_debug_zebra(ifc->address)) {
					zlog_debug("OTGPY7cONJS3RQKY ""Update peer %pBP's current intf global addr from %pI6 to %pI6 and send updates",
						   peer,
...
static int bgp_interface_address_delete(ZAPI_CALLBACK_ARGS)
{
	...
	if (bgp_debug_zebra(ifc->address))
		zlog_debug("YIE2D3CAv55ibgJF ""Rx Intf address del VRF %u IF %s addr %pFX", vrf_id,
			   ifc->ifp->name, ifc->address);
...
static int bgp_interface_nbr_address_add(ZAPI_CALLBACK_ARGS)
{
	...
	if (bgp_debug_zebra(ifc->address))
		zlog_debug("SbPucmt8FriKNwNU ""Rx Intf neighbor add VRF %u IF %s addr %pFX",
...
static int bgp_interface_nbr_address_delete(ZAPI_CALLBACK_ARGS)
{
	...
	if (bgp_debug_zebra(ifc->address))
		zlog_debug("wlWuN9UzC1w9pFR9 ""Rx Intf neighbor del VRF %u IF %s addr %pFX",
			   vrf_id, ifc->ifp->name, ifc->address);
...
static int zebra_read_route(ZAPI_CALLBACK_ARGS)
{
	...
	if (bgp_debug_zebra(&api.prefix)) {
		...
		if (add) {
			...
			zlog_debug("WBju8IBoLRPvKPFK ""Rx route DEL VRF %u %s[%d] %pFX", vrf_id,
	...
	if (local->sa.sa_family == AF_INET6) {
		...
		if (!IN6_IS_ADDR_LINKLOCAL(&local->sin6.sin6_addr)) {
			...
			if (!v6_ll_avail && if_is_loopback(ifp))
				...
				flog_warn(
					EC_BGP_NO_LL_ADDRESS_AVAILABLE,"wBvvNYcUSzMjJx8P "
...
static void bgp_zebra_tm_connect(struct event *t)
{
	...
	if (ret < 0) {
		zlog_err("0mW7IFElrnkR0hkj ""Error connecting to table manager!");
		bgp_tm_status_connected = false;
	} else {
		if (!bgp_tm_status_connected) {
			if (BGP_DEBUG(zebra, ZEBRA))
				zlog_debug("ehv9r4y4RPMtitS3 "
			      ...
			      uint32_t *start, uint32_t *end)
{
	...
	if (ret < 0) {
		flog_err(EC_BGP_TABLE_CHUNK,"vGTxbIrnWZZ4xyvX "
			 "BGP: Error getting table chunk %u", chunk_size);
		return -1;
	}
	zlog_info("0p4ENYqH5lX4Hk4P ""BGP: Table Manager returns range from chunk %u is [%u %u]",
		...
		*mpinfo_cp = *mpinfo;
		...
		if (bgp_debug_zebra(&api->prefix)) {
			if (bgp_path_info_num_labels(mpinfo)) {
				zlog_debug("xYPPuvs9pDA0iSsk ""%s: p=%pFX, bgp_is_valid_label: %d",
					   __func__, p,
					   bgp_is_valid_label(
						   &mpinfo->extra->labels
							    ->label[0]));
			} else {
				zlog_debug("uUIcxqxr4L65QDvv ""%s: p=%pFX, no label", __func__, p);
			}
		}
		...
		if (CHECK_FLAG(api_nh->flags, ZAPI_NEXTHOP_FLAG_EVPN) &&
			...
			snprintf(eth_buf, sizeof(eth_buf), " RMAC %s",
				 prefix_mac2str(&api_nh->rmac, buf1,
						sizeof(buf1)));
		zlog_debug("hZc3uTnfGjtJphXs ""  nhop [%d]: %s if %u VRF %u wt %" PRIu64
			   " %s %s %s",
			  ...
			  struct bgp *bgp)
{
	...
	if (bgp_debug_zebra(p)) {
		...
		zlog_debug("l9EO5QYyQsH2ZDnY ""%s: %pFX: announcing to zebra (recursion %sset)",
						   ...
						   struct bgp *bgp)
{
	...
	if (bgp_debug_zebra(p))
		zlog_debug("I5okCHJLOn7cc448 ""Tx route delete VRF %u %pFX", bgp->vrf_id,
			   &api.prefix);
...
static void bgp_handle_route_announcements_to_zebra(struct event *e)
{
	...
	while (count < ZEBRA_ANNOUNCEMENTS_LIMIT) {
		...
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("S2u9bvIlfe42LR3v ""BGP %s route %pBD(%s) with dest %p and flags 0x%x to zebra",
		...
		if (is_evpn && status == ZCLIENT_SEND_FAILURE)
			flog_err(EC_BGP_EVPN_FAIL,"4GadjWbJ0YTPbMhn "
			 ...
			 unsigned short instance, bool changed)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("Wn1OdJTEAvx4FKhA ""Tx redistribute add VRF %u afi %d %s %d",
			    ...
			    unsigned short instance)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("G49CD6UWBYF0BaBl ""Tx redistribute del/add VRF %u afi %d %s %d",
			   bgp->vrf_id, afi, zebra_route_string(type),
			   instance);
			   ...
			   unsigned short instance)
{
	...
	if (bgp_install_info_to_zebra(bgp)) {
		...
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("klB4BTPoWzlBu8en ""Tx redistribute del VRF %u afi %d %s %d",
				   bgp->vrf_id, afi, zebra_route_string(type),
				   instance);
...
void bgp_zebra_instance_register(struct bgp *bgp)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("IMehTlbB7YWeHeJ6 ""Registering VRF %u", bgp->vrf_id);
...
void bgp_zebra_instance_deregister(struct bgp *bgp)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("mlN1Efa7tQ9qJdgT ""Deregistering VRF %u", bgp->vrf_id);
...
void bgp_zebra_initiate_radv(struct bgp *bgp, struct peer *peer)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("86AE61DrBBVjiz8f ""%u: Initiating RA for peer %s", bgp->vrf_id,
			   peer->host);
...
void bgp_zebra_terminate_radv(struct bgp *bgp, struct peer *peer)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("qCC1v1ovLig17HJe ""%u: Terminating RA for peer %s", bgp->vrf_id,
			   peer->host);
...
int bgp_zebra_dup_addr_detection(struct bgp *bgp)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("Hz7nH2e7VXwcrnLg ""dup addr detect %s max_moves %u time %u freeze %s freeze_time %u",
...
static int rule_notify_owner(ZAPI_CALLBACK_ARGS)
{
	...
	if (!bgp_pbra) {
		...
		if (!bgp_pbr && note != ZAPI_RULE_REMOVED) {
			if (BGP_DEBUG(zebra, ZEBRA))
				zlog_debug("y4C0wwA4StEKNB4F ""%s: Fail to look BGP rule (%u)",
					   __func__, unique);
			return 0;
		}
	}
	...
	case ZAPI_RULE_FAIL_INSTALL:
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("1z70TdIuAfDcs2O7 ""%s: Received RULE_FAIL_INSTALL", __func__);
		if (bgp_pbra) {
	...
	case ZAPI_RULE_INSTALLED:
		...
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("AvIxVLYnBDP9KRQ6 ""%s: Received RULE_INSTALLED", __func__);
		break;
	case ZAPI_RULE_FAIL_REMOVE:
	case ZAPI_RULE_REMOVED:
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("l5n7HLKro3z425ek ""%s: Received RULE REMOVED", __func__);
		break;
	}
...
static int ipset_notify_owner(ZAPI_CALLBACK_ARGS)
{
	...
	if (!bgp_pbim) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("oszehIBFaz3pmfWg ""%s: Fail to look BGP match ( %u, ID %u)",
	...
	case ZAPI_IPSET_FAIL_INSTALL:
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("r3AX72NZinrr5JLO ""%s: Received IPSET_FAIL_INSTALL", __func__);
	...
	case ZAPI_IPSET_INSTALLED:
		...
		bgp_pbim->install_in_progress = false;
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("6LIbf0AX7DZ93OMl ""%s: Received IPSET_INSTALLED", __func__);
		break;
	case ZAPI_IPSET_FAIL_REMOVE:
	case ZAPI_IPSET_REMOVED:
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("rSfLYXda3DJwLwvE ""%s: Received IPSET REMOVED", __func__);
		break;
	}
...
static int ipset_entry_notify_owner(ZAPI_CALLBACK_ARGS)
{
	...
	case ZAPI_IPSET_ENTRY_FAIL_INSTALL:
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("2w5l9JV6fboVEtPC ""%s: Received IPSET_ENTRY_FAIL_INSTALL",
				   __func__);
	...
	case ZAPI_IPSET_ENTRY_INSTALLED:
		...
		bgp_pbime->install_in_progress = false;
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("IgCzethDkngZVWRc ""%s: Received IPSET_ENTRY_INSTALLED",
				   __func__);
	...
	case ZAPI_IPSET_ENTRY_REMOVED:
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("Sn2YPT4IGyuhj1od ""%s: Received IPSET_ENTRY_REMOVED",
				   __func__);
		break;
	}
	return 0;
...
static int iptable_notify_owner(ZAPI_CALLBACK_ARGS)
{
	...
	if (!bgpm) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("wjC3VeBA5maX23Oy ""%s: Fail to look BGP iptable (%u %u)",
				   __func__, note, unique);
		return 0;
	}
	switch (note) {
	case ZAPI_IPTABLE_FAIL_INSTALL:
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("XFdaDsYikgNLyJ2L ""%s: Received IPTABLE_FAIL_INSTALL",
				   __func__);
	...
	case ZAPI_IPTABLE_INSTALLED:
		...
		bgpm->install_iptable_in_progress = false;
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("0XdwnfeWjFjZz33H ""%s: Received IPTABLE_INSTALLED", __func__);
		bgpm->action->refcnt++;
		break;
	case ZAPI_IPTABLE_FAIL_REMOVE:
	case ZAPI_IPTABLE_REMOVED:
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("uON8Z7jb4RfNtiII ""%s: Received IPTABLE REMOVED", __func__);
		break;
	}
	return 0;
}
					...
					zebra_size_t length, vrf_id_t vrf_id)
{
	...
	if (!zapi_route_notify_decode(zclient->ibuf, &p, &table_id, &note,
				      &afi, &safi)) {
		zlog_err("wF4LUOfjmw5EzgPD ""%s : error in msg decode", __func__);
		return -1;
	}
	...
	bgp = bgp_lookup_by_vrf_id(vrf_id);
	if (!bgp) {
		flog_err(EC_BGP_INVALID_BGP_INSTANCE,"fkLEIzv7uoEobci7 "
	...
	if (!dest) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("co3ywf4j7XlIdwvR ""%s: %pFX does not exist in the BGP table, nothing to do for %u",
				   __func__, &p, note);
		return -1;
	}
	...
	case ZAPI_ROUTE_INSTALLED:
		...
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("P1M0mCm8CBxWnuji ""route %pBD : INSTALLED", dest);
		/* Find the best route */
		...
		if (new_select)
			...
			flog_err(EC_BGP_INVALID_ROUTE,"yZ7Qpb6Y5xIrD6TP "
	...
	case ZAPI_ROUTE_REMOVED:
		...
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("Q76KkOER6N8Ji9Ua ""route %pBD: Removed from Fib", dest);
		break;
	case ZAPI_ROUTE_FAIL_INSTALL:
		new_select = NULL;
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("prva4pgOv6aNUEIu ""route: %pBD Failed to Install into Fib",
				   dest);
		UNSET_FLAG(dest->flags, BGP_NODE_FIB_INSTALL_PENDING);
		UNSET_FLAG(dest->flags, BGP_NODE_FIB_INSTALLED);
		for (pi = bgp_dest_get_bgp_path_info(dest); pi; pi = pi->next) {
	...
	case ZAPI_ROUTE_BETTER_ADMIN_WON:
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("eWzAFrfDijMoGOro ""route: %pBD removed due to better admin won",
				   dest);
		new_select = NULL;
		UNSET_FLAG(dest->flags, BGP_NODE_FIB_INSTALL_PENDING);
		UNSET_FLAG(dest->flags, BGP_NODE_FIB_INSTALLED);
	...
	case ZAPI_ROUTE_REMOVE_FAIL:
		zlog_warn("LjcX5dwev5z1dl8J ""%s: Route %pBD failure to remove", __func__, dest);
		break;
	}
...
static int bgp_zebra_process_local_es_del(ZAPI_CALLBACK_ARGS)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("BWL3sxDu5TORnZXs ""Rx del ESI %s",
...
static int bgp_zebra_process_local_es_evi(ZAPI_CALLBACK_ARGS)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("am7H0lhDujZyGgsU ""Rx %s ESI %s VNI %u",
...
static int bgp_zebra_process_local_l3vni(ZAPI_CALLBACK_ARGS)
{
	...
	if (cmd == ZEBRA_L3VNI_ADD) {
		...
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("mRln46ZxzxHsQLH1 ""Rx L3VNI ADD VRF %s VNI %u Originator-IP %pI4 RMAC svi-mac %pEA vrr-mac %pEA filter %s svi-if %u",
		...
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("mOpAHBnaCy1Z5IxP ""Rx L3VNI DEL VRF %s VNI %u",
				   vrf_id_to_name(vrf_id), l3vni);
...
static int bgp_zebra_process_local_macip(ZAPI_CALLBACK_ARGS)
{
	...
	if (ipa_len != 0 && ipa_len != IPV4_MAX_BYTELEN
		...
		flog_err(EC_BGP_MACIP_LEN,"wR1qfe0SzLDnbPsk "
...
static int bgp_zebra_process_local_ip_prefix(ZAPI_CALLBACK_ARGS)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("Vb2s5h1IPACpDUtC ""Recv prefix %pFX %s on vrf %s", &p,
...
static int bgp_ifp_create(struct interface *ifp)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("DuIMIfO7WrutYVRG ""Rx Intf add VRF %u IF %s", ifp->vrf->vrf_id,
			   ifp->name);
...
static int bgp_zebra_process_srv6_locator_chunk(ZAPI_CALLBACK_ARGS)
{
	...
	if (strcmp(bgp->srv6_locator_name, chunk->locator_name) != 0) {
		zlog_err("C2pny5JRcFxUuqKZ ""%s: Locator name unmatch %s:%s", __func__,
...
static bool bgp_zebra_label_manager_connect(void)
{
	...
	if (zclient_socket_connect(zclient_sync) < 0) {
		zlog_warn("iqJpqZGPLsp5oyhM ""%s: failed connecting synchronous zclient!",
			  __func__);
		return false;
	}
	...
	if (zclient_send_hello(zclient_sync) == ZCLIENT_SEND_FAILURE) {
		zlog_warn("3BmFUW3OzC5l80JC ""%s: failed sending hello for synchronous zclient!",
			  __func__);
	...
	if (lm_label_manager_connect(zclient_sync, 0) != 0) {
		zlog_warn("n6hsnEOZJk5RyVMc ""%s: failed connecting to label manager!", __func__);
			      ...
			      bool install)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA)) {
		if (pbr)
			zlog_debug("ukP4tYKbrmfqeRon ""%s: table %d (ip rule) %d", __func__,
				   pbra->table_id, install);
		else
			zlog_debug("nFPn8MgoiAFImI0z ""%s: table %d fwmark %d %d", __func__,
...
void bgp_send_pbr_ipset_match(struct bgp_pbr_match *pbrim, bool install)
{
	...
	if (pbrim->install_in_progress)
		return;
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("I50LKwVBqqvtBEdU ""%s: name %s type %d %d, ID %u", __func__,
				    ...
				    bool install)
{
	...
	if (pbrime->install_in_progress)
		return;
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("mA4ob5rzEzMuH5YC ""%s: name %s %d %d, ID %u", __func__,
			  ...
			  bool install)
{
	...
	if (pbm->install_iptable_in_progress)
		return;
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("asIl4IDy58QupcYS ""%s: name %s type %d mark %d %d, ID %u", __func__,
				...
				afi_t afi, uint32_t table_id, bool announce)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA)) {
		...
		zlog_debug("13eukR5c799hBwoa ""%s: %s default route to %pNHvv(%s) table %d",
...
int bgp_zebra_send_capabilities(struct bgp *bgp, bool disable)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("akEMIYtpax2Csc1u ""%s: Sending %sable for %s", __func__,
			   disable ? "dis" : "en", bgp->name_pretty);

	if (zclient == NULL) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("yxFcR89rv3NoUlQ2 ""%s: %s zclient invalid", __func__,
				   bgp->name_pretty);
	...
	if ((zclient->sock < 0) || (zclient->t_connect)) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("maVsZST9JyoH0fHF ""%s: %s client not connected", __func__,
				   bgp->name_pretty);
		return BGP_GR_FAILURE;
	}
	...
	if (zclient_capabilities_send(ZEBRA_CLIENT_CAPABILITIES, zclient, &api)
		...
		zlog_err("7SIfk46tBI6JlhLL ""%s: %s error sending capability", __func__,
			 bgp->name_pretty);
		ret = BGP_GR_FAILURE;
	} else {
		if (disable)
		...
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("LUUlAGNWBejUAvAE ""%s: %s send capabilty success", __func__,
				   bgp->name_pretty);
		ret = BGP_GR_SUCCESS;
	}
	return ret;
		     ...
		     enum zserv_client_capabilities type)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("ilF64irg7mBr2h1Q ""%s: %s afi: %u safi: %u Command %s", __func__,
			   bgp->name_pretty, afi, safi,
			   zserv_gr_client_cap_string(type));

	if (zclient == NULL) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("k5zngXskeS2JkxKJ ""%s: %s zclient == NULL, invalid", __func__,
				   bgp->name_pretty);
		return BGP_GR_FAILURE;
	}
	...
	if ((zclient->sock < 0) || (zclient->t_connect)) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("7kHZUy0BttJhKo6Q ""%s: %s client not connected", __func__,
				   bgp->name_pretty);
		return BGP_GR_FAILURE;
	}
	...
	if (zclient_capabilities_send(ZEBRA_CLIENT_CAPABILITIES, zclient, &api)
	    == ZCLIENT_SEND_FAILURE) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("6Hf0ASPS79Yz3Xbe ""%s: %s error sending capability", __func__,
				   bgp->name_pretty);
		return BGP_GR_FAILURE;
	}
...
int bgp_zebra_stale_timer_update(struct bgp *bgp)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("QBBaWMKF3CuCVCPa ""%s: %s Timer Update to %u", __func__,
			   bgp->name_pretty, bgp->rib_stale_time);

	if (zclient == NULL) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("rf4jt3MTHVF3UJNU ""zclient invalid");
		return BGP_GR_FAILURE;
	}
	...
	if ((zclient->sock < 0) || (zclient->t_connect)) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("CmdEbd9B0FBZkvH7 ""%s: %s client not connected", __func__,
				   bgp->name_pretty);
		return BGP_GR_FAILURE;
	}
	...
	if (zclient_capabilities_send(ZEBRA_CLIENT_CAPABILITIES, zclient, &api)
	    == ZCLIENT_SEND_FAILURE) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("wT8G0c9onD6bcGZX ""%s: %s error sending capability", __func__,
				   bgp->name_pretty);
		return BGP_GR_FAILURE;
	}
				   ...
				   bool label_auto)
{
	...
	ret = lm_get_label_chunk(zclient_sync, 0, base, chunk_size, &start,
				 &end);
	if (ret < 0) {
		zlog_warn("Jp8PWHxa4BLnvtbZ ""%s: error getting label range!", __func__);
		return false;
	}

	if (start > end || start < MPLS_LABEL_UNRESERVED_MIN ||
	    end > MPLS_LABEL_UNRESERVED_MAX) {
		flog_err(EC_BGP_LM_ERROR,"3QUrZ8MqvnL5CfWq " "%s: Invalid Label chunk: %u - %u",
			 __func__, start, end);
		return false;
	}
...
void bgp_zebra_release_label_range(uint32_t start, uint32_t end)
{
	...
	if (ret < 0)
		zlog_warn("czLCxiO63nadhOF7 ""%s: error releasing label range!", __func__);
}
