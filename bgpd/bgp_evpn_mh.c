		...
		struct bgp_path_info *pi, int install)
{
	...
	if (ret) {
		flog_err(
				EC_BGP_EVPN_FAIL,"rRO0LUeDVcL9nYnK "
			     ...
			     struct bgp_path_info **ri, int *route_changed)
{
	...
	*route_changed = 1;
	...
	if (remote_pi) {
		flog_err(
			EC_BGP_ES_INVALID,"z9eHhEC5pjgwOv0W "
		...
		struct bgp_evpn_es *es, struct prefix_evpn *p)
{
	...
	if (ret != 0)
		flog_err(
			EC_BGP_ES_INVALID,"MceE9TaNv0fSWjOz "
		...
		uint32_t addpath_id)
{
	...
	if (psize != BGP_EVPN_TYPE4_V4_PSIZE &&
		...
		flog_err(EC_BGP_EVPN_ROUTE_INVALID,"Q676AmxvOiUXVdhb "
	...
	if (ipaddr_len == IPV4_MAX_BITLEN) {
		...
		flog_err(
				EC_BGP_EVPN_ROUTE_INVALID,"XtQf7VEI6F8VM3xA "
	...
	for (rd_dest = bgp_table_top(bgp->rib[afi][safi]); rd_dest;
		...
		for (dest = bgp_table_top(table); dest;
			...
			for (pi = bgp_dest_get_bgp_path_info(dest); pi;
				...
				if (ret) {
					flog_err(
						EC_BGP_EVPN_FAIL,"4diQLJkZJXY7Qcf8 "
				       ...
				       struct prefix_evpn *p)
{
	...
	if (vpn) {
		...
		if (ret != 0)
			flog_err(
				EC_BGP_ES_INVALID,"3Jcmq0NQeXnsdPdW "
		...
		if (ret != 0) {
			flog_err(
				EC_BGP_ES_INVALID,"sLZBBeuNwo4Mjpn4 "
					 ...
					 struct bgp_evpn_es *es)
{
	...
	for (ALL_LIST_ELEMENTS_RO(es->es_frag_list, node, es_frag)) {
		...
		if (bgp_evpn_type1_route_update(bgp, es, NULL, es_frag, &p))
			flog_err(
				EC_BGP_EVPN_ROUTE_CREATE,"WjtE3Ok1wp7ZlfDb "
					  ...
					  struct prefix_evpn *p)
{
	if (bgp_evpn_type1_route_update(bgp, es, vpn, NULL, p))
		flog_err(EC_BGP_EVPN_ROUTE_CREATE,"8Zi7uPYjcTOyokQU "
		...
		struct bgp_evpn_es *es)
{
	...
	for (ALL_LIST_ELEMENTS_RO(es->es_evi_list, evi_node, es_evi)) {
		...
		if (bgp_evpn_mh_route_delete(bgp, es, es_evi->vpn, NULL, &p))
			flog_err(EC_BGP_EVPN_ROUTE_CREATE,"0lop4MO7Ts3jDV1h "
		...
		uint32_t addpath_id)
{
	...
	if (psize != BGP_EVPN_TYPE1_PSIZE) {
		flog_err(EC_BGP_EVPN_ROUTE_INVALID,"cXhvCFATc5maElTb "
				      ...
				      struct ecommunity *ecomcfg, bool del)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_RT))
		zlog_debug("X0Bpjw2blJo4n99a ""local ES del/re-add EAD route on export RT change");
	/*
			      ...
			      bool add)
{
	...
	if (!zclient || zclient->sock < 0) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("1fGao2f8OCF65JJJ ""%s: No zclient or zclient->sock exists",
				   __func__);
	...
	if (!IS_BGP_INST_KNOWN_TO_ZEBRA(bgp)) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("qRTiMMxLwxKrr0Yy ""No zebra instance, not installing remote es %s",
					es->esi_str);
		return ZCLIENT_SEND_SUCCESS;
	}
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("1gYURNJ4gSwids2M ""Tx %s Remote ESI %s VTEP %pI4", add ? "ADD" : "DEL",
	...
	struct bgp *bgp, struct bgp_evpn_es_vtep *es_vtep, bool param_change)
{
	...
	if ((old_active != new_active) || (new_active && param_change)) {
		...
		if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
			zlog_debug("6OrqqGTgoh9skxvU ""es %s vtep %pI4 %s df %u/%u",
		     ...
		     uint16_t df_pref, int *zret)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("J6T14hEJf14gxLab ""es %s vtep %pI4 add %s df %u/%u",
			   es_vtep->es->esi_str, &es_vtep->vtep_ip,
			...
			bool esr)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("IpcOuZmVP0u6fVPt ""es %s vtep %pI4 del %s", es_vtep->es->esi_str,
...
static void bgp_evpn_path_es_unlink(struct bgp_path_es_info *es_info)
{
	...
	pi = es_info->pi;
	if (BGP_DEBUG(evpn_mh, EVPN_MH_RT))
		zlog_debug("oegyV6y75iespkwB ""vni %u path %pFX unlinked from es %s", es_info->vni,
...
void bgp_evpn_path_es_link(struct bgp_path_info *pi, vni_t vni, esi_t *esi)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_RT))
		zlog_debug("pvbuYYpzsIQTFERy ""vni %u path %pFX linked to es %s", vni, &pi->net->rn->p,
			   es->esi_str);
				      ...
				      const char *reason)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_RT))
		zlog_debug("Yn56zM1939PsDBtW ""update paths linked to es %s on es-vrf %s %s",
...
static void bgp_evpn_es_frag_free(struct bgp_evpn_es_frag *es_frag)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("o0U41fdLTaMmw3xR ""es %s frag %u free", es->esi_str, es_frag->rd_id);
...
static struct bgp_evpn_es_frag *bgp_evpn_es_frag_new(struct bgp_evpn_es *es)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("k0dTAuUGKFBuGhSL ""es %s frag %u new", es->esi_str, es_frag->rd_id);
	return es_frag;
}
...
static void bgp_evpn_es_frag_evi_add(struct bgp_evpn_es_evi *es_evi)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("L5eDnKTLsQbBdHIH ""es %s vni %d linked to frag %u", es->esi_str,
				     ...
				     bool send_ead_del_if_empty)
{
	...
	es_evi->es_frag = NULL;
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("0fchWPoNwGP7eajw ""es %s vni %d unlinked from frag %u", es->esi_str,
			   es_evi->vpn->vni, es_frag->rd_id);
	...
	if (send_ead_del_if_empty && !listcount(es_frag->es_evi_frag_list)) {
		...
		if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
			zlog_debug("c99e3Q68lrKH2GeJ ""es %s frag %u ead-es route delete",
				   es->esi_str, es_frag->rd_id);
		build_evpn_type1_prefix(&p, BGP_EVPN_AD_ES_ETH_TAG, &es->esi,
					es->originator_ip);
		p.prefix.ead_addr.frag_id = es_frag->rd_id;
...
static void bgp_evpn_es_free(struct bgp_evpn_es *es, const char *caller)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("Nabq11cX6kOVwgo9 ""%s: es %s free", caller, es->esi_str);
...
static void bgp_evpn_mac_update_on_es_oper_chg(struct bgp_evpn_es *es)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("lETbfaouZhxjGyCG ""update paths linked to es %s on oper chg",
			   es->esi_str);
						...
						bool is_local)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("PFJLNjQ4OI7ddVGy ""update paths linked to es %s on chg to %s",
					 ...
					 struct bgp_evpn_es *es)
{
	...
	if (ret) {
		flog_err(EC_BGP_EVPN_ROUTE_DELETE,"Mb8pcu1czbOMwKf6 "
...
static void bgp_evpn_local_es_down(struct bgp *bgp, struct bgp_evpn_es *es)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("pn8HaTcunGantrO5 ""local es %s down", es->esi_str);
				       ...
				       bool regen_ead, bool regen_esr)
{
	...
	if (regen_esr) {
		if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
			zlog_debug("2orr6cHfkyXB1wVq ""local es %s generate ESR", es->esi_str);
		/* generate ESR */
		build_evpn_type4_prefix(&p, &es->esi, es->originator_ip);
		if (bgp_evpn_type4_route_update(bgp, es, &p))
			flog_err(EC_BGP_EVPN_ROUTE_CREATE,"Dzz8WfLhmzlv5Igd "
				 "%u: Type4 route creation failure for ESI %s",
				 bgp->vrf_id, es->esi_str);
	}
	...
	if (regen_ead) {
		if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
			zlog_debug("Yqi5xPRyfoSuReqB ""local es %s generate EAD", es->esi_str);
		/* generate EAD-EVI */
				 ...
				 bool regen_esr)
{
	...
	if (!CHECK_FLAG(es->flags, BGP_EVPNES_OPER_UP)) {
		if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
			zlog_debug("t6qXbdgL8BGz4QUC ""local es %s up", es->esi_str);
					    ...
					    struct bgp_evpn_es *es, bool bypass)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("0cajiBlcUaHw9vtv ""local es %s bypass %s", es->esi_str,
			   bypass ? "set" : "clear");
...
static void bgp_evpn_local_es_do_del(struct bgp *bgp, struct bgp_evpn_es *es)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("AYKN3fJYOO40zMTG ""del local es %s", es->esi_str);
...
int bgp_evpn_local_es_del(struct bgp *bgp, esi_t *esi)
{
	...
	es = bgp_evpn_es_find(esi);
	if (!es) {
		flog_warn(EC_BGP_EVPN_ESI,"l35AimgnWwwsor1f " "%u: ES missing at local ES DEL",
			  bgp->vrf_id);
		return -1;
	}
			  ...
			  uint16_t df_pref, bool bypass)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("xn5yVmozUxYiH3ms ""add local es %s orig-ip %pI4 df_pref %u %s",
					      ...
					      bool v4_nhg)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("tUGOXF2mnAIBl76V ""es %s vrf %u %s nhg %u to zebra", es->esi_str,
	...
	for (ALL_LIST_ELEMENTS_RO(es->es_vtep_list, node, es_vtep)) {
		...
		++api_nhg.nexthop_num;
		if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
			zlog_debug("DuTwptdVW7T3ePSO ""nhg %u vtep %pI4 l3-svi %d", api_nhg.id,
				   &es_vtep->vtep_ip,
					      ...
					      bool v4_nhg)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("HAFZrWBk2BUd1kjb ""es %s vrf %u %s nhg %u to zebra",
...
static void bgp_evpn_l3nhg_deactivate(struct bgp_evpn_es_vrf *es_vrf)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("SZEGGCaSgRgCEAXR ""es %s vrf %u nhg %u de-activate",
			   es_vrf->es->esi_str, es_vrf->bgp_vrf->vrf_id,
			   es_vrf->nhg_id);
...
static void bgp_evpn_l3nhg_activate(struct bgp_evpn_es_vrf *es_vrf, bool update)
{
	...
	if (es_vrf->flags & BGP_EVPNES_VRF_NHG_ACTIVE) {
		...
		if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
			zlog_debug("IQOTcGgibkml0XkO ""es %s vrf %u nhg %u activate",
				   es_vrf->es->esi_str, es_vrf->bgp_vrf->vrf_id,
				   es_vrf->nhg_id);
		es_vrf->flags |= BGP_EVPNES_VRF_NHG_ACTIVE;
		/* MAC-IPs can now be installed via the L3NHG */
...
static void bgp_evpn_l3nhg_update_on_vtep_chg(struct bgp_evpn_es *es)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("lTKwQyUD7dOz76WL ""es %s nhg update on vtep chg", es->esi_str);
						      ...
						      struct bgp *bgp_vrf)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("ZjNPMkctroeNSO0r ""es %s vrf %u nhg %u v6_nhg %d create", es->esi_str,
...
static void bgp_evpn_es_vrf_delete(struct bgp_evpn_es_vrf *es_vrf)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("MT9yfG1xA5XdnLEN ""es %s vrf %u nhg %u delete", es->esi_str,
...
void bgp_evpn_es_vrf_deref(struct bgp_evpn_es_evi *es_evi)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("MFJQF6zQw669x8B4 ""es-evi %s vni %u vrf %u de-ref",
...
void bgp_evpn_es_vrf_ref(struct bgp_evpn_es_evi *es_evi, struct bgp *bgp_vrf)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("VaH9QVixqodC5dt7 ""es-evi %s vni %u vrf %u ref", es_evi->es->esi_str,
...
void bgp_evpn_es_evi_vrf_deref(struct bgpevpn *vpn)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("MHVgwrbENWscVtbp ""es-vrf de-ref for vni %u", vpn->vni);
...
void bgp_evpn_es_evi_vrf_ref(struct bgpevpn *vpn)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("IyBzyhk2WGQczYZe ""es-vrf ref for vni %u", vpn->vni);
				    ...
				    struct bgp_evpn_es_evi_vtep *evi_vtep)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("aUNSOUEnX89d1aOF ""es %s evi %u vtep %pI4 %s",
			 ...
			 struct in_addr vtep_ip, bool ead_es)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("xsUsT7FVhFthsWuK ""add es %s evi %u vtep %pI4 %s",
			   evi_vtep->es_evi->es->esi_str,
			   evi_vtep->es_evi->vpn->vni, &evi_vtep->vtep_ip,
			 ...
			 struct in_addr vtep_ip, bool ead_es)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("GLgByQSjvSVw1gF4 ""del es %s evi %u vtep %pI4 %s",
			   evi_vtep->es_evi->es->esi_str,
			   evi_vtep->es_evi->vpn->vni, &evi_vtep->vtep_ip,
			   ead_es ? "ead_es" : "ead_evi");
...
bgp_evpn_local_es_evi_do_del(struct bgp_evpn_es_evi *es_evi)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("iEKjFbofIDUd8YoF ""del local es %s evi %u",
				es_evi->es->esi_str,
				es_evi->vpn->vni);
	...
	if (bgp) {
		...
		if (CHECK_FLAG(es->flags, BGP_EVPNES_ADV_EVI)) {
			...
			if (bgp_evpn_ead_evi_route_delete(bgp, es, es_evi->vpn,
							  &p))
				flog_err(EC_BGP_EVPN_ROUTE_DELETE,"JXAiHggChtceyLym "
...
int bgp_evpn_local_es_evi_del(struct bgp *bgp, esi_t *esi, vni_t vni)
{
	...
	if (!es) {
		flog_err(
				EC_BGP_ES_CREATE,"jRVKTZCeQDRqqIvb "
	...
	if (!vpn) {
		flog_err(
				EC_BGP_ES_CREATE,"c3DLVd3NiZ4zgzng "
	...
	if (!es_evi) {
		flog_err(
				EC_BGP_ES_CREATE,"IemI0e7xDOyhdFVB "
...
int bgp_evpn_local_es_evi_add(struct bgp *bgp, esi_t *esi, vni_t vni)
{
	...
	if (!es) {
		flog_err(
				EC_BGP_ES_CREATE,"HabQfGDw8g6Hedyb "
	...
	if (!vpn) {
		flog_err(
				EC_BGP_ES_CREATE,"WHh3BruVjCSsLBBE "
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("4XcbVCW5zHYKLADR ""add local es %s evi %u",
				es->esi_str, vni);
						    ...
						    const struct prefix_evpn *p)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("HadEc1bUh8TOHhoP ""add remote %s es %s evi %u vtep %pI4",
...
static void bgp_evpn_es_cons_checks_timer_start(void)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("dGPM1Ms1U0NbLSqs ""periodic consistency checking started");
...
static void bgp_evpn_es_evi_set_inconsistent(struct bgp_evpn_es_evi *es_evi)
{
	if (!CHECK_FLAG(es_evi->flags, BGP_EVPNES_EVI_INCONS_VTEP_LIST)) {
		if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
			zlog_debug("BzHxNmJFcwiAZt7S ""inconsistency detected - es %s evi %u vtep list mismatch",
					es_evi->es->esi_str,
					es_evi->vpn->vni);
		...
		if (!es_evi->es->incons_evi_vtep_cnt &&
			...
			zlog_debug("ZBOxu8eyse1z8NbY ""inconsistency detected - es %s vtep list mismatch",
					es_evi->es->esi_str);
...
static void bgp_evpn_nh_zebra_update_send(struct bgp_evpn_nh *nh, bool add)
{
	...
	if (!IS_BGP_INST_KNOWN_TO_ZEBRA(bgp_vrf)) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("oOy34ZfTtUz1VLph ""No zebra instance, not %s remote nh %s",
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES) || bgp_debug_zebra(NULL)) {
		if (add)
			zlog_debug("asfqceSmXxVjwdjI ""evpn %s nh %s rmac %pEA add to zebra",
				   nh->bgp_vrf->name_pretty, nh->nh_str,
				   &nh->rmac);
		else
			zlog_debug("fI0WaFQCIh9BAokU ""evpn %s nh %s del to zebra",
				   nh->bgp_vrf->name_pretty, nh->nh_str);
	}
					   ...
					   struct bgp_path_info *pi)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("3Zf63fTRTSnIGTt1 ""evpn vrf %s nh %s rmac %pEA add",
...
static void bgp_evpn_nh_del(struct bgp_evpn_nh *n)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("TrvS9dhc3pHtbnBe ""evpn vrf %s nh %s del to zebra",
...
{
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("0v9t9XNTXyBIBp6g ""evpn vrf %s nh init", bgp_vrf->name_pretty);
...
static void bgp_evpn_nh_flush_entry(struct bgp_evpn_nh *nh)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("0fynKECGRsHVRPLL ""evpn vrf %s nh %s flush", nh->bgp_vrf->name_pretty,
			   nh->nh_str);
...
{
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("8nyVtmJx4QZOdeQl ""evpn vrf %s nh finish", bgp_vrf->name_pretty);
	hash_iterate(
...
static void bgp_evpn_nh_update_ref_pi(struct bgp_evpn_nh *nh)
{
	...
	for (ALL_LIST_ELEMENTS_RO(nh->pi_list, node, nh_info)) {
		...
		if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
			zlog_debug("Hf8GT7GT3j8tQsI5 ""evpn %s nh %s ref_pi update",
				   nh->bgp_vrf->name_pretty, nh->nh_str);
		nh->ref_pi = pi;
				     ...
				     struct bgp_path_info *pi)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("2pwedGfNTEq850tx ""evpn vrf %s nh %s ref_pi clear",
			   nh->bgp_vrf->name_pretty, nh->nh_str);
	nh->ref_pi = NULL;
...
static void bgp_evpn_path_nh_unlink(struct bgp_path_evpn_nh_info *nh_info)
{
	...
	pi = nh_info->pi;
	if (BGP_DEBUG(evpn_mh, EVPN_MH_RT))
		zlog_debug("mWtAH1sGqR1vSeqU ""path %s unlinked from %s nh %s pathcount %u",
...
static void bgp_evpn_path_nh_link(struct bgp *bgp_vrf, struct bgp_path_info *pi)
{
	...
	if (!bgp_vrf->evpn_nh_table) {
		if (BGP_DEBUG(evpn_mh, EVPN_MH_RT))
			zlog_debug("0KKjW9Q0biA51ESk ""path %pFX linked to %s failed",
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_RT))
		zlog_debug("SNGvfFUGqud6CrOi ""path %pFX linked to %s nh %s pathcount %u",
...
void bgp_evpn_mh_finish(void)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_RT))
		zlog_debug("7CPKemukaydvx55s ""evpn mh finish");
