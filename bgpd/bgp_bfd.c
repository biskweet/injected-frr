				      ...
				      void *arg)
{
	...
	if (BGP_DEBUG(bfd, BFD_LIB))
		zlog_debug("17bK5DG9hOLnLaUA ""%s: neighbor %s vrf %s(%u) bfd state %s -> %s",
...
void bgp_peer_bfd_update_source(struct peer *p)
{
	...
	if (family == AF_INET) {
		if ((source && source->sin.sin_addr.s_addr != src.v4.s_addr) ||
			...
			if (BGP_DEBUG(bfd, BFD_LIB))
				zlog_debug("qjA9LpUWEq2seZzw ""%s: address [%pI4->%pI4] to [%pI4->%pI4]",
		...
		if ((source && memcmp(&source->sin6, &src.v6, sizeof(src.v6))) ||
			...
			if (BGP_DEBUG(bfd, BFD_LIB))
				zlog_debug("LgguINknhk582ixf ""%s: address [%pI6->%pI6] to [%pI6->%pI6]",
	...
	if (p->nexthop.ifp && bfd_sess_interface(session) == NULL) {
		if (BGP_DEBUG(bfd, BFD_LIB))
			zlog_debug("4EGBk8uWMxbBb1GM ""%s: interface none to %s", __func__,
				   p->nexthop.ifp->name);
	...
	if (!PEER_IS_MULTIHOP(p) && bfd_sess_hop_count(session) > 1) {
		if (BGP_DEBUG(bfd, BFD_LIB))
			zlog_debug("A53XGn5BGepy3ON6 ""%s: TTL %d to 1", __func__,
	...
	if (PEER_IS_MULTIHOP(p) && p->ttl != bfd_sess_hop_count(session)) {
		if (BGP_DEBUG(bfd, BFD_LIB))
			zlog_debug("xTzy4rXhpQKQ2MVZ ""%s: TTL %d to %d", __func__,
