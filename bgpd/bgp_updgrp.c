...
#define SEED2 2147483647
	...
	if (bgp_debug_neighbor_events(peer)) {
		zlog_debug("lpJZ3Yk1SpCHfqVV ""%pBP Update Group Hash: sort: %d sub_sort: %d UpdGrpFlags: %ju UpdGrpAFFlags: %ju",
		...
		zlog_debug("AgC1blEAX4JPTBxC ""%pBP Update Group Hash: addpath: %u UpdGrpCapFlag: %ju UpdGrpCapAFFlag: %u route_adv: %u change local as: %u, as_path_loop_detection: %d",
			   ...
			   !!CHECK_FLAG(peer->flags,
					PEER_FLAG_AS_LOOP_DETECTION));
		zlog_debug("IYabAxd8lIP6N0Cz ""%pBP Update Group Hash: addpath paths-limit: (send %u, receive %u)",
			   peer, peer->addpath_paths_limit[afi][safi].send,
			   peer->addpath_paths_limit[afi][safi].receive);
		zlog_debug("wHBJUWfG49JAFGyJ "
		...
		zlog_debug("U9hlGG56ndeeGKpi ""%pBP Update Group Hash: Lonesoul: %d ORF prefix: %u max prefix out: %ju",
			   peer, !!CHECK_FLAG(peer->flags, PEER_FLAG_LONESOUL),
			   CHECK_FLAG(peer->af_cap[afi][safi],
				      PEER_CAP_ORF_PREFIX_SM_RCV),
			   (intmax_t)CHECK_FLAG(peer->af_flags[afi][safi],
						PEER_FLAG_MAX_PREFIX_OUT));
		zlog_debug("vHe7ENdn31eT8NiT "
			...
			peer->soo[afi][safi]
				...
				: "(NONE)");
		zlog_debug("2sFdxlN5YoKh7vth ""%pBP Update Group Hash key: %u", peer, key);
	}
	return key;
}
...
static struct update_group *update_group_create(struct peer_af *paf)
{
	...
	if (BGP_DEBUG(update_groups, UPDATE_GROUPS))
		zlog_debug("9ahPUdnpuUSpQQiU ""create update group %" PRIu64, updgrp->id);
...
{
	if (BGP_DEBUG(update_groups, UPDATE_GROUPS))
		zlog_debug("q9GselfBvHEUdPuh ""delete update group %" PRIu64, updgrp->id);
...
update_subgroup_create(struct update_group *updgrp)
{
	...
	if (BGP_DEBUG(update_groups, UPDATE_GROUPS))
		zlog_debug("iD0eJ30tc45hHOXi ""create subgroup u%" PRIu64 ":s%" PRIu64, updgrp->id,
			   subgrp->id);
...
static void update_subgroup_delete(struct update_subgroup *subgrp)
{
	...
	if (BGP_DEBUG(update_groups, UPDATE_GROUPS) && subgrp->update_group)
		zlog_debug("oU5siSIFYnfTSDJl ""delete subgroup u%" PRIu64 ":s%" PRIu64,
				     ...
				     int send_enqueued_pkts)
{
	...
	if (BGP_DEBUG(update_groups, UPDATE_GROUPS))
		zlog_debug("tCkHJzLrRfE8Z1oo ""peer %s added to subgroup s%" PRIu64,
						 ...
						 struct peer_af *paf)
{
	...
	if (BGP_DEBUG(update_groups, UPDATE_GROUPS))
		zlog_debug("H8coKyunUSG223NQ ""peer %s deleted from subgroup s%"
			   PRIu64 " peer cnt %d",
				  ...
				  const char *reason)
{
	...
	if (BGP_DEBUG(update_groups, UPDATE_GROUPS))
		zlog_debug("elrL60VubL12NKL0 ""u%" PRIu64 ":s%" PRIu64" (%d peers) merged into u%" PRIu64 ":s%" PRIu64", trigger: %s",
				...
				struct update_group *updgrp)
{
	...
	if (old_subgrp->peer_count == 1) {
		...
		if (BGP_DEBUG(update_groups, UPDATE_GROUPS))
			zlog_debug("plE4IK4xXUPfLQQN ""u%" PRIu64 ":s%" PRIu64" peer %s moved to u%" PRIu64 ":s%" PRIu64,
	...
	if (BGP_DEBUG(update_groups, UPDATE_GROUPS))
		zlog_debug("VufOaj1hH7b5I18x ""u%" PRIu64 ":s%" PRIu64" peer %s split and moved into u%" PRIu64":s%" PRIu64,
...
void update_group_adjust_peer(struct peer_af *paf)
{
	...
	update_subgroup_add_peer(subgrp, paf, 1);
	if (BGP_DEBUG(update_groups, UPDATE_GROUPS))
		zlog_debug("23OthF6GwJXsfRiK ""u%" PRIu64 ":s%" PRIu64 " add peer %s", updgrp->id,
...
void peer_af_announce_route(struct peer_af *paf, int combine)
{
	...
	if (!combine || !all_pending) {
		...
		if (bgp_debug_update(paf->peer, NULL, subgrp->update_group, 0))
			zlog_debug("mINs1gnrBiJo86QU ""u%" PRIu64 ":s%" PRIu64" %s announcing routes",
				   subgrp->update_group->id, subgrp->id,
				   paf->peer->host);
	...
	if (bgp_debug_update(paf->peer, NULL, subgrp->update_group, 0))
		zlog_debug("kRN3lsuEP3NvKigz ""u%" PRIu64 ":s%" PRIu64" announcing routes to %s, combined into %d peers",
			   subgrp->update_group->id, subgrp->id,
