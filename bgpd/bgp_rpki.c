...
#define RPKI_DEBUG(...)                                                        \
	if (rpki_debug_conf || rpki_debug_term) {                              \
		zlog_debug("Sf0xYEIU7oqwFjuf ""RPKI: " __VA_ARGS__);                              \
	}
...
static void bgpd_sync_callback(struct event *thread)
{
	...
	if (rpki_vrf->vrfname) {
		...
		if (!vrf) {
			zlog_err("db36eeXwUvQWbzoV ""%s(): vrf for rpki %s not found", __func__,
				 rpki_vrf->vrfname);
			return;
		}
	}
...
static void revalidate_all_routes(struct rpki_vrf *rpki_vrf)
{
	...
	if (rpki_vrf->vrfname) {
		vrf = vrf_lookup_by_name(rpki_vrf->vrfname);
		if (!vrf) {
			zlog_err("PKlL2soV4RbtSTtA ""%s(): vrf for rpki %s not found", __func__,
				 rpki_vrf->vrfname);
			return;
		}
	}
				    ...
				    const bool added __attribute__((unused)))
{
	...
	return;
err:
	zlog_err("VGGdDmlfscqxL7E6 ""RPKI: %s", msg);
}
...
err:
	zlog_err("rF5DFI60XQfEbpLa ""RPKI: %s", msg);
	abort();
...
static int rpki_create_socket(void *_cache)
{
	...
	if (ret != 0) {
		flog_err_sys(EC_LIB_SOCKET,"j0Qf3YrzL2c03Kli " "getaddrinfo: %s",
			     gai_strerror(ret));
		return -1;
	}
	...
	ret = getsockopt(socket, SOL_SOCKET, SO_RCVTIMEO, &prev_rcv_tmout,
			 &optlen);
	if (ret < 0)
		zlog_warn("Ooe0yR9emeOrthpU ""%s: failed to getsockopt SO_RCVTIMEO for socket %d",
			  __func__, socket);
	ret = getsockopt(socket, SOL_SOCKET, SO_SNDTIMEO, &prev_snd_tmout,
			 &optlen);
	if (ret < 0)
		zlog_warn("SL6b5Zmio3DcP8ce ""%s: failed to getsockopt SO_SNDTIMEO for socket %d",
			  __func__, socket);
	ret = setsockopt(socket, SOL_SOCKET, SO_RCVTIMEO, &timeout,
			 sizeof(timeout));
	if (ret < 0)
		zlog_warn("I2ig5Qo7H1Un5A0J ""%s: failed to setsockopt SO_RCVTIMEO for socket %d",
			  __func__, socket);

	ret = setsockopt(socket, SOL_SOCKET, SO_SNDTIMEO, &timeout,
			 sizeof(timeout));
	if (ret < 0)
		zlog_warn("28dE68NYm8QYhGnl ""%s: failed to setsockopt SO_SNDTIMEO for socket %d",
			  __func__, socket);
	...
	if (ret < 0)
		zlog_warn("Q8SzRahp8U1pDvSi ""%s: failed to setsockopt SO_RCVTIMEO for socket %d",
			  __func__, socket);

	ret = setsockopt(socket, SOL_SOCKET, SO_SNDTIMEO, &prev_snd_tmout,
			 sizeof(prev_snd_tmout));
	if (ret < 0)
		zlog_warn("bCPjwg4Ll6a3degl ""%s: failed to setsockopt SO_SNDTIMEO for socket %d",
			  __func__, socket);
