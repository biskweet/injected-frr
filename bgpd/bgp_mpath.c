				...
				struct bgp_maxpaths_cfg *mpath_cfg)
{
	...
	if (debug)
		zlog_debug("ZQA5fcUwMPPqksAb ""%pBD(%s): starting mpath update, newbest %s num candidates %d old-mpath-count %d old-cum-bw %" PRIu64,
	...
	while (mp_node || cur_mpath) {
		...
		if (debug)
			zlog_debug("MtP238pBdDEz6mA4 ""%pBD(%s): comparing candidate %s with existing mpath %s",
				   dest, bgp->name_pretty,
			...
			if ((mpath_count < maxpaths)
				...
				if (debug) {
					bgp_path_info_path_with_addpath_rx_str(
						cur_mpath, path_buf,
						sizeof(path_buf));
					zlog_debug("uTLkBAMNmru8IxJW ""%pBD: %s is still multipath, cur count %d",
				...
				if (debug) {
					bgp_path_info_path_with_addpath_rx_str(
						cur_mpath, path_buf,
						sizeof(path_buf));
					zlog_debug("89XgfDNg8AHrnu0f ""%pBD: remove mpath %s nexthop %pI4, cur count %d",
						   dest, path_buf,
		...
		if (cur_mpath
			...
			if (debug) {
				...
				zlog_debug("1AQI3Y8V1zQEX6yy ""%pBD: remove mpath %s nexthop %pI4, cur count %d",
					   dest, path_buf,
					   &cur_mpath->attr->nexthop,
					   mpath_count);
			}
			...
			if ((mpath_count < maxpaths) && (new_mpath != new_best)
				...
				if (debug) {
					bgp_path_info_path_with_addpath_rx_str(
						new_mpath, path_buf,
						sizeof(path_buf));
					zlog_debug("qzoDDgmh5F3EMZSn ""%pBD: add mpath %s nexthop %pI4, cur count %d",
						   dest, path_buf,
	...
	if (new_best) {
		...
		if (debug)
			zlog_debug("J2kPNQueTlVGq80v ""%pBD(%s): New mpath count (incl newbest) %d mpath-change %s all_paths_lb %d cum_bw %" PRIu64,
