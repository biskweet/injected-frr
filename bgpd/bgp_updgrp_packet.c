					 ...
					 struct peer_af *paf)
{
	...
	if (nhafi == AFI_IP) {
		...
		default:
			...
			flog_warn(
				EC_BGP_INVALID_NEXTHOP_LENGTH,"smXMrdxAojVjROoe "
		...
		if (bgp_debug_update(peer, NULL, NULL, 0))
			zlog_debug("jexlh3CtXf7P73P8 ""u%" PRIu64 ":s%" PRIu64
		...
		default:
			...
			flog_warn(
				EC_BGP_INVALID_NEXTHOP_LENGTH,"4Q849wa5s7JzIaIU "
		...
		if (bgp_debug_update(peer, NULL, NULL, 0))
			zlog_debug("RjkMMRpHjWqU6LEc ""u%" PRIu64 ":s%" PRIu64
...
struct bpacket *subgroup_update_packet(struct update_subgroup *subgrp)
{
	...
	while (adv) {
		...
		if (stream_empty(s)) {
			...
			if (space_remaining < space_needed) {
				flog_err(
					EC_BGP_UPDGRP_ATTR_LEN,"ZHP3RvDlybJNpy5W "
		...
		if (bgp_debug_update(NULL, dest_p, subgrp->update_group, 0)) {
			...
			if (!send_attr_printed) {
				zlog_debug("LwR3ZDKAUTgNPhbu ""u%" PRIu64 ":s%" PRIu64" send UPDATE w/ attr: %s",
					   subgrp->update_group->id, subgrp->id,
					   send_attr_str);
				if (!stream_empty(snlri)) {
					iana_afi_t pkt_afi;
			...
			zlog_debug("P9YwwoYB6lDunYMU ""u%" PRIu64 ":s%" PRIu64 " send UPDATE %s",
				   subgrp->update_group->id, subgrp->id,
				   pfx_buf);
		}
...
struct bpacket *subgroup_withdraw_packet(struct update_subgroup *subgrp)
{
	...
	while ((adv = bgp_adv_fifo_first(&subgrp->sync->withdraw)) != NULL) {
		...
		if (bgp_debug_update(NULL, dest_p, subgrp->update_group, 0)) {
			...
			zlog_debug("3VvOEzT0f970Zc6Q ""u%" PRIu64 ":s%" PRIu64" send UPDATE %s -- unreachable",
				   subgrp->update_group->id, subgrp->id,
				   pfx_buf);
		}
	...
	if (!stream_empty(s)) {
		...
		if (bgp_debug_update(NULL, NULL, subgrp->update_group, 0))
			zlog_debug("NazNE39nYxE9wvcP ""u%" PRIu64 ":s%" PRIu64" send UPDATE (withdraw) len %zd numpfx %d",
				   subgrp->update_group->id, subgrp->id,
				    ...
				    struct attr *attr, struct peer *from)
{
	...
	if (bgp_debug_update(NULL, &p, subgrp->update_group, 0)) {
		...
		zlog_debug("iV4GpTPOg6gxCMbQ ""u%" PRIu64 ":s%" PRIu64 " send UPDATE %pFX%s %s",
...
void subgroup_default_withdraw_packet(struct update_subgroup *subgrp)
{
	...
	if (bgp_debug_update(NULL, &p, subgrp->update_group, 0)) {
		...
		zlog_debug("kCOvY0rrwCBnkfx7 ""u%" PRIu64 ":s%" PRIu64
