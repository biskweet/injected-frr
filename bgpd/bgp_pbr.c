...
static int bgp_pbr_validate_policy_route(struct bgp_pbr_entry_main *api)
{
	...
	if (api->type ==  BGP_PBR_UNDEFINED) {
		if (BGP_DEBUG(pbr, PBR))
			zlog_debug("kvOpCtZqJHnTxphu ""BGP: pbr entry undefined. cancel.");
		return 0;
	}
	...
	if (api->match_protocol_num > 1) {
		if (BGP_DEBUG(pbr, PBR))
			zlog_debug("75VtngTlSJbG5nRw ""BGP: match protocol operations:multiple protocols ( %d). ignoring.",
	...
	if (api->src_prefix_offset > 0 ||
		...
		if (BGP_DEBUG(pbr, PBR))
			zlog_debug("pj45V96GQgKuYjMs ""BGP: match prefix offset:"
	...
	if (api->match_protocol_num == 1 &&
		...
		if (BGP_DEBUG(pbr, PBR))
			zlog_debug("zTp5eQwnKcVdqafX ""BGP: match protocol operations:protocol (%d) not supported. ignoring",
				   api->match_protocol_num);
		return 0;
	}
	if (!bgp_pbr_extract(api->src_port, api->match_src_port_num, NULL)) {
		if (BGP_DEBUG(pbr, PBR))
			zlog_debug("STyKr2Kc2f4H9FDN ""BGP: match src port operations:too complex. ignoring.");
		return 0;
	}
	if (!bgp_pbr_extract(api->dst_port, api->match_dst_port_num, NULL)) {
		if (BGP_DEBUG(pbr, PBR))
			zlog_debug("EoNEOiIcmqVR2VOt ""BGP: match dst port operations:too complex. ignoring.");
		return 0;
	}
	if (!bgp_pbr_extract_enumerate(api->tcpflags,
				       ...
				       FLOWSPEC_TCP_FLAGS)) {
		if (BGP_DEBUG(pbr, PBR))
			zlog_debug("B9Hzc2YbTmRBFpRH ""BGP: match tcp flags:too complex. ignoring.");
		return 0;
	}
	if (!bgp_pbr_extract(api->icmp_type, api->match_icmp_type_num, NULL)) {
		if (!bgp_pbr_extract_enumerate(api->icmp_type,
					       ...
					       FLOWSPEC_ICMP_TYPE)) {
			if (BGP_DEBUG(pbr, PBR))
				zlog_debug("EqJ82oWacGRCn5j9 ""BGP: match icmp type operations:too complex. ignoring.");
			return 0;
		}
		enumerate_icmp = true;
	}
	if (!bgp_pbr_extract(api->icmp_code, api->match_icmp_code_num, NULL)) {
		if (!bgp_pbr_extract_enumerate(api->icmp_code,
					       ...
					       FLOWSPEC_ICMP_CODE)) {
			if (BGP_DEBUG(pbr, PBR))
				zlog_debug("61JzkTuUxPVB4IpW ""BGP: match icmp code operations:too complex. ignoring.");
			return 0;
		} else if (api->match_icmp_type_num > 1 &&
			   !enumerate_icmp) {
			if (BGP_DEBUG(pbr, PBR))
				zlog_debug("ouNou8fnbEGYH3FG ""BGP: match icmp code is enumerate, and icmp type is not. too complex. ignoring.");
			return 0;
		}
	}
	if (!bgp_pbr_extract(api->port, api->match_port_num, NULL)) {
		if (BGP_DEBUG(pbr, PBR))
			zlog_debug("u14EZpFyLKcYOPvG ""BGP: match port operations:too complex. ignoring.");
		return 0;
	}
	if (api->match_packet_length_num) {
		...
		if (!ret) {
			if (BGP_DEBUG(pbr, PBR))
				zlog_debug("LACTT2nh6G1wqQza ""BGP: match packet length operations:too complex. ignoring.");
			return 0;
		}
	}
	if (api->match_dscp_num) {
		if (!bgp_pbr_extract_enumerate(api->dscp, api->match_dscp_num,
				OPERATOR_UNARY_OR | OPERATOR_UNARY_AND,
					       NULL, FLOWSPEC_DSCP)) {
			if (BGP_DEBUG(pbr, PBR))
				zlog_debug("5a6ztUq7M8bgkElN ""BGP: match DSCP operations:too complex. ignoring.");
			return 0;
		}
	}
	if (api->match_flowlabel_num) {
		if (api->afi == AFI_IP) {
			if (BGP_DEBUG(pbr, PBR))
				zlog_debug("j6PfcuX6zOMLPfcF ""BGP: match Flow Label operations:"
					   "Not for IPv4.");
			return 0;
		}
		if (!bgp_pbr_extract_enumerate(api->flow_label,
			...
			if (BGP_DEBUG(pbr, PBR))
				zlog_debug("prVtJZ5pg2MgZClY ""BGP: match FlowLabel operations:"
					   "too complex. ignoring.");
			return 0;
		}
		if (BGP_DEBUG(pbr, PBR))
			zlog_debug("xKubJAFW3lQgqcsg ""BGP: match FlowLabel operations "
	...
	if (api->match_fragment_num) {
		...
		if (!success) {
			if (BGP_DEBUG(pbr, PBR))
				zlog_debug("1KHMS6ZuZCqXdtWY ""BGP: match fragment operation (%d) %s",
					   api->match_fragment_num,
					   fail_str);
			return 0;
		}
	...
	if (api->match_src_port_num + api->match_dst_port_num +
		...
		if (BGP_DEBUG(pbr, PBR))
			zlog_debug("cKycb24wKqK2FC1h ""BGP: match multiple port operations: too complex. ignoring.");
		return 0;
	}
	if ((api->match_src_port_num || api->match_dst_port_num
	     || api->match_port_num) && (api->match_icmp_type_num
					 || api->match_icmp_code_num)) {
		if (BGP_DEBUG(pbr, PBR))
			zlog_debug("010bKbRhzzKLjDEu ""BGP: match multiple port/imcp operations: too complex. ignoring.");
		return 0;
	}
	...
	if (api->type == BGP_PBR_IPRULE) {
		...
		for (i = 0; i < api->action_num; i++) {
			if (api->actions[i].action == ACTION_TRAFFICRATE &&
				...
				if (BGP_DEBUG(pbr, PBR)) {
					...
					zlog_debug("qBFgFD77nnPBzAG5 ""BGP: iprule match actions drop not supported");
				}
				return 0;
			}
			if (api->actions[i].action == ACTION_MARKING) {
				if (BGP_DEBUG(pbr, PBR)) {
					bgp_pbr_print_policy_route(api);
					zlog_warn("43R88DDPzG6v2BzO ""PBR: iprule set DSCP/Flow Label %u not supported",
			...
			if (api->actions[i].action == ACTION_REDIRECT) {
				if (BGP_DEBUG(pbr, PBR)) {
					bgp_pbr_print_policy_route(api);
					zlog_warn("Hrubwq8BKL7JDdn8 ""PBR: iprule redirect VRF %u not supported",
		...
		if (BGP_DEBUG(pbr, PBR)) {
			bgp_pbr_print_policy_route(api);
			zlog_debug("RdKDp2S5xJLj9PiX ""BGP: match actions without src or dst address can not operate. ignoring.");
		}
		return 0;
	}
	return 1;
				     ...
				     struct bgp_pbr_entry_main *api)
{
		...
		for (i = 0; i < ecom->size; i++) {
			...
			if (action_count > ACTIONS_MAX_NUM) {
				if (BGP_DEBUG(pbr, PBR_ERROR))
					flog_err(
						EC_BGP_FLOWSPEC_PACKET,"bkI2ofEgpFaHv2AZ "
						"%s: %s (max %u)",
						__func__,
	...
	if (path && path->attr && bgp_attr_get_ipv6_ecommunity(path->attr)) {
		...
		for (i = 0; i < ecom->size; i++) {
			...
			if (action_count > ACTIONS_MAX_NUM) {
				if (BGP_DEBUG(pbr, PBR_ERROR))
					flog_err(
						EC_BGP_FLOWSPEC_PACKET,"clz0mkNBUerIpli0 "
	...
	if (api->match_bitmask & PREFIX_DST_PRESENT) {
		...
		if (valid_prefix && afi != family2afi(dst->family)) {
			if (BGP_DEBUG(pbr, PBR)) {
				bgp_pbr_print_policy_route(api);
				zlog_debug("gwDJlLjRvHTq9U2M ""%s: inconsistency: no match for afi src and dst (%u/%u)",
...
void bgp_pbr_print_policy_route(struct bgp_pbr_entry_main *api)
{
	...
	zlog_info("gZDrv72ZrqwKNVir ""%s", return_string);
}
...
static void bgp_pbr_dump_entry(struct bgp_pbr_filter *bpf, bool add)
{
	...
	zlog_debug("sKfu94lclN5q6118 ""BGP: %s FS PBR from %s to %s, %s %s",
		  add ? "adding" : "removing",
		  bpf->src == NULL ? "<all>" :
		...
		if (bpr_found) {
			...
			if (extra && extra->flowspec &&
			    listnode_lookup_nocheck(extra->flowspec->bgp_fs_iprule,
						    bpr)) {
				if (BGP_DEBUG(pbr, PBR_ERROR))
					zlog_err("hOzrnzwJvtrJY3EB ""%s: entry %p/%p already installed in bgp pbr iprule",
						 __func__, path, bpr);
				return;
			}
		}
	...
	for (i = 0; i < api->action_num; i++) {
		...
		case ACTION_TRAFFICRATE:
			...
			if (api->actions[i].u.r.rate == 0) {
				...
				if (BGP_DEBUG(pbr, PBR)) {
					bgp_pbr_print_policy_route(api);
					zlog_warn("rYYpKm89PrbuLBCN ""PBR: ignoring Set action rate %f",
						  api->actions[i].u.r.rate);
				}
			}
			break;
		case ACTION_TRAFFIC_ACTION:
			if (api->actions[i].u.za.filter
				...
				if (BGP_DEBUG(pbr, PBR)) {
					bgp_pbr_print_policy_route(api);
					zlog_warn("nDkJhVIRBWzkPWIu ""PBR: Sample action Ignored");
				}
			}
		...
		case ACTION_MARKING:
			if (BGP_DEBUG(pbr, PBR)) {
				bgp_pbr_print_policy_route(api);
				zlog_warn("gAwoZ8gEoZpFGQjj ""PBR: Set DSCP/FlowLabel %u Ignored",
					  api->actions[i].u.marking_dscp);
			}
			break;
		default:
			  ...
			  bool nlri_update)
{
	...
	if (!bgp_zebra_tm_chunk_obtained()) {
		if (BGP_DEBUG(pbr, PBR_ERROR))
			flog_err(EC_BGP_TABLE_CHUNK,"6SbDCzdrE7e24nXi "
	...
	if (bgp_pbr_build_and_validate_entry(p, info, &api) < 0) {
		if (BGP_DEBUG(pbr, PBR_ERROR))
			flog_err(EC_BGP_FLOWSPEC_INSTALLATION,"vv0k4yn7PhOILrSm "
