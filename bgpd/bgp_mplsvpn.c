...
#define VPN_PREFIXLEN_MIN_BYTES (3 + 8) /* label + RD */
	while (STREAM_READABLE(data) > 0) {
		...
		if (STREAM_READABLE(data) < 1) {
			flog_err(
				EC_BGP_UPDATE_RCV,"z8S3lyxODgRPNjpd "
		...
		if (prefixlen < VPN_PREFIXLEN_MIN_BYTES * 8) {
			flog_err(
				EC_BGP_UPDATE_RCV,"ztSIxc5NUmvNkAhE "
		...
		if (STREAM_READABLE(data) < psize) {
			flog_err(
				EC_BGP_UPDATE_RCV,"l1CpncalB7eKkzkU "
		...
		if ((psize - VPN_PREFIXLEN_MIN_BYTES) > (ssize_t)sizeof(p.u)) {
			flog_err(
				EC_BGP_UPDATE_RCV,"EoUocIRQkD6WpY3S "
		...
		if ((psize - VPN_PREFIXLEN_MIN_BYTES) > prefix_blen(&p)) {
			flog_err(
				EC_BGP_UPDATE_RCV,"nFXzYFW6FhzYabRj "
		...
		if (STREAM_READABLE(data) < BGP_LABEL_BYTES) {
			flog_err(
				EC_BGP_UPDATE_RCV,"1cVVkaJ2AWCXrEyl "
		...
		if (STREAM_READABLE(data) < 8) {
			flog_err(
				EC_BGP_UPDATE_RCV,"8VKTGvRvzwBWrocL "
		...
		default:
			flog_err(EC_BGP_UPDATE_RCV,"wFLNx2BRSma8k05z " "Unknown RD type %d", type);
			break; /* just report */
		}
	...
	if (STREAM_READABLE(data) != 0) {
		flog_err(
			EC_BGP_UPDATE_RCV,"bYoBCJSt5cmyYeWt "
...
stream_failure:
	flog_err(
		EC_BGP_UPDATE_RCV,"RxYi3abvI0rTXOhx "
...
void vpn_leak_zebra_vrf_label_update(struct bgp *bgp, afi_t afi)
{
	...
	if (debug) {
		zlog_debug("zx07qArGi4MUyfTz ""%s: vrf %s: afi %s: setting label %d for vrf id %d",
...
void vpn_leak_zebra_vrf_label_withdraw(struct bgp *bgp, afi_t afi)
{
	...
	if (debug) {
		zlog_debug("F60o2vQG9FWxVjuQ ""%s: deleting label for vrf %s (id=%d)", __func__,
...
void vpn_leak_zebra_vrf_sid_update_per_af(struct bgp *bgp, afi_t afi)
{
	...
	if (bgp->vrf_id == VRF_UNKNOWN) {
		if (debug)
			zlog_debug("g2pMkfh3ICxawDdV ""%s: vrf %s: afi %s: vrf_id not set, can't set zebra vrf label",
				   __func__, bgp->name_pretty, afi2str(afi));
		return;
	}
	...
	if (!tovpn_sid) {
		if (debug)
			zlog_debug("ZUWOQ9bKz12RkSHy ""%s: vrf %s: afi %s: sid not set", __func__,
				   bgp->name_pretty, afi2str(afi));
		return;
	}

	if (debug)
		zlog_debug("yfjMn2gqC9K1DSMw ""%s: vrf %s: afi %s: setting sid %pI6 for vrf id %d",
...
void vpn_leak_zebra_vrf_sid_update_per_vrf(struct bgp *bgp)
{
	...
	if (!tovpn_sid) {
		if (debug)
			zlog_debug("sOmDhiTeiTTw73yh ""%s: vrf %s: sid not set", __func__,
				   bgp->name_pretty);
		return;
	}

	if (debug)
		zlog_debug("FH0WXukIK8lAzaSJ ""%s: vrf %s: setting sid %pI6 for vrf id %d",
...
void vpn_leak_zebra_vrf_sid_update(struct bgp *bgp, afi_t afi)
{
	...
	if (debug)
		zlog_debug("fyS1IQ0fxKOFjRl6 ""%s: vrf %s: afi %s: sid not set", __func__,
			   bgp->name_pretty, afi2str(afi));
}
...
void vpn_leak_zebra_vrf_sid_withdraw_per_af(struct bgp *bgp, afi_t afi)
{
	...
	if (bgp->vrf_id == VRF_UNKNOWN) {
		if (debug)
			zlog_debug("BbFSJCMyO2AmOKLe ""%s: vrf %s: afi %s: vrf_id not set, can't set zebra vrf label",
				   __func__, bgp->name_pretty, afi2str(afi));
		return;
	}

	if (debug)
		zlog_debug("mqHTpfZSFGXh38hM ""%s: deleting sid for vrf %s afi (id=%d)", __func__,
			   bgp->name_pretty, bgp->vrf_id);
...
void vpn_leak_zebra_vrf_sid_withdraw_per_vrf(struct bgp *bgp)
{
	...
	if (debug)
		zlog_debug("Zj4Ierqew1wj6Gy9 ""%s: deleting sid for vrf %s (id=%d)", __func__,
			   bgp->name_pretty, bgp->vrf_id);
	...
	bool allocated)
{
	...
	if (debug)
		zlog_debug("8NFXwmrE6QgFUlOL ""%s: label=%u, allocated=%d",
	...
	if (vp->tovpn_label != MPLS_LABEL_NONE) {
		...
		flog_err(EC_BGP_LABEL,"y54ECu8lC3V8YoVh "
				 ...
				 afi_t afi)
{
	...
	if (debug)
		zlog_debug("vM59ox5MK6SyxWhU ""%s: try to allocate new SID for vrf %s: afi %s",
	...
	if (tovpn_sid_index != 0 && tovpn_sid_auto) {
		zlog_err("BcMFarP0RcTcjlm5 ""%s: index-mode and auto-mode both selected. ignored.",
			 __func__);
		return;
	}
	...
	if (debug)
		zlog_debug("pqZwSwOIVlX0IDX9 ""%s: new sid %pI6 allocated for vrf %s: afi %s",
...
void ensure_vrf_tovpn_sid_per_vrf(struct bgp *bgp_vpn, struct bgp *bgp_vrf)
{
	...
	if (debug)
		zlog_debug("z3586jqGq0Oc7e53 ""%s: try to allocate new SID for vrf %s", __func__,
			   bgp_vrf->name_pretty);
	...
	/* check invalid case both configured index and auto */
	if (tovpn_sid_index != 0 && tovpn_sid_auto) {
		zlog_err("htHcRJnKMEsSoKth ""%s: index-mode and auto-mode both selected. ignored.",
			 __func__);
		return;
	}
	...
	if (tovpn_sid_transpose_label == 0) {
		if (debug)
			zlog_debug("lXj88mSAt5bgMXRr ""%s: not allocated new sid for vrf %s",
	...
	if (debug)
		zlog_debug("Du4b5VpLmAZOWO0h ""%s: new sid %pI6 allocated for vrf %s", __func__,
				 ...
				 afi_t afi)
{
	...
	if (debug)
		zlog_debug("IOUjow4zX6tDZf2N ""%s: try to remove SID for vrf %s: afi %s", __func__,
...
void delete_vrf_tovpn_sid_per_vrf(struct bgp *bgp_vpn, struct bgp *bgp_vrf)
{
	...
	if (debug)
		zlog_debug("I9OVGjFBr0WmVdVU ""%s: try to remove SID for vrf %s", __func__,
			   bgp_vrf->name_pretty);
				      ...
				      const struct prefix *p, int debug)
{
	...
	if (debug)
		zlog_debug("zHfpiJcQWJPWCn0G ""%s: %pFX nexthop is %svalid (in %s)", __func__, p,
	    ...
	    struct prefix *nexthop_orig, int nexthop_self_flag, int debug)
{
	...
	if (bpi) {
		...
		if (debug)
			zlog_debug("dFEMFz4CQT1lUOv8 ""%s: ->%s: %pBD Found route, changed attr",
				   __func__, to_bgp->name_pretty, bn);
	...
	if (debug)
		zlog_debug("uzMK3y7eZFSPVk1c ""%s: ->%s: %pBD: Added new route", __func__,
						...
						void *context, bool allocated)
{
	...
	if (debug)
		zlog_debug("hLNy5JE2kQINlFNM ""%s: label=%u, allocated=%d, nexthop=%pFX", __func__,
			      ...
			      struct bgp_path_info *path_vrf) /* route */
{
	...
	if (debug)
		zlog_debug("ijUFwmXI9dnh3eLs ""%s: from vrf %s", __func__, from_bgp->name_pretty);
	...
	if (debug && bgp_attr_get_ecommunity(path_vrf->attr)) {
		...
		zlog_debug("zIFGzfbW1CFSlGE3 ""%s: %s path_vrf->type=%d, EC{%s}", __func__,
	...
	if (!afi) {
		if (debug)
			zlog_debug("utPQ5FomuxmBoTgM ""%s: can't get afi of prefix", __func__);
		return;
	}
	...
	if (!vpn_leak_to_vpn_active(from_bgp, afi, &debugmsg, false)) {
		if (debug)
			zlog_debug("78w2V4i66ZwupnJd ""%s: %s skipping: %s", __func__,
	...
	if (debug && bgp_attr_get_ecommunity(&static_attr)) {
		...
		zlog_debug("kp8CIH1EPawAVlNM ""%s: post route map static_attr.ecommunity{%s}",
			   __func__, s);
		XFREE(MTYPE_ECOMMUNITY_STR, s);
	}
	...
	if (from_bgp->vpn_policy[afi].rmap[BGP_VPN_POLICY_DIR_TOVPN]) {
		...
		if (RMAP_DENYMATCH == ret) {
			...
			if (debug)
				zlog_debug("ABbTHkySlk82bPAv ""%s: vrf %s route map \"%s\" says DENY, returning",
	...
	if (!ecommunity_has_route_target(new_ecom)) {
		...
		if (debug)
			zlog_debug("kDaBC2tEAjTWL0wc ""%s: %s skipping: waiting for a valid export rt list.",
				   __func__, from_bgp->name_pretty);
		return;
	}
	...
	if (debug && bgp_attr_get_ecommunity(&static_attr)) {
		...
		zlog_debug("WJHaI8EWsSz4PxNA ""%s: post merge static_attr.ecommunity{%s}",
			   __func__, s);
		XFREE(MTYPE_ECOMMUNITY_STR, s);
	}
	...
	if (debug && bgp_attr_get_ecommunity(new_attr)) {
		...
		zlog_debug("RUeZ3733uS3Si54p ""%s: new_attr->ecommunity{%s}", __func__, s);
		XFREE(MTYPE_ECOMMUNITY_STR, s);
	}
				...
				struct bgp_path_info *path_vrf) /* route */
{
	...
	if (!afi) {
		if (debug)
			zlog_debug("zg4fEmJbVhvZQOpV ""%s: can't get afi of prefix", __func__);
		return;
	}
	...
	if (!vpn_leak_to_vpn_active(from_bgp, afi, &debugmsg, true)) {
		if (debug)
			zlog_debug("6y5y9oVE2uzIBSOd ""%s: skipping: %s", __func__, debugmsg);
		return;
	}
	...
	if (!bn)
		return;
	if (debug)
		zlog_debug("Va6RtLKGZ1PDqliJ ""%s: withdrawing (path_vrf=%p)", __func__, path_vrf);
				    ...
				    afi_t afi)
{
	...
	for (pdest = bgp_table_top(to_bgp->rib[afi][safi]); pdest;
		...
		for (bn = bgp_table_top(table); bn; bn = bgp_route_next(bn)) {
			...
			if (debug && bpi) {
				zlog_debug("0pdKi6hfV8kdfMUi ""%s: looking at prefix %pBD",
					   __func__, bn);
			}
			...
			for (; (bpi != NULL) && (next = bpi->next, 1);
			     bpi = next) {
				if (debug)
					zlog_debug("IX7WgmbY0NIO00U0 ""%s: type %d, sub_type %d",
						   __func__, bpi->type,
						   bpi->sub_type);
				...
				if ((struct bgp *)bpi->extra->vrfleak->bgp_orig ==
					...
					/* delete route */
					if (debug)
						zlog_debug("XF6wjROpQ4Oec1ts ""%s: deleting it",
							   __func__);
				  ...
				  afi_t afi)
{
	...
	if (debug)
		zlog_debug("zffhYGb3C6WZEILE ""%s: entry, afi=%d, vrf=%s", __func__, afi,
	...
	for (bn = bgp_table_top(from_bgp->rib[afi][SAFI_UNICAST]); bn;
		...
		if (debug)
			zlog_debug("iIW0lINSg2Kglb7z ""%s: node=%p", __func__, bn);

		for (bpi = bgp_dest_get_bgp_path_info(bn); bpi;
		     bpi = bpi->next) {
			if (debug)
				zlog_debug("78KIRCBmsldJB0Qe "
					  ...
					  struct prefix_rd *prd)
{
	...
	if (debug)
		zlog_debug("q5aV3p8rmqJiLv1e ""%s: updating RD %s, %pFX to %s", __func__, rd_buf,
	...
	if (debug)
		zlog_debug("2TtAxlgSHYl4AnyL ""%s: pfx %pBD: num_labels %d", __func__,
			    ...
			    struct prefix_rd *prd)
{
	...
	if (debug)
		zlog_debug("3U4xGbfdPZXb63BA ""%s: start (path_vpn=%p)", __func__, path_vpn);
...
void vpn_leak_to_vrf_withdraw(struct bgp_path_info *path_vpn)
{
	...
	if (debug)
		zlog_debug("vujWXv8Yvuv4fMNH ""%s: entry: p=%pBD, type=%d, sub_type=%d", __func__,
			   path_vpn->net, path_vpn->type, path_vpn->sub_type);

	if (debug)
		zlog_debug("LjvLznIQtQPXwvO2 ""%s: start (path_vpn=%p)", __func__, path_vpn);
	...
	for (ALL_LIST_ELEMENTS(bm->bgp, mnode, mnnode, bgp)) {
		if (!vpn_leak_from_vpn_active(bgp, afi, &debugmsg)) {
			if (debug)
				zlog_debug("gL2vSrYT6z17sKZ3 ""%s: from %s, skipping: %s",
					   __func__, bgp->name_pretty,
					   debugmsg);
			continue;
		}
		...
		if (debug)
			zlog_debug("lfVi72cDnifhchyu ""%s: withdrawing from vrf %s", __func__,
				   bgp->name_pretty);
		...
		if (bpi) {
			if (debug)
				zlog_debug("U6tXDGt3dZBRwxtV ""%s: deleting bpi %p", __func__,
					   bpi);
...
void vpn_leak_to_vrf_withdraw_all(struct bgp *to_bgp, afi_t afi)
{
	...
	if (debug)
		zlog_debug("v1PFqJZw7gtAmEJQ ""%s: entry", __func__);
	/*
	...
	for (afi = 0; afi < AFI_MAX; ++afi) {
		...
		if (bgp->vpn_policy[afi].rmap_name[BGP_VPN_POLICY_DIR_TOVPN]
			...
			vpn_leak_prechange(BGP_VPN_POLICY_DIR_TOVPN, afi,
					   bgp_get_default(), bgp);
			if (debug)
				zlog_debug("1cQ81U7WPH9RAfvr ""%s: after vpn_leak_prechange",
					   __func__);
			...
			if (debug)
				zlog_debug("lrnhZsEPwQTM4pgP ""%s: after vpn_leak_postchange",
					   __func__);
		}
		...
		if (bgp->vpn_policy[afi].rmap_name[BGP_VPN_POLICY_DIR_FROMVPN]
			...
			if (debug) {
				zlog_debug("sgYV5AePcR2VE0mU ""%s: rmap \"%s\" matches vrf-policy fromvpn for as %d afi %s",
					__func__, rmap_name, bgp->as,
					afi2str(afi));
			}
				 ...
				 bool is_config)
{
	...
	if (!is_config) {
		if (debug)
			zlog_debug("mvlmjh4MwIGiO3Xj ""%s: skipping non explicit router-id change",
				   __func__);
		return;
	}
	...
	for (afi = 0; afi < AFI_MAX; ++afi) {
		...
		if (withdraw) {
			...
			if (debug)
				zlog_debug("HGCztgys0DjZwUhm ""%s: %s after to_vpn vpn_leak_prechange",
			...
			vpn_leak_postchange(BGP_VPN_POLICY_DIR_TOVPN,
					    ...
					    bgp);
			if (debug)
				zlog_debug("zxd8ZFY9S0xMSLZM ""%s: %s after to_vpn vpn_leak_postchange",
					   __func__, export_name);
		}
	}
}
...
void vpn_policy_routemap_event(const char *rmap_name)
{
	...
	if (debug)
		zlog_debug("E52GaMLlIu8o5W9F ""%s: entry", __func__);
			   ...
			   afi_t afi, safi_t safi)
{
	...
	if (debug)
		zlog_debug("F8I50kxks8QsOur0 ""%s from %s to %s", __func__, tmp_name, export_name);
...
void bgp_vpn_leak_unimport(struct bgp *from_bgp)
{
	...
	for (afi = 0; afi < AFI_MAX; ++afi) {
		...
		for (ALL_LIST_ELEMENTS_RO(bm->bgp, next, to_bgp)) {
			...
			if (debug)
				zlog_debug("QnZUP01YCjwwVVUK ""%s: unimport routes from %s to_bgp %s afi %s import vrfs count %u",
					   __func__, from_bgp->name_pretty,
					   to_bgp->name_pretty, afi2str(afi),
	...
	for (afi = 0; afi < AFI_MAX; ++afi) {
		...
		for (ALL_LIST_ELEMENTS_RO(bm->bgp, next, to_bgp)) {
			...
			for (ALL_LIST_ELEMENTS_RO(to_vpolicy->import_vrf,
				...
				if (debug)
					zlog_debug("AOaXW2KWyi0KV6Uv ""%s: found from_bgp %s in to_bgp %s import list, import routes.",
					   __func__,
	...
	case NEXTHOP_TYPE_IPV6_IFINDEX:
		...
		if (nh->nh_label) {
			if (nh->nh_label->num_labels + 1 > MPLS_MAX_LABELS) {
				 ...
				 */
				flog_err(EC_BGP_LABEL,"aB6yRGkJQlJpKFYA "
							...
							bool allocated)
{
	...
	if (BGP_DEBUG(labelpool, LABELPOOL))
		zlog_debug("OBJ5PtwskZb0fvUF ""%s: label=%u, allocated=%d, nexthop=%pFX, label %u",
