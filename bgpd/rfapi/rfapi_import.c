			    ...
			    struct agg_node *rn)       /* req for L2 eth addr */
{
	...
	default:
		zlog_warn("IB2i7hZaYdkB27q1 ""%s: invalid vpn nexthop length: %d", __func__,
	...
	for (pEncap = bgp_attr_get_vnc_subtlvs(bpi->attr); pEncap;
		...
		default:
			zlog_warn("NADfDL1KZDNmQBFu ""%s: unknown VNC option type %d", __func__,
				  pEncap->type);
	...
	for (pEncap = bpi->attr->encap_subtlvs; pEncap; pEncap = pEncap->next) {
		...
		default:
			zlog_warn("JjEsa2IBoqpwTv8i ""%s: unknown Encap Attribute option type %d",
	...
	if (!have_vnc_tunnel_un && bpi->extra) {
		...
		case AF_INET6:
			...
			break;
		default:
			zlog_warn("E9E4NizHADIafsM6 ""%s: invalid UN addr family (%d) for bpi %p",
				 ...
				 struct bgp_path_info *vpn_bpi)
{
	if (!vpn_bpi || !vpn_bpi->extra) {
		zlog_warn("T0chORDDde54HE2f ""%s: no vpn  bpi attr/extra, can't copy UN address",
			  __func__);
		return;
	}
	...
	default:
		zlog_warn("0uiuu97JgTp7CQgm ""%s: invalid encap nexthop length: %d", __func__,
				 ...
				 struct bgp_path_info *vpn_bpi)
{
	if (!encap_bpi) {
		...
		if (!vpn_bpi) {
			zlog_warn("C5RcifOPknJnqUEQ ""%s: missing VPN bpi, can't clear UN addr",
				  __func__);
			return 1;
		}
	...
	uint32_t *label)   /* part of bgp_path_info */
{
	...
	case AFI_L2VPN:
	case AFI_MAX:
		flog_err(EC_LIB_DEVELOPMENT,"CX0bGDSUheQbEBe0 " "%s: bad afi %d", __func__, afi);
		return;
	}
	...
	uint32_t *label)   /* part of bgp_path_info */
{
	...
	case AFI_UNSPEC:
	case AFI_MAX:
		flog_err(EC_LIB_DEVELOPMENT,"4g3sbBoKq87hsV8X " "%s: bad afi %d", __func__, afi);
		return;
	}
	...
	if (rn) {
		...
		if (bpi) {
			...
			if (VNC_DEBUG(VERBOSE) && bpi->type != type)
				...
				zlog_warn("S8sZ4gwfELbHRPwQ ""%s: type mismatch! (bpi=%d, arg=%d)",
...
rfapiBgpInfoFilteredImportFunction(safi_t safi)
{
	...
	case SAFI_MAX:
		/* not expected */
		flog_err(EC_LIB_DEVELOPMENT,"ABZmK9T5tg5QAxUE " "%s: bad safi %d", __func__, safi);
