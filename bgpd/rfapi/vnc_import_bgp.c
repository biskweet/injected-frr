	...
	struct bgp_path_info *info)		      /* unicast info */
{
	...
	if (!afi) {
		flog_err(EC_LIB_DEVELOPMENT,"q7P8NzyE2mZ1tIXN " "%s: can't get afi of prefix",
			 __func__);
		return;
	}
						...
						struct bgp_path_info *info)
{
	...
	if (!afi) {
		flog_err(EC_LIB_DEVELOPMENT,"pjjtijIeqk07wpCb " "%s: can't get afi of prefix",
			 __func__);
		return;
	}
	...
	struct bgp_path_info *info, struct rfapi_nve_group_cfg *rfg)
{
	...
	if (!afi) {
		flog_err(EC_LIB_DEVELOPMENT,"eozIIHbVNGoUpFsv " "%s: can't get afi of prefix",
			 __func__);
		return;
	}
			      ...
			      struct bgp_path_info *info)
{
	...
	if (!afi) {
		flog_err(EC_LIB_DEVELOPMENT,"4f46tYyA7vwhaQnY " "%s: can't get afi of prefix",
			 __func__);
		return;
	}
