				 ...
				 struct bgp_path_info *bpi)
{
	...
	if (!afi) {
		flog_err(EC_LIB_DEVELOPMENT,"xwk3dYOrr7b9Ap1K " "%s: can't get afi of route node",
			 __func__);
		return;
	}
				 ...
				 struct bgp_path_info *bpi)
{
	...
	if (!afi) {
		flog_err(EC_LIB_DEVELOPMENT,"z84Bdm9TEmRGwVB3 " "%s: bad afi", __func__);
		return;
	}
			       ...
			       struct agg_node *rn)
{
	...
	if (!afi) {
		flog_err(EC_LIB_DEVELOPMENT,"Y6szyYr6jrrBiwp5 " "%s: can't get afi of route node",
			 __func__);
		return;
	}
			       ...
			       struct agg_node *rn)
{
	...
	if (!afi) {
		flog_err(EC_LIB_DEVELOPMENT,"oXsziYMca7sWCPZP " "%s: can't get afi route node",
			 __func__);
		return;
	}
...
void vnc_direct_bgp_add_nve(struct bgp *bgp, struct rfapi_descriptor *rfd)
{
	...
	if (!afi) {
		flog_err(EC_LIB_DEVELOPMENT,"Q31sNAUckRE45f5u " "%s: can't get afi of nve vn addr",
			 __func__);
		return;
	}
	...
	for (ALL_LIST_ELEMENTS(bgp->rfapi_cfg->rfg_export_direct_bgp_l, node,
		...
		if (rfgn->rfg == rfg) {
			...
			if (afi == AFI_IP || afi == AFI_IP6) {
				...
				flog_err(EC_LIB_DEVELOPMENT,"bffDVwbE5Ltkgy8j " "%s: bad afi %d",
					 __func__, afi);
				return;
			}
...
void vnc_direct_bgp_del_nve(struct bgp *bgp, struct rfapi_descriptor *rfd)
{
	...
	if (!afi) {
		flog_err(EC_LIB_DEVELOPMENT,"D63cItzUUIuCBCUS " "%s: can't get afi of nve vn addr",
			 __func__);
		return;
	}
	...
	for (ALL_LIST_ELEMENTS(bgp->rfapi_cfg->rfg_export_direct_bgp_l, node,
		...
		if (rfg && rfgn->rfg == rfg) {
			...
			if (afi == AFI_IP || afi == AFI_IP6) {
				...
				flog_err(EC_LIB_DEVELOPMENT,"LrKMCqMsWTn1zARa " "%s: bad afi %d",
					 __func__, afi);
				return;
			}
					 ...
					 afi_t afi)
{
	...
	if (afi == AFI_IP || afi == AFI_IP6) {
		...
		flog_err(EC_LIB_DEVELOPMENT,"XBCBDhG67lAc5GnI " "%s: bad afi %d", __func__, afi);
		return;
	}
				 ...
				 struct attr *attr)
{
	...
	if (!afi) {
		flog_err(EC_LIB_DEVELOPMENT,"LwGaLdxVMRdzKcjQ " "%s: can't get afi of route node",
			 __func__);
		return;
	}
				 ...
				 const struct prefix *prefix, struct peer *peer)
{
	...
	if (!afi) {
		flog_err(EC_LIB_DEVELOPMENT,"pqlHWmS8zbbeMIJT " "%s: can't get afi route node",
			 __func__);
		return;
	}
