	       ...
	       rfapi_handle *pHandle)
{
	...
	*pHandle = NULL;
	...
	if (!rfg) {
		...
		{
			char buf[2][INET_ADDRSTRLEN];
			zlog_notice("FXxfoyjlv0oO1hzh ""%s: no matching group VN=%s UN=%s",
				    __func__,
...
void rfapi_delete(struct bgp *bgp)
{
...
#if DEBUG_CLEANUP
	zlog_debug("CsFb5dFzBM82C3Zo ""%s: bgp %p", __func__, bgp);
#endif
	...
	if (h && h->descriptors.count) {
		...
		struct rfapi_descriptor *rfd;
#if DEBUG_CLEANUP
		zlog_debug("8T65jM5JgvEFUPXg ""%s: descriptor count %u", __func__,
			   h->descriptors.count);
#endif
		for (ALL_LIST_ELEMENTS(&h->descriptors, node, nnode, rfd)) {
#if DEBUG_CLEANUP
			zlog_debug("ugFZzJVZRWzw008Q ""%s: closing rfd %p", __func__, rfd);
#endif
					  ...
					  struct vty *vty, uint32_t size)
{
	...
	case RFAPI_RFP_CFG_GROUP_L2:
		...
		break;
	default:
		flog_err(EC_LIB_DEVELOPMENT,"N8qhaEuzDVh1tTwI " "%s: Unknown group type=%d",
			 __func__, type);
		/* should never happen */
	...
	void *criteria, rfp_group_config_search_cb_t *search_cb)
{
	...
	case RFAPI_RFP_CFG_GROUP_L2:
		...
		break;
	default:
		flog_err(EC_LIB_DEVELOPMENT,"mxRAlG9Sg8qmWMBc " "%s: Unknown group type=%d",
			 __func__, type);
		/* should never happen */
		assert("Unknown type" == NULL);
		break;
