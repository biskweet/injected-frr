				     ...
				     int add) /* !0 = add, 0 = del */
{
	...
	if (p->family != AF_INET && p->family != AF_INET6) {
		flog_err(EC_LIB_DEVELOPMENT,"VuZla9b6yvzstQyk "
				  ...
				  int add) /* 0 = del, !0 = add */
{
	...
	if (afi != AFI_IP && afi != AFI_IP6) {
		flog_err(EC_LIB_DEVELOPMENT,"0AH6dZ2xrA3cPSoY " "%s: invalid vn addr family",
			 __func__);
		return;
	}
					...
					afi_t afi, int add)
{
	...
	if (afi == AFI_IP || afi == AFI_IP6) {
		...
		flog_err(EC_LIB_DEVELOPMENT,"bMeA8EA8JL2q75Kd " "%s: bad afi %d", __func__, afi);
		return;
	}

	if (!family) {
		flog_err(EC_LIB_DEVELOPMENT,"h8NVs6uC3et25wAi " "%s: computed bad family: %d",
			 __func__, family);
		return;
	}
