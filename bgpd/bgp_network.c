...
int bgp_set_socket_ttl(struct peer_connection *connection)
{
	...
	if (!peer->gtsm_hops) {
		...
		if (ret) {
			flog_err(
				EC_LIB_SOCKET,"AMAdL5OwFQtBMmqj "
		...
		*/
		...
		if (ret) {
			flog_err(
				EC_LIB_SOCKET,"E8qxw4N8eGlpOzqB "
				"%s: Can't set TxTTL on peer (rtrid %pI4) socket, err = %d",
				__func__, &peer->remote_id, errno);
			return ret;
		}
		...
		if (ret) {
			flog_err(
				EC_LIB_SOCKET,"s7RUfS9bw0b9DPer "
...
static int bgp_get_instance_for_inc_conn(int sock, struct bgp **bgp_inst)
{
	...
	*bgp_inst = NULL;
	...
	if (rc != 0) {
		...
		flog_err(EC_LIB_SOCKET,"x7ythMLhHpOuBnlY "
...
static void bgp_accept(struct event *thread)
{
	...
	if (accept_sock < 0) {
		flog_err_sys(EC_LIB_SOCKET,"AqPglQInbhko1jfE "
	...
	if (bgp_sock < 0) {
		if (save_errno == EINVAL) {
			 ...
			 */
			flog_err_sys(
				EC_LIB_SOCKET,"1zlZnaqJL7StGGms "
			...
			flog_err_sys(
				EC_LIB_SOCKET,"AGkqLSxYOGhfZRJU "
	...
	if (!peer1) {
		...
		if (peer1) {
			...
			if (bgp_set_socket_ttl(connection1) < 0) {
				...
				zlog_err("8elpBC8cu8BN6XXP ""%s: Unable to set min/max TTL on peer %s (dynamic), error received: %s(%d)",
					 __func__, peer1->host,
	...
	if (connection1->status == Clearing || connection1->status == Deleted) {
		if (bgp_debug_neighbor_events(peer1))
			zlog_debug("Bj7vA75Mey324ROc ""[Event] Closing incoming conn for %s (%p) state %d",
				   peer1->host, peer1,
	...
	if (bgp_debug_neighbor_events(peer1))
		zlog_debug("sIidcuvKilRvZ1sE ""[Event] connection from %s fd %d, active peer status %d fd %d",
	...
	if (bgp_set_socket_ttl(connection) < 0)
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("smfIh55dlAWM0yqI ""[Event] Unable to set min/max TTL on peer %s, Continuing",
				   peer->host);
...
int bgp_connect(struct peer_connection *connection)
{
	...
	if (peer->conf_if && BGP_CONNECTION_SU_UNSPEC(connection)) {
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("OC42ezWvVhKFFZ2Y ""Peer address not learnt: Returning from connect");
		return 0;
	}
	frr_with_privs(&bgpd_privs) {
	...
	if (connection->fd < 0) {
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("iqZvyeLeavG1qJA3 ""%s: Failure to create socket for connection to %s, error received: %s(%d)",
	...
	if (bgp_set_socket_ttl(peer->connection) < 0) {
		peer->last_reset = PEER_DOWN_SOCKET_ERROR;
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("3PIOeRTywIxCpkfp ""%s: Failure to set socket ttl for connection to %s, error received: %s(%d)",
				   __func__, peer->host, safe_strerror(errno),
				   errno);
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("nUQLiMvmQyeInjCe ""%s [Event] Connect start to %s fd %d", peer->host,
...
int bgp_getsockname(struct peer *peer)
{
	...
	if (!bgp_zebra_nexthop_set(peer->su_local, peer->su_remote,
				   &peer->nexthop, peer)) {
		flog_err(
			EC_BGP_NH_UPD,"umGIotfGglq8GHu8 "
			...
			struct bgp *bgp)
{
	...
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"yrznKs19PbotUvIT " "bind: %s", safe_strerror(en));
		return ret;
	}

	ret = listen(sock, SOMAXCONN);
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"0VX9CtJxqJud05oJ " "listen: %s", safe_strerror(errno));
		return ret;
	}
...
int bgp_socket(struct bgp *bgp, unsigned short port, const char *address)
{
	...
	if (ret != 0) {
		flog_err_sys(EC_LIB_SOCKET,"kJ4EyAy0XUe0W5s9 " "getaddrinfo: %s",
			     gai_strerror(ret));
		return -1;
	}
	...
	for (ainfo = ainfo_save; ainfo; ainfo = ainfo->ai_next) {
		...
		if (sock < 0) {
			flog_err_sys(EC_LIB_SOCKET,"v4DI7K4mv32JjlSl " "socket: %s",
				     safe_strerror(errno));
			continue;
		}
	...
	if (count == 0 && bgp->inst_type != BGP_INSTANCE_TYPE_VRF) {
		flog_err(
			EC_LIB_SOCKET,"QGKm1AgZ2Ybjhaw1 "
			"%s: no usable addresses please check other programs usage of specified port %d",
			__func__, port);
		flog_err_sys(EC_LIB_SOCKET,"bwErfeiTjvVuvYdR " "%s: Program cannot continue",
			     __func__);
		exit(-1);
	}
