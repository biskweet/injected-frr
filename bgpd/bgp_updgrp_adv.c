...
static int group_announce_route_walkcb(struct update_group *updgrp, void *arg)
{
	...
	if (BGP_DEBUG(update, UPDATE_OUT))
		zlog_debug("D4v7D1JE2zuiMPBd ""%s: afi=%s, safi=%s, p=%pBD", __func__,
...
static void subgroup_coalesce_timer(struct event *thread)
{
	...
	if (bgp_debug_update(NULL, NULL, subgrp->update_group, 0))
		zlog_debug("TCWSZSCN1HAbvkAH ""u%" PRIu64 ":s%" PRIu64" announcing routes upon coalesce timer expiry(%u ms)",
			      ...
			      struct bgp_path_info *path)
{
	...
	if (!CHECK_FLAG(subgrp->sflags, SUBGRP_STATUS_FORCE_UPDATES) &&
		...
		if (BGP_DEBUG(update, UPDATE_OUT)) {
			...
			zlog_debug("e9aSrFIahpkS34Qd ""%s suppress UPDATE w/ attr: %s", peer->host,
				   attr_str);
		}
...
void subgroup_announce_all(struct update_subgroup *subgrp)
{
	...
	if (!subgrp->v_coalesce) {
		if (bgp_debug_update(NULL, NULL, subgrp->update_group, 0))
			zlog_debug("9kc5yxcFsoxyS4Jv ""u%" PRIu64 ":s%" PRIu64" announcing all routes",
