...
static uint16_t bgp_read(struct peer_connection *connection, int *code_p)
{
	...
	if (nbytes < 0 && ERRNO_IO_RETRY(errno)) {
		...
		flog_err(EC_BGP_UPDATE_RCV,"7CXgJQNZjV0w1ave "
		...
		if (bgp_debug_neighbor_events(connection->peer))
			zlog_debug("ZSQ8S2IpYeLD3qWZ ""%s [Event] BGP connection closed fd %d",
...
static bool validate_header(struct peer_connection *connection)
{
	...
	if (type != BGP_MSG_OPEN && type != BGP_MSG_UPDATE
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("qetT16mF7wHC7qOs ""%s unknown message type 0x%02x", peer->host,
				   type);
	...
	if ((size < BGP_HEADER_SIZE) || (size > peer->max_packet_size)
		...
		if (bgp_debug_neighbor_events(peer)) {
			zlog_debug("ZVONBUHPFmKW5Xtq ""%s bad message length - %d for %s",
				   peer->host, size,
