...
struct peer *peer_and_group_lookup_vty(struct vty *vty, const char *peer_str)
{
	...
	zlog_warn("1OfHegrXERmesiQA ""Specify remote-as or peer-group commands first before: %s",
		  vty->buf);
      ...
      )
{
	...
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("IhmWjMw4DwZHYpZO ""[BGP_GR] bgp_graceful_restart_cmd : START ");
	...
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("50jvLftnGhks0HIa ""[BGP_GR] bgp_graceful_restart_cmd : END ");
      ...
      )
{
	...
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("2QzrDN2Z3DgDdEkf ""[BGP_GR] no_bgp_graceful_restart_cmd : START ");
	...
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("WxEonW2kE86y4Vzs ""[BGP_GR] no_bgp_graceful_restart_cmd : END ");
	...
	GR_DISABLE)
{
	...
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("uPp40YKdeXAGMa1k ""[BGP_GR] bgp_graceful_restart_disable_cmd : END ");
       ...
       "Automatically assign a label\n")
{
	...
	if (sid_auto) {
		...
		if (debug)
			zlog_debug("4rH8QZWkPGKsrI7v ""%s: auto sid alloc.", __func__);
		SET_FLAG(bgp->vpn_policy[afi].flags,
			 BGP_VPN_POLICY_TOVPN_SID_AUTO);
	} else if (sid_idx != 0) {
		/* SID allocation index-mode */
		if (debug)
			zlog_debug("Qg1jUSdVBSuj5hk4 ""%s: idx %ld sid alloc.", __func__, sid_idx);
       ...
       "Automatically assign a label\n")
{
	...
	if (sid_auto) {
		/* SID allocation auto-mode */
		if (debug)
			zlog_debug("lBSeMUcyGud0HIfF ""%s: auto per-vrf sid alloc.", __func__);
		SET_FLAG(bgp->vrf_flags, BGP_VRF_TOVPN_SID_AUTO);
	} else if (sid_idx != 0) {
		/* SID allocation index-mode */
		if (debug)
			zlog_debug("jvtOzmSwMXbCJwDY ""%s: idx %ld per-vrf sid alloc.", __func__,
				   sid_idx);
	...
	for (ALL_LIST_ELEMENTS(bm->bgp, node, nnode, bgp)) {
		...
		if (use_json) {
			if (!(json = json_object_new_object())) {
				flog_err(
					EC_BGP_JSON_MEM_ERROR,"6IJoBFceFdEquQNR "
...
{
	zlog_err("LOZ1pM0EdyzbB9jd ""BGP configuration end timer expired after %d seconds.",
