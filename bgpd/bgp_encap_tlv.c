...
{
	if (subtlv->length < 4) {
		zlog_debug("p4bixo0L61qvMELp ""%s, subtlv length %d is less than 4", __func__,
			   subtlv->length);
		return -1;
	}
	...
	if (st->cookie_length > sizeof(st->cookie)) {
		zlog_debug("BNye9fKRuxJX8l6S ""%s, subtlv length %d is greater than %d", __func__,
...
{
	if (subtlv->length != 4) {
		zlog_debug("1wUfRncuSuUB25kJ ""%s, subtlv length %d does not equal 4", __func__,
			   subtlv->length);
		return -1;
	}
				   ...
				   struct bgp_tea_subtlv_encap_pbb *st)
{
	if (subtlv->length != 1 + 3 + 6 + 2) {
		zlog_debug("EIAXvSjm9BIYD67l ""%s, subtlv length %d does not equal %d", __func__,
...
{
	if (subtlv->length != 2) {
		zlog_debug("2n6SyTC6nagy1M1N ""%s, subtlv length %d does not equal 2", __func__,
			   subtlv->length);
		return -1;
	}
...
{
	if (subtlv->length != 8) {
		zlog_debug("mwY5Mb7agZxK6xtI ""%s, subtlv length %d does not equal 8", __func__,
			   subtlv->length);
		return -1;
	}
	if ((subtlv->value[0] != 0x03) || (subtlv->value[1] != 0x0b)
	    || (subtlv->value[2] != 0) || (subtlv->value[3] != 0)) {
		zlog_debug("T92jUsEuT45fiv5e ""%s, subtlv value 1st 4 bytes are not 0x030b0000",
			   __func__);
		return -1;
	}
			      ...
			      struct bgp_tea_subtlv_remote_endpoint *st)
{
	...
	if (subtlv->length != 8 && subtlv->length != 20) {
		zlog_debug("dPc5a0otwgqffFc6 ""%s, subtlv length %d does not equal 8 or 20",
	...
	struct bgp_encap_type_l2tpv3_over_ip *bet) /* caller-allocated */
{
	...
	for (st = stlv; st; st = st->next) {
		...
		default:
			zlog_debug("DZmDrB5Nc5IWAnow ""%s: unexpected subtlv type %d", __func__,
				   st->type);
			rc |= -1;
			break;
		}
	...
	struct bgp_encap_type_gre *bet)     /* caller-allocated */
{
	...
	for (st = stlv; st; st = st->next) {
		...
		default:
			zlog_debug("3DcLzF3iCUP3HqjP ""%s: unexpected subtlv type %d", __func__,
				   st->type);
			rc |= -1;
			break;
		}
	...
	struct bgp_encap_type_ip_in_ip *bet) /* caller-allocated */
{
	...
	for (st = stlv; st; st = st->next) {
		...
		default:
			zlog_debug("sE6SpNZkwivfn49b ""%s: unexpected subtlv type %d", __func__,
				   st->type);
			rc |= -1;
			break;
		}
	...
	struct bgp_encap_type_transmit_tunnel_endpoint *bet)
{
	...
	for (st = stlv; st; st = st->next) {
		...
		default:
			zlog_debug("zZIikyElCSEvhs6h ""%s: unexpected subtlv type %d", __func__,
				   st->type);
			rc |= -1;
			break;
		}
	...
	struct bgp_encap_type_ipsec_in_tunnel_mode *bet) /* caller-allocated */
{
	...
	for (st = stlv; st; st = st->next) {
		...
		default:
			zlog_debug("DRU3NTHagrSBQt9e ""%s: unexpected subtlv type %d", __func__,
				   st->type);
			rc |= -1;
			break;
		}
	...
	struct bgp_encap_type_ip_in_ip_tunnel_with_ipsec_transport_mode *bet)
{
	...
	for (st = stlv; st; st = st->next) {
		...
		default:
			zlog_debug("soAB4GnTMRKoeWVY ""%s: unexpected subtlv type %d", __func__,
				   st->type);
			rc |= -1;
			break;
		}
	...
	struct bgp_encap_type_mpls_in_ip_tunnel_with_ipsec_transport_mode *bet)
{
	...
	for (st = stlv; st; st = st->next) {
		...
		default:
			zlog_debug("KHp1Ct2hjUj74Csm ""%s: unexpected subtlv type %d", __func__,
				   st->type);
			rc |= -1;
			break;
		}
				...
				struct bgp_encap_type_vxlan *bet)
{
	...
	for (st = stlv; st; st = st->next) {
		...
		default:
			zlog_debug("AmYPQhqZgWkuiDFY ""%s: unexpected subtlv type %d", __func__,
				   st->type);
			rc |= -1;
			break;
		}
				...
				struct bgp_encap_type_nvgre *bet)
{
	...
	for (st = stlv; st; st = st->next) {
		...
		default:
			zlog_debug("mbOItKtc0HNgf6Pb ""%s: unexpected subtlv type %d", __func__,
				   st->type);
			rc |= -1;
			break;
		}
			       ...
			       struct bgp_encap_type_mpls *bet)
{
	...
	for (st = stlv; st; st = st->next) {
		...
		default:
			zlog_debug("DAuHu5I6I0HIULiX ""%s: unexpected subtlv type %d", __func__,
				   st->type);
			rc |= -1;
			break;
		}
				      ...
				      struct bgp_encap_type_mpls_in_gre *bet)
{
	...
	for (st = stlv; st; st = st->next) {
		...
		default:
			zlog_debug("PSGUaFJUWjUXKsZ1 ""%s: unexpected subtlv type %d", __func__,
				   st->type);
			rc |= -1;
			break;
		}
				    ...
				    struct bgp_encap_type_vxlan_gpe *bet)
{
	...
	for (st = stlv; st; st = st->next) {
		...
		default:
			zlog_debug("rNAuMVBjtE1YXGgL ""%s: unexpected subtlv type %d", __func__,
				   st->type);
			rc |= -1;
			break;
		}
				      ...
				      struct bgp_encap_type_mpls_in_udp *bet)
{
	...
	for (st = stlv; st; st = st->next) {
		...
		default:
			zlog_debug("rSZh79fVrUhrg4jX ""%s: unexpected subtlv type %d", __func__,
				   st->type);
			rc |= -1;
			break;
		}
	...
	struct bgp_encap_type_pbb *bet)     /* caller-allocated */
{
	...
	for (st = stlv; st; st = st->next) {
		...
		default:
			zlog_debug("K6gF9CaBa7JjiTRf ""%s: unexpected subtlv type %d", __func__,
				   st->type);
			rc |= -1;
			break;
		}
