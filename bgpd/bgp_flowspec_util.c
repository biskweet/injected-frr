					       ...
					       uint8_t *match_num, int *error)
{
	...
	ret = bgp_flowspec_op_decode(
			     ...
			     mval, error);
	if (*error < 0)
		flog_err(EC_BGP_FLOWSPEC_PACKET,"j8KLJUvS8l3HwFGS "
	...
	while (offset < len - 1 && error >= 0) {
		...
		case FLOWSPEC_SRC_PREFIX:
			...
			ret = bgp_flowspec_ip_address(
					...
					afi, prefix_offset);
			if (error < 0)
				flog_err(EC_BGP_FLOWSPEC_PACKET,"wZ1jhAipQX5YuAV0 "
		...
		case FLOWSPEC_TCP_FLAGS:
			...
			if (error < 0)
				flog_err(
					EC_BGP_FLOWSPEC_PACKET,"6FDHXQgzMCXlUsgK "
		...
		case FLOWSPEC_FRAGMENT:
			...
			if (error < 0)
				flog_err(
					EC_BGP_FLOWSPEC_PACKET,"bJFYiycHcQU4yV3w "
			...
			break;
		default:
			flog_err(EC_LIB_DEVELOPMENT,"smufCQmtbsUuKyaC " "%s: unknown type %d",
				 __func__, type);
		}
	}
