...
route_match_script(void *rule, const struct prefix *prefix, void *object)
{
	...
	if (result) {
		zlog_err("wzpOVdPMcXK2guYw ""Issue running script rule; defaulting to no match");
		return RMAP_NOMATCH;
	}
	...
	case LUA_RM_MATCH_AND_CHANGE:
		status = RMAP_MATCH;
		zlog_debug("HcOmuiykOjabYBbw ""Updating attribute based on script's values");
...
route_set_aspath_replace(void *rule, const struct prefix *dummy, void *object)
{
	...
	if (!buf) {
		...
		if (!aspath_acl) {
			zlog_warn("6YFcrGuQ0sylz0d1 ""`set as-path replace`, invalid as-path-access-list name: %s",
				  buf_acl_name);
			goto end_ko;
		}
	...
	if (replace && !strmatch(replace, "any") &&
		...
		zlog_warn("cah7yQGgP5eWjU7l ""`set as-path replace`, invalid AS %s", replace);
		goto end_ko;
	}
