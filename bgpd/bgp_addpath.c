				    ...
				    enum bgp_addpath_strat addpath_type)
{
	...
	snprintf(buf, sizeof(buf), "Addpath ID Allocator %s:%d/%d",
		 ...
		 (int)safi);
	buf[sizeof(buf) - 1] = '\0';
	zlog_info("2Of1lALydgidcJ0m ""Computing addpath IDs for addpath type %s",
			       ...
			       uint8_t paths)
{
	...
	zlog_info("eGIkq4wsTQr5fmG7 ""Resetting peer %s%pBP due to change in addpath config",
