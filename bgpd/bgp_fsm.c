...
static struct peer *peer_xfer_conn(struct peer *from_peer)
{
	...
	FOREACH_AFI_SAFI (afi, safi) {
		if (from_peer->afc[afi][safi] != peer->afc[afi][safi]) {
			flog_err(
				EC_BGP_DOPPELGANGER_CONFIG,"LcB6VkXgZ6R0VaYn "
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("UYiwLBWFNb0kZmM7 ""%s: peer transfer %p fd %d -> %p fd %d)",
	...
	if (bgp_getsockname(peer) < 0) {
		flog_err(EC_LIB_SOCKET,"hVRiDf5LFLV9hhYN "
	...
	if (going_away->status > Active) {
		if (bgp_getsockname(from_peer) < 0) {
			flog_err(EC_LIB_SOCKET,"ZpOjCCdkWFIMQi9j "
...
void bgp_timer_set(struct peer_connection *connection)
{
	...
	case Clearing:
		...
		break;
	case BGP_STATUS_MAX:
		flog_err(EC_LIB_DEVELOPMENT,"LrLeTdaaTJCtQI0F "
...
static void bgp_start_timer(struct event *thread)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("u4MtnBOtAQNd1CBL ""%s [FSM] Timer (start timer expire).", peer->host);
...
static void bgp_connect_timer(struct event *thread)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("ryHt5sLXZaIE5Gqe ""%s [FSM] Timer (connect timer expire)", peer->host);
...
static void bgp_holdtime_timer(struct event *thread)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("nBIeSfG3hErNDrJe ""%s [FSM] Timer (holdtime timer expire)",
			   peer->host);
...
void bgp_routeadv_timer(struct event *thread)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("CvDw43xU7NhNja6J ""%s [FSM] Timer (routeadv timer expire)", peer->host);
...
void bgp_delayopen_timer(struct event *thread)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("bTvAVn2TmryHdpGq ""%s [FSM] Timer (DelayOpentimer expire)",
			   peer->host);
					   ...
					   struct peer *peer)
{
	...
	if (peer_dynamic_neighbor(peer) &&
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("XEHSjCxDILUY95fK ""%s (dynamic neighbor) deleted (%s)",
				   peer->host, __func__);
		peer_delete(peer);
	}
...
static void bgp_llgr_stale_timer_expire(struct event *thread)
{
	 ...
	 */
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("TMk7Lr3xsh79mmLA ""%pBP Long-lived stale timer (%s) expired", peer,
...
static void bgp_graceful_restart_timer_expire(struct event *thread)
{
	...
	if (bgp_debug_neighbor_events(peer)) {
		zlog_debug("gSyqgHZTBodzCe3Y ""%pBP graceful restart timer expired", peer);
		zlog_debug("gFnK4g53DlBB3Jcr ""%pBP graceful restart stalepath timer stopped",
			   peer);
	}
...
static void bgp_graceful_stale_timer_expire(struct event *thread)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("ooJcGdNYQ5u0c0pl ""%pBP graceful restart stalepath timer expired",
			   peer);
...
void bgp_start_routeadv(struct bgp *bgp)
{
	...
	zlog_info("naYsmIywyId9G0XG ""%s, update hold status %d", __func__,
...
static void bgp_maxmed_onstartup_timer(struct event *thread)
{
	...
	zlog_info("AsROblFih7LTpqNU ""Max med on startup ended - timer expired.");
...
static void bgp_maxmed_onstartup_begin(struct bgp *bgp)
{
	...
	zlog_info("lvdrz998Z4luIrVO ""Begin maxmed onstartup mode - timer %d seconds",
...
static void bgp_update_delay_timer(struct event *thread)
{
	...
	zlog_info("dAzC3e6smz3czRLk ""Update delay ended - timer expired.");
...
static void bgp_establish_wait_timer(struct event *thread)
{
	...
	zlog_info("DpQeZ0hkIBTCCe14 ""Establish wait - timer expired.");
			   ...
			   enum bgp_fsm_status status)
{
	...
	if (bgp_debug_neighbor_events(peer)) {
		...
		zlog_debug("DDlnZpmMWOzWLDLu ""%s : vrf %s(%u), Status: %s established_peers %u", __func__,
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("VQZJrhaWlb5BdUoc ""%s fd %d went from %s to %s", peer->host,
			   connection->fd,
...
enum bgp_fsm_state_progress bgp_stop(struct peer_connection *connection)
{
	...
	if (peer_dynamic_neighbor_no_nsf(peer) &&
	    !(CHECK_FLAG(peer->flags, PEER_FLAG_DELETE))) {
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("iZKWYrxjOaWpxg5Y ""%s (dynamic neighbor) deleted (%s)",
				   peer->host, __func__);
		peer_delete(peer);
	...
	if (peer_established(connection)) {
		...
		if (BGP_PEER_GRACEFUL_RESTART_CAPABLE(peer)) {
			FOREACH_AFI_SAFI (afi, safi) {
				...
				if (BGP_DEBUG(update, UPDATE_OUT))
					zlog_debug("WtR8sp3t9TaNvsSX ""peer %s, EOR_required %d",
						   peer->host,
						   gr_info->eor_required);
		...
		if (BGP_DEBUG(update_groups, UPDATE_GROUPS))
			zlog_debug("sW0IKCLEw1wxwrp1 ""%s remove from all update group",
				   peer->host);
...
bgp_stop_with_error(struct peer_connection *connection)
{
	...
	if (peer_dynamic_neighbor_no_nsf(peer)) {
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("XbDuhuS4vxgjPUHA ""%s (dynamic neighbor) deleted (%s)",
				   peer->host, __func__);
		peer_delete(peer);
		     ...
		     uint8_t sub_code)
{
	...
	if (peer_dynamic_neighbor_no_nsf(peer)) {
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("lhPPdSnXpe6HgQkE ""%s (dynamic neighbor) deleted (%s)",
				   peer->host, __func__);
		peer_delete(peer);
		return BGP_FSM_FAILURE;
	}
...
static void bgp_connect_check(struct event *thread)
{
	...
	if (ret < 0) {
		zlog_err("F1p0GXJVS87TFR0o ""can't get sockopt for nonblocking connect: %d(%s)",
	...
	if (status == 0) {
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("fTlVLJOOwefEWYGw ""%s [Event] Connect failed %d(%s)",
...
bgp_connect_success(struct peer_connection *connection)
{
	...
	if (connection->fd < 0) {
		flog_err(EC_BGP_CONNECT,"eg6244251UbC0MnV " "%s peer's fd is negative value %d",
			 __func__, connection->fd);
		return bgp_stop(connection);
	}

	if (bgp_getsockname(peer) < 0) {
		flog_err_sys(EC_LIB_SOCKET,"N3nDoTFViUPSIYAy "
	...
	if (bgp_debug_neighbor_events(peer)) {
		if (!CHECK_FLAG(peer->sflags, PEER_STATUS_ACCEPT_PEER))
			zlog_debug("77HGHruIBGvj6pw6 ""%s open active, local address %pSU",
				   peer->host, peer->su_local);
		else
			zlog_debug("jP70k7HI4mtqFHI7 ""%s passive open", peer->host);
	}
...
bgp_connect_success_w_delayopen(struct peer_connection *connection)
{
	...
	if (connection->fd < 0) {
		flog_err(EC_BGP_CONNECT,"ZlVNPeWTzc9JvnbT " "%s: peer's fd is negative value %d",
			 __func__, connection->fd);
		return bgp_stop(connection);
	}

	if (bgp_getsockname(peer) < 0) {
		flog_err_sys(EC_LIB_SOCKET,"IgEr75FL6N1kfwbj "
			     "%s: bgp_getsockname(): failed for peer %s, fd %d",
			     __func__, peer->host, connection->fd);
		bgp_notify_send(peer->connection, BGP_NOTIFY_FSM_ERR,
				bgp_fsm_error_subcode(connection->status));
	...
	if (bgp_debug_neighbor_events(peer)) {
		if (!CHECK_FLAG(peer->sflags, PEER_STATUS_ACCEPT_PEER))
			zlog_debug("PaTIP4GuOPYQCoCt ""%s open active, local address %pSU",
				   peer->host, peer->su_local);
		else
			zlog_debug("oIR6TxYUVgPPratH ""%s passive open", peer->host);
	}
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("JDHN4LbIfP29LA0v ""%s [FSM] BGP OPEN message delayed for %d seconds",
...
bgp_connect_fail(struct peer_connection *connection)
{
	...
	if (peer_dynamic_neighbor_no_nsf(peer)) {
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("SQo72tup7elbUwmf ""%s (dynamic neighbor) deleted (%s)",
				   peer->host, __func__);
		peer_delete(peer);
		return BGP_FSM_FAILURE_AND_DELETE;
	}
...
static enum bgp_fsm_state_progress bgp_start(struct peer_connection *connection)
{
	...
	if (BGP_PEER_START_SUPPRESSED(peer)) {
		if (bgp_debug_neighbor_events(peer))
			flog_err(EC_BGP_FSM,"QOlB9fvmlYdhAUtd "
	...
	case connect_error:
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("ilae5gf8kJSGiKvI ""%s [FSM] Connect error", peer->host);
		BGP_EVENT_ADD(connection, TCP_connection_open_failed);
		break;
	case connect_success:
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("0fW1z6RftjZr8qId ""%s [FSM] Connect immediately success, fd %d",
	...
	case connect_in_progress:
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("L1IxEGkdb5eDJSvy ""%s [FSM] Non blocking connect waiting result, fd %d",
				   peer->host, connection->fd);
		if (connection->fd < 0) {
			flog_err(EC_BGP_FSM,"sUf699Aia9gx6hI8 " "%s peer's fd is negative value %d",
...
bgp_fsm_event_error(struct peer_connection *connection)
{
	...
	flog_err(EC_BGP_FSM,"txdwlutaPOFUbrNa " "%s [FSM] unexpected packet received in state %s",
		 peer->host,
		 lookup_msg(bgp_status_msg, connection->status, NULL));
...
bgp_fsm_holdtime_expire(struct peer_connection *connection)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("uyxamKgLpbo1dIRm ""%s [FSM] Hold timer expire", peer->host);
				    ...
				    struct graceful_restart_info *gr_info)
{
	...
	if (BGP_DEBUG(update, UPDATE_OUT))
		zlog_debug("B7aIfzLb1JHf5DzD ""Started the deferral timer for %s eor_required %d",
			   get_afi_safi_str(afi, safi, false),
			   gr_info->eor_required);
	return 0;
}
...
static int bgp_update_gr_info(struct peer *peer, afi_t afi, safi_t safi)
{
	...
	if ((afi < AFI_IP) || (afi >= AFI_MAX)) {
		if (BGP_DEBUG(update, UPDATE_OUT))
			zlog_debug("xntr1RAchHF6jmeB ""%s : invalid afi %d", __func__, afi);
		return -1;
	}

	if ((safi < SAFI_UNICAST) || (safi > SAFI_MPLS_VPN)) {
		if (BGP_DEBUG(update, UPDATE_OUT))
			zlog_debug("6mWPbiyAE0eNyMqD ""%s : invalid safi %d", __func__, safi);
		return -1;
	}
...
bgp_establish(struct peer_connection *connection)
{
	...
	if (!peer) {
		flog_err(EC_BGP_CONNECT,"3T7um1WH50lGJWpt " "%%Neighbor failed in xfer_conn");
	...
	if (CHECK_FLAG(peer->bgp->flags, BGP_FLAG_LOG_NEIGHBOR_CHANGES)) {
		struct vrf *vrf = vrf_lookup_by_id(peer->bgp->vrf_id);
		zlog_info("OfQmIhFJgDFP3Tep ""%%ADJCHANGE: neighbor %pBP in vrf %s Up", peer,
			  vrf ? ((vrf->vrf_id != VRF_DEFAULT)
					 ? vrf->name
					 : VRF_DEFAULT_NAME)
			      : "");
	...
	if (bgp_debug_neighbor_events(peer)) {
		if (BGP_PEER_RESTARTING_MODE(peer))
			zlog_debug("zRbfxl4YFYE6EcmY ""%pBP BGP_RESTARTING_MODE", peer);
		else if (BGP_PEER_HELPER_MODE(peer))
			zlog_debug("khFmCRetgKRLGZVE ""%pBP BGP_HELPER_MODE", peer);
	}
	...
	if (connection->t_gr_restart) {
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("3mbRopwYBijVnt3p ""%pBP graceful restart timer stopped", peer);
	}
...
static enum bgp_fsm_state_progress bgp_ignore(struct peer_connection *connection)
{
	...
	flog_err(EC_BGP_FSM,"Lrf0oX504kk6I2CM "
...
bgp_fsm_exception(struct peer_connection *connection)
{
	...
	flog_err(EC_BGP_FSM,"i37oKXr5W3nnpvfG "
		     ...
		     enum bgp_fsm_events event)
{
	...
	if (bgp_debug_neighbor_events(peer) && connection->status != next)
		zlog_debug("urEFW6xdEStNF8R8 ""%s [FSM] %s (%s->%s), fd %d", peer->host,
			   bgp_event_str[event],
			   lookup_msg(bgp_status_msg, connection->status, NULL),
	...
	case BGP_FSM_FAILURE:
		...
		if (!dyn_nbr && !passive_conn && peer->bgp &&
		    ret != BGP_FSM_FAILURE_AND_DELETE) {
			flog_err(EC_BGP_FSM,"Xcyj0PHDnVz0X8Bn "
				    ...
				    enum global_mode global_old_state)
{
	...
	for (ALL_LIST_ELEMENTS(bgp->peer, node, nnode, peer)) {
		...
		if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
			zlog_debug("6jsIiYoquVSgiAJn ""%s [BGP_GR] Peer: (%s) :", __func__,
				   peer->host);
		...
		if (peer_old_state == PEER_GLOBAL_INHERIT) {
			...
			case GLOBAL_DISABLE:
				...
				break;
			case GLOBAL_INVALID:
				zlog_debug("rhRwVgW5g5IJ1Hnu ""%s [BGP_GR] GLOBAL_INVALID",
					   __func__);
...
int bgp_gr_update_all(struct bgp *bgp, enum global_gr_command global_gr_cmd)
{
	...
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("XlbyR3ukguh9vpqI ""%s [BGP_GR]START: global_gr_cmd :%s:", __func__,
			   print_global_gr_cmd(global_gr_cmd));

	global_old_state = bgp_global_gr_mode_get(bgp);

	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("qLdI4qBkg8ljPPH0 ""[BGP_GR] global_old_gr_state :%s:",
	...
	if (global_old_state != GLOBAL_INVALID) {
		...
		if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
			zlog_debug("JfmZXAR3v84RAw5X ""[BGP_GR] global_new_gr_state :%s:",
				   print_global_gr_mode(global_new_state));
	} else {
		zlog_err("07cbXf2yMnfDwKPT ""%s [BGP_GR] global_old_state == GLOBAL_INVALID",
			 __func__);
		return BGP_ERR_GR_OPERATION_FAILED;
	}

	if (global_new_state == GLOBAL_INVALID) {
		zlog_err("qjKZeHIjyLWIxlXk ""%s [BGP_GR] global_new_state == GLOBAL_INVALID",
			 __func__);
				  ...
				  enum peer_gr_command peer_gr_cmd)
{
	...
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("SiHaMpn7v3fWKpMu ""%s [BGP_GR] START:Peer: (%s) : peer_gr_cmd :%s:",
			   __func__, peer->host,
	...
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("p0jcQykSZSBJIeuw ""%s [BGP_GR] peer_old_state: %d", __func__,
			   peer_old_state);
	...
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("7RJzmA0anWTaYHpE ""%s [BGP_GR] peer_new_state: %d", __func__,
			   peer_new_state);
				...
				enum peer_mode new_peer_state)
{
	 ...
	 */
		 ...
		 */
		if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
			zlog_debug("BjyrbIlXTcTLsZ4o ""[BGP_GR] Peer state changed from :%s",
				   print_peer_gr_mode(old_peer_state));
...

{
	...
	case PEER_GLOBAL_INHERIT:
		...
		if (bgp_global_gr_mode == GLOBAL_HELPER) {
			...
			zlog_err("vmfn4FwlMKTDUDXF "
				"[BGP_GR] Default switch inherit mode ::: SOMETHING IS WRONG !!!");
		}
		break;
	default:
		zlog_err("TZyXIteX2C85ozza "
			"[BGP_GR] Default switch mode ::: SOMETHING IS WRONG !!!");
		break;
	}
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("MEtQEca32s0IdEzu ""[BGP_GR] Peer state changed  --to-->  : %d : !",
			   new_state);
}
...
{
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("h6uffqJ7v39aZYOF ""%s [BGP_GR] called !", __func__);
	...
	if (!CHECK_FLAG(peer->flags, PEER_FLAG_GRACEFUL_RESTART)
		...
		zlog_debug("fHXefnMu8WLfOH4b ""[BGP_GR] Peer %s UNSET PEER_STATUS_NSF_MODE!",
			   peer->host);
