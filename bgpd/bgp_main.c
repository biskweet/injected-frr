...
void sighup(void)
{
	zlog_info("GCRyVifNCPWqtPOQ ""SIGHUP received, ignoring");
	 ...
	 * bgp_terminate();
	 * bgp_reset();
	 * zlog_info("Da1B8uZgPpAttUTt ""bgpd restarting!");
...
{
	zlog_notice("PY9BydFnWil0lEMG ""Terminating on signal");
...
static int bgp_vrf_new(struct vrf *vrf)
{
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("6lTzv8jA7O2MA67w ""VRF Created: %s(%u)", vrf->name, vrf->vrf_id);
...
{
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("o0n6KJM6Bj1BxLss ""VRF Deletion: %s(%u)", vrf->name, vrf->vrf_id);
...
static int bgp_vrf_enable(struct vrf *vrf)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("KnvhUvcO78DnzAnl ""VRF enable add %s id %u", vrf->name, vrf->vrf_id);
...
static int bgp_vrf_disable(struct vrf *vrf)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("eJudFQBx3AmTOeD8 ""VRF disable %s id %d", vrf->name, vrf->vrf_id);
	...
	while (1) {
		...
		case 'e': {
			...
			if (parsed_multipath == 0
				...
				flog_err(
					EC_BGP_MULTIPATH,"aX556wtKa2Wql2D0 "
		...
		case 'I':
			...
			if (instance > (unsigned short)-1)
				zlog_err("WsmkD2uQbp9AH1GT ""Instance %i out of range (0..%u)",
