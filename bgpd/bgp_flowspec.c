			    ...
			    struct bgp_nlri *packet, bool withdraw)
{
	...
	if (packet->length >= FLOWSPEC_NLRI_SIZELIMIT_EXTENDED) {
		flog_err(EC_BGP_FLOWSPEC_PACKET,"QKd6T3xTX36qzrgN "
	...
	for (; pnt < lim; pnt += psize) {
		...
		if (pnt + psize > lim) {
			flog_err(
				EC_BGP_FLOWSPEC_PACKET,"rUFPCm1JiegsvxLg "
		...
		if (psize == 0) {
			flog_err(EC_BGP_FLOWSPEC_PACKET,"XjNTBXojVKt3VSm3 "
		...
		if (bgp_fs_nlri_validate(pnt, psize, afi) < 0) {
			flog_err(
				EC_BGP_FLOWSPEC_PACKET,"dW57ay10QIEfDN9P "
		...
		if (BGP_DEBUG(flowspec, FLOWSPEC)) {
			...
			snprintf(local_string, sizeof(local_string),
				 ...
				 attr != NULL ? ec_string : "");
			zlog_info("OYKGpCVtzD1LgRem ""%s", local_string);
		}
		/* Process the route. */
		if (!withdraw) {
