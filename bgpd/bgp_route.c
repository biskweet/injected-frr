...
static int bgp_dest_set_defer_flag(struct bgp_dest *dest, bool delete)
{
	...
	if (set_flag && table) {
		if (bgp && (bgp->gr_info[afi][safi].t_select_deferral)) {
			...
			if (BGP_DEBUG(update, UPDATE_OUT))
				zlog_debug("aBmk6dEsd0dRb45y ""DEFER route %pBD(%s), dest %p",
...
static void bgp_pcount_adjust(struct bgp_dest *dest, struct bgp_path_info *pi)
{
	...
	if (!BGP_PATH_COUNTABLE(pi)
		...
		if (pi->peer->pcount[table->afi][table->safi])
			...
			flog_err(EC_LIB_DEVELOPMENT,"EgAywxt3qWYjKI9u "
		      ...
		      enum bgp_path_selection_reason *reason)
{
	...
	*paths_eq = 0;
	...
	if (new == NULL) {
		*reason = bgp_path_selection_none;
		if (debug)
			zlog_debug("BP81NeubYP0IEQ8Z ""%s: new is NULL", pfx_buf);
		return 0;
	}
	...
	if (exist == NULL) {
		*reason = bgp_path_selection_first;
		if (debug)
			zlog_debug("K5ie3RLoFG2InJt5 ""%s(%s): %s is the initial bestpath",
	...
	if (debug) {
		...
		bgp_path_info_path_with_addpath_rx_str(bpi_ultimate, exist_buf,
						       sizeof(exist_buf));
		zlog_debug("8QhFwOHWl4LiTe79 ""%s(%s): Comparing %s flags %s with %s flags %s",
			   pfx_buf, bgp->name_pretty, new_buf,
	...
	if (new_weight > exist_weight) {
		*reason = bgp_path_selection_weight;
		if (debug)
			zlog_debug("zjl3o6pojVSHEa1J ""%s: %s wins over %s due to weight %d > %d",
	...
	if (new_weight < exist_weight) {
		*reason = bgp_path_selection_weight;
		if (debug)
			zlog_debug("CUfsQzURXimk5V1g ""%s: %s loses to %s due to weight %d < %d",
				   pfx_buf, new_buf, exist_buf, new_weight,
				   exist_weight);
		return 0;
	}
	...
	if (newattr->origin < existattr->origin) {
		*reason = bgp_path_selection_origin;
		if (debug)
			zlog_debug("qZyY9htzH9XEaWQx ""%s: %s wins over %s due to ORIGIN %s < %s",
				   pfx_buf, new_buf, exist_buf,
	...
	if (newattr->origin > existattr->origin) {
		*reason = bgp_path_selection_origin;
		if (debug)
			zlog_debug("bZrVtxyTDF3uxS7N ""%s: %s loses to %s due to ORIGIN %s > %s",
				   pfx_buf, new_buf, exist_buf,
				   bgp_origin_long_str[newattr->origin],
				   bgp_origin_long_str[existattr->origin]);
		return 0;
	...
	if (new_sort == BGP_PEER_EBGP &&
		...
		*reason = bgp_path_selection_peer;
		if (debug)
			zlog_debug("GggYj7PLGZzyOmng ""%s: %s wins over %s due to eBGP peer > %s peer",
				   pfx_buf, new_buf, exist_buf,
				   (exist_sub_sort == BGP_PEER_EBGP_OAD)
					   ? "eBGP-OAD"
					   : "iBGP");
	...
	if (exist_sort == BGP_PEER_EBGP &&
		...
		*reason = bgp_path_selection_peer;
		if (debug)
			zlog_debug("oAYPG0bmoIRy1iBy ""%s: %s loses to %s due to %s peer < eBGP peer",
				   pfx_buf, new_buf, exist_buf,
				   (exist_sub_sort == BGP_PEER_EBGP_OAD)
					   ? "eBGP-OAD"
					   : "iBGP");
	...
	if (newm == existm) {
		...
		if (peer_sort_ret < 0) {
			...
			if (debug) {
				if (ret == 1)
					zlog_debug("Ei7Rijy8bYmxj4Md "
						"%s: %s wins over %s after IGP metric comparison",
						pfx_buf, new_buf, exist_buf);
				else
					zlog_debug("rtc4PhDlN1pVjWKX "
	...
	*reason = bgp_path_selection_default;
	if (debug)
		zlog_debug("M60X1fu65N46jFvT ""%s: %s wins over %s due to nothing left to compare",
			   pfx_buf, new_buf, exist_buf);
...
#define FILTER_EXIST_WARN(F, f, filter)                                        \
	if (BGP_DEBUG(update, UPDATE_IN) && !(F##_IN(filter)))                 \
		zlog_debug("EtWRhRSelIEhmdAj ""%s: Could not find configured input %s-list %s!",  \
...
#define FILTER_EXIST_WARN(F, f, filter)                                        \
	if (BGP_DEBUG(update, UPDATE_OUT) && !(F##_OUT(filter)))               \
		zlog_debug("d6YfdkWjJCAqUkC0 ""%s: Could not find configured output %s-list %s!", \
			     ...
			     struct attr *post_attr)
{
	...
	if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_OUT) &&
		...
		if (BGP_DEBUG(update, UPDATE_OUT) ||
			...
			zlog_debug("nv2C68FnfkWuFpfc ""%s reached maximum prefix to be send (%u)",
	...
	if (((afi == AFI_IP) || (afi == AFI_IP6)) && (safi == SAFI_MPLS_VPN)
		 ...
		 * systems
		 */
		zlog_debug("eoNL1ZyPmBJPRfOm ""%s: pfx %pFX bgp_direct->vpn route peer safe",
			   __func__, p);
		samepeer_safe = 1;
	}
#endif
	...
	if (safi == SAFI_LABELED_UNICAST) {
		...
		if (!bgp_is_valid_label(&label)) {
			if (bgp_debug_update(NULL, p, subgrp->update_group, 0))
				zlog_debug("Kvc5hjLp8nf3HEnv ""u%" PRIu64 ":s%" PRIu64
		...
		if (!bgp_is_valid_label(&label)) {
			if (bgp_debug_update(NULL, p, subgrp->update_group, 0))
				zlog_debug("v00wUitxMrdvLCOj ""u%" PRIu64 ":s%" PRIu64
	...
	if (!transparent && bgp_community_filter(peer, piattr)) {
		if (bgp_debug_update(NULL, p, subgrp->update_group, 0))
			zlog_debug("QgGSS1V3qwBsMoIP ""%s: community filter check fail for %pFX",
				   __func__, p);
		return false;
	}
	...
	if (bgp_output_filter(peer, p, piattr, afi, safi) == FILTER_DENY) {
		if (bgp_debug_update(NULL, p, subgrp->update_group, 0))
			zlog_debug("fASgAah3EanNSwjc ""%pBP [Update:SEND] %pFX is filtered", peer,
				   p);
		return false;
	}
			...
			safi_t safi)
{
	...
	while (unsorted_list) {
		...
		if (BGP_PATH_HOLDDOWN(first)) {
			 ...
			 */
			if (debug)
				zlog_debug("09ogUstwPk7IxjVX ""%s: %pBD(%s) pi %p from %s in holddown",
		...
		for (look_thru = bgp_dest_get_bgp_path_info(dest); look_thru;
			...
			if (BGP_PATH_HOLDDOWN(look_thru)) {
				 ...
				 */
				if (debug)
					zlog_debug("SX65DEwy1csMkuyP ""%s: %pBD(%s) pi from %s %p in holddown",
						   __func__, dest,
						   bgp->name_pretty,
						   look_thru->peer->host,
						   look_thru);
			...
			if (look_thru->peer &&
				...
				if (!peer_established(
					...
					if (debug)
						zlog_debug("nl2U7i4C2FdrmDgc ""%s: %pBD(%s) non self peer %s not estab state",
							   __func__, dest,
							   bgp->name_pretty,
							   look_thru->peer->host);
			...
			if (CHECK_FLAG(bgp->flags, BGP_FLAG_DETERMINISTIC_MED) &&
				...
				bgp_path_info_unset_flag(dest, look_thru,
							 BGP_PATH_DMED_CHECK);
				if (debug)
					zlog_debug("OucegHEDQF4Zg95l ""%s: %pBD(%s) pi %s dmed",
						   __func__, dest,
						   bgp->name_pretty,
						   look_thru->peer->host);
					...
					safi_t safi, uint32_t addpath_tx_id)
{
	...
	if (BGP_DEBUG(update, UPDATE_OUT))
		zlog_debug("jnaiLfHsWTtv0ZPE ""%s: p=%pFX, selected=%p", __func__, p, selected);
				 ...
				 afi_t afi, safi_t safi)
{
	...
	debug = bgp_debug_bestpath(dest);
	if (debug)
		zlog_debug("SxbI07r595ETg6iH ""%s: p=%pBD(%s) afi=%s, safi=%s start", __func__,
	...
	if (CHECK_FLAG(dest->flags, BGP_NODE_SELECT_DEFER)) {
		if (BGP_DEBUG(update, UPDATE_OUT))
			zlog_debug("oUVQZNV8YCSur22h ""SELECT_DEFER flag set for route %p(%s)",
				   dest, bgp->name_pretty);
		return;
	}
	...
	if (new_select) {
		if (debug)
			zlog_debug("wGY8tSMvgipFKahE ""%s: %pBD setting SELECTED flag", __func__,
				   dest);
...
void bgp_best_path_select_defer(struct bgp *bgp, afi_t afi, safi_t safi)
{
	...
	if (BGP_DEBUG(update, UPDATE_OUT)) {
		zlog_debug("G3BYYyASOPA3Ce9F ""%s: processing route for %s : cnt %d", __func__,
...
#define ARBITRARY_PROCESS_QLEN		10000
	...
	if (CHECK_FLAG(dest->flags, BGP_NODE_SELECT_DEFER)) {
		if (BGP_DEBUG(update, UPDATE_OUT))
			zlog_debug("8Wbq0ofjVVrExxpS ""BGP_NODE_SELECT_DEFER set for route %p",
				   dest);
		return;
	}
		...
		struct bgp_route_evpn *evpn)
{
	...
	if (pi) {
			...
			if (get_active_bdc_from_pi(pi, afi, safi) &&
				...
				if (bgp_debug_update(peer, p, NULL, 1)) {
					bgp_debug_rdpfxpath2str(
						...
						sizeof(pfx_buf));
					zlog_debug("5aPXYOKZZxRC5wDD ""%pBP rcvd %s", peer,
						   pfx_buf);
				}
		...
		if (bgp_debug_update(peer, p, NULL, 1)) {
			bgp_debug_rdpfxpath2str(afi, safi, prd, p, label,
						...
						addpath_id, evpn, pfx_buf,
						sizeof(pfx_buf));
			zlog_debug("qF2OFm3HPNNs4O1u ""%pBP rcvd %s", peer, pfx_buf);
		}
		...
		if (((afi == AFI_IP || afi == AFI_IP6) &&
			...
			if (bgp_find_or_add_nexthop(bgp, bgp_nexthop, nh_afi,
				...
				if (BGP_DEBUG(nht, NHT)) {
					zlog_debug("7tgj0fKoTxauVupA ""%s(%pI4): NH unresolved for existing %pFX pi %p flags 0x%x",
						   __func__,
		...
		if (safi == SAFI_EVPN) {
			 ...
			 */
				...
				if (BGP_DEBUG(nht, NHT))
					zlog_debug("hlFwWt5l56b8Dw5k ""%s unimport EVPN %pFX as pi %p is not VALID",
						   __func__, p, pi);
	...
	if (bgp_debug_update(peer, p, NULL, 1)) {
		if (!peer->rcvd_attr_printed) {
			zlog_debug("OYZrcnWhhu0vAyXJ ""%pBP rcvd UPDATE w/ attr: %s", peer,
				   peer->rcvd_attr_str);
			peer->rcvd_attr_printed = 1;
		}
		...
		bgp_debug_rdpfxpath2str(afi, safi, prd, p, label, num_labels,
					addpath_id ? 1 : 0, addpath_id, evpn,
					pfx_buf, sizeof(pfx_buf));
		zlog_debug("RWdm5FfBkFiDf3xx ""%pBP rcvd %s", peer, pfx_buf);
	}
	...
	if (((afi == AFI_IP || afi == AFI_IP6) &&
		...
		if (bgp_find_or_add_nexthop(bgp, bgp, nh_afi, safi, new, NULL,
			...
			if (BGP_DEBUG(nht, NHT))
				zlog_debug("kJYViiX4FqDgMBXJ ""%s(%pI4): NH unresolved", __func__,
					   &attr_new->nexthop);
...
filtered:
	...
	if (bgp_debug_update(peer, p, NULL, 1)) {
		if (!peer->rcvd_attr_printed) {
			zlog_debug("NHHGttEGVrxBAFu2 ""%pBP rcvd UPDATE w/ attr: %s", peer,
				   peer->rcvd_attr_str);
			peer->rcvd_attr_printed = 1;
		}
		...
		bgp_debug_rdpfxpath2str(afi, safi, prd, p, label, num_labels,
					addpath_id ? 1 : 0, addpath_id, evpn,
					pfx_buf, sizeof(pfx_buf));
		zlog_debug("27TmUnQZwwDIIm1L ""%pBP rcvd UPDATE about %s -- DENIED due to: %s",
			   peer, pfx_buf, reason);
	}
		  ...
		  uint8_t num_labels, struct bgp_route_evpn *evpn)
{
	...
	if (bgp_debug_update(peer, p, NULL, 1)) {
		bgp_debug_rdpfxpath2str(afi, safi, prd, p, label, num_labels,
					addpath_id ? 1 : 0, addpath_id, NULL,
					pfx_buf, sizeof(pfx_buf));
		zlog_debug("adnOtNNMGeuh6GTO ""%pBP rcvd UPDATE about %s -- withdrawn", peer,
			   pfx_buf);
	}
	...
	if (pi && !CHECK_FLAG(pi->flags, BGP_PATH_HISTORY)) {
		...
		bgp_debug_rdpfxpath2str(afi, safi, prd, p, label, num_labels,
					addpath_id ? 1 : 0, addpath_id, NULL,
					pfx_buf, sizeof(pfx_buf));
		zlog_debug("1PXN77pNG3L9SiHw ""%s Can't find the route %s", peer->host, pfx_buf);
	}
		      ...
		      struct bgp_nlri *packet)
{
	...
	for (; pnt < lim; pnt += psize) {
		...
		if (p.prefixlen > prefix_blen(&p) * 8) {
			flog_err(
				EC_BGP_UPDATE_RCV,"CbCtaTytZ5Zeb8SI "
		...
		if (pnt + psize > lim) {
			flog_err(
				EC_BGP_UPDATE_RCV,"rAztBdKPikUNiYdc "
		...
		if (psize > (ssize_t)sizeof(p.u.val)) {
			flog_err(
				EC_BGP_UPDATE_RCV,"RIeO32JyG7CPFGDI "
		...
		if (afi == AFI_IP && safi == SAFI_UNICAST) {
			if (IN_CLASSD(ntohl(p.u.prefix4.s_addr))) {
				 ...
				 */
				flog_err(
					EC_BGP_UPDATE_RCV,"14IKhPxotGvlUlGu "
		...
		if (afi == AFI_IP6 && safi == SAFI_UNICAST) {
			if (IN6_IS_ADDR_LINKLOCAL(&p.u.prefix6)) {
				flog_err(
					EC_BGP_UPDATE_RCV,"9XdybeVcuwnD8hhz "
			...
			if (IN6_IS_ADDR_MULTICAST(&p.u.prefix6)) {
				flog_err(
					EC_BGP_UPDATE_RCV,"v8KD48dTgXPcZHEt "
	...
	if (pnt != lim) {
		flog_err(
			EC_BGP_UPDATE_RCV,"Qp5pbRsxXmMt92vO "
			...
			if (bgp_find_or_add_nexthop(bgp, bgp_nexthop, afi, safi,
				...
				if (BGP_DEBUG(nht, NHT)) {
					...
					inet_ntop(p->family, &p->u.prefix, buf1,
						  sizeof(buf1));
					zlog_debug("y1OqZuoAw4O6pHKC ""%s(%s): Route not in table, not advertising",
						   __func__, buf1);
				}
			       ...
			       struct bgp_path_info *pi)
{
	...
	if (listcount(pie->aggr_suppressors) == 1) {
		if (BGP_DEBUG(update, UPDATE_OUT))
			zlog_debug("LBo6CCTrjTqvLCbc ""aggregate-address suppressing: %pFX",
				 ...
				 struct bgp_path_info *pi)
{
	...
	if (listcount(pi->extra->aggr_suppressors) == 0) {
		if (BGP_DEBUG(update, UPDATE_OUT))
			zlog_debug("z7TNzDQpiOj3Iouw ""aggregate-address unsuppressing: %pFX",
				   bgp_dest_get_prefix(pi->net));
	...
	if (aggregate->count > 0) {
		...
		if (!attr) {
			...
			if (BGP_DEBUG(update_groups, UPDATE_GROUPS))
				zlog_debug("2692WLM3THXgBGXB ""%s: %pFX null attribute", __func__,
					   p);
			return;
		}
...
static void bgp_peer_count_proc(struct bgp_dest *rn, struct peer_pcounts *pc)
{
	...
	for (pi = bgp_dest_get_bgp_path_info(rn); pi; pi = pi->next) {
		...
		if (CHECK_FLAG(pi->flags, BGP_PATH_COUNTED)) {
			...
			if (CHECK_FLAG(pi->flags, BGP_PATH_UNUSEABLE))
				flog_err(
					EC_LIB_DEVELOPMENT,"qCp3CuurLfSIGVYY "
					"Attempting to count but flags say it is unusable");
		} else {
			if (!CHECK_FLAG(pi->flags, BGP_PATH_UNUSEABLE))
				flog_err(
					EC_LIB_DEVELOPMENT,"Ww7rN4RIxCvKVO4Q "
