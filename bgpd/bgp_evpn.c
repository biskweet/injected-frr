...
static struct vrf_irt_node *vrf_import_rt_new(struct ecommunity_val *rt)
{
	...
	if (!bgp_evpn) {
		flog_err(EC_BGP_NO_DFLT,"hoRBKFHDJKHtzItc "
...
static void vrf_import_rt_free(struct vrf_irt_node *irt)
{
	...
	bgp_evpn = bgp_get_evpn();
	if (!bgp_evpn) {
		flog_err(EC_BGP_NO_DFLT,"dWa8xbjFIOlWtvuV "
...
static struct vrf_irt_node *lookup_vrf_import_rt(struct ecommunity_val *rt)
{
	...
	if (!bgp_evpn) {
		flog_err(
			EC_BGP_NO_DFLT,"rJtC2Rg0PzxMjw3r "
	...
	uint8_t flags, uint32_t seq, esi_t *esi)
{
	...
	if (!zclient || zclient->sock < 0) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("khPfynSYtiz6dHdV ""%s: No zclient or zclient->sock exists",
				   __func__);
			   ...
			   int add)
{
	...
	if (!zclient || zclient->sock < 0) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("dfdycDtZ1yYboT6I ""%s: No zclient or zclient->sock exists",
				   __func__);
		return ZCLIENT_SEND_SUCCESS;
	}
	...
	if (is_evpn_prefix_ipaddr_v4(p))
		...
		flog_err(
			EC_BGP_VTEP_INVALID,"lV7AmbitP8HRjljr "
	...
	if (bgp_debug_zebra(NULL))
		zlog_debug("xmou8bT9NN6C9PrM ""Tx %s Remote VTEP, VNI %u remote VTEP %pI4",
			   add ? "ADD" : "DEL", (vpn ? vpn->vni : 0),
					...
					struct bgp_path_info *new_select)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_RT)) {
		...
		zlog_debug("blOxOSTcnIXL0ncb ""local path deleted %pFX es %s; new-path-es %s", evp,
				  ...
				  struct bgp_path_info *local_pi)
{
	...
	if (bgp_debug_zebra(NULL))
		zlog_debug("Ho6IFFonpNsyW4tK ""evicting local evpn prefix %pBD as remote won",
			   dest);
					   ...
					   struct bgp_path_info *parent_pi)
{
		...
		if (bgp_find_or_add_nexthop(bgp_vrf, bgp_vrf, afi, safi, pi,
			...
			if (BGP_DEBUG(nht, NHT)) {
				inet_ntop(pp->family,
					  ...
					  buf1, sizeof(buf1));
				zlog_debug("nuiqMm1biBdG3cjp ""%s: gateway IP NH unresolved",
					   buf1);
			}
	...
	if (bgp_debug_zebra(NULL)) {
		...
		zlog_debug("zwXeFUJ3OBaMS8b3 ""... %s pi %s dest %p (l %d) pi %p (l %d, f 0x%x) nh %pIA",
			   new_pi ? "new" : "update",
	...
	struct bgp_dest *dest, struct bgp_path_info *parent_pi)
{
	...
	if (!pi) {
		...
		if ((old_local_es != new_local_es)
			...
			if (BGP_DEBUG(evpn_mh, EVPN_MH_RT))
				zlog_debug("DNL4BvdjkWwFBm5G ""VNI %d path %pFX chg to %s es",
					   vpn->vni, &pi->net->rn->p,
					   new_local_es ? "local"
							: "non-local");
			bgp_path_info_set_flag(dest, pi, BGP_PATH_ATTR_CHANGED);
					     ...
					     struct bgp_path_info *parent_pi)
{
	...
	if (bgp_debug_zebra(NULL)) {
		...
		zlog_debug("1iEWVHV3OVWFbyzq ""... delete pi %s dest %p (l %d) pi %p (l %d, f 0x%x) nh %pIA",
			   bgp_vrf->name_pretty, dest,
			   bgp_dest_get_lock_count(dest), pi, pi->lock,
			   pi->flags, &nhip);
	}
				    ...
				    struct bgp_path_info *parent_pi)
{
	...
	if (ret) {
		flog_err(
			EC_BGP_EVPN_FAIL,"MjSE1dJBcFF0QqX6 "
	...
	if (ret) {
		flog_err(
			EC_BGP_EVPN_FAIL,"neFDGzsu28gA3fiG "
				      ...
				      struct bgp_path_info *parent_pi)
{
	...
	if (ret) {
		flog_err(
			EC_BGP_EVPN_FAIL,"AMbtCIVdpU0tuAg0 "
	...
	if (ret) {
		flog_err(
			EC_BGP_EVPN_FAIL,"M5WwC4yKpbXfTCpG "
					      ...
					      int install)
{
	...
	if (is_route_matching_for_vrf(bgp_vrf, pi)) {
		...
		if (ret)
			flog_err(EC_BGP_EVPN_FAIL,"8FPQX75dr54pZOKT "
	...
	for (rd_dest = bgp_table_top(bgp->rib[afi][safi]); rd_dest;
		...
		for (dest = bgp_table_top(table); dest;
			...
			for (pi = bgp_dest_get_bgp_path_info(dest); pi;
				...
				if (ret) {
					flog_err(EC_BGP_EVPN_FAIL,"Ro8fcyk7SQgkLrFt "
					   ...
					   struct list *vrfs, int install)
{
	...
	for (ALL_LIST_ELEMENTS(vrfs, node, nnode, bgp_vrf)) {
		...
		if (ret) {
			flog_err(EC_BGP_EVPN_FAIL,"cRQAGHCDixHRZFA3 "
					   ...
					   struct list *vnis, int install)
{
	...
	for (ALL_LIST_ELEMENTS(vnis, node, nnode, vpn)) {
		...
		if (ret) {
			flog_err(EC_BGP_EVPN_FAIL,"Cqrp062KcmMcEJhr "
				 "%u: Failed to %s EVPN %s route in VNI %u",
...
static void create_advertise_type3(struct hash_bucket *bucket, void *data)
{
	...
	if (update_evpn_route(bgp, vpn, &p, 0, 0, NULL))
		flog_err(EC_BGP_EVPN_ROUTE_CREATE,"DNqdqZn3Xkz2zqj4 "
			       ...
			       uint32_t addpath_id)
{
	...
	if (psize != 33 && psize != 37 && psize != 49 && psize != 36
	    && psize != 40 && psize != 52) {
		flog_err(EC_BGP_EVPN_ROUTE_INVALID,"c9GECCgUyOvrsryj "
	...
	if (macaddr_len == (ETH_ALEN * 8)) {
		...
		flog_err(
			EC_BGP_EVPN_ROUTE_INVALID,"aufqMwpQccnZ6mcN "
	...
	if (ipaddr_len != 0 && ipaddr_len != IPV4_MAX_BITLEN
		...
		flog_err(
			EC_BGP_EVPN_ROUTE_INVALID,"2GrWcocgJN6ItlQc "
...
fail:
stream_failure:
	flog_err(EC_BGP_EVPN_ROUTE_INVALID,"PjR6BEtFdUGSWYd0 "
			       ...
			       uint32_t addpath_id)
{
	...
	if (psize != 17 && psize != 29) {
		flog_err(EC_BGP_EVPN_ROUTE_INVALID,"f5BEjtkUL7kBNQhh "
	...
	if (attr &&
		...
		if (pmsi_tnl_type != PMSI_TNLTYPE_INGR_REPL
			...
			flog_warn(
				EC_BGP_EVPN_PMSI_PRESENT,"LLgF2c3lkcQ2CUWW "
	...
	if (ipaddr_len == IPV4_MAX_BITLEN) {
		...
		flog_err(
			EC_BGP_EVPN_ROUTE_INVALID,"zqEugZt4k36LMFPT "
			       ...
			       uint32_t addpath_id)
{
	...
	if (psize != 34 && psize != 58) {
		flog_err(EC_BGP_EVPN_ROUTE_INVALID,"WGHCxu3zbbe4t65c "
	...
	if (ippfx_len > IPV6_MAX_BITLEN) {
		flog_err(
			EC_BGP_EVPN_ROUTE_INVALID,"ghWAuKaoEZs8i3kc "
	...
	if (bgp_evpn_is_esi_valid(&evpn.eth_s_id) &&
		...
		flog_err(EC_BGP_EVPN_ROUTE_INVALID,"lmBduDi9qJYaYNFO "
	...
	if (attr) {
		if (is_zero_mac(&attr->rmac) &&
			...
			flog_err(EC_BGP_EVPN_ROUTE_INVALID,"JpK0Se8SEFNBE7QE "
				   ...
				   afi_t afi, safi_t safi)
{
	...
	if (ret)
		flog_err(
			EC_BGP_EVPN_ROUTE_DELETE,"NGeTlJ3PATkN43cx "
				    ...
				    safi_t safi)
{
	...
	if (ret)
		flog_err(EC_BGP_EVPN_ROUTE_CREATE,"FmYWxkjCjS2sjLlU "
	...
	for (; pnt < lim; pnt += psize) {
		...
		case BGP_EVPN_MAC_IP_ROUTE:
			if (process_type2_route(peer, afi, safi,
						...
						psize, addpath_id)) {
				flog_err(
					EC_BGP_EVPN_FAIL,"QOQd9Ya91UALzdv0 "
		...
		case BGP_EVPN_IMET_ROUTE:
			if (process_type3_route(peer, afi, safi,
						...
						psize, addpath_id)) {
				flog_err(
					EC_BGP_PKT_PROCESS,"79o0q7wTWzoyJ2W0 "
		...
		case BGP_EVPN_ES_ROUTE:
			if (bgp_evpn_type4_route_process(peer, afi, safi,
						...
						psize, addpath_id)) {
				flog_err(
					EC_BGP_PKT_PROCESS,"pd2ZmvI5VLE1Mbo4 "
		...
		case BGP_EVPN_AD_ROUTE:
			if (bgp_evpn_type1_route_process(peer, afi, safi,
						...
						psize, addpath_id)) {
				flog_err(
					EC_BGP_PKT_PROCESS,"J0YQMg7yQWFwUODB "
		...
		case BGP_EVPN_IP_PREFIX_ROUTE:
			if (process_type5_route(peer, afi, safi,
						...
						psize, addpath_id)) {
				flog_err(
					EC_BGP_PKT_PROCESS,"MdxQLnILjNMG0YRK "
			     ...
			     struct ipaddr *ip, int state)
{
	...
	if (!vpn || !is_vni_live(vpn)) {
		flog_warn(EC_BGP_EVPN_VPN_VNI,"IVYLtb9WTBPYWdjs "
		...
		struct ipaddr *ip, uint8_t flags, uint32_t seq, esi_t *esi)
{
	...
	vpn = bgp_evpn_lookup_vni(bgp, vni);
	if (!vpn || !is_vni_live(vpn)) {
		flog_warn(EC_BGP_EVPN_VPN_VNI,"AItyPKRoJzi4ifk4 "
	...
	if (update_evpn_route(bgp, vpn, &p, flags, seq, esi)) {
		flog_err(
			EC_BGP_EVPN_ROUTE_CREATE,"dAuqDV99QQYjIUig "
			     ...
			     bool is_anycast_mac)
{
	...
	if (!bgp_evpn) {
		flog_err(
			EC_BGP_NO_DFLT,"TRanWFfU4M9nv6rI "
	...
	if (CHECK_FLAG(bgp_evpn->flags, BGP_FLAG_DELETE_IN_PROGRESS)) {
		flog_err(EC_BGP_NO_DFLT,"hZIiT9NOYngaTxVG "
	...
	if (!bgp_vrf) {
		...
		case BGP_ERR_AS_MISMATCH:
			flog_err(EC_BGP_EVPN_AS_MISMATCH,"W2V41PCEYj1M8O7v "
				 "BGP instance is already running; AS is %s",
				 bgp_vrf->as_pretty);
			return -1;
		case BGP_ERR_INSTANCE_MISMATCH:
			flog_err(EC_BGP_EVPN_INSTANCE_MISMATCH,"5jvb8V2xqevs6nZD "
...
int bgp_evpn_local_l3vni_del(vni_t l3vni, vrf_id_t vrf_id)
{
	...
	if (!bgp_vrf) {
		flog_err(
			EC_BGP_NO_DFLT,"OgjuBQvoeyCqNgt4 "
	...
	if (!bgp_evpn) {
		flog_err(
			EC_BGP_NO_DFLT,"Fyme1B37JCq7hV9h "
	...
	if (CHECK_FLAG(bgp_evpn->flags, BGP_FLAG_DELETE_IN_PROGRESS)) {
		flog_err(EC_BGP_NO_DFLT,"ioLaOcmH1uaD6dTl "
			  "Cannot process L3VNI %u ADD - EVPN BGP instance is shutting down",
			  l3vni);
		return -1;
	}
			   ...
			   ifindex_t svi_ifindex)
{
	...
	if (bgp_evpn_vni_flood_mode_get(bgp, vpn)
		...
		build_evpn_type3_prefix(&p, vpn->originator_ip);
		if (update_evpn_route(bgp, vpn, &p, 0, 0, NULL)) {
			flog_err(EC_BGP_EVPN_ROUTE_CREATE,"a9ZfIHqjmQzmmN1w "
...
{
	zlog_info("cj24DqVR6kUMHWJE ""L2VPN EVPN BUM handling is %s",
						...
						bool resolve)
{
	...
	if (BGP_DEBUG(nht, NHT))
		zlog_debug("tutRuOmnAc70Tfue ""%s(%u): vni %u mac/ip %s for NH %pFX",
	...
	for (agg_dest = bgp_table_top(bgp->aggregate[afi][safi]); agg_dest;
		...
		for (dest = bgp_node_get(table, aggr_p); dest;
			...
			for (pi = bgp_dest_get_bgp_path_info(dest); pi;
				...
				if (BGP_DEBUG(zebra, ZEBRA))
					zlog_debug("wXF4DJXnhetuQi6m ""%s aggregated %pFX remove suppressed route %pFX",
