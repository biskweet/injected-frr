...
#define BMP_PEERUP_INFO_TYPE_STRING 0
	...
	if (peer_established(peer->connection) && !down) {
		...
		if (bbpeer && bbpeer->open_tx)
			...
			zlog_warn("yN3NBk0GHnLMKKZm ""bmp: missing TX OPEN message for peer %s",
				  peer->host);
		}
		if (bbpeer && bbpeer->open_rx)
			...
			stream_put(s, dummy_open, sizeof(dummy_open));
			zlog_warn("JgsQakzCRo8Jrxto ""bmp: missing RX OPEN message for peer %s",
				  peer->host);
		}
	...
	while (bmpbgp->mirror_qsize > bmpbgp->mirror_qsizelimit) {
		...
		frr_each(bmp_targets, &bmpbgp->targets, bt) {
			...
			frr_each(bmp_session, &bt->sessions, bmp) {
				...
				zlog_warn("mxADrzaJn1Q8olKN ""bmp[%s] lost mirror messages due to buffer size limit",
						bmp->remote);
...
static bool bmp_wrmirror(struct bmp *bmp, struct pullwr *pullwr)
{
	...
	if (!peer) {
		zlog_info("V537U5VDKW1xBJAO ""bmp: skipping mirror message for deleted peer");
		goto out;
	}
...
static bool bmp_wrsync(struct bmp *bmp, struct pullwr *pullwr)
{
	...
	if (bmp->syncafi == AFI_MAX) {
		FOREACH_AFI_SAFI (afi, safi) {
			...
			bmp->syncrdpos = NULL;
			zlog_info("qo1EPuCEwmjdAvUW ""bmp[%s] %s %s sending table",
					bmp->remote,
					afi2str(bmp->syncafi),
...
afibreak:
	...
	do {
		if (!bn) {
			...
			eor:
				zlog_info("n5Oe0sKhIW7GAbLd ""bmp[%s] %s %s table completed (EoR)",
...
static bool bmp_wrqueue(struct bmp *bmp, struct pullwr *pullwr)
{
	...
	peer = QOBJ_GET_TYPESAFE(bqe->peerid, peer);
	if (!peer) {
		zlog_info("s9WeimIaQP5Q1NvN ""bmp: skipping queued item for deleted peer");
		goto out;
	}
...
{
	if (eof)
		zlog_info("8p7P51d0YUryQejH ""bmp[%s] disconnected", bmp->remote);
	else
		flog_warn(EC_LIB_SYSTEM_CALL,"CRSDhb8LTj7ngmW2 " "bmp[%s] connection error: %s",
...
static void bmp_read(struct event *t)
{
	...
	if (n >= 1) {
		zlog_info("P5wI8eTFqxXi9QtY ""bmp[%s]: unexpectedly received %zu bytes", bmp->remote, n);
	} else if (n == 0) {
...
static struct bmp *bmp_open(struct bmp_targets *bt, int bmp_sock)
{
	...
	if (ret == FILTER_DENY) {
		bt->cnt_aclrefused++;
		zlog_info("252hXVwrOSLoj9GN ""bmp[%s] connection refused by access-list", buf);
		close(bmp_sock);
		return NULL;
	}
	bt->cnt_accept++;
	...
	if (setsockopt(bmp_sock, SOL_SOCKET, SO_KEEPALIVE, &on, sizeof(on)) < 0)
		flog_err(EC_LIB_SOCKET,"DwBJBiRwszeBpGvy " "bmp: %d can't setsockopt SO_KEEPALIVE: %s(%d)",
			 bmp_sock, safe_strerror(errno), errno);
	if (setsockopt(bmp_sock, IPPROTO_TCP, TCP_NODELAY, &on, sizeof(on)) < 0)
		flog_err(EC_LIB_SOCKET,"y0l5ADgBUqArZDCN " "bmp: %d can't setsockopt TCP_NODELAY: %s(%d)",
			 bmp_sock, safe_strerror(errno), errno);

	zlog_info("7IWK5sbV025AHoLE ""bmp[%s] connection established", buf);
...
static void bmp_accept(struct event *thread)
{
	...
	if (bmp_sock < 0) {
		zlog_info("O80f22nAfDjQAA7X ""bmp: accept_sock failed: %s", safe_strerror(errno));
		return;
	}
...
static void bmp_active_connect(struct bmp_active *ba)
{
	...
	for (; ba->addrpos < ba->addrtotal; ba->addrpos++) {
		if (ba->ifsrc) {
			...
			if (!ifp) {
				zlog_warn("BKIrIFXMtqHs0l1l ""bmp[%s]: failed to find interface",
					  ba->ifsrc);
				continue;
			}
			...
			if (bgp_update_address(ifp, &ba->addrs[ba->addrpos],
					       &ba->addrsrc)){
				zlog_warn("JgZSIlTsiCnJjsOy ""bmp[%s]: failed to find matching address",
					  ba->ifsrc);
				continue;
			}
			zlog_info("TfgNfuRupMkH9gxd ""bmp[%s]: selected source address : %pSU",
				  ba->ifsrc, &ba->addrsrc);
		}

		ba->socket = sockunion_socket(&ba->addrs[ba->addrpos]);
		if (ba->socket < 0) {
			zlog_warn("uK2praKJqkFlxRzW ""bmp[%s]: failed to create socket",
				  ba->hostname);
			continue;
		}
		...
		switch (res) {
		case connect_error:
			zlog_warn("Se3SUc8vdu9NZzcF ""bmp[%s]: failed to connect to %pSU:%d",
		...
		case connect_success:
			zlog_info("XL6pbtvi9tscuWFS ""bmp[%s]: connected to  %pSU:%d",
				  ba->hostname, &ba->addrs[ba->addrpos],
				  ba->port);
			break;
		case connect_in_progress:
			zlog_warn("jMHSP49taafFzzrV ""bmp[%s]: connect in progress  %pSU:%d",
				  ba->hostname, &ba->addrs[ba->addrpos],
				  ba->port);
			bmp_active_setup(ba);
			return;
				...
				int numaddrs, union sockunion *addr)
{
	...
	if (numaddrs <= 0) {
		zlog_warn("VRR6wqVuwS2QlQuH ""bmp[%s]: hostname resolution failed: %s",
			  ba->hostname, errstr);
		ba->last_err = errstr;
		ba->curretry += ba->curretry / 2;
		ba->addrpos = 0;
...
static void bmp_active_thread(struct event *t)
{
	...
	if (ret < 0 || status != 0) {
		...
		zlog_warn("684p595xDxCF1pQP ""bmp[%s]: failed to connect to %pSU:%d: %s",
			  ba->hostname, &ba->addrs[ba->addrpos], ba->port,
			  ba->last_err);
		goto out_next;
	}

	zlog_warn("eYNGFYx70LDTiw8I ""bmp[%s]: outbound connection to %pSU:%d", ba->hostname,
			    ...
			    struct bgp_path_info *new_route)
{
	...
	if (!updated_route) {
		zlog_warn("CZhZPf0pHUstNhVc ""%s: no updated route found!", __func__);
		return 0;
	}
