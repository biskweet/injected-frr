...
void bgp_option_norib_set_runtime(void)
{
	...
	zlog_info("lQgMGeDhsEVmOIaT ""Disabled BGP route installation to RIB (Zebra)");
	...
	zlog_info("dMnon7wv51h1s2jP ""All routes have been withdrawn from RIB (Zebra)");
}
...
void bgp_option_norib_unset_runtime(void)
{
	...
	zlog_info("ffU9uQnHKXSI2o1y ""Enabled BGP route installation to RIB (Zebra)");
	...
	zlog_info("HpoNMiO7LFd2rbDg ""All routes have been installed in RIB (Zebra)");
}
...
void bgp_suppress_fib_pending_set(struct bgp *bgp, bool set)
{
	...
	if (send_msg) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("ScSyBYf0IYIYCfvm ""Sending ZEBRA_ROUTE_NOTIFY_REQUEST");
...
int peer_af_delete(struct peer *peer, afi_t afi, safi_t safi)
{
	...
	if (PAF_SUBGRP(af)) {
		if (BGP_DEBUG(update_groups, UPDATE_GROUPS))
			zlog_debug("CQxfGMyLgdGC7XxG ""u%" PRIu64 ":s%" PRIu64 " remove peer %s",
...
int bgp_global_gr_init(struct bgp *bgp)
{
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("VWlOYZ46o4pLgOnO ""%s called ..", __func__);
...
{
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("PMDvzfe9y0lBGNPe ""%s called ..", __func__);
	...
	frr_each (if_connected, ifp->connected, ifc) {
		if (ifc->address && (ifc->address->family == AF_INET)) {
			...
			if (p.prefixlen == 30) {
				...
				zlog_debug("Xqnqigt5rjKiWB6w ""%s: IPv4 interface address is not /30 or /31, v4 session not started",
...
static int peer_activate_af(struct peer *peer, afi_t afi, safi_t safi)
{
	...
	if (CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP)) {
		flog_err(EC_BGP_PEER_GROUP,"STVn100QT9S7twVf " "%s was called for peer-group %s",
			 __func__, peer->host);
		return 1;
	}
...
{
	if (CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP)) {
		flog_err(EC_BGP_PEER_GROUP,"DdUt8sQk00ZVI6HS " "%s was called for peer-group %s",
			 __func__, peer->host);
		return true;
	}
	...
	if (peer_af_delete(peer, afi, safi) != 0) {
		flog_err(EC_BGP_PEER_DELETE,"KrzRzsQkRbdJEbyj "
...
void peer_nsf_stop(struct peer *peer)
{
	...
	if (peer->connection->t_gr_restart) {
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("pAmLRt0856vRqhy0 ""%pBP graceful restart timer stopped", peer);
	}
	if (peer->connection->t_gr_stale) {
		EVENT_OFF(peer->connection->t_gr_stale);
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("4HtxuYERMGZpAR39 "
			 ...
			 int as_type, const char *as_str)
{
	...
	for (ALL_LIST_ELEMENTS(group->peer, node, nnode, peer)) {
		if (((peer->as_type == AS_SPECIFIED) && peer->as != *as) ||
			...
			peer_as_change(peer, *as, as_type, as_str);
			if (bgp_debug_neighbor_events(peer))
				zlog_debug("2ALRw6Xu6s8xanO9 ""%s peer %s set to as_type %u curr status %s trigger BGP_Start",
			      ...
			      enum asnotation_mode asnotation)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA)) {
		if (inst_type == BGP_INSTANCE_TYPE_DEFAULT)
			zlog_debug("kR5M4aHsZJHc6cJ8 ""Creating Default VRF, AS %s",
				   bgp->as_pretty);
		else
			zlog_debug("5ZlX0lyhLQrbG0YJ ""Creating %s %s, AS %s",
	    ...
	    enum asnotation_mode asnotation)
{
	...
	*bgp_val = bgp;
	...
	if (IS_BGP_INST_KNOWN_TO_ZEBRA(bgp)) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("N5ODAkLBsX19He5r ""%s: Registering BGP instance %s to zebra",
...
int bgp_delete(struct bgp *bgp)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("DA8GP9xrnUJPlQKp ""Zebra Announce Fifo cleanup count before %u and after %u during BGP %s deletion",
	...
	if (BGP_DEBUG(zebra, ZEBRA)) {
		if (bgp->inst_type == BGP_INSTANCE_TYPE_DEFAULT)
			zlog_debug("d21ARILeQOxhFp4K ""Deleting Default VRF");
		else
			zlog_debug("fhFyoWpSF4wxlzJ8 ""Deleting %s %s",
				   (bgp->inst_type == BGP_INSTANCE_TYPE_VRF)
					   ? "VRF"
					   : "VIEW",
				   bgp->name);
...
struct peer *peer_lookup_dynamic_neighbor(struct bgp *bgp, union sockunion *su)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("90oXierVsbmC7QOI ""%s Dynamic Neighbor added, group %s count %d",
			   peer->host, group->name, dncount);

	if (dncount == gbgp->dynamic_neighbors_limit) {
		zlog_warn("NvH85Z7E52OUKZyo ""Dynamic Neighbor %s added as last connection. Peer-group %s reached maximum listen limit %d",
...
static void peer_drop_dynamic_neighbor(struct peer *peer)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("pOihFpiqgAQTYOt0 ""%s dropped from group %s, count %d", peer->host,
...
void bgp_shutdown_enable(struct bgp *bgp, const char *msg)
{
	...
	zlog_info("HIcGPnC7AAcuekme ""Enabled administrative shutdown on BGP instance AS %u",
		  bgp->as);
...
void bgp_shutdown_disable(struct bgp *bgp)
{
	...
	/* informational log message */
	zlog_info("mOxE8MBUdpMcfhI2 ""Disabled administrative shutdown on BGP instance AS %u",
		  bgp->as);
...
int peer_ttl_security_hops_set(struct peer *peer, int gtsm_hops)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("f6HeOmfcf2YEBQbJ ""%s: set gtsm_hops to %d for %s", __func__,
			   gtsm_hops, peer->host);
...
int peer_ttl_security_hops_unset(struct peer *peer)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("Qemk0aOKPl5q5OoH ""%s: set gtsm_hops to zero for %s", __func__,
			   peer->host);
...
static int peer_unshut_after_cfg(struct bgp *bgp)
{
	...
	for (ALL_LIST_ELEMENTS_RO(bgp->peer, node, peer)) {
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("0pYJyTdIsGOUGImP ""%s: released from config-pending hold",
				   peer->host);
...
void bgp_gr_apply_running_config(void)
{
	...
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("fxJ5RB4mzDnpPLSD ""[BGP_GR] %s called !", __func__);
