			   ...
			   struct peer *peer, struct stream *s)
{
	...
	frr_with_mutex (&connection->io_mtx) {
		...
		if (!holdtime) {
			...
			flog_err(
				EC_BGP_SENDQ_STUCK_PROPER,"RxZDrELRiLVllD2r "
			...
			flog_warn(
				EC_BGP_SENDQ_STUCK_WARN,"Rzb46hDHOuFG82yI "
					    ...
					    safi_t safi)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("qAch094hrqr0gyva ""send End-of-RIB for %s to %s",
...
void bgp_check_update_delay(struct bgp *bgp)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("svVMRUvEwp7yoZjV ""Checking update delay, T: %d R: %d I:%d E: %d",
...
void bgp_update_restarted_peers(struct peer *peer)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("SqFCVJpZbM4Sb9Ki ""Peer %s: Checking restarted", peer->host);
...
void bgp_update_implicit_eors(struct peer *peer)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("y2Gp1F172lvWablJ ""Peer %s: Checking implicit EORs", peer->host);
...
static void bgp_update_explicit_eors(struct peer *peer)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("DjKfHqYgJiyLU0fx ""Peer %s: Checking explicit EORs", peer->host);
...
void bgp_keepalive_send(struct peer *peer)
{
	...
	if (bgp_debug_keepalive(peer))
		zlog_debug("XlVt1OSZB6eO6JSo ""%s sending KEEPALIVE", peer->host);
			 ...
			 int capability_code, int action)
{
	...
	case CAPABILITY_CODE_SOFT_VERSION:
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("DUe19jXgRm1p2hjs ""%pBP sending CAPABILITY has %s %s for afi/safi: %s/%s",
				   peer,
	...
	case CAPABILITY_CODE_MP:
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("o53bNVdEMTmYsIhL ""%pBP sending CAPABILITY has %s %s for afi/safi: %s/%s",
				   peer,
				   action == CAPABILITY_ACTION_SET
					   ? "Advertising"
					   : "Removing",
	...
	case CAPABILITY_CODE_RESTART:
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("TAsZRBSwepcEwJ2j ""%pBP sending CAPABILITY has %s %s for afi/safi: %s/%s",
				   peer,
				   action == CAPABILITY_ACTION_SET
					   ? "Advertising"
					   : "Removing",
	...
	case CAPABILITY_CODE_LLGR:
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("8TwGZ9bKKoJeUDE4 ""%pBP sending CAPABILITY has %s %s for afi/safi: %s/%s",
				   peer,
				   action == CAPABILITY_ACTION_SET
					   ? "Advertising"
					   : "Removing",
	...
	case CAPABILITY_CODE_ADDPATH:
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("VcXT2KtrTppwmkI1 ""%pBP sending CAPABILITY has %s %s for afi/safi: %s/%s",
				   peer,
				   action == CAPABILITY_ACTION_SET
					   ? "Advertising"
					   : "Removing",
	...
	case CAPABILITY_CODE_PATHS_LIMIT:
		...
		FOREACH_AFI_SAFI (afi, safi) {
			...
			if (bgp_debug_neighbor_events(peer))
				zlog_debug("TbBxvghcHnJ0G5ld ""%pBP sending CAPABILITY has %s %s for afi/safi: %s/%s, limit: %u",
					   peer,
					   action == CAPABILITY_ACTION_SET
						   ? "Advertising"
						   : "Removing",
	...
	case CAPABILITY_CODE_ORF:
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("tHu564sSvmeLkPtP ""%pBP sending CAPABILITY has %s %s for afi/safi: %s/%s",
				   peer,
				   action == CAPABILITY_ACTION_SET
					   ? "Advertising"
					   : "Removing",
	...
	case CAPABILITY_CODE_FQDN:
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("WOoxuPnAaUcWsfRJ ""%pBP sending CAPABILITY has %s %s for afi/safi: %s/%s",
				   peer,
				   action == CAPABILITY_ACTION_SET
					   ? "Advertising"
					   : "Removing",
				...
				struct peer *new, struct in_addr remote_id)
{
	...
	if (ntohl(peer->local_id.s_addr) < ntohl(remote_id.s_addr)
		...
		if (ntohl(peer->local_id.s_addr) == ntohl(remote_id.s_addr)
		    && peer->local_as == peer->as)
			flog_err(EC_BGP_ROUTER_ID_SAME,"vjKyj2KAUVTtAPd0 "
			    ...
			    struct peer *peer, bgp_size_t size)
{
	...
	if (optlen == BGP_OPEN_NON_EXT_OPT_LEN
		...
		if (STREAM_READABLE(peer->curr) < 1) {
			flog_err(
				EC_BGP_PKT_OPEN,"LegehIqcL6GWnYUb "
		...
		if (opttype == BGP_OPEN_NON_EXT_OPT_TYPE_EXTENDED_LENGTH) {
			if (STREAM_READABLE(peer->curr) < 2) {
				flog_err(
					EC_BGP_PKT_OPEN,"rksslPJ75ALeSdhb "
	...
	if (optlen != 0) {
		...
		if (STREAM_READABLE(peer->curr) < optlen) {
			flog_err(EC_BGP_PKT_OPEN,"x3Gc2BNxHmcnrKkq "
	...
	if (CHECK_FLAG(peer->cap, PEER_CAP_AS4_RCV) && !as4) {
		flog_err(EC_BGP_PKT_OPEN,"aJGi2E70SOBPOMlo "
	...
	if (remote_as == BGP_AS_ZERO) {
		flog_err(EC_BGP_PKT_OPEN,"6M5SjYuMW7D6VLdH " "%s bad OPEN, got AS set to 0",
			 peer->host);
		bgp_notify_send(connection, BGP_NOTIFY_OPEN_ERR,
				BGP_NOTIFY_OPEN_BAD_PEER_AS);
		return BGP_Stop;
	...
	if (remote_as == BGP_AS_TRANS) {
		...
		if (as4 == BGP_AS_TRANS) {
			flog_err(
				EC_BGP_PKT_OPEN,"LTl4UJXNkOrrCaNA "
		...
		if (CHECK_FLAG(peer->cap, PEER_CAP_AS4_RCV)
			...
			flog_err(
				EC_BGP_PKT_OPEN,"mlLW4J9amDVMqJai "
	...
	if (remote_id.s_addr == INADDR_ANY
		...
		&& ntohl(peer->local_id.s_addr) == ntohl(remote_id.s_addr))) {
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("MSI36pi2zyQL5NhQ ""%s bad OPEN, wrong router identifier %pI4",
				   peer->host, &remote_id);
		bgp_notify_send_with_data(connection, BGP_NOTIFY_OPEN_ERR,
	...
	if (peer->as_type == AS_UNSPECIFIED) {
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("FUs0F07Dm81GqWjF ""%s bad OPEN, remote AS is %u, expected %u",
	...
	if (bgp_getsockname(peer) < 0) {
		flog_err_sys(EC_LIB_SOCKET,"3t9GWIg0y1pHLVA1 "
	...
	if (optlen != 0) {
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("MDWKHyQTkKhLbEtI ""%s rcvd OPEN w/ OPTION parameter len: 0",
				   peer->host);
	}
	...
	if (peer->afc_nego[AFI_IP][SAFI_UNICAST]
		...
		if (peer->nexthop.v4.s_addr == INADDR_ANY) {
			...
			zlog_warn("2OGxwe5qIT9Z69LC ""%s: No local IPv4 addr, BGP routing may not work",
				  peer->host);
#endif
		}
	}
	if (peer->afc_nego[AFI_IP6][SAFI_UNICAST]
		...
		if (IN6_IS_ADDR_UNSPECIFIED(&peer->nexthop.v6_global) &&
			...
			flog_err(EC_BGP_SND_FAIL,"xMCrYExVNscr1I3E "
...
{
	if (bgp_debug_keepalive(peer))
		zlog_debug("1LonJteosIjHXfmh ""%s KEEPALIVE rcvd", peer->host);
	...
	if (peer->rtt > peer->rtt_expected) {
		...
		if (peer->rtt_keepalive_rcv > peer->rtt_keepalive_conf) {
			...
			snprintfrr(
				...
				peer->rtt_keepalive_rcv);
			zlog_warn("Iu6kTDeIwRiNo3ZD ""%s %s", peer->host, rtt_shutdown_reason);
			      ...
			      struct peer *peer, bgp_size_t size)
{
	...
	if (!peer_established(connection)) {
		flog_err(EC_BGP_INVALID_STATUS,"LLGJ83trTxtOxLz9 "
	...
	if (stream_pnt(s) + 2 > end) {
		flog_err(EC_BGP_UPDATE_RCV,"XMeScakQIkiWUsl4 "
	...
	if (stream_pnt(s) + withdraw_len > end) {
		flog_err(EC_BGP_UPDATE_RCV,"LtfuEgcEw8f91af2 "
	...
	if (stream_pnt(s) + 2 > end) {
		flog_warn(
			EC_BGP_UPDATE_PACKET_SHORT,"auXIGPTeXs38JkCL "
	...
	if (stream_pnt(s) + attribute_len > end) {
		flog_warn(
			EC_BGP_UPDATE_PACKET_LONG,"5qXfrjDleVeUBhwb "
...
#define NLRI_ATTR_ARG (attr_parse_ret != BGP_ATTR_PARSE_WITHDRAW ? &attr : NULL)
	...
	if (attr_parse_ret == BGP_ATTR_PARSE_WITHDRAW
		...
		if (attr_parse_ret == BGP_ATTR_PARSE_WITHDRAW)
			flog_err(
				EC_BGP_UPDATE_RCV,"lxwV0pJmAZTXuAxU "
		...
		if (ret && bgp_debug_update(peer, NULL, NULL, 1) &&
			...
			zlog_debug("WxD8n1UCzT4YUDmp ""%pBP rcvd UPDATE w/ attr: %s", peer,
				   peer->rcvd_attr_str);
	...
	if (BGP_DEBUG(update, UPDATE_IN) && BGP_DEBUG(update, UPDATE_DETAIL))
		zlog_debug("1VjpOlgcPp9SH6Lb ""%pBP rcvd UPDATE wlen %d attrlen %d alen %d", peer,
	...
	for (int i = NLRI_UPDATE; i < NLRI_TYPE_MAX; i++) {
		...
		if (nlri_ret < BGP_NLRI_PARSE_OK
			...
			flog_err(EC_BGP_UPDATE_RCV,"siO8exZiAgx3dHq8 "
				     ...
				     struct peer *peer, bgp_size_t size)
{
	...
	if (!CHECK_FLAG(peer->cap, PEER_CAP_REFRESH_ADV)) {
		flog_err(EC_BGP_NO_CAP,"z8JCJrdxv3EBxmv4 "
	...
	/* Status must be Established. */
	if (!peer_established(connection)) {
		flog_err(EC_BGP_INVALID_STATUS,"CYEv9lIUSrgP8uHg "
	...
	if (size != BGP_MSG_ROUTE_REFRESH_MIN_SIZE - BGP_HEADER_SIZE) {
		...
		if (subtype) {
			 ...
			 */
			if (subtype > 2)
				zlog_err("IDE8UkJxJvjelCIq "
					"%s Enhanced Route Refresh invalid subtype",
					peer->host);
		}

		if (msg_length < 5) {
			zlog_info("qpxZHFWpXFeVUERZ ""%s ORF route refresh length error",
				  peer->host);
			bgp_notify_send(connection, BGP_NOTIFY_CEASE,
					BGP_NOTIFY_SUBCODE_UNSPECIFIC);
			return BGP_Stop;
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("G6VpJpWYUTuqBt9l ""%pBP rcvd Refresh %s ORF request", peer,
					   ...
					   struct peer *peer)
{
	...
	if (action == CAPABILITY_ACTION_SET) {
		if (len % CAPABILITY_CODE_ADDPATH_LEN) {
			flog_warn(EC_BGP_CAPABILITY_INVALID_LENGTH,"8DTbjUQGE3jj0q3E "
		...
		while (data + CAPABILITY_CODE_ADDPATH_LEN <= end) {
			...
			if (!bac.flags || bac.flags > 3) {
				flog_warn(EC_BGP_CAPABILITY_INVALID_LENGTH,"dy6tFu0mgW45nsjP "
			...
			if (bgp_debug_neighbor_events(peer))
				zlog_debug("kOQSwB4lhimDeTdH ""%s OPEN has %s capability for afi/safi: %s/%s%s%s",
					   peer->host,
			...
			if (bgp_map_afi_safi_iana2int(pkt_afi, pkt_safi, &afi,
						      &safi)) {
				if (bgp_debug_neighbor_events(peer))
					zlog_debug("QkIo4arkb6wyZdgE ""%s Addr-family %s/%s(afi/safi) not supported. Ignore the Addpath Attribute for this AFI/SAFI",
						   peer->host,
						   iana_afi2str(pkt_afi),
						   iana_safi2str(pkt_safi));
				goto ignore;
				...
				if (bgp_debug_neighbor_events(peer))
					zlog_debug("FaGycXRQk0Cxn3dp ""%s Addr-family %s/%s(afi/safi) not enabled. Ignore the AddPath capability for this AFI/SAFI",
						   peer->host,
						   iana_afi2str(pkt_afi),
						   iana_safi2str(pkt_safi));
				goto ignore;
					       ...
					       struct peer *peer)
{
	...
	if (action == CAPABILITY_ACTION_SET) {
		if (len % CAPABILITY_CODE_PATHS_LIMIT_LEN) {
			flog_warn(EC_BGP_CAPABILITY_INVALID_LENGTH,"W6rRBMMDPT4GOV9x "
		...
		if (!CHECK_FLAG(peer->cap, PEER_CAP_ADDPATH_RCV)) {
			flog_warn(EC_BGP_CAPABILITY_INVALID_DATA,"IcNBBeWVk9z9yImG "
		...
		while (data + CAPABILITY_CODE_PATHS_LIMIT_LEN <= end) {
			...
			if (bgp_debug_neighbor_events(peer))
				zlog_debug("szcOsAcM97tgySIE ""%s OPEN has %s capability for afi/safi: %s/%s limit: %u",
					   peer->host,
					   lookup_msg(capcode_str, hdr->code,
						      NULL),
					   iana_afi2str(pkt_afi),
			...
			if (bgp_map_afi_safi_iana2int(pkt_afi, pkt_safi, &afi,
						      &safi)) {
				if (bgp_debug_neighbor_events(peer))
					zlog_debug("I40AuJlbFGFDDZDS ""%s Addr-family %s/%s(afi/safi) not supported. Ignore the Paths-Limit capability for this AFI/SAFI",
						   peer->host,
						   iana_afi2str(pkt_afi),
						   iana_safi2str(pkt_safi));
				goto ignore;
				...
				if (bgp_debug_neighbor_events(peer))
					zlog_debug("cInVlqAwaJUfZnpw ""%s Addr-family %s/%s(afi/safi) not enabled. Ignore the Paths-Limit capability for this AFI/SAFI",
						   peer->host,
						   iana_afi2str(pkt_afi),
						   iana_safi2str(pkt_safi));
				goto ignore;
				       ...
				       struct peer *peer)
{
	...
	if (data + CAPABILITY_CODE_ORF_LEN > end) {
		flog_warn(EC_BGP_CAPABILITY_INVALID_LENGTH,"koEc4X9JEW769dJZ "
	...
	if (bgp_map_afi_safi_iana2int(pkt_afi, pkt_safi, &afi, &safi)) {
		zlog_info("69xhPztpwkrdRKYn ""%pBP Addr-family %d/%d not supported. Ignoring the ORF capability",
	...
	if (CAPABILITY_CODE_ORF_LEN + (num * 2) > hdr->length) {
		zlog_info("4V36Bhbye3qpxwn3 ""%pBP ORF Capability entry length error, Cap length %u, num %u",
			  peer, hdr->length, num);
		return;
	}
	...
	for (i = 0; i < num; i++) {
		if (data + 1 > end) {
			flog_err(EC_BGP_CAPABILITY_INVALID_LENGTH,"5cY3uxrGpHXWrbJP "
		...
		if (data + 1 > end) {
			flog_err(EC_BGP_CAPABILITY_INVALID_LENGTH,"tIoPI1vuOBbLlYdd "
		...
		default:
			if (bgp_debug_neighbor_events(peer))
				zlog_debug("uIFPFGh82T6LoVkj ""%pBP Addr-family %d/%d has ORF type/mode %d/%d not supported",
		...
		if (!((afi == AFI_IP && safi == SAFI_UNICAST) ||
			...
			if (bgp_debug_neighbor_events(peer))
				zlog_debug("5m46IdbxkvivaTtm ""%pBP Addr-family %d/%d unsupported AFI/SAFI received",
					   peer, afi, safi);
			continue;
		}

		if (bgp_debug_neighbor_events(peer))
			zlog_debug("7oiM1q0IU5zoMIde ""%pBP OPEN has %s ORF capability as %s for afi/safi: %s/%s",
					...
					struct peer *peer)
{
	...
	if (action == CAPABILITY_ACTION_SET) {
		/* hostname */
		if (data + 1 >= end) {
			zlog_err("81Qf1Q04hMO8UKen ""%pBP: Received invalid FQDN capability (host name length)",
				 peer);
			return;
		}
		...
		len = *data;
		if (data + len + 1 > end) {
			zlog_err("fgxLqkzMdicoCgy5 ""%pBP: Received invalid FQDN capability length (host name) %d",
				 peer, hdr->length);
			return;
		}
		data++;
		...
		if (data + 1 >= end) {
			zlog_err("n7JaZTbuWqNvrnx4 ""%pBP: Received invalid FQDN capability (domain name length)",
				 peer);
			return;
		}
		...
		len = *data;
		if (data + len + 1 > end) {
			zlog_err("fuTnrJ4dhV2Hetmz ""%pBP: Received invalid FQDN capability length (domain name) %d",
				 peer, len);
			return;
		}
		data++;
					...
					struct peer *peer)
{
	...
	if (action == CAPABILITY_ACTION_SET) {
		if (len < BGP_CAP_LLGR_MIN_PACKET_LEN) {
			zlog_err("now4w1RejsV54ALG ""%pBP: Received invalid Long-Lived Graceful-Restart capability length %zu",
				 peer, len);
			return;
		}
		...
		while (data + BGP_CAP_LLGR_MIN_PACKET_LEN <= end) {
			...
			if (bgp_map_afi_safi_iana2int(pkt_afi, pkt_safi, &afi,
						      &safi)) {
				if (bgp_debug_neighbor_events(peer))
					zlog_debug("KxscPk3sfJeWo55g ""%s Addr-family %s/%s(afi/safi) not supported. Ignore the Long-lived Graceful Restart capability for this AFI/SAFI",
						   peer->host,
						   iana_afi2str(pkt_afi),
						   iana_safi2str(pkt_safi));
			} else if (!peer->afc[afi][safi] ||
				...
				if (bgp_debug_neighbor_events(peer))
					zlog_debug("3oZLiPXNjBK8tQ4e ""%s Addr-family %s/%s(afi/safi) not enabled. Ignore the Long-lived Graceful Restart capability",
						   peer->host,
						   iana_afi2str(pkt_afi),
						   iana_safi2str(pkt_safi));
			} else {
				if (bgp_debug_neighbor_events(peer))
					zlog_debug("VNfK4yQBbiMezjYI ""%s Addr-family %s/%s(afi/safi) Long-lived Graceful Restart capability stale time %u sec",
						   peer->host,
						   iana_afi2str(pkt_afi),
						   iana_safi2str(pkt_safi),
						   stale_time);
...
#define GRACEFUL_RESTART_CAPABILITY_PER_AFI_SAFI_SIZE 4
	...
	if (action == CAPABILITY_ACTION_SET) {
		if (len < sizeof(gr_restart_flag_time)) {
			zlog_err("BiGHR0kSiYAwkIvn ""%pBP: Received invalid Graceful-Restart capability length %d",
				 peer, hdr->length);
			return;
		}
		...
		while (data + GRACEFUL_RESTART_CAPABILITY_PER_AFI_SAFI_SIZE <=
			...
			if (bgp_map_afi_safi_iana2int(pkt_afi, pkt_safi, &afi,
						      &safi)) {
				if (bgp_debug_neighbor_events(peer))
					zlog_debug("MdU96b3R6b5anTEr ""%pBP: Addr-family %s/%s(afi/safi) not supported. Ignore the Graceful Restart capability for this AFI/SAFI",
						   peer, iana_afi2str(pkt_afi),
						   iana_safi2str(pkt_safi));
			} else if (!peer->afc[afi][safi]) {
				if (bgp_debug_neighbor_events(peer))
					zlog_debug("mFsBdLOOkoB1IVFV ""%pBP: Addr-family %s/%s(afi/safi) not enabled. Ignore the Graceful Restart capability",
						   peer, iana_afi2str(pkt_afi),
						   iana_safi2str(pkt_safi));
			} else {
				if (bgp_debug_neighbor_events(peer))
					zlog_debug("9f6yio8H8BFXQgwr ""%pBP: Address family %s is%spreserved",
						   peer,
						    ...
						    struct peer *peer)
{
	...
	if (action == CAPABILITY_ACTION_SET) {
		if (data + len + 1 > end) {
			zlog_err("4RuGMV798Le5r47O ""%pBP: Received invalid Software Version capability length %d",
				 peer, len);
			return;
		}
		data++;
				    ...
				    bgp_size_t length)
{
	...
	while (pnt < end) {
		 ...
		 * length. */
		if (pnt + 3 > end) {
			zlog_err("D3XaZC9BHGXGxr0S ""%pBP: Capability length error", peer);
		...
		if (action != CAPABILITY_ACTION_SET
			...
			zlog_err("OPBRKilNN7DgDqDf ""%pBP: Capability Action Value error %d", peer,
				 action);
			bgp_notify_send(peer->connection, BGP_NOTIFY_CEASE,
					BGP_NOTIFY_SUBCODE_UNSPECIFIC);
			goto done;
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("lDewiqCsfQidOoOU ""%pBP: CAPABILITY has action: %d, code: %u, length %u",
				   peer, action, hdr->code, hdr->length);

		/* Capability length check. */
		if ((pnt + hdr->length + 3) > end) {
			zlog_err("UrNeq9q8Ofy8RIMQ ""%pBP: Capability length error", peer);
			bgp_notify_send(peer->connection, BGP_NOTIFY_CEASE,
					BGP_NOTIFY_SUBCODE_UNSPECIFIC);
			/*
			 * If we did not return then
		...
		case CAPABILITY_CODE_PATHS_LIMIT:
			if (hdr->length < cap_minsizes[hdr->code]) {
				zlog_info("rx9FRw0WisrLPI8E ""%pBP: %s Capability length error: got %u, expected at least %u",
			...
			if (hdr->length &&
				...
				zlog_info("625KA2z1ngdQKy6L ""%pBP %s Capability length error: got %u, expected a multiple of %u",
					  peer, capability, hdr->length,
		...
		case CAPABILITY_CODE_MP:
			if (hdr->length < sizeof(struct capability_mp_data)) {
				zlog_err("9hNSGqber3rV2Lzw ""%pBP: Capability (%s) structure is not properly filled out, expected at least %zu bytes but header length specified is %d",
					 peer, capability,
			...
			if (bgp_map_afi_safi_iana2int(pkt_afi, pkt_safi, &afi,
						      &safi)) {
				if (bgp_debug_neighbor_events(peer))
					zlog_debug("I7aqymMFer5iO7Uh ""%pBP: Dynamic Capability %s afi/safi invalid (%s/%s)",
						   peer, capability,
						   iana_afi2str(pkt_afi),
						   iana_safi2str(pkt_safi));
				goto done;
			...
			if (bgp_debug_neighbor_events(peer))
				zlog_debug("cnnHk5TH9B5SIAbp ""%pBP: CAPABILITY has %s %s CAP for afi/safi: %s/%s",
					   peer,
					   action == CAPABILITY_ACTION_SET
						   ? "Advertising"
						   : "Removing",
		...
		case CAPABILITY_CODE_ROLE:
			...
			break;
		default:
			flog_warn(EC_BGP_UNRECOGNIZED_CAPABILITY,"XptCfCw8rWc1KEaZ "
			   ...
			   struct peer *peer, bgp_size_t size)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("RqhMu15usfTSjzz0 ""%s rcv CAPABILITY", peer->host);

	if (!CHECK_FLAG(peer->cap, PEER_CAP_DYNAMIC_ADV) ||
	    !CHECK_FLAG(peer->cap, PEER_CAP_DYNAMIC_RCV)) {
		flog_err(EC_BGP_NO_CAP,"PUnp9U1o8RjJfw5m "
	...
	/* Status must be Established. */
	if (!peer_established(connection)) {
		flog_err(EC_BGP_NO_CAP,"EBHmQ9UktrXhYuyI "
...
void bgp_process_packet(struct event *thread)
{
	...
	while (processed < rpkt_quanta_old) {
		...
		case BGP_MSG_OPEN:
			...
			if (mprc == BGP_Stop)
				flog_err(
					EC_BGP_PKT_OPEN,"ZacLSyw9CTPLbB8B "
		...
		case BGP_MSG_UPDATE:
			...
			if (mprc == BGP_Stop)
				flog_err(
					EC_BGP_UPDATE_RCV,"pjYHdgW5sFkBVFkw "
		...
		case BGP_MSG_NOTIFY:
			...
			if (mprc == BGP_Stop)
				flog_err(
					EC_BGP_NOTIFY_RCV,"lYVVhqemNiZw16v0 "
		...
		case BGP_MSG_KEEPALIVE:
			...
			if (mprc == BGP_Stop)
				flog_err(
					EC_BGP_KEEP_RCV,"9ZCi6JIVblciIas1 "
		...
		case BGP_MSG_ROUTE_REFRESH_OLD:
			...
			if (mprc == BGP_Stop)
				flog_err(
					EC_BGP_RFSH_RCV,"1YqkDEIw79VvmuTt "
		...
		case BGP_MSG_CAPABILITY:
			...
			if (mprc == BGP_Stop)
				flog_err(
					EC_BGP_CAP_RCV,"66AQyVXbOBQInxnm "
...
void bgp_packet_process_error(struct event *thread)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("HM2PcuHtjQYf9O1a ""%s [Event] BGP error %d on fd %d", peer->host, code,
			   connection->fd);
