	...
	while (length) {
		...
		if (tlv_type == BGP_AIGP_TLV_METRIC) {
			...
			if (*aigp == BGP_AIGP_TLV_METRIC_MAX) {
				zlog_err("FfY5s1WUPfwMKHrX ""Bad AIGP TLV (%s) length: %llu",
...
static bool bgp_attr_aigp_valid(uint8_t *pnt, int length)
{
	...
	if (length < 3) {
		zlog_err("RfjJ65HLTuLHbMKj ""Bad AIGP attribute length (MUST be minimum 3): %u",
			 length);
		return false;
	}
	...
	while (length) {
		...
		if (tlv_length < 3) {
			zlog_err("leJv32Eywg605mKC ""Bad AIGP TLV length (MUST be minimum 3): %u",
				 tlv_length);
			return false;
		}
		...
		if (tlv_type == BGP_AIGP_TLV_METRIC &&
			...
			zlog_err("hLMxuIAvhEHV9bWn ""Bad AIGP TLV (%s) length: %u",
		   ...
		   bgp_size_t length)
{
	...
	if (bgp_debug_update(peer, NULL, NULL, 1)) {
		...
		zlog_debug("AnLxQ8eJl6FQ1oIo ""%s: attributes: %s", __func__, attr_str);
	}
	...
	default:
		 ...
		 */
		flog_err(EC_BGP_ATTR_FLAG,"pd2VtUuhP3Enw0rK "
...
)
{
	...
	for (i = 0; i <= 2; i++) /* O,T,P, but not E */
		if (CHECK_FLAG(desired_flags, attr_flag_str[i].key)
			...
			flog_err(EC_BGP_ATTR_FLAG,"HGdlPa11hsHSpi87 "
...
static bool bgp_attr_flag_invalid(struct bgp_attr_parser_args *args)
{
	...
	if (!CHECK_FLAG(BGP_ATTR_FLAG_OPTIONAL, flags)
		...
		flog_err(
			EC_BGP_ATTR_FLAG,"8COcO6P0OvNSYiBO "
	...
	if (CHECK_FLAG(flags, BGP_ATTR_FLAG_PARTIAL)) {
		if (!CHECK_FLAG(flags, BGP_ATTR_FLAG_OPTIONAL)) {
			flog_err(EC_BGP_ATTR_FLAG,"Gx4TBgO2SZZuqyEv "
		...
		if (CHECK_FLAG(flags, BGP_ATTR_FLAG_OPTIONAL)
			...
			flog_err(EC_BGP_ATTR_FLAG,"8YmECNohEoydBhG5 "
...
bgp_attr_origin(struct bgp_attr_parser_args *args)
{
	...
	/* If any recognized attribute has Attribute Length that conflicts
	   ...
	   value). */
	if (length != 1) {
		flog_err(EC_BGP_ATTR_LEN,"iYhm4aT97LX5Cat3 "
	...
	if ((attr->origin != BGP_ORIGIN_IGP) && (attr->origin != BGP_ORIGIN_EGP)
		...
		flog_err(EC_BGP_ATTR_ORIGIN,"5MPwxqeZLe047sxu "
...
static int bgp_attr_aspath(struct bgp_attr_parser_args *args)
{
	...
	if (!attr->aspath) {
		flog_err(EC_BGP_ATTR_MAL_AS_PATH,"cixSNHocs1GBQjmi "
	...
	if (peer->bgp && peer->bgp->reject_as_sets &&
		...
		flog_err(EC_BGP_ATTR_MAL_AS_PATH,"Qn1iRxclWnRGryAb "
						     ...
						     struct attr *const attr)
{
	...
	if ((peer->sort == BGP_PEER_CONFED
		...
		flog_err(EC_BGP_ATTR_MAL_AS_PATH,"0FuUvMcbXN0Uusec " "Malformed AS path from %s",
			 peer->host);
		return BGP_ATTR_PARSE_WITHDRAW;
	}
	...
	if (CHECK_FLAG(peer->flags, PEER_FLAG_ENFORCE_FIRST_AS)) {
		if (peer->sort == BGP_PEER_EBGP
			...
			flog_err(EC_BGP_ATTR_FIRST_AS,"m5jIxXZVSW2lqsHr "
	...
	if (peer->sort == BGP_PEER_EBGP && aspath_check_as_zero(attr->aspath)) {
		flog_err(
			EC_BGP_ATTR_MAL_AS_PATH,"PRIA3aNQIkcXVmjf "
			     ...
			     struct aspath **as4_path)
{
	...
	*as4_path = aspath_parse(peer->curr, length, 1, asnotation);
	...
	/* In case of IBGP, length will be zero. */
	if (!*as4_path) {
		flog_err(EC_BGP_ATTR_MAL_AS_PATH,"YWkRxOuj8OJ2NJZG "
	...
	if (peer->bgp->reject_as_sets && aspath_check_as_sets(attr->aspath)) {
		flog_err(EC_BGP_ATTR_MAL_AS_PATH,"KEUBoAnqNgnXtzBl "
			 "AS_SET and AS_CONFED_SET are deprecated from %pBP",
			 peer);
		return bgp_attr_malformed(args, BGP_NOTIFY_UPDATE_MAL_AS_PATH,
					  0);
					       ...
					       struct attr *attr)
{
	...
	if (ipv4_martian(&attr->nexthop) && !bgp->allow_martian) {
		...
		flog_err(EC_BGP_ATTR_MARTIAN_NH,"YzywnK0UjBqzcUDx " "Martian nexthop %pI4",
			 &attr->nexthop);
...
bgp_attr_nexthop(struct bgp_attr_parser_args *args)
{
	...
	if (length != 4) {
		flog_err(EC_BGP_ATTR_LEN,"goltu5OA4vhLbyj4 "
...
static enum bgp_attr_parse_ret bgp_attr_med(struct bgp_attr_parser_args *args)
{
	...
	/* Length check. */
	if (length != 4) {
		flog_err(EC_BGP_ATTR_LEN,"ilUxmWtickrviFTY "
...
bgp_attr_local_pref(struct bgp_attr_parser_args *args)
{
	...
	if ((peer->sort == BGP_PEER_IBGP ||
	    ...
	    length != 4) {
		flog_err(EC_BGP_ATTR_LEN,"HnG6ohDVqMMMoBtg "
...
static int bgp_attr_atomic(struct bgp_attr_parser_args *args)
{
	...
	/* Length check. */
	if (length != 0) {
		flog_err(EC_BGP_ATTR_LEN,"wqGyRvhR0HNURKko "
...
static int bgp_attr_aggregator(struct bgp_attr_parser_args *args)
{
	...
	if (length != wantedlen) {
		flog_err(EC_BGP_ATTR_LEN,"qIEjPgleZ76GhyUn "
	...
	if (aggregator_as == BGP_AS_ZERO) {
		flog_err(EC_BGP_ATTR_LEN,"ewnK9Ky90L86jubS "
		...
		if (bgp_debug_update(peer, NULL, NULL, 1)) {
			...
			zlog_debug("FChxL0nYVf30Z1Ge ""%s: attributes: %s", __func__, attr_str);
		}
	} else {
		attr->flag |= ATTR_FLAG_BIT(BGP_ATTR_AGGREGATOR);
	}
			...
			struct in_addr *as4_aggregator_addr)
{
	...
	if (length != 8) {
		flog_err(EC_BGP_ATTR_LEN,"exrkw9H1EisuHSc1 " "New Aggregator length is not 8 [%d]",
			 length);
		return bgp_attr_malformed(args, BGP_NOTIFY_UPDATE_ATTR_LENG_ERR,
					  0);
	}
	...
	*as4_aggregator_as = aggregator_as;
	...
	/* Codification of AS 0 Processing */
	if (aggregator_as == BGP_AS_ZERO) {
		flog_err(EC_BGP_ATTR_LEN,"uRHD3hZgF0Ek8Qa6 "
		...
		if (bgp_debug_update(peer, NULL, NULL, 1)) {
			...
			zlog_debug("tcXlDMU1Gij9BXor ""%s: attributes: %s", __func__, attr_str);
		}
	} else {
	...
	if (CHECK_FLAG(peer->cap, PEER_CAP_AS4_RCV)) {
		...
		if (BGP_DEBUG(as4, AS4)) {
			if (attr->flag & (ATTR_FLAG_BIT(BGP_ATTR_AS4_PATH)))
				zlog_debug("Hemwx0nsynEIIzGY ""[AS4] %s %s AS4_PATH", peer->host,
					   "AS4 capable peer, yet it sent");

			if (attr->flag
			    & (ATTR_FLAG_BIT(BGP_ATTR_AS4_AGGREGATOR)))
				zlog_debug("VzaipPcgEyj5T7lD ""[AS4] %s %s AS4_AGGREGATOR",
					   peer->host,
					   "AS4 capable peer, yet it sent");
		}
...
bgp_attr_originator_id(struct bgp_attr_parser_args *args)
{
	 ...
	 */
	if (length != 4) {
		flog_err(EC_BGP_ATTR_LEN,"FCXrZB1FqrBLI2hX " "Bad originator ID length %d",
			 length);
...
bgp_attr_cluster_list(struct bgp_attr_parser_args *args)
{
	...
	if (length == 0 || length % 4) {
		flog_err(EC_BGP_ATTR_LEN,"1RVUJn9EZkRga6XP " "Bad cluster list length %d", length);
...
#define LEN_LEFT	(length - (stream_get_getp(s) - start))
	if ((length > STREAM_READABLE(s)) || (length < BGP_MP_REACH_MIN_SIZE)) {
		zlog_info("kd2BsJT7LKSKUwzi ""%s: %s sent invalid length, %lu, of MP_REACH_NLRI",
	...
	case 0:
		if (safi != SAFI_FLOWSPEC) {
			zlog_info("a2VviNUPh91Gp2UA ""%s: %s sent wrong next-hop length, %d, in MP_REACH_NLRI",
				  __func__, peer->host, attr->mp_nexthop_len);
			return BGP_ATTR_PARSE_ERROR_NOTIFYPLS;
		}
		break;
	...
	case BGP_ATTR_NHLEN_VPNV6_GLOBAL:
		...
		if (IN6_IS_ADDR_LINKLOCAL(&attr->mp_nexthop_global)) {
			if (!peer->nexthop.ifp) {
				zlog_warn("5moLsreaXCMBU5j0 ""%s sent a v6 global attribute but address is a V6 LL and there's no peer interface information. Hence, withdrawing",
					  peer->host);
				return BGP_ATTR_PARSE_WITHDRAW;
			}
	...
	case BGP_ATTR_NHLEN_VPNV6_GLOBAL_AND_LL:
		...
		if (IN6_IS_ADDR_LINKLOCAL(&attr->mp_nexthop_global)) {
			if (!peer->nexthop.ifp) {
				zlog_warn("DiwLKUS2D44MTktW ""%s sent a v6 global and LL attribute but global address is a V6 LL and there's no peer interface information. Hence, withdrawing",
					  peer->host);
				return BGP_ATTR_PARSE_WITHDRAW;
			}
			attr->nh_ifindex = peer->nexthop.ifp->ifindex;
		...
		if (!peer->nexthop.ifp) {
			zlog_warn("Z1Nz1PZniJCANusF ""%s sent a v6 LL next-hop and there's no peer interface information. Hence, withdrawing",
				  peer->host);
			return BGP_ATTR_PARSE_WITHDRAW;
		}
		...
		break;
	default:
		zlog_info("9GT1vxFyTtSGbg3Q ""%s: %s sent wrong next-hop length, %d, in MP_REACH_NLRI",
			  __func__, peer->host, attr->mp_nexthop_len);
		return BGP_ATTR_PARSE_ERROR_NOTIFYPLS;
	}

	if (!LEN_LEFT) {
		zlog_info("q3thLVODDSkp6u48 ""%s: %s sent SNPA which couldn't be read",
			  __func__, peer->host);
		return BGP_ATTR_PARSE_ERROR_NOTIFYPLS;
	}

	{
		...
		if ((val = stream_getc(s)))
			flog_warn(
				EC_BGP_DEFUNCT_SNPA_LEN,"RFEWsrS0IAyLIZoH "
	...
	if (nlri_len > STREAM_READABLE(s)) {
		zlog_info("zdPkbfHsCB2HYuau ""%s: %s sent MP_REACH_NLRI which couldn't be read",
			  __func__, peer->host);
		return BGP_ATTR_PARSE_ERROR_NOTIFYPLS;
	}

	if (!nlri_len) {
		zlog_info("OhCtWyaMK9hj7lUS ""%s: %s sent a zero-length NLRI. Hence, treating as a EOR marker",
			  __func__, peer->host);
...
bgp_attr_ext_communities(struct bgp_attr_parser_args *args)
{
	...
	if (bgp_attr_rmac(attr, &attr->rmac)) {
		if (bgp_debug_update(peer, NULL, NULL, 1)
			...
			zlog_debug("HXoKlwyUKKyCipzz ""%s: router mac %pEA is self mac", __func__,
				   &attr->rmac);
	}
...
static int bgp_attr_encap(struct bgp_attr_parser_args *args)
{
	...
	if (!CHECK_FLAG(flag, BGP_ATTR_FLAG_TRANS)
		...
		zlog_err("bGfESSgXPOeRXEFp ""Tunnel Encap attribute flag isn't optional and transitive %d",
			 flag);
		return bgp_attr_malformed(args, BGP_NOTIFY_UPDATE_OPT_ATTR_ERR,
					  args->total);
	}
	...
	if (BGP_ATTR_ENCAP == type) {
		...
		if (tlv_length != length) {
			zlog_info("BdzI71ZQGXDxEgRY ""%s: tlv_length(%d) != length(%d)",
	...
	while (STREAM_READABLE(BGP_INPUT(peer)) >= 4) {
		...
		if (sublength > length) {
			zlog_err("xWrjalGLMUuy7sPs ""Tunnel Encap attribute sub-tlv length %d exceeds remaining length %d",
				 sublength, length);
			return bgp_attr_malformed(args,
						  BGP_NOTIFY_UPDATE_OPT_ATTR_ERR,
						  args->total);
		...
		if (STREAM_READABLE(BGP_INPUT(peer)) < sublength) {
			zlog_err("NDaEktmoet4y9Tge ""Tunnel Encap attribute sub-tlv length %d exceeds remaining stream length %zu",
	...
	if (length) {
		...
		zlog_err("gmWdZ8CWyx21pqON ""Tunnel Encap attribute length is bad: %d leftover octets",
			 length);
		return bgp_attr_malformed(args, BGP_NOTIFY_UPDATE_OPT_ATTR_ERR,
					  args->total);
	}
...
bgp_attr_srv6_service_data(struct bgp_attr_parser_args *args)
{
	...
	if (STREAM_READABLE(peer->curr) < headersz) {
		flog_err(
			EC_BGP_ATTR_LEN,"2LoVdv0IQnwLjZAN "
	...
	if (STREAM_READABLE(peer->curr) < length) {
		flog_err(
			EC_BGP_ATTR_LEN,"meTpUOKMri8ltIC8 "
	...
	if (length < BGP_PREFIX_SID_SRV6_L3_SERVICE_SID_STRUCTURE_LENGTH) {
		flog_err(
			EC_BGP_ATTR_LEN,"DZWFeCtecJeRxK5D "
	...
	if (type == BGP_PREFIX_SID_SRV6_L3_SERVICE_SID_STRUCTURE) {
		if (STREAM_READABLE(peer->curr) <
		    BGP_PREFIX_SID_SRV6_L3_SERVICE_SID_STRUCTURE_LENGTH) {
			flog_err(
				EC_BGP_ATTR_LEN,"ndFK617asKyCEm9z "
...
bgp_attr_srv6_service(struct bgp_attr_parser_args *args)
{
	...
	if (STREAM_READABLE(peer->curr) < headersz) {
		flog_err(
			EC_BGP_ATTR_LEN,"dHHStRMCKbH6iNw7 "
	...
	if (STREAM_READABLE(peer->curr) < length) {
		flog_err(
			EC_BGP_ATTR_LEN,"6gBco5LmalyuQCF9 "
	...
	if (type == BGP_PREFIX_SID_SRV6_L3_SERVICE_SID_INFO) {
		if (STREAM_READABLE(peer->curr) <
			...
			flog_err(
				EC_BGP_ATTR_LEN,"aDUJkxwP7AhjGZ3R "
		...
		/* Configure from Info */
		if (attr->srv6_l3vpn) {
			flog_err(EC_BGP_ATTRIBUTE_REPEATED,"sPKpfkbUCzzU9fbu "
		  ...
		  struct bgp_attr_parser_args *args)
{
	...
	if (STREAM_READABLE(peer->curr) < length) {
		flog_err(
			EC_BGP_ATTR_LEN,"gthmqRPJ249Ga3om "
	...
	if (type == BGP_PREFIX_SID_LABEL_INDEX) {
		if (length != BGP_PREFIX_SID_LABEL_INDEX_LENGTH) {
			flog_err(EC_BGP_ATTR_LEN,"STFDNNEjD6X5s5Jq "
		...
		if (length != BGP_PREFIX_SID_IPV6_LENGTH) {
			flog_err(EC_BGP_ATTR_LEN,"8XZT3coJN5aEiFJa "
		...
		if (length < (2 + BGP_PREFIX_SID_ORIGINATOR_SRGB_LENGTH)) {
			flog_err(
				EC_BGP_ATTR_LEN,"eCAkmxpynQEE6Grm "
		...
		if (length % BGP_PREFIX_SID_ORIGINATOR_SRGB_LENGTH) {
			flog_err(
				EC_BGP_ATTR_LEN,"TzdO1A6qw8AIJVcM "
		...
		if (length != BGP_PREFIX_SID_VPN_SID_LENGTH) {
			flog_err(EC_BGP_ATTR_LEN,"HSEXPQfcOUZ2Osq4 "
		...
		if (STREAM_READABLE(peer->curr) < 1) {
			flog_err(
				EC_BGP_ATTR_LEN,"UlXWW7Ecz7PXaoOs "
...
enum bgp_attr_parse_ret bgp_attr_prefix_sid(struct bgp_attr_parser_args *args)
{
	...
	while (STREAM_READABLE(peer->curr) > 0
		...
		if (STREAM_READABLE(peer->curr) < headersz) {
			flog_err(
				EC_BGP_ATTR_LEN,"v4Jzu9iL7UkNNuLt "
		...
		if (STREAM_READABLE(peer->curr) < length) {
			flog_err(
				EC_BGP_ATTR_LEN,"LiM2CwA7SWvXWdiF "
		...
		if (psid_parsed_length > args->length) {
			flog_err(
				EC_BGP_ATTR_LEN,"b0J85txUJnYHBQOp "
...
bgp_attr_pmsi_tunnel(struct bgp_attr_parser_args *args)
{
	...
	if (length < attr_parse_len) {
		flog_err(EC_BGP_ATTR_LEN,"0qJDQ38bJwSc2wEY " "Bad PMSI tunnel attribute length %d",
			 length);
		return bgp_attr_malformed(args, BGP_NOTIFY_UPDATE_ATTR_LENG_ERR,
					  args->total);
	}
	...
	if (tnl_type > PMSI_TNLTYPE_MAX) {
		flog_err(EC_BGP_ATTR_PMSI_TYPE,"jJLSnOTagnpWiVU3 "
	...
	if (tnl_type == PMSI_TNLTYPE_INGR_REPL) {
		if (length != 9) {
			flog_err(EC_BGP_ATTR_PMSI_LEN,"T6S5vqcSHVEiXSOB "
...
static enum bgp_attr_parse_ret bgp_attr_otc(struct bgp_attr_parser_args *args)
{
	...
	/* Length check. */
	if (length != 4) {
		flog_err(EC_BGP_ATTR_LEN,"MLe9KQWmc1mmuHkb " "OTC attribute length isn't 4 [%u]",
			 length);
		return bgp_attr_malformed(args, BGP_NOTIFY_UPDATE_ATTR_LENG_ERR,
					  args->total);
	}
	...
	if (!attr->otc) {
		flog_err(EC_BGP_ATTR_MAL_AS_PATH,"KO3Yi9FjQk1sOo0E " "OTC attribute value is 0");
		return bgp_attr_malformed(args, BGP_NOTIFY_UPDATE_MAL_AS_PATH,
					  args->total);
	}
			  ...
			  bgp_size_t length)
{
	 ...
	 */
	if (type) {
		flog_warn(EC_BGP_MISSING_ATTRIBUTE,"r5TDHNcg5Db5fMNp "
				       ...
				       struct bgp_nlri *mp_withdraw)
{
	...
	while (BGP_INPUT_PNT(peer) < endp) {
		...
		if ((endp - startp) < BGP_ATTR_MIN_LEN) {
			...
			flog_warn(
				EC_BGP_ATTRIBUTE_TOO_SMALL,"VqicR107K3UkJ52J "
		...
		if (CHECK_FLAG(flag, BGP_ATTR_FLAG_EXTLEN)
			...
			flog_warn(
				EC_BGP_EXT_ATTRIBUTE_TOO_SMALL,"tqCewwFt59zBa1UH "
		...
		if (attr_endp > endp) {
			flog_warn(
				EC_BGP_ATTRIBUTE_TOO_LARGE,"n7wz5b9xhvf9ySia "
			...
			if (peer->sort != BGP_PEER_EBGP) {
				 ...
				 */
				flog_warn(
					EC_BGP_ATTRIBUTE_PARSE_WITHDRAW,"tkig17rS9Nx6txDP "
		...
		if (CHECK_BITMAP(seen, type)) {
			...
			if (peer->sort != BGP_PEER_EBGP ||
				...
				flog_warn(
					EC_BGP_ATTRIBUTE_REPEATED,"Vxs72TUWr5JiinV3 "
				...
				flog_warn(
					EC_BGP_ATTRIBUTE_REPEATED,"wW1hmMUQOlC6T8Uo "
		...
		if (ret == BGP_ATTR_PARSE_ERROR) {
			flog_warn(EC_BGP_ATTRIBUTE_PARSE_ERROR,"cT5f9HlorNAjsEvg "
		...
		if (ret == BGP_ATTR_PARSE_WITHDRAW) {
			flog_warn(
				EC_BGP_ATTRIBUTE_PARSE_WITHDRAW,"7uEatsREcuczxbcH "
				"%s: Attribute %s, parse error - treating as withdrawal",
				peer->host, lookup_msg(attr_str, type, NULL));
			stream_forward_getp(BGP_INPUT(peer), endp - BGP_INPUT_PNT(peer));
			goto done;
		...
		if (BGP_INPUT_PNT(peer) != attr_endp) {
			flog_warn(EC_BGP_ATTRIBUTE_FETCH_ERROR,"Fpo9uXwidqf60tPY "
	...
	if (BGP_INPUT_PNT(peer) != endp) {
		flog_warn(EC_BGP_ATTRIBUTES_MISMATCH,"VJb4UjoJi1jV7aKX "
			       ...
			       struct attr *attr)
{
	...
	case AFI_L2VPN:
		if (safi != SAFI_FLOWSPEC)
			flog_err(
				EC_BGP_ATTR_NH_SEND_LEN,"NIJdfIVILc77NBXy "
				  ...
				  uint8_t attrtype)
{
	...
	if (attrlenfield > 0xffff) {
		zlog_info("t2z6fBuhDQfrILPO ""%s attribute is too long (length=%d), can't send it",
...
enum bgp_attr_parse_ret bgp_attr_ignore(struct peer *peer, uint8_t type)
{
	...
	if (bgp_debug_update(peer, NULL, NULL, 1) && (discard || withdraw))
		zlog_debug("r5LuX0U7BoQXPb7A ""%pBP: Ignoring attribute %s (%s)", peer,
			   lookup_msg(attr_str, type, NULL),
