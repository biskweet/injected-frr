...
static void bgp_unlink_nexthop_check(struct bgp_nexthop_cache *bnc)
{
	if (LIST_EMPTY(&(bnc->paths)) && !bnc->nht_info) {
		if (BGP_DEBUG(nht, NHT))
			zlog_debug("TQYHI0kQzHG8QOiy ""%s: freeing bnc %pFX(%d)(%u)(%s)", __func__,
			    ...
			    const struct prefix *orig_prefix)
{
	...
	if (!bnc) {
		...
		bnc->bgp = bgp_nexthop;
		if (BGP_DEBUG(nht, NHT))
			zlog_debug("o5C7yQHlr0yL81Lz ""Allocated bnc %pFX(%d)(%u)(%s) peer %p",
				   &bnc->prefix, bnc->ifindex_ipv6_ll,
				   bnc->srte_color, bnc->bgp->name_pretty,
				   peer);
	} else {
		if (BGP_DEBUG(nht, NHT))
			zlog_debug("Pkw6J2J9cyRMiN8Z "
				     ...
				     struct interface *ifp, bool up)
{
	...
	if (ifp->ifindex == IFINDEX_INTERNAL) {
		zlog_warn("AhgJmxiGkjkYnLCU ""%s: The interface %s ignored", __func__, ifp->name);
		return;
	}
			...
			struct zapi_route *nhr)
{
	...
	if (!vrf->info) {
		flog_err(EC_BGP_NH_UPD,"4CKGljSiKIm3Yg56 "
	...
	if (bnc_nhc)
		...
		zlog_debug("HsfMt2D0FmJQPIUE ""parse nexthop update %pFX(%u)(%s): bnc info not found for nexthop cache",
	...
	if (bnc_import) {
		...
		zlog_debug("x0NuntL1KlmRW4Cd ""parse nexthop update %pFX(%u)(%s): bnc info not found for import check",
			   &nhr->prefix, nhr->srte_color,
			   bgp->name_pretty);
...
static void sendmsg_zebra_rnh(struct bgp_nexthop_cache *bnc, int command)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("7h5NIry4To8uG7Vt ""%s: sending cmd %s for %pFX (vrf %s)", __func__,
	...
	if (ret == ZCLIENT_SEND_FAILURE) {
		flog_warn(EC_BGP_ZEBRA_SEND,"Htj4x98zgB7dq4zQ "
...
void bgp_nht_reg_enhe_cap_intfs(struct peer *peer)
{
	...
	bgp = peer->bgp;
	if (!sockunion2hostprefix(&peer->connection->su, &p)) {
		zlog_warn("xEaOvpw9doGXHMA3 ""%s: Unable to convert sockunion to prefix for %s",
			  __func__, peer->host);
		return;
	}
...
void bgp_nht_dereg_enhe_cap_intfs(struct peer *peer)
{
	...
	if (!sockunion2hostprefix(&peer->connection->su, &p)) {
		zlog_warn("4WdRRv8qrZ2USSg1 ""%s: Unable to convert sockunion to prefix for %s",
			  __func__, peer->host);
		return;
	}
