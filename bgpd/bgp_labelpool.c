...
static wq_item_status lp_cbq_docallback(struct work_queue *wq, void *data)
{
	...
	if (debug)
		zlog_debug("VKGNatRNV6rgZ94m ""%s: calling callback with labelid=%p label=%u allocated=%d",
			__func__, lcbq->labelid, lcbq->label, lcbq->allocated);

	if (lcbq->label == MPLS_LABEL_NONE) {
		/* shouldn't happen */
		flog_err(EC_BGP_LABEL,"9tKhtQCBetj44tWb " "%s: error: label==MPLS_LABEL_NONE",
			 __func__);
		return WQ_SUCCESS;
	}
	...
	if (lcbq->allocated && rc) {
		 ...
		 */
		if (debug)
			zlog_debug("fiyag7Y5l748XHuF ""%s: callback rejected allocation, releasing labelid=%p label=%u",
...
void bgp_lp_init(struct event_loop *master, struct labelpool *pool)
{
	if (BGP_DEBUG(labelpool, LABELPOOL))
		zlog_debug("Ni7EqkYEKotg3yvM ""%s: entry", __func__);
...
static mpls_label_t get_label_from_pool(void *labelid)
{
	...
	for (ALL_LIST_ELEMENTS_RO(lp->chunks, node, chunk)) {
		...
		if (debug)
			zlog_debug("yC8TdNBQRVlgvxHe ""%s: chunk first=%u last=%u",
		...
		if (skiplist_insert(lp->inuse, (void *)lbl, labelid)) {
			...
			zlog_err("LglAFBAxZgxd9rBz ""%s: unable to insert inuse label %u (id %p)",
	...
	int	(*cbfunc)(mpls_label_t label, void *labelid, bool allocated))
{
	...
	if (debug)
		zlog_debug("UKApzb4kjhuBOjye ""%s: labelid=%p", __func__, labelid);
	...
	if (!skiplist_search(lp->ledger, labelid, (void **)&lcb)) {
		...
		if (debug)
			zlog_debug("De6mUBYy4WGRdEuN ""%s: inserting lcb=%p label=%u",
				__func__, lcb, lcb->label);
		int rc = skiplist_insert(lp->ledger, labelid, lcb);

		if (rc) {
			/* shouldn't happen */
			flog_err(EC_BGP_LABEL,"cIWrhQCQzx2v7z44 "
	...
	if (debug)
		zlog_debug("N0wUKI6AOFjTv5aK ""%s: slow path. lcb=%p label=%u",
			__func__, lcb, lcb->label);
...
static void bgp_sync_label_manager(struct event *e)
{
	...
	while ((lf = lp_fifo_pop(&lp->requests))) {
		...
		if (skiplist_search(lp->ledger, labelid, (void **)&lcb)) {
			...
			if (debug) {
				zlog_debug("NAsKxANbyZLeRYW6 ""%s: labelid %p: request no longer in effect",
					   __func__, labelid);
			}
		...
		if (lcb->label != MPLS_LABEL_NONE) {
			...
			if (debug) {
				zlog_debug("kfidXHKYag3E7WzL ""%s: labelid %p: request already has a label: %u=0x%x, lcb=%p",
		...
		if (lcb->label == MPLS_LABEL_NONE) {
			 ...
			 */
			if (debug) {
				zlog_debug("fnz4xrLnGLK9OYye ""%s: out of labels, await more",
					   __func__);
			}
		...
		if (debug)
			zlog_debug("cLg0lt5r0FM2i1tE ""%s: assigning label %u to labelid %p",
...
void bgp_lp_event_chunk(uint32_t first, uint32_t last)
{
	...
	if (last < first) {
		flog_err(EC_BGP_LABEL,"zu7k2HBtSZHSP3Eg "
