...
static int bgp_capability_mp(struct peer *peer, struct capability_header *hdr)
{
	...
	if (hdr->length != 4) {
		flog_warn(
			EC_BGP_CAPABILITY_INVALID_LENGTH,"clBRtS3ePOzovzFa "
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("bikeClvIstLqHyIL ""%s OPEN has %s capability for afi/safi: %s/%s",
				    ...
				    struct capability_header *hdr)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("07hJ48AAORrGxxNA ""%s ORF Cap entry for afi/safi: %s/%s", peer->host,
			   iana_afi2str(mpc.afi), iana_safi2str(mpc.safi));

	/* Convert AFI, SAFI to internal values, check. */
	if (bgp_map_afi_safi_iana2int(pkt_afi, pkt_safi, &afi, &safi)) {
		zlog_info("buXV2gQWtukReWUb "
				  ...
				  struct capability_header *caphdr)
{
	...
	if ((caphdr->length - 2) % 4) {
		flog_warn(
			EC_BGP_CAPABILITY_INVALID_LENGTH,"UScCcU4XfStgTxgO "
...
static as_t bgp_capability_as4(struct peer *peer, struct capability_header *hdr)
{
	if (hdr->length != CAPABILITY_CODE_AS4_LEN) {
		flog_err(EC_BGP_PKT_OPEN,"ZO6S8uTaPOD356jI "
				      ...
				      struct capability_header *hdr)
{
	if (hdr->length != CAPABILITY_CODE_EXT_MESSAGE_LEN) {
		flog_err(
			EC_BGP_PKT_OPEN,"4aw4vU9yw1FFVMuu "
				  ...
				  struct capability_header *hdr)
{
	...
	if (hdr->length % CAPABILITY_CODE_ADDPATH_LEN) {
		flog_warn(
			EC_BGP_CAPABILITY_INVALID_LENGTH,"8C7gsPjG0EHFtja5 "
	...
	while (stream_get_getp(s) + CAPABILITY_CODE_ADDPATH_LEN <= end) {
		...
		if (!send_receive || send_receive > 3) {
			flog_warn(EC_BGP_CAPABILITY_INVALID_DATA,"b3tL530Fs0skiIDu "
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("BnWiojxq4l8dfTOx ""%s OPEN has %s capability for afi/safi: %s/%s%s%s",
				   peer->host,
				      ...
				      struct capability_header *hdr)
{
	...
	if (hdr->length % CAPABILITY_CODE_PATHS_LIMIT_LEN) {
		flog_warn(EC_BGP_CAPABILITY_INVALID_LENGTH,"2VoxfFPthb38IwC2 "
	...
	if (!CHECK_FLAG(peer->cap, PEER_CAP_ADDPATH_RCV)) {
		flog_warn(EC_BGP_CAPABILITY_INVALID_DATA,"MW9O2xOx06ERN2TC "
	...
	while (stream_get_getp(s) + CAPABILITY_CODE_PATHS_LIMIT_LEN <= end) {
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("0mbXDp89LnYiD19N ""%s OPEN has %s capability for afi/safi: %s/%s limit: %u",
				   peer->host,
				   lookup_msg(capcode_str, hdr->code, NULL),
				   iana_afi2str(pkt_afi),
		...
		if (bgp_map_afi_safi_iana2int(pkt_afi, pkt_safi, &afi, &safi)) {
			if (bgp_debug_neighbor_events(peer))
				zlog_debug("S9bWL2zaSrcr9kL4 ""%s Addr-family %s/%s(afi/safi) not supported. Ignore the Paths-Limit capability for this AFI/SAFI",
					   peer->host, iana_afi2str(pkt_afi),
					   iana_safi2str(pkt_safi));
			continue;
		} else if (!peer->afc[afi][safi]) {
			if (bgp_debug_neighbor_events(peer))
				zlog_debug("3AGBtStcCPxgOuhC ""%s Addr-family %s/%s(afi/safi) not enabled. Ignore the Paths-Limit capability for this AFI/SAFI",
					   peer->host, iana_afi2str(pkt_afi),
					   iana_safi2str(pkt_safi));
			continue;
		}
...
static int bgp_capability_enhe(struct peer *peer, struct capability_header *hdr)
{
	...
	if (hdr->length % 6) {
		flog_warn(
			EC_BGP_CAPABILITY_INVALID_LENGTH,"VMNmaj2DGzVru34L "
	...
	while (stream_get_getp(s) + 6 <= end) {
		...
		if (afi != AFI_IP || nh_afi != AFI_IP6
			...
			flog_warn(
				EC_BGP_CAPABILITY_INVALID_DATA,"RrmZkEoStAclm4jO "
				   ...
				   struct capability_header *hdr)
{
	...
	if (stream_get_getp(s) + len > end) {
		flog_warn(
			EC_BGP_CAPABILITY_INVALID_DATA,"bOYJXCZvcELsVpWN "
	...
	if (stream_get_getp(s) + 1 > end) {
		flog_warn(
			EC_BGP_CAPABILITY_INVALID_DATA,"wLEkD0ylWII1t5qn "
	...
	if (stream_get_getp(s) + len > end) {
		flog_warn(
			EC_BGP_CAPABILITY_INVALID_DATA,"TQVTGeJRjID6WmCr "
	...
	if (bgp_debug_neighbor_events(peer)) {
		zlog_debug("om1xHJyE8izntHpX ""%s received hostname %s, domainname %s", peer->host,
...
static int bgp_capability_role(struct peer *peer, struct capability_header *hdr)
{
	if (hdr->length != CAPABILITY_CODE_ROLE_LEN) {
		flog_warn(EC_BGP_CAPABILITY_INVALID_LENGTH,"mLnnif2lnyfTkFMW "
					   ...
					   struct capability_header *hdr)
{
	...
	if (stream_get_getp(s) + len > end) {
		flog_warn(
			EC_BGP_CAPABILITY_INVALID_DATA,"XSYPhKUUi6tpRA0r "
	...
	if (len > BGP_MAX_SOFT_VERSION) {
		flog_warn(EC_BGP_CAPABILITY_INVALID_LENGTH,"uOHDoJa92O427xXR "
	...
	if (len) {
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("9DaYoSlzIC9ANywv ""%s received Software Version: %s",
				...
				int *mp_capability, uint8_t **error)
{
	...
	while (stream_get_getp(s) < end) {
		...
		if (stream_get_getp(s) + 2 > end) {
			zlog_info("k4Z3xwvjgOpIKxfp ""%s Capability length error (< header)",
				  peer->host);
			bgp_notify_send(peer->connection, BGP_NOTIFY_OPEN_ERR,
					BGP_NOTIFY_OPEN_MALFORMED_ATTR);
			return -1;
		...
		if (start + caphdr.length > end) {
			zlog_info("RD4C1vjmpqfe2OcA ""%s Capability length error (< length)",
				  peer->host);
			bgp_notify_send(peer->connection, BGP_NOTIFY_OPEN_ERR,
					BGP_NOTIFY_OPEN_MALFORMED_ATTR);
			return -1;
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("Lnaa75VeBr9ZyIyJ ""%s OPEN has %s capability (%u), length %u",
				   peer->host,
		...
		default:
			if (caphdr.code > 128) {
				/* We don't send Notification for unknown vendor
				   ...
				   */
				flog_warn(EC_BGP_CAPABILITY_VENDOR,"oXkoyTkTRmVyNaZy "
					  "%s Vendor specific capability %d",
					  peer->host, caphdr.code);
			} else {
				flog_warn(
					EC_BGP_CAPABILITY_UNKNOWN,"q1KekLUi7ZxGRHWE "
		...
		if (stream_get_getp(s) != (start + caphdr.length)) {
			if (stream_get_getp(s) > (start + caphdr.length))
				flog_warn(
					EC_BGP_CAPABILITY_INVALID_LENGTH,"ZXRmafs3BHt3jj2Q "
			  ...
			  int *mp_capability)
{
	...
	if (bgp_debug_neighbor_events(peer))
		zlog_debug("LcA0236ZS1SmNvbE ""%s rcv OPEN w/ OPTION parameter len: %u",
			   peer->host, length);
	...
	while (stream_get_getp(s) < end) {
		...
		if (STREAM_READABLE(s) < 1) {
			zlog_info("5uCm8wCSRgicQgrM ""%s Option length error", peer->host);
			bgp_notify_send(peer->connection, BGP_NOTIFY_OPEN_ERR,
					BGP_NOTIFY_OPEN_MALFORMED_ATTR);
			return -1;
		}
		...
		if (BGP_OPEN_EXT_OPT_PARAMS_CAPABLE(peer)) {
			if (STREAM_READABLE(s) < 2) {
				zlog_info("A4sjuGtXpnJBwQUK ""%s Option length error", peer->host);
				bgp_notify_send(peer->connection,
						BGP_NOTIFY_OPEN_ERR,
						BGP_NOTIFY_OPEN_MALFORMED_ATTR);
				return -1;
			...
			if (STREAM_READABLE(s) < 1) {
				zlog_info("l25vj0LKax1euDEe ""%s Option length error", peer->host);
				bgp_notify_send(peer->connection,
						BGP_NOTIFY_OPEN_ERR,
						BGP_NOTIFY_OPEN_MALFORMED_ATTR);
				return -1;
		...
		if (STREAM_READABLE(s) < opt_length) {
			zlog_info("5JoTKhQq87oGbkRb ""%s Option length error (%d)", peer->host,
				  opt_length);
			bgp_notify_send(peer->connection, BGP_NOTIFY_OPEN_ERR,
					BGP_NOTIFY_OPEN_MALFORMED_ATTR);
			return -1;
	...
	if (*mp_capability
		...
		if (!peer->afc_nego[AFI_IP][SAFI_UNICAST]
			...
			flog_err(EC_BGP_PKT_OPEN,"sZMKTgLaZCh247tT "
					...
					bool ext_opt_params)
{
	...
	if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
		zlog_debug("as15ckwso6trnvZh ""[BGP_GR] Sending helper Capability for Peer :%s :",
			   peer->host);
	...
	if (peer->bgp->t_startup) {
		...
		if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
			zlog_debug("eMARXg5ckrXysIqn ""[BGP_GR] Sending R-Bit for peer: %s",
				   peer->host);
	}
	...
	if (CHECK_FLAG(peer->bgp->flags, BGP_FLAG_GRACEFUL_NOTIFICATION)) {
		...
		if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
			zlog_debug("9EvwE5S0YN9nrviu ""[BGP_GR] Sending N-Bit for peer: %s",
				   peer->host);
	}
	...
	if (CHECK_FLAG(peer->flags, PEER_FLAG_GRACEFUL_RESTART)) {
		if (CHECK_FLAG(peer->bgp->flags, BGP_FLAG_GR_PRESERVE_FWD)
			...
			zlog_debug("m7RFogF20ClzENPw ""[BGP_GR] F bit Set");
			     ...
			     bool ext_opt_params)
{
	...
	if (peergroup_flag_check(peer, PEER_FLAG_CAPABILITY_SOFT_VERSION) ||
		...
		if (bgp_debug_neighbor_events(peer))
			zlog_debug("y9XE67r5OEEDyOHC ""%s Sending Software Version cap, value: %s",
