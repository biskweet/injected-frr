...
int bgp_parse_fec_update(void)
{
	...
	bgp = bgp_get_default();
	if (!bgp) {
		zlog_debug("38R8UKIVIU8Lau9G ""no default bgp instance");
		return -1;
	}

	table = bgp->rib[afi][safi];
	if (!table) {
		zlog_debug("7H0he0rHm3LUA6j0 ""no %u unicast table", p.family);
		return -1;
	}
	dest = bgp_node_lookup(table, &p);
	if (!dest) {
		zlog_debug("PjWpxL2We3LzDupx ""no node for the prefix");
		return -1;
	}
					    ...
					    uint32_t label_index)
{
	...
	if (BGP_DEBUG(labelpool, LABELPOOL))
		zlog_debug("VK8SXvHt3WjphqwQ ""%s: FEC %sregister %pBD label_index=%u label=%u",
			       ...
			       bool allocated)
{
	...
	if (BGP_DEBUG(labelpool, LABELPOOL))
		zlog_debug("8gpvJoqU2omNgHPl ""%s: FEC %pBD label=%u, allocated=%d", __func__,
			     ...
			     bool reg)
{
	...
	if (BGP_DEBUG(labelpool, LABELPOOL))
		zlog_debug("0CmND4Df79DB8Yea ""%s: %pFX: %s ", __func__, p,
			   (reg ? "reg" : "dereg"));
			       ...
			       mpls_label_t *label)
{
	...
	if (label_depth > 1)
		zlog_info("GT6ySwXJSiqO45wA ""%pBP rcvd UPDATE with label stack %d deep", peer,
			  label_depth);

	if (!(bgp_is_withdraw_label(label) || label_bos(label)))
		flog_warn(
			EC_BGP_INVALID_LABEL_STACK,"bSw5WhqVt8ukjMFI "
			 ...
			 struct bgp_nlri *packet)
{
	...
	for (; pnt < lim; pnt += psize) {
		...
		if ((pnt + psize) > lim) {
			flog_err(
				EC_BGP_UPDATE_RCV,"ea9oPiI31rQ3X57n "
		...
		if (llen == 0) {
			flog_err(
				EC_BGP_UPDATE_RCV,"U5w8eNFVG1DLmnfQ "
		...
		if (prefixlen < 24) {
			flog_err(EC_BGP_UPDATE_RCV,"o3VASWRK0xoHGqRT "
		...
		if (afi == AFI_IP && safi == SAFI_LABELED_UNICAST) {
			if (IN_CLASSD(ntohl(p.u.prefix4.s_addr))) {
				 ...
				 * ignored.
				  */
				flog_err(
					EC_BGP_UPDATE_RCV,"xuJ5rvQV1nGZJ62l "
		...
		if (afi == AFI_IP6 && safi == SAFI_LABELED_UNICAST) {
			if (IN6_IS_ADDR_LINKLOCAL(&p.u.prefix6)) {
				flog_err(
					EC_BGP_UPDATE_RCV,"xb1t1etNt5ctUXla "
			...
			if (IN6_IS_ADDR_MULTICAST(&p.u.prefix6)) {
				flog_err(
					EC_BGP_UPDATE_RCV,"5unXtAE8KvuRZbRr "
	...
	if (pnt != lim) {
		flog_err(
			EC_BGP_UPDATE_RCV,"5T02gRvzfI9tKX0R "
