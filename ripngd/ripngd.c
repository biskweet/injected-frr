...
int ripng_make_socket(struct vrf *vrf)
{
	...
	frr_with_privs(&ripngd_privs) {
		sock = vrf_socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP,
				  vrf->vrf_id, vrf_dev);
		if (sock < 0) {
			flog_err_sys(EC_LIB_SOCKET,"iR5AfIpzAoMF6vfT "
	...
	frr_with_privs(&ripngd_privs) {
		...
		if (ret < 0) {
			zlog_err("i3Seep9krWE62qtA ""Can't bind ripng socket: %s.",
				 safe_strerror(errno));
			goto error;
		}
	}
		      ...
		      struct interface *ifp)
{
	...
	if (IS_RIPNG_DEBUG_SEND) {
		if (to)
			zlog_debug("vMJPREQoAFEumqIu ""send to %pI6", &to->sin6_addr);
		zlog_debug("eSL38qroheubZzE3 ""  send interface %s", ifp->name);
		zlog_debug("k8TckB6cRJQt71WA ""  send packet size %d", bufsize);
	}
	...
	if (ret < 0) {
		if (to)
			flog_err_sys(EC_LIB_SOCKET,"7fVdTxzrEPIundh9 "
				     "RIPng send fail on %s to %pI6: %s",
				     ifp->name, &to->sin6_addr,
				     safe_strerror(errno));
		else
			flog_err_sys(EC_LIB_SOCKET,"qAsTYJ7I9m9W0TwT " "RIPng send fail on %s: %s",
		       ...
		       const char *sndrcv)
{
	...
	/* Dump packet header. */
	zlog_debug("Wr9Hv5ZlNmUom4VQ ""%s %s version %d packet size %d", sndrcv, command_str,
		   packet->version, size);
	...
	for (lim = (caddr_t)packet + size; (caddr_t)rte < lim; rte++) {
		if (rte->metric == RIPNG_METRIC_NEXTHOP)
			zlog_debug("uVOWhpFOnVtWH2rt ""  nexthop %pI6/%d", &rte->addr,
				   rte->prefixlen);
		else
			zlog_debug("cyHZRkBL32hVJ6ZG ""  %pI6/%d metric %d tag %" ROUTE_TAG_PRI,
			      ...
			      struct ripng_nexthop *nexthop)
{
	...
	if (IS_RIPNG_DEBUG_RECV)
		zlog_debug("OUWWobz4aOUmNA7G ""RIPng nexthop RTE address %pI6 tag %" ROUTE_TAG_PRI
			   " prefixlen %d",
	...
	zlog_warn("kpr0IW2tEmgWaHBd ""RIPng nexthop RTE with non link-local address %pI6 from %pI6",
			...
			struct ripng_interface *ri)
{
	...
	if (ri->list[ripng_distribute]) {
		if (access_list_apply(ri->list[ripng_distribute],
			...
			if (IS_RIPNG_DEBUG_PACKET)
				zlog_debug("gxpj6kQttZKH48zz ""%pFX filtered by distribute %s", p,
					   inout);
			return -1;
		}
	}
	if (ri->prefix[ripng_distribute]) {
		if (prefix_list_apply(ri->prefix[ripng_distribute],
			...
			if (IS_RIPNG_DEBUG_PACKET)
				zlog_debug("KRyJSkhIzhay6q3x ""%pFX filtered by prefix-list %s", p,
					   inout);
			return -1;
		}
	}
				   ...
				   struct interface *ifp, int hoplimit)
{
	...
	if (ntohs(from->sin6_port) != RIPNG_PORT_DEFAULT) {
		zlog_warn("4kRMoP0rwhdQyI7e ""RIPng packet comes from non RIPng port %d from %pI6",
	...
	if (!IN6_IS_ADDR_LINKLOCAL(&from->sin6_addr)) {
		zlog_warn("z9a6d1plXRaeP3sK ""RIPng packet comes from non link local address %pI6",
			  &from->sin6_addr);
		ripng_peer_bad_packet(ripng, from);
		return;
	}
	...
	for (lim = ((caddr_t)packet) + size; (caddr_t)rte < lim; rte++) {
		...
		if (rte->prefixlen > IPV6_MAX_BITLEN) {
			zlog_warn("kCudpjp67PHEYBSE ""Invalid prefix length %pI6/%d from %pI6%%%s",
				  &rte->addr, rte->prefixlen,
		...
		if (!(rte->metric >= 1 && rte->metric <= 16)) {
			zlog_warn("8hsVzpPYR5EMGoXB ""Invalid metric %d from %pI6%%%s",
...
static void ripng_read(struct event *thread)
{
	...
	len = ripng_recv_packet(sock, STREAM_DATA(ripng->ibuf),
				...
				&hoplimit);
	if (len < 0) {
		zlog_warn("uUgyW7Pq1XiODdwB ""RIPng recvfrom failed (VRF %s): %s.",
	...
	if (((len - 4) % 20) != 0) {
		zlog_warn("cuuO8COP2sqnn30d ""RIPng invalid packet size %d from %pI6 (VRF %s)",
	...
	case RIPNG_RESPONSE:
		...
		break;
	default:
		zlog_warn("QXQK5ZZyOLBg1C6X ""Invalid RIPng command %d (VRF %s)", packet->command,
			  ripng->vrf_name);
		ripng_peer_bad_packet(ripng, &from);
		break;
	}
...
static void ripng_update(struct event *t)
{
	...
	if (IS_RIPNG_DEBUG_EVENT)
		zlog_debug("aCaZsByCHbTEvsJT ""RIPng update timer expired!");
...
void ripng_triggered_update(struct event *t)
{
	...
	if (IS_RIPNG_DEBUG_EVENT)
		zlog_debug("JBCt0M6t8kuzmaY2 ""RIPng triggered update!");
			  ...
			  int route_type)
{
	...
	if (IS_RIPNG_DEBUG_EVENT) {
		if (to)
			zlog_debug("0ovQtxqN6BSeYLCq ""RIPng update routes to neighbor %pI6",
				   &to->sin6_addr);
		else
			zlog_debug("UlXhQ9zwQkJhiNwo ""RIPng update routes on interface %s",
				   ifp->name);
	}
...
int ripng_request(struct interface *ifp)
{
	...
	if (IS_RIPNG_DEBUG_EVENT)
		zlog_debug("emxQFmsUxG3XPD8R ""RIPng send request to %s", ifp->name);
...
{
	if (IS_RIPNG_DEBUG_EVENT)
		zlog_debug("M9lLibif2hWn7y1w ""%s: VRF created: %s(%u)", __func__, vrf->name,
			   vrf->vrf_id);
...
{
	if (IS_RIPNG_DEBUG_EVENT)
		zlog_debug("h2aakWbfR5qxC0kB ""%s: VRF deleted: %s(%u)", __func__, vrf->name,
			   vrf->vrf_id);
...
static int ripng_vrf_enable(struct vrf *vrf)
{
	...
	if (IS_RIPNG_DEBUG_EVENT)
		zlog_debug("3lUE2jgqdZ0NgfZ1 ""%s: VRF %s(%u) enabled", __func__, vrf->name,
			   vrf->vrf_id);
...
static int ripng_vrf_disable(struct vrf *vrf)
{
	...
	if (IS_RIPNG_DEBUG_EVENT)
		zlog_debug("zlLnUppZHBJhrX3k ""%s: VRF %s(%u) disabled", __func__, vrf->name,
			   vrf->vrf_id);
