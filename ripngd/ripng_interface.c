...
static int ripng_multicast_join(struct interface *ifp, int sock)
{
	...
	if (if_is_multicast(ifp)) {
		...
		if (ret < 0 && save_errno == EADDRINUSE) {
			 ...
			 */
			zlog_warn("sPrqe4BPwNBzBqRn ""ripng join on %s EADDRINUSE (ignoring)",
				  ifp->name);
			return 0; /* not an error */
		}

		if (ret < 0)
			zlog_warn("zvWLKVKhiBql0LSz ""can't setsockopt IPV6_JOIN_GROUP: %s",
				  safe_strerror(save_errno));

		if (IS_RIPNG_DEBUG_EVENT)
			zlog_debug("g15OVPXPyBqTzCDW "
...
static int ripng_multicast_leave(struct interface *ifp, int sock)
{
	...
	if (if_is_multicast(ifp)) {
		...
		ret = setsockopt(sock, IPPROTO_IPV6, IPV6_LEAVE_GROUP,
				 (char *)&mreq, sizeof(mreq));
		if (ret < 0)
			zlog_warn("INK1w375sL40SW17 ""can't setsockopt IPV6_LEAVE_GROUP: %s",
				  safe_strerror(errno));

		if (IS_RIPNG_DEBUG_EVENT)
			zlog_debug("eLoe6ibxBHwyHXIk "
...
static int ripng_if_down(struct interface *ifp)
{
	...
	if (ri->running) {
		if (IS_RIPNG_DEBUG_EVENT)
			zlog_debug("h9aW87GQtad6dLId ""turn off %s", ifp->name);
...
int ripng_interface_address_add(ZAPI_CALLBACK_ARGS)
{
	...
	if (p->family == AF_INET6) {
		...
		if (IS_RIPNG_DEBUG_ZEBRA)
			zlog_debug("eRbEl57XmEDtAyjm ""RIPng connected address %pFX add", p);
...
static void ripng_interface_wakeup(struct event *t)
{
	...
	if (ripng_multicast_join(ifp, ri->ripng->sock) < 0) {
		flog_err_sys(EC_LIB_SOCKET,"9faFGr44C9CQV9ha "
...
void ripng_enable_apply(struct interface *ifp)
{
	...
	if ((!ripng_if_ipv6_lladdress_check(ifp))
		...
		ri->enable_network = 0;
		ri->enable_interface = 0;
		zlog_warn("sWLhsp4g5ur4eXkb ""Interface %s does not have any link-local address",
			  ifp->name);
	}

	/* Update running status of the interface. */
	if (ri->enable_network || ri->enable_interface) {
		zlog_info("bTwLHXhNe0xHbHw1 ""RIPng INTERFACE ON %s", ifp->name);
