				  ...
				  uint8_t cmd)
{
	...
	if (IS_RIPNG_DEBUG_ZEBRA) {
		if (ripng->ecmp)
			zlog_debug("y3D3d94edzBGFnQY ""%s: %pRN nexthops %d",
				   (cmd == ZEBRA_ROUTE_ADD)
					   ? "Install into zebra"
					   : "Delete from zebra",
				   rp, count);
		else
			zlog_debug("F3rmuLotHRPT4tE5 ""%s: %pRN",
				   (cmd == ZEBRA_ROUTE_ADD)
					   ? "Install into zebra"
					   : "Delete from zebra",
				   rp);
...
void ripng_zebra_vrf_register(struct vrf *vrf)
{
	...
	if (IS_RIPNG_DEBUG_EVENT)
		zlog_debug("AgE8IyZAlou2umEW ""%s: register VRF %s(%u) to zebra", __func__,
			   vrf->name, vrf->vrf_id);
...
void ripng_zebra_vrf_deregister(struct vrf *vrf)
{
	...
	if (IS_RIPNG_DEBUG_EVENT)
		zlog_debug("eg2dwTXH7h3vYQ0l ""%s: deregister VRF %s(%u) from zebra.", __func__,
			   vrf->name, vrf->vrf_id);
