...
int eigrp_sock_init(struct vrf *vrf)
{
	...
	frr_with_privs(&eigrpd_privs) {
		...
		if (eigrp_sock < 0) {
			zlog_err("aMkzFGvJxQUqMiW2 ""%s: socket: %s",
		...
		ret = setsockopt(eigrp_sock, IPPROTO_IP, IP_HDRINCL, &hincl,
				 sizeof(hincl));
		if (ret < 0) {
			zlog_warn("mCUx5OtOKRsoYXlz ""Can't set IP_HDRINCL option for fd %d: %s",
...
#warning "using IPTOS_PREC_INTERNETCONTROL"
		...
		if (ret < 0) {
			zlog_warn("FphPvjBtbpyuH8yJ ""can't set sockopt IP_TOS %d to socket %d: %s",
...
#warning "IP_HDRINCL not available, nor is IPTOS_PREC_INTERNETCONTROL"
		zlog_warn("TUOE3Tw3vUd0mOeR ""IP_HDRINCL option not available");
#endif /* IP_HDRINCL */

		ret = setsockopt_ifindex(AF_INET, eigrp_sock, 1);
		if (ret < 0)
			zlog_warn("eSkFfYNF9ZBlIAjM ""Can't set pktinfo option for fd %d",
				  eigrp_sock);
	}
...
void eigrp_adjust_sndbuflen(struct eigrp *eigrp, unsigned int buflen)
{
	...
	if (newbuflen < 0 || newbuflen < (int)buflen)
		zlog_warn("yBqp54VNftuSCfZW ""%s: tried to set SO_SNDBUF to %u, but got %d",
			  __func__, buflen, newbuflen);
	if (newbuflen >= 0)
		eigrp->maxsndbuflen = (unsigned int)newbuflen;
	else
		zlog_warn("txaox10uWKtS9tOD ""%s: failed to get SO_SNDBUF", __func__);
}
			 ...
			 unsigned int ifindex)
{
	...
	ret = setsockopt(top->fd, IPPROTO_IP, IP_MULTICAST_TTL, (void *)&val,
			 len);
	if (ret < 0)
		zlog_warn("VQRcIhnFnpHdSbJe ""can't setsockopt IP_MULTICAST_TTL (1) for fd %d: %s",
			  top->fd, safe_strerror(errno));

	ret = setsockopt_ipv4_multicast_if(top->fd, p->u.prefix4, ifindex);
	if (ret < 0)
		zlog_warn("f7tdTzvxiki2ggGy "
			       ...
			       unsigned int ifindex)
{
	...
	if (ret < 0)
		...
		zlog_debug("2R0BnalemUdb8nEh ""interface %pI4 [%u] join EIGRP Multicast group.",
			   &p->u.prefix4, ifindex);
				...
				unsigned int ifindex)
{
	...
	ret = setsockopt_ipv4_multicast(
		...
		htonl(EIGRP_MULTICAST_ADDRESS), ifindex);
	if (ret < 0)
		zlog_warn("c7fMBuIjbIbqhxd0 "
			"can't setsockopt IP_DROP_MEMBERSHIP (fd %d, addr %pI4, ifindex %u, AllSPFRouters): %s",
			top->fd, &p->u.prefix4, ifindex, safe_strerror(errno));
	else
		zlog_debug("vcdPz3MdkLCjIcQ6 ""interface %pI4 [%u] leave EIGRP Multicast group.",
			   &p->u.prefix4, ifindex);
...
int eigrp_network_set(struct eigrp *eigrp, struct prefix *p)
{
	...
	FOR_ALL_INTERFACES (vrf, ifp) {
		zlog_debug("3BKPNVXfyUiJB88S ""Setting up %s", ifp->name);
