			   ...
			   struct eigrp_neighbor *nbr, uint8_t flags)
{
	...
	if (memcmp(orig, digest, EIGRP_AUTH_TYPE_MD5_LEN) != 0) {
		zlog_warn("D9ZOQ3DCAqNhA1rX ""interface %s: eigrp_check_md5 checksum mismatch",
			  IF_NAME(nbr->ei));
		return 0;
	}
...
#define EIGRP_WRITE_IPHL_SHIFT 2
	...
	if (!ep) {
		flog_err(EC_LIB_DEVELOPMENT,"Fp3LQTuCclLV822j "
			 "%s: Interface %s no packet on queue?", __func__,
			 ei->ifp->name);
		goto out;
	}
	if (ep->length < EIGRP_HEADER_LEN) {
		flog_err(EC_EIGRP_PACKET,"Il5aHxzfXj87aToL " "%s: Packet just has a header?",
			 __func__);
					...
					struct stream *ibuf)
{
	...
	if (ret < 0) {
		zlog_warn("3htysh3JXpMbPFHU ""stream_recvmsg failed: %s", safe_strerror(errno));
		return NULL;
	}
	if ((unsigned int)ret < sizeof(*iph)) /* ret must be > 0 now */
	{
		zlog_warn("a1OZ02hZ4AcqM7gQ "
			      ...
			      uint32_t flags, uint32_t sequence, uint32_t ack)
{
	...
	if (IS_DEBUG_EIGRP_TRANSMIT(0, PACKET_DETAIL))
		zlog_debug("sureCqVXfuks6OXq ""Packet Header Init Seq [%u] Ack [%u]",
					 ...
					 struct eigrp_prefix_descriptor *pe)
{
	...
	case 32:
		...
		break;
	default:
		flog_err(EC_LIB_DEVELOPMENT,"bJKLWiekBucp0eYg " "%s: Unexpected prefix length: %d",
