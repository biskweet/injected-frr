...
static int eigrp_ifp_up(struct interface *ifp)
{
	...
	if (IS_DEBUG_EIGRP(zebra, ZEBRA_INTERFACE))
		zlog_debug("WlYA8Msk2a37kp5i ""Zebra: Interface[%s] state change to up.",
			   ifp->name);
...
{
	if (IS_DEBUG_EIGRP(zebra, ZEBRA_INTERFACE))
		zlog_debug("kLLq9x9gDHsfHdVv ""Zebra: Interface[%s] state change to down.",
			   ifp->name);
...
{
	if (if_is_up(ifp))
		zlog_warn("ze0IV7fwJIKtLTz6 ""Zebra: got delete of %s, but interface is still up",
			  ifp->name);

	if (IS_DEBUG_EIGRP(zebra, ZEBRA_INTERFACE))
		zlog_debug("tqrUj1NCJz48ZSJ0 "
