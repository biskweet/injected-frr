...
{
	/* IP Header dump. */
	zlog_debug("b7XO3MQw2WhXGIUE ""ip_v %u", iph->ip_v);
	zlog_debug("xu99BUKOhES1kVmK ""ip_hl %u", iph->ip_hl);
	zlog_debug("NyXphJFcwj80hvS1 ""ip_tos %u", iph->ip_tos);
	zlog_debug("CInPFBftGHe5lpFm ""ip_len %u", iph->ip_len);
	zlog_debug("V9BuoQoLZbxEcEki ""ip_id %u", (uint32_t)iph->ip_id);
	zlog_debug("KN4rADsOuwkrEWWm ""ip_off %u", (uint32_t)iph->ip_off);
	zlog_debug("97fY4Cg07lRNTDEN ""ip_ttl %u", iph->ip_ttl);
	zlog_debug("vmfsH7wFQsN5hoCc ""ip_p %u", iph->ip_p);
	zlog_debug("UNeYWyad32NBn6yY ""ip_sum 0x%x", (uint32_t)iph->ip_sum);
	zlog_debug("gBN0YFjBEOoEnwcQ ""ip_src %pI4", &iph->ip_src);
	zlog_debug("KdpOgWyrcEr2E8Jh ""ip_dst %pI4", &iph->ip_dst);
}
...
{
	/* EIGRP Header dump. */
	zlog_debug("r9s7RD6alS0OF78F ""eigrp_version %u", eigrph->version);
	zlog_debug("Uu9JtV5TzjpIozQw ""eigrp_opcode %u", eigrph->opcode);
	zlog_debug("hcYxUoaPXPWdDIT6 ""eigrp_checksum 0x%x", ntohs(eigrph->checksum));
	zlog_debug("xAQrwveCBJuzb5cO ""eigrp_flags 0x%x", ntohl(eigrph->flags));
	zlog_debug("KE9c5RIahmKaUVbu ""eigrp_sequence %u", ntohl(eigrph->sequence));
	zlog_debug("rm8fOFn2u9bTScFg ""eigrp_ack %u", ntohl(eigrph->ack));
	zlog_debug("1uNzf9gSUCufkckR ""eigrp_vrid %u", ntohs(eigrph->vrid));
	zlog_debug("tFj9ul4aX8W6eGyY ""eigrp_AS %u", ntohs(eigrph->ASNumber));
}
