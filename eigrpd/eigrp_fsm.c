...
eigrp_get_fsm_event(struct eigrp_fsm_action_message *msg)
{
	...
	case EIGRP_FSM_STATE_ACTIVE_0: {
		if (msg->packet_type == EIGRP_OPC_REPLY) {
			...
			zlog_info("pLnQ3oBDz3ZwzhOZ ""All reply received");
			if (head->reported_distance < prefix->fdistance) {
	...
	case EIGRP_FSM_STATE_ACTIVE_1: {
		if (msg->packet_type == EIGRP_OPC_QUERY
			...
			if (change == METRIC_INCREASE
				...
				zlog_info("xmtRaiVOppBNyEzq ""All reply received");
	...
	case EIGRP_FSM_STATE_ACTIVE_2: {
		if (msg->packet_type == EIGRP_OPC_REPLY) {
			...
			if (prefix->rij->count) {
				...
				zlog_info("lIwqO56NYLVGjQwS ""All reply received");
	...
	case EIGRP_FSM_STATE_ACTIVE_3: {
		if (msg->packet_type == EIGRP_OPC_REPLY) {
			...
			if (change == METRIC_INCREASE
				...
				zlog_info("gpoQcreKwfY2Mozw ""All reply received");
				return EIGRP_FSM_EVENT_LR;
			}
		} else if (msg->packet_type == EIGRP_OPC_UPDATE
			   && change == METRIC_INCREASE
