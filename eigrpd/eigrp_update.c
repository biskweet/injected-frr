					...
					struct list *nbr_prefixes)
{
	...
	for (ALL_LIST_ELEMENTS_RO(nbr_prefixes, node1, prefix)) {
		zlog_debug("2sogvvHdXW27VzAV ""GR receive: Neighbor not advertised %pFX",
			   prefix->destination);
			  ...
			  struct eigrp_interface *ei, int size)
{
	...
	if ((flags == (EIGRP_INIT_FLAG + EIGRP_RS_FLAG + EIGRP_EOT_FLAG))
		...
		zlog_info("j4t46drHPKJU8xBQ ""Neighbor %pI4 (%s) is resync: peer graceful-restart",
			  &nbr->src,
			  ifindex2ifname(nbr->ei->ifp->ifindex, eigrp->vrf_id));
		...
		zlog_info("83wJuLtH9OVIlqIL ""Neighbor %pI4 (%s) is resync: peer graceful-restart",
			  &nbr->src,
			  ifindex2ifname(nbr->ei->ifp->ifindex, eigrp->vrf_id));
		...
		if (nbr->state == EIGRP_NEIGHBOR_UP) {
			...
			nbr->recv_sequence_number = ntohl(eigrph->sequence);
			zlog_info("0OcS91A3G1L0g24z ""Neighbor %pI4 (%s) is down: peer restarted",
				  &nbr->src,
				  ifindex2ifname(nbr->ei->ifp->ifindex,
						 eigrp->vrf_id));
			eigrp_nbr_state_set(nbr, EIGRP_NEIGHBOR_PENDING);
			zlog_info("z2UjZMwfbXsIum4Q "
...
void eigrp_update_send_init(struct eigrp_neighbor *nbr)
{
	...
	if (IS_DEBUG_EIGRP_PACKET(0, RECV))
		zlog_debug("PHOGzDdraRcOqQ6F ""Enqueuing Update Init Seq [%u] Ack [%u]",
...
void eigrp_update_send(struct eigrp_interface *ei)
{
	...
	if (IS_DEBUG_EIGRP_PACKET(0, RECV))
		zlog_debug("vRmxRmHKe3xtcL1P ""Enqueuing Update length[%u] Seq [%u]", length,
			   ep->sequence_number);
...
static void eigrp_update_send_GR_part(struct eigrp_neighbor *nbr)
{
	...
	for (rn = route_top(eigrp->topology_table); rn; rn = route_next(rn)) {
		...
		if (eigrp_update_prefix_apply(eigrp, ei, EIGRP_FILTER_OUT,
			...
			zlog_info("n1RkJz070X3tCofI ""Filtered prefix %pI4 won't be sent out.",
				  &dest_addr->u.prefix4);
		} else {
		...
		if (eigrp_update_prefix_apply(eigrp, ei, EIGRP_FILTER_IN,
					      dest_addr)) {
			/* do not send filtered route */
			zlog_info("xVpz780JZKhWmFwu ""Filtered prefix %pI4 will be removed.",
				  &dest_addr->u.prefix4);
			  ...
			  struct vty *vty)
{
	...
	if (gr_type == EIGRP_GR_FILTER) {
		...
		zlog_info("vE4WVwENN67R6z4B ""Neighbor %pI4 (%s) is resync: manually cleared",
			  &nbr->src,
			  ifindex2ifname(ei->ifp->ifindex, eigrp->vrf_id));
