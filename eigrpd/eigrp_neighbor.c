...
void holddown_timer_expired(struct event *thread)
{
	...
	zlog_info("zFg8BecfbWOBQkiP ""Neighbor %pI4 (%s) is down: holding time expired", &nbr->src,
...
void eigrp_nbr_hard_restart(struct eigrp_neighbor *nbr, struct vty *vty)
{
	...
	zlog_debug("wl7hZVpJZgCMA2PC ""Neighbor %pI4 (%s) is down: manually cleared", &nbr->src,
		   ifindex2ifname(nbr->ei->ifp->ifindex, eigrp->vrf_id));
	if (vty != NULL) {
