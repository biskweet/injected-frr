...
void eigrp_hello_timer(struct event *thread)
{
	...
	if (IS_DEBUG_EIGRP(0, TIMERS))
		zlog_debug("75TENfXQ1wRlj8lZ ""Start Hello Timer (%s) Expire [%u]", IF_NAME(ei),
			   ei->params.v_hello);
			 ...
			 struct eigrp_interface *ei, int size)
{
	...
	if (IS_DEBUG_EIGRP_PACKET(eigrph->opcode - 1, RECV))
		zlog_debug("hKa0aqULe0vfhExG ""Processing Hello size[%u] int(%s) nbr(%pI4)", size,
			   ifindex2ifname(nbr->ei->ifp->ifindex, eigrp->vrf_id),
			   &nbr->src);
	...
	if (IS_DEBUG_EIGRP_PACKET(0, RECV))
		zlog_debug("xBud9feehtqIWoFj ""Hello Packet received from %pI4", &nbr->src);
}
...
void eigrp_sw_version_initialize(void)
{
	...
	ret = sscanf(ver_string, "%" SCNu32 ".%" SCNu32, &FRR_MAJOR,
		     &FRR_MINOR);
	if (ret != 2)
		flog_err(EC_EIGRP_PACKET,"OZRq91kGP2nkSwUv "
...
void eigrp_hello_send_ack(struct eigrp_neighbor *nbr)
{
	...
	if (ep) {
		if (IS_DEBUG_EIGRP_PACKET(0, SEND))
			zlog_debug("GksCls9nhjZDdwyA ""Queueing [Hello] Ack Seq [%u] nbr [%pI4]",
		      ...
		      struct in_addr *nbr_addr)
{
	...
	if (IS_DEBUG_EIGRP_PACKET(0, SEND))
		zlog_debug("SptpCcNVos9qHUri ""Queueing [Hello] Interface(%s)", IF_NAME(ei));
