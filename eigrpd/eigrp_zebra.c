...
static int eigrp_interface_address_add(ZAPI_CALLBACK_ARGS)
{
	...
	if (IS_DEBUG_EIGRP(zebra, ZEBRA_INTERFACE))
		zlog_debug("Tld7xw5pz2I3A4sG ""Zebra: interface %s address add %pFX", c->ifp->name,
			   c->address);
...
static int eigrp_interface_address_delete(ZAPI_CALLBACK_ARGS)
{
	...
	if (IS_DEBUG_EIGRP(zebra, ZEBRA_INTERFACE))
		zlog_debug("S6giuiFjqTahoh91 ""Zebra: interface %s address delete %pFX",
			   ...
			   struct list *successors, uint32_t distance)
{
	...
	if (IS_DEBUG_EIGRP(zebra, ZEBRA_REDISTRIBUTE)) {
		zlog_debug("wFzkfwFK6NZTdgP5 ""Zebra: Route add %pFX", p);
	}
...
void eigrp_zebra_route_delete(struct eigrp *eigrp, struct prefix *p)
{
	...
	if (IS_DEBUG_EIGRP(zebra, ZEBRA_REDISTRIBUTE))
		zlog_debug("dAQ0d3qmBzwujKZC ""Zebra: Route del %pFX", p);
			   ...
			   struct eigrp_metrics metric)
{
	...
	if (eigrp_is_type_redistributed(type, eigrp->vrf_id)) {
		...
		//      if (IS_DEBUG_EIGRP(zebra, ZEBRA_REDISTRIBUTE))
		//        zlog_debug("FIqZDemCNsdWt0ID ""Redistribute[%s]: Refresh  Type[%d],
		//        Metric[%d]",
