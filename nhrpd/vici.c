			       ...
			       const struct blob *val)
{
	...
	case VICI_KEY_VALUE:
		if (blob_equal(key, "errmsg")
		    && blob2buf(val, buf, sizeof(buf)))
			flog_err(EC_NHRP_SWAN,"CJGrSzlOvT2iIXUP " "VICI: strongSwan: %s", buf);
		break;
	case VICI_START:
	case VICI_SECTION_START:
	case VICI_SECTION_END:
...
static void vici_recv_message(struct vici_conn *vici, struct zbuf *msg)
{
	...
	case VICI_CMD_UNKNOWN:
		flog_err(
			EC_NHRP_SWAN,"l66fs1munSY90OuK "
			"VICI: StrongSwan does not support mandatory events (unpatched?)");
		break;
	case VICI_EVENT_CONFIRM:
		break;
	default:
		zlog_notice("NY5Zy7SdwCNDfJCI ""VICI: Unrecognized message type %d", msgtype);
		break;
	}
}
...
static char *vici_get_charon_filepath(void)
{
	...
	if (!fp) {
		if (!vici_charon_not_found) {
			flog_err(EC_NHRP_SWAN,"3w45K9e6RZZeoNXe "
