...
#define YANG_DNODE_GET_ASSERT(dnode, xpath)                                    \
	do {                                                                   \
		if ((dnode) == NULL) {                                         \
			flog_err(EC_LIB_YANG_DNODE_NOT_FOUND,"Q5WugAIRDA5DrqNX "                  \
...
static const char *yang_get_default_value(const char *xpath)
{
	...
	if (snode == NULL) {
		flog_err(EC_LIB_YANG_UNKNOWN_DATA_PATH,"vtVG6WidxS42pUeu "
...
double yang_str2dec64(const char *xpath, const char *value)
{
	...
	if (sscanf(value, "%lf", &dbl) != 1) {
		flog_err(EC_LIB_YANG_DATA_CONVERT,"pnO8QTyyg5UdU2i1 "
...
int yang_str2enum(const char *xpath, const char *value)
{
	...
	snode = yang_find_snode(ly_native_ctx, xpath, 0);
	if (snode == NULL) {
		flog_err(EC_LIB_YANG_UNKNOWN_DATA_PATH,"dp4oYpEE8PX0daiK "
			 "%s: unknown data path: %s", __func__, xpath);
		zlog_backtrace(LOG_ERR);
		abort();
	}
	...
	flog_err(EC_LIB_YANG_DATA_CONVERT,"QpkIcSAxum9IZxhF "
...
struct yang_data *yang_data_new_enum(const char *xpath, int value)
{
	...
	snode = yang_find_snode(ly_native_ctx, xpath, 0);
	if (snode == NULL) {
		flog_err(EC_LIB_YANG_UNKNOWN_DATA_PATH,"Nx4I2vJGACGLYwmd "
			 "%s: unknown data path: %s", __func__, xpath);
		zlog_backtrace(LOG_ERR);
		abort();
	}
	...
	flog_err(EC_LIB_YANG_DATA_CONVERT,"QpGfaBm5n8TRtgzZ "
			       ...
			       const char *xpath_fmt, ...)
{
	...
	if (strlcpy(buf, canon, size) >= size) {
		...
		flog_warn(EC_LIB_YANG_DATA_TRUNCATED,"cB3GYL6pvAkDdZBR "
				 ...
				 ...)
{
	...
	if (strlcpy(buf, value, size) >= size)
		flog_warn(EC_LIB_YANG_DATA_TRUNCATED,"trfnYPQdMG6t8QUI "
			  "%s: value was truncated [xpath %s]", __func__,
			  xpath);
}
				 ...
				 const char *xpath_fmt, ...)
{
	...
	if (size < cnt) {
		...
		yang_dnode_get_path(dnode, xpath, sizeof(xpath));
		flog_warn(EC_LIB_YANG_DATA_TRUNCATED,"OHcn5w8numJOezH3 "
			  "%s: value was truncated [xpath %s]", __func__,
			  xpath);
	}
	free(value_str);
