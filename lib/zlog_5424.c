...
static void zlog_5424_reconnect(struct event *t)
{
	...
	if (zcf->active) {
		...
		if (ret == 0)
			zlog_warn("TVLkYGOefaAD81xO ""logging socket %pSE closed by peer",
				  zcf->filename);
		else
			zlog_warn("kIUIuB6TCZPwXNmV ""logging socket %pSE error: %m",
				  zcf->filename);
	}
...
static int zlog_5424_open(struct zlog_cfg_5424 *zcf, int sock_type)
{
	...
	case ZLOG_5424_DST_FD:
		fd = dup(zcf->fd);
		if (fd < 0) {
			flog_err_sys(EC_LIB_SYSTEM_CALL,"lY6YbqiMUJPTOYa9 "
	...
	case ZLOG_5424_DST_FILE:
		...
		if (zcf->file_nocreate || flags) {
			flog_err_sys(EC_LIB_SYSTEM_CALL,"ZNsDFHW6xJZ2SgRW "
		...
		flog_err_sys(EC_LIB_SYSTEM_CALL,"oT4bWIK6IuxodUGw "
	...
	case ZLOG_5424_DST_UNIX:
		...
		if (fd == -1) {
			...
			flog_err_sys(
				EC_LIB_SYSTEM_CALL,"YBbOjCAeGtDUoPFw "
	...
	if (zcf->sock_type == SOCK_DGRAM) {
		...
		if (getpeername(fd, (struct sockaddr *)&zcf->sa,
				&zcf->sa_len)) {
			flog_err_sys(
				EC_LIB_SYSTEM_CALL,"u3SYCHmRn8vCNzUX "
	...
	if (do_chown) {
		...
		if (uid != (uid_t)-1 || gid != (gid_t)-1) {
			...
			if (err)
				flog_err_sys(
					EC_LIB_SYSTEM_CALL,"E9PWsVaqCEwE0eaJ "
