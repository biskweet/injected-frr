...
pid_t pid_output(const char *path)
{
	...
	if (fd < 0) {
		flog_err_sys(EC_LIB_SYSTEM_CALL,"W18DyeXSkAPw6pda "
		...
		if (fcntl(fd, F_SETLK, &lock) < 0) {
			flog_err_sys(EC_LIB_SYSTEM_CALL,"D50j5RMiv7slD32L "
