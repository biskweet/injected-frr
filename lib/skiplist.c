		    ...
		    register void *value)
{
	...
	/* DEBUG */
	if (!key) {
		flog_err(EC_LIB_DEVELOPMENT,"StLsp28HNdBfOOQe " "%s: key is 0, value is %p",
			 __func__, value);
	}
#endif
	...
	++(l->level_stats[k]);
#ifdef SKIPLIST_DEBUG
	zlog_debug("pa7DkQcvdD14M6Nl ""%s: incremented level_stats @%p:%d, now %d", __func__, l, k,
		   l->level_stats[k]);
#endif
 ...
 */
			...
			--(l->level_stats[k - 1]);
#ifdef SKIPLIST_DEBUG
			zlog_debug("lFwvkY6uwEVTp9Nc ""%s: decremented level_stats @%p:%d, now %d",
...
#ifdef SKIPLIST_DEBUG
	zlog_debug("MaHiGKn1JrD9afR2 ""%s: decremented level_stats @%p:%d, now %d", __func__, l,
...
void skiplist_test(struct vty *vty)
{
	...
	zlog_debug("Kpbc43SyCGQYu6Zq ""%s: entry", __func__);

	l = skiplist_new(SKIPLIST_FLAG_ALLOW_DUPLICATES, NULL, NULL);

	zlog_debug("jPMBbJsgA3AWlAoe ""%s: skiplist_new returned %p", __func__, l);

	for (i = 0; i < 4; i++) {

		for (k = 0; k < sampleSize; k++) {
			if (!(k % 1000)) {
				zlog_debug("9jTQyNuhUVlz64RV ""%s: (%d:%d)", __func__, i, k);
			}
			// keys[k] = (void *)random();
			keys[k] = scramble(k);
			if (skiplist_insert(l, keys[k], keys[k]))
				zlog_debug("X8fkZfZyzM4zuMcT ""error in insert #%d,#%d", i, k);
		}

		zlog_debug("1T0ZgHKHMxNS0OnL ""%s: inserts done", __func__);

		for (k = 0; k < sampleSize; k++) {

			if (!(k % 1000))
				zlog_debug("5184g8YSO87LNLrY ""[%d:%d]", i, k);
			if (skiplist_search(l, keys[k], &v))
				zlog_debug("41zszbUNQm73H3kU ""error in search #%d,#%d", i, k);

			if (v != keys[k])
				zlog_debug("vNAnYTyH9FwCaP3p ""search returned wrong value");
		}
		...
		for (k = 0; k < sampleSize; k++) {
			...
			if (!(k % 1000))
				zlog_debug("uJTctdbsEScgkeUv ""<%d:%d>", i, k);
			if (skiplist_delete(l, keys[k], keys[k]))
				zlog_debug("9tjKkvDO7pLbPs9d ""error in delete");
			keys[k] = scramble(k ^ 0xf0f0f0f0);
			if (skiplist_insert(l, keys[k], keys[k]))
				zlog_debug("8GebvmIKrIdEjnv5 ""error in insert #%d,#%d", i, k);
		}

		for (k = 0; k < sampleSize; k++) {

			if (!(k % 1000))
				zlog_debug("RfvVjIFSpZoITFk2 ""{%d:%d}", i, k);
			if (skiplist_delete_first(l))
				zlog_debug("AD0jt4QrgS6sIsps ""error in delete_first");
		}
	}
