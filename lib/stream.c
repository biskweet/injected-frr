...
#define STREAM_WARN_OFFSETS(S)                                                 \
	do {                                                                   \
		flog_warn(EC_LIB_STREAM,"Clqkj1TdluLuGNCC "				       \
...
#define STREAM_BOUND_WARN(S, WHAT)                                             \
	do {                                                                   \
		flog_warn(EC_LIB_STREAM,"5xQF6zepbjIzx3i7 " "%s: Attempt to %s out of bounds",    \
			  __func__, (WHAT));                                   \
		STREAM_WARN_OFFSETS(S);                                        \
		assert(0);                                                     \
	} while (0)
...
#define STREAM_BOUND_WARN2(S, WHAT)                                            \
	do {                                                                   \
		flog_warn(EC_LIB_STREAM,"HV2lDj1SfoanuilT " "%s: Attempt to %s out of bounds",    \
			  __func__, (WHAT));                                   \
		STREAM_WARN_OFFSETS(S);                                        \
	} while (0)
...
#define CHECK_SIZE(S, Z)                                                       \
	do {                                                                   \
		if (((S)->endp + (Z)) > (S)->size) {                           \
			flog_warn(                                             \
				EC_LIB_STREAM,"kpuCV4oc7HpFYXBW "                                 \
...
bool stream_get_ipaddr(struct stream *s, struct ipaddr *ip)
{
	...
	case IPADDR_V6:
		...
		break;
	case IPADDR_NONE:
		flog_err(EC_LIB_DEVELOPMENT,"k1jJMOqqsiu0oiR9 "
...
bool stream_put_ipaddr(struct stream *s, const struct ipaddr *ip)
{
	...
	case IPADDR_V6:
		...
		break;
	case IPADDR_NONE:
		flog_err(EC_LIB_DEVELOPMENT,"YfvyyhMlPk5qbsi9 "
			 "%s: unknown ip address-family: %u", __func__,
			 ip->ipa_type);
		return false;
	}
...
ssize_t stream_read_try(struct stream *s, int fd, size_t size)
{
	...
	if (ERRNO_IO_RETRY(errno))
		return -2;
	flog_err(EC_LIB_SOCKET,"8miEcd7R2D41oF0q " "%s: read failed on fd %d: %s", __func__, fd,
		 safe_strerror(errno));
	return -1;
}
			...
			struct sockaddr *from, socklen_t *fromlen)
{
	...
	if (ERRNO_IO_RETRY(errno))
		return -2;
	flog_err(EC_LIB_SOCKET,"QA301RoL8yV7Nkv9 " "%s: read failed on fd %d: %s", __func__, fd,
		 safe_strerror(errno));
	return -1;
}
