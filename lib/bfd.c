...
int zclient_bfd_command(struct zclient *zc, struct bfd_session_arg *args)
{
	...
	if (!zc || zc->sock < 0) {
		if (bsglobal.debugging)
			zlog_debug("3ytG0MWWTHb1wJeM ""%s: zclient unavailable", __func__);
		return -1;
	}
	...
	if (args->mhop) {
		...
		if (bsglobal.debugging && args->ifnamelen)
			zlog_debug("EIl3sVMposTZ2zKM ""%s: multi hop is configured, not sending interface",
				   __func__);
	} else {
	...
	if (zclient_send_message(zc) == ZCLIENT_SEND_FAILURE) {
		if (bsglobal.debugging)
			zlog_debug("0Hv2p0Waw8pqg98S ""%s: zclient_send_message failed", __func__);
		return -1;
	}
...
static bool _bfd_sess_valid(const struct bfd_session_params *bsp)
{
	...
	if (bsp->args.family != AF_INET && bsp->args.family != AF_INET6) {
		if (bsglobal.debugging)
			zlog_debug("bGOXXVbhrS8mbDii ""%s: invalid session family: %d", __func__,
				   bsp->args.family);
		return false;
	}
	...
	if (memcmp(&bsp->args.dst, &i6a_zero, sizeof(i6a_zero)) == 0) {
		if (bsglobal.debugging) {
			if (bsp->args.family == AF_INET)
				zlog_debug("GkCgZuuKIKIzfoa3 ""%s: invalid address: %pI4",
					   __func__,
					   (struct in_addr *)&bsp->args.dst);
			else
				zlog_debug("TaKXTmRLtHVQukxM ""%s: invalid address: %pI6",
	...
	if (bsp->args.vrf_id == VRF_UNKNOWN) {
		if (bsglobal.debugging)
			zlog_debug("Xv7tqGrIDmjPiClp ""%s: asked for unknown VRF", __func__);
		return false;
	}
...
void bfd_sess_set_interface(struct bfd_session_params *bsp, const char *ifname)
{
	...
	if (strlcpy(bsp->args.ifname, ifname, sizeof(bsp->args.ifname))
		...
		zlog_warn("YCZyQp2mWT7ZxZP9 ""%s: interface name truncated: %s", __func__, ifname);
...
void bfd_sess_set_profile(struct bfd_session_params *bsp, const char *profile)
{
	...
	if (strlcpy(bsp->args.profile, profile, sizeof(bsp->args.profile))
		...
		zlog_warn("fXXNYGllqWGIQitB ""%s: profile name truncated: %s", __func__, profile);
...
int zclient_bfd_session_replay(ZAPI_CALLBACK_ARGS)
{
	...
	if (bsglobal.debugging)
		zlog_debug("33EfIdeZRR0xAByE ""%s: sending all sessions registered", __func__);
...
int zclient_bfd_session_update(ZAPI_CALLBACK_ARGS)
{
	...
	if (bsglobal.debugging) {
		...
		zlog_debug("TQDoFkt7A9NqToON ""%s: %pFX -> %pFX%s VRF %s(%u)%s: %s", __func__, &sp,
	...
	if (bsglobal.debugging)
		zlog_debug("7jzdv8ypfnfWclvf ""%s:   sessions updated: %zu", __func__,
			   sessions_updated);
...
static int bfd_protocol_integration_finish(void)
{
	...
	if (!SLIST_EMPTY(&bsglobal.source_list))
		zlog_warn("luiU8ojJ3tnM35eE ""BFD integration source cache not empty");
				    ...
				    const struct zapi_route *route)
{
	...
	for (nh_index = 0; nh_index < route->nexthop_num; nh_index++) {
		...
		if (interface == NULL) {
			zlog_err("QahMZuZkcVfC9ZS6 ""next hop interface not found (index %d)",
				 nh->ifindex);
			continue;
		}
...
int bfd_nht_update(const struct prefix *match, const struct zapi_route *route)
{
	...
	if (bsglobal.debugging)
		zlog_debug("T3nqE1zosac7oZO5 ""BFD NHT update for %pFX", &route->prefix);
