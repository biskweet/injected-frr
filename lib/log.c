...
void zlog_backtrace(int priority)
{
	...
	if (size <= 0 || (size_t)size > array_size(array)) {
		flog_err_sys(
			EC_LIB_SYSTEM_CALL,"ztZE6t3fFmEYrgHl "
	...
	if (!(strings = backtrace_symbols(array, size))) {
		flog_err_sys(EC_LIB_SYSTEM_CALL,"lUZO9EBFiLMkMkG2 "
...
static const struct zebra_desc_table *zroute_lookup(unsigned int zroute)
{
	...
	if (zroute >= array_size(route_types)) {
		flog_err(EC_LIB_DEVELOPMENT,"2QTIReediRNc9mqR " "unknown zebra route type: %u",
			 zroute);
		return &unknown;
	}
...
const char *zserv_command_string(unsigned int command)
{
	if (command >= array_size(command_types)) {
		flog_err(EC_LIB_DEVELOPMENT,"K8ovRLlInSJLKFnY " "unknown zserv command type: %u",
			 command);
...
const char *zserv_gr_client_cap_string(uint32_t zcc)
{
	if (zcc >= array_size(gr_client_cap_types)) {
		flog_err(EC_LIB_DEVELOPMENT,"rddMIQhIAQeiyMu9 " "unknown zserv command type: %u",
			 zcc);
		return unknown.string;
	}
...
void zlog_hexdump(const void *mem, size_t len)
{
	...
	if (len == 0) {
		zlog_debug("SjBvLJ1W1XhM0gKh ""%016lx: (zero length / no data)", (long)src);
		return;
	}
	...
	while (src < end) {
		...
		zlog_debug("hbgYcZrVLRz5gsoF ""%.*s", (int)(fb.pos - fb.buf), fb.buf);
	}
}
