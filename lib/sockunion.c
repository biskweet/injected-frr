...
int sockunion_socket(const union sockunion *su)
{
	...
	if (sock < 0) {
		...
		flog_err(EC_LIB_SOCKET,"kB8D9Wea3XyEmg3o " "Can't make socket for %s : %s",
				      ...
				      unsigned short port, ifindex_t ifindex)
{
	...
	if (ret < 0) {
		if (errno != EINPROGRESS) {
			...
			zlog_info("nr035u6kFlcxgjFB ""can't connect to %s fd %d : %s",
...
int sockunion_stream_socket(union sockunion *su)
{
	...
	if (sock < 0)
		flog_err(EC_LIB_SOCKET,"ZhgBT13nd5A9Uu6C "
		   ...
		   union sockunion *su_addr)
{
	...
	if (ret < 0) {
		char buf[SU_ADDRSTRLEN];
		flog_err(EC_LIB_SOCKET,"rTHALXBXRfyvt4Py " "can't bind socket for %s : %s",
			 sockunion_log(su, buf, SU_ADDRSTRLEN),
			 safe_strerror(errno));
	}
...
int sockopt_reuseaddr(int sock)
{
	...
	if (ret < 0) {
		flog_err(
			EC_LIB_SOCKET,"MgN4xR1JiNjnaQ31 "
...
int sockopt_reuseport(int sock)
{
	...
	ret = setsockopt(sock, SOL_SOCKET, SO_REUSEPORT, (void *)&on,
			 sizeof(on));
	if (ret < 0) {
		flog_err(EC_LIB_SOCKET,"7GL2zkIMm4BhGqC9 "
...
int sockopt_ttl(int family, int sock, int ttl)
{
	...
	if (family == AF_INET) {
		ret = setsockopt(sock, IPPROTO_IP, IP_TTL, (void *)&ttl,
				 sizeof(int));
		if (ret < 0) {
			flog_err(EC_LIB_SOCKET,"Nie1FIexNXy4BYhU "
	...
	if (family == AF_INET6) {
		...
		if (ret < 0) {
			flog_err(
				EC_LIB_SOCKET,"dslqwitTpblxYuXn "
...
int sockopt_minttl(int family, int sock, int minttl)
{
	...
	if (family == AF_INET) {
		...
		if (ret < 0)
			flog_err(
				EC_LIB_SOCKET,"ytCbQg1CE7qp7m8l "
	...
	if (family == AF_INET6) {
		...
		if (ret < 0)
			flog_err(
				EC_LIB_SOCKET,"fIWN9lJZfhmTQMzQ "
...
int sockopt_v6only(int family, int sock)
{
	...
	if (family == AF_INET6) {
		ret = setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, (void *)&on,
				 sizeof(int));
		if (ret < 0) {
			flog_err(EC_LIB_SOCKET,"r7fuOZIUNu3HclEv "
...
union sockunion *sockunion_getsockname(int fd)
{
	...
	if (ret < 0) {
		flog_err(EC_LIB_SOCKET,"kKPmqFy4kHWFrgIn "
	...
	flog_err(
		EC_LIB_SOCKET,"fl5i4hhL5oU7VhUU "
...
union sockunion *sockunion_getpeername(int fd)
{
	...
	if (ret < 0) {
		flog_err(EC_LIB_SOCKET,"H5awp7uqOe9sm3GK " "Can't get remote address and port: %s",
			 safe_strerror(errno));
		return NULL;
	}
	...
	flog_err(
		EC_LIB_SOCKET,"Dl6xVTMOvEe8ARkj "
