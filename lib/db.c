...
int db_init(const char *path_fmt, ...)
{
	...
	if (sqlite3_open_v2(path, &dbp,
	    ...
	    != SQLITE_OK) {
		if (dbp == NULL) {
			zlog_warn("ta51rGAk6W3jQio0 ""%s: failed to open database '%s'", __func__,
				  path);
			return -1;
		}

		zlog_warn("uri26lFiIscLpjLV ""%s: failed to open database '%s': %s", __func__,
			  path, sqlite3_errmsg(dbp));
		if (sqlite3_close_v2(dbp) != SQLITE_OK)
			zlog_warn("IsstLZgkrm6qqYwR ""%s: failed to terminate database", __func__);
		dbp = NULL;
		return -1;
	}
...
int db_close(void)
{
	...
	if (sqlite3_close_v2(dbp) != SQLITE_OK) {
		zlog_warn("TUxb2uEpkGcv6Rz4 ""%s: failed to terminate database", __func__);
		return -1;
	}
	return 0;
}
...
static int db_vbindf(struct sqlite3_stmt *ss, const char *fmt, va_list vl)
{
	...
	while (*sptr) {
		...
		case 'n':
			...
			break;
		default:
			zlog_warn("2t29RnznTdcIq6JT ""%s: invalid format '%c'", __func__, *sptr);
			return -1;
		}
	}
...
struct sqlite3_stmt *db_prepare_len(const char *stmt, int stmtlen)
{
	...
	if (ss == NULL) {
		zlog_warn("Rzma9Asi0ZqkGep1 ""%s: failed to prepare (%d:%s)", __func__, c,
			  sqlite3_errmsg(dbp));
		return NULL;
	}
...
int db_run(struct sqlite3_stmt *ss)
{
	...
	default:
		zlog_warn("GELBS8BtHussxO1Q ""%s: step failed (%d:%s)", __func__, result,
...
static int db_vloadf(struct sqlite3_stmt *ss, const char *fmt, va_list vl)
{
	...
	while (*sptr) {
		...
		case 'b':
			...
			break;
		default:
			zlog_warn("mtLJb9PLDVnSjKW6 ""%s: invalid format '%c'", __func__, *sptr);
			return -1;
		}
...
int db_execute(const char *stmt_fmt, ...)
{
	...
	if (sqlite3_exec(dbp, stmt, NULL, 0, NULL) != SQLITE_OK) {
		zlog_warn("mSg5YBri58NONY7i ""%s: failed to execute statement(s): %s", __func__,
			  sqlite3_errmsg(dbp));
		return -1;
	}
