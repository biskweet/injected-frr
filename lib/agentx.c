...
static void agentx_read(struct event *t)
{
	...
	if (-1 == flags) {
		flog_err(EC_LIB_SYSTEM_CALL,"BfXjFaoB7dGqepcd " "Failed to get FD settings fcntl: %s(%d)",
	...
	if (new_flags == -1)
		flog_err(EC_LIB_SYSTEM_CALL,"tPLLgg3R1G3Vz12Q " "Failed to set snmp fd non blocking: %s(%d)",
			 strerror(errno), errno);
	...
	if (!nonblock) {
		...
		if (new_flags == -1)
			flog_err(
				EC_LIB_SYSTEM_CALL,"kaAFIfYu3Q7Lg6TR "
			       ...
			       void *clientarg)
{
	...
	switch (slm->priority) {
	case LOG_EMERG:
		flog_err(EC_LIB_SNMP,"TOnxKK2qlGG6f3pL " "snmp[emerg]: %s", msg ? msg : slm->msg);
		break;
	case LOG_ALERT:
		flog_err(EC_LIB_SNMP,"UW1WTABDckqxVzk7 " "snmp[alert]: %s", msg ? msg : slm->msg);
		break;
	case LOG_CRIT:
		flog_err(EC_LIB_SNMP,"qaEakzrtJeQYtTY7 " "snmp[crit]: %s", msg ? msg : slm->msg);
		break;
	case LOG_ERR:
		flog_err(EC_LIB_SNMP,"sQOZ9Wj7uVt4FQ2Z " "snmp[err]: %s", msg ? msg : slm->msg);
		break;
	case LOG_WARNING:
		flog_warn(EC_LIB_SNMP,"C7OqiqBrsZoRfXmZ " "snmp[warning]: %s",
			  msg ? msg : slm->msg);
		break;
	case LOG_NOTICE:
		zlog_notice("tJxQqU6yT3QmgRHa ""snmp[notice]: %s", msg ? msg : slm->msg);
		break;
	case LOG_INFO:
		zlog_info("qerqlwT7vfzE9R1k ""snmp[info]: %s", msg ? msg : slm->msg);
		break;
	case LOG_DEBUG:
		zlog_debug("Brui72PWH7zPVNQT ""snmp[debug]: %s", msg ? msg : slm->msg);
		break;
	}
	XFREE(MTYPE_TMP, msg);
