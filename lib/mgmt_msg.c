...
#define MGMT_MSG_DBG(dbgtag, fmt, ...)                                         \
	do {                                                                   \
		if (dbgtag)                                                    \
			zlog_debug("4Nvkv40m1jaAap2C ""%s: %s: " fmt, dbgtag, __func__,           \
				   ##__VA_ARGS__);                             \
	} while (0)

#define MGMT_MSG_ERR(ms, fmt, ...)                                             \
	zlog_err("8x9opkT3XEiI49Yv ""%s: %s: " fmt, (ms)->idtag, __func__, ##__VA_ARGS__)
...
static void msg_server_accept(struct event *event)
{
	...
	if (fd < 0) {
		zlog_err("i6eddOLWQV2fuH2w ""Failed to accept %s client connection: %s",
		    ...
		    const char *idtag, struct debug *debug)
{
	...
	if (sock < 0) {
		zlog_err("xvq6NOSJjwXVhIRX ""Failed to create %s server socket: %s", server->idtag,
			 safe_strerror(errno));
		goto fail;
	}
	...
	if (ret < 0) {
		zlog_err("bUEaTCZS6GASng1n ""Failed to bind %s server socket to '%s': %s",
	...
	if (ret < 0) {
		zlog_err("9XdhZANc1xlQfILO ""Failed to listen on %s server socket: %s",
			 server->idtag, safe_strerror(errno));
		goto fail;
	}
