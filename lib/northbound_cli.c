...
int nb_cli_rpc(struct vty *vty, const char *xpath, struct lyd_node **output_p)
{
	...
	if (!nb_node) {
		flog_warn(EC_LIB_YANG_UNKNOWN_DATA_PATH,"tJ2FJEYdO9juQxVa "
				      ...
				      const char *path, bool replace)
{
	...
	case NB_CFG_FMT_XML:
		...
		err = lyd_parse_data_path(ly_ctx, path, ly_format,
					  ...
					  0, &dnode);
		if (err || !dnode) {
			flog_warn(EC_LIB_LIBYANG,"QMEZ5L12dzQkVU4Y " "%s: lyd_parse_path() failed",
				  __func__);
			vty_out(vty, "%% Failed to load configuration:\n\n");
			vty_out(vty, "%s",
			   ...
			   size_t pathlen)
{
	...
	fd = mkstemp(path);
	if (fd < 0) {
		flog_warn(EC_LIB_SYSTEM_CALL,"6BEv9qgEgv35FzY1 " "%s: mkstemp() failed: %s",
			  __func__, safe_strerror(errno));
		return -1;
	}
	if (fchmod(fd, CONFIGFILE_MASK) != 0) {
		flog_warn(EC_LIB_SYSTEM_CALL,"hFd3Cu24ZGmp7PGH "
			       ...
			       struct yang_data *data, void *arg)
{
	...
	if (err) {
		flog_warn(EC_LIB_LIBYANG,"eH6JqxkCwpZcm8B3 " "%s: lyd_new_path(%s) failed: %s",
