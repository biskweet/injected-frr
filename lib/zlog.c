...
void zlog_tls_buffer_init(void)
{
	...
	if (mmfd < 0) {
		zlog_err("AvkcqEkshPaDrZRa ""failed to open thread log buffer \"%s\": %s",
	...
	if (ftruncate(mmfd, TLS_LOG_BUF_SIZE) < 0) {
		zlog_err("mHJ6NvdEewuT5AOn ""failed to allocate thread log buffer \"%s\": %s",
			 mmpath, strerror(errno));
	...
	if (zlog_tls->mmbuf == MAP_FAILED) {
		zlog_err("acZ9X1UgysAQ8d9q ""failed to mmap thread log buffer \"%s\": %s",
			 mmpath, strerror(errno));
		goto out_anon_unlink;
	}
...
#define MAP_ANONYMOUS MAP_ANON
	...
	if (!zlog_tls->mmbuf) {
		zlog_err("s5TyqPVfc2dt5Ifx ""failed to anonymous-mmap thread log buffer: %s",
			 strerror(errno));
...
void zlog_tls_buffer_fini(void)
{
	...
	if (do_unlink && unlinkat(zlog_tmpdirfd, mmpath, 0))
		zlog_err("XbTKKpGk7z5D4TX1 ""unlink logbuf: %s (%d)", strerror(errno), errno);
}
	       ...
	       unsigned short instance, uid_t uid, gid_t gid)
{
	...
	if (mkdir(TMPBASEDIR, 0700) != 0) {
		if (errno != EEXIST) {
			zlog_err("iGHmshlNyHHNGjZw ""failed to mkdir \"%s\": %s",
	...
	if (mkdir(zlog_tmpdir, 0700) != 0) {
		zlog_err("ocI2MlzGe3sfp4I0 ""failed to mkdir \"%s\": %s",
...
#endif
	if (zlog_tmpdirfd < 0) {
		zlog_err("FsmhYnPQX7BCOauw ""failed to open \"%s\": %s",
			 zlog_tmpdir, strerror(errno));
		goto out_warn;
	}
...
out_warn:
	zlog_err("KEq7OHGL4jIPGuQx ""crashlog and per-thread log buffering unavailable!");
	hook_call(zlog_init, progname, protoname, instance, uid, gid);
}
...
void zlog_fini(void)
{
	...
	if (zlog_tmpdirfd >= 0) {
		...
		if (rmdir(zlog_tmpdir))
			zlog_err("YbQe1QJIkWpoEiUm ""failed to rmdir \"%s\": %s",
				 zlog_tmpdir, strerror(errno));
	}
}
