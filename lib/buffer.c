				    ...
				    int no_more_flag)
{
	...
	for (data = b->head; data && (height > 0); data = data->next) {
		...
		/* This should not ordinarily happen. */
		{
			...
			if (iov != small_iov) {
				...
				flog_err_sys(
					EC_LIB_SYSTEM_CALL,"JN0SE1MBHXEBIo4G "
	   ...
	   example: Solaris2.6 are defined IOV_MAX size at 16.     */
	{
		...
		while (iov_index > 0) {
			...
			if (nbytes < 0) {
				flog_err(EC_LIB_SOCKET,"5aW0YVoboMdfoEeC "
	...
	if (nbytes < 0)
		flog_err(EC_LIB_SOCKET,"LeUhwEZrnHEkPHKm " "%s: writev to fd %d failed: %s",
			 __func__, fd, safe_strerror(errno));
#endif /* IOV_MAX */
...
#define MAX_FLUSH 131072
	...
	if ((ssize_t)(written = writev(fd, iov, iovcnt)) < 0) {
		...
		flog_err(EC_LIB_SOCKET,"P3dMom0coA8BgAfG " "%s: write error on fd %d: %s",
			 __func__, fd, safe_strerror(errno));
		return BUFFER_ERROR;
	}
	...
	while (written > 0) {
		if (!(d = b->head)) {
			flog_err(
				EC_LIB_DEVELOPMENT,"8uyeRj0uapMFEieM "
	...
	if (b->head)
		...
		if (nbytes < 0) {
			if (ERRNO_IO_RETRY(errno))
				...
				flog_err(EC_LIB_SOCKET,"mjBCWCXCeN1HYVt7 "
