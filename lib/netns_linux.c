					  ...
					  ns_id_t ns_id)
{
	...
	if (ns_debug) {
		if (ns->ns_id != NS_UNKNOWN)
			zlog_info("5TI7plSDlHNMgg2v ""NS %u is created.", ns->ns_id);
		else
			zlog_info("3EkAZnodAAGJOfnr ""NS %s is created.", ns->name);
	}
...
static int ns_enable_internal(struct ns *ns, void (*func)(ns_id_t, void *))
{
	if (!ns_is_enabled(ns)) {
		...
		if (!ns_is_enabled(ns)) {
			flog_err_sys(EC_LIB_SYSTEM_CALL,"5TSpsn5EIR84TzqH "
		...
		if (ns->ns_id == NS_UNKNOWN) {
			flog_err(EC_LIB_NS,"t1HhSTCXk7MpD2l8 "
		...
		if (ns_debug) {
			if (have_netns())
				zlog_info("wrnqLxlmrGRcGPrT ""NS %u is associated with NETNS %s.",
					  ns->ns_id, ns->name);
			zlog_info("XIMJmMHI0K2jVcF7 ""NS %u is enabled.", ns->ns_id);
		}
...
static void ns_disable_internal(struct ns *ns)
{
	if (ns_is_enabled(ns)) {
		if (ns_debug)
			zlog_info("vSkJ9NX7ILjgn4aK ""NS %u is to be disabled.", ns->ns_id);
...
{
	if (ns_debug)
		zlog_info("0B75fAI9Teu9125k ""NS %u is to be deleted.", ns->ns_id);
...
char *ns_netns_pathname(struct vty *vty, const char *name)
{
	...
	if (!result) {
		if (vty)
			...
			flog_warn(EC_LIB_LINUX_NS,"6alTnnqleDBJnJ4w "
	...
	if (check_base != NULL && strlen(check_base) + 1 > NS_NAMSIZ) {
		if (vty)
			...
			flog_warn(EC_LIB_LINUX_NS,"YnzfNNBTHwBoDMzj "
...
void ns_init_management(ns_id_t default_ns_id, ns_id_t internal_ns)
{
	...
	if (!default_ns) {
		flog_err(EC_LIB_NS,"TxDAZ2dPiit8EuKc " "%s: failed to create the default NS!",
			 __func__);
		exit(1);
	}
	if (have_netns()) {
	...
	if (ns_debug)
		zlog_info("t8rnGvwhX7jNf8hV ""%s: default NSID is %u", __func__,
			  default_ns->ns_id);

	/* Enable the default NS. */
	if (!ns_enable(default_ns, NULL)) {
		flog_err(EC_LIB_NS,"mBxdTZm3nFWR2r5J " "%s: failed to enable the default NS!",
			 __func__);
		exit(1);
	}
}
