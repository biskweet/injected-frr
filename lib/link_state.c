...
stream_failure:
	zlog_err("BHRzJK76kmQzXLR3 ""LS(%s): Could not parse Link State Node. Abort!", __func__);
	XFREE(MTYPE_LS_DB, node);
	return NULL;
}
...
stream_failure:
	zlog_err("iYuCgemTKK55TCkV ""LS(%s): Could not parse Link State Attributes. Abort!",
		 __func__);
...
stream_failure:
	zlog_err("Qv1n7YgWhLtgEgXH ""LS(%s): Could not parse Link State Prefix. Abort!", __func__);
...
struct ls_message *ls_parse_msg(struct stream *s)
{
	...
	case LS_MSG_TYPE_PREFIX:
		...
		break;
	default:
		zlog_err("quTgwoLcN2MpTOT9 ""Unsupported Payload");
		goto stream_failure;
	}
...
stream_failure:
	zlog_err("oR0w34uwI9oUjWuV ""LS(%s): Could not parse LS message. Abort!", __func__);
...
static int ls_format_msg(struct stream *s, struct ls_message *msg)
{
	...
	default:
		zlog_warn("HgABIdz054B8tOl2 ""Unsupported Payload");
		break;
	}
...
void ls_dump_ted(struct ls_ted *ted)
{
	...
	zlog_debug("PQhNOx2sww9YlhkL ""(%s) Ted init", __func__);

	/* Loop TED, start printing Node, then Attributes and finally Prefix */
	frr_each (vertices, &ted->vertices, vertex) {
		zlog_debug("Jbxu13vBNCEHMV5C ""    Ted node (%s %pI4 %s)",
	...
	frr_each (edges, &ted->edges, edge) {
		zlog_debug("AETBRsICmHXUqgrP ""    Ted edge key:%s src:%pI4 dst:%pI4",
	...
	frr_each (subnets, &ted->subnets, subnet) {
		zlog_debug("B0JY5hlvekSP5LP6 ""    Ted subnet key:%pFX vertex:%pI4",
			   &subnet->ls_pref->pref,
			   &subnet->vertex->node->adv.id.ip.addr);
	}
	zlog_debug("SpVG7GZ0x96M59DP ""(%s) Ted end", __func__);
}
