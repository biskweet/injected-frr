...
int _frrscript_call_lua(struct lua_function_state *lfs, int nargs)
{
	...
	case LUA_OK:
		break;
	case LUA_ERRRUN:
		zlog_err("7XFstwteLHccMc31 ""Lua hook call '%s' : runtime error: %s", lfs->name,
			 lua_tostring(lfs->L, -1));
		break;
	case LUA_ERRMEM:
		zlog_err("4NiRoUNveQehRlil ""Lua hook call '%s' : memory error: %s", lfs->name,
			 lua_tostring(lfs->L, -1));
		break;
	case LUA_ERRERR:
		zlog_err("lH2sUNelJNUyQWNv ""Lua hook call '%s' : error handler error: %s",
			 lfs->name, lua_tostring(lfs->L, -1));
		break;
	case LUA_ERRGCMM:
		zlog_err("0FsRx5fsIRgiRtdY ""Lua hook call '%s' : garbage collector error: %s",
			 lfs->name, lua_tostring(lfs->L, -1));
		break;
	default:
		zlog_err("x22YZ8oWHi8KNJN7 ""Lua hook call '%s' : unknown error: %s", lfs->name,
			 lua_tostring(lfs->L, -1));
		break;
	}
		   ...
		   int (*load_cb)(struct frrscript *))
{
	...
	if (snprintf(script_name, sizeof(script_name), "%s/%s.lua", scriptdir,
		...
		zlog_err("f7E8YREIBMVDJRXe ""frrscript: path to script %s/%s.lua is too long",
			 scriptdir, fs->name);
		goto fail;
	}

	if (luaL_dofile(L, script_name) != 0) {
		zlog_err("fws0oBnPhL9VtAOM ""frrscript: failed loading script '%s': error: %s",
	...
	if (lua_isfunction(L, lua_gettop(L)) == 0) {
		zlog_err("tkZf9ine8ZuqzM66 ""frrscript: loaded script '%s' but %s not found",
