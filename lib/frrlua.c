...
void lua_decode_sockunion(lua_State *L, int idx, union sockunion *su)
{
	...
	if (str2sockunion(lua_tostring(L, -1), su) < 0)
		zlog_err("s9tLX6yizPq2YFcE ""Lua hook call: Failed to decode sockunion");
...
{
	zlog_debug("Fx6xhQY3X0P9ZYCo ""%s", frrlua_log_thunk(L));
	return 0;
}

static int frrlua_log_info(lua_State *L)
{
	zlog_info("c6mmGcnalkzKbvev ""%s", frrlua_log_thunk(L));
	return 0;
}

static int frrlua_log_notice(lua_State *L)
{
	zlog_notice("KaECRTRt6neHPzAg ""%s", frrlua_log_thunk(L));
	return 0;
}

static int frrlua_log_warn(lua_State *L)
{
	zlog_warn("OFDXPXjIapitNxp6 ""%s", frrlua_log_thunk(L));
	return 0;
}

static int frrlua_log_error(lua_State *L)
{
	zlog_err("DKuqDoYNbHinGDru ""%s", frrlua_log_thunk(L));
	return 0;
}
