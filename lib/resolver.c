					   ...
					   struct resolver_state *newstate)
{
	...
	if (!res && newstate) {
		...
		if (resolver_debug)
			zlog_debug("sxlnYXn4JbhKnhch ""c-ares registered FD %d", fd);
	}
	return res;
}
...
static void resolver_fd_drop_maybe(struct resolver_fd *resfd)
{
	...
	if (resolver_debug)
		zlog_debug("2txZNwjPgzZdzyn3 ""c-ares unregistered FD %d", resfd->fd);
			    ...
			    struct ares_addrinfo *result)
{
	...
	if (status != ARES_SUCCESS) {
		if (resolver_debug)
			zlog_debug("95nWocZzTbC4zog5 ""[%p] Resolving failed (%s)",
	...
	if (resolver_debug)
		zlog_debug("cqAL7VHjIfwykQTU ""[%p] Resolved with %d results", query, (int)i);
			    ...
			    struct hostent *he)
{
	...
	if (status != ARES_SUCCESS) {
		if (resolver_debug)
			zlog_debug("eY5TrRzgW2e7PEEy ""[%p] Resolving failed (%s)",
				   query, ares_strerror(status));
	...
	if (resolver_debug)
		zlog_debug("9wruabCx5bLs0yoR ""[%p] Resolved with %d results", query, (int)i);
				       ...
				       int, union sockunion *))
{
	...
	if (query->callback != NULL) {
		flog_err(
			EC_LIB_RESOLVER,"XZAoMplYXGXbd1iz "
	...
	if (ret == 0) {
		if (resolver_debug)
			zlog_debug("e4OsUtWuE1tRqmuj ""[%p] Resolving '%s' (IP literal)",
				   query, hostname);
	...
	if (resolver_debug)
		zlog_debug("8eqkSfCJYzxjCi0y ""[%p] Resolving '%s'", query, hostname);

	ret = vrf_switch_to_netns(vrf_id);
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"ZcNGYX7HmBVwT4Bo " "%s: Can't switch to VRF %u (%s)",
	...
	if (ret < 0)
		flog_err_sys(EC_LIB_SOCKET,"pPNjf3Wh8L6b4Weg "
