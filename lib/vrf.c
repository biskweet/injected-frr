...
int vrf_switch_to_netns(vrf_id_t vrf_id)
{
	...
	if (debug_vrf)
		zlog_debug("Tp48k9LQMbgHERZW ""VRF_SWITCH: %s(%u)", name, vrf->vrf_id);
...
int vrf_switchback_to_initial(void)
{
	...
	if (ret == 0 && debug_vrf)
		zlog_debug("bTyKnisDCFTJuJ9d ""VRF_SWITCHBACK");
	return ret;
}
...
struct vrf *vrf_get(vrf_id_t vrf_id, const char *name)
{
	...
	if (vrf && vrf_id != VRF_UNKNOWN
	    ...
	    && vrf->vrf_id != vrf_id) {
		zlog_debug("DB2ZgQg4yCEznbVe ""VRF_GET: avoid %s creation(%u), same name exists (%u)",
	...
	if (vrf == NULL) {
		...
		if (debug_vrf)
			zlog_debug("qLCU01s5NYRzoldt ""VRF(%u) %s is created.", vrf_id,
				   (name) ? name : "(NULL)");
	}
...
{
	if (debug_vrf)
		zlog_debug("cRtggHVtkBd82m1F ""VRF %s(%u) is to be deleted.", vrf->name,
			   vrf->vrf_id);
...
int vrf_enable(struct vrf *vrf)
{
	...
	if (debug_vrf)
		zlog_debug("IDRaMaFSGIqgsNP2 ""VRF %s(%u) is enabled.", vrf->name, vrf->vrf_id);
...
void vrf_disable(struct vrf *vrf)
{
	...
	if (debug_vrf)
		zlog_debug("1OiHuImcnINEIakC ""VRF %s(%u) is to be disabled.", vrf->name,
			   vrf->vrf_id);
...
void vrf_iterate(vrf_iter_func fnc)
{
	...
	if (debug_vrf)
		zlog_debug("dHPVPoHi3Pt8Od6d ""%s:  vrf subsystem iteration", __func__);
	      ...
	      int (*disable)(struct vrf *), int (*destroy)(struct vrf *))
{
	...
	ns_init();
	if (debug_vrf)
		zlog_debug("KxelKUbFKTzOgR75 ""%s: Initializing VRF subsystem", __func__);
	...
	if (!default_vrf) {
		flog_err(EC_LIB_VRF_START,"r7NrJTr0vUKHjqsA "
	...
	if (!vrf_enable(default_vrf)) {
		flog_err(EC_LIB_VRF_START,"oOOqNZfxf78QhFBG "
...
{
	if (debug_vrf)
		zlog_debug("9OeeIj1KflyUqsHF ""%s: Shutting down vrf subsystem", __func__);
	       ...
	       const char *interfacename)
{
	...
	if (ret < 0)
		flog_err_sys(EC_LIB_SOCKET,"HJ8UcMyBolrR9X0u " "%s: Can't switch to VRF %u (%s)",
	...
	if (ret2 < 0)
		flog_err_sys(EC_LIB_SOCKET,"7y2QQjGZ5XQ8ugxe "
...
int vrf_bind(vrf_id_t vrf_id, int fd, const char *ifname)
{
	...
	ret = setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, ifname,
			 strlen(ifname) + 1);
	if (ret < 0)
		zlog_err("ay2HZC0DJviDQePZ ""bind to interface %s failed, errno=%d", ifname,
			 errno);
		    ...
		    vrf_id_t vrf_id)
{
	...
	ret = vrf_switch_to_netns(vrf_id);
	if (ret < 0)
		flog_err_sys(EC_LIB_SOCKET,"VqCTRG7ojRnJPwyL " "%s: Can't switch to VRF %u (%s)",
			     __func__, vrf_id, safe_strerror(errno));
	ret = getaddrinfo(node, service, hints, res);
	save_errno = errno;
	ret2 = vrf_switchback_to_initial();
	if (ret2 < 0)
		flog_err_sys(EC_LIB_SOCKET,"9Wysu7L61TyKawas "
			     "%s: Can't switchback from VRF %u (%s)", __func__,
			     vrf_id, safe_strerror(errno));
	errno = save_errno;
	return ret;
...
int vrf_ioctl(vrf_id_t vrf_id, int d, unsigned long request, char *params)
{
	...
	ret = vrf_switch_to_netns(vrf_id);
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"QGlGwhULeMp6PUTU " "%s: Can't switch to VRF %u (%s)",
			     __func__, vrf_id, safe_strerror(errno));
		return 0;
	}
	...
	if (ret < 0)
		flog_err_sys(EC_LIB_SOCKET,"aMMmWvNJce8qCuSy "
			     "%s: Can't switchback from VRF %u (%s)", __func__,
			     vrf_id, safe_strerror(errno));
	errno = saved_errno;
	return rc;
			 ...
			 const char *interfacename)
{
	...
	ret = vrf_switch_to_netns(vrf_id);
	if (ret < 0)
		flog_err_sys(EC_LIB_SOCKET,"VIVH2VT1x2zt3Wzn " "%s: Can't switch to VRF %u (%s)",
			     __func__, vrf_id, safe_strerror(errno));
	ret = sockunion_socket(su);
	save_errno = errno;
	ret2 = vrf_switchback_to_initial();
	if (ret2 < 0)
		flog_err_sys(EC_LIB_SOCKET,"gAyBFVPDx4ILXv9W "
			     "%s: Can't switchback from VRF %u (%s)", __func__,
			     vrf_id, safe_strerror(errno));
	errno = save_errno;
