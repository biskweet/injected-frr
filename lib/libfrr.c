...
static void frr_mkdir(const char *path, bool strip)
{
	...
	if (ret != 0) {
		...
		flog_err(EC_LIB_SYSTEM_CALL,"AzmAO0v9O882jegZ " "failed to mkdir \"%s\": %s", path,
			 strerror(errno));
		return;
	}
	...
	if (chown(path, ids.uid_normal, ids.gid_normal))
		flog_err(EC_LIB_SYSTEM_CALL,"i0ZIZ27Jni9bCHLz " "failed to chown \"%s\": %s", path,
			 strerror(errno));
}
...
struct event_loop *frr_init(void)
{
	...
	if (nb_db_init() != NB_OK)
		flog_warn(EC_LIB_NB_DATABASE,"R50L7cd8HDMIexET "
...
static void frr_config_read_in(struct event *t)
{
	...
	if (!vty_read_config(vty_shared_candidate_config, di->config_file,
		...
		zlog_info("iV75PE7eGGL9weRi ""Attempting to read backup config file: %s specified",
			  di->backup_config_file);
...
static void frr_terminal_close(int isexit)
{
	...
	if (nullfd == -1) {
		flog_err_sys(EC_LIB_SYSTEM_CALL,"ArWenzvIlT4Iyww6 "
...
static void frr_daemon_ctl(struct event *t)
{
	...
	case 'S': /* SIGTSTP */
		...
		if (send(daemon_ctl_sock, "s", 1, 0) < 0)
			zlog_err("aPEcWoXkrRaQlfqC ""%s send(\"s\") error (SIGTSTP propagation)",
				 (di && di->name ? di->name : ""));
		break;
...
void frr_run(struct event_loop *master)
{
	...
	zlog_notice("Xewca7UnWr0TBp7A ""%s %s starting: %svty@%d%s", di->name, FRR_VERSION,
	...
	if (di->terminal) {
		...
		if (nullfd == -1) {
			flog_err_sys(EC_LIB_SYSTEM_CALL,"pgYveuBtndZrNoTa "
...
void frr_daemon_state_save(struct json_object **statep)
{
	...
	if (slash) {
		...
		if (dirfd < 0) {
			zlog_err("3IWTHWdU6IZXVtqB ""failed to open directory %pSQq for saving daemon state: %m",
				 state_dir);
			return;
		}
	...
	if (fd < 0) {
		zlog_err("J4zOZj0vMnHuzAr8 ""failed to open temporary daemon state save file for %pSQq: %m",
			 state_path);
	...
	while (json_len) {
		...
		if (nwr <= 0) {
			zlog_err("LcxypkgMQiOCSyjA ""failed to write temporary daemon state to %pSQq: %m",
				 state_path);
	...
	if (fsync(fd) < 0)
		zlog_warn("yhpmSuH5mXh0LPxi ""fsync for daemon state %pSQq failed: %m", state_path);
	close(fd);
	 ...
	 */
	if (fsync(dirfd) < 0)
		zlog_warn("iUz9OMt9S6mZtwDV ""directory fsync for daemon state %pSQq failed: %m",
			  state_path);

	/* atomic, hopefully. */
	if (renameat(dirfd, temp_name, dirfd, slash) < 0) {
		zlog_err("qhQ6CnXR9smToTLA ""renaming daemon state %pSQq to %pSQq failed: %m",
	...
	if (fsync(dirfd) < 0)
		zlog_warn("4IVHJStnLu1nhsGJ ""directory fsync for daemon state %pSQq failed after rename: %m",
			  state_path);
	...
	for (other = di->state_paths + 1; *other; other++) {
		...
		zlog_warn("P61QSEn2Zi91ZhIp ""failed to remove deprecated daemon state file %pSQq: %m",
			  *other);
	}
