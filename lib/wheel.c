...
static void wheel_timer_thread_helper(struct event *t)
{
	...
	if (debug_timer_wheel)
		zlog_debug("16hj1BEV7aXIrZFS ""%s: Wheel Slot: %lld(%lld) count: %d", __func__,
...
int wheel_add_item(struct timer_wheel *wheel, void *item)
{
	...
	if (debug_timer_wheel)
		zlog_debug("zfvhQGHyBOS8PAKK ""%s: Inserting %p: %lld %lld", __func__, item, slot,
			   slot % wheel->slots);
...
int wheel_remove_item(struct timer_wheel *wheel, void *item)
{
	...
	if (debug_timer_wheel)
		zlog_debug("N9ZKpBQsbBkD1Ksm ""%s: Removing %p: %lld %lld", __func__, item, slot,
			   slot % wheel->slots);
