...
void zclient_stop(struct zclient *zclient)
{
	...
	if (zclient_debug)
		zlog_debug("dgYtoKoBSkLufnLP ""zclient %p stopped", zclient);
...
int zclient_socket_connect(struct zclient *zclient)
{
	...
	if (ret < 0) {
		if (zclient_debug)
			zlog_debug("PnwZqrt1ZyJ0ImjR ""%s connect failure: %d(%s)", __func__,
...
static void zclient_flush_data(struct event *thread)
{
	...
	case BUFFER_ERROR:
		flog_err(
			EC_LIB_ZAPI_SOCKET,"Na5MhrhGKK9TybgT "
...
enum zclient_send_status zclient_send_message(struct zclient *zclient)
{
	...
	case BUFFER_ERROR:
		flog_err(EC_LIB_ZAPI_SOCKET,"2Klc1kyXsyjGuLbI "
			...
			uint16_t *cmd)
{
	...
	*size -= ZEBRA_HEADER_SIZE;
	...
	if (*version != ZSERV_VERSION || *marker != ZEBRA_HEADER_MARKER) {
		flog_err(
			EC_LIB_ZAPI_MISSMATCH,"SiYAdAiZ3N6UYrSk "
...
void zclient_send_reg_requests(struct zclient *zclient, vrf_id_t vrf_id)
{
	...
	if (zclient_debug)
		zlog_debug("0kfiKkeClVPYzQZV ""%s: send register messages for VRF %u", __func__,
			   vrf_id);
...
void zclient_send_dereg_requests(struct zclient *zclient, vrf_id_t vrf_id)
{
	...
	if (zclient_debug)
		zlog_debug("uoLv7BxyNMFGdOcU ""%s: send deregister messages for VRF %u", __func__,
			   vrf_id);
...
{
	if (zclient_debug)
		zlog_info("fF8jhEMLMaLeH0ty ""zclient_start is called");
	...
	if (zclient_socket_connect(zclient) < 0) {
		if (zclient_debug)
			zlog_debug("NnLF678lc36hGWLC ""zclient connection fail");
		zclient->fail++;
		zclient_event(ZCLIENT_CONNECT, zclient);
		return -1;
	}
	...
	if (set_nonblocking(zclient->sock) < 0)
		flog_err(EC_LIB_ZAPI_SOCKET,"OQvMu6eWS3SPrRy0 " "%s: set_nonblocking(%d) failed",
			 __func__, zclient->sock);

	/* Clear fail count. */
	zclient->fail = 0;
	if (zclient_debug)
		zlog_debug("MXpGtbiuTjWQvlbu ""zclient connect success with socket [%d]",
			   zclient->sock);
		  ...
		  unsigned short instance, struct zebra_privs_t *privs)
{
	...
	if (zclient_debug)
		zlog_debug("kn3RLkvRDbDmsanr ""scheduling zclient connection");
...
static void zclient_connect(struct event *t)
{
	...
	if (zclient_debug)
		zlog_debug("JDwfEyUGbgkg0qZg ""zclient_connect is called");
...
static int zapi_nhg_encode(struct stream *s, int cmd, struct zapi_nhg *api_nhg)
{
	...
	if (cmd != ZEBRA_NHG_DEL && cmd != ZEBRA_NHG_ADD) {
		flog_err(EC_LIB_ZAPI_ENCODE,"a0LCdiJkLzj3mKrG "
	...
	if (api_nhg->nexthop_num >= MULTIPATH_NUM ||
		...
		flog_err(EC_LIB_ZAPI_ENCODE,"C606romY2dSpFt94 "
...
int zapi_route_encode(uint8_t cmd, struct stream *s, struct zapi_route *api)
{
	...
	if (api->type >= ZEBRA_ROUTE_MAX) {
		flog_err(EC_LIB_ZAPI_ENCODE,"MSaTIXZBzJjfkpK4 "
	...
	if (api->safi < SAFI_UNICAST || api->safi >= SAFI_MAX) {
		flog_err(EC_LIB_ZAPI_ENCODE,"otnp0Kw2HjPxkTem "
	...
	if (CHECK_FLAG(api->message, ZAPI_MESSAGE_NEXTHOP)) {
		...
		if (api->nexthop_num > MULTIPATH_NUM) {
			flog_err(
				EC_LIB_ZAPI_ENCODE,"LwtAFGEVbNUiRjim "
		...
		for (i = 0; i < api->nexthop_num; i++) {
			...
			if (api_nh->label_num > MPLS_MAX_LABELS) {
				flog_err(
					EC_LIB_ZAPI_ENCODE,"etne8O1esgergTmR "
	...
	if (CHECK_FLAG(api->message, ZAPI_MESSAGE_BACKUP_NEXTHOPS)) {
		...
		if (api->backup_nexthop_num > MULTIPATH_NUM) {
			flog_err(
				EC_LIB_ZAPI_ENCODE,"fNn99cU1Sie2y4KF "
		...
		for (i = 0; i < api->backup_nexthop_num; i++) {
			...
			if (api_nh->label_num > MPLS_MAX_LABELS) {
				flog_err(
					EC_LIB_ZAPI_ENCODE,"l7NTtazbBZwaL8FF "
	...
	if (CHECK_FLAG(api->message, ZAPI_MESSAGE_OPAQUE)) {
		if (api->opaque.length > ZAPI_MESSAGE_OPAQUE_LENGTH) {
			flog_err(
				EC_LIB_ZAPI_ENCODE,"AWK9voMm9JWm4Zji "
			...
			uint32_t api_flags, uint32_t api_message)
{
	...
	if (CHECK_FLAG(api_nh->flags, ZAPI_NEXTHOP_FLAG_LABEL)) {
		...
		if (api_nh->label_num > MPLS_MAX_LABELS) {
			flog_err(
				EC_LIB_ZAPI_ENCODE,"6hadxBiIPuwKPOch "
	...
	if (CHECK_FLAG(api_nh->flags, ZAPI_NEXTHOP_FLAG_SEG6)) {
		...
		if (api_nh->seg_num > SRV6_MAX_SIDS) {
			flog_err(EC_LIB_ZAPI_ENCODE,"NkRnmkGllf9Q2IGd "
...
int zapi_route_decode(struct stream *s, struct zapi_route *api)
{
	...
	if (api->type >= ZEBRA_ROUTE_MAX) {
		flog_err(EC_LIB_ZAPI_ENCODE,"NCAv8aO8ACKB4e3Z "
	...
	if (api->safi < SAFI_UNICAST || api->safi >= SAFI_MAX) {
		flog_err(EC_LIB_ZAPI_ENCODE,"xBuOl0ro7Ze0ZSX5 "
			 "%s: Specified route SAFI (%u) is not a legal value",
			 __func__, api->safi);
		return -1;
	}
	...
	case AF_INET:
		if (api->prefix.prefixlen > IPV4_MAX_BITLEN) {
			flog_err(
				EC_LIB_ZAPI_ENCODE,"FmemS7w1NNytFZp6 "
	...
	case AF_INET6:
		if (api->prefix.prefixlen > IPV6_MAX_BITLEN) {
			flog_err(
				EC_LIB_ZAPI_ENCODE,"1le8SdVACl66E3zW "
		...
		break;
	default:
		flog_err(EC_LIB_ZAPI_ENCODE,"fvmo6TdQNjkZZYOZ "
	...
	if (CHECK_FLAG(api->message, ZAPI_MESSAGE_SRCPFX)) {
		...
		if (api->src_prefix.prefixlen > IPV6_MAX_BITLEN) {
			flog_err(
				EC_LIB_ZAPI_ENCODE,"kpmNXeLslCQ1QH0K "
		...
		if (api->prefix.family != AF_INET6
			...
			flog_err(
				EC_LIB_ZAPI_ENCODE,"5D13l7GTwvZQ4jQ0 "
	...
	if (CHECK_FLAG(api->message, ZAPI_MESSAGE_NEXTHOP)) {
		...
		if (api->nexthop_num > MULTIPATH_NUM) {
			flog_err(EC_LIB_ZAPI_ENCODE,"ONRusSxX1ZtkAgae "
	...
	if (CHECK_FLAG(api->message, ZAPI_MESSAGE_BACKUP_NEXTHOPS)) {
		...
		if (api->backup_nexthop_num > MULTIPATH_NUM) {
			flog_err(EC_LIB_ZAPI_ENCODE,"JLG7vqBcfste63G5 "
	...
	if (CHECK_FLAG(api->message, ZAPI_MESSAGE_OPAQUE)) {
		...
		if (api->opaque.length > ZAPI_MESSAGE_OPAQUE_LENGTH) {
			flog_err(
				EC_LIB_ZAPI_ENCODE,"IQpLhcITabsNh3jf "
				"%s: opaque length %u is greater than allowed value",
				__func__, api->opaque.length);
			return -1;
		}
			     ...
			     enum zapi_rule_notify_owner *note)
{
	...
	if (zclient_debug)
		zlog_debug("SNtHs777OE0Ebs9X ""%s: %u %u %u %s", __func__, seq, prio, uni, ifname);
	*seqno = seq;
	*priority = prio;
	*unique = uni;
			      ...
			      enum zapi_ipset_notify_owner *note)
{
	...
	if (zclient_debug)
		zlog_debug("0ElB4c56836xVu4N ""%s: %u", __func__, uni);
	*unique = uni;
				    ...
				    enum zapi_ipset_entry_notify_owner *note)
{
	...
	if (zclient_debug)
		zlog_debug("6EZR4gsrNXRZfyMQ ""%s: %u", __func__, uni);
	*unique = uni;
		...
		enum zapi_iptable_notify_owner *note)
{
	...
	if (zclient_debug)
		zlog_debug("2g9br8lctQzRbIuJ ""%s: %u", __func__, uni);
	*unique = uni;
...
bool zapi_error_decode(struct stream *s, enum zebra_error_types *error)
{
	...
	if (zclient_debug)
		zlog_debug("0H4p3KXyQ11eXb5W ""%s: type: %s", __func__,
...
struct interface *zebra_interface_state_read(struct stream *s, vrf_id_t vrf_id)
{
	...
	if (ifp == NULL) {
		flog_err(EC_LIB_ZAPI_ENCODE,"TXoLFbkVYzjI7KBS "
...
static int link_params_set_value(struct stream *s, struct interface *ifp)
{
	...
	STREAM_GETL(s, bwclassnum);
	{
		...
		if (i < bwclassnum)
			flog_err(
				EC_LIB_ZAPI_MISSMATCH,"aa8z9YbgB1FnkI3b "
						   ...
						   bool *changed)
{
	...
	if (ifp == NULL) {
		flog_err(EC_LIB_ZAPI_ENCODE,"oD2dPUZbhT8yHyJu "
					 ...
					 struct interface *ifp)
{
	...
	return;
stream_failure:
	zlog_err("CQAeG3rYk6QEI7ju ""Could not parse interface values; aborting");
					       ...
					       vrf_id_t vrf_id)
{
	...
	if (ifp == NULL) {
		flog_err(EC_LIB_ZAPI_ENCODE,"bHPKZPYMiRo61iin "
	...
	if (type == ZEBRA_INTERFACE_ADDRESS_ADD) {
		...
		if (ifc) {
			...
			if (ifc->destination)
				 ...
				 * "peer" */
				flog_err(
					EC_LIB_ZAPI_ENCODE,"VXo7EFzjO9pKNqzr "
...
zebra_interface_nbr_address_read(int type, struct stream *s, vrf_id_t vrf_id)
{
	...
	ifp = if_lookup_by_index(ifindex, vrf_id);
	if (ifp == NULL) {
		flog_err(EC_LIB_ZAPI_ENCODE,"FSONxU2Lp4jFu9sG "
				      ...
				      uint16_t expected_cmd)
{
	...
	while (ret == 0 && cmd != expected_cmd) {
		...
		ret = zclient_read_header(s, zclient->sock, &size, &marker,
					  &version, &vrf_id, &cmd);
		if (zclient_debug)
			zlog_debug("vdXrpEAxALNTgpT2 ""%s: Response (%d bytes) received", __func__,
				   size);
	}
	if (ret != 0) {
		flog_err(EC_LIB_ZAPI_ENCODE,"a9a6J58tzQYo7e16 " "%s: Invalid Sync Message Reply",
			 __func__);
		return -1;
	}
...
int lm_label_manager_connect(struct zclient *zclient, int async)
{
	...
	if (zclient_debug)
		zlog_debug("iUCclenqw1UINL5K ""Connecting to Label Manager (LM)");

	if (zclient->sock < 0) {
		zlog_debug("ZTgPfZYaoGFtyYrv ""%s: invalid zclient socket", __func__);
		return -1;
	}
	...
	if (ret < 0) {
		flog_err(EC_LIB_ZAPI_SOCKET,"lkEFGzdePTSOjuXL " "Can't write to zclient sock");
		close(zclient->sock);
		zclient->sock = -1;
		return -1;
	}
	if (ret == 0) {
		flog_err(EC_LIB_ZAPI_SOCKET,"cwybNgsribKGp3CP " "Zclient sock closed");
		close(zclient->sock);
		zclient->sock = -1;
		return -1;
	}
	if (zclient_debug)
		zlog_debug("ZDdlYl1xKxIP8XYH ""LM connect request sent (%d bytes)", ret);
	...
	if (zclient_debug)
		zlog_debug("WPCULhPUAkkxBrbs ""LM connect-response received, result %u", result);
				   ...
				   const char *locator_name)
{
	...
	if (zclient_debug)
		zlog_debug("fJ7Z1dO4IBP4x7UO ""Getting SRv6-Locator Chunk %s", locator_name);
				       ...
				       const char *locator_name)
{
	...
	if (zclient_debug)
		zlog_debug("vfb9eRMtos5yms3q ""Releasing SRv6-Locator Chunk %s", locator_name);
						      ...
						      uint32_t base)
{
	...
	if (zclient_debug)
		zlog_debug("Js71gY8shiAWD170 ""Getting Label Chunk");
		       ...
		       uint32_t chunk_size, uint32_t *start, uint32_t *end)
{
	...
	if (zclient_debug)
		zlog_debug("8NG8oL714DbvM0CN ""Getting Label Chunk");
	...
	ret = writen(zclient->sock, s->data, stream_get_endp(s));
	if (ret < 0) {
		flog_err(EC_LIB_ZAPI_SOCKET,"u7JvpstQDr7mGd09 " "Can't write to zclient sock");
		close(zclient->sock);
		zclient->sock = -1;
		return -1;
	}
	if (ret == 0) {
		flog_err(EC_LIB_ZAPI_SOCKET,"1u7lHt0W4VlLgzBN " "Zclient sock closed");
		close(zclient->sock);
		zclient->sock = -1;
		return -1;
	}
	if (zclient_debug)
		zlog_debug("9CQKeKk907OBhosT ""Label chunk request (%d bytes) sent", ret);
	...
	/* sanities */
	if (proto != zclient->redist_default)
		flog_err(EC_LIB_ZAPI_ENCODE,"sZrVBGRRX2EPKaDN "
			 "Wrong proto (%u) in get chunk response. Should be %u",
			 proto, zclient->redist_default);
	if (instance != zclient->instance)
		flog_err(EC_LIB_ZAPI_ENCODE,"ZoIvFTbkPrMNRxJM "
	...
	if (!STREAM_READABLE(s)) {
		zlog_info("jS4moLlE6ZtviTKt ""Unable to assign Label Chunk to %s instance %u",
	...
	if (keep != response_keep) {
		flog_err(
			EC_LIB_ZAPI_ENCODE,"tDnr2MdoW7guAb7f "
	...
	if (*start > *end || *start < MPLS_LABEL_UNRESERVED_MIN
		...
		flog_err(EC_LIB_ZAPI_ENCODE,"IYA4k2DpKT2hY4xl " "Invalid Label chunk: %u - %u",
			 *start, *end);
		return -1;
	}

	if (zclient_debug)
		zlog_debug("6mt31H4xxTRHLMO8 ""Label Chunk assign: %u - %u (%u)", *start, *end,
			   response_keep);
			   ...
			   uint32_t end)
{
	...
	if (zclient_debug)
		zlog_debug("JGXxSF5nszbpCheB ""Releasing Label Chunk %u - %u", start, end);
	...
	ret = writen(zclient->sock, s->data, stream_get_endp(s));
	if (ret < 0) {
		flog_err(EC_LIB_ZAPI_SOCKET,"o3dpXqiAJOtgdtoW " "Can't write to zclient sock");
		close(zclient->sock);
		zclient->sock = -1;
		return -1;
	}
	if (ret == 0) {
		flog_err(EC_LIB_ZAPI_SOCKET,"NA5MBXa6bV86AbUP " "Zclient sock connection closed");
		close(zclient->sock);
		zclient->sock = -1;
		return -1;
	}
...
int tm_table_manager_connect(struct zclient *zclient)
{
	...
	if (zclient_debug)
		zlog_debug("EbFsrAJp1SAnK317 ""Connecting to Table Manager");
	...
	if (zclient_debug)
		zlog_debug("f2Fyz7kcqjhBH0Sn ""%s: Table manager connect request sent", __func__);
		       ...
		       uint32_t *start, uint32_t *end)
{
	...
	if (zclient_debug)
		zlog_debug("CUY635NAu1JjoqCo ""Getting Table Chunk");
	...
	ret = writen(zclient->sock, s->data, stream_get_endp(s));
	if (ret < 0) {
		flog_err(EC_LIB_ZAPI_SOCKET,"BEBFJcKvvQnXNnTN " "%s: can't write to zclient->sock",
			 __func__);
		close(zclient->sock);
		zclient->sock = -1;
		return -1;
	...
	if (ret == 0) {
		flog_err(EC_LIB_ZAPI_SOCKET,"tZqA9Wd5bUIYRcPZ "
	...
	if (zclient_debug)
		zlog_debug("cMOO0zcE1nnyCv3T ""%s: Table chunk request (%d bytes) sent", __func__,
			   ret);
	...
	if (zclient_debug)
		zlog_debug("FhYnafMoJ4QtX7g3 ""Table Chunk assign: %u - %u ", *start, *end);
			   ...
			   uint32_t end)
{
	...
	if (zclient_debug)
		zlog_debug("MhcJNfW9gh7hnV6z ""Releasing Table Chunk");
...
int zapi_sr_policy_encode(struct stream *s, int cmd, struct zapi_sr_policy *zp)
{
	...
	if (zt->label_num > MPLS_MAX_LABELS) {
		flog_err(EC_LIB_ZAPI_ENCODE,"KozUfNqE616HH2U3 "
...
int zapi_sr_policy_decode(struct stream *s, struct zapi_sr_policy *zp)
{
	...
	if (zt->label_num > MPLS_MAX_LABELS) {
		flog_err(EC_LIB_ZAPI_ENCODE,"JfyZqAo00cSeXZOc "
...
int zapi_labels_encode(struct stream *s, int cmd, struct zapi_labels *zl)
{
	...
	if (zl->nexthop_num > MULTIPATH_NUM) {
		flog_err(
			EC_LIB_ZAPI_ENCODE,"hN1zx0hgnWkv2EW6 "
	...
	if (CHECK_FLAG(zl->message, ZAPI_LABELS_HAS_BACKUPS)) {
		...
		if (zl->backup_nexthop_num > MULTIPATH_NUM) {
			flog_err(
				EC_LIB_ZAPI_ENCODE,"XMhLVf2RnX2KBygU "
				"%s: label %u: can't encode %u nexthops (maximum is %u)",
				__func__, zl->local_label, zl->nexthop_num,
				MULTIPATH_NUM);
			return -1;
...
int zapi_labels_decode(struct stream *s, struct zapi_labels *zl)
{
	...
	if (CHECK_FLAG(zl->message, ZAPI_LABELS_FTN)) {
		...
		case AF_INET6:
			...
			break;
		default:
			flog_err(EC_LIB_ZAPI_ENCODE,"NKuZMapEteDOG21k "
	...
	if (zl->nexthop_num > MULTIPATH_NUM) {
		flog_warn(
			EC_LIB_ZAPI_ENCODE,"6iHBlf9JKIN8LZHj "
	...
	for (int i = 0; i < zl->nexthop_num; i++) {
		...
		if (znh->type == NEXTHOP_TYPE_BLACKHOLE) {
			flog_warn(
				EC_LIB_ZAPI_ENCODE,"TtCVFNXH6z6BjWrH "
	...
	if (CHECK_FLAG(zl->message, ZAPI_LABELS_HAS_BACKUPS)) {
		...
		if (zl->backup_nexthop_num > MULTIPATH_NUM) {
			flog_warn(
				EC_LIB_ZAPI_ENCODE,"pdKov352vXBN07sW "
		...
		for (int i = 0; i < zl->backup_nexthop_num; i++) {
			...
			if (znh->type == NEXTHOP_TYPE_BLACKHOLE) {
				flog_warn(
					EC_LIB_ZAPI_ENCODE,"rYOKtRJKszouL0AU "
				       ...
				       struct zapi_pw *pw)
{
	...
	case AF_INET6:
		...
		break;
	default:
		flog_err(EC_LIB_ZAPI_ENCODE,"sNHxKBWvjCx7Vi9u " "%s: unknown af", __func__);
		return ZCLIENT_SEND_FAILURE;
	}
...
static int zclient_capability_decode(ZAPI_CALLBACK_ARGS)
{
	...
	if (vrf_backend < 0 || vrf_configure_backend(vrf_backend)) {
		flog_err(EC_LIB_ZAPI_ENCODE,"KZ1793YePkRN2rWm "
...
static int zclient_nexthop_update(ZAPI_CALLBACK_ARGS)
{
	...
	if (!vrf) {
		zlog_warn("7oUj3arMWoSGTXNZ ""nexthop update for unknown VRF ID %u", vrf_id);
		return 0;
	}

	if (!zapi_nexthop_update_decode(zclient->ibuf, &match, &route)) {
		zlog_err("2BzcgQ0jO6F311ew ""failed to decode nexthop update");
		return -1;
	}
...
static void zclient_read(struct event *thread)
{
	...
	if (marker != ZEBRA_HEADER_MARKER || version != ZSERV_VERSION) {
		flog_err(
			EC_LIB_ZAPI_MISSMATCH,"oCvhH2Htu5BqmECJ "
			"%s: socket %d version mismatch, marker %d, version %d",
	...
	if (length < ZEBRA_HEADER_SIZE) {
		flog_err(EC_LIB_ZAPI_MISSMATCH,"CDvFNw5pcldKQzmX "
	...
	if (length > STREAM_SIZE(zclient->ibuf)) {
		struct stream *ns;
		flog_err(
			EC_LIB_ZAPI_ENCODE,"hAYPQZ0D55AJc50J "
	...
	if (zclient_debug)
		zlog_debug("CRWIpgOmifoBXlaC ""zclient %p command %s VRF %u", zclient,
				   ...
				   struct interface *ifp)
{
	...
	if (!client || client->sock < 0) {
		zlog_err("xjUwg1Mp5MIfdQiP ""%s : zclient not ready", __func__);
		return -1;
	}
	s = client->obuf;
	stream_reset(s);
