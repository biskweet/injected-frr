						 ...
						 uint index)
{
	...
	if (!nn->cbs.lookup_entry) {
		flog_warn(EC_LIB_NB_OPERATIONAL_DATA,"FW3vBZeEqM9gLwAr "
	...
	if (ni->list_entry == NULL) {
		flog_warn(EC_LIB_NB_OPERATIONAL_DATA,"tV8Qa95TGAv1E2rC "
...
static enum nb_error nb_op_ys_init_node_infos(struct nb_op_yield_state *ys)
{
	...
	if (ret || !node) {
		flog_warn(EC_LIB_LIBYANG,"kJZ3W1ko6kio5slU "
...
static enum nb_error __walk(struct nb_op_yield_state *ys, bool is_resume)
{
	...
	if (ys->schema_path[0]->nodetype == LYS_CHOICE) {
		flog_err(EC_LIB_NB_OPERATIONAL_DATA,"2boFK5F7cJ9tQItr "
	...
	while (true) {
		...
		case LYS_LIST:
			...
			if (list_start &&
				...
				if (!err)
					...
					flog_err(EC_LIB_NB_OPERATIONAL_DATA,"LnHSME46ABAjuxjJ "
			...
			if (list_start) {
				...
				if (ni->query_specific_entry) {
					flog_warn(EC_LIB_NB_OPERATIONAL_DATA,"GEkYNLiJMsfQP1Af "
						  "%s: unexpected state",
						  __func__);
				}
		...
		case LYS_ANYDATA:
			...
			flog_warn(EC_LIB_NB_OPERATIONAL_DATA,"6yh17DJWZnKG1Sme "
					       ...
					       struct nb_node **last)
{
	...
	if (!nblast) {
		flog_warn(EC_LIB_YANG_UNKNOWN_DATA_PATH,"YAdUOvxAiqA8GqeD "
