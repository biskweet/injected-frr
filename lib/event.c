...
struct event_loop *event_master_create(const char *name)
{
	...
	if (rv->fd_limit > STUPIDLY_LARGE_FD_SIZE) {
		zlog_warn("xXDdoNHi55CzOspK ""FD Limit set: %u is stupidly large.  Is this what you intended?  Consider using --limit-fds also limiting size to %u",
...
#define ONEYEAR2SEC (60 * 60 * 24 * 365)
	if (time_relative->tv_sec > ONEYEAR2SEC)
		flog_err(
			EC_LIB_TIMER_TOO_LONG,"zn5PfQtgaSDOLsAH "
			    ...
			    int idx_hint)
{
	...
	if (!found) {
		zlog_debug("IZGEjJvUGG35vRWW "
			"[!] Received cancellation request for nonexistent rw job");
		zlog_debug("y5c1PzLylBPGM2Do ""[!] threadmaster: %s | fd: %d",
				    ...
				    short state, short actual_state, int pos)
{
	...
	if (!thread) {
		if ((actual_state & (POLLHUP|POLLIN)) != POLLHUP)
			flog_err(EC_LIB_NO_THREAD,"3x6LCSBceg6XsKac "
	...
	while ((thread = event_timer_list_first(&m->timer))) {
		...
		if (timercmp(timenow, &prev, >)) {
			...
			if (!displayed && !thread->ignore_timer_late) {
				flog_warn(
					EC_LIB_STARVE_THREAD,"idRJtA9o0KqsaVJg "
...
struct event *event_fetch(struct event_loop *m, struct event *fetch)
{
	...
	do {
		...
		if (num < 0) {
			...
			/* else die */
			flog_err(EC_LIB_SYSTEM_CALL,"RdqSq65nUrR8Fy6h " "poll() error: %s",
				 safe_strerror(errno));
			pthread_mutex_unlock(&m->mtx);
			fetch = NULL;
			break;
...
void event_call(struct event *thread)
{
	...
	if (cputime_enabled_here && cputime_enabled && cputime_threshold
		...
		flog_warn(
			EC_LIB_SLOW_THREAD_CPU,"xrN22yc08cQZSSGM "
		...
		atomic_fetch_add_explicit(&thread->hist->total_wall_warn,
					  1, memory_order_seq_cst);
		flog_warn(
			EC_LIB_SLOW_THREAD_WALL,"RIb4MXMT1oxrizM1 "
...
#  define LAST_SIGNAL 32
	...
	zlog_debug("qCkFINf5I3fLxeaj ""%s: %s", __func__, buf);
}
