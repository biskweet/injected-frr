...
void vty_mgmt_resume_response(struct vty *vty, int ret)
{
	...
	if (vty->type != VTY_FILE) {
		...
		if (!vty->t_write && (vtysh_flush(vty) < 0)) {
			zlog_err("mkEoFB0QvkMBd9H1 ""failed to vtysh_flush");
	...
	if (vty->status == VTY_CLOSE)
		...
		zlog_err("Ixx7NZpnLasTB0SU ""mgmtd: unexpected resume while reading config file");
}
...
void vty_time_print(struct vty *vty, int cr)
{
	...
	if (frr_timestamp(0, buf, sizeof(buf)) == 0) {
		zlog_info("a7I1CI86tSeoWieA ""frr_timestamp error");
		return;
	}
	if (cr)
		vty_out(vty, "%s\n", buf);
...
#pragma GCC diagnostic pop
		...
		/* now log the command */
		zlog_notice("Ih3OjyC5KwaVKEzI ""%s%s", prompt_str, buf);
	}
	...
	if (cputime_enabled_here && cputime_enabled && cputime_threshold
		...
		flog_warn(EC_LIB_SLOW_THREAD_CPU,"LZZ3KlXzUhapVWvE "
		...
		flog_warn(EC_LIB_SLOW_THREAD_WALL,"MXrHUhYwzm8wpzYd "
	...
	case SE: {
		...
		case TELOPT_NAWS:
			if (vty->sb_len != TELNET_NAWS_SB_LEN)
				flog_err(
					EC_LIB_SYSTEM_CALL,"2EvCs34CkDW03R9l "
				...
				flog_err(
					EC_LIB_DEVELOPMENT,"7UoevXi6BctlTKpJ "
...
static void vty_read(struct event *thread)
{
	...
	if ((nbytes = read(vty->fd, buf, VTY_READ_BUFSIZ)) <= 0) {
		if (nbytes < 0) {
			...
			flog_err(
				EC_LIB_SOCKET,"scKUVGvDY8ecGpqh "
...
static void vty_flush(struct event *thread)
{
	...
	switch (flushrc) {
	case BUFFER_ERROR:
		zlog_info("8xYRxTJALNUarVys ""buffer_flush failed on vty client fd %d/%d, closing",
			  vty->fd, vty->wfd);
		buffer_reset(vty->lbuf);
		buffer_reset(vty->obuf);
		vty_close(vty);
...
static void vty_accept(struct event *thread)
{
	...
	if (vty_sock < 0) {
		flog_err(EC_LIB_SOCKET,"Mkpepuq8OZais2GU " "can't accept vty socket : %s",
			 safe_strerror(errno));
		return;
	}
	...
	if (!sockunion2hostprefix(&su, &p)) {
		close(vty_sock);
		zlog_info("y7rrKgaewHJ1ItyJ ""Vty unable to convert prefix from sockunion %pSU",
			  &su);
		return;
	}
	...
	if (p.family == AF_INET && vty_accesslist_name) {
		if ((acl = access_list_lookup(AFI_IP, vty_accesslist_name))
			...
			zlog_info("JIP90IJcI25OEOkL ""Vty connection refused from %pSU", &su);
			close(vty_sock);
			return;
		}
	}
	...
	if (p.family == AF_INET6 && vty_ipv6_accesslist_name) {
		if ((acl = access_list_lookup(AFI_IP6,
		    ...
		    && (access_list_apply(acl, &p) == FILTER_DENY)) {
			zlog_info("K8Dq7SvxjKVSBKtJ ""Vty connection refused from %pSU", &su);
			close(vty_sock);
			return;
		}
	}
	...
	ret = setsockopt(vty_sock, IPPROTO_TCP, TCP_NODELAY, (char *)&on,
			 sizeof(on));
	if (ret < 0)
		zlog_info("dsv8EFR98dfwdXcH ""can't set sockopt to vty_sock : %s",
			  safe_strerror(errno));

	zlog_info("b5UdcLKme1dZlVVY ""Vty connection from %pSU", &su);
...
static void vty_serv_sock_addrinfo(const char *hostname, unsigned short port)
{
	...
	if (ret != 0) {
		flog_err_sys(EC_LIB_SYSTEM_CALL,"BMqOK1NhyWaLKqu8 " "getaddrinfo failed: %s",
			     gai_strerror(ret));
		exit(1);
	}
...
static void vty_serv_un(const char *path)
{
	...
	if (sock < 0) {
		flog_err_sys(EC_LIB_SOCKET,"P9L43QETOPRR8iiD "
	...
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"ErtPvcN46P8LDEeO " "Cannot bind path %s: %s", path,
			     safe_strerror(errno));
		close(sock); /* Avoid sd leak. */
		return;
	}
	...
	ret = listen(sock, 5);
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"1eJeis2pL9cTIo4y " "listen(fd %d) failed: %s", sock,
			     safe_strerror(errno));
		close(sock); /* Avoid sd leak. */
		return;
	}
	...
	if ((int)ids.gid_vty > 0) {
		...
		if (chown(path, -1, ids.gid_vty)) {
			flog_err_sys(EC_LIB_SYSTEM_CALL,"sa4HCfaitX39TfaK "
...
static void vtysh_accept(struct event *thread)
{
	...
	if (sock < 0) {
		flog_err(EC_LIB_SOCKET,"0sBRCF7RVJUJ8F9p " "can't accept vty socket : %s",
			 safe_strerror(errno));
		return;
	}
	...
	if (set_nonblocking(sock) < 0) {
		flog_err(
			EC_LIB_SOCKET,"PTK3tMU4JfPblubG "
...
static int vtysh_flush(struct vty *vty)
{
	...
	case BUFFER_PENDING:
		...
		break;
	case BUFFER_ERROR:
		flog_err(EC_LIB_SOCKET,"TLTCG1wVscDrdVvQ " "%s: write error to fd %d, closing",
			 __func__, vty->fd);
		buffer_reset(vty->lbuf);
		buffer_reset(vty->obuf);
		vty_close(vty);
...
bool mgmt_vty_read_configs(void)
{
	...
	for (index = 0; index < array_size(mgmt_daemons); index++) {
		...
		zlog_info("wN5KMbuEBaaaRi4h ""mgmtd: reading config file: %s", path);
	...
	confp = vty_open_config(path, config_default);
	if (confp) {
		zlog_info("hiPdW9VAp3fpMgRw ""mgmtd: reading config file: %s", path);
	...
	zlog_info("SfIur2qpebdqKbwt ""mgmtd: finished reading config files");
...
static void vtysh_read(struct event *thread)
{
	...
	if ((nbytes = read(sock, buf, VTY_READ_BUFSIZ)) <= 0) {
		if (nbytes < 0) {
			...
			flog_err(
				EC_LIB_SOCKET,"emnQqCdqccIqxNRc "
...
void vty_read_file_finish(struct vty *vty, struct nb_config *config)
{
	...
	for (ALL_LIST_ELEMENTS_RO(vty->error, node, ve)) {
		...
		if (nl)
			*nl = '\0';
		flog_err(EC_LIB_VTY,"ibClUV7Jv7Aypxfm " "%s on config line %u: %s", message,
	...
	if (config_file != NULL) {
		if (!IS_DIRECTORY_SEP(config_file[0])) {
			if (getcwd(cwd, MAXPATHLEN) == NULL) {
				flog_err_sys(
					EC_LIB_SYSTEM_CALL,"MWKcrB38w2EUBBFE "
		...
		if (confp == NULL) {
			flog_warn(
				EC_LIB_BACKUP_CONFIG,"mGVyWjyyfBJ5AuB3 "
				"%s: failed to open configuration file %s: %s, checking backup",
				__func__, fullpath, safe_strerror(errno));

			confp = vty_use_backup_config(fullpath);
			if (confp)
				flog_warn(EC_LIB_BACKUP_CONFIG,"eTzBZv1qWA8UM1SC "
					  "using backup configuration file!");
			else {
				flog_err(
					EC_LIB_VTY,"efvPBwwNZH58DDdR "
		...
		if (confp == NULL) {
			flog_err(
				EC_LIB_SYSTEM_CALL,"d08aquPawCAiGS0A "
				"%s: failed to open configuration file %s: %s, checking backup",
			...
			if (confp) {
				flog_warn(EC_LIB_BACKUP_CONFIG,"9gCxHWhZZuZRw8M1 "
					  "using backup configuration file!");
				fullpath = config_default_dir;
			} else {
				flog_err(EC_LIB_VTY,"Mn4jc7oomBaCxCqH "
...
static void vty_save_cwd(void)
{
	...
	if (!c) {
		 ...
		 */
		if (chdir(SYSCONFDIR)) {
			flog_err_sys(EC_LIB_SYSTEM_CALL,"Id92ZgHfT2RcA0Ds "
				     "Failure to chdir to %s, errno: %d",
				     SYSCONFDIR, errno);
			exit(-1);
		}
		if (getcwd(vty_cwd, sizeof(vty_cwd)) == NULL) {
			flog_err_sys(EC_LIB_SYSTEM_CALL,"PG4mCuKeWBBdG7RA "
				    ...
				    uintptr_t session_id, uintptr_t session_ctx)
{
	...
	if (!success) {
		zlog_err("W8XfeN2Aoki32dkW ""%s session for client %" PRIu64 " failed!",
				      ...
				      char *errmsg_if_any)
{
	...
	if (!success)
		zlog_err("tJQhCf1cPNDzHD8l ""%socking for DS %u failed, Err: '%s' vty %p",
	...
	char *errmsg_if_any)
{
	...
	if (!success) {
		zlog_err("mZaW0Q86qdNfNauh ""SET_CONFIG request for client 0x%" PRIx64
			 " failed, Error: '%s'",
	...
	Mgmtd__DatastoreId dst_ds_id, bool validate_only, char *errmsg_if_any)
{
	...
	if (!success) {
		zlog_err("DyxKUHC0Dt30Fnnk ""COMMIT_CONFIG request for client 0x%" PRIx64
			 " failed, Error: '%s'",
			 client_id, errmsg_if_any ? errmsg_if_any : "Unknown");
		vty_out(vty, "%% Configuration failed.\n\n");
		if (errmsg_if_any)
	...
	size_t num_data, int next_key, char *errmsg_if_any)
{
	...
	if (!success) {
		zlog_err("LcQRUrAGSjkW9x04 ""GET_DATA request for client 0x%" PRIx64
			 " failed, Error: '%s'",
			 client_id, errmsg_if_any ? errmsg_if_any : "Unknown");
			     ...
			     bool lock, bool scok)
{
	...
	if (mgmt_fe_send_lockds_req(mgmt_fe_client, vty->mgmt_session_id,
		...
		zlog_err("PPYdBmUJk2ZgEaIp ""Failed sending %sLOCK-DS-REQ req-id %" PRIu64,
			      ...
			      bool implicit_commit)
{
	...
	if (mgmt_fe_send_setcfg_req(mgmt_fe_client, vty->mgmt_session_id,
				    ...
				    MGMTD_DS_RUNNING)) {
		zlog_err("RI21WJwaDPS70S2K ""Failed to send %zu config xpaths to mgmtd", indx);
...
int vty_mgmt_send_commit_config(struct vty *vty, bool validate_only, bool abort)
{
	if (mgmt_fe_client && vty->mgmt_session_id) {
		...
		if (mgmt_fe_send_commitcfg_req(
			...
			zlog_err("B1KL6NuJrZEJQ32L ""Failed sending COMMIT-REQ req-id %" PRIu64,
				 vty->mgmt_req_id);
			  ...
			  int num_req)
{
	...
	if (mgmt_fe_send_get_req(mgmt_fe_client, vty->mgmt_session_id,
				 ...
				 num_req)) {
		zlog_err("WiRpymLkFU2dU1o0 ""Failed to send GET- to MGMTD for req-id %" PRIu64 ".",
			 vty->mgmt_req_id);
			       ...
			       uint8_t defaults, const char *xpath)
{
	...
	if (mgmt_fe_send_get_data_req(mgmt_fe_client, vty->mgmt_session_id,
		...
		zlog_err("CianJkZyDb20CL5w ""Failed to send GET-DATA to MGMTD session-id: %" PRIu64
			 " req-id %" PRIu64 ".",
			   ...
			   const char *data)
{
	...
	if (mgmt_fe_send_edit_req(mgmt_fe_client, vty->mgmt_session_id,
		...
		zlog_err("EqBwaBzIksbxyicn ""Failed to send EDIT to MGMTD session-id: %" PRIu64
			 " req-id %" PRIu64 ".",
			 vty->mgmt_session_id, vty->mgmt_req_id);
			  ...
			  const char *xpath, const char *data)
{
	...
	if (mgmt_fe_send_rpc_req(mgmt_fe_client, vty->mgmt_session_id,
		...
		zlog_err("X7i8xU0lLQsTPzSN ""Failed to send RPC to MGMTD session-id: %" PRIu64
			 " req-id %" PRIu64 ".",
			 vty->mgmt_session_id, vty->mgmt_req_id);
