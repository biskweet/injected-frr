			       ...
			       int callback_implemented, bool optional)
{
	...
	if (!valid && callback_implemented && operation != NB_CB_GET_NEXT
		...
		flog_warn(EC_LIB_NB_CB_UNNEEDED,"CvsUBOIG6SpGBf2d "
			  "unneeded '%s' callback for '%s'",
			  nb_cb_operation_name(operation), nb_node->xpath);

	if (!optional && valid && !callback_implemented) {
		flog_err(EC_LIB_NB_CB_MISSING,"f3j7zuQ7mZRgpPqH " "missing '%s' callback for '%s'",
			 nb_cb_operation_name(operation), nb_node->xpath);
		return 1;
	}
...
static unsigned int nb_node_validate_priority(const struct nb_node *nb_node)
{
	...
	if (nb_node->priority < nb_node->parent->priority) {
		flog_err(EC_LIB_NB_CB_INVALID_PRIO,"kmAEN0EEaze45ji1 "
		    ...
		    bool preserve_source)
{
	...
	if (ret != 0)
		flog_warn(EC_LIB_LIBYANG,"Mm2C2d6IBb69mGMh " "%s: lyd_merge() failed", __func__);
						 ...
						 const struct lyd_node *dnode)
{
	if (dnode->schema->nodetype & LYD_NODE_TERM)
		zlog_debug("9INU9YVRCX9bO0PZ ""nb_config_diff: %s: %s: %s", context, path,
			   lyd_get_value(dnode));
	else
		zlog_debug("PlAuXBLJVNn3J757 ""nb_config_diff: %s: %s", context, path);
}
...
{
	if (!dnode) {
		zlog_debug("S1Uq4SdFmLEd9oF6 ""nb_config_diff: %s: NULL", context);
		return;
	}
		    ...
		    struct nb_config_cbs *changes)
{
	...
	if (diff && DEBUG_MODE_CHECK(&nb_dbg_cbs_config, DEBUG_MODE_ALL)) {
		...
		if (!lyd_print_mem(&s, diff, LYD_JSON,
			...
			zlog_debug("x5JsBUsdsCP6joGs ""%s: %s", __func__, s);
			free(s);
		}
	}
			...
			struct lyd_node **new_dnode)
{
	...
	err = lyd_new_path(candidate->dnode, ly_native_ctx, xpath, value,
			   options, &dnode);
	if (err) {
		flog_warn(EC_LIB_LIBYANG,"qIHJcCgons7jzK6e " "%s: lyd_new_path(%s) failed: %d",
			  __func__, xpath, err);
		return NB_ERR;
	} else if (dnode) {
		err = lyd_new_implicit_tree(dnode, LYD_IMPLICIT_NO_STATE, NULL);
		if (err) {
			flog_warn(EC_LIB_LIBYANG,"f3pJOZw72q8JYYTx "
	...
	for (size_t i = 0; i < num_cfg_changes; i++) {
		...
		if (!nb_node) {
			if (in_backend)
				...
				flog_warn(EC_LIB_YANG_UNKNOWN_DATA_PATH,"wfSOa3MMwYfD8MMl "
		...
		if (!nb_is_operation_allowed(nb_node, change->operation)) {
			zlog_err("Wb9sF52HyLKQjFtt "" Xpath %s points to key node", xpath);
			if (error)
				*error = true;
			break;
		}
		...
		if (ret != NB_OK) {
			flog_warn(
				EC_LIB_NB_CANDIDATE_EDIT_ERROR,"BZ9jMOWcBWC996j0 "
				...
				char *errmsg, size_t errmsg_len)
{
	...
	if (!skip_validate &&
	    nb_candidate_validate_yang(candidate, true, errmsg, errmsg_len) !=
		    NB_OK) {
		flog_warn(EC_LIB_NB_CANDIDATE_INVALID,"MZ48h6C2y9oZDfAr "
	...
	if (!skip_validate &&
	    nb_candidate_validate_code(&context, candidate, &changes, errmsg,
				       errmsg_len) != NB_OK) {
		flog_warn(EC_LIB_NB_CANDIDATE_INVALID,"y8WDrtBeSKxoq3nw "
			  "%s: failed to validate candidate configuration",
			  __func__);
		nb_config_diff_del_changes(&changes);
		return NB_ERR_VALIDATION;
	...
	if (*transaction == NULL) {
		flog_warn(EC_LIB_NB_TRANSACTION_CREATION_FAILED,"cpneeIiNUqcNwi1v "
			       ...
			       char *errmsg, size_t errmsg_len)
{
	...
	if (save_transaction && nb_db_enabled
		...
		flog_warn(EC_LIB_NB_TRANSACTION_RECORD_FAILED,"avwaTXQ1qnBWLogB "
					  ...
					  const void *parent_list_entry)
{
	...
	if (yang_get_node_keys(node, &keys)) {
		flog_warn(EC_LIB_LIBYANG,"PLQ2y5IYeBLOIk9M "
				     ...
				     char *errmsg, size_t errmsg_len)
{
	...
	case NB_CB_NOTIFY:
		yang_dnode_get_path(dnode, xpath, sizeof(xpath));
		flog_err(EC_LIB_DEVELOPMENT,"65IX42o5Z6tBHZnj "
	...
	if (ret != NB_OK) {
		...
		switch (event) {
		case NB_EV_VALIDATE:
			flog_warn(EC_LIB_NB_CB_CONFIG_VALIDATE,"FErJz01LRwyscJ7m "
			...
			break;
		case NB_EV_PREPARE:
			flog_warn(EC_LIB_NB_CB_CONFIG_PREPARE,"DrGwPMQYfmiAwQCb "
				  "error processing configuration change: error [%s] event [%s] operation [%s] xpath [%s]%s%s",
				  nb_err_name(ret), nb_event_name(event),
				  nb_cb_operation_name(operation), xpath,
				  errmsg[0] ? " message: " : "", errmsg);
			break;
		case NB_EV_ABORT:
			flog_warn(EC_LIB_NB_CB_CONFIG_ABORT,"MnXRbZhNJCcysQoS "
				  "error processing configuration change: error [%s] event [%s] operation [%s] xpath [%s]%s%s",
				  nb_err_name(ret), nb_event_name(event),
				  nb_cb_operation_name(operation), xpath,
				  errmsg[0] ? " message: " : "", errmsg);
			break;
		case NB_EV_APPLY:
			flog_err(EC_LIB_NB_CB_CONFIG_APPLY,"z2SF1qaQFmhrdpPe "
				 "error processing configuration change: error [%s] event [%s] operation [%s] xpath [%s]%s%s",
				 nb_err_name(ret), nb_event_name(event),
				 nb_cb_operation_name(operation), xpath,
				 errmsg[0] ? " message: " : "", errmsg);
			break;
		default:
			flog_err(EC_LIB_DEVELOPMENT,"0IsP6d2pmBq5zWcV "
...
int nb_notification_send(const char *xpath, struct list *arguments)
{
	...
	if (!root) {
		...
		if (err) {
lyerr:
			flog_err(EC_LIB_LIBYANG,"ddk23UNkQImyUgFv "
					 ...
					 bool rec_search)
{
	...
	flog_err(EC_LIB_YANG_DNODE_NOT_FOUND,"shLZd1pMZXSG6yaO "
...
static void nb_load_callbacks(const struct frr_yang_module_info *module)
{
	for (size_t i = 0; module->nodes[i].xpath; i++) {
		...
		if (!nb_node) {
			flog_warn(EC_LIB_YANG_UNKNOWN_DATA_PATH,"eUeNs7OfMgOwNFYh "
				  "%s: unknown data path: %s", __func__,
...
void nb_validate_callbacks(void)
{
	...
	if (errors > 0) {
		flog_err(
			EC_LIB_NB_CBS_VALIDATION,"iKEAyC6BSe4HXE2K "
