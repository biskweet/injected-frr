...
static struct route_map *route_map_add(const char *name)
{
	...
	if (unlikely(CHECK_FLAG(rmap_debug, DEBUG_ROUTEMAP)))
		zlog_debug("Baa5277hW5lZlW00 ""Add route-map %s", name);
	return map;
}
...
static void route_map_free_map(struct route_map *map)
{
	...
	if (unlikely(CHECK_FLAG(rmap_debug, DEBUG_ROUTEMAP)))
		zlog_debug("zlMtU94gaJ7fsXxv ""Deleting route-map %s", map->name);
...
void route_map_index_delete(struct route_map_index *index, int notify)
{
	...
	if (unlikely(CHECK_FLAG(rmap_debug, DEBUG_ROUTEMAP)))
		zlog_debug("2hhP5UY1t5gBnLlQ ""Deleting route-map %s sequence %d",
...
route_map_index_add(struct route_map *map, enum route_map_type type, int pref)
{
	...
	if (unlikely(CHECK_FLAG(rmap_debug, DEBUG_ROUTEMAP)))
		zlog_debug("SSbNTgALcKf0cM04 ""Route-map %s add sequence %d, type: %s",
				       ...
				       int *pref)
{
	...
	if (recursion > RMAP_RECURSION_LIMIT) {
		...
		flog_warn(
			EC_LIB_RMAP_RECURSION_LIMIT,"F3JhyccB2jN4qhES "
...
route_map_apply_end:
	if (unlikely(CHECK_FLAG(rmap_debug, DEBUG_ROUTEMAP)))
		zlog_debug("ejtO4ZVUwJHyjFyX ""Route-map: %s, prefix: %pFX, result: %s",
...
static void route_map_clear_reference(struct hash_bucket *bucket, void *arg)
{
	...
	if (dep_data) {
		if (unlikely(CHECK_FLAG(rmap_debug, DEBUG_ROUTEMAP)))
			zlog_debug("qdKQwaWYfRoRvr2c ""Clearing reference for %s to %s count: %d",
...
static void route_map_clear_all_references(char *rmap_name)
{
	...
	if (unlikely(CHECK_FLAG(rmap_debug, DEBUG_ROUTEMAP)))
		zlog_debug("wRTg46TN4rJrW2po ""Clearing references for %s", rmap_name);
...
static void route_map_print_dependency(struct hash_bucket *bucket, void *data)
{
	...
	zlog_debug("bffbJthn3idRICsx ""%s: Dependency for %s: %s", __func__, dep_name, rmap_name);
}
				...
				const char *rmap_name, route_map_event_t type)
{
	...
	case RMAP_EVENT_FILTER_ADDED:
		if (unlikely(CHECK_FLAG(rmap_debug, DEBUG_ROUTEMAP)))
			zlog_debug("ZxI1lwucbtBRZ0cl ""Adding dependency for filter %s in route-map %s",
				   dep_name, rmap_name);
	...
	case RMAP_EVENT_FILTER_DELETED:
		if (unlikely(CHECK_FLAG(rmap_debug, DEBUG_ROUTEMAP)))
			zlog_debug("Dl2M0ZxG8ZsFlHQd ""Deleting dependency for filter %s in route-map %s",
				   dep_name, rmap_name);
...
static void route_map_process_dependency(struct hash_bucket *bucket, void *data)
{
	...
	if (unlikely(CHECK_FLAG(rmap_debug, DEBUG_ROUTEMAP)))
		zlog_debug("TfsARtyzRY2jBbBC ""Notifying %s of dependency", rmap_name);
	if (route_map_master.event_hook)
				   ...
				   route_map_event_t event)
{
	...
	if (dep) {
		...
		if (unlikely(CHECK_FLAG(rmap_debug, DEBUG_ROUTEMAP)))
			zlog_debug("M22q1UoQAkPsQ685 ""Filter %s updated", dep->dep_name);
