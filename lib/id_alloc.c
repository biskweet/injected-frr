						 ...
						 uint32_t id, int create)
{
	...
	if (page == NULL && create) {
		...
		flog_err(
			EC_LIB_ID_CONSISTENCY,"ZK7t3YjfeJGd2EdG "
...
void idalloc_free(struct id_alloc *alloc, uint32_t id)
{
	...
	if (!page) {
		flog_err(EC_LIB_ID_CONSISTENCY,"SjKyU3eigKWxELzX "
	...
	if ((page->allocated_mask[word] & (1 << offset)) == 0) {
		flog_err(EC_LIB_ID_CONSISTENCY,"35ESVJx1N6SbM3Ce "
...
uint32_t idalloc_allocate(struct id_alloc *alloc)
{
	...
	if (alloc->has_free == NULL) {
		flog_err(EC_LIB_ID_EXHAUST,"tuvsItcylUrfMjTb "
	...
	if (word < 0 || word >= 32) {
		flog_err(EC_LIB_ID_CONSISTENCY,"NSfoCUPyznckmdf7 "
	...
	if (offset < 0 || offset >= 32) {
		flog_err(EC_LIB_ID_CONSISTENCY,"MidRVsKp2zzEM08X "
...
uint32_t idalloc_reserve(struct id_alloc *alloc, uint32_t id)
{
	...
	if (page->allocated_mask[word] & (((uint32_t)1) << offset)) {
		flog_err(EC_LIB_ID_CONSISTENCY,"RedI8clOQNfCgRFz "
