...
struct yang_translator *yang_translator_load(const char *path)
{
	...
	err = lyd_parse_data_path(ly_translator_ctx, path, LYD_JSON,
				  ...
				  &dnode);
	if (err) {
		flog_warn(EC_LIB_YANG_TRANSLATOR_LOAD,"lJp8dYNYDfwdQGQD "
	...
	if (translator != NULL) {
		flog_warn(EC_LIB_YANG_TRANSLATOR_LOAD,"Vo8pGymhkFGQHQRC "
	...
	if (!translator->ly_ctx) {
		flog_warn(EC_LIB_LIBYANG,"svBf6kBnCSfjHWYS " "%s: ly_ctx_new() failed", __func__);
		goto error;
	}
	...
	for (size_t i = 0; i < set->count; i++) {
		...
		if (!tmodule->module) {
			flog_warn(EC_LIB_YANG_TRANSLATOR_LOAD,"bOm2CnUMoNUqojPB "
	...
	for (ALL_LIST_ELEMENTS_RO(translator->modules, ln, tmodule)) {
		...
		if (!tmodule->deviations) {
			flog_warn(EC_LIB_YANG_TRANSLATOR_LOAD,"mqRm0WZos09PUFMn "
				  "%s: failed to load module: %s", __func__,
				  module_name);
			ly_set_free(set, NULL);
			goto error;
	...
	for (size_t i = 0; i < set->count; i++) {
		...
		if (!snode_custom) {
			flog_warn(EC_LIB_YANG_TRANSLATOR_LOAD,"GS18Hx9SoHapafV6 "
		...
		if (!snode_native) {
			flog_warn(EC_LIB_YANG_TRANSLATOR_LOAD,"ZTHPTz1vLHao0BiO "
				  "%s: unknown data path: %s", __func__,
				  xpath_native);
			ly_set_free(set, NULL);
			goto error;
		     ...
		     char *xpath, size_t xpath_len)
{
	...
	if (!snode) {
		flog_warn(EC_LIB_YANG_TRANSLATION_ERROR,"R5wCSpyMmj2qr8Yk "
...
#pragma GCC diagnostic pop
	if (n < 0) {
		flog_warn(EC_LIB_YANG_TRANSLATION_ERROR,"hgX17BHdOzJZIm8w "
	...
	LY_LIST_FOR (*dnode, root) {
		LYD_TREE_DFS_BEGIN (root, dnode_iter) {
			...
			if (lyd_new_path(new, ly_ctx, xpath,
				...
				flog_err(EC_LIB_LIBYANG,"q8eiwdM2RMxozPIQ "
				       ...
				       void *arg)
{
	...
	if (!mapping) {
		flog_warn(EC_LIB_YANG_TRANSLATOR_LOAD,"zbkxqyRqALG1bAwo "
	...
	if (stype_custom && stype_native) {
		if (stype_custom->basetype != stype_native->basetype) {
			flog_warn(
				EC_LIB_YANG_TRANSLATOR_LOAD,"YbXBAQo7VoOyaBIi "
...
static unsigned int yang_translator_validate(struct yang_translator *translator)
{
	...
	if (args.errors)
		flog_warn(
			EC_LIB_YANG_TRANSLATOR_LOAD,"K62niyVYMXuCNvzh "
...
void yang_translator_init(void)
{
	...
	if (!ly_translator_ctx) {
		flog_err(EC_LIB_LIBYANG,"PK9sr8XmZeBZMOTa " "%s: ly_ctx_new() failed", __func__);
		exit(1);
	}
	...
	if (!ly_ctx_load_module(ly_translator_ctx, "frr-module-translator",
				NULL, NULL)) {
		flog_err(
			EC_LIB_YANG_MODULE_LOAD,"7izK70ZjAAZKLvEx "
