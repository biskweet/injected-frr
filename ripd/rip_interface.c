			       ...
			       struct in_addr ifa, ifindex_t ifindex)
{
	...
	if (ret < 0)
		zlog_info("fzh0nerPOE8oWNo3 ""can't setsockopt IP_ADD_MEMBERSHIP %s",
			  safe_strerror(errno));
				...
				struct in_addr ifa, ifindex_t ifindex)
{
	...
	if (ret < 0)
		zlog_info("VKdAazcTZvmGFDvt ""can't setsockopt IP_DROP_MEMBERSHIP");
...
static void rip_request_interface_send(struct interface *ifp, uint8_t version)
{
	...
	if (version == RIPv2 && if_is_multicast(ifp)) {
		...
		if (IS_RIP_DEBUG_EVENT)
			zlog_debug("vDmJlSF4vBnwhaAI ""multicast request on %s", ifp->name);
	...
	if (if_is_pointopoint(ifp) || if_is_broadcast(ifp)) {
		...
		if (IS_RIP_DEBUG_EVENT)
			zlog_debug("PvdiOrGviN1uxb2t ""broadcast request to %s", ifp->name);
		...
		frr_each (if_connected, ifp->connected, connected) {
			...
			if (IS_RIP_DEBUG_EVENT)
				zlog_debug("XwHeVAWR4dPh38C3 ""SEND request to %pI4",
					   &to.sin_addr);
...
static int rip_multicast_join(struct interface *ifp, int sock)
{
	...
	if (if_is_operative(ifp) && if_is_multicast(ifp)) {
		if (IS_RIP_DEBUG_EVENT)
			zlog_debug("5rwcjNqrepQags4I ""multicast join at %s", ifp->name);
...
static void rip_multicast_leave(struct interface *ifp, int sock)
{
	...
	if (if_is_up(ifp) && if_is_multicast(ifp)) {
		if (IS_RIP_DEBUG_EVENT)
			zlog_debug("09fAadMjVSPEkpkY ""multicast leave from %s", ifp->name);
...
int rip_if_down(struct interface *ifp)
{
	...
	if (rip) {
		...
		if (ri->running) {
			if (IS_RIP_DEBUG_EVENT)
				zlog_debug("ge1hEZXwxR7NCqZv ""turn off %s", ifp->name);
...
int rip_interface_address_add(ZAPI_CALLBACK_ARGS)
{
	...
	if (p->family == AF_INET) {
		if (IS_RIP_DEBUG_ZEBRA)
			zlog_debug("ZwjejYN9b83kCeak ""connected address %pFX is added", p);
...
int rip_interface_address_delete(ZAPI_CALLBACK_ARGS)
{
	...
	if (ifc) {
		...
		if (p->family == AF_INET) {
			if (IS_RIP_DEBUG_ZEBRA)
				zlog_debug("6yjkA9IOW97hAKeT ""connected address %pFX is deleted",
					   p);
...
static void rip_interface_wakeup(struct event *t)
{
	...
	if (rip_multicast_join(ifp, ri->rip->sock) < 0) {
		flog_err_sys(EC_LIB_SOCKET,"Aav6sZvJi74TuvnS "
...
void rip_enable_apply(struct interface *ifp)
{
	...
	if (ri->enable_network || ri->enable_interface) {
		if (IS_RIP_DEBUG_EVENT)
			zlog_debug("oGYlpYVbksDxf6cA ""turn on %s", ifp->name);
...
static void rip_passive_interface_apply(struct interface *ifp)
{
	...
	if (IS_RIP_DEBUG_ZEBRA)
		zlog_debug("RBJ3QV1j4QTUWPut ""interface %s: passive = %d", ifp->name,
			   ri->passive);
}
