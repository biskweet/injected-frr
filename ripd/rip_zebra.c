				...
				uint8_t cmd)
{
	...
	if (IS_RIP_DEBUG_ZEBRA) {
		if (rip->ecmp)
			zlog_debug("E9FxNI38CtQrMUsK ""%s: %pFX nexthops %d",
				   (cmd == ZEBRA_ROUTE_ADD)
					   ? "Install into zebra"
					   : "Delete from zebra",
				   &rp->p, count);
		else
			zlog_debug("fCbpm9fXygybDO2Z ""%s: %pFX",
				   (cmd == ZEBRA_ROUTE_ADD)
					   ? "Install into zebra"
...
void rip_zebra_vrf_register(struct vrf *vrf)
{
	...
	if (IS_RIP_DEBUG_EVENT)
		zlog_debug("ZgFANrofd0uI9iSn ""%s: register VRF %s(%u) to zebra", __func__,
			   vrf->name, vrf->vrf_id);
...
void rip_zebra_vrf_deregister(struct vrf *vrf)
{
	...
	if (IS_RIP_DEBUG_EVENT)
		zlog_debug("xCkOAVUelj9SwyRI ""%s: deregister VRF %s(%u) from zebra.", __func__,
			   vrf->name, vrf->vrf_id);
