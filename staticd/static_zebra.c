...
static int route_notify_owner(ZAPI_CALLBACK_ARGS)
{
	...
	case ZAPI_ROUTE_FAIL_INSTALL:
		...
		zlog_warn("ETWdfasJOW8Jgy6J ""%s: Route %pFX failed to install for table: %u",
			  __func__, &p, table_id);
		break;
	case ZAPI_ROUTE_BETTER_ADMIN_WON:
		static_nht_mark_state(&p, safi, vrf_id, STATIC_NOT_INSTALLED);
		zlog_warn("yMzRwJLfwLOJdzXC "
	...
	case ZAPI_ROUTE_REMOVE_FAIL:
		static_nht_mark_state(&p, safi, vrf_id, STATIC_INSTALLED);
		zlog_warn("f09X7DhKwCgGQ1bP ""%s: Route %pFX failure to remove for table: %u",
			  __func__, &p, table_id);
		break;
	}
					...
					struct zapi_route *nhr)
{
	...
	if (nhtd) {
		...
		zlog_err("t8G7dgv1ZiShs0yL ""No nhtd?");
}
...
void static_zebra_nht_register(struct static_nexthop *nh, bool reg)
{
	...
	if (nh->nh_registered) {
		...
		zlog_warn("sEiZckWLmax127Mb ""trying to unregister nexthop %pFX twice",
			  &lookup.nh);
		return;
	}
	...
	if (zclient_send_rnh(zclient, cmd, &lookup.nh, si->safi, false, false,
		...
		zlog_warn("5sOvKhlM8s2Y3AS2 ""%s: Failure to send nexthop %pFX for %pRN to zebra",
