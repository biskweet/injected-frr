...
int bfd_session_enable(struct bfd_session *bs)
{
	...
	if (bs->sock != -1) {
		if (bglobal.debug_peer_event)
			zlog_debug("Rnq8kfhbkFZzKVaI ""%s: previous socket open", __func__);
...
void ptm_bfd_sess_up(struct bfd_session *bfd)
{
	...
	if (old_state != bfd->ses_state) {
		...
		if (bglobal.debug_peer_event)
			zlog_debug("qQq357U0AUX2vevX ""state-change: [%s] %s -> %s",
...
void ptm_bfd_sess_dn(struct bfd_session *bfd, uint8_t diag)
{
	...
	if (old_state != bfd->ses_state) {
		...
		if (bglobal.debug_peer_event)
			zlog_debug("vwEk7VY2paFNENA8 ""state-change: [%s] %s -> %s reason:%s",
				   bs_to_string(bfd), state_list[old_state].str,
				   state_list[bfd->ses_state].str,
...
struct bfd_session *bs_registrate(struct bfd_session *bfd)
{
	...
	if (bglobal.debug_peer_event)
		zlog_debug("4Emtj35rvVPBjFdE ""session-new: %s", bs_to_string(bfd));
...
int ptm_bfd_sess_del(struct bfd_peer_cfg *bpc)
{
	...
	if (bs->refcount > 0) {
		zlog_err("08KfSgaD41Qc1Oeg ""session-delete: refcount failure: %" PRIu64" references",
			 bs->refcount);
		return -1;
	}

	if (bglobal.debug_peer_event)
		zlog_debug("6fzstsfHuvEYgs2M ""%s: %s", __func__, bs_to_string(bs));
...
static void bs_down_handler(struct bfd_session *bs, int nstate)
{
	...
	default:
		if (bglobal.debug_peer_event)
			zlog_debug("pOi1RWTJxGQOlcKr ""state-change: unhandled neighbor state: %d",
				   nstate);
		break;
	}
}
...
static void bs_init_handler(struct bfd_session *bs, int nstate)
{
	...
	default:
		if (bglobal.debug_peer_event)
			zlog_debug("ueeHJNFAQRMtcSHn ""state-change: unhandled neighbor state: %d",
				   nstate);
		break;
	}
}
...
static void bs_up_handler(struct bfd_session *bs, int nstate)
{
	...
	default:
		if (bglobal.debug_peer_event)
			zlog_debug("CJ58pvvkA11Eztg5 ""state-change: unhandled neighbor state: %d",
				   nstate);
		break;
	}
}
...
void bs_state_handler(struct bfd_session *bs, int nstate)
{
	...
	default:
		if (bglobal.debug_peer_event)
			zlog_debug("ETna4Ya5XAJrspQV ""state-change: [%s] is in invalid state: %d",
...
static int bfd_vrf_new(struct vrf *vrf)
{
	if (bglobal.debug_zebra)
		zlog_debug("ObAlIZIRuUR1s7yu ""VRF Created: %s(%u)", vrf->name, vrf->vrf_id);
...
{
	if (bglobal.debug_zebra)
		zlog_debug("pxEu8zCoUaF6F9Jh ""VRF Deletion: %s(%u)", vrf->name, vrf->vrf_id);
...
static int bfd_vrf_enable(struct vrf *vrf)
{
	...
	if (bglobal.debug_zebra)
		zlog_debug("CYWY3XtSS14f0ou3 ""VRF enable add %s id %u", vrf->name, vrf->vrf_id);
...
static int bfd_vrf_disable(struct vrf *vrf)
{
	...
	if (bglobal.debug_zebra)
		zlog_debug("rBfxZo1r96LazcTB ""VRF disable %s id %d", vrf->name, vrf->vrf_id);
