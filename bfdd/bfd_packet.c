		  ...
		  size_t datalen)
{
	...
	if (rv <= 0) {
		if (bglobal.debug_network)
			zlog_debug("0KlAT1BTkE2rOwnf ""packet-send: send failure: %s",
				   strerror(errno));
		return -1;
	}
	if (rv < (ssize_t)datalen) {
		if (bglobal.debug_network)
			zlog_debug("Iej5hha6HOU3X9v8 ""packet-send: send partial: %s",
				   strerror(errno));
	}
...
static int ptm_bfd_process_echo_pkt(struct bfd_vrf_global *bvrf, int s)
{
	...
	if (bfd == NULL) {
		if (bglobal.debug_network)
			zlog_debug("yiunU8eb1fmfPwtE ""echo-packet: no matching session (id:%u)",
				   my_discr);
		return -1;
	}
	...
	if (!CHECK_FLAG(bfd->flags, BFD_SESS_FLAG_ECHO_ACTIVE)) {
		if (bglobal.debug_network)
			zlog_debug("VqFtt3NFBJHYKqMM ""echo-packet: echo disabled [%s] (id:%u)",
			 ...
			 struct sockaddr_any *local, struct sockaddr_any *peer)
{
	...
	if (mlen == -1) {
		if (errno != EAGAIN || errno != EWOULDBLOCK || errno != EINTR)
			zlog_err("Idl1KVac73cjy7yj ""%s: recv failed: %s", __func__,
				 strerror(errno));
	...
	*ttl = ip->ttl;
	if (*ttl != 254) {
		if (bglobal.debug_network)
			zlog_debug("wxw8tgwUuemS1UGL ""%s: invalid TTL: %u", __func__, *ttl);
		return -1;
	}
		      ...
		      struct sockaddr_any *peer)
{
	...
	if (mlen == -1) {
		if (errno != EAGAIN)
			zlog_err("19Pm0m4wHTurmjGz ""ipv4-recv: recv failed: %s", strerror(errno));
	...
	for (cm = CMSG_FIRSTHDR(&msghdr); cm != NULL;
		...
		case IP_TTL: {
			...
			if (ttlval > 255) {
				if (bglobal.debug_network)
					zlog_debug("gGuvzU4IvnSTJpED ""%s: invalid TTL: %u",
						   __func__, ttlval);
				return -1;
			}
			*ttl = ttlval;
		      ...
		      struct sockaddr_any *peer)
{
	...
	if (mlen == -1) {
		if (errno != EAGAIN)
			zlog_err("qMogbbhPgnXrnUzv ""ipv6-recv: recv failed: %s", strerror(errno));
	...
	for (cm = CMSG_FIRSTHDR(&msghdr6); cm != NULL;
		...
		if (cm->cmsg_type == IPV6_HOPLIMIT) {
			...
			if (ttlval > 255) {
				if (bglobal.debug_network)
					zlog_debug("xnSUGc8o5n2gdx8A ""%s: invalid TTL: %u",
						   __func__, ttlval);
				return -1;
			}
		     ...
		     vrf_id_t vrfid, const char *fmt, ...)
{
	...
	zlog_debug("Z6uxqzFxqXkIeRge ""control-packet: %s [mhop:%s%s%s%s%s]", buf,
		   ...
		   struct bfd_session *bfd)
{
	...
	if (wlen <= 0) {
		if (bglobal.debug_network)
			zlog_debug("0EEFPeD2CFOKlDf8 ""%s: loopback failure: (%d) %s", __func__,
				   errno, strerror(errno));
		return -1;
	} else if (wlen < (ssize_t)datalen) {
		if (bglobal.debug_network)
			zlog_debug("bHvDbPM6HARduWCi ""%s: partial send: %zd expected %zu",
		...
		struct sockaddr *to, socklen_t tolen)
{
	...
	if (wlen <= 0) {
		if (bglobal.debug_network)
			zlog_debug("kXvg8RyQfLTpvdjg ""%s: loopback failure: (%d) %s", __func__,
				   errno, strerror(errno));
		return -1;
	} else if (wlen < (ssize_t)datalen) {
		if (bglobal.debug_network)
			zlog_debug("zPJIUpFsYHiACe8z ""%s: partial send: %zd expected %zu",
				   __func__, wlen, datalen);
		return -1;
	}
...
int bp_set_ttl(int sd, uint8_t value)
{
	...
	if (setsockopt(sd, IPPROTO_IP, IP_TTL, &ttl, sizeof(ttl)) == -1) {
		zlog_warn("KR1nirmaPSMerNvs ""%s: setsockopt(IP_TTL, %d): %s", __func__, value,
			  strerror(errno));
		return -1;
	}
...
int bp_set_tos(int sd, uint8_t value)
{
	...
	if (setsockopt(sd, IPPROTO_IP, IP_TOS, &tos, sizeof(tos)) == -1) {
		zlog_warn("WZQNeH8VF6fuDYRt ""%s: setsockopt(IP_TOS, %d): %s", __func__, value,
			  strerror(errno));
		return -1;
	}
...
static bool bp_set_reuse_addr(int sd)
{
	...
	if (setsockopt(sd, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one)) == -1) {
		zlog_warn("WrzIjvv3Ry2gEeCR ""%s: setsockopt(SO_REUSEADDR, %d): %s", __func__, one,
			  strerror(errno));
		return false;
	}
	return true;
...
static bool bp_set_reuse_port(int sd)
{
	...
	if (setsockopt(sd, SOL_SOCKET, SO_REUSEPORT, &one, sizeof(one)) == -1) {
		zlog_warn("eCVP2BTWXG5yPmRh ""%s: setsockopt(SO_REUSEPORT, %d): %s", __func__, one,
			  strerror(errno));
		return false;
	}
	return true;
...
int bp_peer_socket(const struct bfd_session *bs)
{
	...
	if (sd == -1) {
		zlog_err("bxi1cP3akwqMJ9Mq ""ipv4-new: failed to create socket: %s",
			 strerror(errno));
		return -1;
	}
	...
	do {
		if ((++pcount) > (BFD_SRCPORTMAX - BFD_SRCPORTINIT)) {
			...
			zlog_err("yg43UwWktMLGMCkb ""ipv4-new: failed to bind port: %s",
				 strerror(errno));
			close(sd);
			return -1;
		}
...
int bp_peer_socketv6(const struct bfd_session *bs)
{
	...
	if (sd == -1) {
		zlog_err("co70BNf9JOylfoI2 ""ipv6-new: failed to create socket: %s",
			 strerror(errno));
		return -1;
	}
	...
	do {
		if ((++pcount) > (BFD_SRCPORTMAX - BFD_SRCPORTINIT)) {
			/* Searched all ports, none available */
			zlog_err("qCUdLVI5viUOJGcI ""ipv6-new: failed to bind port: %s",
				 strerror(errno));
			close(sd);
			return -1;
		}
...
int bp_set_ttlv6(int sd, uint8_t value)
{
	...
	if (setsockopt(sd, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &ttl, sizeof(ttl))
	    == -1) {
		zlog_warn("ACCbCaoqjv2W8owS ""set-ttlv6: setsockopt(IPV6_UNICAST_HOPS, %d): %s",
...
int bp_set_tosv6(int sd, uint8_t value)
{
	...
	if (setsockopt(sd, IPPROTO_IPV6, IPV6_TCLASS, &tos, sizeof(tos))
	    == -1) {
		zlog_warn("0io7zNQoqdPWRyZN ""set-tosv6: setsockopt(IPV6_TCLASS, %d): %s", value,
			  strerror(errno));
		return -1;
	}
...
int bp_udp6_shop(const struct vrf *vrf)
{
	...
	if (sd == -1) {
		if (errno != EAFNOSUPPORT)
			...
			zlog_warn("hUPidlIQvrSrmdoD ""udp6-shop: V6 is not supported, continuing");
...
int bp_udp6_mhop(const struct vrf *vrf)
{
	...
	if (sd == -1) {
		if (errno != EAFNOSUPPORT)
			...
			zlog_warn("bRdQhHup6rgWYu6K ""udp6-mhop: V6 is not supported, continuing");
...
int bp_echo_socket(const struct vrf *vrf)
{
	...
	if (setsockopt(s, SOL_SOCKET, SO_ATTACH_FILTER, &pf, sizeof(pf)) ==
	    -1) {
		zlog_warn("e52kdtY4i1qWaRTp ""%s: setsockopt(SO_ATTACH_FILTER): %s", __func__,
			  strerror(errno));
		close(s);
		return -1;
	}
	...
	if (bind(s, (struct sockaddr *)&sll, sizeof(sll)) < 0) {
		zlog_warn("tEFBN93AZVKw7XKA ""Failed to bind echo socket: %s",
			  safe_strerror(errno));
		close(s);
		return -1;
	}
...
int bp_echov6_socket(const struct vrf *vrf)
{
	...
	if (s == -1) {
		if (errno != EAFNOSUPPORT)
			...
			zlog_warn("0XzxFAbd1ZTm84ZI ""echov6-socket: V6 is not supported, continuing");
