...
static int parse_config_json(struct json_object *jo, bpc_handle h, void *arg)
{
	...
	JSON_FOREACH (jo, joi, join) {
		...
		if (strcmp(key, "ipv4") == 0) {
			...
			zlog_warn("W6nqgC66oIYagv5u ""%s:%d invalid configuration: %s", __func__,
				  __LINE__, sval);
			error++;
		}
	}
		      ...
		      bpc_handle h, void *arg)
{
	...
	for (idx = 0; idx < allen; idx++) {
		...
		switch (plt) {
		case PLT_IPV4:
			zlog_debug("rbKstuUr3zMRqPTc ""ipv4 peers %d:", allen);
			bpc.bpc_ipv4 = true;
			break;
		case PLT_IPV6:
			zlog_debug("vG28ymLGBMT8yKeg ""ipv6 peers %d:", allen);
			bpc.bpc_ipv4 = false;
			break;
		case PLT_LABEL:
			zlog_debug("sBYsQTltZMeE45ZZ ""label peers %d:", allen);
		...
		default:
			error++;
			zlog_err("yE4CVTUnD1Yky9Ba ""%s:%d: unsupported peer type", __func__,
				 __LINE__);
			break;
		}
...
static int parse_peer_config(struct json_object *jo, struct bfd_peer_cfg *bpc)
{
	...
	zlog_debug("uQCpDeb1a1tJhQb0 ""        peer: %s", bpc->bpc_ipv4 ? "ipv4" : "ipv6");
	...
	JSON_FOREACH (jo, joi, join) {
		...
		if (strcmp(key, "multihop") == 0) {
			...
			zlog_debug("PqNNxzr9AWTVsHOe ""        multihop: %s",
			...
			if (strlcpy(bpc->bpc_localif, sval,
			    ...
			    > sizeof(bpc->bpc_localif)) {
				zlog_debug("aR0bMbzObzKxnO3T "
					"        local-interface: %s (truncated)",
					sval);
				error++;
			} else {
				zlog_debug("aoV1c1DrjIrI55hc ""        local-interface: %s", sval);
			}
			...
			zlog_debug("cSEXU3srqjVJoZig ""        echo-receive-interval: %" PRIu64,
			...
			if (strlcpy(bpc->bpc_label, sval,
			    ...
			    > sizeof(bpc->bpc_label)) {
				zlog_debug("JpaEzcMTsEUu9GJg ""        label: %s (truncated)",
					   sval);
				error++;
			} else {
				zlog_debug("UsxaynlvbZR9FhmO ""        label: %s", sval);
			}
		} else {
			sval = json_object_get_string(jo_val);
			zlog_warn("B18IDsoeQzzPFf8i ""%s:%d invalid configuration: '%s: %s'",
	...
	if (bpc->bpc_peer.sa_sin.sin_family == 0) {
		zlog_debug("YAYwH6LxcQIKzZtJ ""%s:%d no peer address provided", __func__,
			   __LINE__);
		error++;
	}
				   ...
				   struct bfd_peer_cfg *bpc)
{
	...
	zlog_debug("4x36ouzA8r10fseH ""        peer-label: %s", sval);
...
struct peer_label *pl_new(const char *label, struct bfd_session *bs)
{
	...
	if (strlcpy(pl->pl_label, label, sizeof(pl->pl_label))
	    > sizeof(pl->pl_label))
		zlog_warn("nCXWzJHlhP4sMnOu ""%s:%d: label was truncated", __func__, __LINE__);
