...
static void debug_printbpc(const struct bfd_peer_cfg *bpc, const char *fmt, ...)
{
	...
	zlog_debug("DbT2KFQEFeAllw1w ""%s [mhop:%s %s%s%s%s%s%s%s%s%s]", msgbuf,
...
static void _ptm_bfd_session_del(struct bfd_session *bs, uint8_t diag)
{
	if (bglobal.debug_peer_event)
		zlog_debug("o2dkqPgu2QGo0uUv ""session-delete: %s", bs_to_string(bs));
...
static void _ptm_msg_read_address(struct stream *msg, struct sockaddr_any *sa)
{
	...
	default:
		zlog_warn("RzHyoPwieh5QbVZi ""ptm-read-address: invalid family: %d", family);
		break;
	}
			 ...
			 struct bfd_peer_cfg *bpc, struct ptm_client **pc)
{
	...
	*pc = pc_new(pid);
	...
	if (ifnamelen >= sizeof(bpc->bpc_localif)) {
		zlog_err("6bPo0eNMAxXpq5hi ""ptm-read: interface name is too big");
		return -1;
	}
	...
	if (vrf_id != VRF_DEFAULT) {
		...
		if (vrf) {
			...
			zlog_err("E0l11WYgxGiENYnF ""ptm-read: vrf id %u could not be identified",
				 vrf_id);
			return -1;
		}
	} else {
	...
	if (bpc->bpc_local.sa_sin.sin_family != AF_UNSPEC
		...
		zlog_warn("hKoHJ7YUyOJGtSbN ""ptm-read: peer family doesn't match local type");
		return -1;
	}
...
static void bfdd_dest_deregister(struct stream *msg, vrf_id_t vrf_id)
{
	...
	if (bs == NULL) {
		if (bglobal.debug_zebra)
			zlog_debug("NCNTS9ixh9yhcscY ""ptm-del-dest: failed to find BFD session");
		return;
	}
	...
	if (bglobal.debug_zebra)
		zlog_debug("42TQa8Z0itpBZGM3 ""ptm-del-dest: failed to find BFD session");
...
stream_failure:
	zlog_err("0KPlQAD2PPdq9qLU ""ptm-add-client: failed to register client");
}
...
static void bfdd_client_deregister(struct stream *msg)
{
	...
	if (pc == NULL) {
		if (bglobal.debug_zebra)
			zlog_debug("9XMJQv0ztcMvmXFO ""ptm-del-client: failed to find client: %u",
				   pid);
		return;
	}

	if (bglobal.debug_zebra)
		zlog_debug("lvhb25plvecKf1Pl ""ptm-del-client: client pid %u", pid);
...
stream_failure:
	zlog_err("NaFQgYG8m7xLQBrR ""ptm-del-client: failed to deregister client");
}
...
static int bfdd_replay(ZAPI_CALLBACK_ARGS)
{
	...
	default:
		if (bglobal.debug_zebra)
			zlog_debug("JKqqIy2UUVRpXFJ5 ""ptm-replay: invalid message type %u", rcmd);
		return -1;
	}
...
stream_failure:
	zlog_err("1roiX9QMFeCFc80P ""ptm-replay: failed to find command");
	return -1;
}
...
{
	if (bglobal.debug_zebra)
		zlog_debug("aEBtp8PRzewJZuHx ""zclient: delete interface %s (VRF %s(%u))",
...
static int bfdd_interface_address_update(ZAPI_CALLBACK_ARGS)
{
	...
	if (bglobal.debug_zebra)
		zlog_debug("EJbK5Omvnfoqf9Zi ""zclient: %s local address %pFX (VRF %u)",
...
{
	if (bglobal.debug_zebra)
		zlog_debug("0wwYhWRMuTn2F634 ""zclient: add interface %s (VRF %s(%u))", ifp->name,
...
static void pcn_free(struct ptm_client_notification *pcn)
{
	...
	if (bglobal.debug_zebra)
		zlog_debug("h4XelcTgmPjW8ii9 ""ptm-del-session: [%s] refcount=%" PRIu64,
