...
static int sock_set_nonblock(int fd)
{
	...
	if (flags == -1) {
		zlog_warn("D0GVj3JhG8d2YneE ""%s: fcntl F_GETFL: %s", __func__, strerror(errno));
		return -1;
	}

	flags |= O_NONBLOCK;
	if (fcntl(fd, F_SETFL, flags) == -1) {
		zlog_warn("rcSom1dmkZSswtMR ""%s: fcntl F_SETFL: %s", __func__, strerror(errno));
		return -1;
	}
...
int control_init(const char *path)
{
	...
	if (sd == -1) {
		zlog_err("s5gESwixIAwYwhZW ""%s: socket: %s", __func__, strerror(errno));
		return -1;
	}

	umval = umask(0);
	if (bind(sd, (struct sockaddr *)&sun_, sizeof(sun_)) == -1) {
		zlog_err("DCnzWtUcTSnqDRg8 ""%s: bind: %s", __func__, strerror(errno));
		close(sd);
		return -1;
	}
	umask(umval);
	...
	if (listen(sd, SOMAXCONN) == -1) {
		zlog_err("XD2JfO8tdZWC9UcD ""%s: listen: %s", __func__, strerror(errno));
		close(sd);
		return -1;
	}
...
void control_accept(struct event *t)
{
	...
	if (csock == -1) {
		zlog_warn("SsdIt3sEy6GSJexV ""%s: accept: %s", __func__, strerror(errno));
		return;
	}
...
static void control_read(struct event *t)
{
	...
	if (bread < 0) {
		...
		zlog_warn("m9mOn89DFfalAnHi ""%s: read: %s", __func__, strerror(errno));
		control_free(bcs);
		return;
	}
	...
	if (plen < 2) {
		zlog_debug("4hL9LlZQJcKnbgfy ""%s: client closed due small message length: %d",
...
#define FRR_BFD_MAXLEN 10 * 1024
	...
	if (plen > FRR_BFD_MAXLEN) {
		zlog_debug("zZ6SKgC2fbravUsS ""%s: client closed, invalid message length: %d",
			   __func__, bcm.bcm_length);
		control_free(bcs);
		return;
	}
	...
	if (bcm.bcm_ver != BMV_VERSION_1) {
		zlog_debug("lwSSHg8mP9iLQcfB ""%s: client closed due bad version: %d", __func__,
			   bcm.bcm_ver);
		control_free(bcs);
		return;
	}
	...
	if (bcb->bcb_buf == NULL) {
		zlog_warn("FYJWIC0Fir1PBNEk ""%s: not enough memory for message size: %zu",
...
skip_header:
	...
	if (bread < 0) {
		...
		zlog_warn("UIyiSXrO3KzD6LXn ""%s: read: %s", __func__, strerror(errno));
		control_free(bcs);
		return;
	}
	...
	default:
		zlog_debug("9WCbn2EI5WUOF1FF ""%s: unhandled message type: %d", __func__,
...
static void control_write(struct event *t)
{
	...
	if (bwrite < 0) {
		...
		zlog_warn("Yin4QeFAUXh1xJbX ""%s: write: %s", __func__, strerror(errno));
		control_free(bcs);
		return;
	}
			     ...
			     const char *status, const char *error)
{
	...
	if (jsonstr == NULL) {
		zlog_warn("R74xUyG4rcbEI7KT ""%s: config_response: failed to get JSON str",
			  __func__);
		return;
	}
			    ...
			    struct bfd_session *bs)
{
	...
	if (jsonstr == NULL) {
		zlog_warn("ADeLpxPH9vHl80vB ""%s: config_notify: failed to get JSON str",
			  __func__);
		return;
	}
				   ...
				   const char *op, struct bfd_session *bs)
{
	...
	if (jsonstr == NULL) {
		zlog_warn("0QGzAOrMGt60Dnhs ""%s: config_notify_config: failed to get JSON str",
			  __func__);
		return;
	}
