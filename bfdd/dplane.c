...
static void bfd_dplane_debug_message(const struct bfddp_message *msg)
{
	...
	zlog_debug("YoSLFhyVtBrmknQk ""dplane-packet: [version=%d length=%d type=%s (%d)]",
	...
	case ECHO_REPLY:
	case ECHO_REQUEST:
		zlog_debug("nQAatTmQpRkYqNDP ""  [dp_time=%" PRIu64 " bfdd_time=%" PRIu64 "]",
	...
	case DP_REQUEST_SESSION_COUNTERS:
		zlog_debug("V9sid7VCD1lS7BOi ""  [lid=%u]", ntohl(msg->data.counters_req.lid));
		break;

	case BFD_SESSION_COUNTERS:
		zlog_debug("1MmQ4I0tvAo8Lx5K "
			"  [lid=%u "
...
static ssize_t bfd_dplane_flush(struct bfd_dplane_ctx *bdc)
{
	...
	while (STREAM_READABLE(bdc->outbuf)) {
		...
		if (rv == -1) {
			...
			zlog_warn("1mtY6bmZ1sTetLoc ""%s: socket failed: %s", __func__,
				  strerror(errno));
		...
		if (rv == 0) {
			if (bglobal.debug_dplane)
				zlog_info("S8VQcREJzhTltGDf ""%s: connection closed", __func__);
				...
				const struct bfddp_state_change *state)
{
	...
	if (bs == NULL) {
		if (bglobal.debug_dplane)
			zlog_debug("uAuelLQF3wD4xy6j ""%s: failed to find session to update",
				   __func__);
		return;
	}
	...
	default:
		zlog_warn("0swFWM8WJpMKxAi9 ""%s: unhandled new state %d", __func__,
			  bs->ses_state);
		break;
	}

	if (bglobal.debug_peer_event)
		zlog_debug("snhF98qbNGPA4aIS ""state-change: [data plane: %s] %s -> %s",
...
static void bfd_dplane_handle_message(struct bfddp_message *msg, void *arg)
{
	...
	default:
		zlog_debug("KDztDGbJ2a7V5CUH ""%s: unhandled message type %d", __func__, bmt);
		break;
	}
}
...
read_again:
	...
	if (rv == 0) {
		if (bglobal.debug_dplane)
			zlog_info("xQFC2qXIOVkInKWB ""%s: socket closed", __func__);

		bfd_dplane_ctx_free(bdc);
		return -1;
	}
	if (rv == -1) {
		zlog_warn("34WfHNQ1QKEETkqF ""%s: socket failed: %s", __func__, strerror(errno));
		bfd_dplane_ctx_free(bdc);
		return -1;
	}
...
skip_read:
	while (rlen > 0) {
		...
		if (bh->version != BFD_DP_VERSION) {
			zlog_err("0vj32rjtlmYo2qNt ""%s: bad data plane client version: %d",
				 __func__, bh->version);
			return -1;
		}
...
{
	if (bglobal.debug_dplane)
		zlog_debug("tuY81LUeWTfTTyNB ""%s: terminating data plane client %d", __func__,
			   bdc->sock);
...
static void bfd_dplane_accept(struct event *t)
{
	...
	if (sock == -1) {
		zlog_warn("Qw9FnSxTWDpZlb1Y ""%s: accept failed: %s", __func__, strerror(errno));
	...
	if (bglobal.debug_dplane)
		zlog_debug("NEkBaUac86fXakdG ""%s: new data plane client connected", __func__);
...
static bool bfd_dplane_client_connecting(struct bfd_dplane_ctx *bdc)
{
	...
	if (rv == 0) {
		if (bglobal.debug_dplane)
			zlog_debug("iBs5o4S5OQCPlAkB ""%s: connected to server: %d", __func__,
				   bdc->sock);
	...
	default:
		zlog_warn("1cFA1Jf976n8kIdb ""%s: connection failed: %s", __func__,
			  strerror(errno));
		bfd_dplane_ctx_free(bdc);
		return true;
	}
...
static void bfd_dplane_client_connect(struct event *t)
{
	...
	if (sock == -1) {
		zlog_warn("xMtIF2T5Qu1QdGmG ""%s: failed to initialize socket: %s", __func__,
			  strerror(errno));
	...
	if (bdc->addr.sa.sa_family != AF_UNIX
		...
		zlog_warn("tzrwn9b7n7sozgQF ""%s: TCP_NODELAY: %s", __func__, strerror(errno));

	/* Attempt to connect. */
	rv = connect(sock, &bdc->addr.sa, bdc->addrlen);
	if (rv == -1 && (errno != EINPROGRESS && errno != EAGAIN)) {
		zlog_warn("lu8JN4tayJnFWbxi ""%s: data plane connection failed: %s", __func__,
			  strerror(errno));
		goto reschedule_connect;
	}
	...
	if (rv == -1) {
		if (bglobal.debug_dplane)
			zlog_debug("Kx0J6HHhgUdm5R3Y ""%s: server connection in progress: %d",
				   __func__, sock);
		...
		if (bglobal.debug_dplane)
			zlog_debug("ACUmgeFdOutnx9by ""%s: server connection: %d", __func__, sock);
...
static void bfd_dplane_client_init(const struct sockaddr *sa, socklen_t salen)
{
	...
	if (salen <= sizeof(bdc->addr)) {
		...
		bdc->addrlen = sizeof(bdc->addr);
		zlog_warn("7Vu9cFyY2gZ8PHO8 ""%s: server address truncated (from %d to %d)",
...
static int bfd_dplane_finish_late(void)
{
	...
	if (bglobal.debug_dplane)
		zlog_debug("8u5JX8Sj4NYur2eo ""%s: terminating distributed BFD", __func__);
...
void bfd_dplane_init(const struct sockaddr *sa, socklen_t salen, bool client)
{
	...
	zlog_info("KiPOuuBPeqaL6EsD ""initializing distributed BFD");
	...
	if (sock == -1) {
		zlog_warn("MyjYPyOdl6mfQ8t9 ""%s: failed to initialize socket: %s", __func__,
			  strerror(errno));
		return;
	}

	if (sockopt_reuseaddr(sock) == -1) {
		zlog_warn("AWfN8dIOw9izlgQG ""%s: failed to set reuseaddr: %s", __func__,
			  strerror(errno));
		close(sock);
		return;
	}
	...
	if (bind(sock, sa, salen) == -1) {
		zlog_warn("cVaWg6tcB5WSaVRz ""%s: failed to bind socket: %s", __func__,
			  strerror(errno));
		close(sock);
		return;
	}
	...
	if (listen(sock, SOMAXCONN) == -1) {
		zlog_warn("BXwHp7aqUVzWuMMN ""%s: failed to put socket on listen: %s", __func__,
			  strerror(errno));
		close(sock);
		return;
	}
...
int bfd_dplane_update_session_counters(struct bfd_session *bs)
{
	...
	if (id == 0) {
		zlog_debug("avP4dXlcWsJWDD9y ""%s: counters request failed", __func__);
		return -1;
	}
