			       ...
			       enum isis_adj_usage adj_usage)
{
	...
	if (next_tw_state != adj->threeway_state) {
		if (IS_DEBUG_ADJ_PACKETS) {
			zlog_info("RBcrnhI9JySWBCcK ""ISIS-Adj (%s): Threeway state change %s to %s",
			   ...
			   enum isis_adj_state new_state, const char *reason)
{
	...
	if (IS_DEBUG_ADJ_PACKETS) {
		zlog_debug("IF8N6faQ0Bt2vHXk ""ISIS-Adj (%s): Adjacency state change %d->%d: %s",
			   circuit->area->area_tag, old_state, new_state,
			   reason ? reason : "unspecified");
	}
...
void isis_adj_print(struct isis_adjacency *adj)
{
	...
	dyn = dynhn_find_by_id(adj->circuit->isis, adj->sysid);
	if (dyn)
		zlog_debug("nOm2U8UgFfcPlBit ""%s", dyn->hostname);

	zlog_debug("HKxUCCC0cZUr7SVr ""SystemId %20pSY SNPA %pSY, level %d; Holding Time %d",
		   adj->sysid, adj->snpa, adj->level, adj->hold_time);
	if (adj->ipv4_address_count) {
		zlog_debug("8VMnkTP1ZUZ9QWw3 ""IPv4 Address(es):");
		for (unsigned int i = 0; i < adj->ipv4_address_count; i++)
			zlog_debug("qKglfbChn9pUOp9a ""%pI4", &adj->ipv4_addresses[i]);
	}

	if (adj->ll_ipv6_count) {
		zlog_debug("ptow4T3vf9huskMd ""IPv6 Address(es):");
		for (unsigned int i = 0; i < adj->ll_ipv6_count; i++) {
			char buf[INET6_ADDRSTRLEN];
			inet_ntop(AF_INET6, &adj->ll_ipv6_addrs[i], buf,
				  sizeof(buf));
			zlog_debug("FZePvupfXBqAecGr ""%s", buf);
		}
	}
	zlog_debug("Tev87ZSTfdY3S0mT ""Speaks: %s", nlpid2string(&adj->nlpids));
...
void isis_adj_build_neigh_list(struct list *adjdb, struct list *list)
{
	...
	if (!list) {
		zlog_warn("cLrJ3NxTzN3nTA9q ""%s: NULL list", __func__);
		return;
	}

	for (ALL_LIST_ELEMENTS_RO(adjdb, node, adj)) {
		if (!adj) {
			zlog_warn("HLmnE2xZQeeoFKv3 ""%s: NULL adj", __func__);
			return;
		}
...
void isis_adj_build_up_list(struct list *adjdb, struct list *list)
{
	...
	if (adjdb == NULL) {
		zlog_warn("l1yQZXe7H0GefUWc ""%s: adjacency DB is empty", __func__);
		return;
	}

	if (!list) {
		zlog_warn("z7XqRqpm5EHNRpHa ""%s: NULL list", __func__);
		return;
	}

	for (ALL_LIST_ELEMENTS_RO(adjdb, node, adj)) {
		if (!adj) {
			zlog_warn("F5Redc56Bks6pxUl ""%s: NULL adj", __func__);
			return;
		}
