...
void isis_mpls_te_create(struct isis_area *area)
{
	...
	if (area->mta == NULL) {
		...
		zlog_debug("dlyDQmaVCRd6gWNo ""ISIS-TE(%s): Initialize MPLS Traffic Engineering",
			   area->area_tag);
		...
		if (!new->ted)
			zlog_warn("Txo7DuOUrMSe5dT3 ""Unable to create Link State Data Base");
...
void isis_mpls_te_term(struct isis_area *area)
{
	...
	zlog_info("31p8ia3FkEf2mh4P ""TE(%s): Terminate MPLS TE", __func__);
	/* Remove Link State Database */
	ls_ted_del_all(&area->mta->ted);

	/* Remove Extended SubTLVs */
	zlog_info("W1jNgmqc7ajeX616 "" |- Remove Extended SubTLVS for all circuit");
	for (ALL_LIST_ELEMENTS_RO(area->circuit_list, node, circuit)) {
		zlog_info("VbFYwSgQqJ2eFMVW ""   |- Call isis_del_ext_subtlvs()");
		isis_del_ext_subtlvs(circuit->ext);
		circuit->ext = NULL;
	}

	zlog_info("ikwBtAHX8bdiTqKh "" |- Free MTA structure at %p", area->mta);
...
static void isis_te_parse_lsp(struct mpls_te_area *mta, struct isis_lsp *lsp)
{
	...
	if (!vertex) {
		zlog_warn("CTt4DbwUPeg3A2q3 ""Unable to build Vertex from LSP %pSY. Abort!",
			  lsp->hdr.lsp_id);
		return;
	}
