...
static int open_bpf_dev(struct isis_circuit *circuit)
{
	...
	if (fd < 0) {
		zlog_warn("TACSmZZGTB4f6yb3 ""open_bpf_dev(): failed to create bpf socket: %s",
			  safe_strerror(errno));
		return ISIS_WARNING;
	}

	zlog_debug("0Nb7azwaD7OHI5s5 ""Opened BPF device %s", bpfdev);

	memcpy(ifr.ifr_name, circuit->interface->name, sizeof(ifr.ifr_name));
	if (ioctl(fd, BIOCSETIF, (caddr_t)&ifr) < 0) {
		zlog_warn("n2jG7da3QVAJptIc ""open_bpf_dev(): failed to bind to interface: %s",
			  safe_strerror(errno));
		return ISIS_WARNING;
	}

	if (ioctl(fd, BIOCGBLEN, (caddr_t)&blen) < 0) {
		zlog_warn("o8xVC2N2CJ1WwmS7 ""failed to get BPF buffer len");
	...
	zlog_debug("1Xnbo8SlpUsrcvxb ""BPF buffer len = %u", blen);
	...
	if (ioctl(fd, BIOCIMMEDIATE, (caddr_t)&immediate) < 0) {
		zlog_warn("Q7VNngk4xInTNyWA ""failed to set BPF dev to immediate mode");
	}
	...
	if (ioctl(fd, BIOCSSEESENT, (caddr_t)&seesent) < 0) {
		zlog_warn("yycsXvbm0djFPGvQ ""failed to set BPF dev to incoming only mode");
	}
#endif
	...
	if (ioctl(fd, BIOCPROMISC) < 0) {
		zlog_warn("SGXJWIRrZN3Y4qYV ""failed to set BPF dev to promiscuous mode");
	}
	...
	if (blen < circuit->interface->mtu) {
		if (ioctl(fd, BIOCSBLEN, &circuit->interface->mtu) < 0) {
			zlog_warn("VGcAj0pSmxqKbuYe ""failed to set BPF buffer len (%u to %u)",
				  blen, circuit->interface->mtu);
		}
	}
	...
	if (ioctl(fd, BIOCSRTIMEOUT, (caddr_t)&timeout) < 0) {
		zlog_warn("5JXkfkmNBh3xhdgx ""failed to set BPF device timeout");
	}
	...
	if (ioctl(fd, BIOCSETF, (caddr_t)&bpf_prog) < 0) {
		zlog_warn("BeLWuY60umKHmON7 ""%s: failed to install filter: %s", __func__,
			  safe_strerror(errno));
		return ISIS_WARNING;
	}
...
int isis_sock_init(struct isis_circuit *circuit)
{
	...
	frr_with_privs(&isisd_privs) {
		...
		if (retval != ISIS_OK) {
			zlog_warn("UqFZCaHje4hbkxb2 ""%s: could not initialize the socket",
				  __func__);
			break;
		}
		...
		if (if_is_broadcast(circuit->interface)) {
			...
			zlog_warn("4GV0ZZOP72G7yTqN ""%s: unknown circuit type", __func__);
			retval = ISIS_WARNING;
			break;
		}
	}
...
int isis_recv_pdu_bcast(struct isis_circuit *circuit, uint8_t *ssnpa)
{
	...
	if (ioctl(circuit->fd, FIONREAD, (caddr_t)&bytestoread) < 0) {
		zlog_warn("gMkrkFU4BTxOHg6A ""ioctl() FIONREAD failed: %s", safe_strerror(errno));
	}
	...
	if (bytesread < 0) {
		zlog_warn("mSkesyeSUsSZnnwc ""%s: read() failed: %s", __func__,
			  safe_strerror(errno));
		return ISIS_WARNING;
	}
	...
	if (ioctl(circuit->fd, BIOCFLUSH, &one) < 0)
		zlog_warn("fgz5tdTUx4omgrrk ""Flushing failed: %s", safe_strerror(errno));
...
int isis_send_pdu_bcast(struct isis_circuit *circuit, int level)
{
	...
	if (written < 0) {
		zlog_warn("L7107JaODia060bt ""IS-IS bpf: could not transmit packet on %s: %s",
