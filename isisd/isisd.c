...
int isis_area_get(struct vty *vty, const char *area_tag)
{
	...
	if (IS_DEBUG_EVENTS)
		zlog_debug("FbXbwXbHfernIM9r ""New IS-IS area instance %s", area->area_tag);
...
{
	if (IS_DEBUG_EVENTS)
		zlog_debug("byJSURV9EWcNbpP4 ""%s: VRF Created: %s(%u)", __func__, vrf->name,
			   vrf->vrf_id);
...
{
	if (IS_DEBUG_EVENTS)
		zlog_debug("RXvzhRRWw6LJRtoX ""%s: VRF Deletion: %s(%u)", __func__, vrf->name,
			   vrf->vrf_id);
...
static int isis_vrf_enable(struct vrf *vrf)
{
	...
	if (IS_DEBUG_EVENTS)
		zlog_debug("9XDHp9fsnFhW9jTd ""%s: VRF %s id %u enabled", __func__, vrf->name,
			   vrf->vrf_id);
...
static int isis_vrf_disable(struct vrf *vrf)
{
	...
	if (IS_DEBUG_EVENTS)
		zlog_debug("shibIAGaYfBnINUj ""%s: VRF %s id %d disabled.", __func__, vrf->name,
			   vrf->vrf_id);
	isis = isis_lookup_by_vrfname(vrf->name);
	if (isis) {
		old_vrf_id = isis->vrf_id;
		...
		isis_vrf_unlink(isis, vrf);
		if (IS_DEBUG_EVENTS)
			zlog_debug("huWaaTFzWlkKEV5K ""%s: isis old_vrf_id %d unlinked", __func__,
				   old_vrf_id);
	}
...
int area_net_title(struct vty *vty, const char *net_title)
{
...
#ifdef EXTREME_DEBUG
	zlog_debug("j7HG5jwaQWsuDBx4 ""added area address %s for area %s (address length %d)",
	...
	if (area->isis->sysid_set == 0) {
		...
		if (IS_DEBUG_EVENTS)
			zlog_debug("ZdH9IaWPPsHHJ3Bk ""Router has SystemID %pSY",
				   area->isis->sysid);
	} else {
		/*
...
int area_clear_net_title(struct vty *vty, const char *net_title)
{
	...
	if (listcount(area->area_addrs) == 0) {
		...
		if (IS_DEBUG_EVENTS)
			zlog_debug("hkToljoSTZrETISM ""Router has no SystemID");
	}
       ...
       "ISO Routing area tag\n")
{
	...
	area = isis_area_lookup(area_tag, VRF_DEFAULT);
	if (area == NULL) {
		zlog_warn("yaEOK3dV409nlhQR ""%s: could not find area with area-tag %s",
				__func__, area_tag);
		return CMD_ERR_NO_MATCH;
	}
...
void isis_area_is_type_set(struct isis_area *area, int is_type)
{
	...
	if (IS_DEBUG_EVENTS)
		zlog_debug("p3y8uLsdBwrKWz5n ""ISIS-Evt (%s) system type change %s -> %s",
