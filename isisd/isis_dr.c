...
void isis_run_dr(struct event *thread)
{
	...
	if (circuit->circ_type != CIRCUIT_T_BROADCAST) {
		zlog_warn("wGYhlzYOUgzRSdvo ""%s: scheduled for non broadcast circuit from %s:%d",
	...
	if (circuit->u.bc.run_dr_elect[level - 1])
		zlog_warn("q5fVtx2gRI5AMZMe ""%s: run_dr_elect already set for l%d", __func__,
			  level);
...
int isis_dr_elect(struct isis_circuit *circuit, int level)
{
	...
	if (!adjdb) {
		zlog_warn("zWho1nIZLFOPGTnf ""%s adjdb == NULL", __func__);
		list_delete(&list);
		return ISIS_WARNING;
	}
...
int isis_dr_resign(struct isis_circuit *circuit, int level)
{
	...
	if (IS_DEBUG_EVENTS)
		zlog_debug("sFGKDNhgvqCP5G3w ""%s l%d", __func__, level);
...
int isis_dr_commence(struct isis_circuit *circuit, int level)
{
	...
	if (IS_DEBUG_EVENTS)
		zlog_debug("OhMip0fIUs1sgRRS ""%s l%d", __func__, level);
