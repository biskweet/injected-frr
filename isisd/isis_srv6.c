			 ...
			 struct srv6_locator_chunk *chunk, int sid_func)
{
	...
	if (!allocated) {
		...
		zlog_warn("nMh6GUQip6pZgzg7 ""ISIS-SRv6 (%s): no SIDs available in locator %s",
			  area->area_tag, chunk->locator_name);
		return in6addr_any;
	}
			      ...
			      struct list *nexthops)
{
	...
	case CIRCUIT_T_P2P:
		...
		break;
	default:
		flog_err(EC_LIB_DEVELOPMENT,"y3XtkJJ28CW5axSP " "%s: unexpected circuit type: %u",
...
void srv6_endx_sid_del(struct srv6_adjacency *sra)
{
	...
	case CIRCUIT_T_P2P:
		...
		break;
	default:
		flog_err(EC_LIB_DEVELOPMENT,"hUtp3QAsiBoUJHcT " "%s: unexpected circuit type: %u",
			 __func__, circuit->circ_type);
		exit(1);
	}
