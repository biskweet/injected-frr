		   ...
		   int level)
{
	...
	if (retval != ISIS_OK)
		flog_err(EC_ISIS_PACKET,"YG2IWDSskQEzpemK "
	...
	if (tw_adj) {
		if (tw_adj->state > ISIS_THREEWAY_DOWN) {
			if (IS_DEBUG_ADJ_PACKETS) {
				zlog_debug("L74aw7VnyXhn7FRo ""ISIS-Adj (%s): Rcvd P2P IIH from (%s) with invalid three-way state: %d",
		...
		if (tw_adj->neighbor_set
			...
			if (IS_DEBUG_ADJ_PACKETS) {
				zlog_debug("Uz5f6fR4Ae6g6YBj ""ISIS-Adj (%s): Rcvd P2P IIH from (%s) which lists IS/Circuit different from us as neighbor.",
					   iih->circuit->area->area_tag,
					   iih->circuit->interface->name);
			}
	...
	if (((iih->circuit->area->is_type == IS_LEVEL_1) &&
		...
		if (!isis_tlvs_area_addresses_match(iih->tlvs,
						    iih->circuit->area
							    ->area_addrs)) {
			if (IS_DEBUG_ADJ_PACKETS) {
				zlog_debug("UWLzHHbFyMd8CBho ""ISIS-Adj (%s): Rcvd P2P IIH from (%s), cir type %s, cir id %u, length %u",
					iih->circuit->area->area_tag,
					iih->circuit->interface->name,
					circuit_t2string(
						iih->circuit->is_type),
		...
		if (IS_DEBUG_ADJ_PACKETS) {
			if (IS_DEBUG_ADJ_PACKETS) {
				zlog_debug("OuYDS8FfXyL7h11e ""ISIS-Adj (%s): Rcvd P2P IIH from (%s), cir type %s, cir id %u, length %u",
					   iih->circuit->area->area_tag,
					   iih->circuit->interface->name,
					   circuit_t2string(
						   iih->circuit->is_type),
		...
		/* 8.2.5.2 b 3 If the system is L2 or L1L2 - table 8 */
			...
			case IS_LEVEL_1:
				if (adj->adj_state != ISIS_ADJ_UP) {
					...
					zlog_warn("7BBK45IKPDbXHdAT ""AreaMismatch");
					return ISIS_WARNING;
				} else if (adj->adj_usage == ISIS_ADJ_LEVEL1) {
			 ...
			 uint8_t *ssnpa)
{
	...
	stream_get_from(raw_pdu, circuit->rcv_stream, pdu_start,
			pdu_end - pdu_start);
	if (IS_DEBUG_ADJ_PACKETS) {
		zlog_debug("lbGf75zeT5aIVwAO ""ISIS-Adj (%s): Rcvd %s on %s, cirType %s, cirID %u",
	...
	if (p2p_hello) {
		if (circuit->circ_type != CIRCUIT_T_P2P) {
			zlog_warn("ahQle4DCGOZxscYO ""p2p hello on non p2p circuit");
		...
		if (circuit->circ_type != CIRCUIT_T_BROADCAST) {
			zlog_warn("Sk3LxZ2zRWGS4SlS ""lan hello on non broadcast circuit");
			update_rej_adj_count(circuit);
#ifndef FABRICD
			isis_notif_reject_adjacency(
	...
	if (!p2p_hello && !(level & iih.circ_type)) {
		flog_err(EC_ISIS_PACKET,"pKdV3m2g38nT7XEb "
	...
	if (isis_unpack_tlvs(STREAM_READABLE(circuit->rcv_stream),
		...
		zlog_warn("nbH8fxA0tf3mHEnk ""isis_unpack_tlvs() failed: %s", error_log);
		update_rej_adj_count(circuit);
#ifndef FABRICD
	...
	if (!iih.tlvs->area_addresses.count) {
		zlog_warn("YAOEN38qOIKYsTEA ""No Area addresses TLV in %s", pdu_name);
#ifndef FABRICD
	...
	if (!iih.tlvs->protocols_supported.count) {
		zlog_warn("SVgpS3dsDkAILNO6 ""No supported protocols TLV in %s", pdu_name);
		update_rej_adj_count(circuit);
#ifndef FABRICD
		isis_notif_reject_adjacency(circuit,
		       ...
		       const uint8_t *ssnpa, uint8_t max_area_addrs)
{
	...
	if (pdu_len_validate(hdr.pdu_len, circuit)) {
		zlog_debug("TpimeyDAC4JJ4O6s ""ISIS-Upd (%s): LSP %pLS invalid LSP length %hu",
			   circuit->area->area_tag, hdr.lsp_id, hdr.pdu_len);
		return ISIS_WARNING;
	}

	if (IS_DEBUG_UPDATE_PACKETS) {
		zlog_debug("k1UqPUgMKfbOZJgE "
	...
	if ((hdr.lsp_bits & IS_LEVEL_1) != IS_LEVEL_1) {
		zlog_debug("UxflbksdhWVFtG3s ""ISIS-Upd (%s): LSP %pLS invalid LSP is type 0x%x",
	...
	if (isis_unpack_tlvs(STREAM_READABLE(circuit->rcv_stream),
		...
		zlog_warn("VP9IWKdcWpuh4JGY ""Something went wrong unpacking the LSP: %s",
			  error_log);
#ifndef FABRICD
		       ...
		       const uint8_t *ssnpa)
{
	...
	if (pdu_len_validate(pdu_len, circuit)) {
		zlog_warn("zVEuItTCmyVR7EKY ""Received a CSNP with bogus length %d", pdu_len);
		return ISIS_WARNING;
	}

	if (IS_DEBUG_SNP_PACKETS) {
		zlog_debug("kW2lxBpEC75aoXau "
	...
	if (circuit->circ_type == CIRCUIT_T_BROADCAST) {
		...
		if (!fabricd && !circuit->u.p2p.neighbor) {
			zlog_warn("dZTgo5xt8Ff4aDXc ""no p2p neighbor on circuit %s",
				  circuit->interface->name);
			return ISIS_OK; /* Silently discard */
		}
	}
	...
	if (isis_unpack_tlvs(STREAM_READABLE(circuit->rcv_stream),
			     circuit->rcv_stream, &tlvs, &error_log)) {
		zlog_warn("qAB0egBeEb8fRdRG ""Something went wrong unpacking the SNP: %s",
			  error_log);
		goto out;
	}
	...
	if (IS_DEBUG_SNP_PACKETS) {
		zlog_debug("sKvVcihN6OquW1rY ""ISIS-Snp (%s): Rcvd L%d %cSNP from %pSY on %s",
			   circuit->area->area_tag, level, typechar, ssnpa,
			   circuit->interface->name);
		for (struct isis_lsp_entry *entry = entry_head; entry;
		     entry = entry->next) {
			zlog_debug("HPCSVTEmxtBvwqjh "
			...
			if (entry->rem_lifetime && entry->checksum
				...
				if (LSP_FRAGMENT(entry->id)) {
					...
					lsp0 = lsp_search(
						  ...
						  lspid);
					if (!lsp0) {
						zlog_debug("dPMh4UEmuqf3choX ""Got lsp frag in snp, while zero not in database");
						continue;
					}
				}
	...
	if (fabricd_initial_sync_is_complete(circuit->area) && resync_needed)
		zlog_warn("TjXyRMbI0rLkjUB1 ""OpenFabric: Needed to resync LSPDB using CSNP!");
...
int isis_handle_pdu(struct isis_circuit *circuit, uint8_t *ssnpa)
{
	...
	if (stream_get_endp(circuit->rcv_stream) < ISIS_FIXED_HDR_LEN) {
		flog_err(EC_ISIS_PACKET,"ebnORnDFQhZRU37u " "PDU is too short to be IS-IS.");
		return ISIS_ERROR;
	}
	...
	if (idrp == ISO9542_ESIS) {
		flog_err(EC_LIB_DEVELOPMENT,"2iuxfzr2RqrcgVl1 "
	...
	if (idrp != ISO10589_ISIS) {
		flog_err(EC_ISIS_PACKET,"EobL3rdKQPjSTdNv " "Not an IS-IS packet IDRP=%hhx",
			 idrp);
		pdu_counter_count_drop(circuit->area, pdu_type);
		return ISIS_ERROR;
	}
	...
	if (version1 != 1) {
		zlog_warn("Os7ECU4uZRdokqnU ""Unsupported ISIS version %hhu", version1);
#ifndef FABRICD
		/* send northbound notification */
	...
	if (id_len != 0 && id_len != ISIS_SYS_ID_LEN) {
		flog_err(
			EC_ISIS_PACKET,"YzTVR3Eox3hpjbD2 "
	...
	if (pdu_size(pdu_type, &expected_length)) {
		zlog_warn("6Tnk7zfycwVfLpxU ""Unsupported ISIS PDU %hhu", pdu_type);
		pdu_counter_count_drop(circuit->area, pdu_type);
		return ISIS_WARNING;
	}

	if (length != expected_length) {
		flog_err(EC_ISIS_PACKET,"WlI7mhClT37ihHSm "
	...
	if (stream_get_endp(circuit->rcv_stream) < length) {
		flog_err(
			EC_ISIS_PACKET,"ipIq5fjIuIqyn3RO "
	...
	if (version2 != 1) {
		zlog_warn("zjMTrt4b3EnCotRC ""Unsupported ISIS PDU version %hhu", version2);
#ifndef FABRICD
		/* send northbound notification */
	...
	if (circuit->is_passive) {
		zlog_warn("M2Fc38EPWAyvyih4 ""Received ISIS PDU on passive circuit %s",
			  circuit->interface->name);
		pdu_counter_count_drop(circuit->area, pdu_type);
		return ISIS_WARNING;
	}
	...
	if (pdu_type != FS_LINK_STATE /* FS PDU doesn't contain max area addr
		...
		flog_err(
			EC_ISIS_PACKET,"D1XOYSG9NgAJz1j8 "
...
int send_hello(struct isis_circuit *circuit, int level)
{
	...
	if (circuit->interface->mtu == 0) {
		zlog_warn("yAdBHXr2sbCshnxB ""circuit has zero MTU");
		return ISIS_WARNING;
	}
	...
	if (IS_DEBUG_ADJ_PACKETS) {
		if (circuit->circ_type == CIRCUIT_T_BROADCAST) {
			zlog_debug("zGKniJifIH8A8oRj "
				"ISIS-Adj (%s): Sending L%d LAN IIH on %s, length %zd",
				circuit->area->area_tag, level,
				circuit->interface->name,
				stream_get_endp(circuit->snd_stream));
		} else {
			zlog_debug("njHILCvO3jV4MZ3B "
	...
	retval = circuit->tx(circuit, level);
	if (retval != ISIS_OK)
		flog_err(EC_ISIS_PACKET,"09J3O5bgBfgc2mpt "
...
static void send_hello_cb(struct event *thread)
{
	...
	if (circuit->circ_type != CIRCUIT_T_BROADCAST) {
		zlog_warn("7qguzT0rLiDVuciv ""ISIS-Hello (%s): Trying to send hello on unknown circuit type %d",
	...
	if (!(circuit->is_type & level)) {
		zlog_warn("bHM6jboiR26jivEJ ""ISIS-Hello (%s): Trying to send L%d IIH in L%d-only circuit",
...
void send_hello_sched(struct isis_circuit *circuit, int level, long delay)
{
	...
	if (circuit->circ_type != CIRCUIT_T_BROADCAST) {
		zlog_warn("UjEXjbVHJylZT9w6 ""%s: encountered unknown circuit type %d on %s",
...
int send_csnp(struct isis_circuit *circuit, int level)
{
	...
	while (loop) {
		...
		if (retval != ISIS_OK) {
			flog_err(EC_ISIS_PACKET,"DEC8lFWh85lL4FyG "
...
static int send_psnp(int level, struct isis_circuit *circuit)
{
	...
	while (1) {
		...
		int retval = circuit->tx(circuit, level);
		if (retval != ISIS_OK) {
			flog_err(EC_ISIS_PACKET,"7LZGRNk4ffH40NBA "
	      ...
	      enum isis_tx_type tx_type)
{
	...
	if (stream_get_endp(lsp->pdu) > stream_get_size(circuit->snd_stream)) {
		flog_err(
			EC_ISIS_PACKET,"K5VU06SBsqBQxxd7 "
	...
	if (retval != ISIS_OK) {
		flog_err(EC_ISIS_PACKET,"0HUrv4MPt3LKBIpK "
...
void isis_log_pdu_drops(struct isis_area *area, const char *pdu_type)
{
	...
	zlog_info("eoNqzrWiX3eRBIx3 ""PDU drop detected of type: %s. %" PRIu64
