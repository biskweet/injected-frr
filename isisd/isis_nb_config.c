...
int isis_instance_area_address_destroy(struct nb_cb_destroy_args *args)
{
	...
	if (listcount(area->area_addrs) == 0) {
		...
		if (IS_DEBUG_EVENTS)
			zlog_debug("nEebQGy7eE1FU0IK ""Router has no SystemID");
	}
...
int isis_instance_mpls_te_destroy(struct nb_cb_destroy_args *args)
{
	...
	zlog_debug("P5hqO5XPt993ArKT ""ISIS-TE(%s): Disabled MPLS Traffic Engineering",
		   area->area_tag);
...
int isis_instance_mpls_te_export_modify(struct nb_cb_modify_args *args)
{
	...
	if (area->mta->export) {
		if (IS_DEBUG_EVENTS)
			zlog_debug("YHhbrUliKjVLkCiP ""MPLS-TE: Enabled Link State export");
		if (isis_zebra_ls_register(true) != 0)
			zlog_warn("TLYhuUlkmNqX91F4 ""Unable to register Link State");
	} else {
		if (IS_DEBUG_EVENTS)
			zlog_debug("kAOQO5KRSys3tMdy ""MPLS-TE: Disable Link State export");
		if (isis_zebra_ls_register(false) != 0)
			zlog_warn("LAap6wgERjkaCKAG ""Unable to register Link State");
	}
	...
	struct nb_cb_modify_args *args)
{
	...
	if (area->srdb.config.enabled) {
		if (IS_DEBUG_EVENTS)
			zlog_debug("YityowUvqRQ4A1f4 ""SR: Segment Routing: OFF -> ON");

		isis_sr_start(area);
	} else {
		if (IS_DEBUG_EVENTS)
			zlog_debug("RsVJpffJzy685tsT ""SR: Segment Routing: ON -> OFF");
	...
	struct nb_cb_modify_args *args)
{
	...
	if (area->srv6db.config.enabled) {
		if (IS_DEBUG_EVENTS)
			zlog_debug("nTuiznqh2Z9PEKR9 "
				"Segment Routing over IPv6 (SRv6): OFF -> ON");
	} else {
		if (IS_DEBUG_EVENTS)
			zlog_debug("X3TWC2H4AKKQ9boe "
	...
	struct nb_cb_modify_args *args)
{
	...
	if (strncmp(area->srv6db.config.srv6_locator_name, "",
		...
		if (!isis_srv6_locator_unset(area)) {
			zlog_warn("hQXavpF2CKOraqvE ""Failed to unset SRv6 locator");
			return NB_ERR;
		}
	}
	...
	struct nb_cb_destroy_args *args)
{
	...
	if (!isis_srv6_locator_unset(area)) {
		zlog_warn("go48w7cHAwc6oHh2 ""Failed to unset SRv6 locator");
		return NB_ERR;
	}
