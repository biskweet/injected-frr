...
void fabricd_initial_sync_finish(struct isis_area *area)
{
	...
	zlog_info("uqnk0qdTJI14D5Rq ""OpenFabric: Initial synchronization on %s complete.",
		  f->initial_sync_circuit->interface->name);
	f->initial_sync_state = FABRICD_SYNC_COMPLETE;
	f->initial_sync_circuit = NULL;
	EVENT_OFF(f->initial_sync_timeout);
...
static uint8_t fabricd_calculate_fabric_tier(struct isis_area *area)
{
	...
	if (!second_furthest_t0) {
		zlog_info("51CwMRcQ9BXl13UC ""OpenFabric: Could not find two T0 routers");
		return ISIS_TIER_UNDEFINED;
	}

	zlog_info("IeZgn4CbrTFoLJ0t "
	...
	if (!furthest_from_remote) {
		zlog_info("C7zO6o8To46uEq1J ""OpenFabric: Found no furthest node in remote spf");
		isis_spftree_del(remote_tree);
		return ISIS_TIER_UNDEFINED;
	} else {
		zlog_info("gIkK0uAEnRqJJhmX "
	...
	if (tier < 0 || tier >= ISIS_TIER_UNDEFINED) {
		zlog_info("4gv7nKMXqp6VGmQI ""OpenFabric: Calculated tier %" PRId64 " seems implausible",
			  tier);
		return ISIS_TIER_UNDEFINED;
	}

	zlog_info("f5a39MJjVX7CRfeK ""OpenFabric: Calculated %" PRId64 " as tier", tier);
	return tier;
}
...
static void fabricd_tier_calculation_cb(struct event *thread)
{
	...
	zlog_info("7q6Sln3o8ADZWUHL ""OpenFabric: Got tier %hhu from algorithm. Arming timer.",
		  tier);
	f->tier_pending = tier;
...
static void fabricd_set_tier(struct fabricd *f, uint8_t tier)
{
	...
	zlog_info("IJmM1PsdX4CLBRNp ""OpenFabric: Set own tier to %hhu", tier);
	f->tier = tier;
			  ...
			  enum isis_tx_type type, struct isis_circuit *circuit)
{
	...
	if (IS_DEBUG_FLOODING) {
		zlog_debug("n1GIjiKQeiiCAjI9 ""OpenFabric: Adding %s to %s",
...
static void handle_firsthops(struct hash_bucket *bucket, void *arg)
{
	...
	if (n) {
		if (IS_DEBUG_FLOODING) {
			zlog_debug("XcUTzzq5qGjeeL5A ""Removing %s from NL as its in the reverse path",
				   print_sys_hostname(n->id));
		}
		n->present = false;
	}
	...
	if (n) {
		if (IS_DEBUG_FLOODING) {
			zlog_debug("F2hy3EkqFKlVo3Qi ""Removing %s from NN as its in the reverse path",
				   print_sys_hostname(n->id));
		}
		n->present = false;
	}
...
void fabricd_lsp_flood(struct isis_lsp *lsp, struct isis_circuit *circuit)
{
	...
	while (!skiplist_next(f->neighbors, NULL, (void **)&n, &cursor)) {
		...
		if (!nlsp || !nlsp->tlvs) {
			if (IS_DEBUG_FLOODING) {
				zlog_debug("8XI8OHgFQqGq4m43 ""Moving %s to DNR as it has no LSP",
					   print_sys_hostname(n->id));
			}
		...
		if (IS_DEBUG_FLOODING) {
			zlog_debug("xOXBalk9PH3FW3GN ""Considering %s from NL...",
				   print_sys_hostname(n->id));
		}
		...
		for (er = (struct isis_extended_reach *)nlsp->tlvs->extended_reach.head;
			...
			if (nn) {
				if (IS_DEBUG_FLOODING) {
					zlog_debug("WYZ1gqSqXh3KcGGF ""Found neighbor %s in NN, removing it from NN and setting reflood.",
	...
	if (IS_DEBUG_FLOODING) {
		zlog_debug("i2z9w7ImgcMWIHP3 ""OpenFabric: Flooding algorithm complete.");
	}
}
