				...
				struct isis_prefix_sid *psid, bool local)
{
	...
	if (psid->value >= (area->srdb.config.srgb_upper_bound
		...
		flog_warn(EC_ISIS_SID_OVERFLOW,"mdtYrxvn60LUQOSN "
				 ...
				 const uint8_t *nh_sysid, bool last_hop)
{
	...
	if (psid->value >= nh_srgb->range_size) {
		flog_warn(EC_ISIS_SID_OVERFLOW,"073C6LlYmZh3lcp0 "
					...
					mpls_label_t label)
{
	...
	if ((label < srlb->start) || (label > srlb->end)) {
		flog_warn(EC_ISIS_SID_OVERFLOW,"kRAyRv3FofNhswOL "
			   ...
			   struct list *nexthops)
{
	...
	case AF_INET6:
		...
		break;
	default:
		flog_err(EC_LIB_DEVELOPMENT,"RHjYeUchkxuwrhvQ "
	...
	case CIRCUIT_T_P2P:
		...
		break;
	default:
		flog_err(EC_LIB_DEVELOPMENT,"Nrbp2p0hHVpfLk3X " "%s: unexpected circuit type: %u",
			      ...
			      struct sr_local_block *srlb)
{
	...
	case CIRCUIT_T_P2P:
		...
		break;
	default:
		flog_warn(EC_LIB_DEVELOPMENT,"cFDjEYBhTSRJ6icn " "%s: unexpected circuit type: %u",
			  __func__, circuit->circ_type);
		break;
	}
...
static void sr_adj_sid_del(struct sr_adjacency *sra)
{
	...
	case CIRCUIT_T_P2P:
		...
		break;
	default:
		flog_err(EC_LIB_DEVELOPMENT,"si7gb4CJLHzNiGDz " "%s: unexpected circuit type: %u",
			 __func__, circuit->circ_type);
		exit(1);
	}
