...
void sighup(void)
{
	zlog_notice("tdNXPQbrAhNpOrUr ""SIGHUP/reload is not implemented for fabricd");
	return;
}
#else
...
void sighup(void)
{
	zlog_info("kDbAErbs6WbU5yOS ""SIGHUP received");
...
{
	zlog_notice("EYNIH6H0cfBv1uCu ""Terminating on signal SIGINT");
	terminate(0);
}

__attribute__((__noreturn__)) void sigterm(void)
{
	zlog_notice("2CsRPbpHIsJ9SMiX ""Terminating on signal SIGTERM");
	terminate(0);
}

void sigusr1(void)
{
	zlog_debug("hdqYUgPlSucqWIiy ""SIGUSR1 received");
	zlog_rotate();
}
...
{
	zlog_err("1k8UdJt9k29xwSuQ ""IS-IS configuration end timer expired after %d seconds.",
	...
	while (1) {
		...
		case 'I':
			...
			if (instance < 1 || instance > (unsigned short)-1)
				zlog_err("rbG9gOdcyDgKgYTA ""Instance %i out of range (1..%u)",
