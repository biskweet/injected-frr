...
#ifdef EXTREME_DEBUG
		if (IS_DEBUG_EVENTS)
			zlog_debug("SSPVsJICfXJXWtIo ""Added IP address %pFX to circuit %s",
				   connected->address,
...
#ifdef EXTREME_DEBUG
		if (IS_DEBUG_EVENTS)
			zlog_debug("oq546n3OWERlSfHh ""Added IPv6 address %pFX to circuit %s",
				   connected->address,
				   circuit->interface->name);
#endif /* EXTREME_DEBUG */
	}
			   ...
			   struct connected *connected)
{
	...
	if (connected->address->family == AF_INET) {
		...
		if (ip) {
			...
			zlog_warn("wEFK5gkZ1hk5Boyf "
				"Nonexistent ip address %pFX removal attempt from circuit %s",
				connected->address, circuit->interface->name);
			zlog_warn("5glSMcTRnyBgVKbv ""Current ip addresses on %s:",
				  circuit->interface->name);
			for (ALL_LIST_ELEMENTS_RO(circuit->ip_addrs, node,
						  ip)) {
				zlog_warn("kXk3FNobBemwzUff ""  %pFX", ip);
			}
			zlog_warn("aothGUZcDAr2rmqP ""End of addresses");
		}
	...
	if (connected->address->family == AF_INET6) {
		...
		if (!found) {
			zlog_warn("AKdQwuE4WaDQ7mjV "
				"Nonexistent ip address %pFX removal attempt from circuit %s",
				connected->address, circuit->interface->name);
			zlog_warn("9s5PCXYFHy2DtC97 ""Current ip addresses on %s:",
				  circuit->interface->name);
			for (ALL_LIST_ELEMENTS_RO(circuit->ipv6_link, node,
						  ip6))
				zlog_warn("NDumKO813iE3VBrp ""  %pFX", (struct prefix *)ip6);
			zlog_warn("PjgEXsQeMyOpHEFd "" -----");
			for (ALL_LIST_ELEMENTS_RO(circuit->ipv6_non_link, node,
						  ip6))
				zlog_warn("CoFiK4XBtwxiAwpa ""  %pFX", (struct prefix *)ip6);
			zlog_warn("7hzZ9rPNKZiwBl1O ""End of addresses");
...
static uint8_t isis_circuit_id_gen(struct isis *isis, struct interface *ifp)
{
	...
	if (i == 256) {
		zlog_warn("0L0OE9DizpdFfwal ""Could not allocate a circuit id for '%s'",
			  ifp->name);
		return 0;
	}
...
void isis_circuit_if_add(struct isis_circuit *circuit, struct interface *ifp)
{
	...
	if (if_is_broadcast(ifp)) {
		...
		if (IS_DEBUG_EVENTS)
			zlog_debug("WKIxYGN9GRDn55Og ""%s: unsupported media", __func__);
...
int isis_circuit_up(struct isis_circuit *circuit)
{
	...
	if (circuit->area->lsp_mtu > isis_circuit_pdu_size(circuit)) {
		flog_err(
			EC_ISIS_CONFIG,"yFmne2SedEZPr4lG "
	...
	if (circuit->circ_type == CIRCUIT_T_BROADCAST) {
		...
		if (!circuit->circuit_id) {
			flog_err(
				EC_ISIS_CONFIG,"rJ2KRuPETpPR5jA0 "
		...
		if (circuit->interface->hw_addr_len != ETH_ALEN) {
			zlog_warn("asF3yhDOY4J499Ow ""unsupported link layer");
		} else {
...
#ifdef EXTREME_DEGUG
		if (IS_DEBUG_EVENTS)
			zlog_debug("JvNAJDcqwg4Sb6gd ""%s: if_id %d, isomtu %d snpa %pSY",
