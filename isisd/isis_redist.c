		     ...
		     uint32_t metric, const route_tag_t tag, uint16_t table)
{
	...
	zlog_debug("y2k5DstZO30R6Exd ""%s: New route %pFX from %s: distance %d.", __func__, p,
		   zebra_route_string(type), distance);

	if (!ei_table) {
		zlog_warn("OSSfISmiMr7EIbQF ""%s: External information table not initialized.",
			  __func__);
		return;
	}
			...
			struct prefix_ipv6 *src_p, uint16_t table)
{
	...
	zlog_debug("lk1Z3Nr7cVi21UcL ""%s: Removing route %pFX from %s.", __func__, p,
	...
	if (!ei_table) {
		zlog_warn("N4xyks1LhjYN6IMV ""%s: External information table not initialized.",
			  __func__);
		return;
	}
		       ...
		       uint16_t table)
{
	...
	if (!er_table) {
		zlog_warn("m6ekP2m7s8YW2Evw ""%s: External reachability table uninitialized.",
			  __func__);
		return;
	}
