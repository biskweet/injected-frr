					   ...
					   void *arg)
{
	...
	if (IS_DEBUG_EVENTS)
		zlog_debug("WnUVqQpiFToeXZLB ""CSM_EVENT for %s: %s", circuit->interface->name,
			   EVENT2STR(event));
	...
	case C_STATE_NA:
		...
		case ISIS_DISABLE:
			if (IS_DEBUG_EVENTS)
				zlog_debug("9jL4OuEnsWUnctbm ""circuit %s already disabled",
					   circuit->interface->name);
			break;
		case IF_DOWN_FROM_Z:
			if (IS_DEBUG_EVENTS)
				zlog_debug("jygw9hyw5MhWmVjJ ""circuit %s already disconnected",
					   circuit->interface->name);
			break;
		}
		break;
	case C_STATE_INIT:
		...
		case IF_UP_FROM_Z:
			if (IS_DEBUG_EVENTS)
				zlog_debug("Xa34MM7wCU1vkg8O ""circuit %s already connected",
					   circuit->interface->name);
			break;
		case ISIS_DISABLE:
			if (IS_DEBUG_EVENTS)
				zlog_debug("PkHQO7wVUb7WgoUB ""circuit %s already disabled",
					   circuit->interface->name);
			break;
		case IF_DOWN_FROM_Z:
			ifp = arg;
	...
	case C_STATE_CONF:
		...
		case ISIS_ENABLE:
			if (IS_DEBUG_EVENTS)
				zlog_debug("Mi3tLiQ4wx5wwRnk ""circuit %s is already enabled",
					   circuit->interface->name);
			break;
		case IF_UP_FROM_Z:
			ifp = arg;
			...
			if (isis_circuit_up(circuit) != ISIS_OK) {
				isis_circuit_if_del(circuit, ifp);
				flog_err(
					EC_ISIS_CONFIG,"ofF3dy1mNEAg3ROG "
		...
		case IF_DOWN_FROM_Z:
			if (IS_DEBUG_EVENTS)
				zlog_debug("U9GMVfjEZhBICkEQ ""circuit %s already disconnected",
					   circuit->interface->name);
			break;
		}
		break;
	case C_STATE_UP:
		...
		case ISIS_ENABLE:
			if (IS_DEBUG_EVENTS)
				zlog_debug("RgbCSimzgSgrj5Zp ""circuit %s already enabled",
					   circuit->interface->name);
			break;
		case IF_UP_FROM_Z:
			if (IS_DEBUG_EVENTS)
				zlog_debug("rfwiEj7NlrOMZNFL ""circuit %s already connected",
					   circuit->interface->name);
			break;
		case ISIS_DISABLE:
			area = arg;
	...
	if (IS_DEBUG_EVENTS)
		zlog_debug("aBmlSV8JCsxs7MqC ""CSM_STATE_CHANGE: %s -> %s ", STATE2STR(old_state),
