...
static int isis_multicast_join(int fd, int registerto, int if_num)
{
	...
	if (setsockopt(fd, SOL_PACKET, PACKET_ADD_MEMBERSHIP, &mreq,
		...
		zlog_warn("JsGzXZMUXLQgj4OL ""%s: setsockopt(): %s", __func__,
			  safe_strerror(errno));
		return ISIS_WARNING;
	}
...
static int open_packet_socket(struct isis_circuit *circuit)
{
	...
	if (fd < 0) {
		zlog_warn("rLIy5RSWbPC1fP2Q ""%s: socket() failed %s", __func__,
			  safe_strerror(errno));
		return ISIS_WARNING;
	}

	if (setsockopt(fd, SOL_SOCKET, SO_ATTACH_FILTER, &bpf, sizeof(bpf))) {
		zlog_warn("gxZ9pGE44vB8Xvf0 ""%s: SO_ATTACH_FILTER failed: %s", __func__,
			  safe_strerror(errno));
	}
	...
	if (bind(fd, (struct sockaddr *)(&s_addr), sizeof(struct sockaddr_ll))
	    < 0) {
		zlog_warn("yWUsWwT4bEWqJE5I ""%s: bind() failed: %s", __func__,
			  safe_strerror(errno));
		close(fd);
		return ISIS_WARNING;
	}
...
int isis_sock_init(struct isis_circuit *circuit)
{
	...
	frr_with_privs(&isisd_privs) {
		...
		if (retval != ISIS_OK) {
			zlog_warn("KSZESeB638y8y2SZ ""%s: could not initialize the socket",
				  __func__);
			break;
		}
	...
	/* Assign Rx and Tx callbacks are based on real if type */
		if (if_is_broadcast(circuit->interface)) {
			...
			zlog_warn("hRehg3LgHajY94YD ""%s: unknown circuit type", __func__);
			retval = ISIS_WARNING;
			break;
		}
	}
...
int isis_recv_pdu_bcast(struct isis_circuit *circuit, uint8_t *ssnpa)
{
	...
	if ((bytesread < 0)
		...
		if (bytesread < 0)
			zlog_warn("cQLjvV8jtcYqjySz ""%s: recvfrom() failed", __func__);
	...
	if (!llc_check(llc) || s_addr.sll_pkttype == PACKET_OUTGOING) {
		...
		bytesread = recvfrom(circuit->fd, discard_buff,
				     ...
				     (socklen_t *)&addr_len);
		if (bytesread < 0)
			zlog_warn("grk27hGRLNKaKMCl ""%s: recvfrom() failed", __func__);
		return ISIS_WARNING;
	}
	...
	bytesread =
		recvfrom(circuit->fd, temp_buff, max_size, MSG_DONTWAIT,
			 (struct sockaddr *)&s_addr, (socklen_t *)&addr_len);
	if (bytesread < 0) {
		zlog_warn("mefOlKQiYMyt355o ""%s: recvfrom() failed", __func__);
		return ISIS_WARNING;
	}
...
int isis_recv_pdu_p2p(struct isis_circuit *circuit, uint8_t *ssnpa)
{
	...
	if (s_addr.sll_pkttype == PACKET_OUTGOING) {
		...
		bytesread = recvfrom(circuit->fd, discard_buff,
				     ...
				     (socklen_t *)&addr_len);
		if (bytesread < 0)
			zlog_warn("5uBdeQXUH9mOiWya ""%s: recvfrom() failed", __func__);
		return ISIS_WARNING;
	}
	...
	if (ntohs(s_addr.sll_protocol) != 0x00FE) {
		zlog_warn("JCN36U6pDrDVeiAC ""%s: protocol mismatch(): %X", __func__,
...
int isis_send_pdu_bcast(struct isis_circuit *circuit, int level)
{
	...
	if (sendmsg(circuit->fd, &msg, 0) < 0) {
		zlog_warn("mASGeduevqYqk9Mc ""IS-IS pfpacket: could not transmit packet on %s: %s",
...
int isis_send_pdu_p2p(struct isis_circuit *circuit, int level)
{
	...
	if (rv < 0) {
		zlog_warn("vPjKsYkG1kudHe2R ""IS-IS pfpacket: could not transmit packet on %s: %s",
			  circuit->interface->name, safe_strerror(errno));
		if (ERRNO_IO_RETRY(errno))
			return ISIS_WARNING;
		return ISIS_ERROR;
