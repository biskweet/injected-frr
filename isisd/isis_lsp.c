		...
		uint16_t checksum, uint16_t rem_lifetime)
{
	...
	if (seqno > lsp->hdr.seqno
		...
		if (IS_DEBUG_SNP_PACKETS) {
			zlog_debug("h52ALofgbleFH0bP "
				"ISIS-Snp (%s): Compare LSP %pLS seq 0x%08x, cksum 0x%04hx, lifetime %hus",
				areatag, lsp->hdr.lsp_id, seqno, checksum,
				rem_lifetime);
			zlog_debug("pgqotZQI7XA829fi "
	...
	if (IS_DEBUG_SNP_PACKETS) {
		zlog_debug("MWOUJPKKT1wf5ejc "
			"ISIS-Snp (%s): Compare LSP %pLS seq 0x%08x, cksum 0x%04hx, lifetime %hus",
			areatag, lsp->hdr.lsp_id, seqno, checksum,
			rem_lifetime);
		zlog_debug("6020S4jcWpgbJ5Bx "
		...
		struct isis_area *area, int level, bool confusion)
{
	if (lsp->own_lsp) {
		flog_err(
			EC_LIB_DEVELOPMENT,"YcByXhbou8FWQt0Z "
			 ...
			 struct isis_lsp *lsp0, int level)
{
	...
	if (IS_DEBUG_EVENTS)
		zlog_debug("FH8rMCUi5J7fzJ5q ""New LSP with ID %pLS len %d seqnum %08x", lsp_id,
...
static void lsp_build(struct isis_lsp *lsp, struct isis_area *area)
{
	...
	for (ALL_LIST_ELEMENTS_RO(area->circuit_list, node, circuit)) {
		...
		case CIRCUIT_T_LOOPBACK:
			break;
		default:
			zlog_warn("kcIKEsI58voVMBaj ""lsp_area_create: unknown circuit type");
		}
	}
	...
	if (!fragments) {
		zlog_warn("WWkP7Vl9K0cIvBEL ""BUG: could not fragment own LSP:");
...
static int lsp_regenerate(struct isis_area *area, int level)
{
	...
	if (!lsp) {
		flog_err(EC_LIB_DEVELOPMENT,"unJnMWT89aEYqgUJ "
...
static int lsp_regenerate_pseudo(struct isis_circuit *circuit, int level)
{
	...
	if (!lsp) {
		flog_err(EC_LIB_DEVELOPMENT,"WcJKJ93BUCYjipFT "
		...
		const char *func, const char *file, int line)
{
	if (IS_DEBUG_FLOODING) {
		zlog_debug("MwWCnA9BPzZKYQzB ""Flooding LSP %pLS%s%s (From %s %s:%d)",
