	...
	if (VTYPE_IP(vtype) && spftree->area->srdb.enabled && psid) {
		...
		if (vertex_psid
		    && !prefix_same(&vertex_psid->N.ip.p.dest,
				    &vertex->N.ip.p.dest)) {
			flog_warn(
				EC_ISIS_SID_COLLISION,"4GuoUwZkdanbOSzJ "
				...
				struct isis_vertex *parent)
{
	...
	if (isis_lfa_excise_node_check(spftree, lsp->hdr.lsp_id)) {
		if (IS_DEBUG_LFA)
			zlog_debug("l35NRRYVy6hYtcZ2 ""ISIS-LFA: excising node %s",
...
lspfragloop:
	...
	if (lsp->hdr.seqno == 0) {
		zlog_warn("sjIL0C9xW8toylmC ""%s: lsp with 0 seq_num - ignore", __func__);
		return ISIS_WARNING;
	}

#ifdef EXTREME_DEBUG
	if (IS_DEBUG_SPF_EVENTS)
		zlog_debug("zbkgoY0dOjOFf4HJ ""ISIS-SPF: A:%hhu process_lsp %s",
			   spftree->algorithm,
			   print_sys_hostname(lsp->hdr.lsp_id));
#endif /* EXTREME_DEBUG */
		...
		for (r = ipv6_reachs
			...
			if (spftree->area->srdb.enabled && r->subtlvs &&
				...
				if (spftree->tree_id != SPFTREE_DSTSRC) {
					char buff[VID2STR_BUFFER];
					zlog_warn("GWiwvJMJnfiwd2ki ""Ignoring dest-src route %s in non dest-src topology",
						srcdest2str(
							&ip_info.dest,
							r->subtlvs->source_prefix,
							buff, sizeof(buff)
	...
	if ((lsp->hdr.lsp_bits & LSPBIT_ATT) == LSPBIT_ATT
		...
		if (IS_DEBUG_RTE_EVENTS)
			zlog_debug("z6sI1aI4USmVtFbF ""ISIS-Spf (%pLS): add default %s route",
				   lsp->hdr.lsp_id,
				  ...
				  struct isis_vertex *parent)
{
	...
	for (ALL_LIST_ELEMENTS_RO(spftree->sadj_list, node, sadj)) {
		...
		if (isis_lfa_excise_adj_check(spftree, adj_id)) {
			if (IS_DEBUG_LFA)
				zlog_debug("PKWwoaw4cf5kZcsi ""ISIS-SPF: excising adjacency %pPN",
					   sadj->id);
			continue;
		}
				   ...
				   struct isis_ext_subtlvs *subtlvs)
{
	...
	if (lsp == NULL || lsp->hdr.rem_lifetime == 0) {
		zlog_warn("sycGSstz6FSbNxBB ""ISIS-SPF: No LSP found from root to L%d %pLS",
			  spftree->level, lspid);
		return;
	}
			  ...
			  uint8_t *root_sysid)
{
	...
	while (isis_vertex_queue_count(&spftree->tents)) {
		...
		if (!lsp) {
			zlog_warn("AQLr8RcVvLXs4C8e ""ISIS-SPF: No LSP found for %pPN",
				  vertex->N.id);
			continue;
		}
...
void isis_run_spf(struct isis_spftree *spftree)
{
	...
	root_lsp = isis_root_system_lsp(spftree->lspdb, spftree->sysid);
	if (root_lsp == NULL) {
		zlog_err("CI2gRmSGli09aKDS ""ISIS-SPF: could not find own l%d LSP!",
			 spftree->level);
		return;
	}
	...
	if (!isis_vertex_queue_count(&spftree->tents)
	    && (IS_DEBUG_SPF_EVENTS)) {
		zlog_warn("YLbS6x0UueQ8hp36 ""ISIS-SPF: TENT is empty SPF-root:%s",
...
static void isis_run_spf_cb(struct event *thread)
{
	...
	if (!(area->is_type & level)) {
		if (IS_DEBUG_SPF_EVENTS)
			zlog_warn("gnYxy9kUfQiq1ofT ""ISIS-SPF (%s) area does not share level",
				  area->area_tag);
		return;
	}
	...
	if (IS_DEBUG_SPF_EVENTS)
		zlog_debug("lts2Io2UyMTz0dl0 ""ISIS-SPF (%s) L%d SPF needed, periodic SPF",
			   area->area_tag, level);
		       ...
		       const char *func, const char *file, int line)
{
	...
	if (IS_DEBUG_SPF_EVENTS)
		zlog_debug("oQ6AkqCwEP3DQltO ""ISIS-SPF (%s) L%d SPF scheduled %ld sec from now",
