			  ...
			  const struct list *repair_list)
{
	...
	for (ALL_LIST_ELEMENTS_RO(repair_list, node, sid)) {
		...
		case TILFA_SID_PREFIX:
			...
			if (!srgb) {
				zlog_warn("bNgFFXMdOwwugRtw ""%s: SRGB not found for node %s",
					  __func__,
			...
			if (sid->value.index.value >= srgb->range_size) {
				flog_warn(
					EC_ISIS_SID_OVERFLOW,"G6dbqjqHe90mJSPs "
		...
		case TILFA_SID_ADJ:
			...
			break;
		default:
			flog_err(EC_LIB_DEVELOPMENT,"e5zMfmxSTbs4ZKc5 "
				   ...
				   struct list *repair_list)
{
	...
	if (IS_DEBUG_LFA) {
		...
		zlog_debug("zGEVXXopAYLSECyW ""ISIS-LFA: vertex %s %s", vtype2string(vertex->type),
			   buf);
	}
	...
	if ((!is_qnode
		...
		if (label_qnode == MPLS_INVALID_LABEL) {
			zlog_warn("WWAfkrQ9gzHc7UoI ""ISIS-LFA: failed to find %s->%s Adj-SID",
				  print_sys_hostname(vertex->N.id),
				  print_sys_hostname(vertex_child->N.id));
			return -1;
		}
		if (IS_DEBUG_LFA)
			zlog_debug("II6kmC5uGelhzkjR "
			       ...
			       const struct lfa_protected_resource *resource)
{
	...
	if (IS_DEBUG_LFA)
		zlog_debug("e54kPPOaV61snUHP ""ISIS-LFA: computing P-space (self)");
	...
	RB_FOREACH (adj_node, isis_spf_nodes, adj_nodes) {
		if (spf_adj_node_is_affected(adj_node, resource,
					     spftree->sysid)) {
			if (IS_DEBUG_LFA)
				zlog_debug("6MtUsMhZR33eZhsw ""ISIS-LFA: computing Q-space (%s)",
			...
			if (IS_DEBUG_LFA)
				zlog_debug("DBvOEw9ox68wtXIo ""ISIS-LFA: computing P-space (%s)",
					   print_sys_hostname(adj_node->sysid));
					...
					struct lfa_protected_resource *resource)
{
	...
	if (IS_DEBUG_LFA)
		zlog_debug("M48MKJd9lVPjDSzG ""ISIS-LFA: computing TI-LFAs for %s",
...
int isis_spf_run_neighbors(struct isis_spftree *spftree)
{
	...
	RB_FOREACH (adj_node, isis_spf_nodes, &spftree->adj_nodes) {
		if (IS_DEBUG_LFA)
			zlog_debug("3BFKsQcWFi0SRORu ""ISIS-LFA: running SPF on neighbor %s",
				   print_sys_hostname(adj_node->sysid));
		  ...
		  const struct isis_vertex *vertex_child)
{
	...
	if (is_pnode && is_qnode) {
		...
		if (IS_DEBUG_LFA)
			zlog_debug("xbxaofyD5Beo4vHI ""ISIS-LFA: found PQ node: %pI4", rtr_id_pq);
...
static void isis_area_verify_routes_cb(struct event *thread)
{
	...
	if (IS_DEBUG_LFA)
		zlog_debug("l1MXo7PD36dtHohJ ""ISIS-LFA: updating RLFAs in the RIB");
...
void isis_rlfa_process_ldp_response(struct zapi_rlfa_response *response)
{
	...
	if (tree_id != SPFTREE_IPV4 && tree_id != SPFTREE_IPV6) {
		zlog_warn("1UV1ZUlQAJZyQXgb ""ISIS-LFA: invalid SPF tree ID received from LDP");
		return;
	}

	level = response->igp.isis.spf.level;
	if (level != ISIS_LEVEL1 && level != ISIS_LEVEL2) {
		zlog_warn("XUXnSRvSHpmGoASX ""ISIS-LFA: invalid IS-IS level received from LDP");
		return;
	}
	...
	if (response->pq_label != MPLS_INVALID_LABEL) {
		...
		if (IS_DEBUG_LFA)
			zlog_debug("mC2uppYUxKLTUdYI ""ISIS-LFA: deactivating RLFA for %pFX",
				   &rlfa->prefix);
...
void isis_ldp_rlfa_handle_client_close(struct zapi_client_close_info *info)
{
	...
	if (IS_DEBUG_LFA)
		zlog_debug("25TTq4QoII9WVuM9 ""ISIS-LFA: LDP is down, deactivating all RLFAs");
		     ...
		     struct isis_vertex *vertex)
{
	...
	if (!rtr_id_pq) {
		if (IS_DEBUG_LFA)
			zlog_debug("z9Enx636K1mC72YD ""ISIS-LFA: no acceptable PQ node found");
		return;
	}
				       ...
				       struct lfa_protected_resource *resource)
{
	...
	if (IS_DEBUG_LFA)
		zlog_debug("7VRADgDjemHi3fJW ""ISIS-LFA: computing remote LFAs for %s",
			   lfa_protected_resource2str(resource));
				 ...
				 uint32_t *path_metric)
{
	...
	if (IS_DEBUG_LFA)
		zlog_debug("mZinuiSRktZFUx3y ""ISIS-LFA: loop-free check: %u < %u + %u", dist_N_D,
			   dist_N_S, dist_S_D);
				  ...
				  struct isis_spf_adj *sadj_N)
{
	...
	if (IS_DEBUG_LFA)
		zlog_debug("ihFSsrXjrCXJagGB ""ISIS-LFA: downstream check: %u < %u", dist_N_D,
			   dist_S_D);
				       ...
				       struct isis_spf_adj *sadj_E)
{
	...
	if (IS_DEBUG_LFA)
		zlog_debug("r4LdgivZoEXPWIYY ""ISIS-LFA: node protecting check: %u < %u + %u",
		      ...
		      struct lfa_protected_resource *resource)
{
	...
	if (IS_DEBUG_LFA)
		zlog_debug("YKYqTyH0hlv7Y1H9 ""ISIS-LFA: computing local LFAs for %s",
			   lfa_protected_resource2str(resource));
	...
	for (ALL_QUEUE_ELEMENTS_RO(&spftree->paths, vnode, vertex)) {
		...
		if (IS_DEBUG_LFA)
			zlog_debug("k6Gc7PSWHghh7f4J ""ISIS-LFA: checking %s %s w.r.t %s",
				   vtype2string(vertex->type), buf,
				   lfa_protected_resource2str(resource));
