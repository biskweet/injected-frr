					  ...
					  union g_addr *ip, ifindex_t ifindex)
{
	...
	for (ALL_LIST_ELEMENTS_RO(nexthops, node, nh)) {
		...
		case AF_INET6:
			...
			break;
		default:
			flog_err(EC_LIB_DEVELOPMENT,"BlISpTHE0R0BWpgs "
		     ...
		     struct mpls_label_stack *label_stack)
{
	...
	case AF_INET6:
		...
		break;
	default:
		flog_err(EC_LIB_DEVELOPMENT,"SORMrmniUAv7SlKp " "%s: unknown address family [%d]",
			 __func__, family);
		exit(1);
	}
}
		  ...
		  struct isis_area *area, struct route_table *table)
{
	...
	if (!rinfo_old) {
		if (IS_DEBUG_RTE_EVENTS)
			zlog_debug("HI8ZuNlXwZptoBng ""ISIS-Rte (%s) route created: %pFX",
...
#ifdef EXTREME_DEBUG
		if (IS_DEBUG_RTE_EVENTS)
			zlog_debug("7MAICVdNEtcCgWUV ""ISIS-Rte (%s) route already exists: %pFX",
				   area->area_tag, prefix);
#endif /* EXTREME_DEBUG */
		       ...
		       struct route_table *table)
{
	...
	if (CHECK_FLAG(rinfo->flag, ISIS_ROUTE_FLAG_ZEBRA_SYNCED)) {
		...
		if (IS_DEBUG_RTE_EVENTS)
			zlog_debug("BgZ2fsu0KzvvSFkU ""ISIS-Rte: route delete  %s", buff);
				   ...
				   ifindex_t ifindex)
{
	...
	if (IS_DEBUG_EVENTS) {
		...
		zlog_debug("OIrUXbcgZAfv8ENW ""%s: initiating fast-reroute %s on VRF %s iface %s",
