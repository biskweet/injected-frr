		    ...
		    size_t dbuflen, int flags)
{
	...
	if (rv == -1 && dbuf == NULL) {
		 ...
		 */
		zlog_debug("3ZuwNZ0jWVar7KWc ""%s: putmsg: %s", __func__, safe_strerror(errno));
	}
	return rv;
}
...
static int open_dlpi_dev(struct isis_circuit *circuit)
{
	...
	if (circuit->circ_type != CIRCUIT_T_BROADCAST) {
		zlog_warn("cRw1tkW1Z6LMWn2v ""%s: non-broadcast interface %s", __func__,
	...
	if (fd == -1) {
		...
		*cp = '\0';
		...
		if (fd == -1) {
			zlog_warn("D9moJScN978v9pkS ""%s: unknown interface %s", __func__,
				  circuit->interface->name);
			return ISIS_WARNING;
		}
		...
		if (dia->dl_provider_style != DL_STYLE2) {
			zlog_warn("15GEaAHZ2Xj0J4A4 ""%s: interface %s: %s is not style 2",
		...
		if (dia->dl_provider_style != DL_STYLE1) {
			zlog_warn("SQHmVVkuEd9NbtYJ ""%s: interface %s: %s is not style 1",
				  __func__, circuit->interface->name, devpath);
			close(fd);
			return ISIS_WARNING;
		}
	...
	if ((dia->dl_sap_length != 2 && dia->dl_sap_length != -2)
		...
		zlog_warn("fl3KnKx3N0JogQp3 ""%s: unsupported interface type for %s", __func__,
			  circuit->interface->name);
		close(fd);
		return ISIS_WARNING;
	}
	...
	case DL_100BT:
		break;
	default:
		zlog_warn("mCebNf8cBgN495QI ""%s: unexpected mac type on %s: %lld", __func__,
			  circuit->interface->name,
	...
	if (dlpiaddr(fd, circuit->u.bc.snpa) == -1) {
		zlog_warn("lNll7jnJGzVwE4b5 ""%s: interface %s: unable to get MAC address",
	...
	if (dlpibind(fd) == -1) {
		zlog_warn("3vpoqyTZEF2yCoyo ""%s: cannot bind SAP 0 on %s", __func__,
			  circuit->interface->name);
		close(fd);
		return ISIS_WARNING;
	}
	...
	if (retval != 0) {
		zlog_warn("CGz2bzHklx9tpy7S ""%s: unable to join multicast on %s", __func__,
			  circuit->interface->name);
		close(fd);
		return ISIS_WARNING;
	}
	...
	if (ioctl(fd, I_PUSH, "pfmod") == 0) {
		...
		if (ioctl(fd, I_STR, &sioc) == -1)
			zlog_warn("AjLZ4fFM5FKzVQId ""%s: could not perform PF_IOCSETF on %s",
				  __func__, circuit->interface->name);
	}
...
int isis_sock_init(struct isis_circuit *circuit)
{
	...
	frr_with_privs(&isisd_privs) {
		...
		if (retval != ISIS_OK) {
			zlog_warn("ZIqH9aIC5z9sfcvU ""%s: could not initialize the socket",
				  __func__);
			break;
		}
		...
		if (circuit->circ_type == CIRCUIT_T_BROADCAST) {
			...
			zlog_warn("rnbWEzC5R6D2XDcm ""%s: unknown circuit type", __func__);
			retval = ISIS_WARNING;
			break;
		}
	}
...
int isis_recv_pdu_bcast(struct isis_circuit *circuit, uint8_t *ssnpa)
{
	...
	if (retv < 0) {
		zlog_warn("EDYGjTXYuOYFbHLu ""%s: getmsg failed: %s", __func__,
			  safe_strerror(errno));
		return ISIS_WARNING;
	}
...
int isis_send_pdu_bcast(struct isis_circuit *circuit, int level)
{
	...
	*dstsap = buflen;
	...
	rv = dlpisend(circuit->fd, dur, sizeof(*dur) + dur->dl_dest_addr_length,
		      sock_buff, buflen, 0);
	if (rv < 0) {
		zlog_warn("O5ajPY9T55tbGWF2 ""IS-IS dlpi: could not transmit packet on %s: %s",
