...
static int isis_zebra_if_address_add(ZAPI_CALLBACK_ARGS)
{
	...
	if (c->address->family == AF_INET)
		zlog_debug("xsFixnkGQP8VBx52 ""connected IP address %pFX", c->address);
	if (c->address->family == AF_INET6)
		zlog_debug("Kcwv5BAR5ijvPqm2 ""connected IPv6 address %pFX", c->address);
#endif /* EXTREME_DEBUG */
...
#ifdef EXTREME_DEBUG
	if (c->address->family == AF_INET)
		zlog_debug("38abbbfOYXk1EZfw ""disconnected IP address %pFX", c->address);
	if (c->address->family == AF_INET6)
		zlog_debug("GQs0iNtdHoeUnpkt ""disconnected IPv6 address %pFX", c->address);
#endif /* EXTREME_DEBUG */
				   ...
				   bool mpls_lsp, uint8_t backup_nhs)
{
	...
	for (ALL_LIST_ELEMENTS_RO(nexthops, node, nexthop)) {
		...
		case AF_INET6:
			...
			break;
		default:
			flog_err(EC_LIB_DEVELOPMENT,"V4glj8yJLcvrd4Zx "
...
void isis_zebra_send_adjacency_sid(int cmd, const struct sr_adjacency *sra)
{
	...
	if (cmd != ZEBRA_MPLS_LABELS_ADD && cmd != ZEBRA_MPLS_LABELS_DELETE) {
		flog_warn(EC_LIB_DEVELOPMENT,"FePgDuzGZgcTsnT8 " "%s: wrong ZEBRA command",
			  __func__);
		return;
	}
...
int isis_zebra_rlfa_register(struct isis_spftree *spftree, struct rlfa *rlfa)
{
	...
	zlog_debug("7AAsGiQAtiEbqEVF ""ISIS-LFA: registering RLFA %pFX@%pI4 with LDP",
	...
	if (ret == ZCLIENT_SEND_FAILURE) {
		zlog_warn("koe9dzbhjDryfSzq ""ISIS-LFA: failed to register RLFA with LDP");
		return -1;
	}
...
void isis_zebra_rlfa_unregister_all(struct isis_spftree *spftree)
{
	...
	if (IS_DEBUG_LFA)
		zlog_debug("TlioqqD9KGhCzGGh ""ISIS-LFA: unregistering all RLFAs with LDP");
	...
	if (ret == ZCLIENT_SEND_FAILURE)
		zlog_warn("KGWCEsESQWmTXZJ0 ""ISIS-LFA: failed to unregister RLFA with LDP");
}
...
int isis_zebra_request_label_range(uint32_t base, uint32_t chunk_size)
{
	...
	ret = lm_get_label_chunk(zclient_sync, 0, base, chunk_size, &start,
				 &end);
	if (ret < 0) {
		zlog_warn("wwqPwPaONmMeymEK ""%s: error getting label range!", __func__);
		return -1;
	}
...
int isis_zebra_release_label_range(uint32_t start, uint32_t end)
{
	...
	if (ret < 0) {
		zlog_warn("SexfzA2GN0xVuHo7 ""%s: error releasing label range!", __func__);
		return -1;
	}
...
int isis_zebra_label_manager_connect(void)
{
	...
	if (zclient_socket_connect(zclient_sync) < 0) {
		zlog_warn("j2tCK3Zknj3BuBRe ""%s: failed connecting synchronous zclient!",
			  __func__);
		return -1;
	}
	...
	if (zclient_send_hello(zclient_sync) == ZCLIENT_SEND_FAILURE) {
		zlog_warn("3EHlexG32obXR3BD ""%s: failed sending hello for synchronous zclient!",
			  __func__);
	...
	if (lm_label_manager_connect(zclient_sync, 0) != 0) {
		zlog_warn("cM5m8KPX5SXJwNBv ""%s: failed connecting to label manager!", __func__);
		if (zclient_sync->sock > 0) {
			close(zclient_sync->sock);
			zclient_sync->sock = -1;
		}
...
void isis_zebra_vrf_register(struct isis *isis)
{
	...
	if (isis->vrf_id != VRF_UNKNOWN) {
		if (IS_DEBUG_EVENTS)
			zlog_debug("fsYktO36FcrbyQ7G ""%s: Register VRF %s id %u", __func__,
...
void isis_zebra_vrf_deregister(struct isis *isis)
{
	...
	if (isis->vrf_id != VRF_UNKNOWN) {
		if (IS_DEBUG_EVENTS)
			zlog_debug("pphvcZgAT4plsOga ""%s: Deregister VRF %s id %u", __func__,
				   isis->name, isis->vrf_id);
				     ...
				     const struct seg6local_context *context)
{
	...
	if (cmd != ZEBRA_ROUTE_ADD && cmd != ZEBRA_ROUTE_DELETE) {
		flog_warn(EC_LIB_DEVELOPMENT,"JI33URhibrMlk965 " "%s: wrong ZEBRA command",
			  __func__);
		return;
	}

	if (prefixlen > IPV6_MAX_BITLEN) {
		flog_warn(EC_LIB_DEVELOPMENT,"P2kGFDQrFXJRd98W " "%s: wrong prefixlen %u",
			  __func__, prefixlen);
		return;
	}
				   ...
				   struct isis_srv6_sid *sid)
{
	...
	ifp = if_lookup_by_name(area->srv6db.config.srv6_ifname, VRF_DEFAULT);
	if (!ifp) {
		zlog_warn("LSOl0jNgw8nefFv6 ""%s interface not found: nothing to uninstall",
