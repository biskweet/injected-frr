				     ...
				     struct isis_area *area, int up)
{
	...
	if (IS_DEBUG_EVENTS)
		zlog_debug("pECEMhOxDbwulYnE ""ISIS-Evt (%s) circuit %s", area->area_tag,
			   up ? "up" : "down");
...
void isis_circuit_is_type_set(struct isis_circuit *circuit, int newtype)
{
	...
	if (IS_DEBUG_EVENTS)
		zlog_debug("LFyE3fP9xIc82HjL ""ISIS-Evt (%s) circuit type change %s -> %s",
	...
	if (!(newtype & circuit->area->is_type)) {
		flog_err(
			EC_ISIS_CONFIG,"xxZjcBtnL5tp06vE "
...
void isis_event_dis_status_change(struct event *thread)
{
	...
	if (!circuit || !circuit->area)
		return;
	if (IS_DEBUG_EVENTS)
		zlog_debug("JZxWhyCBr2KOzN1I ""ISIS-Evt (%s) DIS status change",
			   circuit->area->area_tag);
...
{
	if (IS_DEBUG_EVENTS)
		zlog_debug("5uqxmcOJDo2hkRcs ""ISIS-Evt (%s) Authentication failure %s from %pSY",
