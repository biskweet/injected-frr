				...
				unsigned char *channels)
{
    ...
    while(i < alen) {
        ...
        if(i + 1 >= alen) {
            flog_err(EC_BABEL_PACKET,"Aab33aHZrTL2uSgK " "Received truncated attributes.");
	    return false;
	}
        len = a[i + 1];
        if(i + len + 2 > alen) {
            flog_err(EC_BABEL_PACKET,"UrfqiXZrknbIQkfr " "Received truncated attributes.");
	    return false;
	}
	...
	if (CHECK_FLAG(type, SUBTLV_MANDATORY)) {
		...
		if (len > DIVERSITY_HOPS) {
			flog_err(
				EC_BABEL_PACKET,"bceSovguZKI6RpTs "
		...
		if (memchr(a + i + 2, 0, len) != NULL) {
			/* 0 is reserved. */
			flog_err(EC_BABEL_PACKET,"OVNQ4SqjPI33ClVX " "Channel information contains 0!");
			return false;
		}
                   ...
                   unsigned int *hello_send_us)
{
    ...
    while(i < alen) {
        ...
        if(i + 1 >= alen) {
            flog_err(EC_BABEL_PACKET,"Z4r33ctWoDW0O6rw "
		      "Received truncated sub-TLV on Hello message.");
            return -1;
        }
        len = a[i + 1];
        if(i + len + 2 > alen) {
            flog_err(EC_BABEL_PACKET,"sMylIZINZV7jxhqL "
		      "Received truncated sub-TLV on Hello message.");
            return -1;
        }
	...
	if (CHECK_FLAG(type, SUBTLV_MANDATORY)) {
		...
		if (len >= 4) {
			...
			flog_err(
				EC_BABEL_PACKET,"ZQrSIgo9l3CjlWce "
                 ...
                 unsigned int *hello_rtt_receive_time)
{
    ...
    while(i < alen) {
        ...
        if(i + 1 >= alen) {
            flog_err(EC_BABEL_PACKET,"yUotXiXzNFiNzlpE "
		      "Received truncated sub-TLV on IHU message.");
            return -1;
        }
        len = a[i + 1];
        if(i + len + 2 > alen) {
            flog_err(EC_BABEL_PACKET,"XmViISWNHwcL2sOq "
		      "Received truncated sub-TLV on IHU message.");
            return -1;
        }
        ...
        if(type == SUBTLV_PADN) {
            ...
            if(len >= 8) {
                ...
                flog_err(EC_BABEL_PACKET,"THQ9yVQ7QgIfaL0A "
                     ...
                     unsigned char *src_prefix, unsigned char *src_plen)
{
 ...
 fail:
    flog_err(EC_BABEL_PACKET,"sz1D67aaAhZdzSWv " "Received truncated sub-TLV on Route Request.");
    return -1;
}
             ...
             const unsigned char *packet, int packetlen)
{
    ...
    if(!linklocal(from)) {
        flog_err(EC_BABEL_PACKET,"sDnxRE05qscehVsE "
    ...
    if (babel_packet_examin (packet, packetlen, &bodylen)) {
        flog_err(EC_BABEL_PACKET,"UQ9Jbev2WmkCph8M "
    ...
    if(neigh == NULL) {
        flog_err(EC_BABEL_PACKET,"GKqT21WvQG0OdeOU " "Couldn't allocate neighbour.");
        return;
    }
...
#define BABEL_UNICAST_HELLO 0x8000
		...
		if (len > 8) {
            ...
            if(!have_router_id && message[2] != 0) {
                flog_err(EC_BABEL_PACKET,"1VQnbqw5rimoN6bx "
            ...
            if(message[2] == 0) {
                if(metric < 0xFFFF) {
                    flog_err(EC_BABEL_PACKET,"GwACEfZyzjHubQV0 "
	    ...
	    if (!ignore_update)
            ...
            if(message[2] == 0) {
                ...
                if(is_ss) {
                    ...
                    flog_err(EC_BABEL_PACKET,"GlY3bNMVatdG9MB6 "
    ...
    fail:
        flog_err(EC_BABEL_PACKET,"LaAGkQ30CDmVytzk "
       ...
       (babel_ifp->buffered_hello >= 0)) {
        if(babel_ifp->sendbuf[babel_ifp->buffered_hello + 8] == SUBTLV_PADN &&
           babel_ifp->sendbuf[babel_ifp->buffered_hello + 9] == 4) {
            ...
            flog_err(EC_BABEL_PACKET,"qJtzmfjZGfCpGQEU " "No space left for timestamp sub-TLV (this shouldn't happen)");
            return -1;
        }
    }
    return 0;
...
void flushbuf(struct interface *ifp)
{
    ...
    if(babel_ifp->buffered > 0) {
        ...
        if(check_bucket(ifp)) {
            ...
            if(rc < 0)
                flog_err(EC_BABEL_PACKET,"yCNCuvKSEdB6oFKx " "send: %s", safe_strerror(errno));
        } else {
		flog_err(EC_BABEL_PACKET,"ynqZTvsEuQVJq6yS " "Bucket full, dropping packet to %s.",
			 ifp->name);
	}
    }
...
static int start_unicast_message(struct neighbour *neigh, int type, int len)
{
    ...
    if(!unicast_buffer) {
        flog_err(EC_BABEL_MEMORY,"D51q5UEa8B58Sdx8 " "malloc(unicast_buffer): %s",
		  safe_strerror(errno));
        return -1;
    }
...
void flush_unicast(int dofree)
{
    ...
    if(check_bucket(unicast_neighbour->ifp)) {
        ...
        rc = babel_send(protocol_socket,
                        ...
                        (struct sockaddr*)&sin6, sizeof(sin6));
        if(rc < 0)
            flog_err(EC_BABEL_PACKET,"TZGcVvBcMHbyULRt " "send(unicast): %s",
		      safe_strerror(errno));
    } else {
	    flog_err(EC_BABEL_PACKET,"8FV5PxptH1WY9dtH "
              ...
              const unsigned char *prefix, unsigned char plen)
{
    ...
    again:
        ...
        if(babel_ifp->buffered_updates == NULL) {
            flog_err(EC_BABEL_MEMORY,"XHTuLJ0WYFhbClAU " "malloc(buffered_updates): %s",
		      safe_strerror(errno));
            if(n > 4) {
            ...
            const unsigned char *prefix, unsigned char plen)
{
    ...
    if(prefix) {
        ...
        if(routes) {
            ...
            flog_err(EC_BABEL_MEMORY,"3BNYlkblo1cMohi2 " "Couldn't allocate route stream.");
        }
...
void send_self_update(struct interface *ifp)
{
    ...
    if(xroutes) {
        ...
        flog_err(EC_BABEL_MEMORY,"a2p7fhrxvbKwf28z " "Couldn't allocate xroute stream.");
    }
}
