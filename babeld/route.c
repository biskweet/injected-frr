...
install_route(struct babel_route *route)
{
    ...
    if(!route_feasible(route))
	    flog_err(EC_BABEL_ROUTE,"BZd06yArJswAPAYW "
    ...
    if(routes[i] != route && routes[i]->installed) {
	    flog_err(
		    EC_BABEL_ROUTE,"zFarx6JQK2YXOg6h "
    ...
    if(rc < 0) {
        int save = errno;
        flog_err(EC_BABEL_ROUTE,"TNzpAoD6wqwEwtbO " "kernel_route(ADD): %s",
		  safe_strerror(errno));
        if(save != EEXIST)
            return;
    }
...
uninstall_route(struct babel_route *route)
{
    ...
    rc = kernel_route(ROUTE_FLUSH, route->src->prefix, route->src->plen,
                      ...
                      metric_to_kernel(route_metric(route)), NULL, 0, 0);
    if(rc < 0)
        flog_err(EC_BABEL_ROUTE,"MliQRGgIP6A8f7re " "kernel_route(FLUSH): %s",
		  safe_strerror(errno));
...
switch_routes(struct babel_route *old, struct babel_route *new)
{
    ...
    if(!route_feasible(new))
	    flog_err(EC_BABEL_ROUTE,"gx5ZbgapNrSK1fY0 "
    ...
    if(rc < 0) {
        flog_err(EC_BABEL_ROUTE,"2PShOzvyRAvmN8yq " "kernel_route(MODIFY): %s",
		  safe_strerror(errno));
        return;
    }
                    ...
                    unsigned refmetric, unsigned cost, unsigned add)
{
    ...
    if(route->installed && old != new) {
        ...
        rc = kernel_route(ROUTE_MODIFY, route->src->prefix, route->src->plen,
                          ...
                          new);
        if(rc < 0) {
            flog_err(EC_BABEL_ROUTE,"XELLjBDDamJioM9f " "kernel_route(MODIFY metric): %s",
		      safe_strerror(errno));
            return;
        }
    }
             ...
             const unsigned char *channels, int channels_len)
{
    ...
    if(martian_prefix(prefix, plen)) {
        flog_err(EC_BABEL_ROUTE,"wwnxORORNDSnMZs5 " "Rejecting martian route to %s through %s.",
    ...
    if(route) {
        ...
        if(new_route == NULL) {
            flog_err(EC_BABEL_ROUTE,"ZU1HsRpmasvKocMr " "Couldn't insert route.");
            free(route);
            return NULL;
        }
