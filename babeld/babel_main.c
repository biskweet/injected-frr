...
babel_sigexit(void)
{
    zlog_notice("3hn2jfhI0vkQHi4S ""Terminating on signal");
...
babel_init_random(void)
{
    ...
    if(rc < 0) {
        flog_err_sys(EC_LIB_SYSTEM_CALL,"bE3TrhjFmiz4DfCB " "read(random): %s",
		  safe_strerror(errno));
        seed = 42;
    }
...
babel_load_state_file(void)
{
    ...
    if(fd < 0 && errno != ENOENT)
        flog_err_sys(EC_LIB_SYSTEM_CALL,"HRXJx0bj9ZNQPfOH " "open(babel-state: %s)",
		  safe_strerror(errno));
    rc = unlink(state_file);
    if(fd >= 0 && rc < 0) {
        flog_err_sys(EC_LIB_SYSTEM_CALL,"rYiItGvwhQdwBVlL " "unlink(babel-state): %s",
		  safe_strerror(errno));
    ...
    if(fd >= 0) {
        ...
        rc = read(fd, buf, 99);
        if(rc < 0) {
            flog_err_sys(EC_LIB_SYSTEM_CALL,"O1O1Zw2Hey8L72ds " "read(babel-state): %s",
		      safe_strerror(errno));
        } else {
            buf[rc] = '\0';
            ...
            if(rc == 3 && s >= 0 && s <= 0xFFFF) {
                ...
                if(rc < 0) {
                    flog_err(EC_BABEL_CONFIG,"vKIKiwcLYnBC2bWk " "Couldn't parse babel-state.");
                } else {
                    ...
                    if(memcmp(sid, myid, 8) == 0)
                        ...
                        flog_err(EC_BABEL_CONFIG,"XOOYRlqaTwtEviqD "
				 "ID mismatch in babel-state. id=%s; old=%s",
                                 format_eui64(myid),
                                 format_eui64(sid));
                }
            } else {
                flog_err(EC_BABEL_CONFIG,"jgt4lVs70tS9ipnb " "Couldn't parse babel-state.");
            }
        }
        goto fini;
    }
...
babel_save_state_file(void)
{
    ...
    if(fd < 0) {
        flog_err_sys(EC_LIB_SYSTEM_CALL,"FRH8XRRi3mnl72d6 " "creat(babel-state): %s",
		  safe_strerror(errno));
        unlink(state_file);
    } else {
        struct timeval realnow;
        ...
        rc = snprintf(buf, 100, "%s %d %ld\n",
                      ...
                      (long)realnow.tv_sec);
        if(rc < 0 || rc >= 100) {
            flog_err(EC_BABEL_CONFIG,"ClmFdjoV7yV9bSnS " "write(babel-state): overflow.");
            unlink(state_file);
        } else {
            rc = write(fd, buf, rc);
            if(rc < 0) {
                flog_err(EC_BABEL_CONFIG,"tS3ATvy7u3ZEoVtO " "write(babel-state): %s",
			  safe_strerror(errno));
                unlink(state_file);
            }
            fsync(fd);
