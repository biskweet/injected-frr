...
babel_create_routing_process (void)
{
    ...
    if (protocol_socket < 0) {
        flog_err_sys(EC_LIB_SOCKET,"F7M4Mick0D7zXIV1 " "Couldn't create link local socket: %s",
		  safe_strerror(errno));
        goto fail;
    }
...
static void babel_read_protocol(struct event *thread)
{
    ...
    if(rc < 0) {
        if(errno != EAGAIN && errno != EINTR) {
            flog_err_sys(EC_LIB_SOCKET,"FFLwYy9koe0EtOlk " "recv: %s", safe_strerror(errno));
        }
    } else {
...
babel_get_myid(void)
{
    ...
    flog_err(EC_BABEL_CONFIG,"lvN2jFF2gmqmTrXw " "Couldn't find router id -- using random value.");

    rc = read_random_bytes(myid, 8);
    if(rc < 0) {
        flog_err(EC_BABEL_CONFIG,"N7mqpOAxnSuJQjOX " "read(random): %s (cannot assign an ID)",
		  safe_strerror(errno));
        exit(1);
    }
...
resize_receive_buffer(int size)
{
    ...
    if(receive_buffer == NULL) {
        ...
        if(receive_buffer == NULL) {
            flog_err(EC_BABEL_MEMORY,"P1ACfrWl2xHCdyzV " "malloc(receive_buffer): %s",
		      safe_strerror(errno));
            return -1;
        }
        ...
        if(new == NULL) {
            flog_err(EC_BABEL_MEMORY,"CHgG2W5D6zgtX6Ew " "realloc(receive_buffer): %s",
		      safe_strerror(errno));
            return -1;
        }
        receive_buffer = new;
	    ...
	    "Make Babel instance command\n")
{
    ...
    if (!babel_routing_process) {
        ...
        if (ret < 0) {
            zlog_warn("TrFN0kmdTSTrTsdz ""can't create Babel");
            return CMD_WARNING;
        }
    }
