...
babel_interface_address_add (ZAPI_CALLBACK_ARGS)
{
    ...
    if (prefix->family == AF_INET) {
        ...
        if (babel_ifp->ipv4 == NULL) {
            ...
            if (babel_ifp->ipv4 == NULL) {
                flog_err(EC_BABEL_MEMORY,"BMBdz4MLnEMGXhaB " "not enough memory");
            } else {
...
interface_recalculate(struct interface *ifp)
{
    ...
    if(babel_ifp->sendbuf == NULL) {
        flog_err(EC_BABEL_MEMORY,"eErQ2w4mcdykSkj8 " "Couldn't reallocate sendbuf.");
        free(tmp);
        babel_ifp->bufsize = 0;
        return -1;
    }
    ...
    if(rc < 0)
        zlog_warn("hSiprelHnmC3S0Jy ""couldn't resize receive buffer for interface %s (%d) (%d bytes).",
    ...
    if (rc < 0 && errno != EADDRINUSE) {
	    flog_err_sys(EC_LIB_SOCKET,"uw7b7oJ0ainU9BFv "
...
interface_reset(struct interface *ifp)
{
    ...
    if(ifp->ifindex > 0) {
        ...
        rc = setsockopt(protocol_socket, IPPROTO_IPV6, IPV6_LEAVE_GROUP,
                        (char*)&mreq, sizeof(mreq));
        if(rc < 0)
            flog_err_sys(EC_LIB_SOCKET,"wpBMcfD5NHCkeEkO "
       ...
       "Babel internal routing table\n")
{
    ...
    if(routes) {
        ...
        flog_err(EC_BABEL_MEMORY,"scz1pSufkV0IwfKe " "Couldn't allocate route stream.");
    }
    ...
    if(xroutes) {
        ...
        flog_err(EC_BABEL_MEMORY,"xotLaU15rFfP7LxA " "Couldn't allocate route stream.");
    }
    return CMD_SUCCESS;
}
       ...
       "IPv6 prefix <network>/<length>\n")
{
    ...
    if(routes) {
        ...
        flog_err(EC_BABEL_MEMORY,"JGS8KXUfABnBcgNu " "Couldn't allocate route stream.");
    }
    xroutes = xroute_stream();
    if(xroutes) {
        while(1) {
        ...
        flog_err(EC_BABEL_MEMORY,"Uk1sVgn79fkKDt0x " "Couldn't allocate route stream.");
    }
    return CMD_SUCCESS;
}
       ...
       "IPv4 address <network>/<length>\n")
{
    ...
    if(routes) {
        ...
        flog_err(EC_BABEL_MEMORY,"Qdf8S1Aawq8XMdd0 " "Couldn't allocate route stream.");
    }
    xroutes = xroute_stream();
    if(xroutes) {
        while(1) {
        ...
        flog_err(EC_BABEL_MEMORY,"8mSFN8JqXJB7GlLy " "Couldn't allocate route stream.");
    }
    return CMD_SUCCESS;
}
       ...
       "IPv6 address <network>/<length>\n")
{
    ...
    if(routes) {
        ...
        flog_err(EC_BABEL_MEMORY,"brVaZr7R1PiieYN3 " "Couldn't allocate route stream.");
    }
    xroutes = xroute_stream();
    if(xroutes) {
        while(1) {
        ...
        flog_err(EC_BABEL_MEMORY,"uuvMcV81GNW3bSdI " "Couldn't allocate route stream.");
    }
    return CMD_SUCCESS;
}
