...
static void zebra_vrf_add_update(struct zebra_vrf *zvrf)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("frZzvs7N39QQPVrV ""MESSAGE: ZEBRA_VRF_ADD %s", zvrf_name(zvrf));
...
static void zebra_vrf_delete_update(struct zebra_vrf *zvrf)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("P5iKElKm8YVLf967 ""MESSAGE: ZEBRA_VRF_DELETE %s", zvrf_name(zvrf));
...
static int zebra_vrf_new(struct vrf *vrf)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("Cg1avqMqUw3DJqFp ""VRF %s created, id %u", vrf->name, vrf->vrf_id);
...
static int zebra_vrf_enable(struct vrf *vrf)
{
	...
	assert(zvrf);
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("9bFx1lfLh6izWXxR ""VRF %s id %u is now active", zvrf_name(zvrf),
			   zvrf_id(zvrf));
...
static int zebra_vrf_disable(struct vrf *vrf)
{
	...
	assert(zvrf);
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("vrPrhcQv9dqs9sS5 ""VRF %s id %u is now inactive", zvrf_name(zvrf),
			   zvrf_id(zvrf));
...
static int zebra_vrf_delete(struct vrf *vrf)
{
	...
	assert(zvrf);
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("omvGAaDtyIOrjiVa ""VRF %s id %u deleted", zvrf_name(zvrf),
			   zvrf_id(zvrf));
				   ...
				   ns_id_t rel_def_ns_id)
{
	...
	if (vrf->vrf_id != VRF_UNKNOWN && vrf->ns_ctxt == NULL) {
		if (vty)
			...
			zlog_info("kXbvxCNWNWGpGSP0 ""VRF %u is already configured with VRF %s",
				  vrf->vrf_id, vrf->name);
		return CMD_WARNING_CONFIG_FAILED;
	}
	if (vrf->ns_ctxt != NULL) {
	...
	if (ns && ns->vrf_ctxt) {
		...
		if (vty)
			...
			zlog_info("XiyfT1zDZ3EqK0NQ ""NS %s is already configured with VRF %u(%s)",
				  ns->name, vrf2->vrf_id, vrf2->name);
		return CMD_WARNING_CONFIG_FAILED;
	}
	...
	if (!ns_enable(ns, vrf_update_vrf_id)) {
		if (vty)
			...
			zlog_info("JrVNPWAc0U276Rem ""Can not associate NS %u with NETNS %s",
				  ns->ns_id, ns->name);
		return CMD_WARNING_CONFIG_FAILED;
	}
