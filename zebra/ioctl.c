...
int if_ioctl(unsigned long request, caddr_t buffer)
{
	...
	frr_with_privs(&zserv_privs) {
		...
		if (sock < 0) {
			zlog_err("KNVZAKOM3l0kVHut ""Cannot create UDP socket: %s",
				 safe_strerror(errno));
			exit(1);
		}
...
int vrf_if_ioctl(unsigned long request, caddr_t buffer, vrf_id_t vrf_id)
{
	...
	frr_with_privs(&zserv_privs) {
		...
		if (sock < 0) {
			zlog_err("ymTjWQwRbDc9oWvj ""Cannot create UDP socket: %s",
				 safe_strerror(errno));
			exit(1);
		}
...
static int if_ioctl_ipv6(unsigned long request, caddr_t buffer)
{
	...
	frr_with_privs(&zserv_privs) {
		...
		if (sock < 0) {
			zlog_err("7fd7PESk8cHJMxiG ""Cannot create IPv6 datagram socket: %s",
				 safe_strerror(errno));
			exit(1);
		}
...
void if_get_mtu(struct interface *ifp)
{
	...
	if (vrf_if_ioctl(SIOCGIFMTU, (caddr_t)&ifreq, ifp->vrf->vrf_id) < 0) {
		zlog_info("XsClyUGF5IxxnLQI ""Can't lookup mtu by ioctl(SIOCGIFMTU) for %s(%u)",
...
#else
	zlog_info("P91WHsBYkFg0nEDl ""Can't lookup mtu on this system for %s(%u)", ifp->name,
		  ifp->vrf->vrf_id);
	ifp->mtu6 = ifp->mtu = -1;
#endif
}
	...
	struct zebra_dplane_ctx *ctx)
{
	...
	if (dplane_ctx_get_op(ctx) == DPLANE_OP_ADDR_INSTALL) {
		...
		if (IS_ZEBRA_DEBUG_DPLANE)
			zlog_debug("jTo1I6FOvucGnlRM ""Invalid op in interface-addr install");
	}
...
void if_get_flags(struct interface *ifp)
{
	...
	ret = vrf_if_ioctl(SIOCGIFFLAGS, (caddr_t)&ifreqflags,
			   ifp->vrf->vrf_id);
	if (ret < 0) {
		flog_err_sys(EC_LIB_SYSTEM_CALL,"OEaz0VbewJnIz3i6 "
	...
	if (ret == -1)
		...
		flog_err_sys(EC_LIB_SYSTEM_CALL,"SnzeO0L8vWcivQIa "
	...
	if (if_ioctl(SIOCGIFMEDIA, (caddr_t)&ifmr) == -1) {
		if (errno != EINVAL)
			flog_err_sys(EC_LIB_SYSTEM_CALL,"spGEZnkQpJYKCp1k "
...
int if_set_flags(struct interface *ifp, uint64_t flags)
{
	...
	if (ret < 0) {
		zlog_info("ODkQu1Rx6p31cT77 ""can't set interface %s(%u) flags %" PRIu64,
...
int if_unset_flags(struct interface *ifp, uint64_t flags)
{
	...
	if (ret < 0) {
		zlog_warn("4pGnuMKr56sinsjc ""can't unset interface %s(%u) flags %" PRIu64,
			  ifp->name, ifp->vrf->vrf_id, flags);
		return ret;
	}
	return 0;
