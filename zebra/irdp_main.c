...
int irdp_sock_init(void)
{
	...
	if (sock < 0) {
		flog_err_sys(EC_LIB_SOCKET,"QYmUIU2GKsXY5q6L " "IRDP: can't create irdp socket %s",
	...
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"ceIh80p3DXC8xTSI " "IRDP: can't do irdp sockopt %s",
			     safe_strerror(errno));
		close(sock);
		return ret;
	};
	...
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"OxY8v0nWNosg20Fm " "IRDP: can't do irdp sockopt %s",
			     safe_strerror(errno));
		close(sock);
		return ret;
	};
...
void irdp_send_thread(struct event *t_advert)
{
	...
	if (irdp->flags & IF_DEBUG_MISC)
		zlog_debug("tY4kpAPmOFJZiETx ""IRDP: New timer for %s set to %u", ifp->name,
			   timer);
...
static int irdp_finish(void)
{
	...
	zlog_info("WQplu2lEDN15h0rR ""IRDP: Received shutdown notification.");
