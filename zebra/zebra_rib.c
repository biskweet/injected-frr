...
#define rnode_debug(node, vrf_id, msg, ...)                                    \
	zlog_debug("DOcMtTBshqxHMqBj ""%s: (%u:%pZNt):%pZN: " msg, __func__, vrf_id, node, node,  \
		   ##__VA_ARGS__)

#define rnode_info(node, vrf_id, msg, ...)                                     \
	zlog_info("UzVgHQ0CP65S1jdX ""%s: (%u:%pZNt):%pZN: " msg, __func__, vrf_id, node, node,   \
		  ##__VA_ARGS__)
			   ...
			   struct nhg_hash_entry *new_entry)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED || IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("iw4wMeZPtvnHXtWI ""%s: replacing routes nhe (%u) OLD %p NEW %p",
					...
					struct route_node **rn_out)
{
	...
	if (IS_ZEBRA_DEBUG_RIB) {
		...
		zlog_debug("OXq14WJj0BPXx4ft ""%s: %s: %pRN vrf: %s(%u) found %s, using %s",
			...
			struct route_entry *old)
{
	...
	{
		flog_err(EC_ZEBRA_DP_INSTALL_FAIL,"uNsXqZn6ZEeFDzwB "
...
void rib_uninstall_kernel(struct route_node *rn, struct route_entry *re)
{
	...
	case ZEBRA_DPLANE_REQUEST_QUEUED:
		...
		break;
	case ZEBRA_DPLANE_REQUEST_FAILURE:
		flog_err(EC_ZEBRA_DP_INSTALL_FAIL,"3NgvH7ny3GtX7toA "
				    ...
				    bool rt_delete)
{
	...
	while (rn) {
		...
		if (rt_delete && (!dest || !rnh_list_count(&dest->nht))) {
			if (IS_ZEBRA_DEBUG_NHT_DETAILED)
				zlog_debug("QE62Rz7xcn1qnMeM ""%pRN has no tracking NHTs. Bailing",
					   rn);
			break;
		}
		if (!dest) {
				...
				struct route_entry *new)
{
	...
	if (IS_ZEBRA_DEBUG_RIB)
		zlog_debug("rA7s5pbIGVnabXoa ""%s(%u:%u):%pRN: Adding route rn %p, re %p (%s)",
				...
				struct route_entry *old)
{
	...
	if (IS_ZEBRA_DEBUG_RIB)
		zlog_debug("2GwiKvvMMeVrJKa7 ""%s(%u:%u):%pRN: Deleting route rn %p, re %p (%s)",
...
static void rib_process(struct route_node *rn)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED) {
		...
		zlog_debug("Yi6nHX9lurqLOwf7 ""%s(%u:%u):%pRN: Processing rn %p",
	...
	for (ALL_NEXTHOPS_PTR(re_nhg, nexthop)) {
		...
		if (!nexthop_same(ctx_nexthop, nexthop)) {
			...
			if (IS_ZEBRA_DEBUG_RIB_DETAILED ||
				...
				zlog_debug("CuQKjYfcySY7Lpde ""%s: no ctx match for rib nh %pNHv %s",
					   __func__, nexthop,
		...
		if (CHECK_FLAG(ctx_nexthop->flags, NEXTHOP_FLAG_FIB)) {
			if (!CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_FIB)) {
				if (IS_ZEBRA_DEBUG_NHG_DETAIL)
					zlog_debug("A0NouCDSNxShGqJ4 ""%s: rib nh %pNHv -> installed",
						   __func__, nexthop);
			...
			if (CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_FIB)) {
				if (IS_ZEBRA_DEBUG_NHG_DETAIL)
					zlog_debug("DIsmNMjKBRKDYMHp ""%s: rib nh %pNHv -> uninstalled",
						   __func__, nexthop);
				   ...
				   struct zebra_dplane_ctx *ctx)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED)
		zlog_debug("dlRPi0d67lBwUuna ""update_from_ctx: %s(%u:%u):%pRN: %sSELECTED, re %p",
...
static void rib_process_result(struct zebra_dplane_ctx *ctx)
{
	...
	if (op == DPLANE_OP_ROUTE_INSTALL || op == DPLANE_OP_ROUTE_UPDATE) {
		if (status == ZEBRA_DPLANE_REQUEST_SUCCESS) {
			...
			zlog_warn("tn3omO9w8RHjor39 ""%s(%u:%u):%pRN: Route install failed",
				  VRF_LOGNAME(vrf), dplane_ctx_get_vrf(ctx),
		...
		if (status == ZEBRA_DPLANE_REQUEST_SUCCESS) {
			...
			zlog_warn("sIFE1foEyHp1ijtP ""%s(%u:%u):%pRN: Route Deletion failure",
				  VRF_LOGNAME(vrf), dplane_ctx_get_vrf(ctx),
				  dplane_ctx_get_table(ctx), rn);
		}
...
static void rib_process_dplane_notify(struct zebra_dplane_ctx *ctx)
{
	...
	if (debug_p)
		zlog_debug("vr7n607LJkQdNpkl ""%s(%u:%u):%pRN Processing dplane notif ctx %p",
			   VRF_LOGNAME(vrf), dplane_ctx_get_vrf(ctx),
...
static void process_subq_nhg(struct listnode *lnode)
{
	...
	if (w->type == WQ_NHG_WRAPPER_TYPE_CTX) {
		...
		if (IS_ZEBRA_DEBUG_RIB_DETAILED)
			zlog_debug("rUwplwUMCekvddo3 ""NHG %u dequeued from sub-queue %s", nhe->id,
				   subqueue2str(qindex));
...
static void process_subq_route(struct listnode *lnode, uint8_t qindex)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED) {
		...
		zlog_debug("JFtRogTQ4alIYqFs ""%s(%u:%u):%pRN rn %p dequeued from sub-queue %s",
...
static void process_subq_early_route_add(struct zebra_early_route *ere)
{
	...
	if (re->nhe_id > 0) {
		...
		if (!nhe) {
			 ...
			 */
			flog_err(
				EC_ZEBRA_TABLE_LOOKUP_FAILED,"TY5AKLQZZbxwTkH7 "
		...
		if (!nhe) {
			...
			flog_err(
				EC_ZEBRA_TABLE_LOOKUP_FAILED,"nNaezKNG1LGH14hO "
...
static void process_subq_early_route_delete(struct zebra_early_route *ere)
{
	...
	if (!rn) {
		if (IS_ZEBRA_DEBUG_RIB) {
			...
			zlog_debug("4z0Pwloq0cwUSoPj ""%s[%d]:%pRN%s%s doesn't exist in rib",
...
static int rib_meta_queue_nhg_ctx_add(struct meta_queue *mq, void *data)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED)
		zlog_debug("hNPhXFuPAwYFs8hS ""NHG Context id=%u queued into sub-queue %s",
			   ctx->id, subqueue2str(qindex));
				      ...
				      bool deletion)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED)
		zlog_debug("PF51novUhfDe9O8J ""NHG id=%u queued into sub-queue %s", nhe->id,
			   subqueue2str(qindex));
			  ...
			  int (*mq_add_func)(struct meta_queue *mq, void *data))
{
	if (zrouter.ribq == NULL) {
		flog_err(EC_ZEBRA_WQ_NONEXISTENT,"rmeQ9KaMVSRr6ahZ "
			...
			uint8_t route_instance)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED)
		zlog_debug("wUZgIPiYK90AGDlb ""Early Label Handling for %pFX", prefix);
			      ...
			      const struct zapi_labels *zl)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED)
		zlog_debug("b6JgUSywtjsDpE6z ""Early Label Handling: Labels Process");
...
int rib_queue_add(struct route_node *rn)
{
	 ...
	 */
	if (!rnode_to_ribs(rn)) {
		zlog_debug("U3BWHSwtCwVv6qtz ""%s: called for route_node (%p, %u) with no ribs",
				   ...
				   const struct prefix *host_prefix)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED)
		zlog_debug("bDZdJfVItD1nff67 ""%s: (%u)%pIA, host prefix %pFX enqueued", __func__,
				   ...
				   const struct prefix *host_prefix)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED)
		zlog_debug("A6vP3C8FxiTaZdtl ""%s: (%u)%pIA, host prefix %pFX enqueued", __func__,
			   vrf_id, vtep_ip, host_prefix);
				    ...
				    uint16_t df_pref)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED)
		zlog_debug("YU3vWJJAn05C89dJ ""%s: vtep %pI4, esi %s enqueued", __func__, vtep_ip,
				    ...
				    const struct in_addr *vtep_ip)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED) {
		...
		zlog_debug("BBf1G66xQTud8O0M ""%s: vtep %pI4, esi %s enqueued", __func__, vtep_ip,
			   buf);
	}
				       ...
				       struct in_addr vtep_ip, const esi_t *esi)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED) {
		...
		zlog_debug("VIiKTfHlSRrznRzk ""%s: mac %pEA, vtep %pI4, esi %s enqueued", __func__,
			   macaddr, &vtep_ip, buf);
	}
				       ...
				       struct in_addr vtep_ip)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED)
		zlog_debug("S691OalcHE1On1fQ ""%s: mac %pEA, vtep %pI4 enqueued", __func__,
			   macaddr, &vtep_ip);
				      ...
				      struct in_addr vtep_ip, int flood_control)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED)
		zlog_debug("CfCc0INfuksujw2x ""%s: vrf %u, vtep %pI4 enqueued", __func__, vrf_id,
			   &vtep_ip);
				      ...
				      struct in_addr vtep_ip)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED)
		zlog_debug("5bzkJB7EkjhAZrMj ""%s: vrf %u, vtep %pI4 enqueued", __func__, vrf_id,
			   &vtep_ip);
...
static void rib_queue_init(void)
{
	if (!(zrouter.ribq = work_queue_new(zrouter.master,
		...
		flog_err(EC_ZEBRA_WQ_NONEXISTENT,"MecLc3bSi3EAZGjQ "
	...
	if (!(zrouter.mq = meta_queue_new())) {
		flog_err(EC_ZEBRA_WQ_NONEXISTENT,"Y59VLdz4dxK3nASf "
...
void rib_delnode(struct route_node *rn, struct route_entry *re)
{
	...
	if (is_zebra_import_table_enabled(afi, re->vrf_id, re->table)) {
		...
		if (IS_ZEBRA_DEBUG_RIB)
			zlog_debug("f4lviHl2SGkXExBN ""%s(%u):%pRN: Freeing route rn %p, re %p (%s)",
				 ...
				 const struct nexthop *nexthop)
{
	...
	zlog_debug("YRUdC7QsRCfLCmh3 ""%s: %s %s[%u] %svrf %s(%u) %s%s with flags %s%s%s%s%s%s%s%s%s",
		       ...
		       const struct route_entry *re)
{
	...
	zlog_debug("WIXuH27dbP4RZasP ""%s: dumping RE entry %p for %s%s%s vrf %s(%u)", func,
		   (const void *)re, straddr,
		   is_srcdst ? " from " : "",
		   is_srcdst ? prefix2str(src_pp, srcaddr, sizeof(srcaddr))
			     : "",
		   VRF_LOGNAME(vrf), re->vrf_id);
	zlog_debug("TdoCrMoSErRNouz1 ""%s: uptime == %lu, type == %u, instance == %d, table == %d",
		   straddr, (unsigned long)re->uptime, re->type, re->instance,
		   re->table);
	zlog_debug("IVVccvDNw9KaO1Jr "
	...
	zlog_debug("qBGOK0PB3kZVOXf2 ""%s: tag == %u, nexthop_num == %u, nexthop_active_num == %u",
	...
	if (zebra_nhg_get_backup_nhg(re->nhe)) {
		zlog_debug("ZhOu5WQ4Sdnbg0Xp ""%s: backup nexthops:", straddr);
	...
	zlog_debug("WPpIPpP7n2zDfTh0 ""%s: dump complete", straddr);
}
...
static int rib_meta_queue_gr_run_add(struct meta_queue *mq, void *data)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED)
		zlog_debug("VSSE4Fnatp4RYjF2 ""Graceful Run adding");
...
static int rib_meta_queue_early_route_add(struct meta_queue *mq, void *data)
{
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED)
		zlog_debug("qb6lum2mJkPYIsek ""Route %pFX(%u) (%s) queued for processing into sub-queue %s",
			   &ere->p, ere->re->vrf_id,
		      ...
		      int rtype)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT) {
		...
		zlog_debug("ZROzzytsd3mxkjTu ""%s: %s VRF %s Table %u event %s Route type: %s", __func__,
			   table->info ? afi2str(
...
static void rib_update_handle_vrf_all(enum rib_update_event event, int rtype)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("i8RTzldOvD7h2jMP ""%s: Handling VRF (ALL) event %s", __func__,
...
void rib_update(enum rib_update_event event)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("lcyCXFSkwSUR452i ""%s: Scheduled VRF (ALL), event %s", __func__,
			   rib_update_event2str(event));
}
...
void rib_sweep_table(struct route_table *table)
{
	...
	if (IS_ZEBRA_DEBUG_RIB)
		zlog_debug("f1X31MZmRNI5RrQu ""%s: starting", __func__);
	...
	if (IS_ZEBRA_DEBUG_RIB)
		zlog_debug("Cm32ahr8xYPvyQRd ""%s: ends", __func__);
}
