			...
			char *buf)
{
	...
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"8fpTFoBAynguiRMF " "netlink( %u) sendmsg() error: %s",
	...
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"ni7sLBtvEMjoCS8m "
			     "netlink recvmsg: error %d (errno %u)", ret,
			     errno);
		return -1;
	}
	if (msg.msg_flags & MSG_TRUNC) {
		flog_err(EC_ZEBRA_NETLINK_LENGTH_ERROR,"Jnht8vndPyjZALEO "
	...
	if (nlh->nlmsg_seq != seq) {
		flog_err(
			EC_ZEBRA_NETLINK_BAD_SEQUENCE,"0Qmh9jbyiLH9PECc "
...
ns_id_t zebra_ns_id_get(const char *netnspath, int fd_param)
{
	...
	if (sock < 0) {
		flog_err_sys(EC_LIB_SOCKET,"UfHAqTiDZgV35W9l " "netlink( %u) socket() error: %s",
			     sock, safe_strerror(errno));
		if (netnspath)
			close(fd);
		return NS_UNKNOWN;
	...
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"mJwbjqRJUQiibSJF "
	...
	if (errno != EEXIST && ret != 0) {
		flog_err(EC_LIB_SOCKET,"uIfK1QOfrPpGnwLx "
		...
		if (netnspath)
			close(fd);
		if (errno == ENOTSUP) {
			zlog_debug("SZxSt9Vg8Ke5NBcg ""NEWNSID locally generated");
...
static void zebra_ns_create_netns_directory(void)
{
	...
	if (mkdir(NS_RUN_DIR, 0755)) {
		if (errno != EEXIST) {
			flog_warn(EC_ZEBRA_NAMESPACE_DIR_INACCESSIBLE,"CIo7dp8TEcJhO52s "
