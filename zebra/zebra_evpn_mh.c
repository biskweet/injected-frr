						       ...
						       struct zebra_evpn *zevpn)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("lFUUowcBtM01byH7 ""es %s evi %d new",
...
static void zebra_evpn_es_evi_free(struct zebra_evpn_es_evi *es_evi)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("mTCFUwapzSUx2ztK ""es %s evi %d free",
				es_evi->es->esi_str, es_evi->zevpn->vni);
...
static void zebra_evpn_local_es_evi_do_del(struct zebra_evpn_es_evi *es_evi)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("wt12A6fEkI2gZFT7 ""local es %s evi %d del",
				es_evi->es->esi_str, es_evi->zevpn->vni);
...
static void zebra_evpn_es_evi_mac_install(struct zebra_evpn_es_evi *es_evi)
{
	...
	if (listcount(es->mac_list) && IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("VJ7n0n2f5IhA5GJm ""dp-mac install on es %s evi %d add", es->esi_str,
			   es_evi->zevpn->vni);
					...
					struct zebra_evpn *zevpn)
{
	...
	if (!es_evi) {
		...
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("v6zb2ZywMo3ppXxP ""local es %s evi %d add",
					es_evi->es->esi_str, es_evi->zevpn->vni);
		es_evi->flags |= ZEBRA_EVPNES_EVI_LOCAL;
...
zebra_evpn_acc_vl_new(vlanid_t vid, struct interface *br_if)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("E9GZ9WvSDUFh8nuB ""%s access vlan %d bridge %s add", __func__, vid,
			   br_if->name);
	...
	if (vlan_if) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("BnIvMu0nV8EP86sk ""%s vlan %d bridge %s SVI %s set", __func__,
...
{
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("otvPV1vS6ijAn6Au ""access vlan %d del", acc_bd->vid);
			       ...
			       struct zebra_if *br_zif, bool is_up)
{
	...
	if (is_up) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("Zt9TUP1IufEinP1u ""vlan %d bridge %s SVI %s set", vid,
		...
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("mCU2qZIcm5jGSkiK ""vlan %d bridge %s SVI clear", vid,
				   tmp_br_zif->ifp->name);
				       ...
				       struct zebra_evpn *old_zevpn)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("caYv1Gc4OepNcP6D ""access vlan %d bridge %s l2-vni %u set",
			   ...
			   struct zebra_if *vxlan_zif)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("GKK9Gwx7rPzjTi0y ""%s bridge %s access vlan %d vni %u ref", __func__,
			     ...
			     struct zebra_if *vxlan_zif)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("WmjviPW9fpOpoAuq ""access vlan %d bridge %s vni %u deref", acc_bd->vid,
			   br_if->name, vni_id);
					 ...
					 struct zebra_evpn_access_bd *acc_bd)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("JMKSRmfXKOYuXpAC ""access bd vlan %d bridge %s cleanup", acc_bd->vid,
			   br_if->name);
...
void zebra_evpn_vl_mbr_ref(uint16_t vid, struct zebra_if *zif)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("dtJPQfjIvQKmj9w9 ""access vlan %d bridge %s mbr %s ref", vid,
...
void zebra_evpn_vl_mbr_deref(uint16_t vid, struct zebra_if *zif)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("VRvcfai1by8faKju ""access vlan %d bridge %s mbr %s deref", vid,
			   br_if->name, zif->ifp->name);
...
static void zebra_evpn_nhg_mac_update(struct zebra_evpn_es *es)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
		zlog_debug("gncZ6xAe8VF1qWhQ ""mac update on es %s nhg %s", es->esi_str,
...
static void zebra_evpn_nhg_update(struct zebra_evpn_es *es)
{
	...
	if (nh_cnt) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_NH) {
			...
			zlog_debug("eWDfzhz4Qm4oiy31 ""es %s nhg %u add %s", es->esi_str,
				   es->nhg_id, nh_str);
		}
		...
		if (es->flags & ZEBRA_EVPNES_NHG_ACTIVE) {
			if (IS_ZEBRA_DEBUG_EVPN_MH_NH)
				zlog_debug("sHGfu0Ta8agkY7hG ""es %s nhg %u del", es->esi_str,
					   es->nhg_id);
			es->flags &= ~ZEBRA_EVPNES_NHG_ACTIVE;
...
static void zebra_evpn_l2_nh_es_vtep_ref(struct zebra_evpn_es_vtep *es_vtep)
{
	...
	if (!es_vtep->nh) {
		zlog_warn("AdVGCq0E9yxTHA95 ""es %s vtep %pI4 nh ref failed", es_vtep->es->esi_str,
			  &es_vtep->vtep_ip);
		return;
	}
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_NH)
		zlog_debug("WU7QNQ4mh4ap43ET ""es %s vtep %pI4 nh %u ref %u", es_vtep->es->esi_str,
...
static void zebra_evpn_l2_nh_es_vtep_deref(struct zebra_evpn_es_vtep *es_vtep)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_NH)
		zlog_debug("HJZzeOMCN8x6i4kg ""es %s vtep %pI4 nh %u deref %u",
...
static bool zebra_evpn_es_br_port_dplane_clear(struct zebra_evpn_es *es)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("tHs2TouEdxsy76TL ""es %s br-port dplane clear", es->esi_str);
						...
						const char *caller)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("zA52jQ4v6NsXQeKI ""es %s br-port dplane update by %s", es->esi_str,
			   caller);
	...
	if (es->flags & ZEBRA_EVPNES_BYPASS) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("Y8ukptrubvfZ7v77 "
				"es %s SPH filter disabled as it is in bypass",
				es->esi_str);
	} else {
		if (listcount(es->es_vtep_list) > ES_VTEP_MAX_CNT) {
			zlog_warn("510RErk2Mnzw4xeq ""es %s vtep count %d exceeds filter cnt %d",
				    ...
				    const char *caller, const char *reason)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("iIXFjqcF8nwdOXGx ""df-change es %s %s to %s; %s: %s", es->esi_str,
				   ...
				   uint8_t df_alg, uint16_t df_pref)
{
	...
	if (!es_vtep) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("HOiTSxOfNFuQpOYW ""es %s vtep %pI4 add",
					es->esi_str, &vtep_ip);
		...
		struct in_addr vtep_ip)
{
	...
	if (es_vtep) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("e2KMmwdB6ePxumMG ""es %s vtep %pI4 del",
					es->esi_str, &vtep_ip);
...
static struct zebra_evpn_es *zebra_evpn_es_new(const esi_t *esi)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("nT4ZeHFH5cpbCfFC ""es %s nhg %u new", es->esi_str, es->nhg_id);
...
static void zebra_evpn_es_free(struct zebra_evpn_es **esp)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("wrW9VHS0TjZEs2k8 ""es %s free", es->esi_str);
...
static int zebra_evpn_es_send_del_to_client(struct zebra_evpn_es *es)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("FI6IZUnmeJyLXdZG ""send del local es %s to %s", es->esi_str,
				zebra_route_string(client->proto));
					   ...
					   struct interface *ifp, bool add)
{
	...
	for (ALL_LIST_ELEMENTS(es->mac_list, node, nnode, mac)) {
		 ...
		 * over */
		if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
			zlog_debug("ySl1jgARGf2KwomJ ""VNI %u mac %pEA update; local ES %s %s",
				   mac->zevpn->vni,
				   &mac->macaddr,
...
void zebra_evpn_es_local_br_port_update(struct zebra_if *zif)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("sfIwM46dgA72IeXw ""es %s br_port change old %u new %u", es->esi_str,
...
static void zebra_evpn_mh_advertise_reach_neigh_only(void)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("GmQnYbo6yTPHrFSq ""evpn-mh: only REACHABLE neigh advertised");
...
static void zebra_evpn_mh_advertise_svi_mac(void)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("VFAf5d40WyxvPfnk ""evpn-mh: advertise SVI MAC");
...
static void zebra_evpn_es_df_delay_exp_cb(struct event *t)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("oGGmVfJbJIYc5z95 ""es %s df-delay expired", es->esi_str);
		...
		struct zebra_if *zif)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("VYh0ZAq1mk0vSyFY ""local es %s add; nhg %u if %s", es->esi_str,
...
static void zebra_evpn_local_es_del(struct zebra_evpn_es **esp)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES) {
		zif = es->zif;
		zlog_debug("fVa0j6a3iFt01MKs ""local es %s del; nhg %u if %s", es->esi_str,
...
static void zebra_evpn_es_remote_info_re_eval(struct zebra_evpn_es **esp)
{
	...
	if (listcount(es->es_vtep_list)) {
		if (!(es->flags & ZEBRA_EVPNES_REMOTE)) {
			...
			if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
				zlog_debug("UWiX64ixGKOpJ7EZ ""remote es %s add; nhg %u",
		...
		if (es->flags & ZEBRA_EVPNES_REMOTE) {
			es->flags &= ~ZEBRA_EVPNES_REMOTE;
			if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
				zlog_debug("bAHIkcjdEatWmikb ""remote es %s del; nhg %u",
					   es->esi_str, es->nhg_id);
			zebra_evpn_es_free(esp);
		}
	}
...
int zebra_evpn_remote_es_del(const esi_t *esi, struct in_addr vtep_ip)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("Uryw3LIopA3af1bQ ""remote es %s vtep %pI4 del",
			   esi_to_str(esi, buf, sizeof(buf)), &vtep_ip);

	es = zebra_evpn_es_find(esi);
	if (!es) {
		zlog_warn("zNoZWNpxBRls2ibg ""remote es %s vtep %pI4 del failed, es missing",
			  esi_to_str(esi, buf, sizeof(buf)), &vtep_ip);
		return -1;
	}
...
static void zebra_evpn_remote_es_flush(struct zebra_evpn_es **esp)
{
	...
	for (ALL_LIST_ELEMENTS(es->es_vtep_list, node, nnode, es_vtep)) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("1hMkOTMSiGj2SB5Y ""es %s vtep %pI4 flush",
					es->esi_str,
					&es_vtep->vtep_ip);
		zebra_evpn_es_vtep_free(es_vtep);
	}
			     ...
			     bool esr_rxed, uint8_t df_alg, uint16_t df_pref)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("pJTPRGUn95NLdyro ""remote es %s vtep %pI4 add %s df_alg %d df_pref %d",
...
bool zebra_evpn_es_mac_ref(struct zebra_mac *mac, const esi_t *esi)
{
	...
	if (!es) {
		...
		if (memcmp(esi, zero_esi, sizeof(esi_t))) {
			es = zebra_evpn_es_new(esi);
			if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
				zlog_debug("LOV0nd0oH8Bi0GPP ""auto es %s add on mac ref",
					   es->esi_str);
		}
	}
					    ...
					    struct zebra_evpn *zevpn, bool add)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("xlJd6GYhIrYmmyPV ""send %s local es %s evi %u to %s",
				add ? "add" : "del",
					     ...
					     struct interface *ifp, bool bypass)
{
	...
	for (ALL_LIST_ELEMENTS(es->mac_list, node, nnode, mac)) {
		...
		if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
			zlog_debug("GWYrZzLSOYN1lPfo ""VNI %u mac %pEA %s update es %s",
				   mac->zevpn->vni,
				   &mac->macaddr,
	...
	for (ALL_LIST_ELEMENTS(zif->mac_list, node, nnode, mac)) {
		...
		if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
			zlog_debug("SeweiAhI7hgRtbGs ""VNI %u mac %pEA %s update ifp %s",
				   mac->zevpn->vni,
				   &mac->macaddr,
				 ...
				 struct interface *ifp, bool bypass)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("20xmhKVGaHu3k4SA ""bond %s es %s lacp bypass changed to %s", ifp->name,
...
static void zebra_evpn_local_mac_oper_state_change(struct zebra_evpn_es *es)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
		zlog_debug("ylJj6HXmQrvLgyiR ""mac slow-fail on es %s %s ", es->esi_str,
...
void zebra_evpn_es_if_oper_state_change(struct zebra_if *zif, bool up)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("JzCf110q67MY3iAw ""es %s state changed to %s ",
				es->esi_str,
				up ? "up" : "down");
	if (up)
		es->flags |= ZEBRA_EVPNES_OPER_UP;
...
void zebra_evpn_es_set_base_evpn(struct zebra_evpn *zevpn)
{
	...
	if (zmh_info->es_base_evpn) {
		...
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("5NmF3Uog4dFCpwvo ""es base vni set to %d",
					zevpn->vni);
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("dHKXIZtN61z9bRDw ""es originator ip set to %pI4",
...
void zebra_evpn_es_clear_base_evpn(struct zebra_evpn *zevpn)
{
	...
	if (!zmh_info->es_base_evpn && zmh_info->es_originator_ip.s_addr) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("6hC2eWTVRzFuiqLc ""es originator ip cleared");
					     ...
					     const char *caller)
{
	...
	if (zebra_if_update_protodown_rc(zif->ifp, new_protodown,
					 new_protodown_rc) == 0) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("WnYlvfig6tdtwEy5 ""%s protodown %s", zif->ifp->name,
...
static void zebra_evpn_mh_update_protodown(uint32_t protodown_rc, bool set)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("J69hmHISRk2gLliv ""mh protodown_rc changed; old 0x%x new 0x%x",
...
{
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("KqJN1WTaBgJDrNSN ""startup-delay expired");
...
static void zebra_evpn_mh_startup_delay_timer_start(const char *rc)
{
	if (zmh_info->startup_delay_timer) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("W1J0Rw1Msg3j57xT ""startup-delay timer cancelled");
		EVENT_OFF(zmh_info->startup_delay_timer);
	}

	if (zmh_info->startup_delay_time) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("Y3s8YKhjO44LehHG "
...
void zebra_evpn_proc_remote_nh(ZAPI_HANDLER_ARGS)
{
	...
	if (hdr->command == ZEBRA_EVPN_REMOTE_NH_ADD) {
		...
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("o2ItEH7f85pMAex4 ""evpn remote nh %d %pIA del pfx %pFX",
