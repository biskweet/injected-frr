...
static void if_zebra_speed_update(struct event *thread)
{
	...
	if (new_speed != ifp->speed) {
		zlog_info("FHaAxtoHRPoPXbPn ""%s: %s old speed: %u new speed: %u", __func__,
...
int if_subnet_delete(struct interface *ifp, struct connected *ifc)
{
	...
	if (!(rn && rn->info)) {
		flog_warn(EC_ZEBRA_REMOVE_ADDR_UNKNOWN_SUBNET,"TpadcMV0kv7JVJeR "
	...
	if (!listnode_lookup(addr_list, ifc)) {
		flog_warn(
			EC_ZEBRA_REMOVE_UNREGISTERED_ADDR,"lwVrXpJn0EHjfX0L "
		...
		if (CHECK_FLAG(ifc->conf, ZEBRA_IFC_CONFIGURED)
			...
			if (p->family == AF_INET) {
				...
				if (dplane_res ==
					...
					flog_err_sys(
						EC_ZEBRA_IFACE_ADDR_ADD_FAILED,"w5tiKBaKNSvD2LXA "
			...
			if (p->family == AF_INET6) {
				...
				if (dplane_res ==
				    ZEBRA_DPLANE_REQUEST_FAILURE) {
					flog_err_sys(
						EC_ZEBRA_IFACE_ADDR_ADD_FAILED,"CZEqdTyDFropRQjF "
						"Can't set interface's address: %s",
						dplane_res2str(dplane_res));
					continue;
				}
...
void if_add_update(struct interface *ifp)
{
	...
	if (!CHECK_FLAG(ifp->status, ZEBRA_INTERFACE_ACTIVE)) {
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("51ni1iZ2kCFyFu5v ""interface %s vrf %s(%u) index %d is added.",
				   ifp->name, ifp->vrf->name, ifp->vrf->vrf_id,
				   ifp->ifindex);
	}
}
...
void if_delete_update(struct interface **pifp)
{
	...
	if (if_is_up(ifp)) {
		flog_err(
			EC_LIB_INTERFACE,"CaWGICynBmsB5za7 "
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("SVtGQGZWz9zrcqEC ""interface %s vrf %s(%u) index %d is now inactive.",
			   ifp->name, ifp->vrf->name, ifp->vrf->vrf_id,
			   ifp->ifindex);
	...
	if (!ifp->configured) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("r4TFQF0hT35x06Oi ""interface %s is being deleted from the system",
				   ifp->name);
		if_delete(pifp);
	}
}
...
void if_up(struct interface *ifp, bool install_connected)
{
	...
	if (ifp->ptm_enable && (ifp->ptm_status == ZEBRA_PTM_STATUS_DOWN)) {
		flog_warn(EC_ZEBRA_PTM_NOT_READY,"uT1I0BWHSjZrcsLa "
...
void zebra_if_update_all_links(struct zebra_ns *zns)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_info("G9h5Zu8avevUsVT1 ""fixup link dependencies");
	...
	for (rn = route_top(zns->if_table); rn; rn = route_next(rn)) {
		...
		if ((IS_ZEBRA_IF_BOND_SLAVE(ifp))
			...
			if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug("UWEYq6mQ9gcD1pyR ""bond mbr %s map to bond %d",
					   zif->ifp->name,
		...
		if ((zif->link_ifindex != IFINDEX_INTERNAL) && !zif->link) {
			...
			if (IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug("QCiXzF1b5jrH4Yb4 ""interface %s/%d's lower fixup to %s/%d",
...
#else
	zlog_warn("MiRuck3GRpepC2IG ""Protodown is not supported on this platform");
#endif
	return 0;
}
				     ...
				     struct interface *ifp)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("iZCVQCggYYs2GHZD ""%s: %s: ifindex %s(%u), addr %pFX", __func__,
				...
				struct interface *ifp)
{
	...
	if (!ifp) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("PIxN9bPVp7slSVCw ""%s: Can't find ifp", __func__);
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("liHijlHRRis9BvG0 ""%s: %s: if %s(%u) ctx-protodown %s ctx-reason %d",
	...
	if (!zif) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("DciMRiPhaYppwMU4 ""%s: if %s(%u) zebra info pointer is NULL",
	...
	if (dp_res != ZEBRA_DPLANE_REQUEST_SUCCESS) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("mwdzIY2VMCYMXC9E ""%s: if %s(%u) dplane update failed",
				   __func__, ifp->name, ifp->ifindex);
		goto done;
	}
					...
					ifindex_t ifindex)
{
	...
	if (!ifp && ifindex != -1 && ifindex != -2) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("lqYh5m5kdUOqDrNp ""%s: Can't find ifp(%u)", __func__, ifindex);
				 ...
				 ns_id_t ns_id)
{
	...
	if (op == DPLANE_OP_INTF_DELETE) {
		if (IS_ZEBRA_DEBUG_DPLANE)
			zlog_debug("FuMoDPPfdn3T5TVn ""DPLANE_OP_INTF_DELETE for VRF %s(%u)", name,
				   ifindex);

		vrf = vrf_lookup_by_id((vrf_id_t)ifindex);
		if (!vrf) {
			flog_warn(EC_ZEBRA_VRF_NOT_FOUND,"4CxsLHE5lrO4RFv1 "
		...
		if (exist_id != VRF_DEFAULT) {
			...
			if (!vrf_lookup_by_id((vrf_id_t)ifindex) && !vrf) {
				flog_err(EC_ZEBRA_VRF_NOT_FOUND,"1ABNXviG19vapBgK "
			...
			if (vrf && strcmp(name, vrf->name)) {
				flog_err(EC_ZEBRA_VRF_MISCONFIGURED,"9b9TGWxfexbPBLkX "
		...
		if (!vrf) {
			flog_err(EC_LIB_INTERFACE,"sMFl018cPL1khYmZ " "VRF %s id %u not created",
				 name, ifindex);
			return;
		}
		...
		if (!vrf_enable(vrf)) {
			flog_err(EC_LIB_INTERFACE,"9hCbswHkFIR4JFCU "
				   ...
				   uint32_t rc_bitfield)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_DPLANE)
		zlog_debug("ZOsSiZ7InuJFzf5h ""interface %s dplane change, protodown %s curr reason_extern %u",
	...
	if (zebra_evpn_is_es_bond_member(ifp)) {
		...
		if (!protodown && reason_extern) {
			if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug("aYOoRyaiAEYKoh2U ""bond member %s has protodown reason external and clear the reason, skip reinstall.",
					   ifp->name);
			return;
		}

		if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("fuQdZzQjRckHhsgB "
...
static void if_sweep_protodown(struct zebra_if *zif)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("KX0UwEBbOrFQRBTJ ""interface %s sweeping protodown %s reason 0x%x",
					  ...
					  struct interface *ifp)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE)
		zlog_debug("RJrNQxwDIUHotAm9 ""Access VLAN %u for VxLAN IF %s(%u)", bvinfo->vid,
			   ifp->name, ifp->ifindex);
				      ...
				      enum zebra_iftype zif_type)
{
	...
	if (!ifp) {
		zlog_warn("7K3bMVw25QA5W9ng ""Cannot find bridge if %s(%u)",
...
static void zebra_if_dplane_ifp_handling(struct zebra_dplane_ctx *ctx)
{
	...
	zns = zebra_ns_lookup(ns_id);
	if (!zns) {
		zlog_err("HwPtgewQlQzb4rGA ""Where is our namespace?");
		return;
	}

	if (IS_ZEBRA_DEBUG_DPLANE)
		zlog_debug("9JKbuxYZE55YapsH ""%s for %s(%u)", dplane_op2str(op), name, ifindex);
...
void zebra_if_dplane_result(struct zebra_dplane_ctx *ctx)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL || IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("8b8M3bkKMDLzBgli ""Intf dplane ctx %p, op %s, ifindex (%u), result %s",
	...
	if (zns == NULL) {
		/* No ns - deleted maybe? */
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("KvqIHx898ezbZCDK ""%s: can't find zns id %u", __func__, ns_id);
...
void if_arp(struct interface *ifp, bool enable)
{
	...
	if (ret < 0) {
		zlog_debug("mwwI4eFtXvjQglmG ""Can't %sset noarp flag on interface %s",
			   enable ? "" : "un", ifp->name);
		return;
	}
...
int if_multicast_set(struct interface *ifp)
{
	...
	if (CHECK_FLAG(ifp->status, ZEBRA_INTERFACE_ACTIVE)) {
		if (if_set_flags(ifp, IFF_MULTICAST) < 0) {
			zlog_debug("PdNIFsinSgNN955X ""Can't set multicast flag on interface %s",
				   ifp->name);
			return -1;
		}
		if_refresh(ifp);
...
int if_multicast_unset(struct interface *ifp)
{
	...
	if (CHECK_FLAG(ifp->status, ZEBRA_INTERFACE_ACTIVE)) {
		if (if_unset_flags(ifp, IFF_MULTICAST) < 0) {
			zlog_debug("WAzWDqWE7ghdFOwY ""Can't unset multicast flag on interface %s",
				   ifp->name);
			return -1;
		}
		if_refresh(ifp);
...
int if_shutdown(struct interface *ifp)
{
	...
	if (ifp->ifindex != IFINDEX_INTERNAL) {
		...
		if (if_unset_flags(ifp, IFF_UP) < 0) {
			zlog_debug("Do57dENadpES2nKP ""Can't shutdown interface %s", ifp->name);
			return -1;
		}
		if_refresh(ifp);
	}
...
int if_no_shutdown(struct interface *ifp)
{
	...
	if (ifp->ifindex != IFINDEX_INTERNAL) {
		if (if_set_flags(ifp, IFF_UP | IFF_RUNNING) < 0) {
			zlog_debug("V6wxbNpSSZo9qckQ ""Can't up interface %s", ifp->name);
			return -1;
		}
		if_refresh(ifp);
