				    ...
				    uint16_t cmd)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("DIwERF1DRvvIaQmE ""Send ip prefix %pFX %s on vrf %s", p,
			    ...
			    struct ipaddr *ip)
{
	...
	if (!mac) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("W7M96zGWIs2ltCN5 ""MAC %pEA doesn't exist for neigh %pIA on VNI %u",
				   &n->emac, ip, zevpn->vni);
		return -1;
	}
					   ...
					   void *ctxt)
{
	...
	if (zevpn->advertise_gw_macip) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("798MqGU9lUOWUW6B ""VNI: %u GW-MACIP enabled, retain gw-macip", zevpn->vni);
		return;
	}
					    ...
					    void *ctxt)
{
	...
	if (zevpn->advertise_svi_macip) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("mhAQBexSK2RPVx1T ""VNI: %u SVI-MACIP enabled, retain svi-macip",
				   zevpn->vni);
		return;
	}
					    ...
					    struct interface *svi_if)
{
	...
	if (!svi_if) {
		zlog_debug("RNshD091mUIMXGlX ""svi_if is not passed.");
		return NULL;
	}
...
int zebra_evpn_send_del_to_client(struct zebra_evpn *zevpn)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("Ie7NBJC13cN7K6Ac ""Send EVPN_DEL %u to %s", zevpn->vni,
			   zebra_route_string(client->proto));
...
{
	if (!zevpn->vxlan_if) {
		zlog_debug("oSi6Mf3wM3YbI2rj ""VNI %u hash %p couldn't be uninstalled - no intf",
			   zevpn->vni, zevpn);
		return -1;
	}
			      ...
			      struct in_addr vtep_ip, const esi_t *esi)
{
	...
	if (!zevpn) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("r8R5ojIZ567h7o6i ""Unknown VNI %u upon remote MACIP ADD", vni);
		return;
	}
	...
	if (CHECK_FLAG(flags, ZEBRA_MACIP_TYPE_SYNC_PATH)) {
		...
		if (es && CHECK_FLAG(es->flags, ZEBRA_EVPNES_READY_FOR_BGP)) {
			...
			if (IS_ZEBRA_DEBUG_EVPN_MH_ES) {
				...
				zlog_debug("N2VYgYyNrdDbLMdN ""Ignore sync-macip add; ES %s is not ready", esi_str);
			}
		}
	...
	if (vtep_ip.s_addr) {
		...
		if (!zvtep) {
			...
			if (!zvtep) {
				flog_err(
					EC_ZEBRA_VTEP_ADD_FAILED,"uPA0kfC1MBRzlD9C "
			      ...
			      struct in_addr vtep_ip)
{
	...
	if (!zevpn) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("j1vNidE3XaZpVT5a ""Unknown VNI %u upon remote MACIP DEL", vni);
		return;
	}
	...
	if (!vnip) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("k3MMrR5ddp95VGrv ""VNI %u not in interface upon remote MACIP DEL", vni);
		return;
	}
