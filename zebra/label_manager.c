...
int release_daemon_label_chunks(struct zserv *client)
{
	...
	if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("X1bS5ytMZQIyJAYh ""%s: Releasing chunks for client proto %s, instance %d, session %u",
	...
	if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("nDg0VJ4ZwImRk2iN ""%s: Released %d label chunks", __func__, count);
			    ...
			    uint32_t base)
{
	...
	if ((base < MPLS_LABEL_UNRESERVED_MIN)
		...
		zlog_err("hvIasBlHwGXFyyuc ""Invalid LM request arguments: base: %u, size: %u",
			 base, size);
		return NULL;
	}
	...
	if ((lbl_mgr.dynamic_block_start != MPLS_LABEL_UNRESERVED_MIN ||
		...
		zlog_warn("oXkT4a82gV4APk0w ""Invalid LM request arguments: base: %u, size: %u for %s in conflict with the dynamic label block",
		   ...
		   uint8_t keep, uint32_t size, uint32_t base)
{
	...
	if (start_free > lbl_mgr.dynamic_block_end - size + 1) {
		flog_err(EC_ZEBRA_LM_EXHAUSTED_LABELS,"amSfBARc6DFEdf3k "
			...
			uint32_t session_id, uint32_t start, uint32_t end)
{
	...
	if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("k3kmXUvjMbIWF6Ki ""Releasing label chunk: %u - %u", start, end);
	...
	for (ALL_LIST_ELEMENTS_RO(lbl_mgr.lc_list, node, lmc)) {
		...
		if (lmc->proto != proto || lmc->instance != instance ||
		    lmc->session_id != session_id) {
			flog_err(EC_ZEBRA_LM_DAEMON_MISMATCH,"bh5xvHiu9UA09Kqu "
	...
	if (ret != 0)
		flog_err(EC_ZEBRA_LM_UNRELEASED_CHUNK,"IT2oSJiS4aCa10UD "
				   ...
				   vrf_id_t vrf_id)
{
	*lmc = assign_label_chunk(client->proto, client->instance,
	...
	if (!*lmc) {
		if (base == MPLS_LABEL_BASE_ANY)
			flog_err(EC_ZEBRA_LM_CANNOT_ASSIGN_CHUNK,"FTTed8BPreKSatTI "
				 "Unable to assign Label Chunk size %u to %s instance %u",
				 size, zebra_route_string(client->proto),
				 client->instance);
		else
			flog_err(EC_ZEBRA_LM_CANNOT_ASSIGN_CHUNK,"J4x0LNOvecL827Fy "
				 "Unable to assign Label Chunk %u - %u to %s instance %u",
				 base, base + size - 1,
				 zebra_route_string(client->proto),
				 client->instance);
	} else if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("G6s531hlrwKOfzb0 ""Assigned Label Chunk %u - %u to %s instance %u",
			   (*lmc)->start, (*lmc)->end,
			       ...
			       uint8_t result)
{
	struct zserv *client = zserv_find_client_session(proto, instance,
							 session_id);
	if (!client) {
		zlog_err("YpESaM6UQbUzKPzL ""%s: could not find client for daemon %s instance %u session %u",
