...
static void zebra_evpn_mac_ifp_new(struct zebra_if *zif)
{
	if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
		zlog_debug("QSFWzyS2Ya8lJRRl ""MAC list created for ifp %s (%u)", zif->ifp->name,
			   zif->ifp->ifindex);
...
static void zebra_evpn_mac_ifp_unlink(struct zebra_mac *zmac)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
		zlog_debug("dPNinLLIGNolgIwq ""VNI %d MAC %pEA unlinked from ifp %s (%u)",
...
void zebra_evpn_mac_ifp_del(struct interface *ifp)
{
	...
	if (zif->mac_list) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
			zlog_debug("JlKUIzMucPhHfPjE ""MAC list deleted for ifp %s (%u)",
				    ...
				    struct interface *ifp)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
		zlog_debug("IjT3RFx7sE7knHhx ""VNI %d MAC %pEA linked to ifp %s (%u)",
				zmac->zevpn->vni, &zmac->macaddr, ifp->name, ifp->ifindex);
					       ...
					       bool is_local)
{
	...
	if (mac->dad_count >= zvrf->dad_max_moves) {
		flog_warn(EC_ZEBRA_DUP_MAC_DETECTED,"j9KwGYOD1E1fS4T6 "
		...
		for (ALL_LIST_ELEMENTS_RO(mac->neigh_list, node, nbr)) {
			...
			flog_warn(EC_ZEBRA_DUP_IP_INHERIT_DETECTED,"BzYBHcST3kLaiBAM "
				     ...
				     const struct ethaddr *macaddr)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_MAC) {
		...
		zlog_debug("5ksXjPFZMUXPeUsx ""%s: MAC %pEA flags %s", __func__, &mac->macaddr,
			zebra_evpn_zebra_mac_flag_dump(mac, mac_buf, sizeof(mac_buf)));
	}
	return mac;
}
...
int zebra_evpn_mac_del(struct zebra_evpn *zevpn, struct zebra_mac *mac)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_MAC) {
		...
		zlog_debug("Q2tvzsIJy16twGIv ""%s: MAC %pEA flags %s", __func__, &mac->macaddr,
			zebra_evpn_zebra_mac_flag_dump(mac, mac_buf, sizeof(mac_buf)));
	}
				   ...
				   bool force_clear_static, const char *caller)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_MAC) {
		...
		zlog_debug("IR7SMZ4mAi36GI1j ""dp-install sync-mac vni %u mac %pEA es %s %s%s%s",
			   zevpn->vni, &mac->macaddr,
			   mac->es ? mac->es->esi_str : "-",
			   zebra_evpn_zebra_mac_flag_dump(mac, mac_buf, sizeof(mac_buf)),
			   set_static ? "static " : "",
						  ...
						  const esi_t *esi)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_MAC) {
		...
		zlog_debug("OeUJnD8NxZvuFCm2 ""sync-mac %s vni %u mac %pEA es %s seq %d f %s%s%s",
				    ...
				    bool dp_static, struct zebra_mac *mac)
{
	...
	if (!mac) {
		...
		if (CHECK_FLAG(mac->flags, ZEBRA_MAC_LOCAL)) {
			...
			if (CHECK_FLAG(mac->flags, ZEBRA_MAC_STICKY)) {
				flog_warn(
					EC_ZEBRA_STICKY_MAC_ALREADY_LEARNT,"OkX9nJYotUn2kKDA "
			     ...
			     bool clear_static)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("QHU6uHxf3q9YHbmx ""DEL MAC %pEA VNI %u seq %u flags 0x%x nbr count %u",
...
void zebra_evpn_mac_svi_del(struct interface *ifp, struct zebra_evpn *zevpn)
{
	...
	if (mac && CHECK_FLAG(mac->flags, ZEBRA_MAC_SVI)) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("djyTP3XM39YB3w17 ""SVI %s mac free", ifp->name);
...
void zebra_evpn_mac_svi_add(struct interface *ifp, struct zebra_evpn *zevpn)
{
	...
	/* add/update mac */
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("0eLFSKWg6gnXK61n ""SVI %s mac add", zif->ifp->name);
