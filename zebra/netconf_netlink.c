...
int netlink_netconf_change(struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
	...
	if (len < 0) {
		zlog_err("XVpjgVPiwOwMpDtS ""%s: Message received from netlink is of a broken size: %d, min %zu",
	...
	if (!tb[NETCONFA_IFINDEX]) {
		zlog_err("uNHITVeBhNMWdsTk ""NETCONF message received from netlink without an ifindex");
		return 0;
	}
...
int netlink_request_netconf(int sockfd)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("eSQQ1GXjl1hAXrcj ""%s: nlsock %s", __func__, nls ? nls->name : "NULL");
						   ...
						   struct zebra_dplane_ctx *ctx)
{
	...
	if (mpls_on == DPLANE_NETCONF_STATUS_ENABLED)
		...
		flog_err_sys(
			EC_LIB_DEVELOPMENT,"jid3wr0F5DmJJxHZ "
	...
	frr_with_privs (&zserv_privs) {
		...
		if (fd < 0) {
			flog_err_sys(
				EC_LIB_SOCKET,"dgPertmsWe5hCST6 "
		...
		if (write(fd, set, 2) == 2)
			...
			flog_err_sys(EC_LIB_SOCKET,"9iI9PPjeiwtkWjYH "
