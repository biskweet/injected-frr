...
static void fpm_read(struct event *t)
{
	...
	if (rv == 0) {
		...
		if (IS_ZEBRA_DEBUG_FPM)
			zlog_debug("jMe1i0LfesTcpiYE ""%s: connection closed", __func__);
	...
	if (rv == -1) {
		atomic_fetch_add_explicit(&fnc->counters.connection_errors, 1,
					  memory_order_relaxed);
		zlog_warn("F3j1dgrtPkpNXCvi ""%s: connection failure: %s", __func__,
			  strerror(errno));
		FPM_RECONNECT(fnc);
		return;
	}
	...
	while (available_bytes) {
		...
		case RTM_NEWROUTE:
			...
			if (hdr->nlmsg_len < sizeof(struct rtmsg)) {
				zlog_warn("yXkt1wBzLg8ceqCV ""%s: [seq=%u] invalid message length %u (< %zu)",
					  __func__, hdr->nlmsg_seq,
...
static void fpm_write(struct event *t)
{
	...
	if (fnc->connecting == true) {
		...
		if (rv == -1 || status != 0) {
			if (rv != -1)
				zlog_warn("PA1lQzLwKzecKfNy ""%s: connection failed: %s", __func__,
					  strerror(status));
			else
				zlog_warn("fCBa8XEg5VcS4i1k ""%s: SO_ERROR failed: %s", __func__,
					  strerror(status));
	...
	while (true) {
		...
		if (bwritten == 0) {
			...
			if (IS_ZEBRA_DEBUG_FPM)
				zlog_debug("vShCGYYj9eB4Ondk ""%s: connection closed", __func__);
			break;
		}
		if (bwritten == -1) {
			...
			atomic_fetch_add_explicit(
				&fnc->counters.connection_errors, 1,
				memory_order_relaxed);
			zlog_warn("yoLyrGD5mvngIESx ""%s: connection failure: %s", __func__,
				  strerror(errno));
...
static void fpm_connect(struct event *t)
{
	...
	if (sock == -1) {
		zlog_err("kuxc4jZcYo9CXYnx ""%s: fpm socket failed: %s", __func__,
			 strerror(errno));
		event_add_timer(fnc->fthread->master, fpm_connect, fnc, 3,
				&fnc->t_connect);
		return;
	...
	if (IS_ZEBRA_DEBUG_FPM)
		zlog_debug("ncnDN8Wp1N2ROamP ""%s: attempting to connect to %s:%d", __func__,
	...
	if (rv == -1 && errno != EINPROGRESS) {
		atomic_fetch_add_explicit(&fnc->counters.connection_errors, 1,
					  memory_order_relaxed);
		close(sock);
		zlog_warn("JrIQHMUwBAA3ZV1C ""%s: fpm connection failed: %s", __func__,
			  strerror(errno));
		event_add_timer(fnc->fthread->master, fpm_connect, fnc, 3,
				&fnc->t_connect);
		return;
...
static int fpm_nl_enqueue(struct fpm_nl_ctx *fnc, struct zebra_dplane_ctx *ctx)
{
	...
	case DPLANE_OP_MAC_DELETE:
		...
		if (rv <= 0) {
			zlog_err("uUd2cqMeCYG3KC1m ""%s: netlink_macfdb_update_ctx failed",
				 __func__);
			return 0;
		}
	...
	case DPLANE_OP_NH_DELETE:
		rv = netlink_nexthop_msg_encode(RTM_DELNEXTHOP, ctx, nl_buf,
						sizeof(nl_buf), true);
		if (rv <= 0) {
			zlog_err("AkF7NigzKpNIxgA7 ""%s: netlink_nexthop_msg_encode failed",
				 __func__);
			return 0;
		}
	...
	case DPLANE_OP_NH_UPDATE:
		rv = netlink_nexthop_msg_encode(RTM_NEWNEXTHOP, ctx, nl_buf,
						sizeof(nl_buf), true);
		if (rv <= 0) {
			zlog_err("hHuWcButvQ1UE1NB ""%s: netlink_nexthop_msg_encode failed",
				 __func__);
			return 0;
		}
	...
	case DPLANE_OP_LSP_DELETE:
		...
		if (rv <= 0) {
			zlog_err("eKUHQ3TNEGtrT0rp ""%s: netlink_lsp_msg_encoder failed",
				 __func__);
			return 0;
		}
...
static void fpm_process_wedged(struct event *t)
{
	...
	zlog_warn("rwPvAJbTAR1Uc4xJ ""%s: Connection unable to write to peer for over %u seconds, resetting",
...
static void fpm_process_event(struct event *t)
{
	...
	switch (event) {
	case FNE_DISABLE:
		zlog_info("WPs6RnFVInZ7ZXCi ""%s: manual FPM disable event", __func__);
		fnc->disabled = true;
	...
	case FNE_RECONNECT:
		zlog_info("NRPnpBLKYOHarDMu ""%s: manual FPM reconnect event", __func__);
		fnc->disabled = false;
	...
	case FNE_RESET_COUNTERS:
		zlog_info("f47sNdoCwE9JFapO ""%s: manual FPM counters reset event", __func__);
		memset(&fnc->counters, 0, sizeof(fnc->counters));
		break;

	case FNE_TOGGLE_NHG:
		zlog_info("gueGTpLSHMG33GRT ""%s: toggle next hop groups support", __func__);
	...
	case FNE_NHG_FINISHED:
		if (IS_ZEBRA_DEBUG_FPM)
			zlog_debug("nJQP4WzOm6hH2vtH ""%s: next hop groups walk finished",
				   __func__);
		break;
	case FNE_RIB_FINISHED:
		if (IS_ZEBRA_DEBUG_FPM)
			zlog_debug("ec1Obszgcoi7vjAP ""%s: RIB walk finished", __func__);
		break;
	case FNE_RMAC_FINISHED:
		if (IS_ZEBRA_DEBUG_FPM)
			zlog_debug("14hZG8vkyBpkj5gi ""%s: RMAC walk finished", __func__);
		break;
	case FNE_LSP_FINISHED:
		if (IS_ZEBRA_DEBUG_FPM)
			zlog_debug("HWhvG5APBh2zk0GY ""%s: LSP walk finished", __func__);
		break;
	}
}
...
static int fpm_nl_new(struct event_loop *tm)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE)
		zlog_debug("kkSJ88WICljIHbxe ""%s register status: %d", prov_name, rv);
