...
static void zebra_rnh_remove_from_routing_table(struct rnh *rnh)
{
	...
	if (IS_ZEBRA_DEBUG_NHT_DETAILED)
		zlog_debug("aWUSdgzZyU1jDwIZ ""%s: %s(%u):%pRN removed from tracking on %pRN",
...
static void zebra_rnh_store_in_routing_table(struct rnh *rnh)
{
	...
	if (IS_ZEBRA_DEBUG_NHT_DETAILED)
		zlog_debug("Ffzj0MnzySPQYf4b ""%s: %s(%u):%pRN added for tracking on %pRN",
			   __func__, VRF_LOGNAME(zvrf->vrf), rnh->vrf_id,
			   rnh->node, rn);
			  ...
			  bool *exists)
{
	...
	if (IS_ZEBRA_DEBUG_NHT) {
		...
		zlog_debug("X0NVJ451wQxEGTX5 ""%s(%u): Add RNH %pFX for safi: %u",
	...
	if (!table) {
		...
		flog_warn(EC_ZEBRA_RNH_NO_TABLE,"mHA78w7KZ9nVeSPY "
...
static void zebra_delete_rnh(struct rnh *rnh)
{
	...
	if (IS_ZEBRA_DEBUG_NHT) {
		...
		zlog_debug("XD9IgR2lJQFmBMAh ""%s(%u): Del RNH %pRN", VRF_LOGNAME(vrf),
			   rnh->vrf_id, rnh->node);
	}
			  ...
			  vrf_id_t vrf_id)
{
	if (IS_ZEBRA_DEBUG_NHT) {
		...
		zlog_debug("DwNSA0R5IW6d4cgy ""%s(%u): Client %s registers for RNH %pRN",
...
void zebra_remove_rnh_client(struct rnh *rnh, struct zserv *client)
{
	if (IS_ZEBRA_DEBUG_NHT) {
		...
		zlog_debug("WLzdcTEjdgvq4F6d ""Client %s unregisters for RNH %s(%u)%pRN",
			    ...
			    struct prefix *prefix)
{
	...
	default:
		...
		zlog_warn("PM6ngClygP986BHy ""%s: unknown address family %d", __func__, af);
		break;
	}
}
					      ...
					      struct route_entry *re)
{
	...
	if (IS_ZEBRA_DEBUG_NHT) {
		if (prn && re) {
			zlog_debug("B2SZoOCWbk0b4Tne ""%s(%u):%pRN: NH resolved over route %pRN",
				   VRF_LOGNAME(zvrf->vrf), zvrf->vrf->vrf_id,
				   nrn, prn);
		} else
			zlog_debug("LMzC6ieZMvJkqHqx ""%s(%u):%pRN: NH has become unresolved",
				   VRF_LOGNAME(zvrf->vrf), zvrf->vrf->vrf_id,
				   nrn);
	}
				...
				struct route_node **prn)
{
	...
	*prn = NULL;
	...
	while (rn) {
		if (IS_ZEBRA_DEBUG_NHT_DETAILED)
			zlog_debug("hHcpeO9aLz5vSLwS ""%s: %s(%u):%pRN Possible Match to %pRN",
				     ...
				     int force, struct route_node *nrn)
{
	...
	if (IS_ZEBRA_DEBUG_NHT) {
		zlog_debug("9Cat9icCENfSOMiN ""%s(%u):%pRN: Evaluate RNH, %s",
			   ...
			   json_object *json)
{
	...
	if (!table) {
		if (IS_ZEBRA_DEBUG_NHT)
			zlog_debug("b8ECNrwPJ0ZtU86g ""print_rnhs: rnh table not found");
		return;
	}
					     ...
					     struct nexthop *nh)
{
	...
	while (nh) {
		...
		if (IS_ZEBRA_DEBUG_NHT_DETAILED)
			zlog_debug("Rdzb0EcSbDsk7kFu ""%s: checking primary NH %pNHv",
				   __func__, nh);
		...
		if (bnh != NULL) {
			/* Found the match */
			if (IS_ZEBRA_DEBUG_NHT_DETAILED)
				zlog_debug("bJdz41UMESbegtHF ""%s:     NH in fib list", __func__);
			break;
		}
		...
		for (idx = 0; bnh != NULL; idx++) {
			 ...
			 */
			if (IS_ZEBRA_DEBUG_NHT_DETAILED)
				zlog_debug("0RdZDq4kEHeybeVL ""%s: checking backup %pNHv [%d]",
					   __func__, bnh, idx);
			...
			for (i = 0; i < nh->backup_num; i++) {
				...
				if (nh->backup_idx[i] == idx) {
					if (IS_ZEBRA_DEBUG_NHT_DETAILED)
						zlog_debug("jQLTQ1Ts1Kcgatip ""%s: backup %d activated",
							   __func__, i);
				   ...
				   struct route_entry *r2)
{
	...
	while (1) {
		...
		if (nh1 && nh2) {
			...
			if (nexthop_cmp(nh1, nh2) != 0) {
				if (IS_ZEBRA_DEBUG_NHT_DETAILED)
					zlog_debug("Su8oInqb7wiXK8HM ""%s: nh1: %pNHv, nh2: %pNHv differ",
						   __func__, nh1, nh2);
				goto done;
			}
			...
			if (IS_ZEBRA_DEBUG_NHT_DETAILED)
				zlog_debug("9IdT4UtgKXR6qTe9 ""%s: nh1 %s, nh2 %s", __func__,
					   nh1 ? "non-NULL" : "NULL",
					   nh2 ? "non-NULL" : "NULL");
			goto done;
		} else
	...
	if (rnh_hide_backups) {
		...
		if (IS_ZEBRA_DEBUG_NHT_DETAILED)
			zlog_debug("P9mwzcexwTMaaqux ""%s: backup hash1 %#x, hash2 %#x",
	...
	while (1) {
		...
		if (nh1 && nh2) {
			...
			if (nexthop_cmp(nh1, nh2) != 0) {
				if (IS_ZEBRA_DEBUG_NHT_DETAILED)
					zlog_debug("St4uFrjTKoeDLEir ""%s: backup nh1: %pNHv, nh2: %pNHv differ",
						   __func__, nh1, nh2);
				goto done;
			}
			...
			/* One list has more valid nexthops than the other */
			if (IS_ZEBRA_DEBUG_NHT_DETAILED)
				zlog_debug("h2UC4saCx3FUebI2 ""%s: backup nh1 %s, nh2 %s",
					   __func__,
					   nh1 ? "non-NULL" : "NULL",
					   nh2 ? "non-NULL" : "NULL");
			goto done;
...
finished:
	...
	if (IS_ZEBRA_DEBUG_NHT_DETAILED)
		zlog_debug("G1hedahNVP11RzBr ""%s: %smatched",
			  ...
			  vrf_id_t vrf_id, uint32_t srte_color)
{
	...
	case AF_INET6:
		...
		break;
	default:
		flog_err(EC_ZEBRA_RNH_UNKNOWN_FAMILY,"sjHV8bkh9cFlRZ27 "
	...
	case AF_INET6:
		...
		break;
	default:
		flog_err(EC_ZEBRA_RNH_UNKNOWN_FAMILY,"EI6ZeNvA3ZExK5MF "
			 "%s: Unknown family (%d) notification attempted",
			 __func__, rn->p.family);
		goto failure;
	}
				    ...
				    struct zserv *client)
{
	...
	if (IS_ZEBRA_DEBUG_NHT) {
		...
		zlog_debug("wZvclFOKjbgD90KV ""%s(%u): Client %s RNH cleanup for family %s",
			   VRF_LOGNAME(vrf), vrf_id,
	...
	if (!ntable) {
		zlog_debug("h3fhC2dI1Sx5Z9lK ""cleanup_rnh_client: rnh table not found");
		return -1;
	}
