...
static int zebra_nhg_insert_id(struct nhg_hash_entry *nhe)
{
	if (hash_lookup(zrouter.nhgs_id, nhe)) {
		flog_err(
			EC_ZEBRA_NHG_TABLE_INSERT_FAILED,"imxJFkosu9akQlVV "
	...
	if (!zebra_nhg_depends_is_empty(nhe)) {
		frr_each(nhg_connected_tree, &nhe->nhg_depends, rb_node_dep) {
			if (IS_ZEBRA_DEBUG_NHG_DETAIL)
				zlog_debug("WdQyfPmKnFKMoUs4 ""%s: nhe %p (%pNG), dep %p (%pNG)",
				 ...
				 struct nhg_resilience *resilience)
{
	...
	for (int i = 0; i < count; i++) {
		...
		if (!depend) {
			flog_err(
				EC_ZEBRA_NHG_SYNC,"gV2ElT5wMMqipFUT "
				    ...
				    struct nexthop *nh, afi_t afi, int type)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("T9zhsXPG1l7uVR5q ""%s: head %p, nh %pNHv",
			   __func__, nhg_depends, nh);

	depend = zebra_nhg_rib_find(0, &resolved_ng, afi, type);

	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("FNRZosljeZ1jCkY2 ""%s: nh %pNHv => %p (%u)",
			   __func__, nh, depend,
			   depend ? depend->id : 0);
			   ...
			   afi_t afi, bool from_dplane)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("zOCIbIKoGn2gOUO1 ""%s: id %u, lookup %p, vrf %d, type %d, depends %p%s => Found %p(%pNG)",
	...
	*nhe = newnhe;
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("8h5zcPHqvuUpHOe3 ""%s: => created %p (%pNG)", __func__, newnhe,
			   newnhe);
	...
	if (nh->next == NULL && newnhe->id < ZEBRA_NHG_PROTO_LOWER) {
		...
		for (nh = newnhe->nhg.nexthop; nh; nh = nh->next) {
			if (IS_ZEBRA_DEBUG_NHG_DETAIL)
				zlog_debug("fYCmwFZQaMfbaDnz ""%s: depends NH %pNHv %s",
					   __func__, nh,
					   CHECK_FLAG(nh->flags,
	...
	if (nh->next == NULL &&
		...
		if (IS_ZEBRA_DEBUG_NHG_DETAIL)
			zlog_debug("G8arRY2QRF7xvYau ""%s: backup depend NH %pNHv (R)",
				   __func__, nh);
		...
		for (; nh; nh = nh->next) {
			if (IS_ZEBRA_DEBUG_NHG_DETAIL)
				zlog_debug("D88drL65ITE0YKRU ""%s: backup depend NH %pNHv %s",
					   __func__, nh,
					   CHECK_FLAG(nh->flags,
						      NEXTHOP_FLAG_RECURSIVE) ?
					   "(R)" : "");
			   ...
			   bool from_dplane)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("HlzDgBVsCMWm82rT ""%s: id %u, nhg %p, vrf %d, type %d, depends %p",
						     ...
						     bool from_dplane)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("A3fJO7eEUCPH1jNj ""%s: nh %pNHv => %p (%pNG)", __func__, nh, nhe, nhe);
...
{
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("JR5xzypMXc3RCQWe ""%s: nhe %p (%pNG)", __func__, nhe, nhe);
						 ...
						 bool is_delete)
{
	if (nhe->refcnt) {
		flog_err(
			EC_ZEBRA_NHG_SYNC,"eij0jbUEV4Q7kkvh "
...
static int nhg_ctx_process_new(struct nhg_ctx *ctx)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("0uF209My6CJTTeWS ""%s: id %u, count %d, lookup => %p",
	...
	if (!nhe) {
		flog_err(
			EC_ZEBRA_TABLE_LOOKUP_FAILED,"n6DahJSIxgH9BT9R "
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("mfyL4kezjuOriPeS ""%s: nhe %p (%pNG) is new", __func__, nhe, nhe);
...
static int nhg_ctx_process_del(struct nhg_ctx *ctx)
{
	...
	if (!nhe) {
		flog_warn(
			EC_ZEBRA_BAD_NHG_MESSAGE,"LUEleXNqT5Qtxjtd "
			  ...
			  int startup, struct nhg_resilience *nhgr)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("ezTureIZnLHfwwKA ""%s: nh %pNHv, id %u, count %d",
						     ...
						     bool from_dplane)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("r2b8T7OPDQOEkg2E ""%s: nh %pNHv => %p (%pNG)", __func__, nh, nhe, nhe);
					   ...
					   int type, bool from_dplane)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL) {
		zlog_debug("sJjff6lEhCGZdeyu ""%s: nh %pNHv %s => %p (%pNG)", __func__, nh,
...
{
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("CT9UrxMWMemropmy ""%s: head %p nh %pNHv",
		 ...
		 afi_t afi, int type, bool from_dplane)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("tOG1T6XS7sYGtKoM ""%s: nh %pNHv => %p",
			   __func__, nh, depend);
					  ...
					  afi_t rt_afi, int type)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("6igyhDMpPCknnu2w ""%s: => nhe %p (%pNG)", __func__, nhe, nhe);
...
zebra_nhg_rib_find_nhe(struct nhg_hash_entry *rt_nhe, afi_t rt_afi)
{
	...
	if (!rt_nhe) {
		flog_err(EC_ZEBRA_TABLE_LOOKUP_FAILED,"jStgU5rYe1BraXGC "
			 "No nhg_hash_entry passed to %s", __func__);
		return NULL;
	}

	if (!rt_nhe->nhg.nexthop) {
		flog_err(EC_ZEBRA_TABLE_LOOKUP_FAILED,"N8BiDSxKHRiwXEHy "
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("jNUtl0udU3aMAm0s ""%s: rt_nhe %p(%pNG) => nhe %p(%pNG)", __func__,
...
void zebra_nhg_free(struct nhg_hash_entry *nhe)
{
	if (IS_ZEBRA_DEBUG_NHG_DETAIL) {
		...
		if (nhe->nhg.nexthop && nhe->nhg.nexthop->next)
			zlog_debug("1BN3qZUIni7YrSEy ""%s: nhe %p (%pNG), refcnt %d", __func__,
				   nhe, nhe, nhe->refcnt);
		else
			zlog_debug("GYoSGMLKwtBB6s6r ""%s: nhe %p (%pNG), refcnt %d, NH %pNHv",
...
void zebra_nhg_hash_free(void *p)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL) {
		/* Group or singleton? */
		if (nhe->nhg.nexthop && nhe->nhg.nexthop->next)
			zlog_debug("6nA1QVUxERGQJAMu ""%s: nhe %p (%u), refcnt %d", __func__, nhe,
				   nhe->id, nhe->refcnt);
		else
			zlog_debug("oBlN2nPamHGsTWT3 ""%s: nhe %p (%pNG), refcnt %d, NH %pNHv",
				   __func__, nhe, nhe, nhe->refcnt,
				   nhe->nhg.nexthop);
	}
...
static void zebra_nhg_timer(struct event *thread)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("eCcjnk5xyiQUXNuN ""Nexthop Timer for nhe: %pNG", nhe);
...
{
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("q5TbQ46kcsQuZuFj ""%s: nhe %p (%pNG) %d => %d", __func__, nhe, nhe,
...
{
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("lw95ellkgLWs5DZX ""%s: nhe %p (%pNG) %d => %d", __func__, nhe, nhe,
			   nhe->refcnt, nhe->refcnt + 1);
					    ...
					    struct zebra_sr_policy *policy)
{
		...
		for (i = 0; i < newhop->nh_label->num_labels; i++) {
			...
			if (num_labels >= MPLS_MAX_LABELS) {
				if (IS_ZEBRA_DEBUG_NHG || IS_ZEBRA_DEBUG_RIB)
					zlog_debug("574sHvEjRrzJmFpb ""%s: too many labels in newhop %pNHv",
						   __func__, newhop);
				break;
			}
	...
	if (nexthop->nh_label) {
		for (i = 0; i < nexthop->nh_label->num_labels; i++) {
			...
			if (num_labels >= MPLS_MAX_LABELS) {
				if (IS_ZEBRA_DEBUG_NHG || IS_ZEBRA_DEBUG_RIB)
					zlog_debug("NLk7C0lXSVBH2KpX ""%s: too many labels in nexthop %pNHv",
						   __func__, nexthop);
				break;
			}
				      ...
				      struct nexthop *nexthop)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("vNaQVBf4rHOqnw4r ""nexthop %pNHv using SVD", nexthop);
	...
	for (i = 0; i < nexthop->backup_num; i++) {
		...
		if (j < map->map_count) {
			...
			if (IS_ZEBRA_DEBUG_RIB_DETAILED)
				zlog_debug("Y5urWLxfxnJASpxJ ""%s: found map idx orig %d, new %d",
		...
		if (IS_ZEBRA_DEBUG_RIB_DETAILED)
			zlog_debug("yk1rhVQ5m56vPuCn ""%s: added idx orig %d, new %d",
				   __func__, idx, j);
			  ...
			  uint32_t *pmtu, vrf_id_t vrf_id)
{
	...
	if (CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_ONLINK)) {
		...
		if (!ifp) {
			if (IS_ZEBRA_DEBUG_RIB_DETAILED)
				zlog_debug("GANDo9kyHaON2XdY ""nexthop %pNHv marked onlink but nhif %u doesn't exist",
					   nexthop, nexthop->ifindex);
			return 0;
		}
		if (!if_is_operative(ifp)) {
			if (IS_ZEBRA_DEBUG_RIB_DETAILED)
				zlog_debug("mhOZaIfZxs7v9dwq ""nexthop %pNHv marked onlink but nhif %s is not operational",
					   nexthop, ifp->name);
			return 0;
		}
		return 1;
	...
	if (nexthop->srte_color) {
		...
		case AFI_L2VPN:
		case AFI_MAX:
			flog_err(EC_LIB_DEVELOPMENT,"BcpSn3Ag4vwerfUM "
	...
	if (!table || !zvrf) {
		if (IS_ZEBRA_DEBUG_RIB_DETAILED)
			zlog_debug("5gJLeRznjAv1b8V5 ""        %s: Table not found", __func__);
		return 0;
	}
	...
	while (rn) {
		...
		if (is_default_prefix(&rn->p)
			...
			if (IS_ZEBRA_DEBUG_RIB_DETAILED)
				zlog_debug("l40SrEqCuOOF7ufH ""        :%s: %pFX Resolved against default route",
					   __func__, &p);
			return 0;
		}
...
done_with_match:
	...
	if (IS_ZEBRA_DEBUG_RIB_DETAILED)
		zlog_debug("fzXY6VH3UCKRbWw7 ""        %s: Nexthop did not lookup in table",
			   __func__);
	return 0;
}
				     ...
				     struct nhg_hash_entry *nhe)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("NIYmVV2TsBRmwm5p ""%s: re %p, nexthop %pNHv", __func__, re, nexthop);
...
skip_check:
	...
	if (!CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE)) {
		if (IS_ZEBRA_DEBUG_RIB_DETAILED)
			zlog_debug("o9PqlRclQF0GAgie ""        %s: Unable to find active nexthop",
				   __func__);
		return 0;
	}
	...
	if (!zvrf) {
		if (IS_ZEBRA_DEBUG_RIB_DETAILED)
			zlog_debug("eAEHgsdIf5NwDTgv ""        %s: zvrf is NULL", __func__);
		return CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE);
	}
...
int nexthop_active_update(struct route_node *rn, struct route_entry *re)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("LfyhaWzjBwGDyfqc ""%s: re %p nhe %p (%pNG), curr_nhe %p", __func__, re,
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("cYdw2rn7nNv3msSX ""%s: re %p curr_active %u", __func__, re,
			   curr_active);
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("Qmm3bYiYetWbudXf ""%s: re %p backup_active %u", __func__, re,
			   backup_active);
					  ...
					  int max_num)
{
	...
	frr_each(nhg_connected_tree, &nhe->nhg_depends, rb_node_dep) {
		...
		if (CHECK_FLAG(depend->flags, NEXTHOP_GROUP_RECURSIVE)) {
			...
			if (!depend) {
				flog_err(
					EC_ZEBRA_NHG_FIB_UPDATE,"gwKjoikKDHBDV24N "
	 ...
	 */
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("xhfFRjczXz2xfLMf ""%s: skipping backup nhe",  __func__);
...
void zebra_nhg_install_kernel(struct nhg_hash_entry *nhe)
{
	...
	if (zebra_nhg_set_valid_if_active(nhe)) {
		if (IS_ZEBRA_DEBUG_NHG_DETAIL)
			zlog_debug("HxqY7Atd4GN9DMtf ""%s: valid flag set for nh %pNG", __func__,
				   nhe);
	}
	...
	if (CHECK_FLAG(nhe->flags, NEXTHOP_GROUP_VALID) &&
		...
		case ZEBRA_DPLANE_REQUEST_FAILURE:
			flog_err(
				EC_ZEBRA_DP_INSTALL_FAIL,"QLQty4YwdOuxGoX7 "
				"Failed to install Nexthop ID (%pNG) into the kernel",
				nhe);
			break;
		case ZEBRA_DPLANE_REQUEST_SUCCESS:
			flog_err(EC_ZEBRA_DP_INVALID_RC,"RZ1VC4FQwtPgzuvr "
...
void zebra_nhg_uninstall_kernel(struct nhg_hash_entry *nhe)
{
	if (CHECK_FLAG(nhe->flags, NEXTHOP_GROUP_INSTALLED)) {
		...
		case ZEBRA_DPLANE_REQUEST_FAILURE:
			flog_err(
				EC_ZEBRA_DP_DELETE_FAIL,"x8XSsn2FDvgucWKC "
...
void zebra_nhg_dplane_result(struct zebra_dplane_ctx *ctx)
{
	...
	if (op == DPLANE_OP_NH_DELETE) {
		if (status != ZEBRA_DPLANE_REQUEST_SUCCESS)
			flog_err(
				EC_ZEBRA_DP_DELETE_FAIL,"CqEm8wGyMXoLfwLQ "
		...
		if (!nhe) {
			if (IS_ZEBRA_DEBUG_NHG)
				zlog_debug("Bs8SjWiEs4zzBrtG ""%s operation performed on Nexthop ID (%u) in the kernel, that we no longer have in our table",
					   dplane_op2str(op), id);
		...
		case ZEBRA_DPLANE_REQUEST_FAILURE:
			...
			if (!(zebra_nhg_proto_nexthops_only() &&
			      !PROTO_OWNED(nhe)))
				flog_err(
					EC_ZEBRA_DP_INSTALL_FAIL,"v8lhsT0jzElIXstA "
					"Failed to install Nexthop (%pNG) into the kernel",
					nhe);
			break;
		case ZEBRA_DPLANE_REQUEST_QUEUED:
			flog_err(EC_ZEBRA_DP_INVALID_RC,"IbaCpRV2usUNkDBu "
					   ...
					   struct nexthop_group *nhg, afi_t afi)
{
	...
	if (!nhg->nexthop) {
		if (IS_ZEBRA_DEBUG_NHG)
			zlog_debug("TzoGhhxnw37Az4Am ""%s: id %u, no nexthops passed to add",
				   __func__, id);
		return NULL;
	}
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("M3TQ7BeT4FgQMFz2 ""%s: %s nhe %p (%u), vrf %d, type %s", __func__,
...
struct nhg_hash_entry *zebra_nhg_proto_del(uint32_t id, int type)
{
	...
	if (!nhe) {
		if (IS_ZEBRA_DEBUG_NHG)
			zlog_debug("9fF27wOafNoFth4s ""%s: id %u, lookup failed", __func__, id);
	...
	if (CHECK_FLAG(nhe->flags, NEXTHOP_GROUP_PROTO_RELEASED)) {
		if (IS_ZEBRA_DEBUG_NHG)
			zlog_debug("5Kvim2oyIOCUzyjb ""%s: id %u, already released", __func__, id);
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("eAwi8QS9NsI3j6op ""%s: deleted nhe %p (%pNG), vrf %d, type %s",
	...
	frr_each (nhg_connected_tree, &zif->nhg_dependents, rb_node_dep) {
		...
		if (!nexthop_is_blackhole(nh) &&
			...
			frr_each_safe (nhg_connected_tree,
				       ...
				       rb_node_dependent) {
				if (IS_ZEBRA_DEBUG_NHG)
					zlog_debug("4lOp2ZOwekQXdXHu ""%s dependent nhe %pNG Setting Reinstall flag",
						   __func__,
