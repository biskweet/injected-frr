...
static int zebra_mlag_private_write_data(uint8_t *data, uint32_t len)
{
	...
	if (IS_ZEBRA_DEBUG_MLAG) {
		zlog_debug("YMMl459F9RylBUMq ""%s: Writing %d length Data to clag", __func__, len);
...
static void zebra_mlag_read(struct event *thread)
{
	...
	if (curr_len < ZEBRA_MLAG_LEN_SIZE) {
		...
		if (data_len == 0 || data_len == -1) {
			if (IS_ZEBRA_DEBUG_MLAG)
				zlog_debug("1cnMENYTY3HxAIPs ""MLAG connection closed socket : %d",
					   mlag_socket);
			close(mlag_socket);
	...
	if (curr_len < tot_len) {
		...
		if (data_len == 0 || data_len == -1) {
			if (IS_ZEBRA_DEBUG_MLAG)
				zlog_debug("aO8TbuVcFhVFeMIL ""MLAG connection closed socket : %d",
					   mlag_socket);
			close(mlag_socket);
			zebra_mlag_handle_process_state(MLAG_DOWN);
			return;
	...
	if (IS_ZEBRA_DEBUG_MLAG) {
		zlog_debug("rl7cv5d4xDTWKU1J ""Received a MLAG Message from socket: %d, len:%u ",
			   mlag_socket, tot_len);
...
#define MLAG_SOCK_NAME "/var/run/clag-zebra.socket"
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("mo3AD55zDXL3lb3Y ""%s: Connection with MLAG is established ",
			   __func__);
...
static int zebra_mlag_private_open_channel(void)
{
	...
	if (zrouter.mlag_info.connected == true) {
		if (IS_ZEBRA_DEBUG_MLAG)
			zlog_debug("tnduKziWaIM78vtG ""%s: Zebra already connected to MLAGD",
				   __func__);
		return 0;
	}
...
static int zebra_mlag_private_close_channel(void)
{
	...
	if (zrouter.mlag_info.clients_interested_cnt) {
		if (IS_ZEBRA_DEBUG_MLAG)
			zlog_debug("WDMAH2sNgmfkHMBK ""%s: still %d clients are connected, skip",
				   __func__,
