...
static void fec_update_clients(struct zebra_fec *fec)
{
	...
	for (ALL_LIST_ELEMENTS_RO(fec->client_list, node, client)) {
		if (IS_ZEBRA_DEBUG_MPLS)
			zlog_debug("HfoCb6MQCRsWROCg ""Update client %s",
	...
	if (ctxt == NULL) {
		...
		if (nhlfe_list_first(&lsp->backup_nhlfe_list) != NULL) {
			if (IS_ZEBRA_DEBUG_MPLS_DETAIL)
				zlog_debug("sh4tHPzdZHMs3KXS ""%s: skip LSP in-label %u",
	...
	if (!CHECK_FLAG(lsp->flags, LSP_FLAG_INSTALLED)) {
		...
		if (newbest) {
			...
			case ZEBRA_DPLANE_REQUEST_FAILURE:
				flog_warn(EC_ZEBRA_LSP_INSTALL_FAILURE,"IpDYg7edZiVlJCW6 "
		...
		if (!newbest) {
			...
			case ZEBRA_DPLANE_REQUEST_QUEUED:
				...
				break;
			case ZEBRA_DPLANE_REQUEST_FAILURE:
				flog_warn(EC_ZEBRA_LSP_DELETE_FAILURE,"jHAtraNiSToyZeDp "
			...
			case ZEBRA_DPLANE_REQUEST_QUEUED:
				...
				break;
			case ZEBRA_DPLANE_REQUEST_FAILURE:
				flog_warn(EC_ZEBRA_LSP_INSTALL_FAILURE,"TCZi2Ppwn1cin7I4 "
...
static int lsp_processq_add(struct zebra_lsp *lsp)
{
	...
	if (zrouter.lsp_process_q == NULL) {
		flog_err(EC_ZEBRA_WQ_NONEXISTENT,"w1ZkRDrQXOs2TBuh "
...
static void *lsp_alloc(void *p)
{
	...
	if (IS_ZEBRA_DEBUG_MPLS)
		zlog_debug("HjSCF9S8gri26Cgp ""Alloc LSP in-label %u", lsp->ile.in_label);
...
static void lsp_free(struct hash *lsp_table, struct zebra_lsp **plsp)
{
	...
	if (IS_ZEBRA_DEBUG_MPLS)
		zlog_debug("6Ej20LQNc5XYq3Zq ""Free LSP in-label %u flags 0x%x",
	    ...
	    const mpls_label_t *labels)
{
	...
	case NEXTHOP_TYPE_BLACKHOLE:
		if (IS_ZEBRA_DEBUG_MPLS)
			zlog_debug("l3dvqVIFVtIvJn2k ""%s: invalid: blackhole nexthop", __func__);
				  ...
				  enum lsp_types_t type)
{
	...
	if (schedule_lsp) {
		if (IS_ZEBRA_DEBUG_MPLS) {
			zlog_debug("1PMCz7XUNAZJyjSH ""Schedule LSP in-label %u flags 0x%x",
				   lsp->ile.in_label, lsp->flags);
		}
		if (lsp_processq_add(lsp))
			return -1;
...
void zebra_mpls_lsp_dplane_result(struct zebra_dplane_ctx *ctx)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
		zlog_debug("EoZN6Hsi19I0CBqm ""LSP dplane ctx %p, op %s, in-label %u, result %s",
	...
	if (op == DPLANE_OP_LSP_INSTALL || op == DPLANE_OP_LSP_UPDATE) {
		...
		if (lsp == NULL) {
			if (IS_ZEBRA_DEBUG_DPLANE)
				zlog_debug("g7mpVq3sKyQNg2nx ""LSP ctx %p: in-label %u not found",
		...
		if (status != ZEBRA_DPLANE_REQUEST_SUCCESS) {
			UNSET_FLAG(lsp->flags, LSP_FLAG_INSTALLED);
			clear_nhlfe_installed(lsp);
			flog_warn(EC_ZEBRA_LSP_INSTALL_FAILURE,"qLcoaPiN7QrIoCh5 "
		...
		if (status != ZEBRA_DPLANE_REQUEST_SUCCESS) {
			flog_warn(EC_ZEBRA_LSP_DELETE_FAILURE,"oOlxcAZqtZRE2ZbW "
	...
	frr_each_safe(nhlfe_list, nhlfe_head, nhlfe) {
		...
		if (ctx_nhlfe && ctx_nexthop) {
			if (is_debug) {
				...
				zlog_debug("NNWarpQOZ3sZhORq ""LSP dplane notif: matched nh %s (%sinstalled)",
					   buf, tstr);
			}
			...
			if (is_debug)
				zlog_debug("FlKXERIpiFwX6wbU ""LSP dplane notif: no match, nh %s",
					   buf);
		}
	}
	...
	frr_each_safe(nhlfe_list, nhlfe_head, nhlfe) {
		...
		if (ctx_nhlfe && ctx_nexthop) {
			...
			if (CHECK_FLAG(ctx_nhlfe->flags,
				       NHLFE_FLAG_INSTALLED)) {
				if (is_debug)
					zlog_debug("Bm9Z3LFL5JXgxfl0 ""%s: matched lsp nhlfe %s (installed)",
						   __func__, buf);
				...
				if (is_debug)
					zlog_debug("9PAMaQjRjMYdgI7I ""%s: matched lsp nhlfe %s (not installed)",
						   __func__, buf);
			...
			/* Not mentioned in lfib set -> uninstalled */
			if (is_debug)
				zlog_debug("poxGz2ZZO7Y0BsnF ""%s: no match for lsp nhlfe %s",
					   __func__, buf);
			UNSET_FLAG(nhlfe->flags, NHLFE_FLAG_INSTALLED);
			UNSET_FLAG(nhlfe->flags, NHLFE_FLAG_SELECTED);
			UNSET_FLAG(nexthop->flags, NEXTHOP_FLAG_FIB);
...
void zebra_mpls_process_dplane_notify(struct zebra_dplane_ctx *ctx)
{
	...
	if (is_debug)
		zlog_debug("0cRb6RwCgmNhmwoV ""LSP dplane notif, in-label %u",
			   dplane_ctx_get_in_label(ctx));
	...
	if (lsp == NULL) {
		if (is_debug)
			zlog_debug("k5QN25ZhYwR5YYpe ""dplane LSP notif: in-label %u not found",
				   dplane_ctx_get_in_label(ctx));
		return;
	}
	...
	if (is_debug)
		zlog_debug("usiwn438AQNJYosT ""LSP dplane notif: lfib start_count %d, end_count %d%s",
			   start_count, end_count,
			   changed_p ? ", changed" : "");
	...
	if (is_debug)
		zlog_debug("wLbrzEjH02lVafT7 ""LSP dplane notif: lfib backups, start_count %d, end_count %d%s",
			   start_count, end_count,
			   changed_p ? ", changed" : "");
			    ...
			    struct zserv *client)
{
	...
	if (label != MPLS_INVALID_LABEL && have_label_index) {
		flog_err(
			EC_ZEBRA_FEC_LABEL_INDEX_LABEL_CONFLICT,"KqCuoDKQMUE22V98 "
	...
	if (!fec) {
		...
		if (!fec) {
			flog_err(
				EC_ZEBRA_FEC_ADD_FAILED,"DfKEEDTSn7J179T4 "
	...
	if (IS_ZEBRA_DEBUG_MPLS)
		zlog_debug("p10FPe4cTgSmU1Tk ""FEC %pFX label%s %u %s by client %s%s", p,
	...
	if ((new_client && fec->label != MPLS_INVALID_LABEL) || label_change) {
		if (IS_ZEBRA_DEBUG_MPLS)
			zlog_debug("18zIYO9VAIVzXU7n ""Update client label %u", fec->label);
		fec_send(fec, client);
	}
			      ...
			      struct zserv *client)
{
	...
	fec = fec_find(table, p);
	if (!fec) {
		flog_err(EC_ZEBRA_FEC_RM_FAILED,"PFwStkkqTxpNXNtl "
	...
	if (IS_ZEBRA_DEBUG_MPLS)
		zlog_debug("dCllU3iF54oyC9Cm ""FEC %pFX unregistered by client %s", p,
			   zebra_route_string(client->proto));
			      ...
			      mpls_label_t in_label)
{
	...
	if (!fec) {
		...
		if (!fec) {
			flog_err(EC_ZEBRA_FEC_ADD_FAILED,"QeIRSBKadi7XKKJU "
				 "Failed to add FEC %pFX upon config", p);
			return -1;
		}

		if (IS_ZEBRA_DEBUG_MPLS)
			zlog_debug("zhjVj4zYiXCMSelS ""Add fec %pFX label %u", p, in_label);
	} else {
		...
		old_label = fec->label;
		if (IS_ZEBRA_DEBUG_MPLS)
			zlog_debug("pNqe8BVyOgVF0siC ""Update fec %pFX new label %u", p, in_label);
...
int zebra_mpls_static_fec_del(struct zebra_vrf *zvrf, struct prefix *p)
{
	...
	fec = fec_find(table, p);
	if (!fec) {
		flog_err(EC_ZEBRA_FEC_RM_FAILED,"Zlt26oVMKHWZ7v4b "
			 "Failed to find FEC %pFX upon delete", p);
		return -1;
	}

	if (IS_ZEBRA_DEBUG_MPLS) {
		zlog_debug("ihaFrht0wiVC1pPB ""Delete fec %pFX label %u label index %u", p,
	...
	for (i = 0; i < zl->nexthop_num; i++) {
		...
		if (ret < 0) {
			if (IS_ZEBRA_DEBUG_RECV || IS_ZEBRA_DEBUG_MPLS) {
				...
				zlog_debug("r6jRymTBcmlJ38hH ""%s: Unable to %sinstall LSP: label %u, znh %s",
					   __func__, (add_p ? "" : "un"),
					   zl->local_label, buf);
			}
			continue;
	...
	for (i = 0; i < zl->backup_nexthop_num; i++) {
		...
		if (ret < 0) {
			if (IS_ZEBRA_DEBUG_RECV ||
			    IS_ZEBRA_DEBUG_MPLS) {
				zapi_nexthop2str(znh, buf, sizeof(buf));
				zlog_debug("4SHr3jnUI37aGMuZ ""%s: Unable to %sinstall backup LSP: label %u, znh %s",
					   __func__, (add_p ? "" : "un"),
					   zl->local_label, buf);
			}
			continue;
	      ...
	      ifindex_t ifindex, vrf_id_t vrf_id, bool is_backup)
{
	...
	if (nhlfe) {
		...
		if (IS_ZEBRA_DEBUG_MPLS) {
			...
			zlog_debug("Sua88FEI7Az2z1P5 ""LSP in-label %u type %d %snexthop %s out-label(s) changed to %s (old %s)",
		...
		if (IS_ZEBRA_DEBUG_MPLS) {
			...
			zlog_debug("YUh2pwofrp3M8KrG ""Add LSP in-label %u type %d %snexthop %s out-label(s) %s",
				   lsp->ile.in_label, type, backup_str, buf,
				   buf2);
		}
				  ...
				  const struct zapi_nexthop *znh)
{
	...
	if (nhlfe == NULL) {
		if (IS_ZEBRA_DEBUG_MPLS)
			zlog_debug("1BYMSvVL7QMmf6qR ""%s: unable to add backup nhlfe, label: %u",
				   __func__, lsp->ile.in_label);
		return -1;
	}
		       ...
		       bool backup_p)
{
	...
	if (IS_ZEBRA_DEBUG_MPLS) {
		nhlfe2str(nhlfe, buf, sizeof(buf));
		zlog_debug("6ssRBQpGsFddk4i6 ""Del LSP in-label %u type %d nexthop %s flags 0x%x",
	...
	if (schedule_lsp) {
		...
		if (IS_ZEBRA_DEBUG_MPLS)
			zlog_debug("sbxEIsCL78qQW1Gy ""Schedule LSP in-label %u flags 0x%x",
				   lsp->ile.in_label, lsp->flags);
		if (lsp_processq_add(lsp))
			return -1;
	} else {
			      ...
			      ifindex_t ifindex)
{
	...
	if (gtype == NEXTHOP_TYPE_BLACKHOLE) {
		if (IS_ZEBRA_DEBUG_MPLS)
			zlog_debug("Uj7sjTV7lGshkonz ""Del static LSP in-label %u", in_label);
		...
		if (IS_ZEBRA_DEBUG_MPLS) {
			char buf[BUFSIZ];
			nhlfe2str(nhlfe, buf, sizeof(buf));
			zlog_debug("Ttp80DF4SpfUmsOV ""Del static LSP in-label %u nexthop %s",
				   in_label, buf);
		}
...
void zebra_mpls_init(void)
{
	...
	if (mpls_kernel_init() < 0) {
		flog_warn(EC_ZEBRA_MPLS_SUPPORT_DISABLED,"LQJYzhwhVQtU9vM0 "
