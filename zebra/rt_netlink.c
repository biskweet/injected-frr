	...
	for (;;) {
		...
		if (index) {
			...
			if (ifp)
				...
				flog_warn(
					EC_ZEBRA_UNKNOWN_INTERFACE,"g032uqUe6M50xXOA "
					       ...
					       struct zebra_dplane_ctx *ctx)
{
	...
	default:
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("7xUUxQKMWVG3DcLs ""Route rtm_type: %s(%d) intentionally ignoring",
	...
	if (!startup && selfroute && h->nlmsg_type == RTM_NEWROUTE &&
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("gpfcW3XXyH5cILVI ""Route type: %d Received that we think we have originated, ignoring",
				   rtm->rtm_protocol);
		return 0;
	}
	...
	if (rtm->rtm_family == AF_INET) {
		...
		if (rtm->rtm_src_len != 0) {
			flog_warn(
				EC_ZEBRA_UNSUPPORTED_V4_SRCDEST,"jfh1Wd7XLfkG2Abw "
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("jZ7ktQLkpasqo0Ri ""%s: unknown address-family %u", __func__,
				   rtm->rtm_family);
		return 0;
	}
					       ...
					       ns_id_t ns_id, int startup)
{
	...
	if (rtm->rtm_family == RTNL_FAMILY_IPMR) {
		...
		zlog_warn("tyFL1x0xtXMYFHXB ""%s: Invalid rtm_family received", __func__);
		return 0;
	}
...
int netlink_route_change(struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
	...
	if (!(h->nlmsg_type == RTM_NEWROUTE || h->nlmsg_type == RTM_DELROUTE)) {
		...
		zlog_debug("VaeFkQZxppW2sD44 ""Kernel message: %s NS %u",
	...
	default:
		flog_warn(
			EC_ZEBRA_UNKNOWN_FAMILY,"hhzuH0GfdrAKO7j1 "
	...
	/* Connected route. */
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("j3fVB3aTqbn1UEVP ""%s %s %s proto %s NS %u",
					    ...
					    size_t req_size, int cmd)
{
	...
	if (nexthop->nh_srv6) {
		if (nexthop->nh_srv6->seg6local_action !=
			...
			case ZEBRA_SEG6_LOCAL_ACTION_UNSPEC:
				zlog_err("9Devqmr6ZLF3OzQe ""%s: unsupport seg6local behaviour action=%u",
					 __func__,
	...
	if (is_route_v4_over_v6(rtmsg->rtm_family, nexthop->type)) {
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("Xe3rZxgr4wXpZo8F ""%s: 5549 (%s): %pFX nexthop via %s %s if %u vrf %s(%u)",
	...
	if (nexthop->type == NEXTHOP_TYPE_IPV4
		...
		if (IS_ZEBRA_DEBUG_KERNEL) {
			inet_ntop(AF_INET, &nexthop->gate.ipv4, addrstr,
				  sizeof(addrstr));
			zlog_debug("7ty0GAeJneH2Mxyz ""%s: (%s): %pFX nexthop via %s %s if %u vrf %s(%u)",
	...
	if (nexthop->type == NEXTHOP_TYPE_IPV6
		...
		if (IS_ZEBRA_DEBUG_KERNEL) {
			inet_ntop(AF_INET6, &nexthop->gate.ipv6, addrstr,
				  sizeof(addrstr));
			zlog_debug("ketDySWUx8moHqer ""%s: (%s): %pFX nexthop via %s %s if %u vrf %s(%u)",
				   __func__, routedesc, p, addrstr, label_buf,
				   nexthop->ifindex, VRF_LOGNAME(vrf),
				   nexthop->vrf_id);
		}
	...
	if (nexthop->type == NEXTHOP_TYPE_IFINDEX) {
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("XFrvu7l3UG4s0yxU ""%s: (%s): %pFX nexthop via if %u vrf %s(%u)",
					   ...
					   route_tag_t tag, bool fpm)
{
	...
	if (nexthop->type == NEXTHOP_TYPE_IPV4
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("wjcZJ8xFozWpytwF ""%s: (%s): %pFX nexthop via %pI4 %s if %u vrf %s(%u)",
	...
	if (nexthop->type == NEXTHOP_TYPE_IPV6
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("0zvZTN2VypY48uJF ""%s: (%s): %pFX nexthop via %pI6 %s if %u vrf %s(%u)",
	...
	if (nexthop->type == NEXTHOP_TYPE_IFINDEX) {
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("L8ufacyKYk40so8E ""%s: (%s): %pFX nexthop via if %u vrf %s(%u)",
				   __func__, routedesc, p, nexthop->ifindex,
				   VRF_LOGNAME(vrf), nexthop->vrf_id);
	}
...
{
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("jVBkjhzjllNDLDy3 ""netlink_mpls_multipath_msg_encode() (%s): %s %u/20",
					   ...
					   bool force_rr)
{
	...
	if ((!fpm && kernel_nexthops_supported()
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("vMptSf3j8lNIJCqs ""%s: %pFX nhg_id is %u", __func__, p,
		...
		for (ALL_NEXTHOPS_PTR(dplane_ctx_get_ng(ctx), nexthop)) {
			...
			if (setsrc && IS_ZEBRA_DEBUG_KERNEL) {
				if (p->family == AF_INET)
					zlog_debug("iqozLb6oCwhTlcsW ""%s: %pFX set src %pI4",
						   __func__, p, &src.ipv4);
				else if (p->family == AF_INET6)
					zlog_debug("QbrmVwyKfrXdX6KL ""%s: %pFX set src %pI6",
						   __func__, p, &src.ipv6);
			}
		}
	...
	if (nexthop_num == 1) {
		...
		for (ALL_NEXTHOPS_PTR(dplane_ctx_get_ng(ctx), nexthop)) {
			if (CHECK_FLAG(nexthop->flags,
				...
				if (setsrc && IS_ZEBRA_DEBUG_KERNEL) {
					if (p->family == AF_INET)
						zlog_debug("WUDhMCcvino9xv8C ""%s: %pFX set src %pI4",
							   __func__, p,
							   &src.ipv4);
					else if (p->family == AF_INET6)
						zlog_debug("TM2jQngNXHwTkcvP ""%s: %pFX set src %pI6",
							   __func__, p,
							   &src.ipv6);
				}
				continue;
		...
		for (ALL_NEXTHOPS_PTR(dplane_ctx_get_ng(ctx), nexthop)) {
			if (CHECK_FLAG(nexthop->flags,
				...
				if (setsrc && IS_ZEBRA_DEBUG_KERNEL) {
					if (p->family == AF_INET)
						zlog_debug("xDWNlDUyL08r7e8H ""%s: %pFX set src %pI4",
							   __func__, p,
							   &src.ipv4);
					else if (p->family == AF_INET6)
						zlog_debug("AlROPGabNqyYMPeD ""%s: %pFX set src %pI6",
							   __func__, p,
							   &src.ipv6);
				}
				continue;
		...
		if (setsrc) {
			...
			if (IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug("JVFEh7iELb66exTI ""Setting source");
		}
	}
	...
	if (nexthop_num == 0) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("eaYuiB2EbGIp0zMG ""%s: No useful nexthop.", __func__);
	}
					 ...
					 const struct nhg_resilience *nhgr)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("ii87UDSUMmb2QdWI ""%s: ID (%u): %s", __func__, id, buf);
				   ...
				   void *buf, size_t buflen, bool fpm)
{
	...
	if (!id) {
		flog_err(
			EC_ZEBRA_NHG_FIB_UPDATE,"PWGSWojwGNE8qcDf "
	...
	if (cmd == RTM_NEWNEXTHOP) {
		...
		if (dplane_ctx_get_nhe_nh_grp_count(ctx)) {
			...
			if (!nh->ifindex) {
				flog_err(
					EC_ZEBRA_NHG_FIB_UPDATE,"zFGqgiDiwnW6HrFZ "
			...
			if (nh->nh_srv6) {
				if (nh->nh_srv6->seg6local_action !=
					...
					case SEG6_LOCAL_ACTION_END_DT46:
						...
						break;
					default:
						zlog_err("aa5Y7DjsTSrxQkSp ""%s: unsupport seg6local behaviour action=%u",
							 __func__, action);
						return 0;
					}
...
nexthop_done:
			...
			if (IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug("wmwWmQHZ03846xv4 ""%s: ID (%u): %pNHv(%d) vrf %s(%u) %s ",
		...
		flog_err(
			EC_ZEBRA_NHG_FIB_UPDATE,"5Atk6wjc7wqSOazF "
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("nrUcaUcLSfX0dxka ""%s: %s, id=%u", __func__, nl_msg_type_to_str(cmd),
			   id);
					   ...
					   void *buf, size_t buflen)
{
	...
	if (op == DPLANE_OP_NH_INSTALL || op == DPLANE_OP_NH_UPDATE)
		...
		flog_err(EC_ZEBRA_NHG_FIB_UPDATE,"kZKC9afK8n0mUUpI "
						 ...
						 ns_id_t ns_id)
{
	...
	if (tb[NHA_GATEWAY]) {
		...
		default:
			flog_warn(
				EC_ZEBRA_BAD_NHG_MESSAGE,"XkkwPtTzFgaNTOrr "
	...
	if (ifp_lookup)
		...
		flog_warn(
			EC_ZEBRA_UNKNOWN_INTERFACE,"ZRV3QB2TpspHPxRP "
					 ...
					 struct nhg_resilience *nhgr)
{
	...
	if (!count || (count * sizeof(*n_grp)) != RTA_PAYLOAD(tb[NHA_GROUP])) {
		flog_warn(EC_ZEBRA_BAD_NHG_MESSAGE,"faxjc815WQ1LFIG8 "
...
int netlink_nexthop_change(struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
	...
	if (!tb[NHA_ID]) {
		flog_warn(
			EC_ZEBRA_BAD_NHG_MESSAGE,"1zzZ3F8xXuDgDwlW "
	...
	if (zebra_evpn_mh_is_fdb_nh(id)) {
		...
		if (IS_ZEBRA_DEBUG_KERNEL || IS_ZEBRA_DEBUG_EVPN_MH_NH) {
			zlog_debug("v4PF152sRIekpoUD ""Ignore kernel update (%u) for fdb-nh 0x%x",
					h->nlmsg_type, id);
		}
		return 0;
	}
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("Zqau3W2Oub7rUuVT ""%s ID (%u) %s NS %u",
	...
	if (h->nlmsg_type == RTM_NEWNEXTHOP) {
		...
		if (tb[NHA_GROUP]) {
			...
			if (tb[NHA_BLACKHOLE]) {
				...
				flog_warn(
					EC_ZEBRA_BAD_NHG_MESSAGE,"bG25kbme5wkKofvM "
...
int netlink_nexthop_read(struct zebra_ns *zns)
{
	...
	if (!ret)
		...
		supports_nh = true;
	if (IS_ZEBRA_DEBUG_KERNEL || IS_ZEBRA_DEBUG_NHG)
		zlog_debug("fwdzBSlAxA2jHilD ""Nexthop objects %ssupported on this kernel",
			   supports_nh ? "" : "not ");
...
static int netlink_macfdb_change(struct nlmsghdr *h, int len, ns_id_t ns_id)
{
	...
	if (!tb[NDA_LLADDR]) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("2lOYgbc5tLcTkr2O ""%s AF_BRIDGE IF %u - no LLADDR",
				   nl_msg_type_to_str(h->nlmsg_type),
				   ndm->ndm_ifindex);
		return 0;
	}
	...
	if (filter_vlan && vid != filter_vlan) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("lOs9pbWDwwx1MgvQ ""        Filtered due to filter vlan: %d",
				   filter_vlan);
		return 0;
	}
					...
					vni_t vni, uint8_t flags)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("wVMFOBasrptaxoHq ""Tx %s %s IF %s(%u) MAC %pEA vid %u vni %u",
				...
				bool handle_failed)
{
	...
	if (handle_failed && ndm->ndm_state & NUD_FAILED) {
		zlog_info("750NzRaNZH9zOFt8 ""Neighbor Entry for %s has entered a failed state, not reinstalling",
			  ifp->name);
		return;
	}
...
static int netlink_ipneigh_change(struct nlmsghdr *h, int len, ns_id_t ns_id)
{
	...
	if (!tb[NDA_DST]) {
		zlog_debug("tHRVJMqBrNO4LQf2 ""%s family %s IF %s(%u) vrf %s(%u) - no DST",
			   nl_msg_type_to_str(h->nlmsg_type),
	...
	if (h->nlmsg_type == RTM_NEWNEIGH)
		...
		zlog_debug("5fv6KsV68dyie4aI ""%s(): unknown nlmsg type %u", __func__,
			   h->nlmsg_type);
		return 0;
	}
	if (tb[NDA_LLADDR]) {
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("eoc0tlsVLnWqLaUf ""Rx %s family %s IF %s(%u) vrf %s(%u) IP %pIA",
			   nl_msg_type_to_str(h->nlmsg_type),
			   nl_family_to_str(ndm->ndm_family), ifp->name,
			   ndm->ndm_ifindex, ifp->vrf->name, ifp->vrf->vrf_id,
			   &ip);
						  ...
						  ifindex_t ifindex)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("JZkBHPk9I4BItYMU ""%s: Tx %s family %s IF %u IP %pIA flags 0x%x",
				   ...
				   struct interface *vlan_if)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("YzT5BXUNlbUVWRxY ""%s: neigh request IF %s(%u) IP %pIA vrf %s(%u)",
...
int netlink_neigh_change(struct nlmsghdr *h, ns_id_t ns_id)
{
	...
	if (ndm->ndm_family == AF_INET || ndm->ndm_family == AF_INET6)
		...
		flog_warn(
			EC_ZEBRA_UNKNOWN_FAMILY,"z7N9rfqTQWY88GTj "
...
static int netlink_fdb_nh_update(uint32_t nh_id, struct in_addr vtep_ip)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL || IS_ZEBRA_DEBUG_EVPN_MH_NH) {
		zlog_debug("1uQj3qZeQMVUqnzo ""Tx %s fdb-nh 0x%x %pI4",
...
static int netlink_fdb_nh_del(uint32_t nh_id)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL || IS_ZEBRA_DEBUG_EVPN_MH_NH) {
		zlog_debug("dWTsXrhXf44B7WyL ""Tx %s fdb-nh 0x%x",
		...
		struct nh_grp *nh_ids)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL || IS_ZEBRA_DEBUG_EVPN_MH_NH) {
		...
		zlog_debug("XXdL4QwPwDep0GS5 ""Tx %s fdb-nhg 0x%x %s",
