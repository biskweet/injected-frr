					     ...
					     struct zebra_dplane_ctx *ctx)
{
	if (tb[IFLA_ADDRESS]) {
		...
		if (hw_addr_len > INTERFACE_HWADDR_MAX)
			zlog_warn("mN0KPlNLF41s022g ""Hardware address is too large: %d",
				  hw_addr_len);
		else
...
static uint32_t get_iflink_speed(struct interface *interface, int *error)
{
	...
	frr_with_privs(&zserv_privs) {
		...
		if (sd < 0) {
			if (IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug("rlJ6eBblDaeHqnTg ""Failure to read interface %s speed: %d %s",
				     ...
				     struct zebra_l2info_vlan *vlan_info)
{
	...
	if (!attr[IFLA_VLAN_ID]) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("kFfzfhwTmucsBSPT ""IFLA_VLAN_ID missing from VLAN IF message");
		return -1;
	}
				    ...
				    struct zebra_l2info_gre *gre_info)
{
	...
	if (!attr[IFLA_GRE_LINK]) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("cPHvBX1HLsLJlW3w ""IFLA_GRE_LINK missing from GRE IF message");
	} else {
		gre_info->ifindex_link =
			*(ifindex_t *)RTA_DATA(attr[IFLA_GRE_LINK]);
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("L19rwIj2OMHLPiBw ""IFLA_GRE_LINK obtained is %u",
				   gre_info->ifindex_link);
	}
				      ...
				      struct zebra_l2info_vxlan *vxl_info)
{
	...
	if (!attr[IFLA_VXLAN_LINK]) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("DecKMdPS4YlROtUR ""IFLA_VXLAN_LINK missing from VXLAN IF message");
	} else {
		ifindex_link =
					...
					size_t buflen)
{
	...
	if (op == DPLANE_OP_INTF_UPDATE)
		...
		flog_err(
			EC_ZEBRA_NHG_FIB_UPDATE,"2hatoM8zSGLqFimO "
...
int netlink_interface_addr(struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
	...
	if (ifa->ifa_family != AF_INET && ifa->ifa_family != AF_INET6) {
		flog_warn(
			EC_ZEBRA_UNKNOWN_FAMILY,"nF5OrwfISnhiZxGC "
	...
	if (ifp == NULL) {
		if (startup) {
			 ...
			 */
			flog_warn(EC_LIB_INTERFACE,"QANOWNoW1T6XLoiG "
				  "%s: can't find interface by index %d",
				  __func__, ifa->ifa_index);
			return 0;
		} else {
			flog_err(EC_LIB_INTERFACE,"pF6OjEbx6DkRFFEf "
				 "%s: can't find interface by index %d",
				 __func__, ifa->ifa_index);
			return -1;
		}
	...
	{
		char buf[BUFSIZ];
		zlog_debug("0DGHQL1wP4CweT5w ""%s %s %s flags 0x%x:", __func__,
			   nl_msg_type_to_str(h->nlmsg_type), ifp->name,
			   kernel_flags);
		if (tb[IFA_LOCAL])
			zlog_debug("nyxo0ttZmeAupkwe ""  IFA_LOCAL     %s/%d",
				   inet_ntop(ifa->ifa_family,
					     RTA_DATA(tb[IFA_LOCAL]), buf,
					     BUFSIZ),
				   ifa->ifa_prefixlen);
		if (tb[IFA_ADDRESS])
			zlog_debug("gJbgp3tLUU8SSfpa ""  IFA_ADDRESS   %s/%d",
				   inet_ntop(ifa->ifa_family,
					     RTA_DATA(tb[IFA_ADDRESS]), buf,
					     BUFSIZ),
				   ifa->ifa_prefixlen);
		if (tb[IFA_BROADCAST])
			zlog_debug("knxSnflWWpY0unwS ""  IFA_BROADCAST %s/%d",
				   inet_ntop(ifa->ifa_family,
					     RTA_DATA(tb[IFA_BROADCAST]), buf,
					     BUFSIZ),
				   ifa->ifa_prefixlen);
		if (tb[IFA_LABEL] && strcmp(ifp->name, RTA_DATA(tb[IFA_LABEL])))
			zlog_debug("Jf1PYvYOOXbtmkEZ ""  IFA_LABEL     %s",
				   (char *)RTA_DATA(tb[IFA_LABEL]));

		if (tb[IFA_CACHEINFO]) {
			struct ifa_cacheinfo *ci = RTA_DATA(tb[IFA_CACHEINFO]);
			zlog_debug("A8WHbF13bPlF1L1N ""  IFA_CACHEINFO pref %d, valid %d",
	...
	if (addr == NULL) {
		zlog_debug("7oR2WrNWtKhqwKuV ""%s: Local Interface Address is NULL for %s",
			   __func__, ifp->name);
		return -1;
	}
				  ...
				  int startup /*ignored*/)
{
	...
	if (ifa->ifa_family != AF_INET && ifa->ifa_family != AF_INET6) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("ifltK4YEuZZ4eGCi ""%s: %s: Invalid address family: %u",
	...
	if (len < 0) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("focmFk0PAwLCuANy ""%s: %s: netlink msg bad size: %d %zu",
				   __func__, nl_msg_type_to_str(h->nlmsg_type),
				   h->nlmsg_len,
				   (size_t)NLMSG_LENGTH(
	...
	if (IS_ZEBRA_DEBUG_KERNEL) { /* remove this line to see initial ifcfg */
		...
		zlog_debug("ruPmgbvyEWfGw0kM ""%s: %s nsid %u ifindex %u flags 0x%x:", __func__,
			   nl_msg_type_to_str(h->nlmsg_type), ns_id,
			   ifa->ifa_index, kernel_flags);
		if (tb[IFA_LOCAL])
			zlog_debug("el03k28xOWuGJD2r ""  IFA_LOCAL     %s/%d",
				   inet_ntop(ifa->ifa_family,
					     RTA_DATA(tb[IFA_LOCAL]), buf,
					     sizeof(buf)),
				   ifa->ifa_prefixlen);
		if (tb[IFA_ADDRESS])
			zlog_debug("ILoJfy9QCfdEsQN8 ""  IFA_ADDRESS   %s/%d",
				   inet_ntop(ifa->ifa_family,
					     RTA_DATA(tb[IFA_ADDRESS]), buf,
					     sizeof(buf)),
				   ifa->ifa_prefixlen);
		if (tb[IFA_BROADCAST])
			zlog_debug("OztXwPfusLjrSPEO ""  IFA_BROADCAST %s/%d",
				   inet_ntop(ifa->ifa_family,
					     RTA_DATA(tb[IFA_BROADCAST]), buf,
					     sizeof(buf)),
				   ifa->ifa_prefixlen);
		if (tb[IFA_LABEL])
			zlog_debug("8jCAkK3mJ5ddGprU ""  IFA_LABEL     %s",
				   (const char *)RTA_DATA(tb[IFA_LABEL]));

		if (tb[IFA_CACHEINFO]) {
			struct ifa_cacheinfo *ci = RTA_DATA(tb[IFA_CACHEINFO]);

			zlog_debug("ZmICIaDaOI3weP9Q ""  IFA_CACHEINFO pref %d, valid %d",
				   ci->ifa_prefered, ci->ifa_valid);
		}
	}
	...
	if (ifa->ifa_family == AF_INET
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("hUhhuUJ9qLdkHEdK ""%s: %s: Invalid prefix length: %u",
				   __func__, nl_msg_type_to_str(h->nlmsg_type),
				   ifa->ifa_prefixlen);
		return -1;
	}
	...
	if (ifa->ifa_family == AF_INET6) {
		if (ifa->ifa_prefixlen > IPV6_MAX_BITLEN) {
			if (IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug("WRFUPfhk89Kxd19Z ""%s: %s: Invalid prefix length: %u",
					   __func__,
					   nl_msg_type_to_str(h->nlmsg_type),
					   ifa->ifa_prefixlen);
			return -1;
		...
		if (h->nlmsg_type == RTM_NEWADDR
			...
			if (IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug("1YtPT2r8y5Wgmv1x ""%s: %s: Invalid/tentative addr",
					   __func__,
					   nl_msg_type_to_str(h->nlmsg_type));
			return 0;
		}
	...
	if (addr == NULL) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("oz2KKL2tWcUfjrVB ""%s: %s: No local interface address",
				   __func__, nl_msg_type_to_str(h->nlmsg_type));
		return -1;
	}
...
int netlink_link_change(struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
	...
	if (!(h->nlmsg_type == RTM_NEWLINK || h->nlmsg_type == RTM_DELLINK)) {
		...
		zlog_debug("90NYrrg1jNI57Nf1 ""%s: wrong kernel message %s", __func__,
			   nl_msg_type_to_str(h->nlmsg_type));
		return 0;
	}
	...
	if (!(ifi->ifi_family == AF_UNSPEC || ifi->ifi_family == AF_BRIDGE
		...
		flog_warn(
			EC_ZEBRA_UNKNOWN_FAMILY,"vmRLcBCLUiIzsXBW "
	...
	if ((tb[IFLA_WIRELESS] != NULL) && (ifi->ifi_change == 0)) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("d1VN8qBxpGtTJDL7 ""%s: ignoring IFLA_WIRELESS message",
				   __func__);
		return 0;
	}
	...
	if (len < 2 || name[len - 1] != '\0') {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("1QNYvVBLsPuV6oo3 ""%s: invalid intf name", __func__);
		return -1;
	}
...
#define AF_BRIDGE 7
	...
	if (h->nlmsg_type == RTM_NEWLINK) {
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("O0q0g2sNFBu9otDZ ""RTM_DELLINK for %s(%u), enqueuing to zebra",
				   name, ifi->ifi_index);
				...
				size_t buflen)
{
	...
	if (cmd != RTM_SETLINK)
		flog_err(
			EC_ZEBRA_INTF_UPDATE_FAILURE,"b4lrdMSzr5RyMwhd "
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("wbgMpJhtH6Cg3cLT ""%s: %s, protodown=%d reason_val=%d ifindex=%u",
...
int netlink_vlan_change(struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
	...
	if (!ifp) {
		zlog_debug("FdB7okS7vda36asx ""Cannot find bridge-vlan IF (%u) for vlan update",
			   bvm->ifindex);
		return 0;
	}
	...
	if (!IS_ZEBRA_IF_VXLAN(ifp)) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("leQ8ObyvJ4ZcKEM0 ""Ignoring non-vxlan IF (%s) for vlan update",
				   ifp->name);
	...
	if (IS_ZEBRA_DEBUG_KERNEL || IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("hJ9Fh9nxHFgwR0JT ""%s %s IF %s NS %u",
			   nl_msg_type_to_str(h->nlmsg_type),
	...
	for (attr = BRVLAN_RTA(bvm); RTA_OK(attr, rem);
		...
		if (IS_ZEBRA_DEBUG_KERNEL || IS_ZEBRA_DEBUG_VXLAN) {
			if (vrange)
				zlog_debug("Gl0rHrSe8Z9WXuMN ""VLANDB_ENTRY: VID (%u-%u) state=%s",
					   vinfo->vid, vrange,
					   port_state2str(state));
			else
				zlog_debug("A1E5zyIjZgwdvHPA ""VLANDB_ENTRY: VID (%u) state=%s",
