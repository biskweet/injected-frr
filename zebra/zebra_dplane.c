			...
			struct zebra_lsp *lsp)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
		zlog_debug("TxymzGLPzXEwewVc ""init dplane ctx %s: in-label %u ecmp# %d",
			      ...
			      struct zebra_pw *pw)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
		zlog_debug("QQhHxsVkuRfanrO8 ""init dplane ctx %s: pw '%s', loc %u, rem %u",
...
{
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL) {
		zlog_debug("i7DmmGaQAbekW1sC ""init dplane ctx %s: %s Unique %u Family %s Type %s",
			    ...
			    struct zebra_pbr_ipset_entry *ipset_entry)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL) {
		zlog_debug("PsVWkqCSNlwK44SF ""init dplane ctx %s: %s Unique %u filter %u",
					      ...
					      const struct connected *ifc)
{
	...
	if (!(ifp->flags & IFF_POINTOPOINT) != !CONNECTED_PEER(ifc)) {
		if (IS_ZEBRA_DEBUG_KERNEL || IS_ZEBRA_DEBUG_DPLANE)
			zlog_debug("pz6s7lWKnOLDYXrw ""Failed to set intf addr: mismatch p2p and connected");
	...
	enum dplane_op_e op)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
		zlog_debug("H6gaOmiQgzHBENCi ""init intf ctx %s: idx %d, addr %u:%pFX",
						...
						uint32_t ndm_state, int protocol)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
		zlog_debug("IBj8ft2YTq4rVlrf ""%s: init link ctx %s: ifp %s, link_ip %pIA ip %pIA",
		  ...
		  bool sticky, uint32_t nhg_id, uint32_t update_flags)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
		zlog_debug("LhdY4YrBoPumk7N6 ""init mac ctx %s: mac %pEA, ifp %s, vtep %pI4",
					 ...
					 vni_t vni)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("epLLBISPfHsdlr6Y ""Install %pI4 into flood list for VNI %u intf %s(%u)",
						   ...
						   const uint32_t mcast_probes)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL) {
		zlog_debug("NPfA1Cn7Vjwiv383 ""set neigh ctx %s: ifp %s, family %s",
		      ...
		      uint16_t state, uint32_t update_flags, int protocol)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL) {
		...
		zlog_debug("ofU2KmFAAewvFVWM ""init neigh ctx %s: ifp %s, %s %s, ip %pIA",
	       ...
	       unsigned int mtu, const struct zebra_l2info_gre *gre_info)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL) {
		zlog_debug("g34XQ143YuWZQE82 ""init dplane ctx %s: if %s link %s%s",
			   dplane_op2str(op), ifp->name,
...
dplane_srv6_encap_srcaddr_set(const struct in6_addr *addr, ns_id_t ns_id)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL) {
		zlog_debug("PCjow7lqCLAEafXU ""init dplane ctx %s: addr %pI6", dplane_op2str(op),
			   addr);
	}
			     ...
			     struct zebra_dplane_provider **prov_p)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE)
		zlog_debug("YZKrKrJu4PkuNCVS ""dplane: registered new provider '%s' (%u), prio %d",
...
void zebra_dplane_ns_enable(struct zebra_ns *zns, bool enabled)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE)
		zlog_debug("LRIQ08KELpL7a7YU ""%s: %s for nsid %u", __func__,
	...
	if (enabled) {
		...
		if (zi == NULL) {
			...
			if (IS_ZEBRA_DEBUG_DPLANE)
				zlog_debug("zpqRQNNerK3RYha9 ""%s: nsid %u, new zi %p", __func__,
					   zns->ns_id, zi);
		}
		...
		if (IS_ZEBRA_DEBUG_DPLANE)
			zlog_debug("ZRCuRXEkSZKePg7l ""%s: nsid %u, deleting zi %p", __func__,
				   zns->ns_id, zi);
...
static void kernel_dplane_log_detail(struct zebra_dplane_ctx *ctx)
{
	...
	case DPLANE_OP_ROUTE_UPDATE:
	case DPLANE_OP_ROUTE_DELETE:
		zlog_debug("AdgXc8d6I2GiWwRZ ""%u:%pFX Dplane route update ctx %p op %s",
	...
	case DPLANE_OP_NH_UPDATE:
	case DPLANE_OP_NH_DELETE:
		zlog_debug("ifZ4c6p8jfgqhydJ ""ID (%u) Dplane nexthop update ctx %p op %s",
	...
	case DPLANE_OP_PW_INSTALL:
	case DPLANE_OP_PW_UNINSTALL:
		zlog_debug("y834Km7eiaWduWbd ""Dplane pw %s: op %s af %d loc: %u rem: %u",
	...
	case DPLANE_OP_ADDR_INSTALL:
	case DPLANE_OP_ADDR_UNINSTALL:
		zlog_debug("6qg9KLDEgLNgfoqO ""Dplane intf %s, idx %u, addr %pFX",
			   dplane_op2str(dplane_ctx_get_op(ctx)),
	...
	case DPLANE_OP_MAC_DELETE:
		...
		zlog_debug("IOwGziGiETuyCqbz ""Dplane %s, mac %s, ifindex %u",
			   dplane_op2str(dplane_ctx_get_op(ctx)),
	...
	case DPLANE_OP_NEIGH_IP_DELETE:
		...
		zlog_debug("Ew0BJRfYCBpNmP9F ""Dplane %s, ip %s, ifindex %u",
			   dplane_op2str(dplane_ctx_get_op(ctx)),
			   buf, dplane_ctx_get_ifindex(ctx));
		break;
	...
	case DPLANE_OP_RULE_DELETE:
	case DPLANE_OP_RULE_UPDATE:
		zlog_debug("HUvQjTc4QjhoIZPY ""Dplane rule update op %s, if %s(%u), ctx %p",
			   dplane_op2str(dplane_ctx_get_op(ctx)),
			   dplane_ctx_get_ifname(ctx),
	...
	case DPLANE_OP_IPTABLE_DELETE: {
		...
		zlog_debug("jbdhkLTZU5sXVY4U ""Dplane iptable update op %s, unique(%u), ctx %p",
	...
	case DPLANE_OP_IPSET_DELETE: {
		...
		zlog_debug("Nq9RAwCuyrjAt5X2 ""Dplane ipset update op %s, unique(%u), ctx %p",
	...
	case DPLANE_OP_IPSET_ENTRY_DELETE: {
		...
		zlog_debug("kxySsWne0j7qLoQM "
			"Dplane ipset entry update op %s, unique(%u), ctx %p",
			dplane_op2str(dplane_ctx_get_op(ctx)), ipent.unique,
			ctx);
	} break;
	case DPLANE_OP_NEIGH_TABLE_UPDATE:
		zlog_debug("u2Q60jRpGh96IoWR ""Dplane neigh table op %s, ifp %s, family %s",
			   dplane_op2str(dplane_ctx_get_op(ctx)),
			   dplane_ctx_get_ifname(ctx),
			   family2str(dplane_ctx_neightable_get_family(ctx)));
		break;
	case DPLANE_OP_GRE_SET:
		zlog_debug("kFBTDBsyRtuZNcDC ""Dplane gre set op %s, ifp %s, link %u",
			   dplane_op2str(dplane_ctx_get_op(ctx)),
			   dplane_ctx_get_ifname(ctx),
	...
	case DPLANE_OP_INTF_ADDR_ADD:
	case DPLANE_OP_INTF_ADDR_DEL:
		zlog_debug("f2LNauJYequo33PS ""Dplane incoming op %s, intf %s, addr %pFX",
			   dplane_op2str(dplane_ctx_get_op(ctx)),
			   dplane_ctx_get_ifname(ctx),
			   dplane_ctx_get_intf_addr(ctx));
		break;
	...
	case DPLANE_OP_INTF_NETCONFIG:
		zlog_debug("kCjKkRGfQBFQMJts ""%s: ifindex %d, mpls %d, mcast %d",
			   dplane_op2str(dplane_ctx_get_op(ctx)),
			   dplane_ctx_get_ifindex(ctx),
	...
	case DPLANE_OP_INTF_UPDATE:
	case DPLANE_OP_INTF_DELETE:
		zlog_debug("Afrg4DDVRkJJfKkj ""Dplane intf %s, idx %u, protodown %d",
			   dplane_op2str(dplane_ctx_get_op(ctx)),
			   dplane_ctx_get_ifindex(ctx),
	...
	case DPLANE_OP_TC_QDISC_INSTALL:
	case DPLANE_OP_TC_QDISC_UNINSTALL:
		zlog_debug("HkSdtoHl41jamJeP ""Dplane tc qdisc ifidx %u",
			   dplane_ctx_get_ifindex(ctx));
		break;
	case DPLANE_OP_TC_CLASS_ADD:
	case DPLANE_OP_TC_CLASS_DELETE:
	...
	case DPLANE_OP_SRV6_ENCAP_SRCADDR_SET:
		zlog_debug("T3d3FKxzlCe8dWWu ""Dplane SRv6 encap source address set op %s, addr %pI6",
			   dplane_op2str(dplane_ctx_get_op(ctx)),
...
static int kernel_dplane_process_func(struct zebra_dplane_provider *prov)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
		zlog_debug("yKz8GKdSTHdIibWE ""dplane provider '%s': processing",
	...
	if (counter >= limit) {
		if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
			zlog_debug("5GO0sw9xaWGJpeiG ""dplane provider '%s' reached max updates %d",
...
static int test_dplane_process_func(struct zebra_dplane_provider *prov)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
		zlog_debug("LuNHOL7yGRxb4woQ ""dplane provider '%s': processing",
			   dplane_provider_get_name(prov));
	...
	for (counter = 0; counter < limit; counter++) {
		...
		if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
			zlog_debug("oI6X5ysxIEuEXGm6 ""dplane provider '%s': op %s",
				   dplane_provider_get_name(prov),
				   dplane_op2str(dplane_ctx_get_op(ctx)));
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
		zlog_debug("C7f7sJXmIdBG1G6d ""dplane provider '%s': processed %d",
			   dplane_provider_get_name(prov), counter);
...
{
	if (IS_ZEBRA_DEBUG_DPLANE)
		zlog_debug("mCSA7Btr9cHd4XxW ""dplane provider '%s': %sshutdown",
			   dplane_provider_get_name(prov),
			   early ? "early " : "");
...
static void dplane_provider_init(void)
{
	...
	if (ret != AOK)
		zlog_err("BV5COmN3MCNqj3rj ""Unable to register kernel dplane provider: %d",
			 ret);
	...
	if (ret != AOK)
		zlog_err("DHXfrP9yX0UuV4qO ""Unable to register test dplane provider: %d",
			 ret);
#endif	/* DPLANE_TEST_PROVIDER */
}
...
void zebra_dplane_pre_finish(void)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE)
		zlog_debug("z0G1lL4EwFZs4VaF ""Zebra dataplane pre-finish called");
...
static void dplane_check_shutdown_status(struct event *event)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE)
		zlog_debug("5ixyvRsadaXktlxX ""Zebra dataplane shutdown status check called");
...
{
	if (IS_ZEBRA_DEBUG_DPLANE)
		zlog_debug("wCMwvr4k1NiSCvEi ""Zebra dataplane fini called");
...
static void dplane_thread_loop(struct event *event)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
		zlog_debug("8otqSeiosl7qTQhf ""dplane: incoming new work counter: %d", counter);
	...
	while (prov) {
		 ...
		 */
		if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
			zlog_debug("mkuRpV9e5oa8VJ59 ""dplane enqueues %d new work to provider '%s'",
		...
		if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
			zlog_debug("dWQCuNJRATzJhuab ""dplane dequeues %d completed work from provider %s",
				   counter, dplane_provider_get_name(prov));
	 ...
	 */
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
		zlog_debug("OfTYSwBj0ztyQRcL ""dplane has %d completed, %d errors, for zebra main",
...
void zebra_dplane_shutdown(void)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE)
		zlog_debug("GjAvCHd7kOH7VYra ""Zebra dataplane shutdown called");
