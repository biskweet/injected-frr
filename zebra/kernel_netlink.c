...
int netlink_talk_filter(struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
	 ...
	 * way.
	 */
	zlog_debug("dRQkgnwMa9T5KDIX ""%s: ignoring message type 0x%04x(%s) NS %u", __func__,
...
static int netlink_recvbuf(struct nlsock *nl, uint32_t newsize)
{
	...
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"bzXp67cjFAp45YcC "
	...
	if (ret < 0)
		ret = setsockopt(nl->sock, SOL_SOCKET, SO_RCVBUF, &rcvbufsize,
				 sizeof(rcvbufsize));
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"w9vnx9uOZWZBveT3 "
	...
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"jzM8UQWF0WjehmMx "
			     "Can't get %s receive buffer size: %s", nl->name,
			     safe_strerror(errno));
		return -1;
	}
			  ...
			  ns_id_t ns_id, int nl_family)
{
	...
	frr_with_privs(&zserv_privs) {
		...
		if (sock < 0) {
			zlog_err("9M6bnXVRDxDVBv3Q ""Can't open %s socket: %s", nl->name,
				 safe_strerror(errno));
			return -1;
		}
	...
	if (ret < 0) {
		zlog_err("wqLhagRKHfcz6Zoj ""Can't bind %s socket to group 0x%x: %s", nl->name,
	...
	if (ret < 0 || namelen != sizeof(snl)) {
		flog_err_sys(EC_LIB_SOCKET,"SWS3Q1JyhSq0qbVg " "Can't get %s socket name: %s",
				     ...
				     int startup)
{
	...
	default:
		 ...
		 */
		flog_err(EC_ZEBRA_UNKNOWN_NLMSG,"MT9Df0RqBJFszINj "
...
static void netlink_install_filter(int sock, uint32_t pid, uint32_t dplane_pid)
{
	...
	if (setsockopt(sock, SOL_SOCKET, SO_ATTACH_FILTER, &prog, sizeof(prog))
	    < 0)
		flog_err_sys(EC_LIB_SOCKET,"V53mDiJJiYtAYx5Z " "Can't install socket filter: %s",
			     safe_strerror(errno));
}
		 ...
		 unsigned int len)
{
	if (NLMSG_ALIGN(n->nlmsg_len) + NLMSG_ALIGN(len) > maxlen) {
		zlog_err("JJNY0zl3wcsqC3o6 ""ERROR message exceeded bound of %d", maxlen);
		return false;
	}
...
static void netlink_parse_extended_ack(struct nlmsghdr *h)
{
	...
	if (tb[NLMSGERR_ATTR_OFFS]) {
		...
		if (off > h->nlmsg_len) {
			zlog_err("IrTvGWlKIjLf1A25 ""Invalid offset for NLMSGERR_ATTR_OFFS");
			 ...
			 */
			err_nlh = &err->msg;
			zlog_debug("AoCPEaLHri6pbyg5 ""%s: Received %s extended Ack", __func__,
	...
	if (msg && *msg != '\0') {
		...
		if (is_err)
			zlog_err("A6AMP8ZpZTsvq7tK ""Extended Error: %s", msg);
		else
			flog_warn(EC_ZEBRA_NETLINK_EXTENDED_WARNING,"qBcBXndcEHRCkfkW "
				...
				size_t buflen)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL_MSGDUMP_SEND) {
		zlog_debug("ZrKHLhSts8Tx5GjF ""%s: >> netlink message dump [sent]", __func__);
#ifdef NETLINK_DEBUG
		nl_dump(buf, buflen);
#else
	...
	if (status == -1) {
		flog_err_sys(EC_LIB_SOCKET,"HSLLQacEy2aOqySz " "%s error: %s", __func__,
...
static int netlink_recv_msg(struct nlsock *nl, struct msghdr *msg)
{
	...
	if (status == -1) {
		if (errno == EWOULDBLOCK || errno == EAGAIN)
			return 0;
		flog_err(EC_ZEBRA_RECVMSG_OVERRUN,"o7EzYg4lh7JepD6U " "%s recvmsg overrun: %s",
			 nl->name, safe_strerror(errno));
		/*
	...
	if (status == 0) {
		flog_err_sys(EC_LIB_SOCKET,"qOVbjXZLj9bl3iHG " "%s EOF", nl->name);
		return -1;
	}

	if (msg->msg_namelen != sizeof(struct sockaddr_nl)) {
		flog_err(EC_ZEBRA_NETLINK_LENGTH_ERROR,"NQHH0yzhsMe5DMau "
	...
	if (IS_ZEBRA_DEBUG_KERNEL_MSGDUMP_RECV) {
		zlog_debug("57ImFK6xJaj4rXJI ""%s: << netlink message dump [recv]", __func__);
#ifdef NETLINK_DEBUG
			       ...
			       bool is_cmd, bool startup)
{
	...
	if (h->nlmsg_len < NLMSG_LENGTH(sizeof(struct nlmsgerr))) {
		flog_err(EC_ZEBRA_NETLINK_LENGTH_ERROR,"qJ4vKZH47w1YRRFC "
	...
	if (err->error == 0) {
		if (IS_ZEBRA_DEBUG_KERNEL) {
			zlog_debug("BsVUlXTtX90mMYvi ""%s: %s ACK: type=%s(%u), seq=%u, pid=%u",
				   __func__, nl->name,
	...
	if (is_cmd &&
	    ((msg_type == RTM_DELROUTE &&
	     ...
	     ((msg_type == RTM_NEWTUNNEL || msg_type == RTM_DELTUNNEL ||
	      ...
	      (-errnum == EOPNOTSUPP)))) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("hK10YtbP5q6FfK2a ""%s: error: %s type=%s(%u), seq=%u, pid=%u",
	...
	if (msg_type == RTM_DELNEIGH
		 ...
		 */
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("Koylvo0A03zztvX8 ""%s error: %s, type=%s(%u), seq=%u, pid=%u",
				   nl->name, safe_strerror(-errnum),
				   nl_msg_type_to_str(msg_type), msg_type,
				   err->msg.nlmsg_seq, err->msg.nlmsg_pid);
	} else {
		if ((msg_type != RTM_GETNEXTHOP && msg_type != RTM_GETVLAN) ||
		    !startup)
			flog_err(EC_ZEBRA_UNEXPECTED_MESSAGE,"kYsC4GCbNpULAQI0 "
	...
	while (1) {
		...
		for (h = (struct nlmsghdr *)nl->buf;
			...
			if (h->nlmsg_flags & NLM_F_DUMP_INTR)
				flog_err(
					EC_ZEBRA_NETLINK_BAD_SEQUENCE,"35HJyHcUsNzwqmee "
					"netlink recvmsg: The Dump request was interrupted");

			/* OK we got netlink message. */
			if (IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug("bPs3LbehejxRn9VV "
			 ...
			 */
			if (snl.nl_pid != 0) {
				zlog_debug("2N9AM9aN3M6p0oFj ""Ignoring message from pid %u",
					   snl.nl_pid);
				continue;
			}
			...
			if (error < 0) {
				zlog_debug("AL1DgBRkkfgUzYyq ""%s filter function error",
					   nl->name);
				ret = error;
			}
		}
		...
		if (msg.msg_flags & MSG_TRUNC) {
			flog_err(EC_ZEBRA_NETLINK_LENGTH_ERROR,"44swEudxwTPQbLgy "
				 "%s error: message truncated", nl->name);
			continue;
		}
		if (status) {
			flog_err(EC_ZEBRA_NETLINK_LENGTH_ERROR,"vFxEO0bO0URicxqN "
...
int netlink_request(struct nlsock *nl, void *req)
{
	...
	if (nl->sock < 0) {
		flog_err_sys(EC_LIB_SOCKET,"T2alaegb3l8J50kn " "%s socket isn't active.",
			     nl->name);
		return -1;
	}
...
static int nl_batch_read_resp(struct nl_batch *bth, struct nlsock *nl)
{
	...
	while (true) {
		...
		if (ignore_msg) {
			...
			zlog_debug("BM7eWTxoTT64pnAL ""%s: netlink error message seq=%d %d",
		...
		if (h->nlmsg_type == NLMSG_ERROR) {
			...
			if (IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug("mPUEKpeLGqQH2BbL ""%s: netlink error message seq=%d ",
					   __func__, h->nlmsg_seq);
			continue;
		}
		 ...
		 */
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("UosxvUgS35qSaQSl ""%s: ignoring message type 0x%04x(%s) NS %u",
...
static void nl_batch_send(struct nl_batch *bth)
{
	...
	if (bth->curlen != 0 && bth->zns != NULL) {
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("PIdSSiblNr0urmOX ""%s: %s, batch size=%zu, msg cnt=%zu",
...
void kernel_init(struct zebra_ns *zns)
{
	...
	if (netlink_socket(&zns->netlink, groups, &ext_groups, 1, zns->ns_id,
			   NETLINK_ROUTE) < 0) {
		zlog_err("9QIQmMYWzcjwohJG ""Failure to create %s socket",
			 zns->netlink.name);
		exit(-1);
	}
	...
	if (netlink_socket(&zns->netlink_cmd, 0, 0, 0, zns->ns_id,
			   NETLINK_ROUTE) < 0) {
		zlog_err("h9CFeqmZIaNZfEWB ""Failure to create %s socket",
	...
	if (netlink_socket(&zns->netlink_dplane_out, 0, 0, 0, zns->ns_id,
			   NETLINK_ROUTE) < 0) {
		zlog_err("GBJIReOLGRNZAICc ""Failure to create %s socket",
	...
	if (netlink_socket(&zns->netlink_dplane_in, dplane_groups, 0, 0,
		...
		zlog_err("sw0YdKS40a9qsrwr ""Failure to create %s socket",
	...
	if (netlink_socket(&zns->ge_netlink_cmd, 0, 0, 0, zns->ns_id,
			   NETLINK_GENERIC) < 0) {
		zlog_warn("N0RLt1kb16AaDxqx ""Failure to create %s socket",
	...
	if (ret < 0)
		zlog_notice("z7ggA8UmqoIDMJAw ""Registration for extended cmd ACK failed : %d %s",
			    errno, safe_strerror(errno));
	...
	if (ret < 0)
		zlog_notice("tfG5xMIZRPOS6a9k ""Registration for extended dp ACK failed : %d %s",
			    errno, safe_strerror(errno));
	...
	if (zns->ge_netlink_cmd.sock >= 0) {
		...
		ret = setsockopt(zns->ge_netlink_cmd.sock, SOL_NETLINK,
				 NETLINK_EXT_ACK, &one, sizeof(one));
		if (ret < 0)
			zlog_err("CpFZYckRx7a2FImO ""Registration for extended generic netlink cmd ACK failed : %d %s",
				 errno, safe_strerror(errno));
	}
