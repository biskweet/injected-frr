...
static void zd_dpdk_rule_update(struct zebra_dplane_ctx *ctx)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DPDK_DETAIL)
		zlog_debug("IpyizY2iLR09ud1m ""Dplane %s", dplane_op2str(dplane_ctx_get_op(ctx)));
...
static int zd_dpdk_process(struct zebra_dplane_provider *prov)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DPDK_DETAIL)
		zlog_debug("3pIvcrxoRfGml0Ga ""processing %s", dplane_provider_get_name(prov));
...
static void zd_dpdk_port_init(void)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DPDK)
		zlog_debug("q8gQkEBukH4rUzrS ""dpdk port init");
	count = 0;
	RTE_ETH_FOREACH_DEV(port_id)
	{
		if (IS_ZEBRA_DEBUG_DPLANE_DPDK)
			zlog_debug("yyVUwW6ivE6ZK52Z ""dpdk port init %d", port_id);
		dport = &dpdk_ctx->dpdk_ports[count];
		count++;
		...
		if (rte_eth_dev_info_get(port_id, dev_info) < 0) {
			zlog_warn("YYtZl1V40CzvlnWN ""failed to get dev info for %u, %s", port_id,
				  rte_strerror(rte_errno));
			continue;
		}
		SET_FLAG(dport->flags, ZD_DPDK_PORT_FLAG_INITED);
		if (IS_ZEBRA_DEBUG_DPLANE_DPDK)
			zlog_debug("Oq6Eo7x274Ns8Y6u ""port %u, dev %s, ifI %d, sw_name %s, sw_domain %u, sw_port %u",
				   port_id,
				   dev_info->device
		...
		if (rte_flow_isolate(port_id, 1, &error)) {
			if (IS_ZEBRA_DEBUG_DPLANE_DPDK)
				zlog_debug("vwdGK8Jr7Dek2U3S "
					"Flow isolate on port %u failed %d", port_id, error.type);
		} else {
			if (IS_ZEBRA_DEBUG_DPLANE_DPDK)
				zlog_debug("NlUZW046EesdWg87 ""Flow isolate on port %u", port_id);
		}
		rc = rte_eth_dev_start(port_id);
		if (rc) {
			zlog_warn("DzPExHLJmlVe7krD ""DPDK port %d start error: %s", port_id,
				  rte_strerror(-rc));
			continue;
		}
		if (IS_ZEBRA_DEBUG_DPLANE_DPDK)
			zlog_debug("bxNimcyic886TBJa ""DPDK port %d started in promiscuous mode ", port_id);
	}

	if (!count) {
		if (IS_ZEBRA_DEBUG_DPLANE_DPDK)
			zlog_debug("AIcq3i6D5jSDGzP9 ""no probed ethernet devices");
	}
}
...
static int zd_dpdk_init(void)
{
	...
	if (rc < 0) {
		zlog_warn("l85sEPGkp8dpN9en ""EAL init failed %s", rte_strerror(rte_errno));
		return -1;
	}
...
{
	if (IS_ZEBRA_DEBUG_DPLANE_DPDK)
		zlog_debug("Ui6kzh62R3w6u0h8 ""%s start", dplane_provider_get_name(prov));
...
static int zd_dpdk_finish(struct zebra_dplane_provider *prov, bool early)
{
	...
	if (early) {
		if (IS_ZEBRA_DEBUG_DPLANE_DPDK)
			zlog_debug("jDt0X7xGnIslc4i5 ""%s early finish", dplane_provider_get_name(prov));

		return 0;
	}

	if (IS_ZEBRA_DEBUG_DPLANE_DPDK)
		zlog_debug("I61z4kq0imiXaG8Q ""%s finish", dplane_provider_get_name(prov));
	...
	if (rc < 0)
		zlog_warn("obsErW6l2azt3yc5 ""EAL cleanup failed %s", rte_strerror(rte_errno));
...
static int zd_dpdk_plugin_init(struct event_loop *tm)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DPDK)
		zlog_debug("POkH7ooDkqgtJAb4 ""%s register status %d", plugin_name, ret);
