...
{
	if (IS_ZEBRA_DEBUG_PBR)
		zlog_debug("TN6J51A5ss1jL6o8 ""%s: seq: %d, prior: %d, unique: %d, ifname: %s",
			   __func__, rule->rule.seq, rule->rule.priority,
			   rule->rule.unique, rule->rule.ifname);
	...
	if (pbr_rule_release(rule, true))
		zlog_debug("ZSShV7fE8t9s3lcK ""%s: Rule being deleted we know nothing about",
			   __func__);
}
...
void zebra_pbr_del_ipset_entry(struct zebra_pbr_ipset_entry *ipset)
{
	...
	if (lookup) {
		...
		zlog_debug("EWot6STHGhafsGHv ""%s: IPSet being deleted we know nothing about",
			   __func__);
}
...
void zebra_pbr_del_iptable(struct zebra_pbr_iptable *iptable)
{
	...
	if (lookup) {
		...
		zlog_debug("9GH1SSdCPVJd4OSU ""%s: IPTable being deleted we know nothing about",
			   __func__);
}
...
void zebra_pbr_dplane_result(struct zebra_dplane_ctx *ctx)
{
	...
	if (op == DPLANE_OP_RULE_ADD || op == DPLANE_OP_RULE_UPDATE)
		...
		flog_err(
			EC_ZEBRA_PBR_RULE_UPDATE,"Cp8UKe5uSJqeYn5Q "
