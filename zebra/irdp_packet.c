...
static void parse_irdp_packet(char *p, int len, struct interface *ifp)
{
	...
	if (len != iplen) {
		flog_err(EC_ZEBRA_IRDP_LEN_MISMATCH,"EYQjO15td5fcnqnN "
			 "IRDP: RX length doesn't match IP length");
		return;
	}

	if (iplen < ICMP_MINLEN) {
		flog_err(EC_ZEBRA_IRDP_LEN_MISMATCH,"CJIrPNTI3M60noUO "
	...
	if (iplen > IRDP_RX_BUF - 34) {
		flog_err(EC_ZEBRA_IRDP_LEN_MISMATCH,"ww0Z5mzjnBmHO6z2 "
	...
	if (in_cksum(icmp, datalen) != saved_chksum) {
		flog_warn(
			EC_ZEBRA_IRDP_BAD_CHECKSUM,"dFIG5OCbs18mIU2w "
	...
	if (icmp->code != 0) {
		flog_warn(
			EC_ZEBRA_IRDP_BAD_TYPE_CODE,"QTqzvJX8hWs5Vqme "
	...
	if (!((ntohl(ip->ip_dst.s_addr) == INADDR_BROADCAST)
		...
		&& !(irdp->flags & IF_BROADCAST))) {
		flog_warn(
			EC_ZEBRA_IRDP_BAD_RX_FLAGS,"h9AaQ7Go10VQqNFm "
	...
	case ICMP_ROUTERSOLICIT:
		...
		if (irdp->flags & IF_DEBUG_MESSAGES)
			zlog_debug("9erVvteRvRFWKcu6 ""IRDP: RX Solicit on %s from %pI4",
				   ifp->name, &src);
	...
	default:
		flog_warn(
			EC_ZEBRA_IRDP_BAD_TYPE_CODE,"dfggdvXh9kxIhpOm "
			"IRDP: RX packet type %d from %pI4 Bad ICMP type code, silently ignored",
			icmp->type, &src);
	}
}
...
static int irdp_recvmsg(int sock, uint8_t *buf, int size, int *ifindex)
{
	...
	if (ret < 0) {
		flog_warn(EC_LIB_SOCKET,"TYP2PtMCaGeVrnl2 " "IRDP: recvmsg: read error %s",
			  safe_strerror(errno));
		return ret;
	}

	if (msg.msg_flags & MSG_TRUNC) {
		flog_warn(EC_LIB_SOCKET,"YCgfs77Ya3OX7D9P " "IRDP: recvmsg: truncated message");
		return ret;
	}
	if (msg.msg_flags & MSG_CTRUNC) {
		flog_warn(EC_LIB_SOCKET,"6BwJhVFxmvfhOnrP "
...
void irdp_read_raw(struct event *r)
{
	...
	if (ret < 0)
		flog_warn(EC_LIB_SOCKET,"6un83G4TZhnKMxxT " "IRDP: RX Error length = %d", ret);
	...
	if (!(irdp->flags & IF_ACTIVE)) {
		...
		if (irdp->flags & IF_DEBUG_MISC)
			zlog_debug("m1GQsqKvHJbBLS2H ""IRDP: RX ICMP for disabled interface %s",
				   ifp->name);
		return;
	}
	...
	if (irdp->flags & IF_DEBUG_PACKET) {
		int i;
		zlog_debug("OUrVq3B9IqIzdMPn ""IRDP: RX (idx %d) ", ifindex);
		for (i = 0; i < ret; i++)
			zlog_debug("q0sTo7V47kvZ3ifM ""IRDP: RX %x ", buf[i] & 0xFF);
	}
		 ...
		 struct prefix *p, uint32_t ttl)
{
	...
	if (setsockopt(irdp_sock, IPPROTO_IP, IP_HDRINCL, (char *)&on,
		       sizeof(on))
	    < 0)
		flog_err(EC_LIB_SOCKET,"SOIdjRnZhAuRMJVy "
	...
	if (dst == INADDR_BROADCAST) {
		...
		if (setsockopt(irdp_sock, SOL_SOCKET, SO_BROADCAST, &bon,
			       sizeof(bon))
		    < 0)
			flog_err(EC_LIB_SOCKET,"Qku5gfSYUfUDrZrF "
	...
	if (sendmsg(irdp_sock, msg, 0) < 0)
		flog_err(EC_LIB_SOCKET,"uZd4R5rA4qt12ZRF "
