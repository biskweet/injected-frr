...
void zebra_mlag_send_register(void)
{
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("r5F7oxUFgX6tHK9a ""%s: Enqueued MLAG Register to MLAG Thread ",
			   __func__);
}
...
void zebra_mlag_send_deregister(void)
{
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("lbeZreq9eX9na8Ir ""%s: Enqueued MLAG De-Register to MLAG Thread ",
			   __func__);
}
...
void zebra_mlag_process_mlag_data(uint8_t *data, uint32_t len)
{
	...
	if (msg_type <= 0) {
		...
		stream_free(s);
		zlog_err("nwsDu6CvIeZ347aN ""%s: failed to process mlag data-%d, %u", __func__,
			 msg_type, len);
		return;
	}
...
static void zebra_mlag_client_msg_handler(struct event *event)
{
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("vnuAADWYer8njuYn "":%s: Processing MLAG write, %u messages in queue",
			   __func__, wr_count);
	...
	for (wr_count = 0; wr_count < max_count; wr_count++) {
		...
		if (!s) {
			zlog_debug("myLnoumPzjrel3li "":%s: Got a NULL Messages, some thing wrong",
				   __func__);
			break;
		}
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("wZ2JUSyHuV4asd0f "":%s: Posted  %d messages to MLAGD", __func__,
			   wr_count);
	/*
...
{
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("OzSuho2UMPystiSj "":%s: Scheduling MLAG write", __func__);
	/*
					     ...
					     zebra_message_types_t msg_type)
{
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("nOqhDGzgWHtWbUW1 ""%s: Publishing MLAG process state:%s to %s Client",
			   __func__,
...
{
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("XJAXVnMRt0zHkmZr ""Zebra MLAG write thread terminate called");

	if (zrouter.mlag_info.clients_interested_cnt) {
		if (IS_ZEBRA_DEBUG_MLAG)
			zlog_debug("Yjl7yINk9AfT7YZX "
...
void zebra_mlag_client_register(ZAPI_HANDLER_ARGS)
{
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("ewuOYdE8z9oVIEqO ""Received MLAG Registration from client-proto:%d",
			   client->proto);
	...
	client->mlag_reg_mask1 = reg_mask;
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("lfTXQnTBYXt2aFOo ""Registering for MLAG Updates with mask: 0x%x, ",
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("Ry8otoTElgaK2xBI ""Client Registered successfully for MLAG Updates");
...
{
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("ASwyJRvt1GZ9rQTo ""Received MLAG De-Registration from client-proto:%d",
			   client->proto);
	...
	if (zrouter.mlag_info.clients_interested_cnt == 0) {
		 ...
		 */
		if (IS_ZEBRA_DEBUG_MLAG)
			zlog_debug("QCVeY9A5qmT05Elv ""Last client for MLAG, close the channel ");
...
void zebra_mlag_forward_client_msg(ZAPI_HANDLER_ARGS)
{
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("lfJxE4oetXBy3tcT ""Received Client MLAG Data from client-proto:%d",
			   client->proto);
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("AUA7saFm7QRX7sza ""%s: Enqueued Client:%d data to MLAG Thread ",
				      ...
				      const char *secondary)
{
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("tEk0KjyXzAsXmUPz ""Test: Changing role from %s to %s",
...
int zebra_mlag_protobuf_encode_client_data(struct stream *s, uint32_t *msg_type)
{
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("VnNojNraxatymDzN ""%s: Entering..", __func__);
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("7oyoXUFeS6N8gw08 ""%s: Mlag ProtoBuf encoding of message:%s, len:%d",
			   __func__,
	...
	*msg_type = mlag_msg.msg_type;
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("mvsjjyIu1dYfKm6D ""%s: length of Mlag ProtoBuf encoded message:%s, %d",
			   __func__,
			   mlag_lib_msgid_to_str(mlag_msg.msg_type, buf,
						 sizeof(buf)),
			   len);
				       ...
				       uint32_t len)
{
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("DYxoTAivPgtKKtI7 ""%s: Mlag ProtoBuf decoding of message:%s", __func__,
