...
void zserv_log_message(const char *errmsg, struct stream *msg,
		       struct zmsghdr *hdr)
{
	zlog_debug("omqyX1B9SuVl5A33 ""Rx'd ZAPI message");
	if (errmsg)
		zlog_debug("J0nnPftyt6ZWH6at ""%s", errmsg);
	if (hdr) {
		zlog_debug("LMnxahBo19x1v1Ld "" Length: %d", hdr->length);
		zlog_debug("vHryySCzqVgfgMj5 ""Command: %s", zserv_command_string(hdr->command));
		zlog_debug("hFtJsrcp1hv5nE7J ""    VRF: %u", hdr->vrf_id);
	}
	stream_hexdump(msg);
}
...
{
	flog_warn(
		EC_ZEBRA_CLIENT_IO_ERROR,"KAYcgxQa63c9Jrx8 "
...
zwrite_fail:
	flog_warn(EC_ZEBRA_CLIENT_WRITE_FAILED,"A7KXncVaBZbpqXY2 "
...
static void zserv_read(struct event *thread)
{
	...
	while (p2p) {
		...
		if (already < ZEBRA_HEADER_SIZE) {
			...
			if ((nb == 0 || nb == -1)) {
				if (IS_ZEBRA_DEBUG_EVENT)
					zlog_debug("vyvY3jg3zn8U2KJ3 ""connection closed socket [%d]",
						   sock);
				goto zread_fail;
			}
		...
		if (IS_ZEBRA_DEBUG_PACKET)
			zlog_debug("oK1aEVq2UT3BCIaY ""zebra message[%s:%u:%u] comes from socket [%d]",
	...
	if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("sOtmD0nGEx0tUi1G ""Read %d packets from client: %s. Current ibuf fifo count: %zu. Conf P2p %d",
...
static void zserv_client_free(struct zserv *client)
{
	...
	if (DYNAMIC_CLIENT_GR_DISABLED(client) || zebra_router_in_shutdown()) {
		if (IS_ZEBRA_DEBUG_EVENT)
			zlog_debug("TtNTT7PlvDiku67z ""%s: Deleting client %s", __func__,
				   zebra_route_string(client->proto));
		...
		if (IS_ZEBRA_DEBUG_EVENT)
			zlog_debug("IGB02V0fM9LvzUrU ""%s: client %s restart enabled", __func__,
				   zebra_route_string(client->proto));
		if (zebra_gr_client_disconnect(client) < 0)
			zlog_err("exWlEym9fxHsAIH7 "
...
void zserv_close_client(struct zserv *client)
{
	...
	if (client->pthread) {
		...
		if (IS_ZEBRA_DEBUG_EVENT)
			zlog_debug("9PpqAMeLzVny6VcR ""Closing client '%s'",
				   zebra_route_string(client->proto));
...
static void zserv_accept(struct event *thread)
{
	...
	if (client_sock < 0) {
		flog_err_sys(EC_LIB_SOCKET,"LS6lkuAVkA41fwhM " "Can't accept zebra socket: %s",
			     safe_strerror(errno));
		return;
	}
...
void zserv_start(char *path)
{
	...
	if (zsock < 0) {
		flog_err_sys(EC_LIB_SOCKET,"X4ratmjQS8Zxnnnw " "Can't create zserv socket: %s",
			     safe_strerror(errno));
		return;
	}
	...
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"ehPpQj6dC0lYU1r0 " "Can't bind zserv socket on %s: %s",
	...
	ret = listen(zsock, 5);
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"HPn9cMCGoDstqNIP "
