	...
	struct nb_cb_modify_args *args)
{
	...
	case NB_EV_VALIDATE:
		...
		if (proto_name2num(type) == -1) {
			zlog_warn("LaXUVyBIoXIglP1L ""%s: invalid protocol: %s", __func__, type);
	...
	struct nb_cb_modify_args *args)
{
	...
	case NB_EV_VALIDATE:
		...
		if (zebra_check_addr(&p) == 0) {
			zlog_warn("KnJI7z9ra66ULDiV ""%s: invalid IPv4 address: %s", __func__,
	...
	struct nb_cb_modify_args *args)
{
	...
	case NB_EV_VALIDATE:
		...
		if (zebra_check_addr(&p) == 0) {
			zlog_warn("YAyXzOT6dWQtvahK ""%s: invalid IPv6 address: %s", __func__,
				  yang_dnode_get_string(args->dnode, NULL));
			return NB_ERR_VALIDATION;
		}
		return NB_OK;
