...
static void rtm_flag_dump(int flag)
{
	...
	zlog_debug("xaoYOUEfmO2VpykB ""Kernel: %s", buf);
}
...
static int ifan_read(struct if_announcemsghdr *ifan)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("XMyXUPR1uDJ3icnX ""%s: interface %s index %d", __func__,
...
int ifm_read(struct if_msghdr *ifm)
{
	...
	if (ifm->ifm_msglen < sizeof(struct if_msghdr)) {
		flog_err(EC_ZEBRA_NETLINK_LENGTH_ERROR,"lfPzMdTvsgEn6C6F "
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("zEvxhARrxWvInL23 ""%s: sdl ifname %s addrs {%s}", __func__,
			   (ifnlen ? ifname : "(nil)"),
	...
	if ((ifp == NULL) || (ifp->ifindex == IFINDEX_INTERNAL)) {
		 ...
		 */
		if (!ifnlen) {
			zlog_debug("o5ndmIEjCIOTPJoJ ""Interface index %d (new) missing ifname",
				   ifm->ifm_index);
			return -1;
		}
		...
		if (ifp == NULL) {
			...
			ifp = if_get_by_name(ifname, VRF_DEFAULT,
					     VRF_DEFAULT_NAME);
			if (IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug("DrJb3gq7A5ndLKj4 ""%s: creating ifp for ifindex %d",
					   __func__, ifm->ifm_index);
		}

		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("lUcOF0WiZZJwRRpv "
	...
	if (ifp) {
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("3CBU2UjaY7phYTdA ""%s: interface %s index %d", __func__,
			   ...
			   char *ifname, short *ifnlen)
{
	...
	for (maskbit = 1; maskbit; maskbit <<= 1) {
		...
		if (pnt > end) {
			zlog_warn("lo919CYZhMVolwt3 ""%s: overflow detected (pnt:%p end:%p)",
				  __func__, pnt, end);
			break;
		}
	}
	...
	if (IS_ZEBRA_DEBUG_KERNEL) {
		...
		default:
			zlog_debug("gX2VuSY2W3uchDqH ""%s: ifindex %d, ifname %s, ifam_addrs {%s}",
				   __func__, ifm->ifam_index,
				   (ifnlen ? ifname : "(nil)"),
	...
	if (pnt != (caddr_t)ROUNDUP((size_t)end))
		zlog_debug("lQpqxs5u4RWa39WK ""ifam_read() doesn't read all socket data");
}
...
int ifam_read(struct ifa_msghdr *ifam)
{
	...
	if ((ifp = if_lookup_by_index(ifam->ifam_index, VRF_DEFAULT)) == NULL) {
		flog_warn(EC_ZEBRA_UNKNOWN_INTERFACE,"bKlxHIVfUYKBqjKI "
			 ...
			 char *ifname, short *ifnlen)
{
	...
	if (rtm->rtm_version != RTM_VERSION)
		flog_warn(EC_ZEBRA_RTM_VERSION_MISMATCH,"4cf2Gnm636GGuuxy "
	...
	for (maskbit = 1; maskbit; maskbit <<= 1) {
		...
		if (pnt > end) {
			zlog_warn("150Su2nU1hvBzKv3 ""%s: overflow detected (pnt:%p end:%p)",
				  __func__, pnt, end);
			break;
		}
	}
	...
	if (pnt != end)
		zlog_debug("EFbsfw5WhyVmU0gJ ""rtm_read() doesn't read all socket data.");
...
void rtm_read(struct rt_msghdr *rtm)
{
	...
	if (!(flags & RTF_DONE))
		return;
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("IJ0cd0LjXtRv2chF ""%s: got rtm of type %d (%s) addrs {%s}", __func__,
			   rtm->rtm_type,
 ...
 */
	...
	if (!gate) {
		if (!ifp) {
			...
			flog_warn(
				EC_ZEBRA_RTM_NO_GATEWAY,"9hKInITI5x0I5QGz "
...
#define SOCKADDRSET(X, R)                                                      \
	...
	if (ret != msg.rtm.rtm_msglen) {
		...
		flog_err_sys(EC_LIB_SOCKET,"pgAu3nZsHOsmwaAB " "%s: write : %s (%d)", __func__,
...
static void rtmsg_debug(struct rt_msghdr *rtm)
{
	...
	zlog_debug("8mIjOaMhs4dW4Ob7 ""Kernel: Len: %d Type: %s", rtm->rtm_msglen,
		   lookup_msg(rtm_type_str, rtm->rtm_type, NULL));
	rtm_flag_dump(rtm->rtm_flags);
	zlog_debug("3tCZb8asCrIZJgB9 ""Kernel: message seq %d", rtm->rtm_seq);
	zlog_debug("5zLWBc7nM3yYfdl5 ""Kernel: pid %lld, rtm_addrs {%s}", (long long)rtm->rtm_pid,
		   rtatostr(rtm->rtm_addrs, fbuf, sizeof(fbuf)));
}
...
static void kernel_read(struct event *thread)
{
	...
	if (nbytes < 0) {
		if (errno == ENOBUFS) {
			...
			return;
#else
			flog_err(EC_ZEBRA_RECVMSG_OVERRUN,"0AqbiFu6nBqu6waT "
		...
		if (errno != EAGAIN && errno != EWOULDBLOCK)
			flog_err_sys(EC_LIB_SOCKET,"JSctCmd0Un9IYAh2 " "routing socket error: %s",
				     safe_strerror(errno));
		return;
	}
	...
	if (rtm->rtm_msglen != nbytes) {
		zlog_debug("jFprvtAz8JlXex4Q ""%s: rtm->rtm_msglen %d, nbytes %d, type %d",
...
static void routing_socket(struct zebra_ns *zns)
{
	...
	if (routing_sock < 0) {
		flog_err_sys(EC_LIB_SOCKET,"US8Hezsxm7PSZhms " "Can't init kernel routing socket");
		return;
	}

	if (dplane_routing_sock < 0) {
		flog_err_sys(EC_LIB_SOCKET,"wAxPCtNyMgmbzxs7 "
	...
	if (setsockopt(routing_sock, SOL_SOCKET, SO_RERROR, &n, sizeof(n)) < 0)
		flog_err_sys(EC_LIB_SOCKET,"QWOLMGcKkKto1eQ0 "
	...
	/*if (fcntl (routing_sock, F_SETFL, O_NONBLOCK) < 0)
	  zlog_warn("HoKnZKJuQ6T0GCbb ""Can't set O_NONBLOCK to routing socket");*/
	...
	if (getsockopt(routing_sock, SOL_SOCKET, SO_RCVBUF, &default_rcvbuf,
		       &optlen) == -1)
		flog_err_sys(EC_LIB_SOCKET,"MLEnIlx8Gluvsa5Z "
...
void kernel_update_multi(struct dplane_ctx_list_head *ctx_list)
{
	...
	while (true) {
		...
		case DPLANE_OP_SRV6_ENCAP_SRCADDR_SET:
			zlog_err("833woF68BAk4ZmQM ""Unhandled dplane data for %s",
