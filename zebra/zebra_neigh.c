...
zebra_neigh_new(ifindex_t ifindex, struct ipaddr *ip, struct ethaddr *mac)
{
	...
	if (IS_ZEBRA_DEBUG_NEIGH)
		zlog_debug("PqL328kn6v5ET1OV ""zebra neigh new if %d %pIA %pEA", n->ifindex,
			   &n->ip, &n->mac);
...
static void zebra_neigh_free(struct zebra_neigh_ent *n)
{
	...
	if (IS_ZEBRA_DEBUG_NEIGH)
		zlog_debug("Y6i5td19QfBRlG1c ""zebra neigh free if %d %pIA %pEA", n->ifindex,
			   &n->ip, &n->mac);
...
void zebra_neigh_del(struct interface *ifp, struct ipaddr *ip)
{
	...
	if (IS_ZEBRA_DEBUG_NEIGH)
		zlog_debug("F997f5plqbnh1av0 ""zebra neigh del if %s/%d %pIA", ifp->name,
			   ifp->ifindex, ip);
...
void zebra_neigh_del_all(struct interface *ifp)
{
	...
	if (IS_ZEBRA_DEBUG_NEIGH)
		zlog_debug("i8dmhN9ovacM6yXP ""zebra neigh delete all for interface %s/%d",
		     ...
		     struct ethaddr *mac)
{
	...
	if (IS_ZEBRA_DEBUG_NEIGH)
		zlog_debug("fGfqdpaVZHlHs3cR ""zebra neigh add if %s/%d %pIA %pEA", ifp->name,
			   ifp->ifindex, ip, mac);
...
void zebra_neigh_deref(struct zebra_pbr_rule *rule)
{
	...
	if (IS_ZEBRA_DEBUG_NEIGH)
		zlog_debug("Z9yGcPV5ife5f6mH ""zebra neigh deref if %d %pIA by pbr rule %u",
		     ...
		     struct zebra_pbr_rule *rule)
{
	...
	if (IS_ZEBRA_DEBUG_NEIGH)
		zlog_debug("MgjA2D1ZQMZTokwA ""zebra neigh ref if %d %pIA by pbr rule %u", ifindex,
			   ip, rule->rule.seq);
