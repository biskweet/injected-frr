...
static void zebra_evpn_neigh_hold_exp_cb(struct event *t)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
		zlog_debug("r1dPna4eimfggUUD ""sync-neigh vni %u ip %pIA mac %pEA 0x%x hold expired",
			   n->zevpn->vni, &n->ip, &n->emac, n->flags);
...
static inline void zebra_evpn_neigh_start_hold_timer(struct zebra_neigh *n)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
		zlog_debug("i9jAifVPd5sqtgCZ ""sync-neigh vni %u ip %pIA mac %pEA 0x%x hold start",
			   n->zevpn->vni, &n->ip, &n->emac, n->flags);
...
void zebra_evpn_sync_neigh_del(struct zebra_neigh *n)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
		zlog_debug("kZqlvO2sHrrpOWUT ""sync-neigh del vni %u ip %pIA mac %pEA f 0x%x",
			   n->zevpn->vni, &n->ip, &n->emac, n->flags);
						  ...
						  bool es_change)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("ykHDPMFuDzt4oA24 ""Processing neighbors on local MAC %pEA %s, VNI %u",
					       ...
					       struct zebra_mac *zmac)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("z44lOeQrwBI1EV8n ""Processing neighbors on local MAC %pEA DEL, VNI %u",
						...
						struct zebra_mac *zmac)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("alTCq5kX97qiRAv8 ""Processing neighbors on remote MAC %pEA ADD, VNI %u",
			   &zmac->macaddr, zevpn->vni);
	...
	bool inform_dataplane, bool inform_bgp, const char *sfx)
{
	...
	zlog_debug("m7FLMDpBr5MFqOZd ""%s neigh vni %u ip %pIA mac %pEA f 0x%x%s%s%s%s%s%s %s", pfx,
		   n->zevpn->vni, &n->ip, &n->emac, n->flags,
		   is_router ? " router" : "",
	...
	bool do_dad, bool *is_dup_detect, bool is_local)
{
	...
	if (nbr->dad_count >= zvrf->dad_max_moves) {
		flog_warn(
			EC_ZEBRA_DUP_IP_DETECTED,"Z3kg8C3QOxbfmml8 "
				  ...
				  bool local_inactive, bool dp_static)
{
	...
	if (!zmac) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("oUyMtqSvETIEHfrF ""AUTO MAC %pEA created for neigh %pIA on VNI %u",
				   macaddr, ip, zevpn->vni);

		zmac = zebra_evpn_mac_add_auto(zevpn, macaddr);
		if (!zmac) {
			zlog_debug("94cxr2lur8SfjG7k ""Failed to add MAC %pEA VNI %u", macaddr,
				   zevpn->vni);
			return -1;
		}
	} else {
	...
	if (!zvrf) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("To1JDBmjGEZ1iWOY ""        Unable to find vrf for: %d",
	...
	if (zebra_evpn_ip_inherit_dad_from_mac(zvrf, is_old_mac_dup, zmac, n)) {
		flog_warn(
			EC_ZEBRA_DUP_IP_INHERIT_DETECTED,"DYooIKJwfXbxUHrC "
	...
	if (!neigh_on_hold) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("TAGnG7WXpWTL3518 ""        Neighbor on hold not sending");
	}
	return 0;
}
...
void zebra_evpn_clear_dup_neigh_hash(struct hash_bucket *bucket, void *ctxt)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN) {
		...
		zlog_debug("tAYRTono2dn1Kq9B ""%s: clear neigh %s dup state, flags 0x%x seq %u",
				       ...
				       uint32_t seq)
{
	...
	if (update_neigh) {
		...
		if (zebra_evpn_ip_inherit_dad_from_mac(zvrf, is_old_mac_dup, mac, n)) {
			flog_warn(
				EC_ZEBRA_DUP_IP_INHERIT_DETECTED,"36ENS8AiaDkym9ZB "
...
int zebra_evpn_neigh_del_ip(struct zebra_evpn *zevpn, const struct ipaddr *ip)
{
	...
	if (zebra_evpn_neigh_is_static(n)) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
			zlog_debug("nSpcZOu5DEHxOj2i ""re-add sync neigh vni %u ip %pIA mac %pEA 0x%x",
				   n->zevpn->vni, &n->ip, &n->emac,
				   n->flags);
	...
	zvrf = zevpn->vxlan_if->vrf->info;
	if (!zvrf) {
		zlog_debug("onzGoZMqTK65kVoX ""%s: VNI %u vrf lookup failed.", __func__,
			   zevpn->vni);
		return -1;
	}
