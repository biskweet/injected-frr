...
void zebra_opaque_start(void)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("O3xI6d2vGGyCo7Kz ""%s module starting", LOG_NAME);
...
{
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("ZsOzPbVC7WETxzBw ""%s module stop", LOG_NAME);
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("a04mAHuSTxvbd0Ql ""%s module stop complete", LOG_NAME);
}
...
void zebra_opaque_finish(void)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("h3mYRFubYM8riN5v ""%s module shutdown", LOG_NAME);
...
uint32_t zebra_opaque_enqueue_batch(struct stream_fifo *batch)
{
	...
	if (counter > 0) {
		if (IS_ZEBRA_DEBUG_RECV && IS_ZEBRA_DEBUG_DETAIL)
			zlog_debug("Z6TlRwPlirCiB3D4 ""%s: received %u messages",
				   __func__, counter);
		event_add_event(zo_info.master, process_messages, NULL, 0,
				&zo_info.t_msgs);
	}
...
static void process_messages(struct event *event)
{
	...
	if (IS_ZEBRA_DEBUG_RECV && IS_ZEBRA_DEBUG_DETAIL)
		zlog_debug("QxuRGXWbd3arI22u ""%s: processing %u messages", __func__, i);
...
static int dispatch_opq_messages(struct stream_fifo *msg_fifo)
{
	...
	while ((msg = stream_fifo_pop(msg_fifo)) != NULL) {
		...
		if (reg == NULL) {
			if (IS_ZEBRA_DEBUG_RECV && IS_ZEBRA_DEBUG_DETAIL)
				zlog_debug("CTNzrnZQCGk9Yb7Z ""%s: no registrations for opaque type %u, flags %#x",
		...
		for (client = reg->clients; client; client = client->next) {
			...
			if (CHECK_FLAG(info.flags, ZAPI_OPAQUE_FLAG_UNICAST)) {
				...
				if (IS_ZEBRA_DEBUG_RECV &&
					...
					zlog_debug("9HIMsPk5407YENLT ""%s: found matching unicast client %s",
						   __func__,
						   opq_client2str(buf,
								  sizeof(buf),
								  client));
			...
			if (IS_ZEBRA_DEBUG_SEND && IS_ZEBRA_DEBUG_DETAIL)
				zlog_debug("UkIQ8kjxyMu8VWch ""%s: sending %s to client %s",
			...
			if (opq_send_message(client->proto, client->instance,
				...
				if (IS_ZEBRA_DEBUG_RECV &&
				    IS_ZEBRA_DEBUG_DETAIL)
					zlog_debug("KWUVEMMEHxJzZLe0 ""%s: type %u: no zclient for %s",
						   __func__, info.type,
						   opq_client2str(buf,
								  sizeof(buf),
								  client));
				   ...
				   struct stream *msg)
{
	...
	if (reg) {
		...
		if (client) {
			...
			if (IS_ZEBRA_DEBUG_RECV)
				zlog_debug("VRhBYfQgH1ucMnBI ""%s: duplicate opq reg for client %s",
					   __func__,
					   opq_client2str(buf, sizeof(buf),
							  client));
			goto done;
		...
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("q2vXt7CGju3uelgw ""%s: client %s registers for %u",
				   __func__,
		...
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("vm0fQMy1BDKTLd5l ""%s: client %s registers for new reg %u",
				   __func__,
				   opq_client2str(buf, sizeof(buf), client),
				   info.type);
				     ...
				     struct stream *msg)
{
	...
	if (reg == NULL) {
		...
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("5wEOfcqlXXNn3a3n ""%s: unknown client %s/%u/%u unregisters for unknown type %u",
				   __func__,
	...
	if (client == NULL) {
		...
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("38EiI548n4eVUorw ""%s: unknown client %s/%u/%u unregisters for %u",
	...
	if (IS_ZEBRA_DEBUG_RECV)
		zlog_debug("f1NnRNcFraeMU636 ""%s: client %s unregisters for %u",
...
static int handle_opq_notif_req(const struct zmsghdr *hdr, struct stream *msg)
{
	...
	if (reg) {
		...
		if (client) {
			/* Oops - duplicate ? */
			if (IS_ZEBRA_DEBUG_RECV)
				zlog_debug("xGaO997hL0oFWtRk ""%s: duplicate opq notif reg client %s",
		...
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("AyPmAyxIZABcixca ""%s: client %s registers for notif %u",
				   __func__,
				   opq_client2str(buf, sizeof(buf), client),
				   info.msg_type);
		...
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("KgZLLqaoKl2nJSiv ""%s: client %s registers for new notif %u",
				   __func__,
				   opq_client2str(buf, sizeof(buf), client),
				   info.msg_type);
...
static int handle_opq_notif_unreg(const struct zapi_opaque_notif_info *info)
{
	...
	if (reg == NULL) {
		/* Weird: unregister for unknown message? */
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("M9JGHBvOaybVPmSA ""%s: unknown client %s/%u/%u unregisters notif for unknown type %u",
				   __func__, zebra_route_string(info->proto),
	...
	if (client == NULL) {
		/* Oops - unregister for unknown client? */
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("T0G2y1CxU3vnrcbC ""%s: unknown client %s/%u/%u unregisters notif for %u",
				   __func__, zebra_route_string(info->proto),
				   info->instance, info->session_id,
				   info->msg_type);
		goto done;
	...
	if (IS_ZEBRA_DEBUG_RECV)
		zlog_debug("IJwIsb8dpmkuCR6Z ""%s: client %s unregisters notif for %u", __func__,
			   opq_client2str(buf, sizeof(buf), client),
			   info->msg_type);
