...
void zebra_tc_qdisc_install(struct zebra_tc_qdisc *qdisc)
{
	if (IS_ZEBRA_DEBUG_TC)
		zlog_debug("TvXMqh6Nw1rZqDUw ""%s: install tc qdisc ifindex %d kind %s", __func__,
			   qdisc->qdisc.ifindex,
...
{
	if (IS_ZEBRA_DEBUG_TC)
		zlog_debug("dFyemhiHftZo1duC ""%s: uninstall tc qdisc ifindex %d kind %s",
	...
	if (tc_qdisc_release(qdisc, true))
		zlog_debug("uvgPNLrcm9UdhxP6 ""%s: tc qdisc being deleted we know nothing about",
			   __func__);
}
...
void zebra_tc_class_delete(struct zebra_tc_class *class)
{
	...
	if (tc_class_release(class, true))
		zlog_debug("d5nCd4jvMUUN1S1e ""%s: tc class being deleted we know nothing about",
			   __func__);
}
...
void zebra_tc_filter_delete(struct zebra_tc_filter *filter)
{
	...
	if (tc_filter_release(filter, true))
		zlog_debug("nO2ONSXnCwAmtNQb ""%s: tc filter being deleted we know nothing about",
			   __func__);
}
