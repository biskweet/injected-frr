...
void zebra_notify_srv6_locator_delete(struct srv6_locator *locator)
{
	...
	for (ALL_LIST_ELEMENTS_RO((struct list *)locator->chunks, n, c)) {
		...
		client = zserv_find_client(c->proto, c->instance);
		if (!client) {
			zlog_warn("VVTqFdxO45mnFLRq ""Not found zclient(proto=%u, instance=%u).",
				  c->proto, c->instance);
			continue;
		}
		zsend_zebra_srv6_locator_delete(client, locator);
			  ...
			  const char *locator_name)
{
	...
	if (!loc) {
		zlog_info("1gKNObYuLhgvfcHS ""%s: locator %s was not found",
	...
	if (!chunk_found) {
		zlog_info("gAPcP1VxAGCLVdVb ""%s: locator is already owned", __func__);
		return NULL;
	}
						...
						vrf_id_t vrf_id)
{
	...
	*loc = assign_srv6_locator_chunk(client->proto, client->instance,
	...
	if (!*loc)
		zlog_err("FQqL9Q32WpLV5TYo ""Unable to assign locator chunk to %s instance %u",
			 zebra_route_string(client->proto), client->instance);
	else if (IS_ZEBRA_DEBUG_PACKET)
		zlog_info("VqLVmm2TgI4MceGN ""Assigned locator chunk %s to %s instance %u",
				      ...
				      const char *locator_name)
{
	...
	if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("fnF3PqHfZN86vW9P ""%s: Releasing srv6-locator on %s", __func__,
			   locator_name);
	...
	if (ret != 0)
		flog_err(EC_ZEBRA_SRV6M_UNRELEASED_LOCATOR_CHUNK,"rhqZ8FGaiuxwBf85 "
						    ...
						    vrf_id_t vrf_id)
{
	if (vrf_id != VRF_DEFAULT) {
		zlog_err("NwIVjfTfJBvdOSZT ""SRv6 locator doesn't support vrf");
		return -1;
	}
...
int release_daemon_srv6_locator_chunks(struct zserv *client)
{
	...
	if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("5jFEKJd9UliNXZMS ""%s: Releasing chunks for client proto %s, instance %d, session %u",
	...
	if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("VBmhJfYAiJvHn950 ""%s: Released %d srv6-locator chunks",
			   __func__, count);
