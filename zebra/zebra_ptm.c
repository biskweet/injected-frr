...
void zebra_ptm_init(void)
{
	...
	if (!ptm_cb.out_data) {
		zlog_debug("Pd717xnErEEgmzSw ""%s: Allocation of send data failed", __func__);
		return;
	}

	ptm_cb.in_data = calloc(1, ZEBRA_PTM_MAX_SOCKBUF);
	if (!ptm_cb.in_data) {
		zlog_debug("8kCiCDSbKB4HVc4l ""%s: Allocation of recv data failed", __func__);
...
static void zebra_ptm_flush_messages(struct event *thread)
{
	...
	case BUFFER_ERROR:
		flog_err_sys(EC_LIB_SOCKET,"1vKG2UL5aRk7zKkp " "%s ptm socket error: %s", __func__,
			     safe_strerror(errno));
		close(ptm_cb.ptm_sock);
		ptm_cb.ptm_sock = -1;
...
static int zebra_ptm_send_message(char *data, int size)
{
	...
	case BUFFER_ERROR:
		flog_err_sys(EC_LIB_SOCKET,"GQ4YZtnqeuMYrnhB " "%s ptm socket error: %s", __func__,
			     safe_strerror(errno));
		close(ptm_cb.ptm_sock);
		ptm_cb.ptm_sock = -1;
		zebra_ptm_reset_status(0);
	...
	if (!old_ptm_enable && ptm_cb.ptm_enable) {
		if (!if_is_operative(ifp) && send_linkdown) {
			if (IS_ZEBRA_DEBUG_EVENT)
				zlog_debug("NUXwHjUaKnFMXDqg ""%s: Bringing down interface %s",
					   __func__, ifp->name);
			if_down(ifp);
		}
	}
...
void zebra_if_ptm_disable(struct interface *ifp)
{
	...
	if ((ifp->ifindex != IFINDEX_INTERNAL) && (ifp->ptm_enable)) {
		...
		if (if_is_no_ptm_operative(ifp) && send_linkup) {
			if (IS_ZEBRA_DEBUG_EVENT)
				zlog_debug("7aEpOeR09cUx2vyq ""%s: Bringing up interface %s",
					   __func__, ifp->name);
			if_up(ifp, true);
		}
	}
...
static int zebra_ptm_socket_init(void)
{
	...
	if (set_nonblocking(sock) < 0) {
		if (IS_ZEBRA_DEBUG_EVENT)
			zlog_debug("O9NqiAZMqAjC6AD2 ""%s: Unable to set socket non blocking[%s]",
	...
	if (ret < 0) {
		if (IS_ZEBRA_DEBUG_EVENT)
			zlog_debug("aCs5xBKPN5ulUQpo ""%s: Unable to connect to socket %s [%s]",
				    ...
				    struct interface *ifp)
{
	...
	if (dest_str[0] == '\0') {
		zlog_debug("E4YbY7YwtVZ8m6T2 ""%s: Key %s not found in PTM msg", __func__,
	...
	if (src_str[0] == '\0') {
		zlog_debug("YcnYGoc8WgNndm27 ""%s: Key %s not found in PTM msg", __func__,
	...
	if (vrf_str[0] == '\0') {
		zlog_debug("sk4mT7WggiwC9XXZ ""%s: Key %s not found in PTM msg", __func__,
			   ZEBRA_PTM_BFDVRF_STR);
		return -1;
	}

	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("94of45zFpGwZnodh "
	...
	if (str2prefix(dest_str, &dest_prefix) == 0) {
		flog_err(EC_ZEBRA_PREFIX_PARSE_ERROR,"uruBrZfGn5GaVFzZ "
	...
	if (strcmp(ZEBRA_PTM_INVALID_SRC_IP, src_str)) {
		if (str2prefix(src_str, &src_prefix) == 0) {
			flog_err(EC_ZEBRA_PREFIX_PARSE_ERROR,"mhOTQyOPPg5ScBoz "
				    ...
				    struct interface *ifp, char *cbl_str)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("wd5p8vV46HElwVqp ""%s: Recv Port [%s] cbl status [%s]", __func__,
			   ifp->name, cbl_str);
...
static int zebra_ptm_handle_msg_cb(void *arg, void *in_ctxt)
{
	...
	if (port_str[0] == '\0') {
		zlog_debug("kduXSq0NnOgmXYLX ""%s: Key %s not found in PTM msg", __func__,
			   ZEBRA_PTM_PORT_STR);
		return -1;
	}
	...
	if (strcmp(ZEBRA_PTM_INVALID_PORT_NAME, port_str)) {
		...
		if (!ifp) {
			flog_warn(EC_ZEBRA_UNKNOWN_INTERFACE,"8cYE0GqrkbjchMJy "
				  "%s: %s not found in interface list",
				  __func__, port_str);
			return -1;
		}
		if (count > 1) {
			flog_warn(EC_ZEBRA_UNKNOWN_INTERFACE,"mWvWSJUGTE6cd16B "
...
void zebra_ptm_sock_read(struct event *thread)
{
	...
	if (((rc == 0) && !errno)
		...
		flog_err_sys(EC_LIB_SOCKET,"5uom9evbVjU8S5pw "
...
void zebra_ptm_bfd_dst_register(ZAPI_HANDLER_ARGS)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("0aTdVTfu4RAqg7hH ""bfd_dst_register msg from client %s: length=%d",
	...
	if (IS_ZEBRA_DEBUG_SEND)
		zlog_debug("5VIHPACKNqfRBvHz ""%s: Sent message (%d) %s", __func__, data_len,
			   ptm_cb.out_data);
...
void zebra_ptm_bfd_dst_deregister(ZAPI_HANDLER_ARGS)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("csQ3pzsU5sUi55IB ""bfd_dst_deregister msg from client %s: length=%d",
			   zebra_route_string(client->proto), hdr->length);
	...
	ptm_lib_complete_msg(ptm_hdl, out_ctxt, ptm_cb.out_data, &data_len);
	if (IS_ZEBRA_DEBUG_SEND)
		zlog_debug("8Al6I9LAx0XQcZhf ""%s: Sent message (%d) %s", __func__, data_len,
			   ptm_cb.out_data);
...
void zebra_ptm_bfd_client_register(ZAPI_HANDLER_ARGS)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("ejPA355xYNflZ295 ""bfd_client_register msg from client %s: length=%d",
			   zebra_route_string(client->proto), hdr->length);
	...
	if (IS_ZEBRA_DEBUG_SEND)
		zlog_debug("HnSmnaJ5g1WrP0Cx ""%s: Sent message (%d) %s", __func__, data_len,
			   ptm_cb.out_data);
	zebra_ptm_send_message(ptm_cb.out_data, data_len);
...
int zebra_ptm_bfd_client_deregister(struct zserv *client)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("wE8dvvWe2edYw05W ""bfd_client_deregister msg for client %s",
	...
	if (IS_ZEBRA_DEBUG_SEND)
		zlog_debug("IXzWYuhUBbCK9yIi ""%s: Sent message (%d) %s", __func__, data_len,
			   ptm_cb.out_data);
...
static struct ptm_process *pp_new(pid_t pid, struct zserv *zs)
{
	...
	TAILQ_FOREACH(pp, &ppqueue, pp_entry) {
		if (pp->pp_pid == pid && pp->pp_zs != zs)
			zlog_err("YoTSdlJLzog4PSjd ""%s:%d pid and client pointer doesn't match",
				 __FILE__, __LINE__);
	}
#endif /* PTM_DEBUG */
...
static int _zebra_ptm_bfd_client_deregister(struct zserv *zs)
{
	...
	pp = pp_lookup_byzs(zs);
	if (pp == NULL) {
		zlog_err("lDGDBlr5O889dYDv ""%s:%d failed to find process pid registration",
			 __FILE__, __LINE__);
		return -1;
	}
	...
	if (msg == NULL) {
		zlog_debug("H5ICgi97uTyH3UeS ""%s: not enough memory", __func__);
		return 0;
	}
			       ...
			       struct stream *msg, uint32_t command)
{
	...
	if (STREAM_READABLE(msg) > STREAM_WRITEABLE(msgc)) {
		zlog_warn("EwqEk78iXDkjNUCj ""Cannot fit extended BFD header plus original message contents into ZAPI packet; dropping message");
		goto stream_failure;
	}
...
stream_failure:
	if (msgc)
		stream_free(msgc);
	zlog_err("tiU0rpRY24v5vpHq ""%s:%d failed to registrate client pid", __FILE__, __LINE__);
}

void zebra_ptm_bfd_dst_register(ZAPI_HANDLER_ARGS)
{
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("ol041fpjml11FqlU ""bfd_dst_register msg from client %s: length=%d",
			   zebra_route_string(client->proto), hdr->length);
...
{
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("WARUfW0IYEf8mpE9 ""bfd_dst_deregister msg from client %s: length=%d",
			   zebra_route_string(client->proto), hdr->length);
...
{
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("qElck5FhwhzCcllT ""bfd_client_register msg from client %s: length=%d",
			   zebra_route_string(client->proto), hdr->length);
...
void zebra_ptm_bfd_dst_replay(ZAPI_HANDLER_ARGS)
{
	 ...
	 */
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("fxLTD72vNNPv73e0 ""bfd_dst_update msg from client %s: length=%d",
			   zebra_route_string(client->proto), hdr->length);
	...
	if (stream_getl2(msg, &cmd) == false) {
		zlog_err("1j6aU60Led2233kt ""%s: expected at least 4 bytes (command)", __func__);
		return;
	}
	...
	msgc = stream_new(ZEBRA_MAX_PACKET_SIZ);
	if (msgc == NULL) {
		zlog_debug("8JjsmimmLzCm3Do7 ""%s: not enough memory", __func__);
		return;
	}
