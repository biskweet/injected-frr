...
#define MIBSIZ 6
	...
	if (sysctl(mib, MIBSIZ, NULL, &bufsiz, NULL, 0) < 0) {
		flog_warn(EC_ZEBRA_SYSCTL_FAILED,"1DEPTShuFRB63COP " "sysctl() error by %s",
			  safe_strerror(errno));
		return;
	}
	...
	if (sysctl(mib, MIBSIZ, buf, &bufsiz, NULL, 0) < 0) {
		flog_warn(EC_ZEBRA_SYSCTL_FAILED,"aHJuCJ2gZXXbjbSx " "sysctl error by %s",
			  safe_strerror(errno));
		XFREE(MTYPE_TMP, ref);
		return;
	}
...
#define MIBSIZ 6
	...
	if (zns->ns_id != NS_DEFAULT) {
		zlog_debug("yyPolGLyFHhs6cS8 ""%s: ignore NS %u", __func__, zns->ns_id);
		return;
	}

	/* Query buffer size. */
	if (sysctl(mib, MIBSIZ, NULL, &bufsiz, NULL, 0) < 0) {
		flog_err_sys(EC_ZEBRA_IFLIST_FAILED,"s8oLRx2qk5dmLQdy "
	...
	/* Fetch interface information into allocated buffer. */
	if (sysctl(mib, MIBSIZ, buf, &bufsiz, NULL, 0) < 0) {
		flog_err_sys(EC_ZEBRA_IFLIST_FAILED,"Ce79Ew223J4xaRyF "
			     "Could not enumerate interfaces: %s",
			     safe_strerror(errno));
		return;
	}
	...
	for (end = buf + bufsiz; buf < end; buf += ifm->ifm_msglen) {
		...
		case RTM_NEWADDR:
			...
			break;
		default:
			zlog_info("Rax7WQfDVRgzaaee ""%s: unexpected message type", __func__);
			XFREE(MTYPE_TMP, ref);
			return;
			break;
		}
