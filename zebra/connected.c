...
void connected_up(struct interface *ifp, struct connected *ifc)
{
	...
	if (!zvrf) {
		flog_err(
			EC_ZEBRA_VRF_NOT_FOUND,"LMtXwgapUyH62cUM "
	...
	case AFI_L2VPN:
	case AFI_MAX:
		flog_warn(EC_ZEBRA_CONNECTED_AFI_UNKNOWN,"k33c2DOCCCsRZxWC "
...
void connected_down(struct interface *ifp, struct connected *ifc)
{
	...
	if (!zvrf) {
		flog_err(
			EC_ZEBRA_VRF_NOT_FOUND,"KLCd5eBDlWSRWx2H "
	...
	if (CHECK_FLAG(ifc->conf, ZEBRA_IFC_DOWN)) {
		if (IS_ZEBRA_DEBUG_RIB)
			zlog_debug("TmZ6gr7w5ORDOIkN ""%s: ifc %p, %pFX already DOWN",
	...
	case AFI_L2VPN:
	case AFI_MAX:
		zlog_warn("jR1dBq2Vb7OksrPh ""Unknown AFI: %s", afi2str(afi));
		break;
	}
