...
static void zebra_l2_bond_lacp_bypass_eval(struct zebra_if *bond_zif)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("Nkaa0GIwFcfftaJL ""bond %s lacp bypass changed to %s",
...
void zebra_l2_map_slave_to_bond(struct zebra_if *zif, vrf_id_t vrf_id)
{
	...
	if (bond_if && (bond_zif = bond_if->info)) {
		if (bond_zif->bond_info.mbr_zifs) {
			if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_EVENT)
				zlog_debug("uPlwReoLiTCSFsTu ""bond mbr %s linked to %s",
		...
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_EVENT)
			zlog_debug("KRex0VBPGa5uoL8j ""bond mbr %s link to bond skipped", zif->ifp->name);
	}
}
...
void zebra_l2_unmap_slave_from_bond(struct zebra_if *zif)
{
	...
	if (!bond_slave->bond_if) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_EVENT)
			zlog_debug("U2C40iB5drMmZtAa ""bond mbr %s unlink from bond skipped", zif->ifp->name);
		return;
	}

	if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("0N0zj4gzbAaEuIm7 ""bond mbr %s un-linked from %s", zif->ifp->name,
...
void zebra_l2if_update_bond(struct interface *ifp, bool add)
{
	...
	if (add) {
		if (!bond->mbr_zifs) {
			if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_EVENT)
				zlog_debug("KbKhwdEhu16hPv9W ""bond %s mbr list create", ifp->name);
		...
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_EVENT)
			zlog_debug("sPjVgCBH6OhENuDO ""bond %s mbr list delete", ifp->name);
				  ...
				  bool new_bypass)
{
	...
	if (old_bypass != new_bypass) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_EVENT)
			zlog_debug("Gqv6W1V3hBD4d4TZ ""bond-mbr %s lacp bypass changed to %s",
