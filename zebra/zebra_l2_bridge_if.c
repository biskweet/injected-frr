					 ...
					 void *ctxt)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("CLtSMjKWtCNmnWlh ""access vlan %d bridge %s cleanup", bvlan->vid,
			   zif->ifp->name);
...
int zebra_l2_bridge_if_cleanup(struct interface *ifp)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("uRFbGUMmLXTONTKu ""bridge %s cleanup", ifp->name);
...
{
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("Zi1nQRoxLwkEp0w0 ""bridge %s delete", ifp->name);
