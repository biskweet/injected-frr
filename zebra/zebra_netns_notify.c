...
static struct vrf *vrf_handler_create(struct vty *vty, const char *vrfname)
{
	if (strlen(vrfname) > VRF_NAMSIZ) {
		flog_warn(EC_LIB_VRF_LENGTH,"doVTBExP9RsqP4CG "
...
static void zebra_ns_notify_create_context_from_entry_name(const char *name)
{
	...
	if (!vrf) {
		flog_warn(EC_ZEBRA_NS_VRF_CREATION_FAILED,"YMNhoE4eOeGdxtES "
	...
	if (ret != CMD_SUCCESS) {
		flog_warn(EC_ZEBRA_NS_VRF_CREATION_FAILED,"lR7Twamy4wQ070zk "
			  "NS notify : failed to create NS %s", netnspath);
		ns_map_nsid_with_external(ns_id, false);
		vrf_delete(vrf);
		return;
	}
	zlog_info("G4Km9mvtpoVCjs27 ""NS notify : created VRF %s NS %s", name, netnspath);
}
...
static int zebra_ns_delete(char *name)
{
	...
	if (!vrf) {
		flog_warn(EC_ZEBRA_NS_DELETION_FAILED_NO_VRF,"1V70zUKydkIPu0a9 "
	...
	zlog_info("3B88SD66GP8ovC23 ""NS notify : deleted VRF %s", name);
	return 0;
}
...
static void zebra_ns_ready_read(struct event *t)
{
	...
	if (strmatch(VRF_DEFAULT_NAME, netnspath_basename)) {
		zlog_warn("nuUmkQfNbgwjFayo ""NS notify : NS %s is already default VRF.Cancel VRF Creation", netnspath_basename);
		zebra_ns_continue_read(zns_info, 1);
		return;
	}
	if (zebra_ns_notify_is_default_netns(netnspath_basename)) {
		zlog_warn("1Z2Q1xDGSH3VC297 "
...
static void zebra_ns_notify_read(struct event *t)
{
	...
	if (len < 0) {
		flog_err_sys(EC_ZEBRA_NS_NOTIFY_READ,"qXrBslatnVU7yGK3 "
	...
	for (event = (struct inotify_event *)buf; (char *)event < &buf[len];
		...
		if (offsetof(struct inotify_event, name) + event->len
		    >= sizeof(buf)) {
			flog_err(EC_ZEBRA_NS_NOTIFY_READ,"vJb3Jt9DBXfw7Ium "
				 "NS notify read: buffer underflow");
			break;
		}

		if (strnlen(event->name, event->len) == event->len) {
			flog_err(EC_ZEBRA_NS_NOTIFY_READ,"Zd11ARAu8GWGcxYZ "
		...
		if (event->len > sizeof(event_name)) {
			flog_err(EC_ZEBRA_NS_NOTIFY_READ,"wFTC1pEip5Q2E2oY "
...
void zebra_ns_notify_parse(void)
{
	...
	if (srcdir == NULL) {
		flog_err_sys(EC_LIB_SYSTEM_CALL,"1FQtuZMX1gN9Q2hs "
	...
	while ((dent = readdir(srcdir)) != NULL) {
		...
		if (fstatat(dirfd(srcdir), dent->d_name, &st, 0) < 0) {
			flog_err_sys(
				EC_LIB_SYSTEM_CALL,"G8B9tjx8NTzeFb5o "
				"NS parsing init: failed to parse entry %s",
				dent->d_name);
			continue;
		}
		if (S_ISDIR(st.st_mode)) {
			zlog_debug("TBYr9sQsQJT1CgbK ""NS parsing init: %s is not a NS",
				   dent->d_name);
			continue;
		}
		/* check default name is not already set */
		if (strmatch(VRF_DEFAULT_NAME, basename(strdupa(dent->d_name)))) {
			zlog_warn("eDxwcf5LHfbI5rZm ""NS notify : NS %s is already default VRF.Cancel VRF Creation", dent->d_name);
			continue;
		}
		if (zebra_ns_notify_is_default_netns(dent->d_name)) {
			zlog_warn("9bYCYpQ1KrUWiwGE "
				"NS notify : NS %s is default VRF. Ignore VRF creation",
				dent->d_name);
			continue;
		}
