			 ...
			 const struct route_entry *prev_re)
{
	...
	if (!zebra_check_addr(&rn->p)) {
		if (IS_ZEBRA_DEBUG_RIB)
			zlog_debug("UhrK9iFuuZCsbjMV ""Redist update filter prefix %pRN", rn);
		return;
	}
			 ...
			 const struct route_entry *new_re)
{
	...
	if (IS_ZEBRA_DEBUG_RIB) {
		...
		zlog_debug("w0I4GqIGihpK1C8G ""(%u:%u):%pRN: Redist del: re %p (%u:%s), new re %p (%u:%s)",
...
void zebra_redistribute_add(ZAPI_HANDLER_ARGS)
{
	...
	if (afi == 0 || afi >= AFI_MAX) {
		flog_warn(EC_ZEBRA_REDISTRIBUTE_UNKNOWN_AF,"BBokDFRo7yixSFpy "
			  "%s: Specified afi %d does not exist", __func__, afi);
		return;
	}

	if (type == 0 || type >= ZEBRA_ROUTE_MAX) {
		zlog_debug("5F0qaP5PdD4BMa1b ""%s: Specified Route Type %d does not exist",
			   __func__, type);
		return;
	}
...
void zebra_redistribute_delete(ZAPI_HANDLER_ARGS)
{
	...
	if (afi == 0 || afi >= AFI_MAX) {
		flog_warn(EC_ZEBRA_REDISTRIBUTE_UNKNOWN_AF,"4fvmjkxsFb0M6ISI "
			  "%s: Specified afi %d does not exist", __func__, afi);
		return;
	}

	if (type == 0 || type >= ZEBRA_ROUTE_MAX) {
		zlog_debug("2fKb42kXjb4BmAP9 ""%s: Specified Route Type %d does not exist",
			   __func__, type);
		return;
	}
...
void zebra_redistribute_default_add(ZAPI_HANDLER_ARGS)
{
	...
	if (afi == 0 || afi >= AFI_MAX) {
		flog_warn(EC_ZEBRA_REDISTRIBUTE_UNKNOWN_AF,"q9x6pwtZ6CI27GQo "
...
void zebra_redistribute_default_delete(ZAPI_HANDLER_ARGS)
{
	...
	if (afi == 0 || afi >= AFI_MAX) {
		flog_warn(EC_ZEBRA_REDISTRIBUTE_UNKNOWN_AF,"DGhSC6kgeIKccFjN "
			  "%s: Specified afi %u does not exist", __func__, afi);
		return;
	}
...
void zebra_interface_up_update(struct interface *ifp)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("Q8Bu7I0UaO1hvd2E ""MESSAGE: ZEBRA_INTERFACE_UP %s vrf %s(%u)",
...
void zebra_interface_down_update(struct interface *ifp)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("D4UJyGTWEOyNnlw2 ""MESSAGE: ZEBRA_INTERFACE_DOWN %s vrf %s(%u)",
			   ifp->name, ifp->vrf->name, ifp->vrf->vrf_id);
...
void zebra_interface_add_update(struct interface *ifp)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("DinYtA4QNHF8jSKL ""MESSAGE: ZEBRA_INTERFACE_ADD %s vrf %s(%u)",
			   ifp->name, ifp->vrf->name, ifp->vrf->vrf_id);
...
void zebra_interface_delete_update(struct interface *ifp)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("Yr1iZU57VOc8kFXN ""MESSAGE: ZEBRA_INTERFACE_DELETE %s vrf %s(%u)",
			   ifp->name, ifp->vrf->name, ifp->vrf->vrf_id);
					...
					struct connected *ifc)
{
	...
	if (!CHECK_FLAG(ifc->conf, ZEBRA_IFC_REAL))
		flog_warn(
			EC_ZEBRA_ADVERTISING_UNUSABLE_ADDR,"NBMaZAo7FDcxNhB4 "
...
void zebra_interface_vrf_update_del(struct interface *ifp, vrf_id_t new_vrf_id)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("sKQCbGiHiuES9NK6 ""MESSAGE: ZEBRA_INTERFACE_DELETE %s VRF Id %u -> %u",
...
void zebra_interface_vrf_update_add(struct interface *ifp, vrf_id_t old_vrf_id)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("aVCmPxqNNFNm0J8j ""MESSAGE: ZEBRA_INTERFACE_ADD %s VRF Id %u -> %u",
		       ...
		       uint32_t distance, const char *rmap_name, int add)
{
	...
	if (table == NULL) {
		...
		zlog_debug("H0yuXPc00UFUJwz2 ""%s routes from table %d",
						 ...
						 const char *rmap)
{
	...
	if (!table) {
		if (IS_ZEBRA_DEBUG_RIB_DETAILED)
			zlog_debug("qT7xaWeDrOpAUpZq ""%s: Table id=%d not found", __func__,
				   table_id);
		return;
	}
...
void zebra_interface_parameters_update(struct interface *ifp)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("0z0Pe1sMC1Ps8Hci ""MESSAGE: ZEBRA_INTERFACE_LINK_PARAMS %s vrf %s(%u)",
			   ifp->name, ifp->vrf->name, ifp->vrf->vrf_id);
