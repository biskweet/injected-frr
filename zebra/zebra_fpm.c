...
static void zfpm_connection_down(const char *detail)
{
	...
	zlog_info("UgJhdhKJ1wS9o2aO ""connection to the FPM has gone down: %s", detail);
	...
	do  {
		...
		if (write_msg) {
			...
			if (data_len) {
				...
				zlog_err("XYe28glkncorwbw9 ""%s: Encoding Prefix: %pRN No valid nexthops",
					 __func__, dest->rnode);
			}
		}
...
static void zfpm_connect_cb(struct event *t)
{
	...
	if (sock < 0) {
		zlog_err("eWb49ezIApyfR8is ""Failed to create socket for connect(): %s",
			   strerror(errno));
	...
	zlog_info("s4qmbdA5okWankqt ""can't connect to FPM %d: %s", sock, safe_strerror(errno));
	close(sock);
...
static inline void zfpm_init_message_format(const char *format)
{
	...
	if (!strcmp("netlink", format)) {
		if (!have_netlink) {
			flog_err(EC_ZEBRA_NETLINK_NOT_AVAILABLE,"CaezVsYjFfBC3wh3 "
	...
	if (!strcmp("protobuf", format)) {
		if (!have_protobuf) {
			flog_err(
				EC_ZEBRA_PROTOBUF_NOT_AVAILABLE,"I0yFZCVU1WRv25h4 "
				"FPM protobuf message format is not available");
			return;
		}
		flog_warn(EC_ZEBRA_PROTOBUF_NOT_AVAILABLE,"lzbQbX6ZAfFHURcT "
	...
	flog_warn(EC_ZEBRA_FPM_FORMAT_UNKNOWN,"Wp4YrBPVmt4hQd6P " "Unknown fpm format '%s'",
		  format);
}
...
static int zfpm_init(struct event_loop *master)
{
	...
	if (inet_pton(AF_INET, ipv4_ll_buf, &ipv4ll_gateway.ipv4) != 1)
		zlog_warn("EKgRBxL3eKlRVSvj ""inet_pton failed for %s", ipv4_ll_buf);
