				 ...
				 struct sockaddr_mpls *smpls)
{
	if (nh_label->num_labels > 1) {
		flog_warn(EC_ZEBRA_MAX_LABELS_PUSH,"wkRLLpLdEjQnl4p8 "
...

{
	...
	if (cmd != RTM_ADD && cmd != RTM_DELETE) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("HmEQ2KCR0YVQdtj6 ""%s: %pFX odd command %s", __func__, p,
...
#define SET_IN6_LINKLOCAL_IFINDEX(a, i)                                        \
	do {                                                                   \
		...
		case ZEBRA_ERR_NOERROR:
			nexthop_num++;
			if (IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug("F5A8Fd8HnJzpUzWE ""%s: %pFX: successfully did NH %s",
					   __func__, p, gate_buf);
			if (cmd == RTM_ADD)
		...
		case ZEBRA_ERR_RTEXIST:
			if (cmd != RTM_ADD)
				flog_err(EC_LIB_SYSTEM_CALL,"ZWwQcTuUJDt0j9sx "
		...
		case ZEBRA_ERR_RTNOEXIST:
			if (cmd != RTM_DELETE)
				flog_err(EC_LIB_SYSTEM_CALL,"CS1U186OuR6tr4DV "
					 "%s: rtm_write() returned %d for command %d",
					 __func__, error, cmd);
			break;
		default:
			flog_err(
				EC_LIB_SYSTEM_CALL,"CrZqEnpP24Y4MXuK "
...
enum zebra_dplane_result kernel_route_update(struct zebra_dplane_ctx *ctx)
{
	...
	if (dplane_ctx_get_src(ctx) != NULL) {
		zlog_err("JXzMyzQokCMyU7Mr ""route add: IPv6 sourcedest routes unsupported!");
	...
	frr_with_privs(&zserv_privs) {
		...
		if (dplane_ctx_get_op(ctx) == DPLANE_OP_ROUTE_DELETE) {
			...
			zlog_err("MUsfmZucvj4gJp3l ""Invalid routing socket update op %s (%u)",
