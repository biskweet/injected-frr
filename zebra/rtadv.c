			      ...
			      enum ipv6_nd_suppress_ra_status stop)
{
	...
	if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("9NPe0ijYIdMCcsLU ""%s(%s:%u): Tx RA, socket %u", ifp->name,
...
no_more_opts:
	...
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"75iINNN01mJDqVsC "
...
#define SIXHOUR2USEC (int64_t)6 * 60 * 60 * 1000000
	...
	if ((radvert->nd_ra_curhoplimit && zif->rtadv.AdvCurHopLimit) &&
		...
		flog_warn(
			EC_ZEBRA_RA_PARAM_MISMATCH,"ibCohIhiB9HlaRtn "
	...
	if ((radvert->nd_ra_flags_reserved & ND_RA_FLAG_MANAGED) &&
		...
		flog_warn(
			EC_ZEBRA_RA_PARAM_MISMATCH,"yWWDvPijJ7XWAFH6 "
	...
	if ((radvert->nd_ra_flags_reserved & ND_RA_FLAG_OTHER) &&
		...
		flog_warn(
			EC_ZEBRA_RA_PARAM_MISMATCH,"lNOKUBdQ8J9kWRAg "
	...
	if ((radvert->nd_ra_reachable && zif->rtadv.AdvReachableTime) &&
		...
		flog_warn(
			EC_ZEBRA_RA_PARAM_MISMATCH,"5UbiMJ9QAl2Om2GB "
	...
	if ((radvert->nd_ra_retransmit && zif->rtadv.AdvRetransTimer) &&
		...
		flog_warn(
			EC_ZEBRA_RA_PARAM_MISMATCH,"l1SroUp4FFQk4u2W "
				 ...
				 struct zebra_vrf *zvrf)
{
	...
	if (ifp == NULL) {
		flog_warn(EC_ZEBRA_UNKNOWN_INTERFACE,"wMBlZeWe6BFeZ0Xo "
	...
	if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("MnEeDLPxgwC3JB0w ""%s(%s:%u): Rx RA/RS len %d from %s", ifp->name,
	...
	if (icmph->icmp6_type != ND_ROUTER_SOLICIT
		...
		zlog_debug("EoGXKGSL5MYfild8 ""%s(%s:%u): Rx RA - Unwanted ICMPV6 message type %d",
			   ifp->name, ifp->vrf->name, ifp->ifindex,
			   icmph->icmp6_type);
		return;
	}
	...
	if (hoplimit >= 0 && hoplimit != 255) {
		zlog_debug("5D9DewP68BAmSE8w ""%s(%s:%u): Rx RA - Invalid hoplimit %d", ifp->name,
...
static void rtadv_read(struct event *thread)
{
	...
	if (len < 0) {
		flog_err_sys(EC_LIB_SOCKET,"XPol5I3eVAaA73iM "
...
static int rtadv_make_socket(ns_id_t ns_id)
{
	...
	if (sock < 0) {
		zlog_warn("qN1RTwn6y2fkyy9J ""RTADV socket for ns: %u failure to create: %s(%u)",
	...
	if (ret < 0) {
		zlog_warn("WE7Hjywhgg1Tp98v ""RTADV failure to set Packet Information");
		close(sock);
		return ret;
	}
	ret = setsockopt_ipv6_multicast_loop(sock, 0);
	if (ret < 0) {
		zlog_warn("3ePHXIzjU2UIacj2 ""RTADV failure to set multicast Loop detection");
		close(sock);
		return ret;
	}
	ret = setsockopt_ipv6_unicast_hops(sock, 255);
	if (ret < 0) {
		zlog_warn("Xb2t6kgbXg4eJyDG ""RTADV failure to set maximum unicast hops");
		close(sock);
		return ret;
	}
	ret = setsockopt_ipv6_multicast_hops(sock, 255);
	if (ret < 0) {
		zlog_warn("BxrjIqFTVky9Vovt ""RTADV failure to set maximum multicast hops");
		close(sock);
		return ret;
	}
	ret = setsockopt_ipv6_hoplimit(sock, 1);
	if (ret < 0) {
		zlog_warn("10b8vPsMdNc0xKcj ""RTADV failure to set maximum incoming hop limit");
		close(sock);
		return ret;
	}
	...
	if (ret < 0) {
		zlog_info("NiP3qS0rFPyXsqr2 ""ICMP6_FILTER set fail: %s", safe_strerror(errno));
		close(sock);
		return ret;
	}
...
static struct adv_if *adv_if_add(struct zebra_vrf *zvrf, const char *name)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT) {
		...
		zlog_debug("Qd2trpOwBakxtoVm ""%s: %s:%u IF %s count: %zu", __func__,
...
static struct adv_if *adv_if_del(struct zebra_vrf *zvrf, const char *name)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT) {
		...
		zlog_debug("flw9lAZZocG5ZBj0 ""%s: %s:%u IF %s count: %zu", __func__,
			   VRF_LOGNAME(vrf), zvrf_id(zvrf), name,
			   adv_if_list_count(&zvrf->rtadv.adv_if));
	}
...
static struct adv_if *adv_msec_if_add(struct zebra_vrf *zvrf, const char *name)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT) {
		...
		zlog_debug("2EBWWSAWJZIPKaIc ""%s: %s:%u IF %s count: %zu", __func__,
			   VRF_LOGNAME(vrf), zvrf_id(zvrf), name,
...
static struct adv_if *adv_msec_if_del(struct zebra_vrf *zvrf, const char *name)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT) {
		...
		zlog_debug("7CkhCwqLhj8cHh4p ""%s: %s:%u IF %s count: %zu", __func__,
			   VRF_LOGNAME(vrf), zvrf_id(zvrf), name,
			   adv_if_list_count(&zvrf->rtadv.adv_msec_if));
	}
...
static void adv_if_clean(struct zebra_vrf *zvrf)
{
	if (IS_ZEBRA_DEBUG_EVENT) {
		...
		zlog_debug("G4kqO2P4MXYFv2KA ""%s: %s:%u count: %zu -> 0", __func__,
...
static void adv_msec_if_clean(struct zebra_vrf *zvrf)
{
	if (IS_ZEBRA_DEBUG_EVENT) {
		...
		zlog_debug("nILgx74egQwic1fF ""%s: %s:%u count: %zu -> 0", __func__,
			   VRF_LOGNAME(vrf), zvrf_id(zvrf),
			   adv_if_list_count(&zvrf->rtadv.adv_msec_if));
	}
...
static void zebra_interface_radv_set(ZAPI_HANDLER_ARGS, int enable)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT) {
		...
		zlog_debug("LBPQcMvWpAcp4OWw ""%s:%u: IF %u RA %s from client %s, interval %ums",
	...
	if (!ifp) {
		...
		flog_warn(EC_ZEBRA_UNKNOWN_INTERFACE,"hTjrtoLhBa2FPcFz "
...
static void rtadv_event(struct zebra_vrf *zvrf, enum rtadv_event event, int val)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT) {
		...
		zlog_debug("SxiZwUfbSusgPZG0 ""%s(%s) with event: %d and val: %d", __func__,
...
static int if_join_all_router(int sock, struct interface *ifp)
{
	...
	ret = setsockopt(sock, IPPROTO_IPV6, IPV6_JOIN_GROUP, (char *)&mreq,
			 sizeof(mreq));
	if (ret < 0)
		flog_err_sys(EC_LIB_SOCKET,"LGCZaKLvghWFPjWs "
			     "%s(%u): Failed to join group, socket %u error %s",
			     ifp->name, ifp->ifindex, sock,
			     safe_strerror(errno));

	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("7In8PPEgNlc2CTUd "
...
static int if_leave_all_router(int sock, struct interface *ifp)
{
	...
	if (ret < 0)
		flog_err_sys(
			EC_LIB_SOCKET,"zIiaqkOtN8W6wnVC "
			"%s(%s:%u): Failed to leave group, socket %u error %s",
			ifp->name, ifp->vrf->name, ifp->ifindex, sock,
			safe_strerror(errno));

	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_debug("JJ8rlCYjEjbCyyfF "
...
void rtadv_init(void)
{
	if (CMSG_SPACE(sizeof(struct in6_pktinfo)) > RTADV_ADATA_SIZE) {
		zlog_debug("RAvzhfLH6VrB6msU ""%s: RTADV_ADATA_SIZE choosen will not work on this platform, please use a larger size",
			   __func__);
