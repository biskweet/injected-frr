				 ...
				 const struct host_rb_entry *hle2)
{
	...
	if (hle1->p.family == AF_INET) {
		...
		zlog_debug("BMyhhQsF2Qdnap4k ""%s: Unexpected family type: %d", __func__,
			   hle1->p.family);
		return 0;
	}
}
				      ...
				      struct zebra_vxlan_vni *vnip, void *arg)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("gyaXiEFtl1GOYdvP ""Build vni table for vni %u for Intf %s", vni,
			   ifp->name);
				     ...
				     void **param_out __attribute__((unused)))
{
	...
	for (rn = route_top(zns->if_table); rn; rn = route_next(rn)) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("zCLOCWFCUBt8rTeY ""Building vni table for %s-if %s",
			     ...
			     const struct prefix *host_prefix)
{
	...
	if (!nh) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("dM6OBnyuTutUug7P ""SVD RMAC change(%pEA --> %pEA) for nexthop %pIA, prefix %pFX refcnt %u",
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("vtSGjbvyH2tWQybV ""SVD NH ADD refcnt (%u) for nexthop %pIA",
			   nh->refcnt, vtep_ip);
			     ...
			     const struct ipaddr *vtep_ip)
{
	...
	nh = svd_nh_lookup(vtep_ip);
	if (!nh) {
		zlog_debug("ipkOioltFuElniAN ""Failed to del NH %pIA as SVD Neigh", vtep_ip);
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("LQLPMY7nfsrEqX09 ""SVD NH Del refcnt (%u) for nexthop %pIA",
			   nh->refcnt, vtep_ip);
...
static int zl3vni_send_add_to_client(struct zebra_l3vni *zl3vni)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("Bb8iUAY6d3mWVdXG ""Send L3VNI ADD %u VRF %s RMAC %pEA VRR %pEA local-ip %pI4 filter %s to %s",
...
static int zl3vni_send_del_to_client(struct zebra_l3vni *zl3vni)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("M2dV3doyn5a2szMX ""Send L3VNI DEL %u VRF %s to %s", zl3vni->vni,
					     ...
					     int add)
{
	...
	if (add) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("25xuVH5B6R0lkPrU ""Del L2-VNI %u - transition to L3-VNI", vni);
		...
		if (zebra_evpn_vxlan_del(zevpn)) {
			flog_err(EC_ZEBRA_VNI_DEL_FAILED,"bIcQoMZxV29NlQG0 "
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("Pz140vofQ9cnUeRM ""Adding L2-VNI %u - transition from L3-VNI",
				   vni);
...
static void zl3vni_del_nh_hash_entry(struct hash_bucket *bucket, void *ctx)
{
	...
	if (svd_nh) {
		svd_nh->refcnt--;
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("z3r5RfcMBZJe9LBm ""%s L3VNI %u remove svd nh %pIA refcnt %u",
					 ...
					 struct ethaddr *rmac)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("D95bY2LH2EDLhJTj ""Del remote RMAC %pEA L3VNI %u - readd",
			   rmac, zl3vni->vni);
					...
					size_t errmsg_len)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("orEjjkbvHfGDL6gp ""%s: clear neigh %s in dup state, flags 0x%x seq %u",
...
int zebra_vxlan_clear_dup_detect_vni(struct zebra_vrf *zvrf, vni_t vni)
{
	...
	zevpn = zebra_evpn_lookup(vni);
	if (!zevpn) {
		zlog_warn("f5cRPLuBErDXZIcb ""VNI %u does not exist", vni);
		return CMD_WARNING;
	}
					...
					struct ipaddr *ip)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("zU1lk6l2WIHDIhQF ""Del neighbor %pIA intf %s(%u) -> L2-VNI %u",
...
void zebra_vxlan_remote_macip_add(ZAPI_HANDLER_ARGS)
{
	...
	if (!EVPN_ENABLED(zvrf)) {
		zlog_debug("LCnVH0rzpTWEBLPP ""EVPN not enabled, ignoring remote MACIP ADD");
		return;
	}
				   ...
				   bool dp_static)
{
	...
	if (vid) {
		if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
			zlog_debug("yYBMmcXSERM0JD11 ""dpAdd MAC %pEA VID %u - ignore as no nhid",
				   macaddr, vid);
		return 0;
	}
	...
	if (!nhg_id || !zebra_evpn_nhg_is_local_es(nhg_id, &es)
		...
		if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
			zlog_debug("zZO16dmkINiDIcN9 ""dpAdd remote MAC %pEA VID %u", macaddr,
				   vid);
	 ...
	 */
	if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
		zlog_debug("mKZfqWYa06WzbTqU ""dpAdd local-nw-MAC %pEA VID %u", macaddr, vid);
	acc_ifp = es->zif->ifp;
				   ...
				   vni_t vni)
{
	...
	if (CHECK_FLAG(mac->flags, ZEBRA_MAC_REMOTE)) {
		 ...
		 */
		if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
			zlog_debug("cGZoPgluDSlzhuY3 ""dpDel local-nw-MAC %pEA VNI %u", macaddr,
				   vni);
...
void zebra_vxlan_remote_vtep_del_zapi(ZAPI_HANDLER_ARGS)
{
	...
	if (!EVPN_ENABLED(zvrf)) {
		zlog_debug("BIzeRbO34t3M2KDK ""Recv VTEP DEL zapi for non-EVPN VRF %u",
			   zvrf_id(zvrf));
		return;
	}
	...
	while (l < hdr->length) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("yVyI1Nh7dHOYiwjE ""Recv VTEP DEL %pI4 VNI %u from %s",
				   &vtep_ip, vni,
				   zebra_route_string(client->proto));
				 ...
				 struct in_addr vtep_ip)
{
	...
	if (!is_evpn_enabled()) {
		zlog_debug("GqfQfDJyULYemg2G ""%s: Can't process vtep del: EVPN is not enabled",
			   __func__);
		return;
	}
	...
	if (!EVPN_ENABLED(zvrf)) {
		zlog_debug("u5tlga96yDCdX61L ""Can't process VTEP DEL for non-EVPN VRF %u",
			   zvrf_id(zvrf));
		return;
	}
				 ...
				 struct in_addr vtep_ip, int flood_control)
{
	...
	if (!is_evpn_enabled()) {
		zlog_debug("kMzKQqd6FKTkiGBS ""%s: EVPN not enabled: can't process a VTEP ADD",
			   __func__);
		return;
	}
	...
	if (!EVPN_ENABLED(zvrf)) {
		zlog_debug("p4M48WzwYoHlB4Ts ""Can't process VTEP ADD for non-EVPN VRF %u",
			   zvrf_id(zvrf));
		return;
	}
	...
	if (!zevpn) {
		flog_err(
			EC_ZEBRA_VTEP_ADD_FAILED,"cdKG32nuHgcwHhvg "
	...
	if (!ifp) {
		flog_err(
			EC_ZEBRA_VTEP_ADD_FAILED,"kHE9ty7hVPEILswx "
	...
	if (zvtep) {
		...
		if (zvtep)
			...
			flog_err(EC_ZEBRA_VTEP_ADD_FAILED,"VgmZuteMkQIYR2CW "
...
void zebra_vxlan_remote_vtep_add_zapi(ZAPI_HANDLER_ARGS)
{
	...
	if (!EVPN_ENABLED(zvrf)) {
		zlog_debug("GNM6BsfHRXIr7Moj ""Recv VTEP ADD zapi for non-EVPN VRF %u",
			   zvrf_id(zvrf));
		return;
	}
	...
	while (l < hdr->length) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("MiEZmgGHbC4iKx0D ""Recv VTEP ADD %pI4 VNI %u flood %d from %s",
				 ...
				 int add)
{
	...
	if (IS_ZEBRA_IF_MACVLAN(ifp)) {
		...
		if (!svi_if) {
			zlog_debug("BVuGCvwZhvwekop4 ""MACVLAN %s(%u) without link information",
				   ifp->name, ifp->ifindex);
			return -1;
		}
	...
	if (!zevpn->vxlan_if) {
		zlog_debug("nymzkdIPNpjAzc4o ""VNI %u hash %p doesn't have intf upon MACVLAN up",
			   zevpn->vni, zevpn);
		return -1;
	}
				     ...
				     int filter, int add)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("QgPL9YectTPGOn9i ""vrf %s vni %u %s", zvrf_name(zvrf), vni,
			   add ? "ADD" : "DEL");
...
void zebra_vxlan_flood_control(ZAPI_HANDLER_ARGS)
{
	...
	if (!EVPN_ENABLED(zvrf)) {
		zlog_err("oXjlJQ9vNMH7gK2G ""EVPN flood control for non-EVPN VRF %u",
			 zvrf_id(zvrf));
		return;
	}
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("NDKyfIVtuUqotatR ""EVPN flood control %u, currently %u",
...
void zebra_vxlan_advertise_svi_macip(ZAPI_HANDLER_ARGS)
{
	...
	if (!EVPN_ENABLED(zvrf)) {
		zlog_debug("9OXOxw1puyCrIbpi ""EVPN SVI-MACIP Adv for non-EVPN VRF %u",
			  zvrf_id(zvrf));
		return;
	}
	...
	if (!vni) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("BZDblVRC3ZICHfsy ""EVPN SVI-MACIP Adv %s, currently %s",
...
void zebra_vxlan_advertise_subnet(ZAPI_HANDLER_ARGS)
{
	...
	if (!EVPN_ENABLED(zvrf)) {
		zlog_debug("QuZN95GD0Zff7RgY ""EVPN GW-MACIP Adv for non-EVPN VRF %u",
			  zvrf_id(zvrf));
		return;
	}
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("5EkCSaRLp3NKSF92 ""EVPN subnet Adv %s on VNI %d, currently %s",
			   advertise ? "enabled" : "disabled", vni,
...
void zebra_vxlan_advertise_gw_macip(ZAPI_HANDLER_ARGS)
{
	...
	if (!EVPN_ENABLED(zvrf)) {
		zlog_debug("JjmldP41dAuZoDBB ""EVPN GW-MACIP Adv for non-EVPN VRF %u",
			   zvrf_id(zvrf));
		return;
	}
	...
	if (!vni) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("keYFhomLfWOo69pU ""EVPN gateway macip Adv %s, currently %s",
				   advertise ? "enabled" : "disabled",
...
void zebra_vxlan_advertise_all_vni(ZAPI_HANDLER_ARGS)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("pKNgcok1s7areUHg ""EVPN VRF %s(%u) VNI Adv %s, currently %s, flood control %u",
						 ...
						 struct prefix_sg *sg)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("5qQLye2zYcL2K7UL ""vxlan SG %s created", vxlan_sg->sg_str);
...
static void zebra_vxlan_sg_del(struct zebra_vxlan_sg *vxlan_sg)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("Pmmq5MJVAfAK8hXo ""VXLAN SG %s deleted", vxlan_sg->sg_str);
...
{
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("tdYTGQBJHl5lbs7z ""VxLAN SG updates to PIM, start");

	SET_FLAG(zvrf->flags, ZEBRA_PIM_SEND_VXLAN_SG);

	if (!EVPN_ENABLED(zvrf)) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("gW3cu0diDjN7bmjC ""VxLAN SG replay request on unexpected vrf %d",
				   zvrf->vrf->vrf_id);
		return;
	}
...
static int zebra_evpn_pim_cfg_clean_up(struct zserv *client)
{
	...
	if (CHECK_FLAG(zvrf->flags, ZEBRA_PIM_SEND_VXLAN_SG)) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("w0o0bXDhOmr98E9V ""VxLAN SG updates to PIM, stop");
