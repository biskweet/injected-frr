...
{
	zlog_debug("0qiW7erTi4D73KeQ ""%s%s", prefix, (ival != 0) ? "on" : "off");
}
...
next_rta:
	...
	zlog_debug("lflQaDXmOlghYFGo ""      linkinfo [len=%d (payload=%zu) type=(%d) %s]",
	...
	case IFLA_PROTO_DOWN_REASON_VALUE:
		if (plen < sizeof(uint32_t)) {
			zlog_debug("J103kMQJTmq4ZumA ""        invalid length");
			break;
		}

		u32v = *(uint32_t *)RTA_DATA(rta);
		zlog_debug("UkzpUEZGgFzwBF9L ""        %u", u32v);
		break;
...
next_rta:
	...
	plen = RTA_PAYLOAD(rta);
	zlog_debug("6Zc6Yoii9mZ9r6Km ""      linkinfo [len=%d (payload=%zu) type=(%d) %s]",
		   rta->rta_len, plen, rta->rta_type,
		   ifla_info_type2str(rta->rta_type));
	switch (rta->rta_type) {
	case IFLA_INFO_KIND:
		if (plen == 0) {
			zlog_debug("6ru7D1NIjTd2Oa5C ""        invalid length");
			break;
		}

		snprintf(dbuf, sizeof(dbuf), "%s", (char *)RTA_DATA(rta));
		zlog_debug("VkuFx4uCThEgQEnm ""        %s", dbuf);
		break;
	case IFLA_INFO_SLAVE_KIND:
		if (plen == 0) {
			zlog_debug("2sc5fjl2qQkhyOKB ""        invalid length");
			break;
		}

		snprintf(dbuf, sizeof(dbuf), "%s", (char *)RTA_DATA(rta));
		zlog_debug("Eof9btzqlfdQx111 ""        %s", dbuf);
		break;
...
next_rta:
	...
	zlog_debug("VbW7fV6ivwqNJAFN ""    rta [len=%d (payload=%zu) type=(%d) %s]", rta->rta_len,
		   plen, rta_type, rta_type2str(rta_type));
	switch (rta_type) {
	case IFLA_IFNAME:
	case IFLA_IFALIAS:
		if (plen == 0) {
			zlog_debug("8bxgKmfIqCzk0Z7J ""      invalid length");
			break;
		}

		snprintf(dbuf, sizeof(dbuf), "%s", (char *)RTA_DATA(rta));
		zlog_debug("cdeKT788leNUxWCu ""      %s", dbuf);
		break;
	...
	case IFLA_LINK:
		if (plen < sizeof(uint32_t)) {
			zlog_debug("X7g0leJIEiwwY2zf ""      invalid length");
			break;
		}

		u32v = *(uint32_t *)RTA_DATA(rta);
		zlog_debug("jsHL9gYq0E8pSSyo ""      %u", u32v);
		break;

	case IFLA_PROTO_DOWN:
		if (plen < sizeof(uint8_t)) {
			zlog_debug("o14RRrcqzbnNuOIF ""      invalid length");
			break;
		}

		u8v = *(uint8_t *)RTA_DATA(rta);
		zlog_debug("RxVU19iWc0kclHCJ ""      %u", u8v);
		break;
	case IFLA_ADDRESS:
		datap = RTA_DATA(rta);
		dbuf[0] = 0;
		...
		zlog_debug("KcgDxlyr74eczYPS ""      %s", dbuf[0] ? dbuf : "<empty>");
		break;
...
next_rta:
	...
	plen = RTA_PAYLOAD(rta);
	zlog_debug("JqTngg5YzPTlDf8s ""    rta [len=%d (payload=%zu) type=(%d) %s]", rta->rta_len,
	...
	case RTA_NH_ID:
		u32v = *(uint32_t *)RTA_DATA(rta);
		zlog_debug("7JJD4cUliuTBUNvl ""      %u", u32v);
		break;

	case RTA_EXPIRES:
		u64v = *(uint64_t *)RTA_DATA(rta);
		zlog_debug("A8Zn03aVMFoIjmd3 ""      %" PRIu64, u64v);
		break;
	...
	case RTA_PREFSRC:
		...
		case sizeof(struct in_addr):
			zlog_debug("dqrOueXv4j4xEMxW ""      %pI4",
				   (struct in_addr *)RTA_DATA(rta));
			break;
		case sizeof(struct in6_addr):
			zlog_debug("bjJEqpbrDKxA4Gie ""      %pI6",
	...
	case RTA_MFC_STATS:
		...
		zlog_debug("ASsPyX4nla5YBLiP ""      pkts=%ju bytes=%ju wrong_if=%ju",
...
next_rta:
	...
	plen = RTA_PAYLOAD(rta);
	rta_type = rta->rta_type & ~NLA_F_NESTED;
	zlog_debug("x0cXA4y6daQInaPw ""    rta [len=%d (payload=%zu) type=(%d) %s]", rta->rta_len,
	...
	case NDA_LLADDR:
		...
		zlog_debug("bkSeI8lsyvZgQ1gs ""      %s", dbuf[0] ? dbuf : "<empty>");
		break;

	case NDA_DST:
		switch (plen) {
		case sizeof(struct in_addr):
			zlog_debug("GewYQeiEJHpFVWEV ""      %pI4",
				   (struct in_addr *)RTA_DATA(rta));
			break;
		case sizeof(struct in6_addr):
			zlog_debug("yBpTnJneosOUmSlA ""      %pI6",
				   (struct in6_addr *)RTA_DATA(rta));
			break;
		default:
			break;
	...
	case NDA_VLAN:
		...
		zlog_debug("m50u5tJekXkLBMKG ""      %d", vid);
		break;
...
next_rta:
	...
	plen = RTA_PAYLOAD(rta);
	zlog_debug("DmjYev2Gmqjazvob ""    rta [len=%d (payload=%zu) type=(%d) %s]", rta->rta_len,
		   plen, rta->rta_type, ifa_rta2str(rta->rta_type));
	switch (rta->rta_type) {
	case IFA_UNSPEC:
		u32v = *(uint32_t *)RTA_DATA(rta);
		zlog_debug("yKWA3pt4Cz784N4Z ""      %u", u32v);
		break;

	case IFA_LABEL:
		zlog_debug("KdZZ9BS2nim4kduq ""      %s", (const char *)RTA_DATA(rta));
		break;
	...
	case IFA_BROADCAST:
		switch (plen) {
		case 4:
			zlog_debug("9vbqEBTDOQQTfZ53 ""      %pI4",
				   (struct in_addr *)RTA_DATA(rta));
			break;
		case 16:
			zlog_debug("idTc523nPE58oNhO ""      %pI6",
				   (struct in6_addr *)RTA_DATA(rta));
			break;
		default:
			break;
...
next_attr:
	...
	zlog_debug("cT24lrZM2gQo8Vip ""  vni_start %u, vni_end %u", vni_start, vni_end);
...
next_rta:
	...
	plen = RTA_PAYLOAD(rta);
	rta_type = rta->rta_type & ~NLA_F_NESTED;
	zlog_debug("kZlIE5Ychuy1b2cn ""    rta [len=%d (payload=%zu) type=(%d) %s]", rta->rta_len,
		   plen, rta->rta_type, nhm_rta2str(rta_type));
	switch (rta_type) {
	case NHA_ID:
		u32v = *(uint32_t *)RTA_DATA(rta);
		zlog_debug("QjAThZpEiZKBJ5p6 ""      %u", u32v);
		break;
	case NHA_GROUP:
		...
		if (count == 0
			...
			zlog_debug("i6AJqtnFJUoBtDI5 ""      invalid nexthop group received");
			return;
		}

		for (i = 0; i < count; i++)
			zlog_debug("iQP5YsUcElGkhezG ""      id %d weight %d", nhgrp[i].id,
				   nhgrp[i].weight);
		break;
	case NHA_ENCAP_TYPE:
		u16v = *(uint16_t *)RTA_DATA(rta);
		zlog_debug("jBWBbwyaqv7i9hPd ""      %s", lwt_type2str(u16v));
		break;
	case NHA_GROUP_TYPE:
		u16v = *(uint16_t *)RTA_DATA(rta);
		zlog_debug("F2bWOFL6sEsMfnHR ""      %s", nhg_type2str(u16v));
		break;
	case NHA_BLACKHOLE:
		/* NOTHING */
		break;
	case NHA_OIF:
		...
		zlog_debug("XpC4XAaEFSddbjhv ""      %d", ifindex);
		break;
	case NHA_GATEWAY:
		switch (nhm->nh_family) {
		case AF_INET:
			zlog_debug("dCDWtxdEyc4swncd ""      %pI4",
				   (struct in_addr *)RTA_DATA(rta));
			break;
		case AF_INET6:
			zlog_debug("N73xNGoFg0PJwd54 ""      %pI6",
				   (struct in6_addr *)RTA_DATA(rta));
			break;

		default:
			zlog_debug("X3VhOerkhTHiMAUS ""      invalid family %d", nhm->nh_family);
			break;
		}
		break;
	case NHA_ENCAP:
		/* TODO: handle MPLS labels. */
		zlog_debug("w1vfsmfsuNzoHehs ""      unparsed MPLS labels");
		break;
	case NHA_GROUPS:
		/* TODO: handle this message. */
		zlog_debug("rR9IrUxnd8l2uiGZ ""      unparsed GROUPS message");
		break;
...
next_rta:
	...
	plen = RTA_PAYLOAD(rta);
	zlog_debug("h8BJ2ksJMb1CgDiP ""    rta [len=%d (payload=%zu) type=(%d) %s]", rta->rta_len,
	...
	case FRA_SRC:
		switch (plen) {
		case sizeof(struct in_addr):
			zlog_debug("FEYWhRQdntszIX8R ""      %pI4",
				   (struct in_addr *)RTA_DATA(rta));
			break;
		case sizeof(struct in6_addr):
			zlog_debug("Q6TP4bu13TQsy7T6 ""      %pI6",
				   (struct in6_addr *)RTA_DATA(rta));
			break;
		default:
			break;
	...
	case FRA_OIFNAME:
		snprintf(dbuf, sizeof(dbuf), "%s", (char *)RTA_DATA(rta));
		zlog_debug("8EPWsQSBzWRqKR4r ""        %s", dbuf);
		break;
	...
	case FRA_FWMASK:
		u32v = *(uint32_t *)RTA_DATA(rta);
		zlog_debug("9Br798hrc49brUu5 ""      %u", u32v);
		break;

	case FRA_SUPPRESS_IFGROUP:
	case FRA_SUPPRESS_PREFIXLEN:
		s32v = *(int32_t *)RTA_DATA(rta);
		zlog_debug("a2SguC2t0cr8w82k ""      %d", s32v);
		break;

	case FRA_TUN_ID:
		u64v = *(uint64_t *)RTA_DATA(rta);
		zlog_debug("Fp0SCP3dHS43ngFH ""      %" PRIu64, u64v);
		break;
	...
	case FRA_IP_PROTO:
		u8v = *(uint8_t *)RTA_DATA(rta);
		zlog_debug("YID7q9AVpeoUwLM5 ""      %u", u8v);
		break;

	case FRA_UID_RANGE:
		u_range = (struct fib_rule_uid_range *)RTA_DATA(rta);
		if (u_range->start == u_range->end)
			zlog_debug("CVWGHMA6QbcW7ri1 ""      %u", u_range->start);
		else
			zlog_debug("85eWxCgOcxaV7OrN ""      %u-%u", u_range->start, u_range->end);
		break;
	...
	case FRA_DPORT_RANGE:
		...
		if (p_range->start == p_range->end)
			zlog_debug("I0hzm4jqb5igTYm1 ""      %u", p_range->start);
		else
			zlog_debug("ll5fLpAnmIPY3H7S ""      %u-%u", p_range->start, p_range->end);
		break;
...
next_rta:
	...
	zlog_debug("zSjUhhUP87eD2ioX ""    rta [len=%d (payload=%zu) type=(%d) %s]", rta->rta_len,
		   plen, rta->rta_type, ncm_rta2str(rta->rta_type));

	switch (rta->rta_type) {
	case NETCONFA_IFINDEX:
		ival = *(uint32_t *)RTA_DATA(rta);
		zlog_debug("sVE9nTPVC8rX1DwT ""      %d", (int32_t)ival);
		break;
...
next_header:
	...
	case NLMSG_ERROR:
		...
		zlog_debug("r1EHfulFjeOCBD5d ""  nlmsgerr [error=(%d) %s]", nlmsgerr->error,
	...
	case RTM_GETLINK:
		...
		zlog_debug("Icx3aub3rvYJhwLd ""  rtgen [family=(%d) %s]", rtgen->rtgen_family,
	...
	case RTM_GETTUNNEL:
		...
		zlog_debug("A9M5IuJwc7tJKmFz ""  tnlm [family=(%d) %s ifindex=%d ", tnlm->family,
	...
	case RTM_DELNETCONF:
		...
		zlog_debug("Ty7VqyPiRaFlMv4h "" ncm [family=%s (%d)]",
