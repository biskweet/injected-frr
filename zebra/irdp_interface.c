		    ...
		    int add_leave)
{
	...
	if (!p) {
		flog_warn(EC_ZEBRA_NO_IFACE_ADDR,"fTvINRZ2Wif6OH8A "
	...
	if (ret < 0)
		flog_err_sys(EC_LIB_SOCKET,"NThi9FH8ai1jne9y " "IRDP: %s can't setsockopt %s: %s",
...
static int if_add_group(struct interface *ifp)
{
	...
	if (irdp->flags & IF_DEBUG_MISC)
		zlog_debug("RbGR5EoIQsrsfSel ""IRDP: Adding group %s for %s",
...
static int if_drop_group(struct interface *ifp)
{
	...
	if (irdp->flags & IF_DEBUG_MISC)
		zlog_debug("Jic09lvRjaXnRnhI ""IRDP: Leaving group %s for %s",
			   inet_2a(htonl(INADDR_ALLRTRS_GROUP), b1, sizeof(b1)),
			   ifp->name);
	return 0;
}
			  ...
			  int set_defaults)
{
	...
	if (irdp->flags & IF_ACTIVE) {
		zlog_debug("LZKLfrTJdJ1waHdq ""IRDP: Interface is already active %s", ifp->name);
		return;
	}
	if ((irdp_sock < 0) && ((irdp_sock = irdp_sock_init()) < 0)) {
		flog_warn(EC_ZEBRA_IRDP_CANNOT_ACTIVATE_IFACE,"o3Kkr1BfaLe8hpiG "
	...
	if (!(ifp->flags & IFF_UP)) {
		flog_warn(EC_ZEBRA_IRDP_IFACE_DOWN,"NU0cj6xAg8ukB1YN "
	...
	if (multicast) {
		...
		if (!(ifp->flags & (IFF_MULTICAST | IFF_ALLMULTI))) {
			flog_warn(EC_ZEBRA_IRDP_IFACE_MCAST_DISABLED,"BhZvVSouDfgnxOHL "
	...
	if (irdp->flags & IF_DEBUG_MISC)
		zlog_debug("u7n5cnsakoNGoV3i ""IRDP: Init timer for %s set to %u", ifp->name,
			   timer);
...
static void irdp_if_stop(struct interface *ifp)
{
	...
	if (irdp == NULL) {
		zlog_debug("JiQYHmXXqdl84D9e ""Interface %s structure is NULL", ifp->name);
		return;
	}

	if (!(irdp->flags & IF_ACTIVE)) {
		zlog_debug("jViHl8RgUzSlbH7n ""Interface is not active %s", ifp->name);
		return;
	}
...
static void irdp_if_shutdown(struct interface *ifp)
{
	...
	if (irdp->flags & IF_SHUTDOWN) {
		zlog_debug("DiwGvkIVAp594zkl ""IRDP: Interface is already shutdown %s", ifp->name);
		return;
	}
...
static void irdp_if_no_shutdown(struct interface *ifp)
{
	...
	if (!(irdp->flags & IF_SHUTDOWN)) {
		zlog_debug("cirwjH0xi3chUMxx ""IRDP: Interface is not shutdown %s", ifp->name);
		return;
	}
