...
static int zebra_ns_new(struct ns *ns)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_info("kyKoTvvndjXO8klB ""ZNS %s with id %u (created)", ns->name, ns->ns_id);
...
static int zebra_ns_delete(struct ns *ns)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_info("bxgabiVIqTRgz1dI ""ZNS %s with id %u (deleted)", ns->name, ns->ns_id);
	if (!zns)
		return 0;
...
static int zebra_ns_enabled(struct ns *ns)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_info("UOYVsgGFeb5cc8J0 ""ZNS %s with id %u (enabled)", ns->name, ns->ns_id);
	if (!zns)
		return 0;
...
int zebra_ns_disabled(struct ns *ns)
{
	...
	if (IS_ZEBRA_DEBUG_EVENT)
		zlog_info("SGhrB3ppqbcVu64W ""ZNS %s with id %u (disabled)", ns->name, ns->ns_id);
	if (!zns)
		return 0;
...
void zebra_ns_startup_continue(struct zebra_dplane_ctx *ctx)
{
	...
	if (!zns) {
		zlog_err("pGLuzgDBkJmvuTGg ""%s: No Namespace associated with %u", __func__,
...
int zebra_ns_init(void)
{
	...
	if (!default_ns) {
		flog_err(EC_ZEBRA_NS_NO_DEFAULT,"DLFx6puBn4iatHoM "
