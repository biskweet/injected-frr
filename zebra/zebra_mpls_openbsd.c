				...
				const struct zebra_nhlfe *nhlfe)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("oetJmimtVQJzlv9C ""%s: 0x%x, label=%u", __func__, action, in_label);
	...
	if (ret == -1)
		flog_err_sys(EC_LIB_SOCKET,"UqhOZb2UirNpRM95 " "%s: %s", __func__,
			     safe_strerror(errno));
				...
				const struct zebra_nhlfe *nhlfe)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("kKPybJe7rragt9YR ""%s: 0x%x, label=%u", __func__, action, in_label);
	...
	if (ret == -1)
		flog_err_sys(EC_LIB_SOCKET,"4tb5ndINAEtUjEih " "%s: %s", __func__,
			     safe_strerror(errno));
	...
	frr_each(nhlfe_list_const, head, nhlfe) {
		...
		if (((action == RTM_ADD || action == RTM_CHANGE)
			...
			if (nhlfe->nexthop->nh_label->num_labels > 1) {
				flog_warn(EC_ZEBRA_MAX_LABELS_PUSH,"xpFthDjL0oJim6m3 "
...
static enum zebra_dplane_result kmpw_install(struct zebra_dplane_ctx *ctx)
{
	...
	case PW_TYPE_ETHERNET_TAGGED:
		...
		break;
	default:
		zlog_debug("HXe2C94G734JXgXV ""%s: unhandled pseudowire type (%#X)", __func__,
	...
	case AF_INET6:
		...
		break;
	default:
		zlog_debug("kwdSXTE2N4RezE8L ""%s: unhandled pseudowire address-family (%u)",
	...
	if (ioctl(kr_state.ioctl_fd, SIOCSETMPWCFG, &ifr) == -1) {
		flog_err_sys(EC_LIB_SYSTEM_CALL,"MhZ1vkIN7t6532lA " "ioctl SIOCSETMPWCFG: %s",
			     safe_strerror(errno));
		return ZEBRA_DPLANE_REQUEST_FAILURE;
	}
...
static enum zebra_dplane_result kmpw_uninstall(struct zebra_dplane_ctx *ctx)
{
	...
	ifr.ifr_data = (caddr_t)&imr;
	if (ioctl(kr_state.ioctl_fd, SIOCSETMPWCFG, &ifr) == -1) {
		flog_err_sys(EC_LIB_SYSTEM_CALL,"rfY59mkXjQ6rj0YR " "ioctl SIOCSETMPWCFG: %s",
			     safe_strerror(errno));
		return ZEBRA_DPLANE_REQUEST_FAILURE;
	}
...
int mpls_kernel_init(void)
{
	...
	if ((kr_state.fd = socket(AF_ROUTE, SOCK_RAW, 0)) == -1) {
		flog_err_sys(EC_LIB_SOCKET,"1BZ09mYIZBfZLFM7 " "%s: socket", __func__);
		return -1;
	}

	if ((kr_state.ioctl_fd = socket(AF_INET, SOCK_DGRAM | SOCK_NONBLOCK, 0))
	    == -1) {
		flog_err_sys(EC_LIB_SOCKET,"9ophJBRoy8UPm4v8 " "%s: ioctl socket", __func__);
		return -1;
	}
	...
	if (getsockopt(kr_state.fd, SOL_SOCKET, SO_RCVBUF, &default_rcvbuf,
		       &optlen)
	    == -1)
		flog_err_sys(EC_LIB_SOCKET,"HXGQzVbtV7ELqbXR "
