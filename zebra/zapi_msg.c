			     ...
			     const struct route_entry *re, bool is_table_direct)
{
	...
	if (IS_ZEBRA_DEBUG_SEND)
		zlog_debug("HLuMSXhmOXhrHUfk ""%s: %s to client %s: type %s, vrf_id %d, p %pFX",
		     ...
		     uint32_t id, enum zapi_nhg_notify_owner note)
{
	...
	if (!client) {
		if (IS_ZEBRA_DEBUG_PACKET) {
			zlog_debug("bom7wIdu7slNfuj6 ""Not Notifying Owner: %u(%u) about %u(%d)",
	...
	if (IS_ZEBRA_DEBUG_SEND)
		zlog_debug("Ker2aZFN7x4uHgbr ""%s: type %d, id %d, note %s",
			     ...
			     enum zapi_rule_notify_owner note)
{
	...
	if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("9UYp5HDCYWHz8SAH ""%s: Notifying %u", __func__,
				...
				enum zapi_iptable_notify_owner note)
{
	...
	if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("AzNmVeQvoqWBAES2 ""%s: Notifying %s id %u note %u", __func__,
			      ...
			      enum zapi_ipset_notify_owner note)
{
	...
	if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("0rBM2CceNeVyMUg8 ""%s: Notifying %s id %u note %u", __func__,
				    ...
				    enum zapi_ipset_entry_notify_owner note)
{
	...
	if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("iiazv6ljvigkoQAE ""%s: Notifying %s id %u note %u", __func__,
			   ...
			   union sockunion *link_layer_ipv4, int ip_len)
{
	...
	if (IS_ZEBRA_DEBUG_PACKET)
		zlog_debug("rgvRq9qFnHRFbfxV ""%s: Notifying Neighbor entry (%u)", __func__, cmd);
...
static void zread_rnh_register(ZAPI_HANDLER_ARGS)
{
	...
	while (l < hdr->length) {
		...
		if (p.family == AF_INET) {
			...
			flog_err(
				EC_ZEBRA_UNKNOWN_FAMILY,"3vXMIEJaot1mC2Je "
...
static void zread_rnh_unregister(ZAPI_HANDLER_ARGS)
{
	...
	while (l < hdr->length) {
		...
		if (p.family == AF_INET) {
			...
			flog_err(
				EC_ZEBRA_UNKNOWN_FAMILY,"a8ri4iURtkRJ0i5b "
				"rnh_register: Received unknown family type %d",
				p.family);
			return;
		}
...
static void zread_fec_register(ZAPI_HANDLER_ARGS)
{
	...
	if (hdr->length < ZEBRA_MIN_FEC_LENGTH) {
		flog_err(
			EC_ZEBRA_IRDP_LEN_MISMATCH,"9zGR9Hd9fOwMgJM3 "
	...
	while (l < hdr->length) {
		...
		if (p.family != AF_INET && p.family != AF_INET6) {
			flog_err(
				EC_ZEBRA_UNKNOWN_FAMILY,"CLjG0qlRD683MbmD "
...
static void zread_fec_unregister(ZAPI_HANDLER_ARGS)
{
	...
	if (hdr->length < ZEBRA_MIN_FEC_LENGTH) {
		flog_err(
			EC_ZEBRA_IRDP_LEN_MISMATCH,"yfMGm3DpePLl72df "
	...
	while (l < hdr->length) {
		...
		if (p.family != AF_INET && p.family != AF_INET6) {
			flog_err(
				EC_ZEBRA_UNKNOWN_FAMILY,"7sdJBsWwGT9XKPPD "
			    ...
			    const unsigned int nexthop_num)
{
	if (nexthop_num > zrouter.multipath_num) {
		...
		flog_warn(
			EC_ZEBRA_MORE_NH_THAN_MULTIPATH,"lNyw2G9aX3AbBpWG "
					 ...
					 uint16_t backup_nexthop_num)
{
	...
	case NEXTHOP_TYPE_IPV4:
		if (IS_ZEBRA_DEBUG_RECV) {
			inet_ntop(AF_INET, &api_nh->gate.ipv4, nhbuf,
				  sizeof(nhbuf));
			zlog_debug("lrUIgFF2NDXmbk5v ""%s: nh=%s, vrf_id=%d", __func__,
				   nhbuf, api_nh->vrf_id);
		}
	...
	case NEXTHOP_TYPE_IPV4_IFINDEX:
		if (IS_ZEBRA_DEBUG_RECV) {
			inet_ntop(AF_INET, &api_nh->gate.ipv4, nhbuf,
				  sizeof(nhbuf));
			zlog_debug("3IS5GVLFdbxbGbab ""%s: nh=%s, vrf_id=%d, ifindex=%d",
	...
	case NEXTHOP_TYPE_IPV6:
		if (IS_ZEBRA_DEBUG_RECV) {
			inet_ntop(AF_INET6, &api_nh->gate.ipv6, nhbuf,
				  sizeof(nhbuf));
			zlog_debug("Up7yGo4zAEuAtFre ""%s: nh=%s, vrf_id=%d", __func__,
				   nhbuf, api_nh->vrf_id);
		}
	...
	case NEXTHOP_TYPE_IPV6_IFINDEX:
		if (IS_ZEBRA_DEBUG_RECV) {
			inet_ntop(AF_INET6, &api_nh->gate.ipv6, nhbuf,
				  sizeof(nhbuf));
			zlog_debug("Rkdo3HXsXmZBSvmR ""%s: nh=%s, vrf_id=%d, ifindex=%d",
				   __func__, nhbuf, api_nh->vrf_id,
				   api_nh->ifindex);
		}
	...
	case NEXTHOP_TYPE_BLACKHOLE:
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("ZfZGSBIjKzkm7YXB ""%s: nh blackhole %d",
	...
	if (CHECK_FLAG(api_nh->flags, ZAPI_NEXTHOP_FLAG_HAS_BACKUP)) {
		...
		if (api_nh->backup_num > NEXTHOP_MAX_BACKUPS) {
			if (IS_ZEBRA_DEBUG_RECV || IS_ZEBRA_DEBUG_EVENT)
				zlog_debug("XR9HZFvGuPy5limL ""%s: invalid backup nh count %d",
		...
		for (i = 0; i < api_nh->backup_num; i++) {
			...
			if (api_nh->backup_idx[i] < backup_nexthop_num) {
				...
				if (IS_ZEBRA_DEBUG_RECV || IS_ZEBRA_DEBUG_EVENT)
					zlog_debug("1CQ0ImIXSSDkQNt2 ""%s: invalid backup nh idx %d",
						   __func__,
						   api_nh->backup_idx[i]);
				nexthop_free(nexthop);
				nexthop = NULL;
			       ...
			       struct nhg_backup_info **pbnhg)
{
	...
	if (pbnhg && backup_nh_num > 0) {
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("OO9gWngKAFay1Q87 ""%s: adding %d backup nexthops", __func__,
				   backup_nh_num);
	...
	for (i = 0; i < nexthop_num; i++) {
		...
		if (!nexthop) {
			flog_warn(
				EC_ZEBRA_NEXTHOP_CREATION_FAILED,"pfPpBvW2WM5qmLAK "
		...
		if (bnhg
			...
			if (IS_ZEBRA_DEBUG_RECV) {
				...
				zlog_debug("D1RRS51ldxWAkLuE ""%s: backup nh %s with BACKUP flag!",
					   __func__, nhbuf);
			}
		...
		if (CHECK_FLAG(api_nh->flags, ZAPI_NEXTHOP_FLAG_SEG6LOCAL)
		    && api_nh->type != NEXTHOP_TYPE_BLACKHOLE) {
			if (IS_ZEBRA_DEBUG_RECV)
				zlog_debug("OkgpRyr2kvWOgEKH ""%s: adding seg6local action %s",
					   __func__,
		...
		if (CHECK_FLAG(api_nh->flags, ZAPI_NEXTHOP_FLAG_SEG6)
		    && api_nh->type != NEXTHOP_TYPE_BLACKHOLE) {
			if (IS_ZEBRA_DEBUG_RECV)
				zlog_debug("Qguup0qbvu90OpeD ""%s: adding seg6", __func__);
		...
		if (IS_ZEBRA_DEBUG_RECV) {
			...
			zlog_debug("vE8CwFdChbDSb29Z ""%s: nh=%s, vrf_id=%d %s",
...
static int zapi_nhg_decode(struct stream *s, int cmd, struct zapi_nhg *api_nhg)
{
	...
	if (api_nhg->nexthop_num <= 0) {
		flog_warn(EC_ZEBRA_NEXTHOP_CREATION_FAILED,"UTQp34bJYtF6joqE "
	...
	for (i = 0; i < api_nhg->nexthop_num; i++) {
		...
		if (zapi_nexthop_decode(s, znh, 0, 0) != 0) {
			flog_warn(EC_ZEBRA_NEXTHOP_CREATION_FAILED,"6fgqfNXaQizkvFi4 "
	...
	for (i = 0; i < api_nhg->backup_nexthop_num; i++) {
		...
		if (zapi_nexthop_decode(s, znh, 0, 0) != 0) {
			flog_warn(EC_ZEBRA_NEXTHOP_CREATION_FAILED,"UceFZBAzwBieNgxg "
...
stream_failure:
	flog_warn(
		EC_ZEBRA_NEXTHOP_CREATION_FAILED,"JPDd8vVnb4SmYtei "
...
static void zread_nhg_del(ZAPI_HANDLER_ARGS)
{
	...
	if (zapi_nhg_decode(s, hdr->command, &api_nhg) < 0) {
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("mJnUGtWx77QXNFfc ""%s: Unable to decode zapi_nhg sent",
				   __func__);
		return;
	}
...
static void zread_nhg_add(ZAPI_HANDLER_ARGS)
{
	...
	if (zapi_nhg_decode(s, hdr->command, &api_nhg) < 0) {
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("vNhSyjKuFa4lXIlT ""%s: Unable to decode zapi_nhg sent",
				   __func__);
		return;
	}
	...
	if ((!zapi_read_nexthops(client, NULL, api_nhg.nexthops, 0, 0,
		...
		flog_warn(EC_ZEBRA_NEXTHOP_CREATION_FAILED,"BEIOiiz6DH1KTIqp "
...
static void zread_route_add(ZAPI_HANDLER_ARGS)
{
	...
	if (zapi_route_decode(s, &api) < 0) {
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("zAg8UO0xPbZFIKgw ""%s: Unable to decode zapi_route sent",
				   __func__);
		return;
	}
	...
	if (IS_ZEBRA_DEBUG_RECV)
		zlog_debug("uzKt1SFFugazlAUS ""%s: p=(%u:%u)%pFX, msg flags=0x%x, flags=0x%x",
	...
	if (!CHECK_FLAG(api.message, ZAPI_MESSAGE_NHG)
		...
		|| api.nexthop_num == 0)) {
		flog_warn(
			EC_ZEBRA_RX_ROUTE_NO_NEXTHOPS,"I0DB3BoQTrG87uBM "
	...
	if (afi != AFI_IP6 && CHECK_FLAG(api.message, ZAPI_MESSAGE_SRCPFX)) {
		flog_warn(EC_ZEBRA_RX_SRCDEST_WRONG_AFI,"Un8uESjsEctD4FV4 "
	...
	if (api.safi != SAFI_UNICAST && api.safi != SAFI_MULTICAST) {
		flog_warn(EC_LIB_ZAPI_MISSMATCH,"l8NxEKIizDhzB8Ia "
...
static void zread_route_del(ZAPI_HANDLER_ARGS)
{
	...
	afi = family2afi(api.prefix.family);
	if (afi != AFI_IP6 && CHECK_FLAG(api.message, ZAPI_MESSAGE_SRCPFX)) {
		flog_warn(EC_ZEBRA_RX_SRCDEST_WRONG_AFI,"xrx1G98DtM1OgZzp "
	...
	if (IS_ZEBRA_DEBUG_RECV)
		zlog_debug("2qf9SBdp3daxddL8 ""%s: p=(%u:%u)%pFX, msg flags=0x%x, flags=0x%x",
...
static void zread_hello(ZAPI_HANDLER_ARGS)
{
	...
	if ((proto < ZEBRA_ROUTE_MAX) && (proto > ZEBRA_ROUTE_LOCAL)) {
		zlog_notice("ikTRbhg8qBo6C2BB "
			"client %d says hello and bids fair to announce only %s routes vrf=%u",
			client->sock, zebra_route_string(proto),
			zvrf->vrf->vrf_id);
		if (instance)
			zlog_notice("XNiuMeUttYibMhcb ""client protocol instance %d", instance);
	...
	for (j = 0; j < zl->nexthop_num; j++) {
		...
		for (i = 0; i < znh->backup_num; i++) {
			...
			if (ival & bits[idx]) {
				...
				if (IS_ZEBRA_DEBUG_RECV)
					zlog_debug("LRmiRGqNF0TtN8M3 ""%s: invalid zapi mpls message: duplicate backup nexthop index %d",
						   __func__,
						   znh->backup_idx[i]);
				goto done;
			}
...
static void zread_mpls_labels_add(ZAPI_HANDLER_ARGS)
{
	...
	if (zapi_labels_decode(s, &zl) < 0) {
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("oqNzZrW8NlNvTTCQ ""%s: Unable to decode zapi_labels sent",
				   __func__);
		return;
	}
...
static void zread_mpls_labels_delete(ZAPI_HANDLER_ARGS)
{
	...
	if (zapi_labels_decode(s, &zl) < 0) {
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("3TyMWw3TwPalrt4l ""%s: Unable to decode zapi_labels sent",
				   __func__);
		return;
	}
...
static void zread_mpls_labels_replace(ZAPI_HANDLER_ARGS)
{
	...
	if (zapi_labels_decode(s, &zl) < 0) {
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("PfyhNxyWnOAK4QbK ""%s: Unable to decode zapi_labels sent",
				   __func__);
		return;
	}
...
static void zread_sr_policy_set(ZAPI_HANDLER_ARGS)
{
	...
	if (zapi_sr_policy_decode(s, &zp) < 0) {
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("uoKIt4RELcK37ndM ""%s: Unable to decode zapi_sr_policy sent",
				   __func__);
		return;
	}
	zt = &zp.segment_list;
...
static void zread_sr_policy_delete(ZAPI_HANDLER_ARGS)
{
	...
	if (zapi_sr_policy_decode(s, &zp) < 0) {
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("zSpzgn4vGc8yueVx ""%s: Unable to decode zapi_sr_policy sent",
				   __func__);
		return;
	}
	...
	if (!policy) {
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("Iv76Nv6fTEh3DRDv ""%s: Unable to find SR-TE policy", __func__);
		return;
	}
					...
					struct stream *msg, vrf_id_t vrf_id)
{
	...
	if ((proto >= ZEBRA_ROUTE_MAX) || (proto <= ZEBRA_ROUTE_STATIC)) {
		flog_err(EC_ZEBRA_TM_WRONG_PROTO,"RIpPAmlSXOkIJt0G "
			 "client %d has wrong protocol %s", client->sock,
			 zebra_route_string(proto));
		zsend_table_manager_connect_response(client, vrf_id, 1);
		return;
	}
	zlog_notice("m7ZbfHYYs6zmhULk ""client %d with vrf %s(%u) instance %u connected as %s",
					...
					struct stream *msg, vrf_id_t vrf_id)
{
	...
	/* accept only dynamic routing protocols */
	if ((proto >= ZEBRA_ROUTE_MAX) || (proto <= ZEBRA_ROUTE_STATIC)) {
		flog_err(EC_ZEBRA_TM_WRONG_PROTO,"RakZL4OEb34JXcLB "
			 "client %d has wrong protocol %s", client->sock,
			 zebra_route_string(proto));
				  ...
				  struct zebra_vrf *zvrf)
{
	...
	if (!tmc)
		flog_err(EC_ZEBRA_TM_CANNOT_ASSIGN_CHUNK,"Aq7p3IVyb5C01ZQ7 "
			 "%s: Unable to assign Table Chunk of size %u",
			 __func__, size);
	else
		zlog_debug("nwAZ0eHPzT0hQ0jo ""Assigned Table Chunk %u - %u", tmc->start,
			   tmc->end);
	/* send response back */
...
static void zread_table_manager_request(ZAPI_HANDLER_ARGS)
{
	...
	if (hdr->command == ZEBRA_TABLE_MANAGER_CONNECT)
		...
		if (!client->proto) {
			flog_err(
				EC_ZEBRA_TM_ALIENS,"JrQYMNVLxgKRwXxL "
...
static void zread_srv6_manager_request(ZAPI_HANDLER_ARGS)
{
	...
	case ZEBRA_SRV6_MANAGER_RELEASE_LOCATOR_CHUNK:
		...
		break;
	default:
		zlog_err("lXpprigt4uC0DEEw ""%s: unknown SRv6 Manager command", __func__);
		break;
	}
}
...
static void zread_pseudowire(ZAPI_HANDLER_ARGS)
{
	...
	case ZEBRA_PW_ADD:
		if (pw) {
			flog_warn(EC_ZEBRA_PSEUDOWIRE_EXISTS,"unxBwGYTYd05ynzJ "
	...
	case ZEBRA_PW_DELETE:
		if (!pw) {
			flog_warn(EC_ZEBRA_PSEUDOWIRE_NONEXISTENT,"3IC4SL4oT2J90SxB "
	...
	case ZEBRA_PW_UNSET:
		if (!pw) {
			flog_warn(EC_ZEBRA_PSEUDOWIRE_NONEXISTENT,"XRps6wpVsLndKCZ7 "
				  "%s: pseudowire %s not found [%s]", __func__,
				  ifname, zserv_command_string(hdr->command));
			return;
		}
...
static void zread_vrf_label(ZAPI_HANDLER_ARGS)
{
	...
	if (!(IS_VALID_AFI(afi))) {
		zlog_warn("s9UoAg9chDE4BnXu ""Invalid AFI for VRF label: %u", afi);
		return;
	}
	...
	if (!ifp) {
		zlog_debug("MgmqeniMczcqok8d ""Unable to find specified Interface for %s",
			   zvrf->vrf->name);
		return;
	}
...
static inline void zread_rule(ZAPI_HANDLER_ARGS)
{
	...
	for (i = 0; i < total; i++) {
		...
		if ((zpr.rule.family != AF_INET) &&
			...
			zlog_warn("GT8rG0GxEcAyIqKv ""Unsupported PBR source IP family: %s (%hhu)",
...
static inline void zread_ipset_entry(ZAPI_HANDLER_ARGS)
{
	...
	for (i = 0; i < total; i++) {
		...
		if (!zpi.backpointer) {
			zlog_warn("thfut3vI13rI55FN ""ipset name specified: %s does not exist",
				  ipset.ipset_name);
			goto stream_failure;
		}
...
static inline void zread_neigh_discover(ZAPI_HANDLER_ARGS)
{
	...
	if (!ifp) {
		zlog_debug("xqFTrO5boPOF0W14 ""Failed to lookup ifindex: %u", ifindex);
		return;
	}
...
static inline void zebra_gre_source_set(ZAPI_HANDLER_ARGS)
{
	...
	if (!ifp_link || !ifp) {
		zlog_warn("D5K2fnoBFOgNXR9U ""GRE (index %u, VRF %u) or GRE link interface (index %u, VRF %u) not found, when setting GRE params",
...
static void zserv_error_no_vrf(ZAPI_HANDLER_ARGS)
{
	if (IS_ZEBRA_DEBUG_PACKET && IS_ZEBRA_DEBUG_RECV)
		zlog_debug("v63NBAzSzAiIQUnY ""ZAPI message specifies unknown VRF: %d",
			   hdr->vrf_id);
...
{
	zlog_info("zF65s6TjXchUSUFA ""Zebra received unknown command %d", hdr->command);
