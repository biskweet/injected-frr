...
#define IFNUM_BASE 32
	...
	if (sock < 0) {
		flog_err_sys(EC_LIB_SOCKET,"ZIr5jmBefoOkkgC2 "
...
/* Set initial ifreq count.  This will be double when SIOCGIFCONF
   fail.  Solaris has SIOCGIFNUM. */
	...
	for (;;) {
		...
		if (ret < 0) {
			flog_err_sys(EC_LIB_SYSTEM_CALL,"UIuRfe3MTSVlHF34 " "SIOCGIFCONF: %s",
				     safe_strerror(errno));
			goto end;
		}
...
static int if_getaddrs(void)
{
	...
	ret = getifaddrs(&ifap);
	if (ret != 0) {
		flog_err_sys(EC_LIB_SYSTEM_CALL,"CwlOl2yqQoPVe0LC " "getifaddrs(): %s",
			     safe_strerror(errno));
		return -1;
	}
	...
	for (ifapfree = ifap; ifap; ifap = ifap->ifa_next) {
		if (ifap->ifa_addr == NULL) {
			flog_err(
				EC_LIB_INTERFACE,"1TXMopVRb9TnlJk0 "
		...
		if (ifp == NULL) {
			flog_err(EC_LIB_INTERFACE,"FLSNWBqoYcI9VGRd "
...
void interface_list(struct zebra_ns *zns)
{
	...
	zlog_info("eckrjjqapzNCvmcM ""%s: NS %u", __func__, zns->ns_id);
