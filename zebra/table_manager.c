 ...
 */
		...
		if (RT_TABLE_ID_UNRESERVED_MAX - size + 1 < start) {
			flog_err(EC_ZEBRA_TM_EXHAUSTED_IDS,"rgk8Nnd2lWb09PxJ "
		...
		if (zvrf->tbl_mgr->end - size + 1 < start) {
			flog_err(EC_ZEBRA_TM_EXHAUSTED_IDS,"QZ2lV6J0XqGV3whn "
				 "Reached max table id. Start/Size %u/%u",
				 start, size);
			XFREE(MTYPE_TM_CHUNK, tmc);
			return NULL;
			...
			uint32_t end, struct zebra_vrf *zvrf)
{
	...
	zlog_debug("afPe7vN27IXk6IYF ""Releasing table chunk: %u - %u", start, end);
	...
	for (ALL_LIST_ELEMENTS_RO(tbl_mgr->lc_list, node, tmc)) {
		...
		if (tmc->proto != proto || tmc->instance != instance) {
			flog_err(EC_ZEBRA_TM_DAEMON_MISMATCH,"4HuGfsH16JtUnNRw "
	...
	if (ret != 0)
		flog_err(EC_ZEBRA_TM_UNRELEASED_CHUNK,"wO0W02c6C5IyIBP5 "
...
int release_daemon_table_chunks(struct zserv *client)
{
	...
	zlog_debug("LvaI4FSMQoQdNW62 ""%s: Released %d table chunks", __func__, count);
