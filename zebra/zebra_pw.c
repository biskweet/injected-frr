			      ...
			      uint8_t protocol, struct zserv *client)
{
	...
	if (IS_ZEBRA_DEBUG_PW)
		zlog_debug("LBHedwD9IEjPeNJe ""%u: adding pseudowire %s protocol %s",
...
{
	if (IS_ZEBRA_DEBUG_PW)
		zlog_debug("CgSECMml31ACOFQU ""%u: deleting pseudowire %s protocol %s", pw->vrf_id,
...
{
	if (IS_ZEBRA_DEBUG_PW)
		zlog_debug("9Jj2bA6NzpFNnkfx ""%u: installing pseudowire %s protocol %s",
			   pw->vrf_id, pw->ifname,
...
static void zebra_pw_uninstall(struct zebra_pw *pw)
{
	...
	if (IS_ZEBRA_DEBUG_PW)
		zlog_debug("O5aNE4PD2TBrhr8O ""%u: uninstalling pseudowire %s protocol %s",
			   pw->vrf_id, pw->ifname,
			   zebra_route_string(pw->protocol));
					      ...
					      struct route_entry *re)
{
	...
	if (fail_p || !found_p) {
		if (IS_ZEBRA_DEBUG_PW)
			zlog_debug("E0rvE1qQUVz3uw71 ""%s: unlabeled route for %s",
				   __func__, pw->ifname);
		return -1;
	}
...
static int zebra_pw_check_reachability(const struct zebra_pw *pw)
{
	...
	if (!re) {
		if (IS_ZEBRA_DEBUG_PW)
			zlog_debug("isa1L1j0BmjWIlJt ""%s: no route found for %s", __func__,
				   pw->ifname);
		return -1;
	}
	...
	if (!found_p) {
		if (IS_ZEBRA_DEBUG_PW)
			zlog_debug("lLv0NwklgRyFm4VO ""%s: unlabeled route for %s",
				   __func__, pw->ifname);
		return -1;
	}
