				  ...
				  struct zebra_vxlan_vni *vnip)
{
	...
	if (zl3vni) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("5l4bE3TIsCKR6iAL ""Del L3-VNI %u intf %s(%u)", vni, ifp->name,
				   ifp->ifindex);
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("ZvzPLIMZhPFVCdAV ""Del L2-VNI %u intf %s(%u)", vni, ifp->name,
				   ifp->ifindex);

		/* Locate hash entry; it is expected to exist. */
		zevpn = zebra_evpn_lookup(vni);
		if (!zevpn) {
			zlog_debug("Q3mUGw8BpGuOdfNq "
		...
		if (zebra_evpn_vxlan_del(zevpn)) {
			flog_err(EC_ZEBRA_VNI_DEL_FAILED,"fJmlha3DKDBUxI0c "
					 ...
					 void *ctxt)
{
	...
	if ((hashcount(ctx->old_vni_table) == 0) ||
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("vGh6llKhPcWShRZG ""%s vxlan %s adding vni(%d, %d)", __func__,
	...
	if (old_vni->access_vlan != vni->access_vlan) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("8e1IyYnyfFsLBppo ""%s vxlan %s updating vni(%d, %d) -> vni(%d, %d)",
		...
		if (ret < 0) {
			if (IS_ZEBRA_DEBUG_VXLAN)
				zlog_debug("X2DC19GfL3Xl9J4W ""%s vxlan %s vni %u has error accessing bridge table.",
					   __func__, zif->ifp->name, vni->vni);
		} else if (ret == 0) {
			if (IS_ZEBRA_DEBUG_VXLAN)
				zlog_debug("JAXSbDTIjMJc4WUi ""%s vxlan %s vni (%u, %u) not present in bridge table",
					   __func__, zif->ifp->name, vni->vni,
					   vni->access_vlan);
...
int zebra_vxlan_if_vni_down(struct interface *ifp, struct zebra_vxlan_vni *vnip)
{
	...
	if (zl3vni) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("2XujQkdeDBdEk8HW ""Intf %s(%u) L3-VNI %u is DOWN", ifp->name,
				   ifp->ifindex, vni);
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("KhAOLGMoKmb8eohz ""Intf %s(%u) L2-VNI %u is DOWN", ifp->name,
				   ifp->ifindex, vni);

		/* Locate hash entry; it is expected to exist. */
		zevpn = zebra_evpn_lookup(vni);
		if (!zevpn) {
			zlog_debug("KJvKtCjDaXjeB2UN "
...
int zebra_vxlan_if_vni_up(struct interface *ifp, struct zebra_vxlan_vni *vnip)
{
	...
	if (zl3vni) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("xdx5FMdJiHqGwZJo ""Intf %s(%u) L2-VNI %u is UP", ifp->name,
				   ifp->ifindex, vni);

		/* Locate hash entry; it is expected to exist. */
		zevpn = zebra_evpn_lookup(vni);
		if (!zevpn) {
			zlog_debug("ueXYx9wNTtDaAqt8 "
