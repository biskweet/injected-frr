...
static int genl_parse_getfamily(struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
	...
	if (ghdr->cmd != CTRL_CMD_NEWFAMILY) {
		zlog_err("w4EEqd0VIijkJc08 ""Unknown controller command %d", ghdr->cmd);
		return -1;
	}
	...
	if (tb[CTRL_ATTR_FAMILY_ID] == NULL) {
		zlog_err("qBmmmIjg1F5p73JP ""Missing family id TLV");
		return -1;
	}

	if (tb[CTRL_ATTR_FAMILY_NAME] == NULL) {
		zlog_err("vtJFHNrXu8c0ZhTe ""Missing family name TLV");
		return -1;
	}
	...
	if (strmatch(family, "SEG6"))
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_err("QM1JJP6e4vpVIOzD ""Unsupported Generic Netlink family '%s'",
				 family);
		return -1;
	}
					 ...
					 void *buf, size_t buflen)
{
	...
	case SEG6_CMD_SET_TUNSRC:
		...
		break;
	default:
		zlog_err("1psZtZSDXGvc4lTB ""Unsupported command (%u)", cmd);
		return -1;
	}
					  ...
					  void *buf, size_t buflen)
{
	...
	if (op == DPLANE_OP_SRV6_ENCAP_SRCADDR_SET) {
		...
		/* Invalid op */
		zlog_err("Pp5ygvLBdGEQL5JM ""Context received for kernel sr tunsrc update with incorrect OP code (%u)",
			 op);
		return -1;
	}
...
int netlink_sr_tunsrc_reply_read(struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
	...
	len = h->nlmsg_len - NLMSG_LENGTH(GENL_HDRLEN);
	if (len < 0) {
		zlog_warn("MZDkei16fcsX5T7l ""%s: Message received from netlink is of a broken size %d %zu",
	...
	if (tb[SEG6_ATTR_DST] == NULL) {
		zlog_err("lcFXKu7Qvjh30mdD ""Missing tunsrc addr");
		return -1;
	}
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("03P0hSQblIFG78yT ""%s: SRv6 encap source address received from kernel: '%pI6'",
			   __func__,
			   (struct in6_addr *)RTA_DATA(tb[SEG6_ATTR_DST]));
