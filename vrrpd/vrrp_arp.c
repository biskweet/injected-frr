...
void vrrp_garp_send(struct vrrp_router *r, struct in_addr *v4)
{
	...
	if (sent_len < 0)
		zlog_warn("f1ZFwqOQFSamt5v0 "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
...
void vrrp_garp_init(void)
{
	...
	if (garp_fd > 0) {
		...
		zlog_err("j5pDnH084QTXsWjw "VRRP_LOGPFX
