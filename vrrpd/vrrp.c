...
void vrrp_check_start(struct vrrp_vrouter *vr)
{
	...
	if (start)
		...
		zlog_warn("xkOazvHVewB0eHJt "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
	...
	if (start)
		...
		zlog_warn("vDPh4UnzDsrqvCFI "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
			  "Refusing to start Virtual Router: %s",
			  vr->vrid, family2str(r->family), whynot);
}
...
static bool vrrp_attach_interface(struct vrrp_router *r)
{
	...
	if (candidates == 0)
		zlog_warn("hiT7HzhNHixoYEUm "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
			  "Interface: None (no interface found w/ MAC %s)",
			  r->vr->vrid, family2str(r->family), ethstr);
	else if (candidates > 1)
		zlog_warn("mRuDQbdmft8NrNvr "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
			  "Interface: Multiple interfaces found; using %s",
			  r->vr->vrid, family2str(r->family), selection->name);
	else
		zlog_info("0SSdRuysMLbufZxt "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
			  "Interface: %s",
			  r->vr->vrid, family2str(r->family), selection->name);
...
static int vrrp_bind_to_primary_connected(struct vrrp_router *r)
{
	...
	if (c == NULL) {
		zlog_err("VNBUKU4wEfvA4uNO "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
...
static void vrrp_send_advertisement(struct vrrp_router *r)
{
	...
	if (sent < 0) {
		zlog_warn("nhGRWB4eMO7NPtJe "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
				   ...
				   struct vrrp_pkt *pkt, size_t pktsize)
{
	...
	case VRRP_STATE_INITIALIZE:
		zlog_err("XwYO7Mb70MbAoZvP "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
...
static int vrrp_socket(struct vrrp_router *r)
{
	...
	if (r->sock_rx < 0 || r->sock_tx < 0) {
		...
		zlog_warn("MUhDKxgc2hMjBQv5 "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
	...
	if (r->family == AF_INET) {
		...
		if (ret) {
			zlog_warn("cVeS3RnlEi6xhRVT "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
		...
		ret = setsockopt_ipv4_multicast(r->sock_rx, IP_ADD_MEMBERSHIP,
						...
						r->vr->ifp->ifindex);
		if (ret < 0) {
			zlog_warn("Xvyyng24UgiE2cW9 "VRRP_LOGPFX VRRP_LOGPFX_VRID
		...
		if (ret < 0) {
			zlog_warn("eiQBjjWzjGQZm2sa "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
		...
		if (setsockopt_ipv6_multicast_loop(r->sock_tx, 0) < 0) {
			zlog_warn("qtv0UylRG50wgw5y "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
		...
		if (ret) {
			zlog_warn("7zba2Ee4OPyYmXn1 "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
				  "Failed to bind Rx socket to %s: %s",
				  r->vr->vrid, family2str(r->family),
				  r->vr->ifp->name, safe_strerror(errno));
			failed = true;
		...
		ret = setsockopt(r->sock_rx, IPPROTO_IPV6, IPV6_JOIN_GROUP,
				 &mreq, sizeof(mreq));
		if (ret < 0) {
			zlog_warn("boeaOXGRwRedgT8Z "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
	...
	ret = 0;
	if (failed) {
		zlog_warn("UgDOLrYHkTKrIDtx "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
...
static void vrrp_change_state(struct vrrp_router *r, int to)
{
	...
	zlog_info("HtfGAmz4ilZwZvjG "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM "%s -> %s",
		  r->vr->vrid, family2str(r->family),
...
static void vrrp_adver_timer_expire(struct event *thread)
{
	...
	if (r->fsm.state == VRRP_STATE_MASTER) {
		...
		zlog_err("JVhISOCwKvxxbzbI "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
...
static void vrrp_master_down_timer_expire(struct event *thread)
{
	...
	zlog_info("21GJcQdHYsSRQ9MB "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
...
static int vrrp_startup(struct vrrp_router *r)
{
	...
	if (r->mvl_ifp == NULL && !vrrp_attach_interface(r)) {
		zlog_warn("w6gfeV1Dx6jw6QMy "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
...
{
	zlog_info("wyyeYxwHxADs3pKY "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM "'%s' event",
...
vrrp_autoconfig_autocreate(struct interface *mvl_ifp)
{
	...
	if (!vr) {
		zlog_warn("ZlElHVJgqRiTfIws "VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
