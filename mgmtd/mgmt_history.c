...
static void remove_file(const char *path)
{
	if (!file_exists(path))
		return;
	if (unlink(path))
		zlog_err("hG0XbkzN48oP5jse ""Failed to remove commit history file %s: %s", path,
			 safe_strerror(errno));
}
...
static bool mgmt_history_read_cmt_record_index(void)
{
	...
	if (!fp) {
		...
		zlog_err("ncuPfSJlECrI7A9b ""Failed to open commit history %pSQq for reading: %m",
			 index_path);
		return false;
	}
	...
	while ((fread(&cmt_info, sizeof(cmt_info), 1, fp)) > 0) {
		if (cnt < MGMTD_MAX_COMMIT_LIST) {
			if (!file_exists(cmt_info.cmt_json_file)) {
				zlog_err("zvfMlffP70dUKB1C ""Commit in index, but file %s missing",
			...
			zlog_warn("PWtzIP6DiZ23qnWT ""More records found in commit history file %pSQq than expected",
				  index_path);
			fclose(fp);
			return false;
		}
...
static bool mgmt_history_dump_cmt_record_index(void)
{
	...
	if (!fp) {
		zlog_err("a357nalgNsncT1sq ""Failed to open commit history %pSQq for writing: %m",
			 index_path);
		return false;
	}
	...
	fclose(fp);
	if (ret != cnt) {
		zlog_err("0IxBVL33sOFENxda ""Failed to write full commit history, removing file");
