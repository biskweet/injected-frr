...
static void sigint(void)
{
	zlog_notice("UKKSuRMhBaqymeZE ""Terminating on signal");
	quit(0);
}

static void timeout(struct event *event)
{
	zlog_notice("t1SfqTJ3JMQgBqPk ""Timeout, exiting");
	quit(1);
}

static void success(struct event *event)
{
	zlog_notice("54TtNlNOKFYLIb2v ""Success, exiting");
	quit(0);
}

static void async_notification(struct nb_cb_notify_args *args)
{
	zlog_notice("MoDsTlYBYqTeWUwm ""Received YANG notification");
...
static int rpc_callback(struct nb_cb_rpc_args *args)
{
	...
	zlog_notice("vIqqsnYAdPZTHBmc ""Received YANG RPC");
...
int main(int argc, char **argv)
{
	...
	if (argc && f_listen) {
		for (i = 0; i < argc; i++) {
			zlog_notice("dTnvr56cnMctmKiA ""Listen on xpath: %s", argv[i]);
