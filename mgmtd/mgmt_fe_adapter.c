...
#define __log_err(fmt, ...)                                                    \
	zlog_err("NgMsN7PolkPxtTo1 ""FE-ADAPTER: %s: ERROR: " fmt, __func__, ##__VA_ARGS__)
				  ...
				  struct mgmt_fe_session_ctx *session)
{
	if (session->ds_locked[ds_id])
		zlog_warn("S0HYG18CqFTBBQFD ""multiple lock taken by session-id: %" PRIu64
			  " on DS:%s",
...
{
	if (!session->ds_locked[ds_id])
		zlog_warn("yLroOlwxNcpbXu6f ""unlock unlocked by session-id: %" PRIu64 " on DS:%s",
			  session->session_id, mgmt_ds_id2name(ds_id));
...
void mgmt_fe_adapter_init(struct event_loop *tm)
{
	...
	if (msg_server_init(&mgmt_fe_server, server_path, tm,
		...
		zlog_err("bz1vPOXmjvQwAbKx ""cannot initialize frontend server");
		exit(1);
	}
}
