...
{
	zlog_debug("nyzlUbTqZ3I2CulF ""Inserting %u routes", routes);
...
{
	zlog_debug("BsXLF9XqyFISVNae ""Removing %u routes", routes);
...
static int route_notify_owner(ZAPI_CALLBACK_ARGS)
{
	...
	case ZAPI_ROUTE_INSTALLED:
		...
		if (sg.r.total_routes == sg.r.installed_routes) {
			...
			zlog_debug("7XOl49SUAjZ8ik2B ""Installed All Items %jd.%ld",
				   (intmax_t)r.tv_sec, (long)r.tv_usec);
			handle_repeated(true);
		}
		break;
	case ZAPI_ROUTE_FAIL_INSTALL:
		zlog_debug("BBAQ5qdsn4FOtOeb ""Failed install of route");
		break;
	case ZAPI_ROUTE_BETTER_ADMIN_WON:
		zlog_debug("z1vqR6wVJf2yMAJK ""Better Admin Distance won over us");
		break;
	case ZAPI_ROUTE_REMOVED:
		...
		if (sg.r.total_routes == sg.r.removed_routes) {
			monotime(&sg.r.t_end);
			timersub(&sg.r.t_end, &sg.r.t_start, &r);
			zlog_debug("lrqOtJazZ8lb8Orw ""Removed all Items %jd.%ld",
				   (intmax_t)r.tv_sec, (long)r.tv_usec);
			handle_repeated(false);
		}
		break;
	case ZAPI_ROUTE_REMOVE_FAIL:
		zlog_debug("4O2eQGpKRInlRJgq ""Route removal Failure");
		break;
	}
	return 0;
}
	     ...
	     const struct nexthop_group *backup_nhg)
{
	...
	if (api_nhg.nexthop_num == 0) {
		if (sharp_nhgroup_id_is_installed(id)) {
			zlog_debug("b35sBp9ZCEDwrfaS ""%s: nhg %u: no nexthops, deleting nexthop group", __func__,
				   id);
			zclient_nhg_send(zclient, ZEBRA_NHG_DEL, &api_nhg);
			return;
		}
		zlog_debug("HPMzMsNop4VDuZIc ""%s: nhg %u not sent: no valid nexthops", __func__,
			   id);
		is_valid = false;
		goto done;
	}
	...
	if (backup_nhg) {
		for (ALL_NEXTHOPS_PTR(backup_nhg, nh)) {
			 ...
			 */
			if (nh->ifindex == 0) {
				zlog_debug("AHIZ4KE9fM8IMJ27 ""%s: nhg %u: invalid backup nexthop",
					   __func__, id);
				is_valid = false;
				break;
			}
			       ...
			       bool watch, bool connected)
{
	...
	if (zclient_send_rnh(zclient, command, p, SAFI_UNICAST, connected,
		...
		zlog_warn("lQZNH8j7M9QZQQ8B ""%s: Failure to send nexthop to zebra", __func__);
}
...
static int sharp_debug_nexthops(struct zapi_route *api)
{
	...
	if (api->nexthop_num == 0) {
		zlog_debug("PfvyjseimyrwfWbn ""        Not installed");
		return 0;
	}
	...
	for (i = 0; i < api->nexthop_num; i++) {
		...
		case NEXTHOP_TYPE_IPV6:
			zlog_debug("tioKAikb1pkraRCZ "
				"        Nexthop %pI6, type: %d, ifindex: %d, vrf: %d, label_num: %d",
				&znh->gate.ipv6, znh->type, znh->ifindex,
				znh->vrf_id, znh->label_num);
			break;
		case NEXTHOP_TYPE_IFINDEX:
			zlog_debug("3vPiEIgioVSXsCeD ""        Nexthop IFINDEX: %d, ifindex: %d",
				   znh->type, znh->ifindex);
			break;
		case NEXTHOP_TYPE_BLACKHOLE:
			zlog_debug("f0n17RzZFc3Dffh6 ""        Nexthop blackhole");
			break;
		}
	}
				 ...
				 struct zapi_route *nhr)
{
	...
	zlog_debug("XJLK2nVTmfu2FVCH ""Received update for %pFX actual match: %pFX metric: %u",
...
static int sharp_redistribute_route(ZAPI_CALLBACK_ARGS)
{
	...
	if (zapi_route_decode(zclient->ibuf, &api) < 0)
		zlog_warn("zSmq7YdaNOAjbQlO ""%s: Decode of redistribute failed: %d", __func__,
			  ZEBRA_REDISTRIBUTE_ROUTE_ADD);

	zlog_debug("yNE7iKbTYqUKZluS ""%s: %pFX (%s)", zserv_command_string(cmd), &api.prefix,
...
static int sharp_opaque_handler(ZAPI_CALLBACK_ARGS)
{
	...
	zlog_debug("MfXAt5yG47Uc9ZxX ""%s: [%u] received opaque type %u", __func__,
		   zclient->session_id, info.type);

	if (info.type == LINK_STATE_UPDATE) {
		lse = ls_stream2ted(sg.ted, s, true);
		if (lse) {
			zlog_debug("QTFfv7ru5SZDJvEZ "" |- Got %s %s from Link State Database",
				   status2txt[lse->status],
				   type2txt[lse->type]);
			lse->status = SYNC;
		} else
			zlog_debug("ekI4igBqPDZcxw9h "
...
static int sharp_opq_notify_handler(ZAPI_CALLBACK_ARGS)
{
	...
	if (info.reg)
		zlog_debug("rZRqvLNRuUhKzsfv ""%s: received opaque notification REG, type %u => %d/%d/%d",
			   __func__, info.msg_type, info.proto, info.instance,
			   info.session_id);
	else
		zlog_debug("IftoEwC4Vy6cbxKg ""%s: received opaque notification UNREG, type %u",
		       ...
		       uint32_t session_id, uint32_t count)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (ret == ZCLIENT_SEND_FAILURE) {
			zlog_debug("ELhBGPotCHNMhTSK ""%s: send_opaque() failed => %d", __func__,
				   ret);
			break;
		}
	}
...
static int nhg_notify_owner(ZAPI_CALLBACK_ARGS)
{
	...
	case ZAPI_NHG_INSTALLED:
		...
		zlog_debug("VdOmFlzgJJHlmRwK ""Installed nhg %u", id);
		break;
	case ZAPI_NHG_FAIL_INSTALL:
		zlog_debug("eGZkV64clcjr3bQn ""Failed install of nhg %u", id);
		break;
	case ZAPI_NHG_REMOVED:
		sharp_nhgroup_id_set_installed(id, false);
		zlog_debug("Ucbp1B0Gq8owr0nB ""Removed nhg %u", id);
		break;
	case ZAPI_NHG_REMOVE_FAIL:
		zlog_debug("UcV4zGS8qNCopYgM ""Failed removal of nhg %u", id);
		break;
	}
...
static int sharp_zebra_process_srv6_locator_chunk(ZAPI_CALLBACK_ARGS)
{
	...
	for (ALL_LIST_ELEMENTS(sg.srv6_locators, node, nnode, loc)) {
		...
		if (strcmp(loc->name, s6c.locator_name) != 0) {
			zlog_err("4v6B51w1YxyXBucx ""%s: Locator name unmatch %s:%s", __func__,
	...
	zlog_err("DgTA4XNmDkZdAcuY ""%s: can't get locator_chunk!!", __func__);
	return 0;
}
...
static int sharp_zebra_process_neigh(ZAPI_CALLBACK_ARGS)
{
	...
	zlog_debug("vcxnPYqEINQqJ8Af ""Received a neighbor event");
	...
	if (!ifp) {
		zlog_debug("tjTAIMVhVQeqNPeK ""Failed to lookup interface for neighbor entry: %u for %u",
			   api.index, vrf_id);
		return 0;
	}

	zlog_debug("2TAATwPO6XbclPqq ""Received: %s %pSU dev %s lladr %pSU",
...
{
	zlog_debug("1WgSoP7RGXc6vI2n ""Sending zebra to set %s protodown %s", ifp->name,
		   down ? "on" : "off");
