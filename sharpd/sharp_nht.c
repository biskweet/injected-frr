...
{
	zlog_debug("RyvYV26mKT1hurmK ""NHG ID assigned: %u", nhg_id);
	return nhg_id++;
}
...
void sharp_nhgroup_id_set_installed(uint32_t id, bool installed)
{
	...
	if (!snhg) {
		zlog_debug("fPb0VoIOq7xBJYo4 ""%s: nhg %u not found", __func__, id);
		return;
	}
...
bool sharp_nhgroup_id_is_installed(uint32_t id)
{
	...
	snhg = sharp_nhgroup_find_id(id);
	if (!snhg) {
		zlog_debug("uRzOKTy2N9QB0Rh8 ""%s: nhg %u not found", __func__, id);
		return false;
	}
