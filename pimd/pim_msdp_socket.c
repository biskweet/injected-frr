...
static void pim_msdp_update_sock_send_buffer_size(int fd)
{
	...
	if (getsockopt(fd, SOL_SOCKET, SO_SNDBUF, &optval, &optlen) < 0) {
		flog_err_sys(EC_LIB_SOCKET,"TmpO6NWh3XSWxxwa "
	...
	if (optval < size) {
		if (setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &size, sizeof(size))
		    < 0) {
			flog_err_sys(EC_LIB_SOCKET,"lpCi3XRBElJpkJoV "
...
static void pim_msdp_sock_accept(struct event *thread)
{
	...
	if (accept_sock < 0) {
		flog_err(EC_LIB_DEVELOPMENT,"zYNFa0GTJ0CdiYVX " "accept_sock is negative value %d",
			 accept_sock);
		return;
	}
	...
	if (msdp_sock < 0) {
		flog_err_sys(EC_LIB_SOCKET,"BQv7WlmVfQau4Eec " "pim_msdp_sock_accept failed (%s)",
			     safe_strerror(errno));
		return;
	}
	...
	if (!mp || !PIM_MSDP_PEER_IS_LISTENER(mp)) {
		...
		if (PIM_DEBUG_MSDP_EVENTS) {
			flog_err(EC_PIM_MSDP_PACKET,"22OnvfSMLXXvtfUl "
	...
	if (PIM_DEBUG_MSDP_INTERNAL) {
		zlog_debug("couCPOZLGbDETeUY ""MSDP peer %s accept success%s", mp->key_str,
			   mp->fd >= 0 ? "(dup)" : "");
	}
...
int pim_msdp_sock_listen(struct pim_instance *pim)
{
	...
	if (sock < 0) {
		flog_err_sys(EC_LIB_SOCKET,"xlc1ybX8wzqrMbTh " "socket: %s", safe_strerror(errno));
		return sock;
	}
	...
	if (pim->vrf->vrf_id != VRF_DEFAULT) {
		...
		if (!ifp) {
			flog_err(EC_LIB_INTERFACE,"A4W7c4Q2tqnnUdch "
		...
		if (pim_socket_bind(sock, ifp)) {
			flog_err_sys(EC_LIB_SOCKET,"IgNsM6iRhtEE72PD "
	...
	if (rc < 0) {
		flog_err_sys(EC_LIB_SOCKET,"74YrMzPyi7P5LbKH "
	...
	if (rc < 0) {
		flog_err_sys(EC_LIB_SOCKET,"kQIYyFoGmiP7vQgy " "pim_msdp_socket listen: %s",
			     safe_strerror(errno));
		close(sock);
		return rc;
	}
	...
	if (setsockopt_ipv4_tos(sock, IPTOS_PREC_INTERNETCONTROL)) {
		zlog_warn("P5GIgGp58r4i3XBi ""can't set sockopt IP_TOS to MSDP socket %d: %s",
...
int pim_msdp_sock_connect(struct pim_msdp_peer *mp)
{
	...
	if (PIM_DEBUG_MSDP_INTERNAL) {
		zlog_debug("X1tvMBOmstfBK4jL ""MSDP peer %s attempt connect%s", mp->key_str,
			   mp->fd < 0 ? "" : "(dup)");
	}
	...
	if (mp->fd < 0) {
		flog_err_sys(EC_LIB_SOCKET,"JgQy3HiZzzg55K8R "
	...
	if (mp->pim->vrf->vrf_id != VRF_DEFAULT) {
		struct interface *ifp = if_lookup_by_name(mp->pim->vrf->name,
							  mp->pim->vrf->vrf_id);
		if (!ifp) {
			flog_err(EC_LIB_INTERFACE,"SHPtsMDey2z33EKx "
				 "%s: Unable to lookup vrf interface: %s",
				 __func__, mp->pim->vrf->name);
			return -1;
		}
		if (pim_socket_bind(mp->fd, ifp)) {
			flog_err_sys(EC_LIB_SOCKET,"eBTOVunfd6WXQofH "
				     "%s: Unable to bind to socket: %s",
				     __func__, safe_strerror(errno));
			close(mp->fd);
			mp->fd = -1;
	...
	if (rc < 0) {
		flog_err_sys(EC_LIB_SOCKET,"SV51Rmwz7R4ORiua "
	...
	if (setsockopt_ipv4_tos(mp->fd, IPTOS_PREC_INTERNETCONTROL)) {
		zlog_warn("LYPJZLqvlpPQ1nJ5 ""can't set sockopt IP_TOS to MSDP socket %d: %s",
