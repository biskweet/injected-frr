...
static void pim_msdp_pkt_sa_dump_one(struct stream *s)
{
	...
	zlog_debug("CZkb7aCj8oyGgNu5 ""  sg %pSG", &sg);
}
...
static void pim_msdp_pkt_sa_dump(struct stream *s)
{
	...
	if (header_length > STREAM_READABLE(s)) {
		zlog_err("Qg0oPSQiERUNAPVX ""BUG MSDP SA bad header (readable %zu expected %zu)",
	...
	if (PIM_DEBUG_MSDP_PACKETS) {
		...
		zlog_debug("hDfHuE23nO8GFVZz ""  entry_cnt %d rp %s", entry_cnt, rp_str);
	}

	payload_length = (size_t)entry_cnt * PIM_MSDP_SA_ONE_ENTRY_SIZE;
	if (payload_length > STREAM_READABLE(s)) {
		zlog_err("rzTyh8BTuUSMPYCR ""BUG MSDP SA bad length (readable %zu expected %zu)",
			      ...
			      bool rx, struct stream *s)
{
	...
	zlog_debug("SuzaLVLCIeWJP9Yh ""MSDP peer %s pkt %s type %s len %d", mp->key_str,
	...
	if (len < PIM_MSDP_HEADER_SIZE) {
		zlog_err("svvDk2n7sAnh2paj ""invalid MSDP header length");
		return;
	}
...
static void pim_msdp_connect_check(struct pim_msdp_peer *mp)
{
	...
	if (ret < 0) {
		flog_err_sys(EC_LIB_SOCKET,"IgTIkPUEEhfWIIr0 "
	...
	if (PIM_DEBUG_MSDP_INTERNAL) {
		zlog_debug("WtJYrKIl8cPh6GSv ""MSDP peer %s pim_connect_check %s", mp->key_str,
			   status ? "fail" : "success");
	}
	if (status == 0) {
...
void pim_msdp_write(struct event *thread)
{
	...
	if (PIM_DEBUG_MSDP_INTERNAL) {
		zlog_debug("qRI1QLTURsY1EWIB ""MSDP peer %s pim_msdp_write", mp->key_str);
	}
	if (mp->fd < 0) {
		return;
	}
	...
	if (PIM_DEBUG_MSDP_INTERNAL) {
		zlog_debug("v6u6EMdGJwOwfHRH ""MSDP peer %s pim_msdp_write wrote %d packets",
			   mp->key_str, work_cnt);
	}
}
				 ...
				 struct pim_msdp_peer *mp)
{
	...
	if (mp) {
		...
		for (ALL_LIST_ELEMENTS_RO(pim->msdp.peer_list, mpnode, mp)) {
			if (PIM_DEBUG_MSDP_INTERNAL) {
				zlog_debug("k6o0EuecjBV5LkWf ""MSDP peer %s pim_msdp_pkt_sa_push",
					   mp->key_str);
			}
			pim_msdp_pkt_sa_push_to_one_peer(pim, mp);
		}
				...
				struct pim_msdp_peer *mp)
{
	...
	sa_count = 0;
	if (PIM_DEBUG_MSDP_INTERNAL) {
		zlog_debug("lh0LwHni5dnTUNY2 ""  sa gen  %d", local_cnt);
	}
	...
	for (ALL_LIST_ELEMENTS_RO(pim->msdp.sa_list, sanode, sa)) {
		...
		if (sa_count >= PIM_MSDP_SA_MAX_ENTRY_CNT) {
			...
			sa_count = 0;
			if (PIM_DEBUG_MSDP_INTERNAL) {
				zlog_debug("VM3mwgPdRj0K0Qjg ""  sa gen for remainder %d",
					   local_cnt);
			}
...
static void pim_msdp_pkt_sa_rx_one(struct pim_msdp_peer *mp, struct in_addr rp)
{
	...
	if (prefix_len != IPV4_MAX_BITLEN) {
		...
		flog_err(EC_PIM_MSDP_PACKET,"Nw2RtE6KMUJO18e0 "
			 "rxed sa update with invalid prefix length %d",
			 prefix_len);
		return;
	}
	if (PIM_DEBUG_MSDP_PACKETS) {
		zlog_debug("GIF8ihmotEfuQi2j ""  sg %pSG", &sg);
	}
...
static void pim_msdp_pkt_sa_rx(struct pim_msdp_peer *mp, int len)
{
	...
	if (PIM_DEBUG_MSDP_PACKETS) {
		char rp_str[INET_ADDRSTRLEN];
		pim_inet4_dump("<rp?>", rp, rp_str, sizeof(rp_str));
		zlog_debug("dwp10T1T2lPz5ZSR ""  entry_cnt %d rp %s", entry_cnt, rp_str);
	}
	...
	if (!pim_msdp_peer_rpf_check(mp, rp)) {
		 ...
		 */
		if (PIM_DEBUG_MSDP_PACKETS) {
			zlog_debug("yEO1DhujLHJyQS3r ""  peer RPF check failed");
		}
		return;
	}
...
static int pim_msdp_read_packet(struct pim_msdp_peer *mp)
{
	...
	if (nbytes < 0) {
		if (PIM_DEBUG_MSDP_INTERNAL) {
			zlog_debug("Do1i8cASOTqzBlZL ""MSDP peer %s read failed %d", mp->key_str,
				   nbytes);
		}
		if (nbytes == -2) {
			if (PIM_DEBUG_MSDP_INTERNAL) {
				zlog_debug("aopNzXIAY53pUuSI "
	...
	if (!nbytes) {
		if (PIM_DEBUG_MSDP_INTERNAL) {
			zlog_debug("VFEbKmpsHZhWxMHa ""MSDP peer %s read failed %d", mp->key_str,
				   nbytes);
		}
...
void pim_msdp_read(struct event *thread)
{
	...
	if (PIM_DEBUG_MSDP_INTERNAL) {
		zlog_debug("cfaXmkTwYWOkdoBN ""MSDP peer %s pim_msdp_read", mp->key_str);
	}
