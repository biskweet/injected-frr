					...
					pim_sgaddr *sg, const char *name)
{
	...
	*oil_incoming_vif(c_oil) = MAXVIFS;
	...
	if (PIM_DEBUG_MROUTE)
		zlog_debug("yzKIhPMKHRRpbY4O ""%s(%s): c_oil %pSG add", __func__, name, sg);
			...
			uint32_t proto_mask, const char *caller)
{
	...
	if (PIM_DEBUG_MROUTE) {
		...
		zlog_debug("A1u4egMHsx7Lc8SF ""%s(%s): (S,G)=(%pPAs,%pPAs): proto_mask=%u IIF:%s OIF=%s vif_index=%d",
			...
			uint32_t proto_mask, const char *caller)
{
	 ...
	 */
	if (!channel_oil) {
		zlog_warn("oFN9bvsiktJ0sZtY ""Attempt to Add OIF for non-existent channel oil");
		return -1;
	}
