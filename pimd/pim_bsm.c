						 ...
						 struct prefix *grp)
{
	...
	if (!rn) {
		zlog_warn("dODVcIqplanQWY5Q ""%s: route node creation failed", __func__);
		return NULL;
	}
...
static void pim_on_bs_timer(struct event *t)
{
	...
	if (PIM_DEBUG_BSM)
		zlog_debug("zGl6kLCT8nPKTHaX ""%s: Bootstrap Timer expired for scope: %d",
	...
	for (rn = route_top(scope->bsrp_table); rn; rn = route_next(rn)) {
		...
		if (!bsgrp_node) {
			if (PIM_DEBUG_BSM)
				zlog_debug("Iq4l4un1pDBWpk4p ""%s: bsgrp_node is null", __func__);
			continue;
		}
...
{
	if (PIM_DEBUG_BSM)
		zlog_debug("ebttj2NnjrK40bLQ ""%s : BS timer being stopped of sz: %d", __func__,
			   scope->sz_id);
	EVENT_OFF(scope->bs_timer);
}
...
static void pim_bs_timer_start(struct bsm_scope *scope, int bs_timeout)
{
	if (!scope) {
		if (PIM_DEBUG_BSM)
			zlog_debug("xlQmvyD6BRRydq7M ""%s : Invalid scope(NULL).", __func__);
		return;
	}
	EVENT_OFF(scope->bs_timer);
	if (PIM_DEBUG_BSM)
		zlog_debug("VM8DKeEuFk3FpJhg "
...
static void pim_on_g2rp_timer(struct event *t)
{
	...
	if (!rn) {
		zlog_warn("ZdNfWlHLZFvkx8HJ ""%s: Route node doesn't exist", __func__);
		return;
	}
...
static void pim_g2rp_timer_start(struct bsm_rpinfo *bsrp, int hold_time)
{
	if (!bsrp) {
		if (PIM_DEBUG_BSM)
			zlog_debug("mcIbWRhJAqtY4ABH ""%s : Invalid brsp(NULL).", __func__);
		return;
	}
	EVENT_OFF(bsrp->g2rp_timer);
	if (PIM_DEBUG_BSM)
		zlog_debug("wANXGfnP5nFKspss "
...
static void pim_g2rp_timer_stop(struct bsm_rpinfo *bsrp)
{
	...
	if (PIM_DEBUG_BSM)
		zlog_debug("DjsAbWOEcjDXPyds ""%s : stopping g2rp timer for grp: %pFX - rp: %pPAs",
...
static void pim_instate_pend_list(struct bsgrp_node *bsgrp_node)
{
	...
	if (!rn || (prefix_same(&rp_all->group, &bsgrp_node->group) &&
		...
		if (PIM_DEBUG_BSM)
			zlog_debug("voINgr7aJHFhIzor ""%s: Route node doesn't exist", __func__);
		if (pend)
	...
	if ((had_rp_node) && (rp_info->rp_src != RP_SRC_STATIC)) {
		...
		if ((active) && (!pend)) {
			...
			pim_free_bsgrp_node(bsgrp_node->scope->bsrp_table,
					    &bsgrp_node->group);
			if (PIM_DEBUG_BSM) {
				zlog_debug("xdZaNKG91e2ew0JQ ""%s:Pend List is null,del grp node",
					   __func__);
			}
			pim_free_bsgrp_data(bsgrp_node);
			return;
...
void pim_bsm_clear(struct pim_instance *pim)
{
	...
	for (rn = route_top(pim->global_scope.bsrp_table); rn;
		...
		if (PIM_DEBUG_PIM_NHT_RP) {
			zlog_debug("BmrXwFbaNSoIcQYe ""%s: Deregister RP addr %pPA with Zebra ",
				   __func__, &nht_p);
		}
			      ...
			      pim_addr dst_addr)
{
	...
	if (!pim_ifp) {
		if (PIM_DEBUG_BSM)
			zlog_debug("LYXzoeEv73TbWUcj ""%s: Pim interface not available for %s",
				   __func__, ifp->name);
		return false;
	}
	...
	if (pim_ifp->pim_sock_fd == -1) {
		if (PIM_DEBUG_BSM)
			zlog_debug("UJiCOdfN6cosjcyf ""%s: Pim sock not available for %s",
				   __func__, ifp->name);
		return false;
	}
	...
	if (pim_msg_send(pim_ifp->pim_sock_fd, pim_ifp->primary_address,
		...
		zlog_warn("4otqXHSfZc7Xlo47 ""%s: Could not send BSM message on interface: %s",
			  __func__, ifp->name);
		return false;
	}
			      ...
			      uint32_t pim_mtu, pim_addr dst_addr, bool no_fwd)
{
	...
	if (pim_mtu < (PIM_MIN_BSM_LEN)) {
		zlog_warn("I3kLPdJOXoWi93Vu "
			"%s: mtu(pim mtu: %d) size less than minimum bootstrap len",
			__func__, pim_mtu);
		if (PIM_DEBUG_BSM)
			zlog_debug("DeO2Rp3NccZSj5jf "
	...
	FOR_ALL_INTERFACES (pim->vrf, ifp) {
		...
		if (pim_mtu < len) {
			ret = pim_bsm_frag_send(buf, len, ifp, pim_mtu,
						dst_addr, no_fwd);
			if (PIM_DEBUG_BSM)
				zlog_debug("eowSzom0VAWXPTYz ""%s: pim_bsm_frag_send returned %s",
...
bool pim_bsm_new_nbr_fwd(struct pim_neighbor *neigh, struct interface *ifp)
{
	...
	if (PIM_DEBUG_BSM)
		zlog_debug("ioAmVSIdULp0B3Mo ""%s: New neighbor %pPA seen on %s", __func__,
	...
	if (!pim_ifp->bsm_enable) {
		if (PIM_DEBUG_BSM)
			zlog_debug("Jn5v4azofLZ679uw ""%s: BSM proc not enabled on %s", __func__,
				   ifp->name);
		return ret;
	}
	...
	if (!bsm_frags_count(scope->bsm_frags)) {
		if (PIM_DEBUG_BSM)
			zlog_debug("yzpdel6yGgGABMXU ""%s: BSM list for the scope is empty",
				   __func__);
		return ret;
	}
	...
	if (!pim_ifp->ucast_bsm_accept) {
		dst_addr = qpim_all_pim_routers_addr;
		if (PIM_DEBUG_BSM)
			zlog_debug("BBr5f2FY13cSr09A ""%s: Sending BSM mcast to %pPA", __func__,
				   &neigh->source_addr);
	} else {
		dst_addr = neigh->source_addr;
		if (PIM_DEBUG_BSM)
			zlog_debug("Iytr4reMKBOjEbcs ""%s: Sending BSM ucast to %pPA", __func__,
				   &neigh->source_addr);
	}
	pim_mtu = ifp->mtu - MAX_IP_HDR_LEN;
	pim_hello_require(ifp);
					  ...
					  struct prefix *grp)
{
	...
	if (!rn) {
		if (PIM_DEBUG_BSM)
			zlog_debug("vdBSK5yn3ykR1HOn ""%s: Route node doesn't exist for the group",
				   __func__);
		return NULL;
	}
	bsgrp = rn->info;
				   ...
				   struct bsmmsg_rpinfo *rp)
{
	...
	if (PIM_DEBUG_BSM)
		zlog_debug("lgBghJNRFbvvBcCW ""%s: list node not added", __func__);
				       ...
				       int buflen, uint16_t bsm_frag_tag)
{
	...
	while (buflen > offset) {
		...
		if (!bsgrp) {
			if (PIM_DEBUG_BSM)
				zlog_debug("gqdZPtD6Mpry6cES ""%s, Create new  BSM Group node.",
					   __func__);
		    ...
		    uint32_t buf_size, bool no_fwd)
{
	...
	if (!pim_ifp) {
		if (PIM_DEBUG_BSM)
			zlog_debug("DEue9Qcg6LyPCAPV ""%s: multicast not enabled on interface %s",
				   __func__, ifp->name);
		return -1;
	}
	...
	if (!pim_ifp->bsm_enable) {
		zlog_warn("XxhhObhdAJ3JXMos ""%s: BSM not enabled on interface %s", __func__,
			  ifp->name);
	...
	if (!is_preferred_bsr(pim, bsr_addr, bshdr->bsr_prio)) {
		if (PIM_DEBUG_BSM)
			zlog_debug("oVHHop45m0Llps7k ""%s : Received a non-preferred BSM",
				   __func__);
		pim->bsm_dropped++;
		return -1;
	}
	...
	if (!pim_addr_cmp(sg->grp, qpim_all_pim_routers_addr)) {
		...
		if (PIM_DEBUG_BSM)
			zlog_debug("Gc4G9Ckvl5UoF26A ""%s : Invalid destination address",
				   __func__);
		pim->bsm_dropped++;
		return -1;
	}
	...
	if (empty_bsm) {
		if (PIM_DEBUG_BSM)
			zlog_debug("UJTdhMS2AXIew5EH ""%s : Empty Pref BSM received", __func__);
	}
	...
	if (!pim_bsm_parse_install_g2rp(
		    ...
		    frag_tag)) {
		if (PIM_DEBUG_BSM) {
			zlog_debug("SX3pqu2wnPVUSyS7 ""%s, Parsing BSM failed.", __func__);
		}
		pim->bsm_dropped++;
		return -1;
	}
	...
	if (pim_ifp->pim->global_scope.bsm_frag_tag != frag_tag) {
		if (PIM_DEBUG_BSM) {
			zlog_debug("07XxxjrohduyZ1Jz ""%s: Current frag tag: %d Frag teg rcvd: %d",
				   __func__,
