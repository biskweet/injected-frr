...
static void pim_mlag_zebra_fill_header(enum mlag_msg_type msg_type)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("Hn8o9FE98a7oT4uH "":%s: msg_type: %d/%d len %d",
...
static void pim_mlag_zebra_flush_buffer(void)
{
	...
	if (mlag_bulk_cnt > 1) {
		...
		if (msg_type == MLAG_MROUTE_ADD_BULK) {
			...
			flog_err(EC_LIB_ZAPI_ENCODE,"pCH9IhXQQc1gFYMW "
...
static void pim_mlag_zthread_handler(struct event *event)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("Gx1WGAEGJEsYXkIA "":%s: Processing MLAG write, %d messages in queue",
			   __func__, wr_count);
	...
	for (wr_count = 0; wr_count < PIM_MLAG_POST_LIMIT; wr_count++) {
		...
		if (!read_s) {
			zlog_debug("ktXSuHFj1clLWruy "":%s: Got a NULL Messages, some thing wrong",
				   __func__);
			break;
		}
...
int pim_mlag_signal_zpthread(void)
{
	if (router->master) {
		if (PIM_DEBUG_MLAG)
			zlog_debug("licYiyiFQcsyIMxQ "":%s: Scheduling PIM MLAG write Thread",
				   __func__);
