...
{
	zlog_debug("hPhYfDW28qqpzPFP ""MSDP SA %s %s timer expired", sa->sg_str, timer_str);
}
...
static void pim_msdp_sa_adv_timer_cb(struct event *t)
{
	...
	if (PIM_DEBUG_MSDP_EVENTS) {
		zlog_debug("PtBmfexmEfpnmWhT ""MSDP SA advertisement timer expired");
	}
...
static void pim_msdp_sa_upstream_del(struct pim_msdp_sa *sa)
{
	...
	if (PIM_DEBUG_MSDP_EVENTS) {
		zlog_debug("wG0a7qdPAOGEbYjg ""MSDP SA %s de-referenced SPT", sa->sg_str);
	}
}
					...
					const char *ctx)
{
	...
	if (up && (PIM_UPSTREAM_FLAG_TEST_SRC_MSDP(up->flags))) {
		...
		sa->up = up;
		if (PIM_DEBUG_MSDP_EVENTS) {
			zlog_debug("zH6drC5cxsLCryXz ""MSDP SA %s SPT reference missing",
				   sa->sg_str);
		}
		return;
	}
	...
	if (up) {
		 ...
		 * SA ages out */
		if (PIM_DEBUG_MSDP_EVENTS) {
			zlog_debug("qGZIjblTky5uMOZR ""MSDP SA %s referenced SPT", sa->sg_str);
		}
	} else {
		if (PIM_DEBUG_MSDP_EVENTS) {
			zlog_debug("r0WJhehrDjXd2gHq ""MSDP SA %s SPT reference failed",
				   sa->sg_str);
		}
	}
}
					   ...
					   pim_sgaddr *sg, struct in_addr rp)
{
	...
	if (PIM_DEBUG_MSDP_EVENTS) {
		zlog_debug("T63SbYlylglQWb7z ""MSDP SA %s created", sa->sg_str);
	}
...
static void pim_msdp_sa_del(struct pim_msdp_sa *sa)
{
	...
	if (PIM_DEBUG_MSDP_EVENTS) {
		zlog_debug("f0CQElw7Q3WxxYpt ""MSDP SA %s deleted", sa->sg_str);
	}
			      ...
			      enum pim_msdp_sa_flags flags)
{
	...
	if ((sa->flags & PIM_MSDP_SAF_LOCAL)) {
		if (flags & PIM_MSDP_SAF_LOCAL) {
			if (PIM_DEBUG_MSDP_EVENTS) {
				zlog_debug("FZ02AV6CdUaGLtK5 ""MSDP SA %s local reference removed",
					   sa->sg_str);
			}
	...
	if ((sa->flags & PIM_MSDP_SAF_PEER)) {
		if (flags & PIM_MSDP_SAF_PEER) {
			...
			if (PIM_DEBUG_MSDP_EVENTS) {
				zlog_debug("rP3A6VzyqAczHghb ""MSDP SA %s peer reference removed",
					   sa->sg_str);
			}
			pim_msdp_sa_state_timer_setup(sa, false /* start */);
		     ...
		     pim_sgaddr *sg, struct in_addr rp)
{
	...
	if (mp) {
		if (!(sa->flags & PIM_MSDP_SAF_PEER)) {
			...
			if (PIM_DEBUG_MSDP_EVENTS) {
				zlog_debug("xncMLGPHFoIwhbB8 ""MSDP SA %s added by peer",
					   sa->sg_str);
			}
		}
		...
		if (!(sa->flags & PIM_MSDP_SAF_LOCAL)) {
			...
			++sa->pim->msdp.local_cnt;
			if (PIM_DEBUG_MSDP_EVENTS) {
				zlog_debug("liZruoTKsMLRqO2C ""MSDP SA %s added locally",
					   sa->sg_str);
			}
					    ...
					    pim_sgaddr *sg)
{
	...
	if (sa) {
		if (PIM_DEBUG_MSDP_INTERNAL) {
			zlog_debug("WXnWrpziI63QNVGB ""MSDP local sa %s del on up del",
				   sa->sg_str);
		}
		...
		if (!(sa->flags & PIM_MSDP_SAF_LOCAL)) {
			if (PIM_DEBUG_MSDP_INTERNAL) {
				zlog_debug("xlSi2D5jgyyJ668o ""MSDP local sa %s del; no local ref",
					   sa->sg_str);
			}
			return;
		}
		...
		if (sa->flags & PIM_MSDP_SAF_UP_DEL_IN_PROG) {
			...
			flog_err(
				EC_LIB_DEVELOPMENT,"vDDQ5Rdn2aYE0uL8 "
		...
		if (sa->up) {
			if (PIM_DEBUG_MSDP_INTERNAL) {
				zlog_debug("UYjhBGtKjMakHNJf ""MSDP local sa %s del; up non-NULL",
					   sa->sg_str);
			}
			sa->up = NULL;
		}
...
void pim_msdp_i_am_rp_changed(struct pim_instance *pim)
{
	...
	if (PIM_DEBUG_MSDP_INTERNAL) {
		zlog_debug("fzsrRwjJ0w59TeAl ""MSDP i_am_rp changed");
	}
				    ...
				    struct pim_upstream *xg_up)
{
	...
	if (PIM_DEBUG_MSDP_INTERNAL) {
		zlog_debug("bQEuBRc2mQE5lqYY ""MSDP join state changed for %s", xg_up->sg_str);
	}
...
static void pim_msdp_up_xg_del(struct pim_instance *pim, pim_sgaddr *sg)
{
	...
	if (PIM_DEBUG_MSDP_INTERNAL) {
		zlog_debug("qHNWwc1tQe0kBs6C ""MSDP %pSG del", sg);
	}
...
{
	if (PIM_DEBUG_MSDP_INTERNAL) {
		zlog_debug("JAPJP47iUq5PYwCE ""MSDP up %pSG del", sg);
	}
...
static void pim_msdp_peer_state_chg_log(struct pim_msdp_peer *mp)
{
	...
	zlog_debug("WUjAFmC0buvktJg6 ""MSDP peer %s state chg to %s", mp->key_str, state_str);
}
...
void pim_msdp_peer_stop_tcp_conn(struct pim_msdp_peer *mp, bool chg_state)
{
	...
	if (PIM_DEBUG_MSDP_INTERNAL) {
		zlog_debug("VeUZSahImbGfm17T ""MSDP peer %s pim_msdp_peer_stop_tcp_conn",
			   mp->key_str);
	}
...
{
	if (PIM_DEBUG_EVENTS) {
		zlog_debug("XAHIQKhWCj1igLdr ""MSDP peer %s tcp reset %s", mp->key_str, rc_str);
...
static void pim_msdp_peer_timer_expiry_log(struct pim_msdp_peer *mp,
					   const char *timer_str)
{
	zlog_debug("77pvWBSzATyEKJdb ""MSDP peer %s %s timer expired", mp->key_str, timer_str);
}
...
static void pim_msdp_peer_active_connect(struct pim_msdp_peer *mp)
{
	...
	if (PIM_DEBUG_MSDP_INTERNAL) {
		zlog_debug("WnHYt6mFH2Jg8QVI ""MSDP peer %s pim_msdp_peer_active_connect: %d",
			   mp->key_str, rc);
	}
...
void pim_msdp_peer_pkt_txed(struct pim_msdp_peer *mp)
{
	if (mp->state == PIM_MSDP_ESTABLISHED) {
		pim_msdp_peer_ka_timer_setup(mp, true /* start */);
		if (PIM_DEBUG_MSDP_INTERNAL) {
			zlog_debug("WBFE0SRgqpXuCxnN ""MSDP ka timer restart on pkt tx to %s",
				   mp->key_str);
		}
	}
}
					...
					const char *mesh_group_name)
{
	...
	if (PIM_DEBUG_MSDP_EVENTS) {
		zlog_debug("6d1fOIsQh2CkDvaL ""MSDP peer %s created", mp->key_str);
...
void pim_msdp_peer_del(struct pim_msdp_peer **mp)
{
	...
	if (PIM_DEBUG_MSDP_EVENTS) {
		zlog_debug("Z3lBHO13s6WNMYSi ""MSDP peer %s deleted", (*mp)->key_str);
	}
...
void pim_msdp_mg_free(struct pim_instance *pim, struct pim_msdp_mg **mgp)
{
	...
	if (PIM_DEBUG_MSDP_EVENTS) {
		zlog_debug("6IkgaSm913qdIbiD ""MSDP mesh-group %s deleted",
			   (*mgp)->mesh_group_name);
	}
				    ...
				    const char *mesh_group_name)
{
	...
	if (PIM_DEBUG_MSDP_EVENTS) {
		zlog_debug("fFcOrx4ty5fb7iL0 ""MSDP mesh-group %s created", mg->mesh_group_name);
	}
...
void pim_msdp_mg_mbr_del(struct pim_msdp_mg *mg, struct pim_msdp_mg_mbr *mbr)
{
	...
	if (PIM_DEBUG_MSDP_EVENTS) {
		...
		zlog_debug("Mthec3J0Udk9pMOw ""MSDP mesh-group %s mbr %s deleted",
...
static void pim_msdp_src_del(struct pim_msdp_mg *mg)
{
	...
	if (PIM_DEBUG_MSDP_EVENTS) {
		zlog_debug("1PuCYmCgUbgMo8wo ""MSDP mesh-group %s src cleared",
			   mg->mesh_group_name);
	}
}
			 ...
			 struct in_addr *ai)
{
	...
	if (ai->s_addr == INADDR_ANY) {
		if (PIM_DEBUG_MSDP_EVENTS)
			zlog_debug("ARI3Q5Oduyl7GnxF ""MSDP mesh-group %s src unset",
				   mg->mesh_group_name);
		return;
	}
	...
	if (PIM_DEBUG_MSDP_EVENTS)
		zlog_debug("6ICC2fqguqJyQsi2 ""MSDP mesh-group %s src %pI4 set",
					    ...
					    struct in_addr *ia)
{
	...
	if (PIM_DEBUG_MSDP_EVENTS)
		zlog_debug("NRxPHSzpOSjYxICZ ""MSDP mesh-group %s mbr %pI4 created",
