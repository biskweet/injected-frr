...
static void pim_vxlan_rp_send_null_register_startup(struct event *e)
{
	...
	if (vxlan_sg->null_register_sent > PIM_VXLAN_STARTUP_NULL_REGISTERS) {
		if (PIM_DEBUG_VXLAN)
			zlog_debug("c2gM27MPWp4th3eb ""Null registering stopping for %s",
				   vxlan_sg->sg_str);
		return;
	}
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("5B0U17CNSvoq8kEe ""Sent null register for %s", vxlan_sg->sg_str);
				...
				struct pim_rpf *rpg_changed)
{
	...
	for (listnode = vxlan_info.work_list->head; listnode;
		...
		if (rpg == rpg_changed) {
			if (PIM_DEBUG_VXLAN)
				zlog_debug("SlJFpP84fX5wFIF9 ""VXLAN RP info for %s alive sending",
					   vxlan_sg->sg_str);
...
static void pim_vxlan_do_reg_work(void)
{
	...
	if (sec_count > PIM_VXLAN_NULL_REG_INTERVAL) {
		...
		if (PIM_DEBUG_VXLAN && listnode)
			zlog_debug("3f8ocRzKheAKegWu ""vxlan SG work %s",
	...
	for (; listnode; listnode = listnode->next) {
		...
		if (vxlan_sg->up && (vxlan_sg->up->reg_state == PIM_REG_JOIN)) {
			if (PIM_DEBUG_VXLAN)
				zlog_debug("6felpxVzKnCsBDB5 ""vxlan SG %s periodic NULL register",
						vxlan_sg->sg_str);
		...
		if (work_cnt > vxlan_info.max_work_cnt) {
			...
			if (PIM_DEBUG_VXLAN)
				zlog_debug("OCckYtPDPJzal6aM ""vxlan SG %d work items proc and pause",
					work_cnt);
			return;
		}
	}
	...
	if (work_cnt) {
		if (PIM_DEBUG_VXLAN)
			zlog_debug("nZCcEZgLXrW3HQVr ""vxlan SG %d work items proc", work_cnt);
	}
...
static void pim_vxlan_add_work(struct pim_vxlan_sg *vxlan_sg)
{
	if (vxlan_sg->flags & PIM_VXLAN_SGF_DEL_IN_PROG) {
		if (PIM_DEBUG_VXLAN)
			zlog_debug("h6yKpoCB8RzG2kTn ""vxlan SG %s skip work list; del-in-prog",
					vxlan_sg->sg_str);
		return;
	}
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("v8oNxbpxoEGtb4SO ""vxlan SG %s work list add",
				vxlan_sg->sg_str);
...
static void pim_vxlan_del_work(struct pim_vxlan_sg *vxlan_sg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("E7K6LAak0n6WEmKo ""vxlan SG %s work list del",
				vxlan_sg->sg_str);
		...
		struct pim_upstream *up, bool reg_join)
{
	...
	if (reg_join)
		 ...
		 */
		if (PIM_DEBUG_VXLAN)
			zlog_debug("uJatVTSglRrct7P6 ""Received Register stop for %s",
				   vxlan_sg->sg_str);
...
static void pim_vxlan_orig_mr_up_del(struct pim_vxlan_sg *vxlan_sg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("c0SDz9Og68gslUj1 ""vxlan SG %s orig mroute-up del",
			vxlan_sg->sg_str);
...
static void pim_vxlan_orig_mr_up_iif_update(struct pim_vxlan_sg *vxlan_sg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("AwFgFDihqy6e04D4 ""vxlan SG %s orig mroute-up updated with iif %s",
			vxlan_sg->sg_str,
...
static void pim_vxlan_orig_mr_up_add(struct pim_vxlan_sg *vxlan_sg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("3TqisetDnBNawUDX ""vxlan SG %s orig mroute-up add with iif %s",
			vxlan_sg->sg_str,
			vxlan_sg->iif?vxlan_sg->iif->name:"-");
	...
	if (!up) {
		if (PIM_DEBUG_VXLAN)
			zlog_debug("yUOvGSMKjaKxD3Nw ""vxlan SG %s orig mroute-up add failed",
					vxlan_sg->sg_str);
		return;
	}
...
static void pim_vxlan_orig_mr_oif_add(struct pim_vxlan_sg *vxlan_sg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("WEqO5Eurw7vwiSin ""vxlan SG %s oif %s add",
...
static void pim_vxlan_orig_mr_oif_del(struct pim_vxlan_sg *vxlan_sg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("E25nwHkJrgSHmBJq ""vxlan SG %s oif %s del",
...
static void pim_vxlan_orig_mr_add(struct pim_vxlan_sg *vxlan_sg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("I9lEZ6sbjtmo84e1 ""vxlan SG %s orig-mr add", vxlan_sg->sg_str);
...
{
	if (PIM_DEBUG_VXLAN)
		zlog_debug("FRruWJE9gnzbv3nN ""vxlan SG %s orig-mr del", vxlan_sg->sg_str);
...
static void pim_vxlan_orig_mr_iif_update(struct hash_bucket *bucket, void *arg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("zBfHARMuayHMsqPR ""vxlan SG %s iif changed from %s to %s",
				vxlan_sg->sg_str,
...
static void pim_vxlan_term_mr_oif_add(struct pim_vxlan_sg *vxlan_sg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("yW6MOGgt1JA3qHaT ""vxlan SG %s term-oif %s add",
	...
	if (pim_ifchannel_local_membership_add(vxlan_sg->term_oif,
		...
		zlog_warn("B0oz39ZO4D3iy3po ""vxlan SG %s term-oif %s add failed",
			vxlan_sg->sg_str, vxlan_sg->term_oif->name);
	}
}
...
static void pim_vxlan_term_mr_oif_del(struct pim_vxlan_sg *vxlan_sg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("rn10WObAXEXMlU06 ""vxlan SG %s oif %s del",
			vxlan_sg->sg_str, vxlan_sg->term_oif->name);
...
static void pim_vxlan_term_mr_up_add(struct pim_vxlan_sg *vxlan_sg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("uNjQcunXNzKyZzJh ""vxlan SG %s term mroute-up add",
			vxlan_sg->sg_str);
	...
	if (!up) {
		zlog_warn("PMws3XNAXWNQrFUI ""vxlan SG %s term mroute-up add failed",
			vxlan_sg->sg_str);
		return;
	}
...
static void pim_vxlan_term_mr_up_del(struct pim_vxlan_sg *vxlan_sg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("nM2tb6fWtoeHCNts ""vxlan SG %s term mroute-up del",
			vxlan_sg->sg_str);
	vxlan_sg->up = NULL;
...
{
	if (PIM_DEBUG_VXLAN)
		zlog_debug("xiyDD0522wuy5sbX ""vxlan SG %s term mroute add", vxlan_sg->sg_str);
...
{
	if (PIM_DEBUG_VXLAN)
		zlog_debug("5DrROrWSVuuLmLaf ""vxlan SG %s term mroute del", vxlan_sg->sg_str);
					     ...
					     pim_sgaddr *sg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("bOpTYZMuZW9WdYww ""vxlan SG %s alloc", vxlan_sg->sg_str);
...
static void pim_vxlan_sg_del_item(struct pim_vxlan_sg *vxlan_sg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("oWxCOl6F2spG88lX ""vxlan SG %s free", vxlan_sg->sg_str);
				...
				struct in_addr *reg_addr)
{
	...
	if (PIM_DEBUG_VXLAN) {
		...
		zlog_debug("sXoKpmka7U5G3qCM ""vxlan MLAG update %s state %s role %d rif %s addr %s",
	...
	if (!pim) {
		if (PIM_DEBUG_VXLAN)
			zlog_debug("aZSGzaH0ryGRHQCU ""%s: Unable to find pim instance", __func__);
		return;
	}
				...
				struct interface *ifp)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("ToJd01nymjAHwV09 ""%s: vxlan default iif changed from %s to %s",
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("yYVGzPgU5mFcvlgn ""%s: vxlan orig iif changed from %s to %s", __func__,
			   old_iif ? old_iif->name : "-",
			   ifp ? ifp->name : "-");
	...
	if (up->rpf.source_nexthop.interface &&
			((up->rpf.source_nexthop.interface ==
			 ...
			 (up->rpf.source_nexthop.interface ==
			  old_peerlink_rif))) {
		if (PIM_DEBUG_VXLAN)
			zlog_debug("mYksZGFlFs9Zn8H9 ""RPF cost adjust for %s on peerlink-rif (old: %s, new: %s) change",
					up->sg_str,
					old_peerlink_rif ?
			...
			struct interface *ifp)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("bMPWZyGdlZhrEuaJ ""%s: vxlan peerlink_rif changed from %s to %s",
			   __func__, old_iif ? old_iif->name : "-",
			   ifp ? ifp->name : "-");
	...
	if (old_iif != new_iif) {
		if (PIM_DEBUG_VXLAN)
			zlog_debug("kvxVeSBGUcaU4exC ""%s: vxlan orig iif changed from %s to %s",
				   __func__, old_iif ? old_iif->name : "-",
	...
	if (old_oif != new_oif) {
		if (PIM_DEBUG_VXLAN)
			zlog_debug("7VE8bE6fTKhamugR ""%s: vxlan orig oif changed from %s to %s",
...
static void pim_vxlan_term_mr_oif_update(struct hash_bucket *bucket, void *arg)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("OKvWOobaFYkbkvQE ""vxlan SG %s term oif changed from %s to %s",
			vxlan_sg->sg_str,
		...
		struct interface *ifp)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("TZv1QrmbE3dfPic9 ""vxlan term oif changed from %s to %s",
		...
		struct interface *ifp)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("YzhKAjbmB7bLizyg ""vxlan term oif cfg changed from %s to %s",
...
void pim_vxlan_del_term_dev(struct pim_instance *pim)
{
	...
	if (PIM_DEBUG_VXLAN)
		zlog_debug("oOR0bFFxVwKzk71d ""vxlan term oif cfg changed from %s to -",
				ifp->name);
