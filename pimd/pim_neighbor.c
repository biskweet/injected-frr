...
static void dr_election_by_addr(struct interface *ifp)
{
	...
	if (PIM_DEBUG_PIM_TRACE) {
		zlog_debug("taCLl2MsNam66zve ""%s: on interface %s", __func__, ifp->name);
	}
...
static void dr_election_by_pri(struct interface *ifp)
{
	...
	if (PIM_DEBUG_PIM_TRACE) {
		zlog_debug("J7twKCEpGzZ1f6n0 ""%s: dr pri %u on interface %s", __func__, dr_pri,
			   ifp->name);
	}

	for (ALL_LIST_ELEMENTS_RO(pim_ifp->pim_neighbor_list, node, neigh)) {
		if (PIM_DEBUG_PIM_TRACE) {
			zlog_info("zfxbm8eYw757YGsX ""%s: neigh pri %u addr %pPA if dr addr %pPA",
...
void pim_neighbor_timer_reset(struct pim_neighbor *neigh, uint16_t holdtime)
{
	...
	if (PIM_DEBUG_PIM_TRACE_DETAIL)
		zlog_debug("UzoUXXba5J80qw3M ""%s: starting %u sec timer for neighbor %pPA on %s",
...
static void on_neighbor_jp_timer(struct event *t)
{
	...
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("a9OSF0QEJflCyPwc ""%s:Sending JP Agg to %pPA on %s with %d groups",
		 ...
		 struct list *addr_list)
{
	...
	if (PIM_DEBUG_PIM_EVENTS)
		zlog_debug("8IIfWwMXUUXjWQm6 ""%s: creating PIM neighbor %pPA on interface %s",
			   __func__, &source_addr, ifp->name);

	zlog_notice("tqnX8IO1fusZn2UP ""PIM NEIGHBOR UP: neighbor %pPA on interface %s",
		 ...
		 struct list *addr_list, int send_hello_now)
{
	...
	if (PIM_DEBUG_PIM_TRACE_DETAIL)
		zlog_debug("9tqE1QfrIVDZe4po ""%s: neighbor %pPA added ", __func__, &source_addr);
	/*
	  RFC 4601: 4.3.2.  DR Election
			 ...
			 const char *delete_message)
{
	...
	zlog_notice("MD5GDkI8xHfQoQcG ""PIM NEIGHBOR DOWN: neighbor %pPA on interface %s: %s",
	...
	if (PIM_DEBUG_PIM_TRACE) {
		zlog_debug("beJ7rVMBhQiygaeq ""%s: deleting PIM neighbor %pPA on interface %s",
			 ...
			 uint32_t dr_priority, struct list *addr_list)
{
	...
	if (neigh->prefix_list == addr_list) {
		if (addr_list) {
			flog_err(
				EC_LIB_DEVELOPMENT,"eViv7K4qd21D9gI3 "
