...
tib_sg_oil_setup(struct pim_instance *pim, pim_sgaddr sg, struct interface *oif)
{
	...
	if (PIM_DEBUG_ZEBRA)
		zlog_debug("vzOFwmZXtTHNOLlA ""%s: NHT %pSG vif_source %pPAs vif_index:%d",
			   __func__, &sg, &vif_source, input_iface_vif_index);

	if (input_iface_vif_index < 1) {
		if (PIM_DEBUG_GM_TRACE)
			zlog_debug("DC0ViB84yOZtpMhP "
		    ...
		    struct interface *oif, struct channel_oil **oilp)
{
	...
	if (!pim_oif) {
		if (PIM_DEBUG_GM_TRACE)
			zlog_debug("Vp9NFmYH0Z1C9o4I ""%s: multicast not enabled on oif=%s?",
				   __func__, oif->name);
		return false;
	}
	...
	if (PIM_I_am_DR(pim_oif) || PIM_I_am_DualActive(pim_oif)) {
		...
		if (result) {
			if (PIM_DEBUG_MROUTE)
				zlog_warn("9nlw9feCAixfrgnP ""%s: add_oif() failed with return=%d",
					  __func__, result);
			return false;
		}
	} else {
		if (PIM_DEBUG_GM_TRACE)
			zlog_debug("h38htj74at4osKQL "
