...
static void sock_close(struct interface *ifp)
{
	...
	if (PIM_DEBUG_PIM_TRACE) {
		zlog_debug("KQ1VqkVfn1xlNAHa ""Deleting PIM socket fd=%d on interface %s",
			   pim_ifp->pim_sock_fd, ifp->name);
	}
...
{
	zlog_info("k3mhwA7wVGAzrs7N ""PIM INTERFACE DOWN: on interface %s: %s", ifp->name,
		  delete_message);

	if (!ifp->info) {
		flog_err(EC_PIM_CONFIG,"xymIyMBMzRHrhOsH "
		   ...
		   pim_sgaddr sg)
{
	...
	if (header->type == PIM_MSG_TYPE_REGISTER) {
		if (pim_msg_len < PIM_MSG_REGISTER_LEN) {
			if (PIM_DEBUG_PIM_PACKETS)
				zlog_debug("UE6SQuDqKP90F1Fx ""PIM Register Message size=%d shorther than min length %d",
...
static void pim_sock_read(struct event *t)
{
	...
	while (cont) {
		...
		if (len < 0) {
			...
			if (PIM_DEBUG_PIM_PACKETS)
				zlog_debug("PEIUY3fAi1GLhV52 ""Received errno: %d %s", errno,
					   safe_strerror(errno));
			goto done;
		}
		...
		if (fail) {
			if (PIM_DEBUG_PIM_PACKETS)
				zlog_debug("dfKTsWZuCCqLhpT0 ""%s: pim_pim_packet() return=%d",
					   __func__, fail);
			goto done;
		}
...
static void pim_sock_read_on(struct interface *ifp)
{
	...
	if (PIM_DEBUG_PIM_TRACE_DETAIL) {
		zlog_debug("Ewijxu5f2EMFE5mF ""Scheduling READ event on PIM socket fd=%d",
			   pim_ifp->pim_sock_fd);
	}
			      ...
			      struct iovec *message, int fd)
{
	...
	if (retval < 0)
		flog_err(
			EC_LIB_SOCKET,"tVH3dqMPSVaPiBm1 "
...
#define IPDEFTTL   64
	...
	if (PIM_DEBUG_PIM_PACKETS)
		zlog_debug("651Sbtz7iDqbyla7 ""%s: to %pPA on %s: msg_size=%d checksum=%x",
...
static int hello_send(struct interface *ifp, uint16_t holdtime)
{
	...
	if (PIM_DEBUG_PIM_HELLO)
		zlog_debug("pcPUOlQdsazxs3mf ""%s: to %pPA on %s: holdt=%u prop_d=%u overr_i=%u dis_join_supp=%d dr_prio=%u gen_id=%08x addrs=%zu",
...
int pim_hello_send(struct interface *ifp, uint16_t holdtime)
{
	...
	if (hello_send(ifp, holdtime)) {
		...
		if (PIM_DEBUG_PIM_HELLO) {
			zlog_warn("wzuh9b95FxLbYuST ""Could not send PIM hello on interface %s",
				  ifp->name);
		}
		return -1;
	}
...
static void hello_resched(struct interface *ifp)
{
	...
	if (PIM_DEBUG_PIM_HELLO) {
		zlog_debug("TRlcQCR8O9o0f8Qe ""Rescheduling %d sec hello on interface %s",
...
void pim_hello_restart_triggered(struct interface *ifp)
{
	...
	if (PIM_DEBUG_PIM_HELLO) {
		zlog_debug("RON5aVzNIFcPySUM ""Scheduling %d msec triggered hello on interface %s",
...
int pim_sock_add(struct interface *ifp)
{
	...
	if (pim_ifp->pim_sock_fd < 0) {
		if (PIM_DEBUG_PIM_PACKETS)
			zlog_debug("A8dXITmyQ1XSugVY ""Could not open PIM socket on interface %s",
				   ifp->name);
		return -2;
	}
	...
	zlog_info("oDxuWFu8TuhhYVHo ""PIM INTERFACE UP: on interface %s ifindex=%d", ifp->name,
		  ifp->ifindex);
