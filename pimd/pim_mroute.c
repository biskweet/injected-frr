...
int pim_mroute_set(struct pim_instance *pim, int enable)
{
	...
	if (enable && pim->vrf->vrf_id != VRF_DEFAULT) {
		frr_with_privs (&pimd_privs) {
			...
			if (err) {
				if (err == ENOPROTOOPT)
					zlog_err("6cKBUE0iuC6B2hYk ""%s Kernel is not compiled with CONFIG_IP_MROUTE_MULTIPLE_TABLES and vrf's will not work",
						 __func__);
				else
					zlog_warn("KEXd9oKpiWS7AaZU ""%s %s: failure: setsockopt(fd=%d,PIM_IPPROTO, MRT_TABLE=%d): errno=%d: %s",
						  __FILE__, __func__,
...
int pim_mroute_msg_nocache(int fd, struct interface *ifp, const kernmsg *msg)
{
	...
	if (!pim_is_grp_ssm(pim_ifp->pim, sg.grp)) {
		...
		if (!rpg) {
			if (PIM_DEBUG_MROUTE)
				zlog_debug("ad5pWHKZ4rq3uq1d ""%s: no RPF for packet to %pSG",
					   ifp->name, &sg);
			return 0;
		}
		if (pim_rpf_addr_is_inaddr_any(rpg)) {
			if (PIM_DEBUG_MROUTE)
				zlog_debug("ILk4fTEhMUn3z7ez ""%s: null RPF for packet to %pSG",
					   ifp->name, &sg);
			return 0;
		}
	}
	 ...
	 */
	if (PIM_DEBUG_MROUTE) {
		zlog_debug("WYMnKP5uYa1NWZpz ""%s: Adding a Route %s for WHOLEPKT consumption",
			   __func__, up->sg_str);
	}
			    ...
			    size_t len)
{
	...
	if (!up) {
		...
		if (up && PIM_UPSTREAM_FLAG_TEST_CAN_BE_LHR(up->flags)) {
			...
			if (PIM_DEBUG_MROUTE)
				zlog_debug("j5u9wfOqkkqD6ToS ""%s: Creating %s upstream on LHR",
					   __func__, up->sg_str);
			return 0;
		}
		if (PIM_DEBUG_MROUTE_DETAIL) {
			zlog_debug("97DB4KMb9mdvGRdK "
	...
	if (!up->rpf.source_nexthop.interface) {
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("siEY1qWcFjOgWDUs ""%s: up %s RPF is not present", __func__,
				   up->sg_str);
		return 0;
	}
	...
	if ((pim_rpf_addr_is_inaddr_any(rpg)) || (!pim_ifp) ||
	    (!(PIM_I_am_DR(pim_ifp)))) {
		if (PIM_DEBUG_MROUTE) {
			zlog_debug("PXMjf5PzXcqObkra ""%s: Failed Check send packet", __func__);
		}
		return 0;
	}
...
int pim_mroute_socket_enable(struct pim_instance *pim)
{
	...
	frr_with_privs(&pimd_privs) {
...
#endif
		if (fd < 0) {
			zlog_warn("qdRfYHkYpQexWoAc ""Could not create mroute socket: errno=%d: %s",
				  errno,
				  safe_strerror(errno));
			return -2;
		}
		...
		if (pim->vrf->vrf_id != VRF_DEFAULT
			...
			zlog_warn("YGZ3E6Ztq6kQjN43 ""Could not setsockopt SO_BINDTODEVICE: %s",
				  safe_strerror(errno));
			close(fd);
			return -3;
		}
...
int pim_mroute_socket_disable(struct pim_instance *pim)
{
	...
	if (close(pim->mroute_socket)) {
		zlog_warn("Mc7chqZ8vlE9hwwK ""Failure closing mroute socket: fd=%d errno=%d: %s",
			  pim->mroute_socket, errno, safe_strerror(errno));
		return -3;
	}
		       ...
		       unsigned char flags)
{
	...
	if (PIM_DEBUG_MROUTE)
		zlog_debug("kPbiiQ29SaopBX1x ""%s: Add Vif %d (%s[%s])", __func__,
...
int pim_mroute_del_vif(struct interface *ifp)
{
	...
	if (PIM_DEBUG_MROUTE)
		zlog_debug("qIijnAbiSfgW7VxJ ""%s: Del Vif %d (%s[%s])", __func__,
			   pim_ifp->mroute_vif_index, ifp->name,
			   pim_ifp->pim->vrf->name);
...
static int pim_mroute_add(struct channel_oil *c_oil, const char *name)
{
	...
	if (PIM_DEBUG_MROUTE) {
		char buf[1000];
		zlog_debug("iVpIY6Uy0bGC3C9N ""%s(%s), vrf %s Added Route: %s", __func__, name,
			   pim->vrf->name,
...
int pim_upstream_mroute_iif_update(struct channel_oil *c_oil, const char *name)
{
	...
	*oil_incoming_vif(c_oil) = iif;
	...
	if (PIM_DEBUG_MROUTE_DETAIL)
		zlog_debug("v7Nk3IgeM2pH2Oel ""%s(%s) %s mroute iif update %d",
				__func__, name,
				pim_channel_oil_dump(c_oil, buf,
					sizeof(buf)), iif);
...
int pim_mroute_del(struct channel_oil *c_oil, const char *name)
{
	...
	if (!c_oil->installed) {
		if (PIM_DEBUG_MROUTE) {
			...
			zlog_debug("WD68grLIbbs7WY6w ""%s %s: incoming interface %s for route is %s not installed, do not need to send del req. ",
				   __FILE__, __func__,
	...
	if (PIM_DEBUG_MROUTE) {
		char buf[1000];
		zlog_debug("KK90WpjnSGqhIkFv ""%s(%s), vrf %s Deleted Route: %s", __func__, name,
			   pim->vrf->name,
			   pim_channel_oil_dump(c_oil, buf, sizeof(buf)));
	}
...
void pim_mroute_update_counters(struct channel_oil *c_oil)
{
	...
	if (!c_oil->installed) {
		...
		if (PIM_DEBUG_MROUTE) {
			...
			zlog_debug("WSCyEgIxo39ugWMg ""Channel%pSG is not installed no need to collect data from kernel",
				   &sg);
		}
		return;
	}
