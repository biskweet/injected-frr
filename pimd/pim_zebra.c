...
static void dump_if_address(struct interface *ifp)
{
	...
	zlog_debug("wWVGfcNRb3BR62j4 ""%s %s: interface %s addresses:", __FILE__, __func__,
		   ifp->name);
	...
	frr_each (if_connected, ifp->connected, ifc) {
		...
		zlog_debug("Bdq2AfLUPSKBwm7S ""%s %s: interface %s address %pI4 %s", __FILE__,
...
static int pim_zebra_if_address_add(ZAPI_CALLBACK_ARGS)
{
	...
	*/
	...
	if (PIM_DEBUG_ZEBRA) {
		zlog_debug("Nwi4Ws2PoGHY1l9U ""%s: %s(%s) connected IP address %pFX flags %u %s",
	...
	if (pim_ifp) {
		...
		if (!pim) {
			if (PIM_DEBUG_ZEBRA)
				zlog_debug("90llvoC4N8CPObeq ""%s: Unable to find pim instance",
					   __func__);
			return 0;
		}
...
static int pim_zebra_vxlan_sg_proc(ZAPI_CALLBACK_ARGS)
{
	...
	if (PIM_DEBUG_ZEBRA)
		zlog_debug("livbhnKcYGDc2RqL ""%s:recv SG %s %pSG", VRF_LOGNAME(pim->vrf),
...
void sched_rpf_cache_refresh(struct pim_instance *pim)
{
	...
	if (PIM_DEBUG_ZEBRA) {
		zlog_debug("Gx8fZwlxLl1U5m82 ""%s: triggering %ld msec timer", __func__,
...
void pim_zebra_init(void)
{
	...
	if (PIM_DEBUG_PIM_TRACE) {
		zlog_notice("7rdWDIQKXriiNYaO ""%s: zclient socket initialized", __func__);
	}
...
void pim_forward_start(struct pim_ifchannel *ch)
{
	...
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("zYTodZUMFfuuxyx1 ""%s: (S,G)=%pSG oif=%s (%pPA)", __func__, &ch->sg,
...
void pim_forward_stop(struct pim_ifchannel *ch)
{
	...
	if (PIM_DEBUG_PIM_TRACE) {
		zlog_debug("ApNxh32vtXgpaKYS ""%s: (S,G)=%s oif=%s installed: %d",
