				      ...
				      struct pim_upstream *up, const char *name)
{
	...
	if (PIM_DEBUG_TRACE)
		zlog_debug("fd6wSHGVT7OfJ7ig ""pim_upstream free vrf:%s %s flags 0x%x",
...
void pim_upstream_send_join(struct pim_upstream *up)
{
	if (!up->rpf.source_nexthop.interface) {
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("UJEo8EEqtwrL6VUu ""%s: up %s RPF is not present", __func__,
				   up->sg_str);
		return;
	}

	if (PIM_DEBUG_PIM_TRACE) {
		zlog_debug("Kp7VVN7LC12gdUcJ ""%s: RPF'%s=%pPA(%s) for Interface %s", __func__,
			   up->sg_str, &up->rpf.rpf_addr,
			   pim_upstream_state2str(up->join_state),
			   up->rpf.source_nexthop.interface->name);
		if (pim_rpf_addr_is_inaddr_any(&up->rpf)) {
			zlog_debug("FXZt1TOGvYdkGU9v ""%s: can't send join upstream: RPF'%s=%pPA",
...
static void on_join_timer(struct event *t)
{
	...
	if (!up->rpf.source_nexthop.interface) {
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("g64VZkF4eaKbMpAv ""%s: up %s RPF is not present", __func__,
				   up->sg_str);
		return;
	}
...
{
	if (PIM_DEBUG_PIM_EVENTS) {
		zlog_debug("sC2KjbQ6qxdBnOED ""%s: restarting %d msec timer for upstream (S,G)=%s",
				...
				int holdtime)
{
	...
	if (!up->rpf.source_nexthop.interface) {
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("rPL8ylWWqcoCnldG ""%s: up %s RPF is not present", __func__,
				   up->sg_str);
		return;
	}
						    ...
						    struct pim_upstream *up)
{
	...
	if (!up->rpf.source_nexthop.interface) {
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("dOgynPuk2SSAVn9i ""%s: up %s RPF is not present", __func__,
				   up->sg_str);
		return;
	}
...
int pim_upstream_could_register(struct pim_upstream *up)
{
	...
	if (up->rpf.source_nexthop.interface)
		...
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("0i8ZWXZrQym1lBN6 ""%s: up %s RPF is not present", __func__,
				   up->sg_str);
	}
			...
			bool update_mroute)
{
	...
	if (old_use_rpt != new_use_rpt) {
		if (PIM_DEBUG_PIM_EVENTS)
			zlog_debug("YnkMc2IXSIarLJVq ""%s switched from %s to %s", up->sg_str,
				   old_use_rpt ? "RPT" : "SPT",
				   new_use_rpt ? "RPT" : "SPT");
		if (update_mroute)
			 ...
			 enum pim_upstream_state new_state)
{
	...
	if (pim_addr_is_any(up->upstream_addr)) {
		if (PIM_DEBUG_PIM_EVENTS)
			zlog_debug("5RBJGF7Zf5P226RV ""%s: RPF not configured for %s", __func__,
				   up->sg_str);
		return;
	}
	...
	if (!up->rpf.source_nexthop.interface)  {
		if (PIM_DEBUG_PIM_EVENTS)
			zlog_debug("HHTMAnMCDHjGKChx ""%s: RP not reachable for %s", __func__,
				   up->sg_str);
		return;
	}

	if (PIM_DEBUG_PIM_EVENTS) {
		zlog_debug("7bRiVCLL6E413gDg ""%s: PIM_UPSTREAM_%s: (S,G) old: %s new: %s",
			   __func__, up->sg_str,
			   pim_upstream_state2str(up->join_state),
					     ...
					     struct pim_ifchannel *ch)
{
	...
	if (!pim_rp_set_upstream_addr(pim, &up->upstream_addr, sg->src,
				      sg->grp)) {
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("81gbIydmL1bN0YlQ ""%s: Received a (*,G) with no RP configured",
				   __func__);
	}
...
void pim_upstream_ref(struct pim_upstream *up, int flags, const char *name)
{
	...
	++up->ref_count;
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("t1tn2gzDqxDESm10 ""%s(%s): upstream %s ref count %d increment",
					...
					struct pim_upstream *up)
{
	...
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("5j9n8eSpkZjdc5VZ ""kat expired on %s; clear fhr reg state",
			   up->sg_str);
...
void pim_upstream_keep_alive_timer_start(struct pim_upstream *up, uint32_t time)
{
	if (!PIM_UPSTREAM_FLAG_TEST_SRC_STREAM(up->flags)) {
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("LNsN1rIo1z99i7fi ""kat start on %s with no stream reference",
				   up->sg_str);
	}
	EVENT_OFF(up->t_ka_timer);
			     ...
			     struct interface *incoming)
{
	...
	if (!pim_upstream_evaluate_join_desired(up->channel_oil->pim, up)) {
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("JlyhmEMDdQtFpNyv ""%s: %s Join is not Desired", __func__,
				   up->sg_str);
		return;
	}
	...
	if (pim_if_connected_to_source(up->rpf.source_nexthop.interface,
				       up->sg.src)) {
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("xY9ygaYWqIiKSDhR ""%s: %s is directly connected to the source",
				   __func__, up->sg_str);
	...
	if (pim_upstream_is_sg_rpt(up)
		...
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("huGDLifZwZqDvN78 ""%s: %s OR inherited_olist(S,G,rpt) == NULL",
				   __func__, up->sg_str);
		up->sptbit = PIM_UPSTREAM_SPTBIT_TRUE;
		return;
	}
	...
	if (up->parent && pim_rpf_is_same(&up->rpf, &up->parent->rpf)) {
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("QXdP2RWbWJSY10z0 ""%s: %s RPF'(S,G) is the same as RPF'(*,G)",
				   __func__, up->sg_str);
		up->sptbit = PIM_UPSTREAM_SPTBIT_TRUE;
		return;
	}
...
static void pim_upstream_register_stop_timer(struct event *t)
{
	...
	if (PIM_DEBUG_PIM_TRACE) {
		...
		zlog_debug("dql2kn3aVGsa5J4P ""%s: (S,G)=%s[%s] upstream register stop timer %s",
	...
	case PIM_REG_PRUNE:
		...
		if (!up->rpf.source_nexthop.interface) {
			if (PIM_DEBUG_PIM_TRACE)
				zlog_debug("WQm3GrRUY8vPwpAJ ""%s: up %s RPF is not present",
					   __func__, up->sg_str);
			up->reg_state = PIM_REG_NOINFO;
			PIM_UPSTREAM_FLAG_UNSET_FHR(up->flags);
			return;
...
static void pim_upstream_start_register_stop_timer(struct pim_upstream *up)
{
	...
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("CqUhCoBl9Y2xOYWw ""%s: (S,G)=%s Starting upstream register stop timer %d",
...
static void pim_upstream_register_probe_timer(struct event *t)
{
	...
	if (!up->rpf.source_nexthop.interface ||
		...
		if (PIM_DEBUG_PIM_REG)
			zlog_debug("ZT4wdxt95o84jN2h ""cannot send Null register for %pSG, no path to RP",
				   &up->sg);
	} else
		pim_null_register_send(up);
...
void pim_upstream_start_register_probe_timer(struct pim_upstream *up)
{
	...
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("KBAIEEPMzkvEEZ2j ""%s: (S,G)=%s Starting upstream register stop null probe timer %d",
			   __func__, up->sg_str, time);
					...
					struct pim_upstream *up)
{
	...
	if (!up->rpf.source_nexthop.interface)
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("QEGJwGcFmttXnFZd ""%s: up %s RPF is not present", __func__,
				   up->sg_str);
				  ...
				  struct pim_upstream *up)
{
	...
	if (up->rpf.source_nexthop.interface) {
		...
		if (PIM_DEBUG_PIM_TRACE) {
			zlog_debug("OCuQVkXCbMOoOSbk ""%s: up %s RPF is not present", __func__,
				   up->sg_str);
		}
	}
...
static void pim_upstream_sg_running(void *arg)
{
	...
	if (!up->channel_oil->installed) {
		if (PIM_DEBUG_TRACE)
			zlog_debug("2vccSSUqNWZB8dQo ""%s: %s[%s] is not installed in mroute",
				   __func__, up->sg_str, pim->vrf->name);
		return;
	}
