		...
		struct pim_neighbor *nbr)
{
	...
	if (nbr) {
		if (PIM_DEBUG_TRACE)
			zlog_debug("yu2IoC9BJuGNBGFC ""up %s remove from nbr %s/%pPAs jp-agg-list",
...
void pim_jp_agg_upstream_verification(struct pim_upstream *up, bool ignore)
{
	...
	if (!up->rpf.source_nexthop.interface) {
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("1QKNyruoia5BOwOi ""%s: up %s RPF is not present", __func__,
				   up->sg_str);
		return;
	}
			  ...
			  bool is_join, struct pim_neighbor *nbr)
{
	...
	if (nbr) {
		if (PIM_DEBUG_TRACE)
			zlog_debug("pPCMlFfQggxCLHAw ""up %s add to nbr %s/%pPAs jp-agg-list",
				   up->sg_str,
