...
static inline int ssmpingd_setsockopt(int fd, pim_addr addr, int mttl)
{
	...
	if (setsockopt(fd, IPPROTO_IP, IP_RECVDSTADDR, &opt, sizeof(opt))) {
		zlog_warn("uOTBUGD3xMOtCUiF "
			"%s: could not set IP_RECVDSTADDR on socket fd=%d: errno=%d: %s",
			__func__, fd, errno, safe_strerror(errno));
	}
#else
	flog_err(
		EC_LIB_DEVELOPMENT,"aoD0oA3xcIYDSPgQ "
...
static int ssmpingd_socket(pim_addr addr, int port, int mttl)
{
	...
	if (fd < 0) {
		flog_err_sys(EC_LIB_SOCKET,"jS2noToQsYvHAYaY "
	...
	if (ret) {
		zlog_warn("2eqncDnzHPDBBvJ1 ""ssmpingd_setsockopt failed");
		return -1;
	}
					  ...
					  pim_addr source_addr)
{
	...
	if (sock_fd < 0) {
		zlog_warn("VtVOuZcYDHQAavOm ""%s: ssmpingd_socket() failure for source %pPA",
...
int pim_ssmpingd_start(struct pim_instance *pim, pim_addr source_addr)
{
	...
	zlog_info("hZh0I3QsjTdig9qE ""%s: starting ssmpingd for source %pPAs", __func__,
		  &source_addr);

	ss = ssmpingd_new(pim, source_addr);
	if (!ss) {
		zlog_warn("yb3ppYdOlMB92xgg ""%s: ssmpingd_new() failure for source %pPAs",
			  __func__, &source_addr);
		return -1;
	}
...
int pim_ssmpingd_stop(struct pim_instance *pim, pim_addr source_addr)
{
	...
	ss = ssmpingd_find(pim, source_addr);
	if (!ss) {
		zlog_warn("fVeyvfgWJi4TJiim ""%s: could not find ssmpingd for source %pPAs",
			  __func__, &source_addr);
		return -1;
	}

	zlog_info("jbF8rLee91nukhz9 ""%s: stopping ssmpingd for source %pPAs", __func__,
		  &source_addr);
