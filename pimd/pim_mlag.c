						 ...
						 bool is_df)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("DxdmNGs0SiJPvX86 ""%s: Calculating DF for Dual active if-channel%s",
			   __func__, up->sg_str);
...
static void pim_mlag_inherit_mlag_flags(struct pim_upstream *up, bool is_df)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("5UxwqAguzLVjvIjm ""%s: Updating DF for uptream:%s children", __func__,
			   up->sg_str);
	...
	for (ALL_LIST_ELEMENTS(up->ifchannels, chnode, chnextnode, ch)) {
		...
		for (ALL_LIST_ELEMENTS_RO(up->sources, listnode, child)) {
			if (PIM_DEBUG_MLAG)
				zlog_debug("8uV0JBjPUYErmghE ""%s: Updating DF for child:%s",
		...
		struct pim_upstream *up, bool is_df, const char *reason)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("5njjo6VByV0zIety ""local MLAG mroute %s role changed to %s based on %s",
...
static void pim_mlag_up_peer_add(struct mlag_mroute_add *msg)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("pUj50xGvhPvfGnD9 ""peer MLAG mroute add %s:%pSG cost %d",
...
static void pim_mlag_up_peer_del(struct mlag_mroute_del *msg)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("YkTELaOCpX8N06jJ ""peer MLAG mroute del %s:%pSG", msg->vrf_name, &sg);

	vrf = vrf_lookup_by_name(msg->vrf_name);
	if  (!vrf) {
		if (PIM_DEBUG_MLAG)
			zlog_debug("sLSzFWm6qc0Z0Ykz "
		...
		struct pim_upstream *up)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("8GfaQzoTTwWgtcp3 ""local MLAG mroute add %s:%s",
				vrf->name, up->sg_str);
		...
		struct pim_upstream *up)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("XlWIEjMoNdwSw2II ""local MLAG mroute del %s:%s",
				vrf->name, up->sg_str);
...
static void pim_mlag_up_local_reeval(bool mlagd_send, const char *reason_code)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("lZL7kuQ5wi8lz2tR ""%s re-run DF election because of %s",
...
static void pim_mlag_process_mlagd_state_change(struct mlag_status msg)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("vNP0FiOdrnpZR85j ""%s: msg dump: my_role: %s, peer_state: %s",
			   __func__,
	...
	if (!(router->mlag_flags & PIM_MLAGF_LOCAL_CONN_UP)) {
		if (PIM_DEBUG_MLAG)
			zlog_debug("uPtqcStGI5tf9UCL ""%s: msg ignored mlagd process state down",
					__func__);
		return;
	}
...
{
	if (PIM_DEBUG_MLAG)
		zlog_debug("wP1fcit8XOt3DhiT "
			"%s: msg dump: peer_frr_state: %s", __func__,
			(msg.frr_state == MLAG_FRR_STATE_UP ? "UP" : "DOWN"));

	if (!(router->mlag_flags & PIM_MLAGF_LOCAL_CONN_UP)) {
		if (PIM_DEBUG_MLAG)
			zlog_debug("OcjktujVhSdsOc2D ""%s: msg ignored mlagd process state down",
					__func__);
		return;
	}
...
static void pim_mlag_process_vxlan_update(struct mlag_vxlan *msg)
{
	...
	if (!(router->mlag_flags & PIM_MLAGF_LOCAL_CONN_UP)) {
		if (PIM_DEBUG_MLAG)
			zlog_debug("jIr3NlgXadqMVMpc ""%s: msg ignored mlagd process state down",
					__func__);
		return;
	}
	...
	if (PIM_DEBUG_MLAG) {
		...
		zlog_debug("0bwFkI5LLPnGAADT ""%s: msg dump: local-ip:%s, anycast-ip:%s",
...
static void pim_mlag_process_mroute_add(struct mlag_mroute_add msg)
{
	...
	if (!(router->mlag_flags & PIM_MLAGF_LOCAL_CONN_UP)) {
		if (PIM_DEBUG_MLAG)
			zlog_debug("hX7fxSSISE5RgEhp ""%s: msg ignored mlagd process state down",
					__func__);
		return;
	}
...
static void pim_mlag_process_mroute_del(struct mlag_mroute_del msg)
{
	if (PIM_DEBUG_MLAG) {
		...
		sg.grp.s_addr = ntohl(msg.group_ip);
		sg.src.s_addr = ntohl(msg.source_ip);
		zlog_debug("TSRDCvlfphMJtSzx "
			"%s: msg dump: vrf_name: %s, s.ip: 0x%x, g.ip: 0x%x(%pSG)",
			__func__, msg.vrf_name, msg.source_ip, msg.group_ip,
			&sg);
		zlog_debug("ll4WjvW4IrxN1rzr ""(%pSG)owner_id: %d, vrf_id: 0x%x intf_name: %s",
			   &sg, msg.owner_id, msg.vrf_id, msg.intf_name);
	}

	if (!(router->mlag_flags & PIM_MLAGF_LOCAL_CONN_UP)) {
		if (PIM_DEBUG_MLAG)
			zlog_debug("wuryBwTiRZ0zhjQL ""%s: msg ignored mlagd process state down",
					__func__);
		return;
	}
			      ...
			      uint16_t zapi_length, vrf_id_t vrf_id)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("Gn2eIAhP1xuh3oVt ""%s: Received msg type: %s length: %d, bulk_cnt: %d",
			   __func__,
...
{
	if (PIM_DEBUG_MLAG)
		zlog_debug("D5xBnPhvm7kHhxSD ""%s: Received Process-Up from Mlag", __func__);
...
{
	if (PIM_DEBUG_MLAG)
		zlog_debug("tM6qefEkO4KofWuR ""%s: Received Process-Down from Mlag", __func__);
...
static void pim_mlag_register_handler(struct event *thread)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("RDtDQK0Oj6n9zaSo ""%s: Posting Client Register to MLAG mask: 0x%x",
			   __func__, bit_mask);
...
static void pim_mlag_deregister_handler(struct event *thread)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("orC6o0vDIfAAjgDU ""%s: Posting Client De-Register to MLAG from PIM",
			   __func__);
	router->connected_to_mlag = false;
...
void pim_if_configure_mlag_dualactive(struct pim_interface *pim_ifp)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("agUoLAzdQxUDP5ay ""%s: Configuring active-active on Interface: %s",
			   __func__, "NULL");
...
void pim_if_unconfigure_mlag_dualactive(struct pim_interface *pim_ifp)
{
	...
	if (PIM_DEBUG_MLAG)
		zlog_debug("cd850Tnf32qyyx7C ""%s: UnConfiguring active-active on Interface: %s",
			   __func__, "NULL");
