				    ...
				    struct interface **ifpp)
{
	...
	if (!pim_nexthop_lookup(pim, &nexthop, mtracep->src_addr, 1)) {
		if (PIM_DEBUG_MTRACE)
			zlog_debug("SJbKye7qv3J5Yd8f ""mtrace not found neighbor");
		return false;
	}

	if (PIM_DEBUG_MTRACE)
		zlog_debug("nOQAWdr0YKU47t9T ""mtrace pim_nexthop_lookup OK");

	if (PIM_DEBUG_MTRACE)
		zlog_debug("EvXP0DVnlcF1M8BU ""mtrace next_hop=%pPAs", &nexthop.mrib_nexthop_addr);
			    ...
			    struct interface **ifpp)
{
	...
	if (!up->rpf.source_nexthop.interface) {
		if (PIM_DEBUG_TRACE)
			zlog_debug("1VJuYOJiRrhSMerd ""%s: up %s RPF is not present", __func__,
				   up->sg_str);
		return false;
	}
	...
	if (PIM_DEBUG_MTRACE)
		zlog_debug("UxeeKYIeLUSarVNz ""fwd_info: upstream next hop=%pI4", &nh_addr);
...
static uint32_t query_arrival_time(void)
{
	...
	if (gettimeofday(&tv, NULL) < 0) {
		if (PIM_DEBUG_MTRACE)
			zlog_debug("Jiwk18ewdGJSLEza ""Query arrival time lookup failed: errno=%d: %s",
			      ...
			      struct in_addr group_addr)
{
	...
	if (PIM_DEBUG_MTRACE) {
		...
		zlog_debug("3iv1hLoAYOpZGowP ""Sending mtrace packet to %pI4 on %pI4", &rsp_addr,
			   &if_addr);
	}
	...
	if (IPV4_CLASS_DE(ntohl(dst_addr.s_addr))) {
		...
		if (ret < 0) {
			if (PIM_DEBUG_MTRACE)
				zlog_debug("XBEm0Qb27PKi3Xnm ""Failed to set socket multicast TTL");
			ret = -1;
			goto close_fd;
		}
	}
				    ...
				    struct interface *interface)
{
	...
	if (PIM_DEBUG_MTRACE) {
		zlog_debug("nFD6N7DDdmDDBLfe ""Fwd mtrace packet len=%u to %pI4 ttl=%u",
	...
	if (IPV4_CLASS_DE(ntohl(mtracep->rsp_addr.s_addr))) {
		...
		if (p_rpf == NULL) {
			if (PIM_DEBUG_MTRACE) {
				...
				zlog_debug("DZucqMcKIC2DpmVf ""mtrace no RP for %pI4", &rsp_addr);
			}
			return -1;
		}
		nexthop = p_rpf->source_nexthop;
		if (PIM_DEBUG_MTRACE)
			zlog_debug("GiH8rs0epkrboYEo ""mtrace response to RP");
	} else {
		memset(&nexthop, 0, sizeof(nexthop));
		/* TODO: should use unicast rib lookup */
		if (!pim_nexthop_lookup(pim, &nexthop, mtracep->rsp_addr, 1)) {
			if (PIM_DEBUG_MTRACE)
				zlog_debug("cWLqVcpRnBcqSCdn "
			     ...
			     char *igmp_msg, int igmp_msg_len)
{
	...
	if (!r_len) {
		if (PIM_DEBUG_MTRACE)
			zlog_debug("K4DATQo2eXML8l0f ""Received IGMP multicast traceroute query");

		/* 6.1.1  Packet verification */
		if (!pim_if_connected_to_source(ifp, mtracep->dst_addr)) {
			if (IPV4_CLASS_DE(ntohl(ip_hdr->ip_dst.s_addr))) {
				if (PIM_DEBUG_MTRACE)
					zlog_debug("65zM55wkdryt1V6K "
			...
			if (PIM_DEBUG_MTRACE)
				zlog_debug("GfasBvouLoKt20Nr ""Multicast query on wrong interface");
		}
		if (qry_id == mtracep->qry_id && qry_src == from.s_addr) {
			if (PIM_DEBUG_MTRACE)
				zlog_debug("jPtwFF27gOk8xy4P "
	...
	/* if response fields length is equal to a whole number of responses */
		...
		if (last_rsp_ind > MTRACE_MAX_HOPS) {
			if (PIM_DEBUG_MTRACE)
				zlog_debug("bHLRlxszPuCMOWgl ""Mtrace request of excessive size");
			return -1;
		}
	} else {
		if (PIM_DEBUG_MTRACE)
			zlog_debug("GNbfw5YKKZjc5E1O "
	...
	if (!fwd_info) {
		if (PIM_DEBUG_MTRACE)
			zlog_debug("1kOC1wMnvK5WniWi ""mtrace not found multicast state");
	...
	if (nh_addr.s_addr == INADDR_ANY) {
		...
		if (!out_ifp->info) {
			if (PIM_DEBUG_MTRACE)
				zlog_debug("ENqtorn3WTvWFNgZ ""mtrace not found incoming if w/ pim");
			      ...
			      char *igmp_msg, int igmp_msg_len)
{
	...
	if (qry_id == mtracep->qry_id && rsp_dst == ip_hdr->ip_dst.s_addr) {
		if (PIM_DEBUG_MTRACE)
			zlog_debug("0zBRkBWhMLGKltCG ""duplicate mtrace response packet dropped");
		return -1;
	}
