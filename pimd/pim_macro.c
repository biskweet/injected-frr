...
int pim_macro_ch_lost_assert(const struct pim_ifchannel *ch)
{
	...
	ifp = ch->interface;
	if (!ifp) {
		zlog_warn("L0kBd7oEslfxVUlW ""%s: (S,G)=%s: null interface", __func__, ch->sg_str);
		return 0; /* false */
	}
	...
	pim_ifp = ifp->info;
	if (!pim_ifp) {
		zlog_warn("t0DsD0sRx0boiypi ""%s: (S,G)=%s: multicast not enabled on interface %s",
...
int pim_macro_chisin_pim_include(const struct pim_ifchannel *ch)
{
	...
	if (!pim_ifp) {
		zlog_warn("LsrcBAwI3clgvqfQ ""%s: (S,G)=%s: multicast not enabled on interface %s",
...
int pim_macro_ch_could_assert_eval(const struct pim_ifchannel *ch)
{
	...
	ifp = ch->interface;
	if (!ifp) {
		zlog_warn("uxq655eIVEQir8PJ ""%s: (S,G)=%s: null interface", __func__, ch->sg_str);
		return 0; /* false */
	}
...
int pim_macro_assert_tracking_desired_eval(const struct pim_ifchannel *ch)
{
	...
	ifp = ch->interface;
	if (!ifp) {
		zlog_warn("pEDDqHMCHBV3xNbL ""%s: (S,G)=%s: null interface", __func__, ch->sg_str);
		return 0; /* false */
	}

	pim_ifp = ifp->info;
	if (!pim_ifp) {
		zlog_warn("7GTd0zcVR0iQAflL ""%s: (S,G)=%s: multicast not enabled on interface %s",
			  __func__, ch->sg_str, ch->interface->name);
		return 0; /* false */
	}
