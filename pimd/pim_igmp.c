				  ...
				  struct gm_group *group)
{
	...
	if (!source) {
		zlog_warn("w5zX7Rds9Kg62FR7 ""%s: Failure to create * source", __func__);
		return;
	}
			       ...
			       struct gm_source *source)
{
	...
	if (PIM_DEBUG_GM_TRACE) {
		zlog_debug("Du1bHGhZQ9tOMIkN ""%s: (S,G)=%pSG oif=%s fwd=%d", __func__, &sg,
...
void igmp_source_forward_stop(struct gm_source *source)
{
	...
	if (PIM_DEBUG_GM_TRACE) {
		zlog_debug("csFrH5cMB6pYsSFd ""%s: (S,G)=%pSG oif=%s fwd=%d", __func__, &sg,
			   source->source_group->interface->name,
			   IGMP_SOURCE_TEST_FORWARDING(source->source_flags));
	}
...
static int igmp_sock_open(struct in_addr ifaddr, struct interface *ifp)
{
	...
	*/
	...
	if (!join) {
		flog_err_sys(
			EC_LIB_SOCKET,"Y8QWXX9EwcFMSyCg "
...
static void igmp_sock_dump(array_t *igmp_sock_array)
{
	...
	for (int i = 0; i < size; ++i) {
		...
		zlog_debug("C4BdIEG5s30qHOsm ""%s %s: [%d/%d] igmp_addr=%pI4 fd=%d", __FILE__,
...
static void pim_igmp_other_querier_expire(struct event *t)
{
	...
	if (PIM_DEBUG_GM_TRACE) {
		...
		pim_inet4_dump("<ifaddr?>", igmp->ifaddr, ifaddr_str,
			       sizeof(ifaddr_str));
		zlog_debug("X5MRLUYQShbUtQKW ""%s: Querier %s resuming", __func__, ifaddr_str);
	}
...
int igmp_validate_checksum(char *igmp_msg, int igmp_msg_len)
{
	...
	if (ntohs(checksum) != recv_checksum) {
		zlog_warn("fM5RsQce3PNG8Uaj ""Invalid checksum received %x, calculated %x",
			   ...
			   int igmp_msg_len)
{
	...
	if (!pim_if_connected_to_source(ifp, from)) {
		if (PIM_DEBUG_GM_PACKETS)
			zlog_debug("I5Wu8ma6n85AJyaD ""Recv IGMP query on interface: %s from a non-connected source: %s",
				   ifp->name, from_str);
		return 0;
	}
	...
	if (if_address_is_local(&from, AF_INET, ifp->vrf->vrf_id)) {
		if (PIM_DEBUG_GM_PACKETS)
			zlog_debug("Knv67IY4dTBdB0QA ""Recv IGMP query on interface: %s from ourself %s",
				   ifp->name, from_str);
		return 0;
	}
	...
	if (PIM_DEBUG_GM_PACKETS) {
		...
		pim_inet4_dump("<group?>", group_addr, group_str,
			       sizeof(group_str));
		zlog_debug("Exrdhe78HAehOzXR ""Recv IGMP query v%d from %s on %s for group %s",
		     ...
		     struct in_addr from)
{
	if (PIM_DEBUG_GM_TRACE) {
		...
		zlog_debug("bSZPPxq7Z7CtArg2 ""%s: from %s on %s", label, from_str, ifp->name);
	}
}
			       ...
			       int igmp_msg_len)
{
	...
	if (PIM_DEBUG_GM_TRACE) {
		zlog_warn("23sKMbNNqeqwkO2c ""%s %s: FIXME WRITEME", __FILE__, __func__);
	}
...
bool pim_igmp_verify_header(struct ip *ip_hdr, size_t len, size_t *hlen)
{
	...
	if (len < sizeof(*ip_hdr)) {
		zlog_warn("ES0VTfEsfhBYFMfF ""IGMP packet size=%zu shorter than minimum=%zu", len,
			  sizeof(*ip_hdr));
		return false;
	}
	...
	*hlen = ip_hlen;
	...
	if (igmp_msg_len < PIM_IGMP_MIN_LEN) {
		zlog_warn("XrOndbx3KTpXtHPm ""IGMP message size=%d shorter than minimum=%d",
...
int pim_igmp_packet(struct gm_sock *igmp, char *buf, size_t len)
{
	...
	case PIM_IGMP_MEMBERSHIP_QUERY: {
		...
		*/
		...
		if (igmp_msg_len == 8) {
			...
			zlog_warn("UQwQ5wlrocyyo05n ""Unknown IGMP query version");
			return -1;
		}
	...
	zlog_warn("ay3aYt5OlJKpZB0s ""Ignoring unsupported IGMP message type: %d", msg_type);
...
static void pim_igmp_general_query(struct event *t)
{
	...
	*/
	...
	if (PIM_DEBUG_GM_TRACE) {
		...
		zlog_debug("X0IusYn2688wsyTd ""Querier %s issuing IGMP general query to %s on %s",
...
static void sock_close(struct gm_sock *igmp)
{
	...
	if (close(igmp->fd)) {
		flog_err(
			EC_LIB_SOCKET,"K529Cczrcnysg67A "
	...
	if (PIM_DEBUG_GM_TRACE_DETAIL) {
		zlog_debug("ZbmJFn0sg8QHOrao ""Deleted IGMP socket %pI4 fd=%d on interface %s",
			   &igmp->ifaddr, igmp->fd,
			   igmp->interface->name);
	}
}
...
static void igmp_group_count_decr(struct pim_interface *pim_ifp)
{
	if (pim_ifp->pim->gm_group_count == 0) {
		zlog_warn("ZFYlUGaenM6DBj5I ""Cannot decrement igmp group count below 0(vrf: %s)",
			  VRF_LOGNAME(pim_ifp->pim->vrf));
		return;
	}
...
void igmp_group_delete(struct gm_group *group)
{
	...
	if (PIM_DEBUG_GM_TRACE) {
		...
		pim_inet4_dump("<group?>", group->group_addr, group_str,
			       sizeof(group_str));
		zlog_debug("H7ooMSjwMUFO7L0G ""Deleting IGMP group %s from interface %s",
...
static void igmp_read_on(struct gm_sock *igmp)
{
	...
	if (PIM_DEBUG_GM_TRACE_DETAIL) {
		zlog_debug("gKmcwbXMzxE6c5sc ""Scheduling READ event on IGMP socket fd=%d",
			   igmp->fd);
	}
				  ...
				  bool mtrace_only)
{
	...
	if (fd < 0) {
		zlog_warn("mou8mmiNsByOCE9U ""Could not open IGMP socket for %pI4 on %s",
			  &ifaddr, ifp->name);
		return NULL;
	}
	...
	if (bind(fd, (struct sockaddr *) &sin, sizeof(sin)) != 0) {
		zlog_warn("sEwUIHCm7txLZbG3 ""Could not bind IGMP socket for %pI4 on %s: %s(%d)",
...
static void igmp_group_timer(struct event *t)
{
	...
	if (PIM_DEBUG_GM_TRACE) {
		...
		pim_inet4_dump("<group?>", group->group_addr, group_str,
			       sizeof(group_str));
		zlog_debug("Nx2AWrc1XZto5WIY ""%s: Timer for group %s on interface %s", __func__,
			   group_str, group->interface->name);
	}
...
static void group_timer_off(struct gm_group *group)
{
	...
	if (PIM_DEBUG_GM_TRACE) {
		...
		pim_inet4_dump("<group?>", group->group_addr, group_str,
			       sizeof(group_str));
		zlog_debug("1z9PQvnJNkQTzGm0 ""Cancelling TIMER event for group %s on %s",
			   group_str, group->interface->name);
	}
					...
					struct in_addr group_addr)
{
	...
	if (!pim_is_group_224_4(group_addr)) {
		zlog_warn("ck6vBv72fdkcdRCN ""%s: Group Specified is not part of 224.0.0.0/4",
			  __func__);
		return NULL;
	}
...
void igmp_send_query_on_intf(struct interface *ifp, int igmp_ver)
{
	...
	if (PIM_DEBUG_GM_TRACE)
		zlog_debug("cSR9id5F4CFB8BPe ""Issuing general query on request on %s", ifp->name);
