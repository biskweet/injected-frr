...
static void igmp_sock_query_interval_reconfig(struct gm_sock *igmp)
{
	...
	if (PIM_DEBUG_GM_TRACE)
		zlog_debug("UpwjM63KbCAErBRl ""%s: Querier %pPAs on %s reconfig query_interval=%d",
	...
	struct nb_cb_apply_finish_args *args)
{
	...
	if (!pim_ifp) {
		zlog_debug("baHzQ9G0WlzFmz8S ""Pim not enabled on this interface");
		return;
	}
	...
	case NB_EV_APPLY:
		...
		if (yang_dnode_get(args->dnode, "group-list")) {
			...
			if (!pim_get_all_mcast_group(&group)) {
				flog_err(
					EC_LIB_DEVELOPMENT,"rP6lISiAz3rTGreE "
	...
	struct nb_cb_modify_args *args)
{
	...
	case NB_EV_APPLY:
		...
		yang_dnode_get_pimaddr(&rp_addr, args->dnode, "../rp-address");
		if (!pim_get_all_mcast_group(&group)) {
			flog_err(EC_LIB_DEVELOPMENT,"20LnvxMhASRVCEbZ "
				 "Unable to convert 224.0.0.0/4 to prefix");
			return NB_ERR_INCONSISTENCY;
		}
	...
	struct nb_cb_destroy_args *args)
{
	...
	case NB_EV_APPLY:
		...
		plist = yang_dnode_get_string(args->dnode, NULL);
		if (!pim_get_all_mcast_group(&group)) {
			flog_err(EC_LIB_DEVELOPMENT,"JBTEixxf6SHEHXI5 "
				 "Unable to convert 224.0.0.0/4 to prefix");
			return NB_ERR_INCONSISTENCY;
		}
