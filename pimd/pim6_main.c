...
{
	zlog_info("2sQmIMDNBiLqBbX8 ""SIGHUP received, ignoring");
}

static void pim6_sigint(void)
{
	zlog_notice("MbsWCm0tRzA0DseS ""Terminating on signal SIGINT");
	pim6_terminate();
	exit(1);
}
...
{
	zlog_notice("9Rm37G0dKvt5WWX6 ""Terminating on signal SIGTERM");
	pim6_terminate();
	exit(1);
}
