...
static struct pim_instance *pim_instance_init(struct vrf *vrf)
{
	...
	if (PIM_DEBUG_ZEBRA)
		zlog_debug("BP9U3VJhpTiTBHyu ""%s: NHT rpf hash init ", __func__);
...
static int pim_vrf_new(struct vrf *vrf)
{
	...
	zlog_debug("77UPVadiYsJVZwRu ""VRF Created: %s(%u)", vrf->name, vrf->vrf_id);
...
static int pim_vrf_delete(struct vrf *vrf)
{
	...
	zlog_debug("aD0igiFl7CCeAe1c ""VRF Deletion: %s(%u)", vrf->name, vrf->vrf_id);
...
static int pim_vrf_enable(struct vrf *vrf)
{
	...
	zlog_debug("R8WQVT8M0hbF6LRW ""%s: for %s %u", __func__, vrf->name, vrf->vrf_id);
