...
void pim_ifchannel_delete(struct pim_ifchannel *ch)
{
	...
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("YG3jXBGg5H7LCOYb ""%s: ifchannel entry %s(%s) del start", __func__,
			   ch->sg_str, ch->interface->name);

	if (PIM_I_am_DualActive(pim_ifp)) {
		if (PIM_DEBUG_MLAG)
			zlog_debug("GG9l7cVx2JC91Kiz "
	...
	*/
	...
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("wTeMZSgWBoppoVFX ""%s: ifchannel entry %s(%s) is deleted ", __func__,
			   ch->sg_str, ch->interface->name);
				 ...
				 enum pim_ifjoin_state new_state)
{
	...
	if ((old_state == PIM_IFJOIN_NOINFO)
		...
		if (PIM_DEBUG_PIM_EVENTS) {
			zlog_debug("TK7o9Rmd2u1VAlD6 ""PIM_IFCHANNEL_%s: (S,G)=%s on interface %s",
...
struct pim_ifchannel *pim_ifchannel_find(struct interface *ifp, pim_sgaddr *sg)
{
	...
	if (!pim_ifp) {
		zlog_warn("UATmF3P5DJSIuSAd ""%s: (S,G)=%pSG: multicast not enabled on interface %s",
			     ...
			     enum pim_ifmembership membership)
{
	...
	if (PIM_DEBUG_PIM_EVENTS) {
		zlog_debug("vCg2C2Na0yYM3Zoh ""%s: (S,G)=%s membership now is %s on interface %s",
			   __func__, ch->sg_str,
					...
					uint8_t source_flags, int up_flags)
{
	...
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("VWPO8S4o35popW8G ""%s: ifchannel %s(%s) is created ", __func__,
			   ch->sg_str, ch->interface->name);
...
static void on_ifjoin_expiry_timer(struct event *t)
{
	...
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("GTsn3IJG7Ua5nSqr ""%s: ifchannel %s expiry timer", __func__,
			   ch->sg_str);
...
static void on_ifjoin_prune_pending_timer(struct event *t)
{
	...
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("FDBeELr2juZirNEM ""%s: IFCHANNEL%pSG %s Prune Pending Timer Popped",
			   __func__, &ch->sg,
			   pim_ifchannel_ifjoin_name(ch->ifjoin_state, ch->flags));
				...
				uint8_t source_flags, int holdtime)
{
	...
	if (pim_rpf_addr_is_inaddr_any(&up->rpf)) {
		/* RPF'(S,G) not found */
		zlog_warn("WmHfkXHLN61uFQnL ""%s %s: RPF'%s not found", __FILE__, __func__,
			  up->sg_str);
		return;
	}
			    ...
			    uint8_t source_flags, uint16_t holdtime)
{
	...
	if (ch->ifassert_state == PIM_IFASSERT_I_AM_LOSER) {
		zlog_warn("ChoAjKaSiT5bkbUp ""%s: Assert Loser recv Join%s from %pPA on %s",
			 ...
			 uint16_t holdtime)
{
	...
	if (!ch && !(source_flags & PIM_ENCODE_RPT_BIT)) {
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("sbbqryRCFoDwbSbU ""%s: Received prune with no relevant ifchannel %s%pSG state: %d",
				       ...
				       bool is_vxlan)
{
	...
	if (!pim_ifp) {
		if (PIM_DEBUG_EVENTS)
			zlog_debug("prvg1iMY8cSTsZf6 ""%s:%pSG Expected pim interface setup for %s",
				   __func__, sg, ifp->name);
		return 0;
	}
	...
	if (!pim_ifp->pim_enable) {
		if (PIM_DEBUG_EVENTS)
			zlog_debug("48Je8L2hvPLlgxb5 ""%s:%pSG PIM is not configured on this interface %s",
				   __func__, sg, ifp->name);
		return 0;
	}
	...
	if (pim_addr_is_any(sg->src)) {
		if (pim_is_grp_ssm(pim, sg->grp)) {
			if (PIM_DEBUG_PIM_EVENTS)
				zlog_debug("r5CwwuzULig9Jijt ""%s: local membership (S,G)=%pSG ignored as group is SSM",
					   __func__, sg);
			return 1;
		}
	}
	...
	if (pim_addr_is_any(sg->src)) {
		...
		for (ALL_LIST_ELEMENTS_RO(up->sources, up_node, child)) {
			if (PIM_DEBUG_EVENTS)
				zlog_debug("KwE2voZIB7Ssp4RK ""%s %s: IGMP (S,G)=%s(%s) from %s",
...
void pim_ifchannel_local_membership_del(struct interface *ifp, pim_sgaddr *sg)
{
	...
	if (pim_addr_is_any(sg->src)) {
		...
		for (ALL_LIST_ELEMENTS(up->sources, up_node, up_nnode, child)) {
			...
			if (PIM_DEBUG_EVENTS)
				zlog_debug("CIJi9JZFjZzz4pJ0 ""%s %s: Prune(S,G)=%s(%s) from %s",
...
void pim_ifchannel_update_could_assert(struct pim_ifchannel *ch)
{
	...
	if (PIM_DEBUG_PIM_EVENTS)
		zlog_debug("DMS4obMNBMAsnCPS ""%s: CouldAssert(%pPAs,%pPAs,%s) changed from %d to %d",
