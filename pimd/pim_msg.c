...
size_t pim_msg_get_jp_group_size(struct list *sources)
{
	...
	if (PIM_DEBUG_PIM_PACKETS)
		zlog_debug("Tj9o7JUrvvfQD3DV ""%s: Considering (%s) children for (S,G,rpt) prune",
			   __func__, up->sg_str);
	...
	for (ALL_LIST_ELEMENTS_RO(up->sources, up_node, child)) {
		...
		if (PIM_UPSTREAM_FLAG_TEST_SRC_VXLAN_ORIG(child->flags) &&
			...
			if (PIM_DEBUG_PIM_PACKETS)
				zlog_debug("bhHEsDzCblBcgObI ""%s: %s Vxlan originated S,G route with no ifchannels, not adding prune to compound message",
			...
			if (child->rpf.source_nexthop.interface &&
				...
				PIM_UPSTREAM_FLAG_SET_SEND_SG_RPT_PRUNE(
					child->flags);
				if (PIM_DEBUG_PIM_PACKETS)
					zlog_debug("8rFSeS0iQcBe1VC6 ""%s: SPT Bit and RPF'(%s) != RPF'(S,G): Add Prune (%s,rpt) to compound message",
						   __func__, up->sg_str,
						   child->sg_str);
			} else if (PIM_DEBUG_PIM_PACKETS)
				zlog_debug("c0vUXdtRpdeoUrFS ""%s: SPT Bit and RPF'(%s) == RPF'(S,G): Not adding Prune for (%s,rpt)",
			...
			if (PIM_DEBUG_PIM_PACKETS)
				zlog_debug("MVE7pE3OEu7jjMC9 ""%s: inherited_olist(%s,rpt) is NULL, Add Prune to compound message",
					   __func__, child->sg_str);
		} else if (PIM_DEBUG_PIM_PACKETS)
			zlog_debug("OnG3jqPTWuDMQUNZ ""%s: Do not add Prune %s to compound message %s",
