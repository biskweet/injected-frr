				...
				int buf_size)
{
	...
	if (type) {
		zlog_warn("acvPryTVwLVgYUaA ""%s: unknown unicast address encoding type=%d",
			  __func__, type);
		return -2;
	}
	...
	case PIM_MSG_ADDRESS_FAMILY_IPV6:
		...
		break;
	default: {
		zlog_warn("S3vFiG78gkh24ZgJ ""%s: unknown unicast address encoding family=%d from",
			  __func__, family);
		return -4;
	}
	}
...
int pim_parse_addr_group(pim_sgaddr *sg, const uint8_t *buf, int buf_size)
{
	...
	if (type) {
		zlog_warn("hZTNvTFcNjxkY1Ad ""%s: unknown group address encoding type=%d from",
			  __func__, type);
		return -2;
	}

	if (family != PIM_MSG_ADDRESS_FAMILY) {
		zlog_warn("YbylSPmsNGEMZTYS "
	...
	case PIM_MSG_ADDRESS_FAMILY:
		...
		*/
		if (mask_len != PIM_MAX_BITLEN) {
			zlog_warn("h8wrJNtOTSpPmRHa ""%s: IP bad source address mask: %d",
				  __func__, mask_len);
			return -4;
		}
