		     ...
		     struct in_addr from)
{
	if (PIM_DEBUG_GM_TRACE) {
		...
		zlog_debug("bCc7bHke9agkKhHM ""%s: from %s on %s", label, from_str, ifp->name);
	}
}
	...
	*(uint16_t *)(query_buf + IGMP_CHECKSUM_OFFSET) = checksum;
	...
	if (PIM_DEBUG_GM_PACKETS) {
		...
		pim_inet4_dump("<group?>", group_addr, group_str,
			       sizeof(group_str));
		zlog_debug("6q9PFcZWxzqMSHiV ""Send IGMPv2 QUERY to %s on %s for group %s",
			...
			const char *from_str, char *igmp_msg, int igmp_msg_len)
{
	...
	if (PIM_DEBUG_GM_PACKETS) {
		pim_inet4_dump("<dst?>", group_addr, group_str,
			       sizeof(group_str));
		zlog_debug("DrcoDqJ3nOO5QIvt ""Recv IGMPv2 REPORT from %s on %s for %s", from_str,
			   ifp->name, group_str);
	}
		       ...
		       const char *from_str, char *igmp_msg, int igmp_msg_len)
{
	...
	if (PIM_DEBUG_GM_PACKETS) {
		pim_inet4_dump("<dst?>", group_addr, group_str,
			       sizeof(group_str));
		zlog_debug("RpL61FWc5l0cb4be ""Recv IGMPv2 LEAVE from %s on %s for %s", from_str,
			   ifp->name, group_str);
	}
	/*
