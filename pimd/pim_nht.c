			   ...
			   struct pim_nexthop_cache *pnc, int command)
{
	...
	if (ret == ZCLIENT_SEND_FAILURE)
		zlog_warn("tzdcSwGPkLLqmLIS ""sendmsg_nexthop: zclient_send_message() failed");

	if (PIM_DEBUG_PIM_NHT)
		zlog_debug("YA2ikWnnBR1bur2i "
				...
				struct pim_upstream *up, struct rp_info *rp)
{
	...
	pnc = hash_lookup(pim->rpf_hash, &lookup);
	if (!pnc) {
		zlog_warn("By172Ux1VBrMYY2m ""attempting to delete nonexistent NHT entry %pPA",
			  &addr);
		return;
	}
...
void pim_nht_bsr_del(struct pim_instance *pim, pim_addr addr)
{
	...
	if (!pnc) {
		zlog_warn("8ww6xfHWZiU8aPM2 ""attempting to delete nonexistent NHT BSR entry %pPA",
			  &addr);
		return;
	}
...
static int pim_update_upstream_nh_helper(struct hash_bucket *bucket, void *arg)
{
	...
	if (PIM_DEBUG_PIM_NHT) {
		zlog_debug("FNZK0aHVnz9QvFba ""%s: NHT upstream %s(%s) old ifp %s new ifp %s rpf_result: %d",
			...
			struct zapi_route *nhr)
{
...
#endif
				zlog_debug("KuxG5VhbwLrGIpES ""%s: NHT addr %pFX(%s) %d-nhop via %pPA(%s) type %d distance:%u metric:%u ",
	...
	if (PIM_DEBUG_PIM_NHT)
		zlog_debug("7WQAecbed3YZCB6Y ""%s: NHT Update for %pFX(%s) num_nh %d num_pim_nh %d vrf:%u up %ld rp %d",
			    ...
			    struct prefix *grp, int neighbor_needed)
{
	...
	if (PIM_DEBUG_PIM_NHT_DETAIL)
		zlog_debug("zl9rGx1EHOZ4cvtQ ""%s: Looking up: %pPA(%s), last lookup time: %lld",
	...
	if (pim->ecmp_enable) {
		...
		mod_val = hash_val % consider;
		if (PIM_DEBUG_PIM_NHT_DETAIL)
			zlog_debug("EsmCd6VMgOZLoRLB ""%s: hash_val %u mod_val %u", __func__,
				   hash_val, mod_val);
	}
	...
	while (!found && (i < num_ifindex)) {
		...
		if (neighbor_needed && !pim_if_connected_to_source(ifp, src)) {
			nbr = nbrs[i];
			if (PIM_DEBUG_PIM_NHT_DETAIL)
				zlog_debug("am42U7oFl7S2b43M ""ifp name: %s(%s), pim nbr: %p",
					   ifp->name, pim->vrf->name, nbr);
			if (!nbr && !if_is_loopback(ifp)) {
				if (i == mod_val)
					mod_val++;
				if (PIM_DEBUG_PIM_NHT)
					zlog_debug("yAXyqjerox71GQJr "
