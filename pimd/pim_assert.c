...
static void on_trace(const char *label, struct interface *ifp, pim_addr src)
{
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("hKfFVo0wxBO6ckry ""%s: from %pPAs on %s", label, &src, ifp->name);
}
			 ...
			 struct pim_assert_metric metric)
{
	...
	if (!ifp) {
		if (PIM_DEBUG_PIM_TRACE)
			zlog_debug("7GVLbvJrOAF4UC0S ""%s: channel%s has no associated interface!",
				   __func__, ch->sg_str);
		return -1;
	}
	pim_ifp = ifp->info;
	...
	*/
	...
	if (PIM_DEBUG_PIM_TRACE) {
		zlog_debug("W4CRuUIuEBaq5XMC ""%s: to %s: (S,G)=%s pref=%u metric=%u rpt_bit=%u",
	...
	if (pim_msg_send(pim_ifp->pim_sock_fd, pim_ifp->primary_address,
			 qpim_all_pim_routers_addr, pim_msg, pim_msg_size,
			 ifp)) {
		zlog_warn("4m7uo2zDtOisueIn ""%s: could not send PIM message on interface %s",
			  __func__, ifp->name);
		return -3;
	}
...
static void on_assert_timer(struct event *t)
{
	...
	if (PIM_DEBUG_PIM_TRACE) {
		zlog_debug("jusmCSzCHjynG3H8 ""%s: (S,G)=%s timer expired on interface %s",
			   __func__, ch->sg_str, ifp->name);
	}
...
static void pim_assert_timer_set(struct pim_ifchannel *ch, int interval)
{
	...
	if (PIM_DEBUG_PIM_TRACE) {
		zlog_debug("NF7NsljNaknqTENC ""%s: (S,G)=%s starting %u sec timer on interface %s",
...
int assert_action_a1(struct pim_ifchannel *ch)
{
	...
	pim_ifp = ifp->info;
	if (!pim_ifp) {
		zlog_warn("adGghGq4vy4CCghD ""%s: (S,G)=%s multicast not enabled on interface %s",
			  __func__, ch->sg_str, ifp->name);
...
static int assert_action_a3(struct pim_ifchannel *ch)
{
	...
	if (pim_assert_send(ch)) {
		zlog_warn("4s3QtvhBj7lZZEHG ""%s: (S,G)=%s failure sending assert on interface %s",
			  __func__, ch->sg_str, ch->interface->name);
		return -1;
	}
...
void assert_action_a4(struct pim_ifchannel *ch)
{
	if (pim_assert_cancel(ch)) {
		zlog_warn("mJwb9BHYX7Ja1iES ""%s: failure sending AssertCancel%s on interface %s",
			  __func__, ch->sg_str, ch->interface->name);
		/* log warning only */
	}
