...
void pim_register_join(struct pim_upstream *up)
{
	...
	if (pim_is_grp_ssm(pim, up->sg.grp)) {
		if (PIM_DEBUG_PIM_EVENTS)
			zlog_debug("lqEmrnL69XYWBxeG ""%s register setup skipped as group is SSM",
				   up->sg_str);
		return;
	}
			    ...
			    pim_addr originator)
{
	...
	if (PIM_DEBUG_PIM_REG) {
		zlog_debug("wXXlBpxFvri5il7u ""Sending Register stop for %pSG to %pPA on %s", sg,
			   &originator, ifp->name);
	}
...
int pim_register_stop_recv(struct interface *ifp, uint8_t *buf, int buf_size)
{
	...
	if (wrong_af) {
		zlog_err("TZMo8u2pWDPoSBWu ""invalid AF in Register-Stop on %s", ifp->name);
		return -1;
	}


	if (PIM_DEBUG_PIM_REG)
		zlog_debug("plPbOhex1Z9YECYo ""Received Register stop for %pSG", &sg);
	...
	if (up) {
		...
		for (ALL_LIST_ELEMENTS_RO(up->sources, up_node, child)) {
			if (PIM_DEBUG_PIM_REG)
				zlog_debug("rrvwUbBs2kTjjPKm ""Executing Reg stop for %s",
					   child->sg_str);
		...
		if (PIM_DEBUG_PIM_REG)
			zlog_debug("ZcYHMIw3xxblEjdT ""Executing Reg stop for %s", up->sg_str);
		...
		frr_each (rb_pim_upstream, &pim->upstream_head, up) {
			if (pim_addr_cmp(up->sg.grp, sg.grp) == 0) {
				if (PIM_DEBUG_PIM_REG)
					zlog_debug("WKAf1Xyl5AszvgHM ""Executing Reg stop for %s",
						   up->sg_str);
				pim_reg_stop_upstream(pim, up);
			}
		}
...
struct in6_addr pim_register_get_unicast_v6_addr(struct pim_interface *p_ifp)
{
	...
	zlog_warn("uf9Lpx5CPrb8b9k7 ""No global address found for use to send register message");
	return PIMADDR_ANY;
}
#endif
		       ...
		       struct pim_upstream *up)
{
	...
	if (PIM_DEBUG_PIM_REG) {
		zlog_debug("Wb1YgKHEp6YhisoH ""Sending %s %sRegister Packet to %pPA", up->sg_str,
	...
	if (!ifp) {
		if (PIM_DEBUG_PIM_REG)
			zlog_debug("MEKcY3tfWQXsGfgE ""%s: No interface to transmit register on",
				   __func__);
		return;
	}
	pinfo = (struct pim_interface *)ifp->info;
	...
	if (PIM_DEBUG_PIM_REG) {
		zlog_debug("Re7qovYHfrp3jTeJ ""%s: Sending %s %sRegister Packet to %pPA on %s",
...
#define PIM_MSG_REGISTER_BIT_RESERVED_LEN 4
	...
	rpg = RP(pim, sg.grp);
	if (!rpg) {
		zlog_warn("keWsXDnzkJbYnO0A ""%s: Received Register Message %pSG from %pPA on %s where the RP could not be looked up",
	...
	if (i_am_rp && (!pim_addr_cmp(dest_addr, rp_addr))) {
		...
		if (!upstream) {
			upstream = pim_upstream_add(
				...
				NULL);
			if (!upstream) {
				zlog_warn("NHrhLEWcwn6OoFiG ""Failure to create upstream state");
				return 1;
			}
		...
		if ((upstream->sptbit == PIM_UPSTREAM_SPTBIT_TRUE)
			...
			if (PIM_DEBUG_PIM_REG)
				zlog_debug("rnd1AOTFfnkQU6QB ""(%s) sptbit: %d", upstream->sg_str,
					   upstream->sptbit);
		}
		if ((upstream->sptbit == PIM_UPSTREAM_SPTBIT_TRUE)
		    || (SwitchToSptDesiredOnRp(pim, &sg))) {
		...
		if (PIM_DEBUG_PIM_REG) {
			if (!i_am_rp)
				zlog_debug("qhwzh77LxPdIapgR ""Received Register packet for %pSG, Rejecting packet because I am not the RP configured for group",
					   &sg);
			else
				zlog_debug("gOHZID5C0oxhSO3w ""Received Register packet for %pSG, Rejecting packet because the dst ip address is not the actual RP",
					   &sg);
		}
		pim_register_stop_send(ifp, &sg, dest_addr, src_addr);
	}
