...
static void gm_sg_update(struct gm_sg *sg, bool has_expired)
{
	...
	if (desired != sg->state && !gm_ifp->stopping) {
		if (PIM_DEBUG_GM_EVENTS)
			zlog_debug("rzwBR1SIqfTuB3NQ "log_sg(sg, "%s => %s"), gm_states[sg->state],
				   gm_states[desired]);
	...
	if (new_join && !sg->tib_joined) {
		...
		sg->tib_joined = tib_sg_gm_join(gm_ifp->pim, sg->sgaddr,
						gm_ifp->ifp, &sg->oil);
		if (!sg->tib_joined)
			zlog_warn("b1v5tX4hgc6BFJTp "
				"MLD join for %pSG%%%s not propagated into TIB",
				&sg->sgaddr, gm_ifp->ifp->name);
		else
			zlog_info("GiHZBiebRdDaS7Tv "log_ifp("%pSG%%%s TIB joined"), &sg->sgaddr,
				  gm_ifp->ifp->name);
	...
	if (desired == GM_SG_NOINFO) {
		...
		if (PIM_DEBUG_GM_TRACE)
			zlog_debug("4PZvoZq1z6kDOiHn "log_sg(sg, "dropping"));
...
static void gm_packet_drop(struct gm_packet_state *pkt, bool trace)
{
	for (size_t i = 0; i < pkt->n_sg; i++) {
		...
		if (trace && PIM_DEBUG_GM_TRACE)
			zlog_debug("1rjHftex1XQ6lUHU "log_sg(sg, "general-dropping from %pPA"),
				   &pkt->subscriber->addr);
...
static void gm_sg_expiry_cancel(struct gm_sg *sg)
{
	if (sg->t_sg_expire && PIM_DEBUG_GM_TRACE)
		zlog_debug("T5IoZiqJM38ZopOl "log_sg(sg, "alive, cancelling expiry timer"));
	EVENT_OFF(sg->t_sg_expire);
	sg->query_sbit = true;
}
				...
				size_t len)
{
	...
	if (len < sizeof(*hdr)) {
		if (PIM_DEBUG_GM_PACKETS)
			zlog_debug("cOJsgk0C5Dmbabgd "log_pkt_src(
	...
	if (n_records > len / sizeof(struct mld_v2_rec_hdr)) {
		 ...
		 */
		zlog_warn("rEI3amwCoTckHwwv "log_pkt_src(
	...
	for (i = 0; i < n_records; i++) {
		...
		if (len < sizeof(*rechdr)) {
			zlog_warn("fWjtRCpYKOXlP2qf "log_pkt_src(
				...
				size_t len)
{
	...
	if (len < sizeof(*hdr)) {
		if (PIM_DEBUG_GM_PACKETS)
			zlog_debug("brLA7O4OF2YVWoCm "log_pkt_src(
			       ...
			       size_t len)
{
	...
	if (len < sizeof(*hdr)) {
		if (PIM_DEBUG_GM_PACKETS)
			zlog_debug("2q7BxvIT2Al4EgB6 "log_pkt_src(
...
static void gm_t_expire(struct event *t)
{
	...
	zlog_info("Dk42OlLXKdyu3lye "log_ifp("general expiry timer"));
	...
	while (gm_ifp->n_pending) {
		...
		while ((pkt = gm_packet_expires_first(gm_ifp->expires))) {
			...
			if (PIM_DEBUG_GM_PACKETS)
				zlog_debug("vvQCgCG7wUMcfLai "log_ifp("expire packet %p"), pkt);
	...
	if (PIM_DEBUG_GM_EVENTS)
		zlog_debug("cWoFkGVyJcEOQv5q "log_ifp("next general expiry waiting for query"));
}
			      ...
			      struct timeval expire_wait)
{
	...
	if (PIM_DEBUG_GM_TRACE)
		zlog_debug("DOnIGRx4UG2zhPqE "log_sg(sg, "expiring in %pTVI"), &expire_wait);
...
static void gm_t_grp_expire(struct event *t)
{
	...
	if (PIM_DEBUG_GM_EVENTS)
		zlog_debug("oKvYLC3HspSXpfQR "log_ifp("*,%pPAs S,G timer expired"), &pend->grp);
			      ...
			      struct gm_query_timers *timers, pim_addr grp)
{
	...
	if (pim_addr_is_any(sg->sgaddr.src)) {
		...
		if (PIM_DEBUG_GM_TRACE)
			zlog_debug("zYEUnNsfWGGH4Vqf "log_ifp("*,%pPAs expiry timer starting"),
				   &grp);
		GM_UPDATE_SG_STATE(sg);
		gm_sg_timer_start(gm_ifp, sg, timers->expire_wait);
	...
	if (PIM_DEBUG_GM_TRACE)
		zlog_debug("9pMbH33JhQyjzpZx "log_ifp("*,%pPAs S,G timer started: %pTHD"), &grp,
			   pend->t_expire);
}
...
static void gm_t_other_querier(struct event *t)
{
	...
	zlog_info("COxW9gyh45O3WfuF "log_ifp("other querier timer expired"));
			    ...
			    pim_addr *pkt_dst, char *data, size_t len)
{
	...
	if (len < sizeof(struct mld_v2_query_hdr) &&
		...
		zlog_warn("vTbBXPDftvwx6OSd "log_pkt_src("invalid query size"));
		gm_ifp->stats.rx_drop_malformed++;
		return;
	}
	...
	if (!general_query && !IN6_IS_ADDR_MULTICAST(&hdr->grp)) {
		zlog_warn("MHJGVrobiM2MIFth "log_pkt_src(
	...
	if (len >= sizeof(struct mld_v2_query_hdr)) {
		...
		if (len < sizeof(struct mld_v2_query_hdr) + src_space) {
			zlog_warn("wToR7RBGIHIrnwt3 "log_pkt_src(
		...
		if (general_query && src_space) {
			zlog_warn("C6chfymW4FGMaQZu "log_pkt_src(
...
static void gm_t_recv(struct event *t)
{
	...
	if (nread <= 0) {
		zlog_err("3zI6JeM4e4I743ri ""(VRF %s) RX error: %m", pim->vrf->name);
		pim->gm_rx_drop_sys++;
		return;
	}
	...
	if (nread <= 0) {
		zlog_err("FUSomKbna7wc4Abf ""(VRF %s) RX error: %m", pim->vrf->name);
		pim->gm_rx_drop_sys++;
		goto out_free;
	}
	...
	if (!pktinfo || !hoplimit) {
		zlog_err("FQrHI5obbzgxK1qe "log_ifp(
	...
	if (*hoplimit != 1) {
		zlog_err("WDMjsNSOWHG7ytKD "log_pkt_src("packet with hop limit != 1"));
	...
	if (!ip6_check_hopopts_ra(hopopts, hopopt_len, IP6_ALERT_MLD)) {
		zlog_err("dZuMBgKe7yBTRpMI "log_pkt_src(
	...
	if (!IN6_IS_ADDR_LINKLOCAL(&pkt_src->sin6_addr)) {
		zlog_warn("TiYxrwouoLKGFbL5 "log_pkt_src("packet from invalid source address"));
		gm_ifp->stats.rx_drop_srcaddr++;
		goto out_free;
	}
	...
	if (pktlen < sizeof(struct icmp6_plain_hdr)) {
		zlog_warn("2fimMBQ4XGBr6Klf "log_pkt_src("truncated packet"));
		gm_ifp->stats.rx_drop_malformed++;
		goto out_free;
	}
			  ...
			  const pim_addr *srcs, size_t n_srcs, bool s_bit)
{
	...
	*dp++ = 0;		     /* pad0 */
	...
	if (ret != expect_ret) {
		zlog_warn("xcKNA4cTVpD7cosK "log_ifp("failed to send query: %m"));
...
static void gm_trigger_specific(struct gm_sg *sg)
{
	...
	if (PIM_DEBUG_GM_TRACE)
		zlog_debug("nLKq4aLhq5vnB9gv "log_sg(sg, "triggered query"));
...
static void gm_vrf_socket_incref(struct pim_instance *pim)
{
	...
	frr_with_privs (&pimd_privs) {
		pim->gm_socket = vrf_socket(AF_INET6, SOCK_RAW, IPPROTO_ICMPV6,
					    vrf->vrf_id, vrf->name);
		if (pim->gm_socket < 0) {
			zlog_err("MwLLB27itLSPopxG ""(VRF %s) could not create MLD socket: %m",
				 vrf->name);
			return;
		}
		...
		if (ret)
			zlog_err("VDjplNGiNYEcPDsV ""(VRF %s) failed to set ICMP6_FILTER: %m",
				 vrf->name);
		...
		if (ret)
			zlog_err("t1NOOFXOCo5NBvRY ""(VRF %s) failed to set IPV6_RECVPKTINFO: %m",
				 vrf->name);
		...
		ret = setsockopt(pim->gm_socket, SOL_IPV6, IPV6_RECVHOPOPTS,
				 &intval, sizeof(intval));
		if (ret)
			zlog_err("lp94GtmWgSwGMuXb ""(VRF %s) failed to set IPV6_HOPOPTS: %m",
				 vrf->name);
		...
		ret = setsockopt(pim->gm_socket, SOL_IPV6, IPV6_RECVHOPLIMIT,
				 &intval, sizeof(intval));
		if (ret)
			zlog_err("KWrBTTDfluzQ8B7T ""(VRF %s) failed to set IPV6_HOPLIMIT: %m",
				 vrf->name);
...
static void gm_start(struct interface *ifp)
{
	...
	zlog_info("5dRckJcU3IMQA0JB "log_ifp("starting MLD"));
	...
	frr_with_privs (&pimd_privs) {
		...
		if (ret)
			zlog_err("qQq1v2yubA5G2UiR ""(%s) failed to join ff02::16 (all-MLDv2): %m",
				 ifp->name);
	}
}
...
void gm_ifp_teardown(struct interface *ifp)
{
	...
	if (PIM_DEBUG_GM_EVENTS)
		zlog_debug("2naAL48uZ5kqE4Zv "log_ifp("MLD stop"));
...
static void gm_update_ll(struct interface *ifp)
{
	...
	if (pim_addr_is_any(gm_ifp->cur_ll_lowest)) {
		if (was_querier)
			zlog_info("2NV89we70ezOWQ4a "log_ifp(
				"lost link-local address, stopping querier"));
		return;
	}

	if (was_querier)
		zlog_info("s6ssWyy1LILnHtYN "log_ifp("new link-local %pPA while querier"),
			  &gm_ifp->cur_ll_lowest);
	else if (IPV6_ADDR_CMP(&gm_ifp->cur_ll_lowest, &gm_ifp->querier) < 0 ||
		 pim_addr_is_any(gm_ifp->querier)) {
		zlog_info("obaTT8SDixjgeJkj "log_ifp("new link-local %pPA, becoming querier"),
			  &gm_ifp->cur_ll_lowest);
...
void gm_ifp_update(struct interface *ifp)
{
	...
	if (changed) {
		if (PIM_DEBUG_GM_TRACE)
			zlog_debug("CTQaYsvqnmXsmIs1 "log_ifp(
