...
void pim_rpf_set_refresh_time(struct pim_instance *pim)
{
	...
	if (PIM_DEBUG_PIM_TRACE_DETAIL)
		zlog_debug("JWjT78T7ju3m17fl ""%s: vrf(%s) New last route change time: %" PRId64,
			...
			pim_addr addr, int neighbor_needed)
{
	...
	while (!found && (i < num_ifindex)) {
		...
		if (!pim_ifp || !pim_ifp->pim_enable) {
			...
			if (PIM_DEBUG_PIM_TRACE_DETAIL)
				zlog_debug("9Fm4jzK35Tg27Zav ""ifp name: %s, pim nbr: %p",
					   ifp->name, nbr);
		...
		const char *caller)
{
	...
	if (pim_addr_is_any(up->upstream_addr)) {
		zlog_debug("aK07JDTtpZeTJdwZ ""%s(%s): RP is not configured yet for %s",
	...
	if (pim_rpf_addr_is_inaddr_any(rpf) && PIM_DEBUG_ZEBRA) {
		/* RPF'(S,G) not found */
		zlog_debug("Xw7UYnDfN1t3xtct ""%s(%s): RPF'%s not found: won't send join upstream",
			   __func__, caller, up->sg_str);
		/* warning only */
	}
	...
	if (nexthop_mismatch(&rpf->source_nexthop, &saved.source_nexthop)) {
		...
		if (PIM_DEBUG_ZEBRA)
			zlog_debug("UJc6nOTxXmVibFvw ""%s(%s): (S,G)=%s source nexthop now is: interface=%s address=%pPAs pref=%d metric=%d",
		 __func__, caller,
		 up->sg_str,
	...
	if (saved.source_nexthop.interface != rpf->source_nexthop.interface) {
		...
		if (PIM_DEBUG_ZEBRA) {
			zlog_debug("Bt7j25vFHgc77yFv ""%s(%s): (S,G)=%s RPF_interface(S) changed from %s to %s",
		 __func__, caller,
		 up->sg_str,
...
static pim_addr pim_rpf_find_rpf_addr(struct pim_upstream *up)
{
	...
	if (!up->rpf.source_nexthop.interface) {
		zlog_warn("5hO0I6A9X4BrE41H ""%s: missing RPF interface for upstream (S,G)=%s",
			  __func__, up->sg_str);
