...
static void zclient_lookup_connect(struct event *t)
{
	...
	if (zclient_socket_connect(zlookup) < 0) {
		++zlookup->fail;
		zlog_warn("J7nawizRXQf5mvHp ""%s: failure connecting zclient socket: failures=%d",
	...
	if (zclient_send_hello(zlookup) == ZCLIENT_SEND_FAILURE) {
		if (close(zlookup->sock)) {
			zlog_warn("NKRlIBCvuRwPSWDW ""%s: closing fd=%d: errno=%d %s", __func__,
...
static void zclient_lookup_sched(struct zclient *zlookup, int delay)
{
	...
	zlog_notice("O4xpXenuzZb330of ""%s: zclient lookup connection scheduled for %d seconds",
		    __func__, delay);
}
...
static void zclient_lookup_sched_now(struct zclient *zlookup)
{
	...
	zlog_notice("rPf515O0SZ4VDGjf ""%s: zclient lookup immediate connection scheduled",
		    __func__);
}
...
static void zclient_lookup_failed(struct zclient *zlookup)
{
	if (zlookup->sock >= 0) {
		if (close(zlookup->sock)) {
			zlog_warn("YSxqlj28lNOusf4D ""%s: closing fd=%d: errno=%d %s", __func__,
				  zlookup->sock, errno, safe_strerror(errno));
		}
		zlookup->sock = -1;
	}
...
void zclient_lookup_new(void)
{
	...
	if (!zlookup) {
		flog_err(EC_LIB_ZAPI_SOCKET,"0iSX2fiOFxDiPK9G " "%s: zclient_new() failure",
			 __func__);
		return;
	}
	...
	zlog_notice("soHSODNet2bRPsii ""%s: zclient lookup socket initialized", __func__);
}
				...
				const int tab_size, pim_addr addr)
{
	...
	if (PIM_DEBUG_PIM_NHT_DETAIL)
		zlog_debug("Vn2PXPa3SpFsMtU4 ""%s: addr=%pPAs(%s)", __func__, &addr,
			   pim->vrf->name);
	...
	while (command != ZEBRA_NEXTHOP_LOOKUP_MRIB) {
		...
		if (err < 0) {
			flog_err(EC_LIB_ZAPI_MISSMATCH,"nkmbyzncXxMT1vX0 "
	...
	if (raddr.ipa_type != PIM_IPADDR ||
		...
		zlog_warn("7UpUlgD3XTDEFA6b ""%s: address mismatch: addr=%pPA(%s) raddr=%pIA",
	...
	if (nexthop_num < 1 || nexthop_num > router->multipath) {
		if (PIM_DEBUG_PIM_NHT_DETAIL)
			zlog_debug("A5cd8kplmep650g4 ""%s: socket %d bad nexthop_num=%d", __func__,
				       ...
				       const int tab_size, pim_addr addr)
{
	...
	if (PIM_DEBUG_PIM_NHT_DETAIL)
		zlog_debug("mOWZGArPCpa5vWWY ""%s: addr=%pPAs(%s)", __func__, &addr,
			   pim->vrf->name);

	/* Check socket. */
	if (zlookup->sock < 0) {
		flog_err(EC_LIB_ZAPI_SOCKET,"3j5c6ToH0rhfkpJu "
	...
	if (ret < 0) {
		flog_err(
			EC_LIB_SOCKET,"McjUSJpykMwztC1u "
	...
	if (ret == 0) {
		flog_err_sys(EC_LIB_SOCKET,"TknPVMvePuu07uBc "
...
void zclient_lookup_read_pipe(struct event *thread)
{
	...
	if (!pim) {
		if (PIM_DEBUG_PIM_NHT_DETAIL)
			zlog_debug("TET8KMQ5Gpv0vhRM ""%s: Unable to find pim instance", __func__);
		return;
	}
...
int pim_zlookup_sg_statistics(struct channel_oil *c_oil)
{
	...
	if (PIM_DEBUG_ZEBRA) {
		...
		zlog_debug("Fy3UO4CAUcqmCex6 ""Sending Request for New Channel Oil Information%pSG VIIF %d(%s:%s)",
	...
	if (ret <= 0) {
		flog_err(
			EC_LIB_SOCKET,"TSIgjEMLdmGJenvB "
			"%s: writen() failure: %d writing to zclient lookup socket",
			__func__, errno);
		return -1;
	}
	...
	while (command != ZEBRA_IPMR_ROUTE_STATS) {
		...
		err = zclient_read_header(s, zlookup->sock, &length, &marker,
					  &version, &vrf_id, &command);
		if (err < 0) {
			flog_err(EC_LIB_ZAPI_MISSMATCH,"YeljmSrq7kk9U7MR "
				 "%s: zclient_read_header() failed", __func__);
			zclient_lookup_failed(zlookup);
			return -1;
		}
	...
	if (pim_sgaddr_cmp(sg, more)) {
		if (PIM_DEBUG_ZEBRA)
			flog_err(
				EC_LIB_ZAPI_MISSMATCH,"LOEIBNYMXyXLTjhn "
