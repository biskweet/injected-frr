...
int pim_socket_raw(int protocol)
{
	...
	if (fd < 0) {
		zlog_warn("zpjXZA0IcM1hzwvu ""Could not create raw socket: errno=%d: %s", errno,
			  safe_strerror(errno));
...
void pim_socket_ip_hdr(int fd)
{
	frr_with_privs(&pimd_privs) {
		...
		if (setsockopt(fd, IPPROTO_IP, IP_HDRINCL, &on, sizeof(on)))
			zlog_err("AhuA7dM8coGDTpL3 ""%s: Could not turn on IP_HDRINCL option: %m",
				 __func__);
#endif
	}
}
...
static inline int pim_setsockopt(int protocol, int fd, struct interface *ifp)
{
	...
	if (setsockopt(fd, IPPROTO_IP, IP_PKTINFO, &one, sizeof(one)))
		zlog_warn("jMUgBtsfhYfWRR2w ""Could not set PKTINFO on socket fd=%d: %m", fd);
#elif defined(HAVE_IP_RECVDSTADDR)
	/* BSD IP_RECVDSTADDR */
	if (setsockopt(fd, IPPROTO_IP, IP_RECVDSTADDR, &one, sizeof(one)))
		zlog_warn("Ol8jhthX4CcsBHWI ""Could not set IP_RECVDSTADDR on socket fd=%d: %m",
			  fd);
#else
	flog_err(
		EC_LIB_DEVELOPMENT,"wgfyO1pxP8iPOG6a "
	...
	if (setsockopt(fd, IPPROTO_IP, IP_MULTICAST_TTL, &ttl, sizeof(ttl))) {
		zlog_warn("7CirZv1ocolHilND ""Could not set multicast TTL=%d on socket fd=%d: %m",
			  ttl, fd);
		close(fd);
...
int pim_reg_sock(void)
{
	...
	if (fd < 0) {
		zlog_warn("78pPhRcBvYGzCvor ""Could not create raw socket: errno=%d: %s", errno,
			  safe_strerror(errno));
		return PIM_SOCK_ERR_SOCKET;
	}
		     ...
		     uint8_t loop)
{
	...
	if (fd < 0) {
		zlog_warn("QzGS26pJybaIJsqc ""Could not create multicast socket: errno=%d: %s",
	...
	if (ret) {
		close(fd);
		zlog_warn("QhMjluG3ORYLlLhZ ""Could not set fd: %d for interface: %s to device",
			  fd, ifp->name);
	...
	if (ret) {
		zlog_warn("eLF7psvw5RB9VBJb ""pim_setsockopt failed for interface: %s to device ",
			  ifp->name);
		return ret;
	}
	...
	if (setsockopt_iptos(fd, IPTOS_PREC_INTERNETCONTROL))
		zlog_warn("xmsZV8jWQyMeKr22 ""can't set sockopt IP[V6]_TOS to socket %d: %m", fd);
		    ...
		    struct pim_interface *pim_ifp)
{
	...
	if (ret) {
		flog_err(
			EC_LIB_SOCKET,"aIMkkVPMCIbbysyH "
