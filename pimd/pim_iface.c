					 ...
					 const char *caller)
{
	...
	if (PIM_DEBUG_ZEBRA)
		zlog_debug("3BeOiDJE4ZPTD8qx ""%s: old=%pPA new=%pPA on interface %s: %s",
...
void pim_if_addr_add(struct connected *ifc)
{
	...
	if (PIM_DEBUG_ZEBRA)
		zlog_debug("IZv50HyP91BmOFVw ""%s: %s ifindex=%d connected IP address %pFX %s",
...
void pim_if_addr_del(struct connected *ifc, int force_prim_as_any)
{
	...
	if (PIM_DEBUG_ZEBRA)
		zlog_debug("dNt2RD9zrl6WFrg6 ""%s: %s ifindex=%d disconnected IP address %pFX %s",
			   __func__, ifp->name, ifp->ifindex, ifc->address,
			   CHECK_FLAG(ifc->flags, ZEBRA_IFA_SECONDARY)
				   ? "secondary"
				   : "primary");
...
void pim_if_addr_add_all(struct interface *ifp)
{
	...
	if (!v4_addrs && v6_addrs && !if_is_loopback(ifp) &&
		...
		/* Interface has a valid socket ? */
		zlog_warn("wDR2f9WWjQOzTIQn ""Failure creating PIM socket for interface %s",
			  ifp->name);
	}
	/*
...
int pim_if_add_vif(struct interface *ifp, bool ispimreg, bool is_vxlan_term)
{
	...
	if (pim_ifp->mroute_vif_index > 0) {
		zlog_warn("CJfQ7VamdXe3Pxbz ""%s: vif_index=%d > 0 on interface %s ifindex=%d",
	...
	if (ifp->ifindex < 0) {
		zlog_warn("feWINyVwM2bZwGOC ""%s: ifindex=%d < 0 on interface %s", __func__,
			  ifp->ifindex, ifp->name);
		return -2;
	} else if ((ifp->ifindex == PIM_OIF_PIM_REGISTER_VIF) &&
		   ((strncmp(ifp->name, "pimreg", 6)) &&
		    (strncmp(ifp->name, "pim6reg", 7)))) {
		zlog_warn("tbCoGnqlyfnEeNnP ""%s: ifindex=%d on interface %s", __func__,
			  ifp->ifindex, ifp->name);
		return -2;
	}
...
int pim_if_del_vif(struct interface *ifp)
{
	...
	if (pim_ifp->mroute_vif_index < 1) {
		zlog_warn("CQV7rKs7psecb7aH ""%s: vif_index=%d < 1 on interface %s ifindex=%d",
			  __func__, pim_ifp->mroute_vif_index, ifp->name,
			  ifp->ifindex);
		return -1;
	}
...
struct pim_neighbor *pim_if_find_neighbor(struct interface *ifp, pim_addr addr)
{
	...
	pim_ifp = ifp->info;
	if (!pim_ifp) {
		zlog_warn("KoG83HpzqXNN5Cdw ""%s: multicast not enabled on interface %s", __func__,
			  ifp->name);
		return 0;
	}
				   ...
				   pim_addr source_addr)
{
	...
	join_fd = gm_join_sock(ifp->name, ifp->ifindex, group_addr, source_addr,
			       pim_ifp);
	if (join_fd < 0) {
		zlog_warn("cxEnk1v7CVFq8qb4 ""%s: gm_join_sock() failure for " GM
		       ...
		       pim_addr source_addr)
{
	...
	pim_ifp = ifp->info;
	if (!pim_ifp) {
		zlog_warn("LfGFGiQgN59fVGVC ""%s: multicast not enabled on interface %s", __func__,
			  ifp->name);
		return -1;
	}

	if (!pim_ifp->gm_join_list) {
		zlog_warn("2xwqbW8FTWn7dGvm ""%s: no " GM " join on interface %s", __func__,
			  ifp->name);
		return -2;
	}
	...
	ij = gm_join_find(pim_ifp->gm_join_list, group_addr, source_addr);
	if (!ij) {
		zlog_warn("D0XbeZylfC6XThoy ""%s: could not find " GM
			  " group %pPAs source %pPAs on interface %s",
			  __func__, &group_addr, &source_addr, ifp->name);
		return -3;
	}
...
static void pim_if_gm_join_del_all(struct interface *ifp)
{
	...
	pim_ifp = ifp->info;
	if (!pim_ifp) {
		zlog_warn("JSdG7QpTWjdjfi4V ""%s: multicast not enabled on interface %s", __func__,
			  ifp->name);
		return;
	}
