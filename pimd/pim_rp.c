...
void pim_rp_init(struct pim_instance *pim)
{
	...
	if (!pim_get_all_mcast_group(&rp_info->group)) {
		flog_err(EC_LIB_DEVELOPMENT,"75VevJE2vJklBuXQ "
	...
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("KEqgwoLWOURwjXBs ""Allocated: %p for rp_info: %p(%pFX) Lock: %d", rn,
					       ...
					       const struct prefix *group)
{
	...
	if (!rn) {
		flog_err(
			EC_LIB_DEVELOPMENT,"rHGsP1qh0wMUt0eA "
	...
	if (PIM_DEBUG_PIM_TRACE_DETAIL) {
		if (best)
			zlog_debug("teDyRDq4Xkwz7XQe "
				"Lookedup(%pFX): prefix_list match %s, rn %p found: %pFX",
				group, best->plist, rn, &rp_info->group);
		else
			zlog_debug("hZO1h88vCWs5aXbv ""Lookedup(%pFX): rn %p found:%pFX", group,
				   rn, &rp_info->group);
	}
...
void pim_upstream_update(struct pim_instance *pim, struct pim_upstream *up)
{
	...
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("X4hpUMWSCsRBZmut ""%s: pim upstream update for old upstream %pPA",
	       ...
	       const char *plist, enum rp_source rp_src_flag)
{
	...
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("8FI3OKYXDLHOSDvO ""Allocated: %p for rp_info: %p(%pFX) Lock: %d", rn,
			   rp_info, &rp_info->group,
	...
	if (PIM_DEBUG_PIM_NHT_RP)
		zlog_debug("KVrBxptq97HthT4A ""%s: NHT Register RP addr %pPA grp %pFX with Zebra ",
	       ...
	       const char *plist, enum rp_source rp_src_flag)
{
	...
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("Yn09FxoYNcYyapHK ""%s: Delete RP %pPA for the group %pFX", __func__,
			   &rp_addr, &group);
	...
	nht_p = rp_info->rp.rpf_addr;
	if (PIM_DEBUG_PIM_NHT_RP)
		zlog_debug("Ap6X8xtFLPd8qF3H ""%s: Deregister RP addr %pPA with Zebra ", __func__,
			   &nht_p);
	...
	if (!was_plist) {
		...
		if (rn) {
			if (rn->info != rp_info)
				flog_err(
					EC_LIB_DEVELOPMENT,"W9aWSuQqPjlIJsFE "
					"Expected rn->info to be equal to rp_info");

			if (PIM_DEBUG_PIM_TRACE)
				zlog_debug("6ZpkeP7t1PzUJVep "
		  ...
		  struct prefix group, enum rp_source rp_src_flag)
{
	...
	if (!pim_addr_is_any(old_rp_addr)) {
		nht_p = rp_info->rp.rpf_addr;
		if (PIM_DEBUG_PIM_NHT_RP)
			zlog_debug("Gun3kjMwoE8n4lKE ""%s: Deregister RP addr %pPA with Zebra ",
				   __func__, &nht_p);
		pim_delete_tracked_nexthop(pim, nht_p, NULL, rp_info);
	}
	...
	nht_p = rp_info->rp.rpf_addr;
	if (PIM_DEBUG_PIM_NHT_RP)
		zlog_debug("xWX4LfhLzKE7PHAy ""%s: NHT Register RP addr %pPA grp %pFX with Zebra ",
			   __func__, &nht_p, &rp_info->group);
...
void pim_rp_check_on_if_add(struct pim_interface *pim_ifp)
{
	...
	for (ALL_LIST_ELEMENTS_RO(pim->rp_list, node, rp_info)) {
		...
		if (pim_rp_check_interface_addrs(rp_info, pim_ifp)) {
			...
			rp_info->i_am_rp = 1;
			if (PIM_DEBUG_PIM_NHT_RP)
				zlog_debug("GOlZs9N5Uy5pxPD4 ""%s: %pPA: i am rp", __func__,
					   &rp_info->rp.rpf_addr);
		}
	}
...
void pim_i_am_rp_re_evaluate(struct pim_instance *pim)
{
	...
	for (ALL_LIST_ELEMENTS_RO(pim->rp_list, node, rp_info)) {
		...
		if (old_i_am_rp != rp_info->i_am_rp) {
			...
			if (PIM_DEBUG_PIM_NHT_RP) {
				if (rp_info->i_am_rp)
					zlog_debug("lP3ci8lkGV41X13X ""%s: %pPA: i am rp",
						   __func__,
						   &rp_info->rp.rpf_addr);
				else
					zlog_debug("x2o1yknzjypcNGDJ "
			     ...
			     pim_addr source, pim_addr group)
{
	...
	if (!rp_info || ((pim_rpf_addr_is_inaddr_any(&rp_info->rp)) &&
		...
		if (PIM_DEBUG_PIM_NHT_RP)
			zlog_debug("rbM3K8ujNPELObaO ""%s: Received a (*,G) with no RP configured",
				   __func__);
		*up = PIMADDR_ANY;
		return 0;
	}
