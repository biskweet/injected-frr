...
static void on_trace(const char *label, struct interface *ifp, pim_addr src)
{
	if (PIM_DEBUG_PIM_TRACE)
		zlog_debug("WwmsQZH1pD3a6t7u ""%s: from %pPA on %s", label, &src, ifp->name);
}
		      ...
		      uint8_t source_flags)
{
	...
	if (CHECK_FLAG(source_flags, PIM_WILDCARD_BIT_MASK)) {
		...
		if (!rp) {
			zlog_warn("yEk5NQTg700ZBgeI ""%s: Lookup of RP failed for %pSG", __func__,
				  sg);
			return;
		}
		/*
		       ...
		       uint8_t source_flags)
{
	...
	if (CHECK_FLAG(source_flags, PIM_WILDCARD_BIT_MASK)) {
		 ...
		 */
		if (PIM_DEBUG_PIM_J_P)
			zlog_debug("shGZ3RQRBAieLmQr ""%s: Prune received with RP(%pPAs) for %pSG",
				   __func__, &sg->src, sg);
		       ...
		       pim_addr src_addr, uint8_t *tlv_buf, int tlv_buf_size)
{
	...
	*/
	...
	if (addr_offset < 1) {
		zlog_warn("KuPfzOuzxJroQcpa ""%s: pim_parse_addr_ucast() failure: from %pPA on %s",
...
int pim_joinprune_send(struct pim_rpf *rpf, struct list *groups)
{
	...
	if (rpf->source_nexthop.interface)
		...
		zlog_warn("HJgIT2bpFVwEm2DG ""%s: RPF interface is not present", __func__);
		return -1;
	}
	...
	if (!pim_ifp) {
		zlog_warn("6nSZzSmdgj5RQhnK ""%s: multicast not enabled on interface %s", __func__,
