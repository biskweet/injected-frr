			 ...
			 pcep_main_event_handler_t event_handler)
{
	...
	*fpt = frr_pthread_new(&attr, "PCEP thread", "pcep_controller");
	if (*fpt == NULL) {
		flog_err(EC_PATH_SYSTEM_CALL,"Iumy9foRGWXv0R1W "
			 "failed to initialize PCEP thread");
		return 1;
	}
	ret = frr_pthread_run(*fpt, NULL);
	if (ret < 0) {
		flog_err(EC_PATH_SYSTEM_CALL,"SujonI7CeuflPxsv " "failed to create PCEP thread");
		return ret;
	}
...
void pcep_thread_timer_handler(struct event *thread)
{
	...
	case TM_UNDEFINED:
	case TM_MAX:
		flog_warn(EC_PATH_PCEP_RECOVERABLE_INTERNAL_ERROR,"5VFs0Dmx36hyGD0s "
...
void pcep_thread_event_handler(struct event *thread)
{
	...
	case EV_RESET_PCC_SESSION:
		...
		if (pcc_state) {
			...
			flog_warn(EC_PATH_PCEP_RECOVERABLE_INTERNAL_ERROR,"pPcvPTtoRpOfu4Sn "
	...
	case EV_PCEPLIB_EVENT:
		flog_warn(EC_PATH_PCEP_RECOVERABLE_INTERNAL_ERROR,"qTUrjBLzVcIKV7jV "
					 ...
					 int pcc_id, struct pce_opts *pce_opts)
{
	...
	if (pcep_pcc_update(ctrl_state, pcc_state, pcc_opts, pce_opts)) {
		flog_err(EC_PATH_PCEP_PCC_CONF_UPDATE,"LrSq4gSgaI57odf6 "
