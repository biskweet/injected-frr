...
int pcep_lib_initialize(struct frr_pthread *fpt)
{
	...
	if (!initialize_pcc_infra(&infra)) {
		flog_err(EC_PATH_PCEP_PCC_INIT,"ey3doRER4pn3fPhO " "failed to initialize pceplib");
		return 1;
	}
...
void pcep_lib_finalize(void)
{
	...
	if (!destroy_pcc()) {
		flog_err(EC_PATH_PCEP_PCC_FINI,"ar4gfh5jy208ebmE " "failed to finalize pceplib");
	}
}
...
struct path *pcep_lib_parse_path(struct pcep_message *msg)
{
	...
	for (node = objs->head; node != NULL; node = node->next_node) {
		...
		case CLASS_TYPE(PCEP_OBJ_CLASS_VENDOR_INFO,
			...
			break;
		default:
			flog_warn(EC_PATH_PCEP_UNEXPECTED_PCEP_OBJECT,"zlBh0ZY4tQ0Bovoo "
				 ...
				 struct pcep_caps *caps)
{
	...
	for (node = objs->head; node != NULL; node = node->next_node) {
		...
		case CLASS_TYPE(PCEP_OBJ_CLASS_OPEN, PCEP_OBJ_TYPE_OPEN):
			...
			break;
		default:
			flog_warn(EC_PATH_PCEP_UNEXPECTED_PCEP_OBJECT,"zNhUWUFUUx0oVa0e "
				  "Unexpected PCEP object %s (%u) / %s (%u)",
				  pcep_object_class_name(obj->object_class),
				  obj->object_class,
				  pcep_object_type_name(obj->object_class,
...
void pcep_lib_parse_open(struct pcep_caps *caps, struct pcep_object_open *open)
{
	...
	for (node = tlvs->head; node != NULL; node = node->next_node) {
		...
		case PCEP_OBJ_TYPE_CISCO_BSID:
			flog_warn(EC_PATH_PCEP_UNEXPECTED_PCEP_TLV,"eQNhMCGyTrhbQVkK "
...
void pcep_lib_parse_rp(struct path *path, struct pcep_object_rp *rp)
{
	...
	if (tlvs == NULL) {
		flog_warn(EC_PATH_PCEP_UNEXPECTED_PCEP_TLV,"tzkzj7djjO5fC6Lg "
	...
	for (node = tlvs->head; node != NULL; node = node->next_node) {
		...
		case PCEP_OBJ_TYPE_CISCO_BSID:
		case PCEP_OBJ_TLV_TYPE_PATH_SETUP_TYPE_CAPABILITY:
			flog_warn(EC_PATH_PCEP_UNEXPECTED_PCEP_TLV,"Gf53zrcVhPBg6SKR "
...
void pcep_lib_parse_srp(struct path *path, struct pcep_object_srp *srp)
{
	...
	if (tlvs == NULL) {
		flog_warn(EC_PATH_PCEP_UNEXPECTED_PCEP_TLV,"5KRIi5bg7TUYSzuG "
	...
	for (node = tlvs->head; node != NULL; node = node->next_node) {
		...
		case PCEP_OBJ_TYPE_CISCO_BSID:
		case PCEP_OBJ_TLV_TYPE_ARBITRARY:
			flog_warn(EC_PATH_PCEP_UNEXPECTED_PCEP_TLV,"SPCbNIqtM6gDijXd "
...
void pcep_lib_parse_lsp(struct path *path, struct pcep_object_lsp *lsp)
{
	...
	if (tlvs == NULL) {
		flog_warn(EC_PATH_PCEP_UNEXPECTED_PCEP_TLV,"ZTNbeXC2OZMfhOE9 "
	...
	for (node = tlvs->head; node != NULL; node = node->next_node) {
		...
		case PCEP_OBJ_TLV_TYPE_UNKNOWN:
		case PCEP_OBJ_TLV_TYPE_ARBITRARY:
			flog_warn(EC_PATH_PCEP_UNEXPECTED_PCEP_TLV,"FPnmbluy4mXqvpLk "
...
void pcep_lib_parse_ero(struct path *path, struct pcep_object_ro *ero)
{
	...
	if (objs == NULL) {
		flog_warn(EC_PATH_PCEP_UNEXPECTED_PCEP_TLV,"m27FNu6An9W5Nygh "
	...
	for (node = objs->tail; node != NULL; node = node->prev_node) {
		...
		case RO_SUBOBJ_UNKNOWN:
			flog_warn(EC_PATH_PCEP_UNEXPECTED_PCEP_ERO_SUBOBJ,"OQgKwYfcywKGq5u9 "
				       ...
				       struct pcep_ro_subobj_sr *sr)
{
	...
	*hop = (struct path_hop){.next = next,
	...
	if (!sr->flag_f) {
		...
		case PCEP_SR_SUBOBJ_NAI_UNKNOWN:
			hop->has_nai = false;
			flog_warn(EC_PATH_PCEP_UNEXPECTED_SR_NAI,"EvfR6UU09Kiheol0 "
