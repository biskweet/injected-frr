...
static int path_zebra_router_id_update(ZAPI_CALLBACK_ARGS)
{
	...
	if (pref.family == AF_INET) {
		...
		zlog_warn("LwF0wX8INVC1imn9 ""Unexpected router ID address family for vrf %u: %u",
			  vrf_id, pref.family);
		return 0;
	}
	zlog_info("oAdLH1eBRFnFTJao ""%s Router Id updated for VRF %u: %s", family, vrf_id, buf);
	return 0;
}
...
int path_zebra_request_label(mpls_label_t label)
{
	...
	if (ret < 0) {
		zlog_warn("nqxjvObpbzTmRFZR ""%s: error getting label range!", __func__);
		return -1;
	}
...
void path_zebra_release_label(mpls_label_t label)
{
	...
	if (ret < 0)
		zlog_warn("9bbSZMzy98TNQ4lY ""%s: error releasing label range!", __func__);
}
...
static void path_zebra_label_manager_connect(struct event *event)
{
	if (path_sync_client_level == PATH_SYNC_NONE) {
		...
		if (zclient_socket_connect(zclient_sync) < 0) {
			zlog_warn("4orHfDZ4Ao8zzyFz ""%s: error connecting synchronous zclient!",
				  __func__);
	...
	if (path_sync_client_level == PATH_SYNC_CONN) {
		if (zclient_send_hello(zclient_sync) == ZCLIENT_SEND_FAILURE) {
			zlog_warn("edxgt4BIrIBn2OV6 ""%s: Error sending hello for synchronous zclient!",
				  __func__);
			event_add_timer(master, path_zebra_label_manager_connect,
					NULL, 1, &t_sync_connect);
			return;
	...
	if (path_sync_client_level == PATH_SYNC_HELLO) {
		if (lm_label_manager_connect(zclient_sync, 0) != 0) {
			zlog_warn("QuB1zipaRlIeFgNY ""%s: error connecting to label manager!",
				  __func__);
			event_add_timer(master, path_zebra_label_manager_connect,
					NULL, 1, &t_sync_connect);
			return;
...
static int path_zebra_opaque_msg_handler(ZAPI_CALLBACK_ARGS)
{
	...
	case LINK_STATE_SYNC:
		...
		if (msg) {
			zlog_debug("IlgBtqlAsJegA1sY ""%s: [rcv ted] ls (%s) msg (%s)-(%s) !",
				   __func__,
		...
		break;
	default:
		zlog_debug("smocoipSnezLvwDS ""%s: [rcv ted] unknown opaque event (%d) !",
			   __func__, info.type);
		break;
	}
