...
int pcep_pcc_enable(struct ctrl_state *ctrl_state, struct pcc_state *pcc_state)
{
	...
	if (!CHECK_FLAG(pcc_state->flags, F_PCC_STATE_HAS_IPV4)) {
		if (pcc_state->retry_count < OTHER_FAMILY_MAX_RETRIES) {
			flog_warn(EC_PATH_PCEP_MISSING_SOURCE_ADDRESS,"WwjnkjnStRzu4i7A "
			...
			flog_warn(EC_PATH_PCEP_MISSING_SOURCE_ADDRESS,"TqffXdhDYFD61WBh "
	...
	if (!CHECK_FLAG(pcc_state->flags, F_PCC_STATE_HAS_IPV6)) {
		if (pcc_state->retry_count < OTHER_FAMILY_MAX_RETRIES) {
			flog_warn(EC_PATH_PCEP_MISSING_SOURCE_ADDRESS,"WC6ksakwOmKqv7iQ "
			...
			flog_warn(EC_PATH_PCEP_MISSING_SOURCE_ADDRESS,"aYUhywIWB2zonp8P "
	...
	if (pcc_state->pcc_addr_tr.ipa_type == IPADDR_NONE) {
		flog_warn(EC_PATH_PCEP_MISSING_SOURCE_ADDRESS,"jRxjKTWP6FTM0f57 "
	...
	if (pcc_state->sess == NULL) {
		flog_warn(EC_PATH_PCEP_LIB_CONNECT,"w2n6DYri6Vo3KLjM "
			      ...
			      enum pcep_ctrl_timeout_type type, void *param)
{
	...
	case TO_COMPUTATION_REQUEST:
		...
		flog_warn(EC_PATH_PCEP_COMPUTATION_REQUEST_TIMEOUT,"yQ7dqGZdKThuDXao "
				  ...
				  struct path *path)
{
	...
	case PCEP_PATH_UNDEFINED:
		flog_warn(EC_PATH_PCEP_RECOVERABLE_INTERNAL_ERROR,"welZjqgOSwIFS1xR "
				 ...
				 struct pcc_state *pcc_state, pcep_event *event)
{
	...
	case PCC_CONNECTION_FAILURE:
		flog_warn(EC_PATH_PCEP_UNEXPECTED_PCEPLIB_EVENT,"aIQrcJ6lleeLdsCs "
...
int pcep_pcc_calculate_best_pce(struct pcc_state **pcc)
{
	...
	if (step_0_best != best_pce) {
		...
		if (previous_best_pce != -1
			...
			zlog_debug("pJjLoGu8H2lc5KMr ""multi-pce: previous best pce (%i) ",
				   previous_best_pce + 1);
		}
		...
		if (best_pce != -1) {
			...
			zlog_debug("xk3Xz4rwI6TxMYmx ""multi-pce: best pce (%i) ", best_pce + 1);
		} else {
			if (one_connected_pce != -1) {
				best_pce = one_connected_pce;
				pcc[one_connected_pce]->is_best = true;
				zlog_debug("aAfYABrJv6FSlJm0 "
	...
	for (int idx = 0; idx < MAX_PCC; idx++) {
		if (pcc[idx]) {
			if ((ipaddr_cmp((const struct ipaddr *)&pcc[idx]
				...
				zlog_debug("GSxFhqFRcay7lGD1 ""found pcc_id (%d) idx (%d)",
					   pcc[idx]->id, idx);
				return pcc[idx]->id;
			}
		}
...
struct pcc_state *pcep_pcc_get_pcc_by_id(struct pcc_state **pcc, int id)
{
	...
	for (int i = 0; i < MAX_PCC; i++) {
		if (pcc[i]) {
			if (pcc[i]->id == id) {
				zlog_debug("i4oY9qn7Ea7rPQaT ""found id (%d) pcc_idx (%d)",
					   pcc[i]->id, i);
				return pcc[i];
			}
		}
...
int pcep_pcc_get_pcc_idx_by_id(struct pcc_state **pcc, int id)
{
	...
	for (int idx = 0; idx < MAX_PCC; idx++) {
		if (pcc[idx]) {
			if (pcc[idx]->id == id) {
				zlog_debug("tpwqGWv53KYJryEt ""found pcc_id (%d) array_idx (%d)",
					   pcc[idx]->id, idx);
				return idx;
			}
		}
...
int pcep_pcc_get_free_pcc_idx(struct pcc_state **pcc)
{
	...
	for (int idx = 0; idx < MAX_PCC; idx++) {
		if (pcc[idx] == NULL) {
			zlog_debug("qmQaxboLCrjeYgkq ""new pcc_idx (%d)", idx);
			return idx;
		}
	}
			 ...
			 struct pcc_state *pcc_state, struct pcep_message *msg)
{
	...
	case PCEP_TYPE_MAX:
		flog_warn(EC_PATH_PCEP_UNEXPECTED_PCEP_MESSAGE,"OJi7UUad6BVWdVsP "
			      ...
			      void *payload)
{
	...
	if (validate_incoming_path(pcc_state, path, err, sizeof(err)))
		...
		flog_warn(EC_PATH_PCEP_UNSUPPORTED_PCEP_FEATURE,"v9kgR2tEES5gJ7fC "
			      ...
			      struct pcep_message *msg)
{
	...
	if (!pcc_state->pce_opts->config_opts.pce_initiated) {
		...
		flog_warn(EC_PATH_PCEP_UNSUPPORTED_PCEP_FEATURE,"dsye13s3yN6cMr4T "
	...
	if (path->do_remove) {
		...
		if (path->first_hop == NULL /*ero sets first_hop*/) {
			 ...
			 * object missing). */
			flog_warn(EC_PATH_PCEP_UNSUPPORTED_PCEP_FEATURE,"WypYivpIeyKMP9UA "
		...
		if (path->plsp_id != 0) {
			...
			flog_warn(
				EC_PATH_PCEP_PROTOCOL_ERROR,"3xnJdNTdMrCeyZM7 "
		...
		if (path->name == NULL) {
			 ...
			 * missing) */
			flog_warn(
				EC_PATH_PCEP_PROTOCOL_ERROR,"SAszIuzBjzj45cjP "
	...
	if (validate_incoming_path(pcc_state, path, err, sizeof(err))) {
		...
		/* FIXME: Monitor the amount of errors from the PCE and
		 * possibly disconnect and blacklist */
		flog_warn(EC_PATH_PCEP_UNSUPPORTED_PCEP_FEATURE,"p24tpCjVi7BQqFjD "
			  "Unsupported PCEP protocol feature: %s", err);
		send_pcep_error(pcc_state, PCEP_ERRT_INVALID_OPERATION,
			    ...
			    struct pcep_message *msg)
{
	...
	if (path->no_path) {
		...
		/* FIXME: Monitor the amount of errors from the PCE and
		 * possibly disconnect and blacklist */
		flog_warn(EC_PATH_PCEP_UNSUPPORTED_PCEP_FEATURE,"1ppbQBnMW4tLSNdM "
			  "Unsupported PCEP protocol feature: %s", err);
	}
