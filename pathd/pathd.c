				 ...
				 struct prefix *prefix_cli, uint32_t algo)
{
	...
	if (ted_sid == MPLS_LABEL_NONE) {
		zlog_warn("7Ekp4VsuitbkUitw "" %s: PATHD-TED: SL: ERROR query C : ted-sid (%d)",
			  __func__, ted_sid);
	} else {
		PATH_TED_DEBUG(
				 ...
				 uint32_t local_iface)
{
	...
	if (ted_sid == MPLS_LABEL_NONE) {
		zlog_warn("bxPdnWtgUMF0TvjP "" %s: PATHD-TED: SL: ERROR query E : ted-sid (%d)",
			  __func__, ted_sid);
	} else {
		PATH_TED_DEBUG(
				 ...
				 struct ipaddr *local, struct ipaddr *remote)
{
	...
	if (ted_sid == MPLS_LABEL_NONE) {
		zlog_warn("bFZOnzvLGBPo5ZOg ""%s:SL:  ERROR query F : ted-sid (%d)", __func__,
			  ted_sid);
	} else {
