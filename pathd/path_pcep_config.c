...
int path_pcep_config_initiate_path(struct path *path)
{
	...
	if (path->do_remove) {
		...
		if (candidate) {
			...
			zlog_warn("mhCCiMKssNbFABgL ""(%s)PCE tried to REMOVE not existing LSP!",
				  __func__);
			return ERROR_19_3;
		}
		srte_apply_changes();
