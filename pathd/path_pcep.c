...
void notify_status(struct path *path, bool not_changed)
{
	...
	if ((resp = path_pcep_config_get_path(&path->nbkey))) {
		...
		flog_warn(EC_PATH_PCEP_RECOVERABLE_INTERNAL_ERROR,"1dcEoWGhvjGD2FPG "
			    ...
			    void *payload)
{
	...
	case PCEP_MAIN_EVENT_UNDEFINED:
		flog_warn(EC_PATH_PCEP_RECOVERABLE_INTERNAL_ERROR,"GPnuKXyMLfFAAjjM "
...
int pcep_main_event_initiate_candidate(struct path *path)
{
	...
	if (path->do_remove) {
		...
		case ERROR_19_9:
			...
			break;
		default:
			zlog_warn("NeNKtGyEMcJvMMpN ""(%s)PCE tried to REMOVE unknown error!",
				  __func__);
